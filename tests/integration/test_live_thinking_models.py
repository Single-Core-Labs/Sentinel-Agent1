...
``SENTINEL_AI_LIVE_LLM_TESTS=1`` plus ``HF_TOKEN`` are set. They verify the
...
LIVE_TESTS_ENABLED = os.environ.get("SENTINEL_AI_LIVE_LLM_TESTS") == "1"
REPORT_RESULT_TOOL = [
    {
        "type": "function",
        "function": {
...
def _skip_without_live_flag() -> None:
    if not LIVE_TESTS_ENABLED:
        pytest.skip("set SENTINEL_AI_LIVE_LLM_TESTS=1 to run paid live LLM tests")
