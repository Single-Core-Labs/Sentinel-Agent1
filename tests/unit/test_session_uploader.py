...
def test_dataset_card_readme_has_metadata_and_public_warning():
    readme = dataset_card_readme("lewtun/sentinel-ai-sessions")

    assert readme.startswith("---\n")
    assert 'pretty_name: "Sentinel-AI Session Traces"' in readme
    assert "task_categories:\n- text-generation" in readme
    assert "- agent-traces" in readme
    assert "- coding-agent" in readme
    assert "- sentinel-ai" in readme
    assert 'path: "sessions/**/*.jsonl"' in readme
    assert "Sentinel-AI demo: https://smolagents-sentinel-ai.hf.space" in readme
    assert "Sentinel-AI CLI: https://github.com/platformops/sentinel-ai" in readme
    assert "Repository: https://platformops.co/datasets/" not in readme
    assert (
...
def test_upload_dataset_card_only_for_claude_code_format():
    ...
    _upload_dataset_card(api, "lewtun/sentinel-ai-sessions", "hf_token", "row")
    assert api.calls == []

    _upload_dataset_card(api, "lewtun/sentinel-ai-sessions", "hf_token", "claude_code")
    assert len(api.calls) == 1
    assert api.calls[0]["path_in_repo"] == "README.md"
    assert api.calls[0]["repo_id"] == "lewtun/sentinel-ai-sessions"
...
def test_update_upload_status_preserves_other_uploader_fields(tmp_path):
        ...
        json.dumps(
            {
                ...
                "upload_url": "https://platformops.co/datasets/org/sessions",
    ...
    _update_upload_status(
        ...
        "personal_upload_url",
        "success",
        "https://platformops.co/datasets/user/sentinel-ai-sessions",
    )

    data = json.loads(session_file.read_text())
    assert data["upload_status"] == "success"
    assert data["upload_url"] == "https://platformops.co/datasets/org/sessions"
