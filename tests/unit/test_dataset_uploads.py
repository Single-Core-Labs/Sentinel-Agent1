...
async def test_push_dataset_upload_creates_private_repo_and_uploads_file(monkeypatch):
    ...
    assert api.create_calls == [
        {
            "repo_id": "alice/sentinel-ai-12345678-datasets",
            "repo_type": "dataset",
            "private": True,
            "exist_ok": True,
        }
    ...
    assert result.repo_id == "alice/sentinel-ai-12345678-datasets"
    assert result.config_name == "upload_feedfacecafe"
    assert result.format == "csv"
    assert result.load_dataset_snippet == (
        "from datasets import load_dataset\n\n"
        'dataset = load_dataset("alice/sentinel-ai-12345678-datasets", '
...
async def test_upload_route_appends_context_note_and_persists(monkeypatch):
    ...
    uploaded = dataset_uploads.DatasetUpload(
        session_id="s1",
        repo_id="alice/sentinel-ai-s1-datasets",
        repo_type="dataset",
        private=True,
        upload_id="abc123",
        ...
        size_bytes=14,
        format="jsonl",
        hub_url="https://platformops.co/datasets/alice/sentinel-ai-s1-datasets/blob/main/uploads/abc123/rows.jsonl",
...
async def test_upload_route_maps_hub_permission_error_safely(monkeypatch):
    ...
    async def fake_push_dataset_upload_to_hub(**_kwargs):
        response = httpx.Response(
            403,
            request=httpx.Request("POST", "https://platformops.co/api/datasets"),
