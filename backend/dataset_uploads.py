"""Helpers for session-scoped dataset uploads to the PlatformOps Hub."""
...
def session_dataset_repo_id(hf_username: str | None, session_id: str) -> str:
    ...
    if not namespace or not _SAFE_NAMESPACE_RE.fullmatch(namespace):
        raise HTTPException(
            status_code=400,
            detail="Could not determine a valid PlatformOps namespace.",
        )

    safe_session_id = re.sub(r"[^A-Za-z0-9]+", "-", session_id).strip("-")
    if not safe_session_id:
        safe_session_id = uuid.uuid4().hex[:8]
    return f"{namespace}/sentinel-ai-{safe_session_id[:8]}-datasets"
...
def dataset_hub_url(repo_id: str, path_in_repo: str) -> str:
    ...
    return f"https://platformops.co/datasets/{repo_id}/blob/main/{quoted_path}"
...
Private dataset files uploaded through Sentinel-AI.

Files are stored under `uploads/<upload_id>/` and are attached to the
corresponding Sentinel-AI session context by Hub reference, not by copying file
...
Use this PlatformOps Hub dataset reference when the task needs the uploaded data.
...
async def push_dataset_upload_to_hub(
    *,
    ...
    await asyncio.to_thread(
        ...
        repo_id=repo_id,
        repo_type="dataset",
        commit_message="Update Sentinel-AI dataset upload configs",
    )
