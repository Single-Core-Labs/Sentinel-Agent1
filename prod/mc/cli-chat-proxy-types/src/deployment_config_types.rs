...
pub const MANAGED_POLICY_TYP: &str = "ai.managed_policy.v1";
pub const MANAGED_IDENTITY_TYP: &str = "ai.managed_identity.v1";

/// Client echoes its persisted envelope `nonce` on this header for the server probe.
pub const MANAGED_CONFIG_NONCE_ECHO_HEADER: &str = "x-ai-managed-config-nonce";
...
mod tests {
    ...
    fn managed_identity_claim_round_trips_and_defaults() {
        ...
        let partial: ManagedIdentityClaim = serde_json::from_str(
            r#"{"typ":"ai.managed_identity.v1","principal":"team-007","expires_at":1,"key_id":"v1"}"#,
        )
        .unwrap();
