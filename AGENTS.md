...
│   ├── interfaces/sentinel-cli/src/ai.rs      — Interactive agent REPL (LLM provider, plugins, MCP)
...
│   └── launch-readiness.md           — Public launch checklist
```

## Running

- **AI agent (CLI):** `cargo run --bin sentinel -- ai` — full interactive agent (native terminal REPL)
- **Local REPL (local models):** `cargo run --bin sentinel -- local` — Ollama + zero-cost slash commands
...
- Run `cargo test --workspace` and `cargo check --workspace` after any change
