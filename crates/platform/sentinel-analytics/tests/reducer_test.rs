...
fn test_reducer_concurrent_turns_accumulate_independently() {
    ...
    let facts = vec![
        make_fact(FactKind::TurnStarted {
            turn_id: "t1".into(),
            thread_id: "th1".into(),
        }),
        make_fact(FactKind::TurnStarted {
    ...
    let t1_result = results
        .iter()
        .find(|r| r.turn_id.as_deref() == Some("t1"))
        .unwrap();
    let tokens1 = t1_result.tokens_used.as_ref().unwrap();
    assert_eq!(tokens1.prompt, 100);
    assert_eq!(tokens1.completion, 200);

    let t2_result = results
        .iter()
        .find(|r| r.turn_id.as_deref() == Some("t2"))
        .unwrap();
...
fn test_reducer_tool_calls_accumulate() {
    ...
    let results = reducer.apply(make_fact(FactKind::TurnEnded {
        turn_id: "t1".into(),
        thread_id: "th1".into(),
        tokens_used: 0,
        duration_ms: 1000,
...
fn test_reducer_unknown_fact_kinds_are_noop() {
    let mut reducer = AnalyticsReducer::new();
    let results = reducer.apply_batch(vec![
        make_fact(FactKind::ClientRequest {
            method: "GET".into(),
            path: "/api".into(),
            status: 200,
            duration_ms: 100,
        }),
        make_fact(FactKind::ServerNotification {
            event: "ping".into(),
            payload: serde_json::Value::Null,
        }),
        make_fact(FactKind::SkillInvocation {
            skill_id: "s".into(),
            duration_ms: 50,
            success: true,
        }),
...
fn test_reducer_session_ended_contains_turn_count() {
    ...
    reducer.apply(make_fact(FactKind::TurnEnded {
        turn_id: "t1".into(),
        thread_id: "th1".into(),
        tokens_used: 0,
        duration_ms: 0,
