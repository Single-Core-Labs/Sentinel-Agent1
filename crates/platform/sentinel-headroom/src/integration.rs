...
use sentinel_protocol::{ContentBlock, Message as ProtocolMessage, Role};
...
use crate::config::{HeadroomConfig, Message as HeadroomMessage, MessageRole};
use crate::orchestrator::{CompressOutcome, ContentCompressor};
...
pub fn content_type_for_tool(tool_name: &str) -> Option<ContentType> {
    match tool_name {
        ...
        "bash" if tool_name.contains("test") || tool_name.contains("cargo") => {
            Some(ContentType::BuildLog)
        }
        "bash" if tool_name.contains("diff") || tool_name.contains("git") => {
...
pub async fn create_headroom_compressor_with_tools() -> (
    Arc<dyn sentinel_core::ContentCompressor>,
    Arc<HeadroomRetrieveTool>,
    Vec<Arc<dyn sentinel_tools::Tool>>,
) {
    let config = HeadroomConfig::default();
    let content_compressor = Arc::new(ContentCompressor::from_config(&config));
    let pipeline = Arc::new(AgentCompressionPipeline::new(content_compressor));
    ...
    (
        Arc::new(agent_compressor) as Arc<dyn sentinel_core::ContentCompressor>,
        retrieve_tool,
        memory_tools,
    )
}
...
impl sentinel_core::ContentCompressor for HeadroomAgentCompressor {
    ...
    async fn compress_conversation(
        ...
        let headroom_msgs: Vec<HeadroomMessage> = messages
            .iter()
            .map(|m| {
        ...
        for result_msg in result.messages {
            ...
            match orig {
                ...
                None => {
                    ...
                    output.push(ProtocolMessage::new(
                        role,
...
mod tests {
    ...
    async fn test_retrieve_tool_nonexistent() {
        ...
        let tool = HeadroomRetrieveTool::new(ccr);
        let result = tool
            .execute(json!({"hash": "ccr:nonexistent"}), &ToolContext::new())
            .await;
        assert!(result.is_error);
    ...
    async fn test_retrieve_tool_backward_compat_key() {
        let ccr = Arc::new(CcrStore::new(100));
        ccr.store_with_key("ccr:test", "original data".into(), "text", "preview".into())
            .await;
        let tool = HeadroomRetrieveTool::new(ccr);
        let result = tool
            .execute(json!({"key": "ccr:test"}), &ToolContext::new())
            .await;
        assert!(!result.is_error);
    ...
    async fn test_retrieve_tool_with_query() {
        ...
        let result = tool
            .execute(
                json!({"hash": "ccr:test_q", "query": "authentication"}),
                &ToolContext::new(),
            )
            .await;
        assert!(!result.is_error);
        assert!(
            result.text.contains("authentication"),
    ...
    async fn test_pipeline_skips_errors() {
        ...
        let result = pipeline
            .process_tool_output("bash", "some error", true)
            .await;
