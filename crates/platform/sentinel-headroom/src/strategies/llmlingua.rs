...
impl LLMLinguaCompressor {
    pub fn new() -> Self {
        Self {
            config: LLMLinguaConfig::default(),
            _model_loaded: false,
        }
    }
...
impl Default for LLMLinguaCompressor {
    fn default() -> Self {
        Self::new()
    }
}

pub fn is_llmlingua_loaded() -> bool {
    false
}
...
impl CompressionStrategy for LLMLinguaCompressor {
    ...
    async fn compress(&self, content: &str) -> Option<CompressionResult> {
        ...
        let metrics =
            CompressionMetrics::new(content, &compressed, "llmlingua", "plain_text", took);
...
mod tests {
    ...
    async fn test_preserves_rare_words() {
        ...
        assert!(
            r.text.contains("ZOOGLOPHONIC"),
            "should preserve rare/important words: {}",
            r.text
        );
    }
    ...
    async fn test_compress_rate() {
        let cfg = LLMLinguaConfig {
            text_compression_rate: 0.6,
            ..Default::default()
        };
        let c = LLMLinguaCompressor::with_config(cfg);
        let mut t = String::new();
        for i in 0..20 {
            t.push_str(&format!("Sentence number {} with some unique words.\n", i));
        }
