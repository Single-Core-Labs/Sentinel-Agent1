...
impl SmartCrusher {
    ...
    pub fn crush(&self, items: &[Value], query: Option<&str>) -> SmartCrusherOutput {
        ...
        let field_stats: BTreeMap<&str, FieldStats> = numeric_fields
            .iter()
            .map(|field| {
                let values: Vec<f64> = items.iter().filter_map(|v| get_numeric(v, field)).collect();
        ...
        for (i, item) in items.iter().enumerate() {
            ...
            scored.push(ScoredItem { index: i, score });
        }
        ...
        if self.config.sample_remaining && kept_set.len() < max_keep && kept_set.len() < total {
            let remaining: Vec<usize> = (0..total).filter(|i| !kept_set.contains(i)).collect();
...
pub fn crush_json_array(
    ...
    result.push_str(&format!(
        ...
        (output.total - output.total_kept)
            .checked_mul(100)
            .and_then(|n| n.checked_div(output.total))
            .unwrap_or(0)
    ));
...
mod tests {
    ...
    fn test_smart_crusher_preserves_errors() {
        ...
        let output = crusher.crush(&items, None);
        assert!(
            output.kept_indices.contains(&50),
            "should keep error item at index 50"
        );
    ...
    fn test_smart_crusher_detects_anomalies() {
        ...
        items[80] =
            json!({"id": 80, "name": "item_80", "status": "ok", "score": 9999.0, "group": "a"});
        let config = SmartCrusherConfig {
        ...
        let output = crusher.crush(&items, None);
        assert!(
            output.anomaly_indices.contains(&80),
            "should detect anomaly at 80: {:?}",
    ...
    fn test_smart_crusher_relevance_matching() {
        ...
        assert!(
            matching_relevant,
            "should match relevant items: {:?}",
            output.relevance_indices
        );
    }
    ...
    fn test_sample_remaining_fills_to_max() {
        ...
        let output = crusher.crush(&items, None);
        assert!(
            output.total_kept >= 55,
            "should sample remaining: {}",
            output.total_kept
        );
    }
    ...
    fn test_smart_crusher_keeps_diverse_sample() {
        ...
        let crusher = SmartCrusher::new(config);
        let output = crusher.crush(&items, None);
        let groups: BTreeSet<&str> = output
            .kept_items
            .iter()
