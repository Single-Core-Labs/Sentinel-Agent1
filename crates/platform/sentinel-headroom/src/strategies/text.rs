...
impl TextCompressor {
    pub fn new() -> Self {
        Self {
            config: TextCompressorConfig::default(),
        }
    }
    pub fn with_config(config: TextCompressorConfig) -> Self {
        Self { config }
    }
}
...
impl CompressionStrategy for TextCompressor {
    ...
    async fn compress(&self, content: &str) -> Option<CompressionResult> {
        ...
        for para in &paragraphs {
            ...
            if !para_out.is_empty() {
                kept.push(para_out.to_string());
            }
            if kept.len() >= config.max_paragraphs {
                break;
            }
        }
        ...
        let savings = if orig_words > 0 {
            (orig_words - comp_words) as f64 / orig_words as f64 * 100.0
        } else {
            0.0
        };

        if savings < config.min_sentence_savings_pct {
            return None;
        }
...
mod tests {
    ...
    async fn test_text_preserves_headers() {
        ...
        t.push_str("# Details\n\n");
        for _ in 0..20 {
            t.push_str("Lorem ipsum dolor sit amet consectetur adipiscing elit. ");
        }
        t.push_str("# Conclusion\n\n");
        for _ in 0..20 {
            t.push_str("Final words of wisdom here. ");
        }
        let r = TextCompressor::new().compress(&t).await.unwrap();
    ...
    async fn test_text_with_config() {
        ...
        let mut t = String::new();
        for i in 0..10 {
            t.push_str(&format!("Paragraph {} with lots of unique words and interesting content that should definitely be preserved in the compression process for variety.\n\n", i));
        }
