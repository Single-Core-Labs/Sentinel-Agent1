...
use sentinel_protocol::{
    Choice, CompletionRequest, CompletionResponse, ContentBlock, Delta, DeltaFunction,
    DeltaToolCall, Message, Role, StreamChoice, StreamChunk, Usage,
};
...
impl AnthropicProvider {
    ...
    fn serialize_message(&self, msg: &Message) -> serde_json::Value {
        ...
        let tool_use_blocks: Vec<serde_json::Value> = msg
            .content
            .iter()
            .filter_map(|b| {
        ...
        let tool_result_blocks: Vec<serde_json::Value> = msg
            .content
            .iter()
            .filter_map(|b| {
...
fn event_to_chunk(event: AnthropicStreamEvent) -> Option<StreamChunk> {
    match event {
        ...
        AnthropicStreamEvent::MessageDelta { delta, .. } => Some(StreamChunk {
            id: String::new(),
            model: String::new(),
            choices: vec![StreamChoice {
                index: 0,
...
impl AnthropicProvider {
    fn parse_response(&self, data: serde_json::Value) -> Result<CompletionResponse, ProviderError> {
        ...
        let stop_reason = data["stop_reason"].as_str().map(|s| {
            match s {
                "end_turn" => "stop",
                "tool_use" => "tool_calls",
                "max_tokens" => "length",
