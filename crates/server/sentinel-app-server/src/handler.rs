...
impl RequestHandler {
    ...
    // ── Gap 3: NCU integration ──────────────────────────────────────────────
    /// Run `ncu --csv` on a target binary and parse the output into a
    /// structured bottleneck report.  Falls back gracefully when `ncu` is not
    /// installed so the dashboard still works on non-NVIDIA machines.
    async fn handle_gpu_ncu(&self, params: Option<Value>) -> Result<Value, JsonRpcError> {
        use api::{GpuNcuParams, GpuNcuResult};
        let p: GpuNcuParams = parse_params(params)?;

        // Check whether ncu is available.
        let ncu_path = which_tool("ncu")
            .or_else(|| which_tool("ncu-ui"))
            .or_else(|| {
                // Common install paths on Linux/Windows.
                for candidate in &[
        ...
        let ncu_bin = ncu_path.unwrap();
        let mut cmd = std::process::Command::new(&ncu_bin);
        cmd.arg("--csv");
        cmd.arg("--page").arg("raw");
        if let Some(ref kn) = p.kernel_name {
            cmd.arg("--kernel-name").arg(kn);
        }
        for flag in &p.extra_flags {
            cmd.arg(flag);
        }
        cmd.arg(&p.target);
        ...
        let out = tokio::task::spawn_blocking(move || cmd.output())
            .await
            .map_err(|e| JsonRpcError::internal_error(format!("spawn_blocking failed: {}", e)))?
            .map_err(|e| JsonRpcError::internal_error(format!("ncu exec failed: {}", e)))?;
        ...
        // Parse CSV metrics.
        let metrics = parse_ncu_csv(&raw_stdout);

        // Derive bottleneck summary from key metrics.
        let bottleneck_summary = derive_ncu_bottleneck(&metrics, &raw_stderr);
    ...
    // ── Gap 7: PTX/SASS disassembler ────────────────────────────────────────
    /// Disassemble a .cubin/.fatbin/.ptx file using `nvdisasm` or `cuobjdump`.
    /// Falls back to an emulator-derived PTX sketch when neither tool is found.
    async fn handle_gpu_disasm(&self, params: Option<Value>) -> Result<Value, JsonRpcError> {
        use api::{GpuDisasmParams, GpuDisasmResult};
        let p: GpuDisasmParams = parse_params(params)?;
        ...
        // Try nvdisasm first (ships with CUDA Toolkit).
        let nvdisasm = which_tool("nvdisasm");
        if let Some(ref bin) = nvdisasm {
            let mut cmd = std::process::Command::new(bin);
            if mode == "sass" {
                cmd.arg("--print-code");
            } else {
                cmd.arg("--print-code").arg("--print-ptx");
            }
            if let Some(ref kn) = p.kernel_name {
                cmd.arg("--function").arg(kn);
            }
            cmd.arg(&p.file_path);
            let bin2 = bin.clone();
            ...
            if let Some(out) = out {
                if out.status.success() {
                    ...
                    let result = GpuDisasmResult {
                        disasm: text.chars().take(65_536).collect(),
                        source: "nvdisasm".into(),
                        real_disasm: true,
                    };
                    return Ok(serde_json::to_value(result).unwrap_or_default());
                }
        ...
        if let Some(bin) = cuobjdump {
            ...
            if let Some(out) = out {
                ...
                let result = GpuDisasmResult {
                    disasm: text.chars().take(65_536).collect(),
                    source: "cuobjdump".into(),
                    real_disasm: out.status.success(),
                };
                return Ok(serde_json::to_value(result).unwrap_or_default());
            }
        }
        ...
        Err(JsonRpcError::internal_error(
            "nvdisasm and cuobjdump not found. Install CUDA Toolkit and ensure the bin directory is on PATH."
        ))
    }
}
