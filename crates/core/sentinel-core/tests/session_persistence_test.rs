...
async fn test_session_persistence_roundtrip() {
    ...
    thread
        .conversation
        .add_user_message("Hello, what can you do?");
    ...
    assert_eq!(
        thread.max_iterations, loaded.max_iterations,
        "max_iterations should match"
    );
    ...
    assert_eq!(
        thread.iterations, loaded.iterations,
        "iterations should match"
    );
    assert_eq!(
        thread.parent_thread_id, loaded.parent_thread_id,
        "parent_thread_id should match"
    );
    assert_eq!(
        thread.conversation.total_items(),
        loaded.conversation.total_items(),
        "conversation items should match"
    );
...
async fn test_session_persistence_sanitizes_secrets() {
    ...
    assert!(
        !json_content.contains("sk-abc123def456ghi789jkl012mnopqrs"),
        "File should NOT contain the raw API key"
    );
    assert!(
        !json_content.contains("Bearer xyz.token.here.longer.example.value"),
        "File should NOT contain the raw bearer token"
    );

    // But the [REDACTED] placeholder should exist
    assert!(
        json_content.contains("[REDACTED]"),
    ...
    let loaded = store
        .load_thread(&thread.id.to_string())
        .await
        .expect("load_thread failed after sanitization");
    assert_eq!(
        thread.id, loaded.id,
        "thread id should survive sanitization"
    );
...
async fn test_session_fork() {
    ...
    parent
        .conversation
        .add_user_message("Write a Python script");
    store
        .save_thread(&parent)
        .await
        .expect("save parent failed");

    // Fork
    let forked = store
        .fork_thread(&parent.id.to_string())
        .await
        .expect("fork failed");
    ...
    assert_eq!(
        forked.conversation.total_items(),
        parent.conversation.total_items(),
