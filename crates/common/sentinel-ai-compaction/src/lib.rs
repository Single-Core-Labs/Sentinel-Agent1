...
//! stay in each product host (for example `sentinel-ai-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `sentinel-ai-sampling-types`. It is decoupled from both Ai chat and
//! ai-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — ai-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Ai chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Ai chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Ai chat and
//! ai-build; hosts still own snapshotting and host-only sections).
...
// ai-build's full-replace subsystem now lives under `code_compaction`;
