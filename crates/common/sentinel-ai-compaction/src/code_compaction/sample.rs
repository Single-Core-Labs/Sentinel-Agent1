...
//!
//! The canonical `sample → classify → retry` loop, used by **both** ai-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Ai chat's intra `Shared` summarizer
...
//! the winning summary (ai-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the ai-build host uses to step down its input size).
