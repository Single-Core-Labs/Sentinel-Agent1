//! Compacted-history assembly (ai-build's rebuild structure, generic).
//!
//! Moved from `sentinel-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! ai-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // ai-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn ai_build_order_recent_before_summary() {
