//! Observability seam for the full-replace (ai-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (ai-build:
...
//!
//! Emission points are part of the behavior contract: the ai-build observer
