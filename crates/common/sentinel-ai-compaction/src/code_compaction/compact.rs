//! ai-build's full-replace compaction pass.
//!
//! ai-build does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `sentinel-ai-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). ai-build keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (ai-build's
...
/// Run ai-build's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam ai-build's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // ai-build appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce ai-build's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_ai_build_history_shape() {
        let llm_turns = vec![
