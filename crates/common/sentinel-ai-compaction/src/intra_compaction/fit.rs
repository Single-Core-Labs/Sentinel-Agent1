...
//!                          that alone exceed budget (ai-build style:
...
//!   (one-shot, same `tokens * 4` budget as ai-build)
...
/// One-shot payload shrink (ai-build: `max_bytes = max_tokens * 4`).
///
...
///
/// Matches ai-build's focus: in-place truncate is for oversized tool
...
pub fn fit_turns_for_summarizer<T: CompactionItemBuilder>(
    ...
    // One-shot prefix clip on tool results only (ai-build style).
    ...
    // ai-build: when even the newest unit alone exceeds budget, keep it
...
///
/// Mirrors ai-build `truncate_text_to_bytes` / `truncate_item_to_tokens`:
