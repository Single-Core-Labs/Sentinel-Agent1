...
///
/// - `FullReplace` (default): ai-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from ai-shell's
...
/// time (token threshold only, matching ai-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as ai-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<ai_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "ai-4.20";
