...
pub enum CompactionTarget {
    ...
    /// with a single summary — ai-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by Ai chat's
/// `StreamProcessor` (`Item = Arc<AiTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<AiTurn>`) already satisfies it.
