...
// The `Shared` summarizer reuses ai-build's full-replace summarization core
...
/// `FullReplace` strategy (default): ai-build's full-replace — summarize the
...
/// `<ai_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching ai-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize (possibly fitted) turns through ai-build's shared core.
    ...
    //     Shared with Ai chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    //     and the `<ai_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): ai-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<ai_user_queries>` preamble so
...
/// — ai-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and ai-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // ai-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // ai-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Ai chat's
...
/// the literal error messages produced by the Ai chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Ai chat sampler when the
    ...
    /// — guards the forwarding that Ai chat (`Arc<AiTurn>`) relies on.
    #[test]
