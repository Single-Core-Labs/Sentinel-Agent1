...
//!
//! - Ai chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `AiTurn::get_num_tokens`.
//! - ai-build estimates with `bytes / 4`.
//!
...
//! (Ai chat: `ItemTokenCounter<Arc<AiTurn>>`), so exactly one mechanism
//! is in play.
