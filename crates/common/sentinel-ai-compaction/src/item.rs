...
//! harness implements [`CompactionItem`] for its `AiTurn`;
//! ai-build implements it for `sentinel_ai_sampling_types::ConversationItem`.
//!
...
//! `<ai_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `AiRole` (Ai chat) and the
/// `ConversationItem` variants (ai-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Ai chat) — maps to System on harnesses without a
...
/// extraction (`<ai_file id=".." name=".." />` lines in the
/// `<ai_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Ai chat's
    /// `AiTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Ai chat: a
    ...
    /// strips their `<ai_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<ai_file>` lines in the
    /// `<ai_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Ai chat: a `Developer` turn with category
    ...
    /// requests/results (Ai chat: keep only `Channel` contents of an
    ...
    /// `max_tokens` (ai-build style: `max_bytes = max_tokens * 4`, prefix
...
/// constructs the typed harness items that make up ai-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Ai chat's
/// `AiTurn`; adding these constructors to it as required methods would break
/// that impl. They are also ai-build-specific (Ai chat's tail-keep path
...
///
/// The ai-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Ai chat stores turns as `Arc<AiTurn>`).
