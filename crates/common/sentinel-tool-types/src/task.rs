...
/// The blocking-wait ceiling in effect, honoring `AI_MAX_WAIT_BLOCK_MS`.
///
...
pub fn max_wait_block_ms() -> u64 {
    std::env::var("AI_MAX_WAIT_BLOCK_MS")
        .ok()
...
/// A built-in subagent type shared by the CLI (`sentinel-ai-agent`) and other
...
mod tests {
    ...
    fn task_tool_input_model_parses_explicit() {
        let input: TaskToolInput =
            serde_json::from_str(r#"{"description": "d", "prompt": "p", "model": "ai-3"}"#)
                .unwrap();
        assert_eq!(input.model.as_deref(), Some("ai-3"));
    }
    ...
    fn sanitize_optional_arg_moves_when_no_trim() {
        assert_eq!(
            sanitize_optional_arg(Some("ai-3".into())).as_deref(),
            Some("ai-3")
        );
        assert_eq!(
            sanitize_optional_arg(Some("  ai-3  ".into())).as_deref(),
    ...
    // match what the ai-shell MiniJinja templates render for the default
    // ai-build toolset (monitor + task + bash + read present, POSIX). The
