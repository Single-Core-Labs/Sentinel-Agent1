...
//! ```rust
//! use serde::Serialize;
//! use sentinel_tool_runtime::render::ToolOutput;
...
/// Lets `sentinel_tool_types::TaskOutputOutput` be used directly as a `Tool::Output`
/// (handy for stub/test tools and pass-through proxies).
impl ToolOutput for sentinel_tool_types::TaskOutputOutput {}

/// Lets `sentinel_tool_types::SubagentCompletedOutput` be used directly as a
/// `Tool::Output` (the `task` tool's structured completion output).
impl ToolOutput for sentinel_tool_types::SubagentCompletedOutput {}

/// Lets `sentinel_tool_types::KillTaskOutput` be used directly as a `Tool::Output`
/// (the `kill_task` tool's typed result / not-found output).
impl ToolOutput for sentinel_tool_types::KillTaskOutput {}
...
pub fn extract_content_blocks(value: &Value) -> Vec<ContentBlock> {
    ...
    if let Some(obj) = value.as_object() {
        // ai-build `ToolRunResult`: the model sees `prompt_text` (reminders
