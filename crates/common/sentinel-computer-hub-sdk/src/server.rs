...
fn system_notify_ack_from_outcome(
    ...
    match outcome {
        ...
        ResponseOutcome::Error(err)
            if err.data.is_none()
                && sentinel_tool_protocol::error_codes::string_for(err.code)
...
pub struct ResolvedSessionHandlers {
    ...
    /// [`sentinel_tool_protocol::SessionBindResult::unserved_tool_ids`].
    pub unserved_tool_ids: Vec<String>,
    /// Human-readable reason the resolver failed the toolset closed;
    /// forwarded as [`sentinel_tool_protocol::SessionBindResult::resolve_error`].
...
/// already use [`sentinel_tool_runtime::Tool`] can adapt by calling the
...
pub struct ToolServerBuilder {
    ...
    server_id: Option<sentinel_tool_protocol::ServerId>,
...
impl ToolServerBuilder {
    ...
    /// `AI_TOOL_SERVER_GLOBAL_MAX_INFLIGHT` env var overrides this at
    ...
    pub fn server_id(mut self, id: sentinel_tool_protocol::ServerId) -> Self {
    ...
    /// [`sentinel_tool_protocol::SessionBindResult::binary_version`].
...
impl ToolServer {
    ...
    pub async fn serve(&self, session_id: SessionId) -> Result<(), ClientError> {
        ...
        let tools: Vec<sentinel_tool_protocol::ToolDescriptionWithSchema> = {
            let map = self.inner().session_handlers.read();
            let handlers = map.get(&session_id);
            handlers
                .map(|h| {
                    h.iter()
                        .map(|h| sentinel_tool_protocol::ToolDescriptionWithSchema {
        ...
        let params = sentinel_tool_protocol::ServeParams { tools };
    ...
    pub async fn send_system_notification(
        &self,
        session_id: SessionId,
        params: sentinel_tool_protocol::SystemNotifyParams,
    ) -> Result<SystemNotifyAck, ClientError> {
        ...
        if payload_len > sentinel_tool_protocol::MAX_SYSTEM_NOTIFY_PAYLOAD_BYTES {
            return Err(ClientError::ProtocolError(format!(
                "system.notify payload {payload_len} bytes exceeds {} byte cap",
                sentinel_tool_protocol::MAX_SYSTEM_NOTIFY_PAYLOAD_BYTES
            )));
        }
        let connection = self.inner().borrow.connection();
        let request_id = connection.try_alloc_request_id()?;
    ...
    pub async fn donate_traces(&self, otlp_request_b64: &str) -> Result<(), ClientError> {
        ...
        let connection = self.inner().borrow.connection();
        let notification = sentinel_tool_protocol::JsonRpcNotification {
            jsonrpc: JsonRpcVersion,
            session_id: Some(session),
            seq: None,
            ...
            loop {
                ...
                match method {
                    "session.bind" => {
                        ...
                        let Ok(sid) = sentinel_tool_protocol::SessionId::new(sid_str) else {
                            continue;
                        };
...
async fn execute_call(
    ...
    ctx.extensions.insert(sentinel_tool_runtime::SessionContext(
...
mod tests {
    use super::*;
    use sentinel_tool_runtime::ContentBlock;
