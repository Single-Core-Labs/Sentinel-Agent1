...
pub struct ToolCallResult {
    ...
    /// this crate must not depend on `sentinel-tool-runtime`. Sampler-side wire
...
/// `sentinel_computer_hub_core::registry::ToolSessionBindOutcome`, with one
...
pub struct ToolsListResult {
    pub tools: Vec<sentinel_tool_types::ToolDescription>,
}
...
pub enum IdleWithholdReason {
    ...
    PreviewRouted,
    /// Only the preview pane's own `/__ai-preview/status` liveness poll.
    PreviewStatusOnly,
