...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `AI_HUNK_TRACKER` override
    ...
    /// Speech-to-text language preference for voice dictation. A Ai STT
    /// catalog code (`en`, `es`, `ja`, … — see AI STT supported languages) or
...
pub struct ContextualHints {
    ...
    /// SSH wrap session-load tip (recommend `ai wrap ssh` when the session
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: sentinel_ai_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
