...
//! `sentinel-ai-workspace`. Wire types live in
//! `sentinel_ai_workspace_types::rpc`; this crate adds the connected-state
...
use sentinel_computer_hub_sdk::harness::ToolHarness;
use sentinel_ai_workspace_types::rpc::agents_md::{AgentConfigFile, DiscoverAgentsMdReq};
use sentinel_ai_workspace_types::rpc::code_nav::{
...
use sentinel_ai_workspace_types::rpc::export_github::{ExportGithubReq, ExportGithubResponse};
use sentinel_ai_workspace_types::rpc::fs::{
...
use sentinel_ai_workspace_types::rpc::git::{
...
use sentinel_ai_workspace_types::rpc::hunks::{
...
use sentinel_ai_workspace_types::rpc::search::{
    ContentSearchData, ContentSearchRequest, FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq,
    FuzzyStatusReq,
};
use sentinel_ai_workspace_types::rpc::session::{
    BeginPromptReq, EndPromptReq, FileRewindResponse, RewindToReq,
};
use sentinel_ai_workspace_types::rpc::skills::{DiscoverPluginsReq, DiscoverSkillsReq, SkillInfo};
use sentinel_ai_workspace_types::rpc::workspace::{
...
use sentinel_ai_workspace_types::rpc::worktree::{
...
use sentinel_ai_workspace_types::rpc::{RpcEnvelope, RpcError, WORKSPACE_RPC_TOOL_ID, WorkspaceRpc};
use sentinel_tool_runtime::{ToolCallContext, ToolStreamItem, TypedToolOutput};
...
pub async fn consume_stream_terminal(
    stream: &mut sentinel_tool_runtime::ToolStream<TypedToolOutput>,
) -> Result<TypedToolOutput, sentinel_tool_runtime::ToolError> {
    loop {
...
/// Check whether a [`ToolError`](sentinel_tool_runtime::ToolError) indicates
...
pub fn is_transport_fatal(err: &sentinel_tool_runtime::ToolError) -> bool {
    match err.kind {
        sentinel_tool_runtime::ToolErrorKind::NetworkError => true,
        sentinel_tool_runtime::ToolErrorKind::Custom => err
            .details
            .as_ref()
...
impl WorkspaceClient {
    ...
    pub async fn rpc_raw(
        ...
        let tool_id = sentinel_tool_protocol::ToolId::new(WORKSPACE_RPC_TOOL_ID)
...
mod tests {
    ...
    use sentinel_computer_hub_sdk::harness::LocalRegistry;
    use sentinel_ai_workspace_types::rpc::skills::SkillScope;
    use sentinel_tool_protocol::{SessionId, ToolId};
    use sentinel_tool_runtime::{Tool, ToolError};
    ...
    impl sentinel_tool_runtime::ToolOutput for RawOut {}
    #[derive(Debug)]
    ...
    impl Tool for FakeWorkspaceRpc {
        ...
        fn description(&self, _ctx: &::sentinel_tool_runtime::ListToolsContext) -> ToolDescription {
        ...
        async fn run(&self, _ctx: ToolCallContext, args: Self::Args) -> Result<RawOut, ToolError> {
            ...
            match args.method.as_str() {
                ...
                "workspace.discover_skills" => ok(serde_json::json!([{
                    ...
                    "path": "/workspace/.ai/skills/my-skill/SKILL.md",
                    "scope": "local",
                }])),
    ...
    async fn consume_stream_terminal_returns_ok() {
        ...
        let mut stream = sentinel_tool_runtime::terminal_only(Ok(typed));
        assert_eq!(
    ...
    async fn consume_stream_terminal_returns_err() {
        let mut stream: sentinel_tool_runtime::ToolStream<TypedToolOutput> =
            sentinel_tool_runtime::terminal_only(Err(ToolError::network_error("oops")));
    ...
    async fn consume_stream_terminal_exhausted_stream_is_network_error() {
        ...
        let mut stream = sentinel_tool_runtime::terminal_only::<TypedToolOutput>(Ok(typed));
