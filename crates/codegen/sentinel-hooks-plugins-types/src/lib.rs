...
//! `sentinel-ai-shell` and `sentinel-ai-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `sentinel-ai-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `sentinel-ai-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// Ai's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `sentinel-ai-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.ai/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}
...
mod tests {
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.ai/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.ai/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserAi),
            conflict: None,
        };
