...
const EXCLUDED_DIR_NAMES: &[&str] = &[
    ".ai", ".cache", ".daemon", ".config", ".npm", ".cargo", ".rustup", ".vscode", ".gemini",
    ".hermes", ".claude",
];
...
mod tests {
    ...
    mod config_and_cache {
        ...
        #[test]
        fn ai_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".ai")));
                assert!(!is_project_dir(&home.join(".ai/bin")));
            }
        }

        #[test]
        fn ai_prefixed_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".ai-proxy-work")));
            }
        }
