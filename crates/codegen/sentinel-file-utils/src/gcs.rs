...
use sentinel_ai_auth::{AuthCredentialProvider, StaticAuthCredentialProvider};

use crate::storage_client::{Auth401AttributionCallback, StaticAiAuth, StorageClient};
...
/// `StaticAiAuth` carrying the inline user / deployment keys from
...
fn build_proxy_client_with_fallback(
    ...
    let provider = credentials.unwrap_or_else(|| {
        let mut creds = StaticAiAuth::new(Some(user_token.to_owned()));
