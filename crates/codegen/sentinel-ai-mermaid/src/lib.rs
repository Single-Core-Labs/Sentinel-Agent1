...
//!
//! ```
//! use sentinel_ai_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `AiDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `AiNight`, `TokyoNight`).
    Dark,
}
