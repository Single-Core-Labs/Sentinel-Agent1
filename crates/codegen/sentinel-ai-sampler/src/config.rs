...
//! `sentinel_ai_sampling_types::SamplingConfig` so that the sampler crate
...
/// submit.
///
/// # Construction in `sentinel-ai-shell`
///
...
/// composing chat-state's `sentinel_ai_sampling_types::SamplingConfig`
/// with `Credentials` (api key, client version).
///
/// URL-derived request headers (e.g. `X-AI-Token-Auth` for the
/// cli-chat-proxy) are
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-ai-doom-loop-check` header on
