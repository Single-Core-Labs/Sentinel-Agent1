...
use sentinel_ai_sampling_types::{SamplingError, is_retryable_api_status};
...
/// Resolve max API retries: `AI_MAX_RETRIES` env > model config > default ([`DEFAULT_MAX_RETRIES`]).
pub fn resolve_max_retries(model_max_retries: Option<u32>) -> u32 {
    let env_override = std::env::var("AI_MAX_RETRIES").ok();
...
mod tests {
    ...
    fn zero_retry_budget_never_reuses_a_model_output_cap() {
        for err in [
            ...
            SamplingError::EmptyResponse {
                context: sentinel_ai_sampling_types::EmptyResponseContext {
                    reason: sentinel_ai_sampling_types::EmptyReason::NoVisibleContent,
                    had_reasoning: false,
                    content_len: 0,
                    tool_call_count: 0,
    ...
    fn classify_cloudflare_522_is_retryable() {
        let err = api_err(
            ...
            "Connection to Ai timed out or was interrupted. (HTTP 522).",
        );
    ...
    fn classify_cloudflare_525_is_fatal_even_with_should_retry_true() {
        ...
        for should_retry in [None, Some(true)] {
            let err = SamplingError::Api {
                ...
                message: "Secure connection to Ai failed. (HTTP 525).".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry,
            };
