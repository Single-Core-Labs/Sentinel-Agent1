//! HTTP client for the AI sampling APIs.
//!
...
use sentinel_ai_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
pub use sentinel_ai_sampling_types::ApiBackend;

/// Process-level fallback for the `x-ai-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "ai-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "ai-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-ai-*` headers. Optional fields are skipped when empty/`None`.
struct AiRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    model_id: &'a str,
    session_id: &'a str,
...
impl AiRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-ai-conv-id", self.conv_id)
            .header("x-ai-req-id", self.req_id)
            .header("x-ai-model-override", self.model_id)
            .header("x-ai-session-id", self.session_id)
            .header("x-ai-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-ai-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-ai-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-ai-user-id", id);
        }
        b
    }
}
...
/// Deserialize a Responses API SSE event, with a fallback for AI-specific
...
/// when the API emits the AI-specific `context_details` field.
    ...
    let mut event = match serde_json::from_str::<rs::ResponseStreamEvent>(data) {
        ...
        Err(first_err) => {
            ...
            if let Ok(mut value) = serde_json::from_str::<serde_json::Value>(data) {
                ...
                // (e.g., AI-specific "x_search"). Instead of maintaining a
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = sentinel_ai_sampling_types::reported_cost_ticks(
        value
...
pub(crate) const COST_USD_TICKS_METADATA_KEY: &str = "sentinel.cost_usd_ticks";
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-ai-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-ai-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<sentinel_ai_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Add x-ai-client-version header for version gating at the proxy.
        ...
        {
            headers.insert(HeaderName::from_static("x-ai-user-id"), header_value);
        }

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-ai-client-identifier"),
                    header_value,
                );
            }
        }
    ...
    pub async fn chat_completion(
        ...
        let x_ai_conv_id = &payload.x_ai_conv_id.clone().unwrap_or_default();
        let x_ai_req_id = &payload.x_ai_req_id.clone().unwrap_or_default();
        ...
        let ai_headers = AiRequestHeaders {
            conv_id: x_ai_conv_id,
            req_id: x_ai_req_id,
            model_id: &model_id,
            session_id: payload.x_ai_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_ai_turn_idx.as_deref(),
            agent_id: payload.x_ai_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_ai_deployment_id.as_deref(),
            user_id: payload.x_ai_user_id.as_deref(),
        };
        let SentRequest {
            builder,
            sent_bearer,
        } = self.post(self.endpoint("chat/completions"));
        let http_request = ai_headers.apply(builder).json(&payload);
    ...
    pub async fn create_response(
        ...
        let x_ai_conv_id = request.x_ai_conv_id.as_deref().unwrap_or_default();
        let x_ai_req_id = request.x_ai_req_id.as_deref().unwrap_or_default();
        ...
        let ai_headers = AiRequestHeaders {
            ...
            req_id: x_ai_req_id,
            model_id: &model_id,
            session_id: request.x_ai_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_ai_turn_idx.as_deref(),
            agent_id: request.x_ai_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_ai_deployment_id.as_deref(),
            user_id: request.x_ai_user_id.as_deref(),
        };
        ...
        sentinel_ai_sampling_types::patch_reasoning_text_types(&mut request_body);
        let SentRequest {
            builder,
            sent_bearer,
        } = self.post(self.endpoint("responses"));
        let http_request = ai_headers.apply(builder).json(&request_body);
    ...
    /// gate that adds the opt-in `x-ai-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        // Inject AI-specific fields not in async-openai's CreateResponse type.
        if self.defaults.stream_tool_calls {
            request_body["stream_tool_calls"] = serde_json::json!(true);
        }
        // Inject AI-specific tools (e.g., x_search) that can't be expressed
        ...
        let mut http_request = ai_headers
            .apply(builder)
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn create_message(
        ...
        let http_request = ai_headers.apply(builder).json(&request.inner);
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_ai_conv_id = request.x_ai_conv_id.clone();
        let x_ai_req_id = request.x_ai_req_id.clone();
        let x_ai_session_id = request.x_ai_session_id.clone();
        let x_ai_turn_idx = request.x_ai_turn_idx.clone();
        let x_ai_agent_id = request.x_ai_agent_id.clone();

        // Collect AI-specific tools that can't be expressed via rs::Tool
        // (e.g., x_search). These are injected as raw JSON after serialization.
        let extra_tools = sentinel_ai_sampling_types::extra_tool_entries(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_ai_conv_id = x_ai_conv_id;
        wrapper.x_ai_req_id = x_ai_req_id;
        wrapper.x_ai_session_id = x_ai_session_id;
        wrapper.x_ai_turn_idx = x_ai_turn_idx;
        wrapper.x_ai_agent_id = x_ai_agent_id;
...
mod tests {
    use super::*;
    use indexmap::IndexMap;
    use sentinel_ai_sampling_types::types::ChatRequestMessage;
    ...
    fn stream_collect_error_preserves_should_retry() {
        let info = SamplingErrorInfo {
            ...
            credential: sentinel_ai_sampling_types::SentCredential::Unknown,
        };
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            x_ai_conv_id: None,
            x_ai_req_id: None,
            x_ai_session_id: None,
            x_ai_turn_idx: None,
            x_ai_agent_id: None,
            x_ai_deployment_id: None,
            x_ai_user_id: None,
            trace: None,
        };
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-AI-token-auth".to_string(), "sentinel-ai-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the ai-shell agent product.
        assert!(ua.starts_with("my-client ai-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = sentinel_ai_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "ai-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
