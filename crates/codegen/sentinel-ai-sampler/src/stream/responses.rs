    ...
    async_stream::stream! {
        ...
        loop {
            ...
            match event {
                ...
                // split (ai has no HostedTool::CodeInterpreter, so these events
        ...
        let mut items = sentinel_ai_sampling_types::response_to_conversation_items(response);
        sentinel_ai_sampling_types::inject_streaming_reasoning_fallback(&mut items, reasoning_acc);
...
mod tests {
    ...
    async fn doom_loop_collector_signals_land_on_completed_response() {
        use sentinel_ai_sampling_types::doom_loop::{
