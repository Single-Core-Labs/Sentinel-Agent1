...
//!
//! This crate stays decoupled from `sentinel-ai-shell`: no shell types, no
//! auth-manager dependency.

use std::sync::Arc;

pub use sentinel_ai_auth::bearer_fragment::BEARER_SUFFIX_LEN;
...
/// Sampler endpoints only — tool clients use `sentinel_ai_tools::ToolConsumer`.
