...
//!
//! TODO: Move from sentinel-ai-shell/src/sampling/error.rs
...
/// Flat error from the Ai proxy/gateway: `{"code": "...", "error": "..."}`.
#[derive(Debug, Deserialize)]
...
pub fn status_user_message(status: StatusCode) -> String {
    match status.as_u16() {
        code @ 502..=504 => {
            format!("Ai is temporarily unavailable. Please try again in a moment. (HTTP {code}).")
        }
        // Upstream capacity, not an edge failure — see [`SamplingError::is_overloaded`].
        code @ 529 => {
            format!("Ai is temporarily overloaded. Please try again in a moment. (HTTP {code}).")
        }
        // Cloudflare edge: origin unreachable or timed out (520–524), or an
        // edge-side 1xxx failure (530).
        code @ 520..=524 | code @ 530 => {
            format!(
                "Connection to Ai timed out or was interrupted. Please try again. (HTTP {code})."
            )
        }
        // Cloudflare origin TLS (handshake / invalid certificate) — not transient.
        code @ 525 | code @ 526 => {
            format!("Secure connection to Ai failed. (HTTP {code}).")
        }
...
<html lang="en-US">
<head><title>ai.com | 524: A timeout occurred</title></head>
