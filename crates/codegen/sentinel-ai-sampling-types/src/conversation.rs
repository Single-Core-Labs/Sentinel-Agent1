...
/// tool-call arguments routinely contain. `pub` for `sentinel-ai-shell`.
...
pub struct ConversationRequest {
    ...
    /// Custom headers for AI tracking
    pub x_ai_conv_id: Option<String>,
    pub x_ai_req_id: Option<String>,
    pub x_ai_session_id: Option<String>,
    pub x_ai_turn_idx: Option<String>,
    pub x_ai_agent_id: Option<String>,
    pub x_ai_deployment_id: Option<String>,
    pub x_ai_user_id: Option<String>,
    ...
    /// Sticky routing key for prompt-cache reuse; overrides `x_ai_conv_id` for routing.
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-ai-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the Ai Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/sentinel-ai-compaction` operate over ai-build's
...
impl sentinel_ai_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> sentinel_ai_compaction::CompactionRole {
        use sentinel_ai_compaction::CompactionRole;
        // ai-build has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // ai-build has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing ai-build history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<sentinel_ai_compaction::CompactionFileRef> {
        // ai-build `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `AiTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<ai_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl sentinel_ai_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/sentinel-ai-sampler/src/stream/messages.rs))
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_ai_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_ai_req_id = Some(req_id.into());
        self
    }
...
mod compaction_item_bridge_tests {
    use super::*;
    use sentinel_ai_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // ai-build has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.ai/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.ai/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.ai/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("ai-3".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_ai_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_ai_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        assert_eq!(item.model_id, Some("ai-3".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("ai-3");

        let ConversationItem::Assistant(a) = item else {
            panic!("Expected Assistant");
        };
        assert_eq!(a.model_id, Some("ai-3".to_string()));

        // Non-assistant should be unchanged
        let user = ConversationItem::user("Hi").with_model_id("ai-3");
    ...
    //      (earlier ai-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_ai_build_shape() {
