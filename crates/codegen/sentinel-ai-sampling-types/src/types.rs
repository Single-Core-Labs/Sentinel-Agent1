...
// Re-export ToolDefinition and FunctionTool from sentinel-ai-tools.
...
pub use sentinel_ai_tools::types::definition::{FunctionTool, ToolDefinition};
...
pub struct Usage {
    ...
    /// AI extension: request price in USD ticks (1 USD = 1e10 ticks).
...
/// custom header fields for AI request tracking, similar to
...
pub struct CreateResponseWrapper {
    ...
    /// AI-specific tool definitions that can't be expressed via
...
/// Wrapper around `MessagesRequest` that adds custom header fields for AI
...
mod tests {
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "ai" }).as_object().unwrap()))
                .is_none()
        );
    }
