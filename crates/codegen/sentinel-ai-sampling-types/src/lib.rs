//! Pure data types for the AI sampling / chat-completion API layer.
//!
...
//! AI agent stack.  It intentionally contains **no I/O** (no HTTP clients,
//! no file system access) so it can be depended on by downstream crates
//! (e.g., `sentinel-chat-state`) without pulling in the full `sentinel-ai-shell`.
