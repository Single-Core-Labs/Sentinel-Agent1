...
//! file logger that writes to `~/.ai/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `sentinel_ai_hooks` or `sentinel_ai_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! AI_HOOKS_LOG=1 ai              # enable, write to ~/.ai/logs/hooks.log
//! AI_HOOKS_LOG=/tmp/h.log ai     # write to custom path
//! AI_HOOKS_LOG=0 ai              # explicitly disable
//! tail -f ~/.ai/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "AI_HOOKS_LOG";
...
///
/// Writes to `~/.ai/logs/hooks.log` (or custom path via `AI_HOOKS_LOG`).
/// Filters to hooks (`sentinel_ai_hooks`) and plugins (`sentinel_ai_agent::plugins`) targets.
/// Set `AI_HOOKS_LOG=0` to disable, `AI_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "sentinel_ai_hooks=debug,sentinel_ai_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || ai_home().join("logs").join("hooks.log");
