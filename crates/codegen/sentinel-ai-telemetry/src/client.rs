...
//!
//! Extracted from `sentinel-ai-shell::agent::telemetry::track`. The HTTP client is
...
use serde_json::json;
use sentinel_mixpanel::Mixpanel;
...
/// whichever [`EmitterOrigin`] prefix it carries (`ai-shell-` /
/// `ai-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("SuperAi Heavy")
/// and JWT-derived keys ("superai_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "SuperAi Heavy" | "superai_heavy" => "superai_heavy",
        "SuperAi Plus" | "superai_plus" => "superai_plus",
        "SuperAi" | "superai" => "superai",
        "SuperAi Lite" | "superai_lite" => "superai_lite",
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("Ai Code"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("ai-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("ai-shell-turn"), "turn");
        assert_eq!(
            event_value("ai-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("ai-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("ai-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("SuperAi"), "superai");
        assert_eq!(normalize_tier("SuperAi Heavy"), "superai_heavy");
        assert_eq!(normalize_tier("superai_heavy"), "superai_heavy");
        ...
        assert_eq!(normalize_tier("SuperAi Lite"), "superai_lite");
        assert_eq!(normalize_tier("SuperAi Plus"), "superai_plus");
        assert_eq!(normalize_tier("superai_plus"), "superai_plus");
    ...
    /// (e.g. a future `"ai-shell-ext-"` origin), an earlier `ALL` entry could
