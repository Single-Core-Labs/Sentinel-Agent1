...
//!
//! `sentinel-ai-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "AI_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `AiComConfig`).
///
/// Built by the binaries (`sentinel-ai-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `sentinel_ai_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-AI-Token-Auth` header (typically `"sentinel-ai-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`ai-pager`) -> `client.name`.
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://cli-chat-proxy.ai.com/v1/traces`).
...
///
/// - `client_name`: binary name (e.g. `"ai-tui"`, `"ai-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-ai-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("ai-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid AI_OTEL_FILTER '{}': {}. Using default '{}'.",
...
struct RefreshableSpanExporter {
    ...
    /// Value for `X-AI-Token-Auth`. Only sent when `credentials.needs_token_auth_header()`.
...
fn build_export_headers(
    ...
    snapshot: &sentinel_ai_auth::CredentialSnapshot,
    ...
    if let Some(value) = token_auth_header {
        headers.insert("X-AI-Token-Auth".to_string(), value.to_string());
    }
...
mod tests {
    ...
    use std::sync::Mutex;
    use sentinel_ai_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("sentinel-ai-cli"),
