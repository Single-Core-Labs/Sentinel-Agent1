...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "ai-pager")]
    #[serde(rename = "ai-pager")]
    AiPager,
    #[strum(serialize = "ai-desktop")]
    #[serde(rename = "ai-desktop")]
    AiDesktop,
}
...
/// `~/.ai/logs/unified.jsonl` (those bursts inflate exactly the counters
...
fn log_path() -> PathBuf {
    ...
    ai_home().join(LOG_DIR).join(LOG_FILE)
}
...
/// healing a stale handle rather than re-resolving `$AI_HOME` — which also
...
mod tests {
    ...
    /// writer onto the developer's real `~/.ai/logs/unified.jsonl`.
    #[ctor::ctor]
    ...
    /// an emit lands in a per-process temp file, never under `ai_home()`.
    #[test]
    fn redirect_routes_writes_and_snapshots_to_process_temp_file() {
        ...
        assert!(
            ...
            "the shared file must live under the temp dir, not ai_home(): {}",
            log_path().display()
        );
    }
