...
pub enum TraceUploadReason {
    ...
    FeatureOff,
    /// No ai.com auth or deployment key.
    NoCredentials,
    /// Direct-to-bucket S3 upload.
    DirectS3,
    /// Proxy mode via ai.com auth.
    Proxy,
...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<sentinel_file_utils::UploadMethod>) -> Self {
        match method {
            Some(sentinel_file_utils::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(sentinel_file_utils::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(sentinel_file_utils::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
#[cfg(test)]
mod tests {
    use sentinel_file_utils::UploadMethod;

    use super::TraceUploadReason;

    /// The `ai-shell-doom_loop_recovery` Mixpanel event's name and
