...
pub(crate) fn build_blocking_client(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // roots: the process-wide `AI_EXTRA_CA_BUNDLE` (fail-open,
            // handled inside sentinel-ai-extra-ca) and the external stream's
            // per-call `OTEL_EXPORTER_OTLP_CERTIFICATE` files (fail-closed,
            // validated above).
            let mut builder = sentinel_ai_extra_ca::with_extra_root_certificates_blocking(
