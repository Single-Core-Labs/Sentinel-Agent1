...
//!
//! Extracted from `sentinel-ai-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `sentinel-ai-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `sentinel-ai-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "ai-shell-",
            EmitterOrigin::Workspace => "ai-workspace-",
        }
    }
}
...
/// event-name prefix (`ai-shell-*` vs `ai-workspace-*`).
///
...
mod tests {
    ...
    /// What a command exiting right after emitting (`ai login`) relies on.
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "ai-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "ai-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("ai-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("ai-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "ai-workspace-turn");
    }
