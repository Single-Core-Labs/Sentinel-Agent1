...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "ai_code.session_start",
            Self::SessionEnd => "ai_code.session_end",
            Self::UserPrompt => "ai_code.user_prompt",
            Self::TurnCompleted => "ai_code.turn_completed",
            Self::ApiRequest => "ai_code.api_request",
            Self::ApiError => "ai_code.api_error",
            Self::ToolResult => "ai_code.tool_result",
            Self::ToolDecision => "ai_code.tool_decision",
            Self::McpServerConnection => "ai_code.mcp_server_connection",
            Self::PermissionModeChanged => "ai_code.permission_mode_changed",
            Self::SkillActivated => "ai_code.skill_activated",
            Self::PluginLoaded => "ai_code.plugin_loaded",
            Self::Compaction => "ai_code.compaction",
            Self::Subagent => "ai_code.subagent",
            Self::Auth => "ai_code.auth",
            Self::InternalError => "ai_code.internal_error",
            Self::ModelSwitched => "ai_code.model_switched",
            Self::ContextualTip => "ai_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `ai_code.session.count`.
    SessionCount,
    /// `ai_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
    ...
    /// `ai_code.startup.timeout` (`stuck_in` = phase label).
    StartupTimeout { stuck_in: String, auth_mode: String },
    /// `ai_code.startup.phase_duration` (ms; `phase` = phase label).
    ...
    /// `ai_code.startup.total` (ms from process start to a usable session).
    StartupTotal {
        duration_ms: u64,
        outcome: String,
        auth_mode: String,
...
pub(crate) const METRIC_SESSION_COUNT: &str = "ai_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "ai_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "ai_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "ai_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "ai_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "ai_code.error.count";
pub(crate) const METRIC_STARTUP_PHASE_DURATION: &str = "ai_code.startup.phase_duration";
pub(crate) const METRIC_STARTUP_TIMEOUT: &str = "ai_code.startup.timeout";
pub(crate) const METRIC_STARTUP_TOTAL: &str = "ai_code.startup.total";
...
/// Built-in tool names (closed enum surface in `sentinel-ai-tools`) that pass
...
fn tool_outcome_label(o: &sentinel_file_utils::events::types::ToolOutcome) -> &'static str {
    use sentinel_file_utils::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `ai_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `ai_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `ai_code.session_end`.
...
/// `PromptSubmitted` → `ai_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `ai_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `ai_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `ai_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `ai_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `ai_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `ai_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `ai_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `ai_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `ai_code.permission_mode_changed`.
...
/// `ContextualTip` → `ai_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `ai_code.permission_mode_changed`.
...
/// `SkillDispatched` → `ai_code.skill_activated`. Skill names are details-gated; source and trigger export by default.
...
/// `PluginInstalled` → `ai_code.plugin_loaded`.
...
/// `PluginUsed` → `ai_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `ai_code.compaction`.
...
/// `CompactionTriggered` → `ai_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `ai_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `ai_code.subagent` (`phase=completed`).
...
/// `InternalError` → `ai_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `ai_code.model_switched`.
