...
//!
//! Enterprise customers point the Ai CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `AI_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `AI_EXTERNAL_OTEL` unset (or no exporter
...
//!   data-retention opt-outs (user-confirmed): those govern AI-side
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &sentinel_ai_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             AI_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
