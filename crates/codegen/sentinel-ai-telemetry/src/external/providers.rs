...
//! `Authorization`/`X-AI-Token-Auth`/`x-userid`;
...
fn build_resource(cfg: &ExternalOtelConfig) -> opentelemetry_sdk::Resource {
    let mut attrs = vec![
        ...
        opentelemetry::KeyValue::new("ai_code.schema.version", super::schema::SCHEMA_VERSION),
    ];
    ...
    opentelemetry_sdk::Resource::builder_empty()
        // RQ6 (final): `ai-cli`, a wire commitment.
...
fn grpc_tls_candidates(
    ...
    // Process-wide `AI_EXTRA_CA_BUNDLE` roots (fail-open by that crate's
    ...
    if let Some(extra_pem) = ders_to_pem_bundle(sentinel_ai_extra_ca::extra_root_ders()) {
...
mod tests {
    ...
    /// `X-AI-Token-Auth`, `x-userid`, or `x-teamid` unless customer-supplied
    ...
    fn exporter_headers_are_exactly_customer_headers() {
        ...
        for forbidden in ["Authorization", "X-AI-Token-Auth", "x-userid", "x-teamid"] {
            assert!(
    ...
    /// The DER→PEM re-encode used for `AI_EXTRA_CA_BUNDLE` must produce a
    ...
    fn exporter_metadata_is_customer_headers_only() {
        ...
        for forbidden in ["x-sentinel-token-auth", "x-userid", "x-teamid"] {
