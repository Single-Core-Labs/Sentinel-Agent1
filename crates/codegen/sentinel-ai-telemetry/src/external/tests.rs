...
fn event_names_are_pinned() {
    ...
    let expected: &[(E, &str)] = &[
        (E::SessionStart, "ai_code.session_start"),
        (E::SessionEnd, "ai_code.session_end"),
        (E::UserPrompt, "ai_code.user_prompt"),
        (E::TurnCompleted, "ai_code.turn_completed"),
        (E::ApiRequest, "ai_code.api_request"),
        (E::ApiError, "ai_code.api_error"),
        (E::ToolResult, "ai_code.tool_result"),
        (E::ToolDecision, "ai_code.tool_decision"),
        (E::McpServerConnection, "ai_code.mcp_server_connection"),
        (
            E::PermissionModeChanged,
            "ai_code.permission_mode_changed",
        ),
        (E::SkillActivated, "ai_code.skill_activated"),
        (E::PluginLoaded, "ai_code.plugin_loaded"),
        (E::Compaction, "ai_code.compaction"),
        (E::Subagent, "ai_code.subagent"),
        (E::Auth, "ai_code.auth"),
        (E::InternalError, "ai_code.internal_error"),
        (E::ModelSwitched, "ai_code.model_switched"),
        (E::ContextualTip, "ai_code.contextual_tip"),
    ];
...
fn client_identifier_allowlist_is_pinned() {
    ...
    assert_eq!(
        schema::sanitize_client_identifier("ai-pager"),
        "ai-pager"
    );
    assert_eq!(
...
fn sentinel_session_harness() -> events::SessionHarness {
    events::SessionHarness {
        ...
        client_identifier: Some("ai-pager".into()),
        model_id: "ai-4".into(),
        agent_name: "ai-build-plan".into(),
...
fn session_start_snapshot_counts_not_names() {
    ...
    let ev = &events[0];
    assert_eq!(ev.0, "ai_code.session_start");
...
fn session_new_increments_session_count_only() {
    ...
    assert_eq!(names, vec!["ai_code.session.count".to_owned()]);
}
...
fn agent_connect_timeout_emits_phase_histogram_and_timeout_counter() {
    ...
    assert_eq!(
        names,
        vec![
            "ai_code.startup.phase_duration".to_owned(),
            "ai_code.startup.timeout".to_owned(),
        ]
    );
}
...
fn startup_complete_records_the_total_histogram_only() {
    ...
    assert_eq!(
        ...
        vec!["ai_code.startup.total".to_owned()]
    );
}
...
fn api_request_snapshot_and_token_usage() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ai_code.api_request");
...
fn one_failed_turn_increments_error_count_exactly_once() {
    ...
    let names: Vec<String> = exported_events(&stream)
        ...
        .collect();
    assert_eq!(
        names.iter().filter(|n| *n == "ai_code.api_error").count(),
        2
    );
    ...
    let total: u64 = stream
        ...
        .filter(|m| m.name() == "ai_code.error.count")
        .map(|m| match m.data() {
...
fn turn_error_increments_error_count() {
    ...
    let mut names = exported_metric_names(&stream);
    names.sort();
    assert_eq!(names, vec!["ai_code.error.count", "ai_code.turn.count"]);
}
...
fn tool_result_gates_off_collapses_and_reduces() {
    ...
    emit_event_into(
        ...
        &events::ToolCallCompleted {
            ...
            outcome: sentinel_file_utils::events::types::ToolOutcome::Success,
            duration_ms: 42,
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ai_code.tool_result");
...
fn user_prompt_gates_off_drops_text() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ai_code.user_prompt");
...
fn mcp_connection_collapses_server_name_by_default() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ai_code.mcp_server_connection");
...
fn tool_decision_snapshot() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "ai_code.tool_decision");
...
fn contextual_tip_maps_every_tip_and_action() {
    ...
    for (tip, action, tip_label, action_label) in cases {
        ...
        let events = exported_events(&stream);
        let ev = &events[0];
        assert_eq!(ev.0, "ai_code.contextual_tip");
...
/// sentinel-ai-workspace sampler events, which live outside this crate and have
...
fn validating_metric_exporter_drops_export_on_bad_attr_key() {
    ...
    let rogue = meter.u64_counter("ai_code.session.count").build();
    rogue.add(
        1,
...
fn redacting_log_exporter_drops_record_with_closed_gate_key() {
    ...
    record.set_event_name("ai_code.user_prompt");
...
fn redacting_log_exporter_drops_record_with_unknown_key() {
    ...
    let logger = stream.ext.logger.as_ref().unwrap();
    let mut record = logger.create_log_record();
    record.set_event_name("ai_code.api_request");
...
fn metric_increments_pass_model_through_scrub_and_attach_session_id() {
    ...
    let exported = stream.metrics.get_finished_metrics().unwrap();
    let blob = format!("{exported:?}");
    assert!(blob.contains("ai_code.token.usage"));
    assert!(
