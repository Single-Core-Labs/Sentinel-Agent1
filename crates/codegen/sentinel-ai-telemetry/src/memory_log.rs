...
//!
//! Provides a dedicated tracing target (`sentinel_memory`) with an optional
//! file logger that writes to `~/.ai/logs/memory.log`.
//!
//! ## When to use
//!
...
//! AI_MEMORY_LOG=0 ai                # disable even when enabled
//! tail -f ~/.ai/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "sentinel_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "AI_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.ai/logs/memory.log`. Filters to `sentinel_memory=trace`.
    /// Set `AI_MEMORY_LOG=0` to disable, `AI_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || ai_home().join("logs").join("memory.log");
