//! Tracing layer for `target: "sampling_log"` → `~/.ai/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `AI_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "AI_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = ai_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
