...
use sentinel_ai_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
const SECRET_MODEL: &str = "ai-4-sk-LEAKmodel1234567890abcd";
...
fn external_stream_gates_on_end_to_end() {
    ...
    assert!(!sentinel_ai_telemetry::is_enabled());

    sentinel_ai_telemetry::log_event(sentinel_ai_telemetry::events::SessionHarness {
        session_id: "sess-gates-on".into(),
        client_identifier: Some("ai-pager".into()),
        model_id: "ai-4".into(),
        agent_name: "ai-build-plan".into(),
        permission_mode: sentinel_ai_telemetry::enums::PermissionMode::Ask,
    ...
    sentinel_ai_telemetry::log_event(sentinel_ai_telemetry::events::PromptSubmitted {
        prompt_length: 100,
        model_id: "ai-4".into(),
    ...
    sentinel_ai_telemetry::log_event(sentinel_ai_telemetry::events::ModelResponseReceived {
    ...
    sentinel_ai_telemetry::log_event(sentinel_ai_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "ai_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.sentinel.ai_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("ai-cli"),
        "service.name=ai-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("ai_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "ai_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "ai_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "ai_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "ai_code.session.count");
