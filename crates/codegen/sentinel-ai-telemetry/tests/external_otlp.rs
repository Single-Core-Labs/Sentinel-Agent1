...
fn external_stream_end_to_end() {
    ...
    let mut cfg = sentinel_ai_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = sentinel_ai_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    sentinel_ai_telemetry::external::init(Some(cfg));
    assert!(sentinel_ai_telemetry::external::is_active());
    ...
    sentinel_ai_telemetry::log_event(sentinel_ai_telemetry::events::SessionNew {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("ai-cli")),
        "service.name=ai-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "ai_code.session_start",
        "ai_code.user_prompt",
        "ai_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "ai_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "ai_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "ai_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "ai_code.token.usage"));
    assert_eq!(
    ...
    sentinel_ai_telemetry::external::shutdown();
    assert!(
