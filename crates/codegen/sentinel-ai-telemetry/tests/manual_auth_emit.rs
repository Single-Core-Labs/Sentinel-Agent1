...
//! `ai-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use sentinel_ai_telemetry::client;
use sentinel_ai_telemetry::config::{TelemetryConfig, TelemetryMode};
use sentinel_ai_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn manual_auth_posts_to_events_endpoint_as_ai_shell_manual_auth() {
    ...
    sentinel_ai_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "ai-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no ai-shell-manual_auth POST received"
        );
