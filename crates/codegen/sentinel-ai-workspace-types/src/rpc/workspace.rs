...
/// over `sentinel_ai_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// needs (a slim DTO over `sentinel_ai_tools`'s `TaskSnapshot`).
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// One TODO list item (slim DTO over `sentinel_ai_tools`'s `TodoState`). `status`
...
/// in `sentinel-ai-workspace/src/hub_server.rs`.
