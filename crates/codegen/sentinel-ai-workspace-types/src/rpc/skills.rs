...
//! `sentinel-ai-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    ...
    /// ~/.ai/skills
    User,
    /// ~/.ai/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    // Fixture mirrored field-for-field from the sentinel-ai-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.ai/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.ai/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.ai/plugins/infra-plugin",
            "plugin_data": "/root/.ai/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "ai-4",
            "effort": "high",
            "user_invocable": true,
