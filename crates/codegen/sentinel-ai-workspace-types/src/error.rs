...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] sentinel_ai_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `sentinel-ai-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `sentinel_ai_tools::ToolError`) happens
