//! Wire types for the `sentinel-ai-workspace` API.
//!
...
//! `sentinel-ai-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `sentinel-ai-workspace-types` (instead of `sentinel-ai-workspace` or
/// `sentinel-ai-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `sentinel_ai_workspace::permission`
