...
//! agent-session config types in `sentinel-ai-config` and friends.
...
/// `sentinel-ai-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `sentinel_ai_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
