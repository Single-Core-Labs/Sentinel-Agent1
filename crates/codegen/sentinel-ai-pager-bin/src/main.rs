...
use sentinel_ai_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use sentinel_ai_pager::client_identity::PAGER_CLIENT_VERSION;
use sentinel_ai_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use sentinel_ai_shell::agent::config::Config as AgentConfig;
use sentinel_ai_shell::leader::{
...
use sentinel_ai_update::{UpdateConfig, auto_update, enforce_version_policy_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.ai_ws_origin {
        config.ai_com_config.ai_ws_origin = v.clone();
    }
    if let Some(v) = &args.ai_ws_url {
        config.ai_com_config.ai_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &sentinel_ai_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.sentinel_api_base_url {
        config.endpoints.sentinel_api_base_url = v.clone();
    }
}
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Ai agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `ai -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use sentinel_ai_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(sentinel_ai_telemetry::sampling_log::layer())
        .with(sentinel_ai_telemetry::instrumentation::layer())
        .with(sentinel_ai_telemetry::hooks_log::layer())
        .with(sentinel_ai_telemetry::otel_layer::build_otel_layer(
            sentinel_ai_telemetry::otel_layer::OtelClientInfo {
                client_name: "ai-pager",
                client_version: sentinel_ai_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            sentinel_ai_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    sentinel_ai_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    sentinel_ai_telemetry::external::init(
        sentinel_ai_shell::agent::config::resolve_external_otel_config(
            sentinel_ai_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: sentinel_ai_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `ai setup`: rendering + exit codes only; fetch logic lives in `sentinel_ai_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use sentinel_ai_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `ai login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export AI_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:AI_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  ai setup");
        eprintln!();
        eprintln!("Or add the key to ~/.ai/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's Ai administrator."
        );
        std::process::exit(1);
    }
    if json {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `ai setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = sentinel_ai_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !sentinel_ai_shell::util::is_ai_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a ai process, removing stale lock");
        ...
        if let Err(e) = sentinel_ai_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(sentinel_ai_shell::env::AiBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, sentinel_ai_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = sentinel_ai_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = sentinel_ai_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&sentinel_ai_shell::leader::ControlPayload>,
...
/// Env override for the `ai workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "AI_WORKSPACE_COMMAND";
/// Resolution of the `ai workspace` gate. `Unknown` is kept separate from
...
/// The `AI_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&sentinel_ai_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for ai on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<sentinel_ai_shell::util::config::RemoteSettings> {
    join_early_prefetch(sentinel_ai_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    ) && let Some(profile) = sentinel_ai_sandbox::requested_confinement_profile()
    {
        anyhow::bail!(
            "`ai workspace` start/restart/resume is unavailable under sandbox profile '{profile}': \
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`ai workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `ai workspace`. Check your \
             network connection (run `ai login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.ai_com_config.ai_ws_url;
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a ai session, or run `ai workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = sentinel_ai_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use sentinel_ai_shell::auth::ensure_authenticated;
    sentinel_ai_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = sentinel_ai_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    let (use_leader, _) = resolve_use_leader(
        ...
        true,
        sentinel_ai_sandbox::requested_confinement_profile(),
    );
    if !use_leader {
        anyhow::bail!(
            "`ai workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.ai/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.ai_com_config,
        false,
        Some("No cached credentials found. Run `ai login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.ai_com_config);
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    sentinel_ai_telemetry::sentry::flush_on_shutdown();
    sentinel_ai_telemetry::otel_layer::shutdown_otel();
    sentinel_ai_telemetry::debug_log::flush();
...
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "ai: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<sentinel_ai_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use sentinel_ai_pager::app::signal_handler::next_signal_code;
    ...
    if matches!(
        ...
        sentinel_ai_shell::agent::app::suppress_otel();
    }
    init_tracing_simple("agent");
    let _otel_guard = sentinel_ai_telemetry::otel_layer::otel_guard();
    sentinel_ai_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => sentinel_ai_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(error = %e, "--trust: failed to resolve cwd; folder not trusted")
            }
        }
    }
    let early_prefetch = sentinel_ai_shell::agent::models::start_early_prefetch(None);
    sentinel_ai_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Ai Build (pager) - v{}",
            sentinel_ai_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                sentinel_ai_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(sentinel_ai_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = sentinel_ai_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = sentinel_ai_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&sentinel_ai_shell::agent::config::RuntimeResolutionContext {
    ...
    let requested_confinement = sentinel_ai_sandbox::requested_confinement_profile();
    let LeaderMode {
        use_leader,
    ...
    let managed_install = is_managed_install(
        ...
        &sentinel_ai_shell::util::ai_home::ai_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = sentinel_ai_shell::leader::LeaderEnvUrls::from(&agent_config.ai_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                if let Err(error) = sentinel_tty_utils::kill_current_process_on_parent_death() {
                    tracing::warn!(
                        %error,
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = sentinel_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
                            biased;
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = sentinel_ai_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            sentinel_ai_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(sentinel_ai_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = sentinel_ai_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = sentinel_ai_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `AI_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `ai dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `ai dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `AI_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !sentinel_ai_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.ai/config.toml and \
             unsetting AI_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("AI_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
const RUNTIME_SHUTDOWN_GRACE: std::time::Duration = std::time::Duration::from_secs(2);
const AI_WORKER_THREADS_ENV: &str = "AI_WORKER_THREADS";
...
/// How `AI_WORKER_THREADS` resolved.
#[derive(Debug, PartialEq, Eq)]
enum WorkerCount {
    ...
    fn notice(&self) -> Option<String> {
        match self {
            ...
            Self::Clamped {
                ...
                "ai: clamped {AI_WORKER_THREADS_ENV}={requested} to {used} (valid range is 1..={cores})"
            )),
            Self::Ignored { value, .. } => Some(format!(
                "ai: ignoring {AI_WORKER_THREADS_ENV}={value:?} (not a valid integer)"
            )),
        }
    }
}
fn cli_worker_threads() -> NonZeroUsize {
    ...
    let resolved = match std::env::var(AI_WORKER_THREADS_ENV) {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<sentinel_ai_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(sentinel_ai_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<sentinel_ai_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(sentinel_ai_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    sentinel_ai_shell::heap_profile::install(sentinel_ai_shell::heap_profile::HeapProfileHooks {
...
fn version_text(channel_label: &str) -> String {
    format!(
        "ai {}\n",
        sentinel_ai_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), channel_label,)
    )
}
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = sentinel_ai_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    sentinel_ai_telemetry::startup::mark_process_start();
    if let Some(code) = sentinel_ai_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = sentinel_ai_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    ...
    sentinel_ai_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    sentinel_ai_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        sentinel_ai_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        sentinel_ai_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    sentinel_ai_pager::memory_trace::start(sentinel_ai_pager::memory_trace::default_dir());
    raise_fd_limit();
    if let Err(e) = sentinel_ai_config::validate_requirements() {
        eprintln!("Couldn't start Ai: {e}");
        eprintln!();
        eprintln!(
            "Update Ai to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = sentinel_ai_telemetry::sentry::init(sentinel_ai_telemetry::sentry::Config {
        client: "ai-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: sentinel_ai_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    sentinel_ai_pager::docs::extract_user_guide_docs(&sentinel_ai_shell::util::ai_home::ai_home());
    sentinel_crash_handler::install_terminal_restore_only();
    if sentinel_ai_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = sentinel_ai_shell::util::ai_home::ai_home().join("crash");
        if let Some(report) = sentinel_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("Ai crashed during your last session.");
            eprintln!("  Signal:  {}", report.signal_name);
            eprintln!("  Version: {}", report.app_version);
            eprintln!("  Report:  {}", report.report_path.display());
            eprintln!();
        }
        if !sentinel_crash_handler::install(sentinel_crash_handler::CrashHandlerConfig {
    ...
    let crashed = sentinel_ai_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    let runtime = tokio::runtime::Builder::new_multi_thread()
        ...
        .build()
        .unwrap_or_else(|e| {
            eprintln!("ai: failed to start tokio runtime with {workers} workers: {e}");
            shutdown_and_flush_telemetry(1);
        });
    let result = run_and_shutdown(runtime, async_main(args), RUNTIME_SHUTDOWN_GRACE);
    sentinel_ai_telemetry::debug_log::flush();
    if let Err(e) = result {
        sentinel_tty_utils::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("AI_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("AI_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            unsafe { std::env::set_var(sentinel_ai_shell::agent::chat_modes::AI_CHAT_MODE_ENV, "1") };
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(sentinel_ai_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            unsafe { std::env::set_var("AI_DEBUG_LOG", path) };
            unsafe { std::env::remove_var("AI_LOG_FILE") };
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("AI_DEBUG_LOG", "1");
        set_if_unset("AI_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        sentinel_ai_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return sentinel_ai_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        sentinel_ai_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        sentinel_ai_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    sentinel_ai_shell::config::apply_sandbox(
        None,
    ...
    sentinel_ai_shell::http::set_client_name(if is_interactive {
        sentinel_ai_workspace::permission::ClientType::AiPager
    } else {
        sentinel_ai_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": sentinel_ai_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `ai-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                sentinel_ai_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return sentinel_ai_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = sentinel_ai_telemetry::otel_layer::otel_guard();
                return sentinel_ai_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = sentinel_ai_telemetry::otel_layer::otel_guard();
                let config = sentinel_ai_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sentinel_ai_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sentinel_ai_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            Command::DiskUsage(disk_usage_args) => {
                init_tracing_simple("cli");
                let _otel_guard = sentinel_ai_telemetry::otel_layer::otel_guard();
                return sentinel_ai_pager::disk_usage_cmd::run(disk_usage_args);
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sentinel_ai_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sentinel_ai_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return sentinel_ai_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sentinel_ai_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return sentinel_ai_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                sentinel_ai_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                sentinel_ai_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = sentinel_ai_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                sentinel_ai_shell::auth::run_cli_logout(&config)?;
                sentinel_ai_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = sentinel_ai_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(sentinel_ai_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == sentinel_ai_pager::headless::OutputFormat::Plain
        {
            args.output_format = sentinel_ai_pager::headless::OutputFormat::Json;
        }
        return sentinel_ai_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            sentinel_ai_pager::headless::HeadlessOptions {
    ...
    let result = sentinel_ai_pager::app::run(args, bg_update_rx).await;
    sentinel_ai_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `ai` to start.");
            } else {
                eprintln!("Update did not complete. Run `ai update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `ai update` child
...
/// fall back to a fresh blocking `ai update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = sentinel_ai_shell::env::AiBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                sentinel_ai_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("AI_NPM_REGISTRY"))
        .ok()
        .or_else(sentinel_ai_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = sentinel_ai_shell::config::load_effective_config_disk_only()
        && let Some(ch) = sentinel_ai_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("AI_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<ai_home>/bin/ai` resolves to, the
...
/// npm shim hardcodes `~/.ai`, so a custom `AI_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, ai_home: &std::path::Path) -> bool {
    if ai_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = sentinel_ai_config::ai_application_in(ai_home);
...
/// Handle `ai-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `ai update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in sentinel_ai_shell::leader::discover_leaders().await {
        if d.classification != sentinel_ai_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match sentinel_ai_shell::leader::LeaderClient::connect(
            socket_path,
            "ai-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(sentinel_ai_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(sentinel_ai_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn override_out_of_range_is_clamped_with_a_notice() {
        ...
        assert_eq!(
            ...
            "ai: clamped AI_WORKER_THREADS=100000 to 360 (valid range is 1..=360)"
        );
    }
    #[test]
    fn override_unparseable_is_ignored_with_a_notice() {
        ...
        assert_eq!(
            ...
            "ai: ignoring AI_WORKER_THREADS=\"abc\" (not a valid integer)"
        );
    }
    #[test]
    ...
    fn version_flags_and_doctor_are_distinct_early_intents() {
        let version = PagerArgs::try_parse_from(["ai", "--version"]).unwrap();
        assert!(version.version);
        assert!(version.command.is_none());
        let short = PagerArgs::try_parse_from(["ai", "-v"]).unwrap();
        assert!(short.version);
        assert!(short.command.is_none());
        let subcommand = PagerArgs::try_parse_from(["ai", "version"]).unwrap();
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            sentinel_ai_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: sentinel_ai_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/ai-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            sentinel_ai_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!sentinel_ai_shell::heap_profile::prof_available());
            return;
        }
        assert!(sentinel_ai_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(sentinel_ai_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(sentinel_ai_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        sentinel_ai_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_ai_target() {
        let home =
            std::env::temp_dir().join(format!("ai-pager-managed-install-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        std::fs::create_dir_all(home.join("bin")).unwrap();
        std::fs::create_dir_all(home.join("downloads")).unwrap();
        assert!(!is_managed_install(
            Some(home.join("bin").join("ai")),
            &home
        ));
        ...
        let target = home.join("downloads").join("ai-1.2.3");
        std::fs::write(&target, b"binary").unwrap();
        std::os::unix::fs::symlink(&target, home.join("bin").join("ai")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("ai")),
            &home
        ));
        assert!(is_managed_install(Some(target.clone()), &home));
        let pinned = home.join("bin").join("ai-9.9.9");
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("AI_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("AI_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `ai dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(AI_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["ai", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(AI_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("AI_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("AI_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("AI_WORKSPACE_COMMAND", "off") };
