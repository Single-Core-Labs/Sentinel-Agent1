//! Voice input for Ai Build CLI: an AI streaming STT client and the
...
mod intercept_tests {
    ...
    fn capture_subcommand_matches_only_argv1() {
        assert!(is_capture_subcommand(&argv(&["ai", "__mic-capture"])));
        ...
        assert!(!is_capture_subcommand(&argv(&["ai"])));
        assert!(!is_capture_subcommand(&argv(&["ai", "chat"])));
        assert!(!is_capture_subcommand(&argv(&[
            "ai",
            "chat",
            "__mic-capture"
