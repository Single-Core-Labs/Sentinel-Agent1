...
//! run) — and can be forced with `AI_VOICE_CAPTURE=inprocess`.
...
/// Env escape hatch: `AI_VOICE_CAPTURE=inprocess` forces the legacy
/// in-process cpal backend (accepting its permanent footprint cost).
const CAPTURE_BACKEND_ENV: &str = "AI_VOICE_CAPTURE";
