...
/// `[endpoints].sentinel_api_base_url` so enterprise proxies need no second knob.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(default)]
pub struct VoiceConfig {
    /// HTTPS API root (or bare host). Bases may end in `/v1` or `/sentinel/v1`; the
...
impl VoiceConfig {
    ...
    /// `api_base`: non-empty `[voice].api_base`, else `[endpoints].sentinel_api_base_url`
    ...
    pub fn from_config_table(root: &toml::Table, resolved_endpoints_base: Option<&str>) -> Self {
        ...
        .or_else(|| {
            non_empty_str(
                root.get("endpoints")
                    .and_then(|e| e.get("sentinel_api_base_url"))
                    .and_then(|v| v.as_str()),
            )
        })
...
fn ws_url(api_base: &str, path: &str) -> Result<String, VoiceError> {
    ...
    let rest = strip_scheme(base, "https://")
        ...
        .unwrap_or(base);
    // Default path `/v1/stt`; bases often end in `/v1` or `/sentinel/v1`.
...
mod tests {
    ...
    #[test]
    fn sentinel_v1_base_preserves_prefix() {
        let cfg = VoiceConfig {
            api_base: "https://proxy.example.com/sentinel/v1".into(),
            ..VoiceConfig::default()
        };
        assert_eq!(
            cfg.stt_ws_url().unwrap(),
            "wss://proxy.example.com/sentinel/v1/stt"
        );
    }
    ...
    fn inherits_endpoints_when_voice_api_base_unset() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
sentinel_api_base_url = "https://proxy.example.com/sentinel/v1"
"#,
        )
        .unwrap();
        let cfg = VoiceConfig::from_config_table(&table, None);
        assert_eq!(cfg.api_base, "https://proxy.example.com/sentinel/v1");
        assert_eq!(
            cfg.stt_ws_url().unwrap(),
            "wss://proxy.example.com/sentinel/v1/stt"
        );
...
"#,
    ...
    fn table_endpoints_beat_resolved_endpoints_base() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
sentinel_api_base_url = "https://config.example.com"
"#,
        )
        .unwrap();
