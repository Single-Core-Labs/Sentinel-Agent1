...
//! HPC login nodes) that pins 100+ thread slots per ai process against
//! per-user ceilings — systemd user-slice `pids.max` (commonly 1000) or
//! `RLIMIT_NPROC` — and later thread spawns die with EAGAIN. Ai's runtimes
...
//! the workspace (the `ai` binary, the `workspace_server` daemon) derives
//! its worker count from here so the policy cannot drift across crates.

use std::num::NonZeroUsize;

/// Maximum runtime worker threads for any ai process.
