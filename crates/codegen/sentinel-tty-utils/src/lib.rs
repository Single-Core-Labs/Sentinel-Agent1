...
//!
//! ```rust,no_run
//! use sentinel_tty_utils::{detach_command, pager_env};
...
//!
//! ```rust,no_run
//! use sentinel_tty_utils::{detach_std_command, pager_env};
//! use std::process::Stdio;
//!
...
#[cfg(unix)]
pub const RESET_CHILD_OOM_ENV: &str = "AI_TOOLS_RESET_CHILD_OOM";
...
/// killed ai and pile up on shared hosts. No-op on non-Linux platforms
...
/// the agent SDKs, `ai-desktop` all spawn `ai agent … stdio`): the
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_INTERMEDIATE_ENV: &str = "__AI_TTY_UTILS_PDEATHSIG_INTERMEDIATE";
    ...
    fn stderr_redirect_roundtrip_subprocess() {
        let status = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env("__AI_STDERR_REDIRECT_SUBPROCESS", "1")
            .status()
    ...
    fn stderr_redirect_roundtrip_body() {
        if std::env::var("__AI_STDERR_REDIRECT_SUBPROCESS").is_err() {
