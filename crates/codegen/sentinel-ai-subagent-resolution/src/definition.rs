...
use std::path::Path;
use sentinel_ai_agent::config::{AgentDefinition, IsolationMode};
use sentinel_ai_agent::plugins::PluginRegistry;
use sentinel_ai_agent::prompt::context::{PromptAudience, PromptContext};
...
pub fn discover_agent_definition(
    ...
    sentinel_ai_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        context.cwd,
        context.plugins,
    )
...
pub fn available_agent_names(context: &DefinitionResolutionContext<'_>) -> Vec<String> {
    let mut available: Vec<String> = sentinel_ai_agent::discovery::all_subagents_with_plugins(
        context.cwd,
        context.toggles,
        context.plugins,
    )
...
pub async fn render_subagent_initial_user_message(
    ...
    let agents_md_files = sentinel_ai_agent::prompt::agents_md::read_agents_config_with_paths(
