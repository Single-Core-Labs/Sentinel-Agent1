...
//! `sentinel_ai_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `sentinel-ai-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(persona.model.as_deref(), Some("ai-3-fast"));
