...
//!
//! Extracted from `sentinel-ai-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `sentinel-chat-state::compaction_utils::strip_system_tags` which
...
/// `sentinel-ai-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use sentinel_ai_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(sentinel_ai_sampling_types::synthesized_reasoning_item(text))
    }
