...
use sentinel_sqlite_journal::{BUSY_RETRY_BUDGET, JournalMode};
...
impl WorktreeDb {
    /// Open (or create) the DB at `ai_home/worktrees.db`.
    pub fn open(ai_home: &Path) -> Result<Self> {
        Self::open_at(&ai_home.join(WORKTREES_DB_FILE))
    }
    ...
    /// Open the default DB at `~/.ai/worktrees.db`.
    ///
    /// Discovers ai home via `$AI_HOME`, falling back to the canonicalized
    /// `$HOME/.ai` (matching `sentinel_ai_config::ai_home`).
    ...
    pub fn open_default() -> Result<Self> {
        Self::open(&resolve_ai_home()?)
    }

    fn journal_mode_and_base_path(ai_home: &Path) -> (JournalMode, PathBuf) {
        let base_path = ai_home.join(WORKTREES_DB_FILE);
    ...
    pub fn resolve_db_path(ai_home: &Path) -> PathBuf {
        let (mode, base_path) = Self::journal_mode_and_base_path(ai_home);
    ...
    pub fn open_read_only(ai_home: &Path) -> RegistryOpen {
        let (mode, base_path) = Self::journal_mode_and_base_path(ai_home);
...
pub fn resolve_ai_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("AI_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home = PathBuf::from(std::env::var("HOME").context("neither $AI_HOME nor $HOME is set")?);
    // Canonicalize the home dir so worktree paths share the same physical .ai
    // tree as trust/hooks even when it is symlinked. The dunce canonicalization
    // must stay in sync with sentinel_ai_config::default_ai_home();
    // home resolution deliberately differs ($HOME here vs std::env::home_dir()).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".ai"))
}

/// Serializes tests that mutate the process-global `AI_HOME` env var so they
...
#[cfg(test)]
static AI_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`AI_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `AI_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `AI_HOME` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct AiHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated ai home; pass to `WorktreeDb::open` to read the same DB
...
impl AiHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = AI_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("ai-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via AI_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until AI_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("AI_HOME");
        unsafe { std::env::set_var("AI_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for AiHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => unsafe { std::env::set_var("AI_HOME", p) },
                None => unsafe { std::env::remove_var("AI_HOME") },
            }
        }
    }
}
