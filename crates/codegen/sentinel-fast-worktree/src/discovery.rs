...
/// [`scan_two_level_dir`] and `ai du`'s bucketing have to agree on it.
...
pub fn discover_worktrees(ai_home: &Path) -> DiscoveryReport {
    let mut report = DiscoveryReport::default();
    scan_two_level_dir(
        &ai_home.join(WORKTREES_DIR),
        WorktreeKind::Session,
        &mut report,
    );
    scan_two_level_dir(
        &ai_home.join(WORKTREE_POOL_DIR),
        WorktreeKind::Pool,
        &mut report,
    );
    report
...
pub fn managed_worktree_roots(ai_home: &Path) -> [PathBuf; 2] {
    [
        ai_home.join(WORKTREES_DIR),
        ai_home.join(WORKTREE_POOL_DIR),
    ]
...
pub fn path_under_managed_worktree_roots(path: &Path, ai_home: &Path) -> bool {
    path_under_worktree_roots(path, &managed_worktree_roots(ai_home))
}
...
pub fn rebuild_worktree_db(
    ...
    let discovery = discover_worktrees(ai_home);
    ...
    for wt in discovery.found {
        ...
        if !path_under_worktree_roots(&path, &roots) {
            tracing::warn!(
                path = %path.display(),
                "rebuild skipped path outside ai worktrees/worktree_pool"
            );
            continue;
        }
...
mod tests {
    ...
    fn discover_session_worktrees() {
        ...
        let ai_home = tmp.path();

        let wt = ai_home.join("worktrees/myrepo/worktree-abc123");
        make_fake_linked_worktree(&wt, "/repo/.git/worktrees/abc123");

        let report = discover_worktrees(ai_home);
    ...
    fn discover_pool_worktrees() {
        ...
        let wt = ai_home.join("worktree_pool/inst-1/pool-a");
    ...
    fn skips_dot_prefixed_and_markers() {
        ...
        let base = ai_home.join("worktrees/myrepo");
    ...
    fn rebuild_registers_and_skips_duplicates() {
        ...
        let wt = ai_home.join("worktrees/repo/worktree-sess1");
        make_fake_standalone_worktree(&wt);

        let db = crate::db::WorktreeDb::open_in_memory().unwrap();

        let r1 = rebuild_worktree_db(&db, ai_home).unwrap();
        assert_eq!(r1.discovered, 1);
        assert_eq!(r1.registered, 1);
        assert_eq!(r1.already_tracked, 0);

        let r2 = rebuild_worktree_db(&db, ai_home).unwrap();
    ...
    fn rebuild_keeps_same_basename_worktrees_in_different_repos() {
        ...
        let wt_a = ai_home.join("worktrees/repo-a/wt-abc");
        let wt_b = ai_home.join("worktrees/repo-b/wt-abc");
        make_fake_standalone_worktree(&wt_a);
        make_fake_standalone_worktree(&wt_b);

        let db = crate::db::WorktreeDb::open_in_memory().unwrap();
        let report = rebuild_worktree_db(&db, ai_home).unwrap();
        ...
        let report2 = rebuild_worktree_db(&db, ai_home).unwrap();
    ...
    fn rebuild_sets_last_accessed_at() {
        let tmp = tempfile::TempDir::new().unwrap();
        let ai_home = tmp.path();
        let wt = ai_home.join("worktrees/repo/sess");
        make_fake_standalone_worktree(&wt);
        let db = crate::db::WorktreeDb::open_in_memory().unwrap();
        rebuild_worktree_db(&db, ai_home).unwrap();
