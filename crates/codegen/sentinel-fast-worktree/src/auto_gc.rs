...
use crate::db::{ListFilter, WorktreeDb, WorktreeKind, now_epoch_secs, resolve_ai_home};
...
pub const ENV_AUTO_GC: &str = "AI_WORKTREE_AUTO_GC";
/// `1` / `true` / `on` forces age-count without delete.
pub const ENV_AUTO_GC_DRY_RUN: &str = "AI_WORKTREE_AUTO_GC_DRY_RUN";
/// Default max age in seconds (overrides TOML/remote when set and parseable).
pub const ENV_AUTO_GC_MAX_AGE: &str = "AI_WORKTREE_AUTO_GC_MAX_AGE";
/// `1` / `true` / `on` enables optional discovery rebuild + stale git prune.
pub const ENV_AUTO_GC_REBUILD: &str = "AI_WORKTREE_AUTO_GC_REBUILD";
...
/// Parse `AI_WORKTREE_AUTO_GC_MAX_AGE` as seconds; invalid/absent → None.
...
fn maybe_run_rebuild(
    ...
    let home = match resolve_ai_home() {
        Ok(h) => h,
        Err(e) => {
            tracing::warn!(error = %e, "auto worktree rebuild skipped: ai home unresolved");
            return (None, false);
        }
    };
...
/// Scrub stale ai-owned registrations from each known source repo,
/// scoped to worktrees under the ai home to prove ownership (see
...
fn prune_stale_git_worktree_registrations(repos: &BTreeSet<PathBuf>) -> u64 {
    let Ok(ai_home) = resolve_ai_home() else {
        tracing::warn!("auto worktree registration scrub skipped: ai home unresolved");
        return 0;
    };
    let cleaned: u64 = repos
        .iter()
        .filter(|repo| repo.is_dir())
        .map(|repo| crate::git::remove_stale_worktree_registrations_under(repo, &ai_home))
...
mod tests {
    ...
    fn clear_auto_gc_env() {
        unsafe {
            unsafe { std::env::remove_var(ENV_AUTO_GC) };
            unsafe { std::env::remove_var(ENV_AUTO_GC_DRY_RUN) };
            unsafe { std::env::remove_var(ENV_AUTO_GC_MAX_AGE) };
            unsafe { std::env::remove_var(ENV_AUTO_GC_REBUILD) };
        }
    }
    ...
    fn include_rebuild_false_skips_rebuild_and_does_not_stamp_rebuild_meta() {
        let _g = env_guard();
        clear_auto_gc_env();
        let fx = crate::db::AiHomeFixture::new();
    ...
    #[test]
    fn include_rebuild_true_registers_untracked_under_ai_home() {
        let _g = env_guard();
        clear_auto_gc_env();
        let fx = crate::db::AiHomeFixture::new();
        let db = WorktreeDb::open(&fx.home).unwrap();
        ...
        assert!(
            ...
            "untracked dir under ai_home/worktrees must be registered"
        );
    }
    ...
    #[test]
    fn prune_keeps_foreign_registrations_outside_ai_home() {
        let _g = env_guard();
        clear_auto_gc_env();
        let fx = crate::db::AiHomeFixture::new();
        let db = WorktreeDb::open(&fx.home).unwrap();
        ...
        assert_eq!(
            count_regs(&repo),
            before,
            "user registrations outside ai home must survive"
        );
    }
