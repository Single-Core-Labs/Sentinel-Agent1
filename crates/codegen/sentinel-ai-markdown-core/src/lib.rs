//! Headless markdown analysis sharing Ai Build's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `sentinel-ai-markdown` so analysis matches what Ai
//! Build actually renders 1:1.
//!
//! After parsing, Ai applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Ai Build uses to render markdown.
///
...
/// Offset event stream from Ai's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Ai Build's options; count elements and flag structural issues.
