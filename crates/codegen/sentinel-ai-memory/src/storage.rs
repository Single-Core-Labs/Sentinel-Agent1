...
//! `~/.ai/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use sentinel_ai_tools::util::ai_home::ai_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `sentinel_ai_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.ai/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.ai/memory/sentinel-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.ai/memory/`
    global_dir: PathBuf,
    /// `~/.ai/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.ai/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| ai_home().join("memory"));
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        // mounts (SIGBUS); see sentinel_sqlite_journal::JournalMode::open_readonly.
        sentinel_sqlite_journal::JournalMode::for_db_path(&db_path)
    ...
    ///
    /// File path: `~/.ai/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
    ...
    pub fn ensure_initialized(&self) -> std::io::Result<()> {
        ...
        if !global_file.exists() {
            std::fs::write(
                ...
                "# Global Memory\n\
                 \n\
                 > This file is automatically managed by Ai's memory system.\n\
...
mod tests {
        ...
        INIT.call_once(|| {
            if let Ok(git_bin) = std::env::var("GIT_BIN_PATH") {
                ...
                if let Some(dir) = p.parent() {
                    ...
                    unsafe {
                        unsafe { std::env::set_var("PATH", format!("{}:{}", dir.display(), cur)) };
                    }
                }
            }
        });
    ...
    fn test_compute_workspace_hash_human_readable() {
        let name = compute_workspace_hash(Path::new("/users/me/work/sentinel"));
        assert!(
