...
//! }
//!
//! sentinel_crash_handler::install(sentinel_crash_handler::CrashHandlerConfig {
...
/// `$AI_HOME` they must not be world-readable.
...
mod tests {
    ...
    fn check_previous_crash_returns_none_when_no_file() {
        let dir = PathBuf::from("/tmp/sentinel-crash-handler-test-nonexistent");
