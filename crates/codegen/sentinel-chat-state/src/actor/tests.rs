...
use tokio::sync::mpsc;
use sentinel_ai_sampling_types::{ConversationItem, SamplingConfig};
...
async fn record_last_turn_usage_round_trip() {
    use sentinel_ai_sampling_types::TokenUsage;
...
async fn build_request_includes_all_messages() {
    ...
    assert_eq!(request.x_ai_conv_id, Some("conv-1".to_string()));
    assert_eq!(request.x_ai_req_id, Some("req-1".to_string()));
}
...
async fn build_request_with_tool_definitions() {
    use sentinel_ai_sampling_types::ToolSpec;
...
async fn build_request_uses_sampling_config() {
    ...
    assert_eq!(request.model, Some("ai-3".to_string()));
...
/// Simulates the exact sequence that `sentinel-ai-shell`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools =
        ConversationItem::Assistant(sentinel_ai_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use sentinel_ai_sampling_types::{ContentPart, UserItem};
...
async fn sampling_config_survives_compaction_replacement() {
    use sentinel_ai_sampling_types::ApiBackend;
    ...
    assert_eq!(pre_meta.resolved_model_id.as_deref(), Some("ai-4.5"));
...
/// If that model slug is e.g. "ai-build" and not in the ModelState
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            sentinel_ai_sampling_types::AssistantItem {
...
async fn context_window_downgrade_triggers_auto_compact() {
    ...
    // Initial config: 500k context, Responses backend (matches ai-4.5)
    let config = SamplingConfig {
    ...
    assert_eq!(post.model, "ai-4.5", "model slug must not change");
    assert_eq!(
        post.api_backend,
        ApiBackend::Responses,
...
fn serialize_via_public_api(
    req: &sentinel_ai_sampling_types::ConversationRequest,
) -> serde_json::Value {
    ...
    sentinel_ai_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &sentinel_ai_sampling_types::ConversationRequest,
    extended: &sentinel_ai_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(sentinel_ai_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(sentinel_ai_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use sentinel_ai_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
