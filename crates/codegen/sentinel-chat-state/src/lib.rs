//! sentinel-chat-state — Actor-based chat state management for AI agents.
//!
//! This crate extracts conversation state management from `sentinel-ai-shell`'s
//! `acp_session.rs` into a standalone actor. It follows the same actor pattern
//! as `sentinel-hunk-tracker`:
//!
//! ```text
//! ┌────────────────┐                  ┌──────────────────────────────────────┐
