...
/// emitter in `sentinel-ai-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (ai login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
    /// From user config ([model.*] api_key, env_key, AI_API_KEY). Not refreshable.
    ApiKey,
}
...
/// `sentinel_ai_sampling_types::SamplingConfig` (which is secret-free).
...
mod tests {
    ...
    fn snapshot_round_trips_with_data() {
        use sentinel_ai_sampling_types::ConversationItem;
