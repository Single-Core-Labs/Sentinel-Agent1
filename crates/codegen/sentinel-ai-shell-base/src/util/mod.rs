...
pub mod tips;
pub mod uname;
pub use sentinel_ai_shared::clipboard;
pub use sentinel_ai_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// toggles via `AI_CLI_CHAT_PROXY_BASE_URL` (or similar) pointing at an
...
/// True for AI-operated endpoints (`*.x.ai`, cli-chat-proxy, and optional
/// non-production AI hosts when that feature is enabled).
...
/// *attach* a credential, use [`is_sentinel_api_bearer_url`].
pub fn is_sentinel_api_url(url: &str) -> bool {
    is_sentinel_api_url_impl(url, false)
}
/// Like [`is_sentinel_api_url`], but requires `https` on every arm, so a
...
pub fn is_sentinel_api_bearer_url(url: &str) -> bool {
    is_sentinel_api_url_impl(url, true)
}
fn is_sentinel_api_url_impl(url: &str, require_https: bool) -> bool {
    if require_https {
...
/// True if `pid` is a ai process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
...
pub fn is_ai_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        ...
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("ai"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
/// Stricter [`is_ai_process`] for the auto-kill zombie path: on macOS/BSD it
...
/// exactly, so this delegates there. Use the permissive [`is_ai_process`] for
/// operator-driven `ai leaders kill`.
pub fn is_ai_process_strict(pid: u32) -> bool {
    #[cfg(all(not(target_os = "linux"), not(windows)))]
    {
        ...
        match cmd.output() {
            Ok(out) if out.status.success() => {
                ...
                comm.lines()
                    ...
                    .is_some_and(|name| name.to_ascii_lowercase().contains("ai"))
            }
            _ => false,
        }
    }
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_accepts_proxy_subpath() {
        assert!(is_cli_chat_proxy_url(
            "https://cli-chat-proxy.ai.com/v1/chat/completions"
        ));
    }
    #[test]
    ...
    fn test_is_cli_chat_proxy_url_rejects_spoofed_hostname() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.ai.com.evil.example/v1"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_v11_prefix_confusion() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.ai.com/v11/chat/completions"
        ));
    }
    #[test]
    fn test_is_sentinel_api_url() {
        assert!(is_sentinel_api_url("https://api.x.ai/v1"));
        assert!(is_sentinel_api_url("https://api.x.ai/v1/chat/completions"));
        assert!(is_sentinel_api_url("https://x.ai"));
        assert!(is_sentinel_api_url(
            "https://cli-chat-proxy.ai.com/v1/chat/completions"
        ));
        assert!(!is_sentinel_api_url("https://api.openai.com/v1"));
        assert!(!is_sentinel_api_url("https://api.anthropic.com/v1"));
        assert!(!is_sentinel_api_url("https://generativelanguage.googleapis.com"));
        assert!(!is_sentinel_api_url("https://api.x.ai.evil.example/v1"));
        assert!(!is_sentinel_api_url("https://evil-x.ai.attacker.com/v1"));
        assert!(!is_sentinel_api_url("https://prefixx.ai/v1"));
        assert!(!is_sentinel_api_url("not-a-url"));
        assert!(!is_sentinel_api_url(""));
        assert!(is_sentinel_api_url("http://api.x.ai/v1"));
        assert!(is_sentinel_api_url("http://localhost:11434/v1"));
    }
    #[test]
    fn test_is_sentinel_api_bearer_url() {
        assert!(is_sentinel_api_bearer_url("https://api.x.ai/v1"));
        assert!(!is_sentinel_api_bearer_url("http://api.x.ai/v1"));
        assert!(!is_sentinel_api_bearer_url("http://localhost:11434/v1"));
        {
            assert!(!is_sentinel_api_bearer_url("https://localhost:11434/v1"));
            assert!(!is_sentinel_api_bearer_url("https://127.0.0.2:11434/v1"));
            assert!(!is_sentinel_api_bearer_url("https://[::1]:11434/v1"));
        }
        assert!(is_sentinel_api_bearer_url("https://API.X.AI/v1"));
        assert!(!is_sentinel_api_bearer_url(
            "https://api.x.ai@attacker.example/v1"
        ));
        assert!(!is_sentinel_api_bearer_url("https://х.ai/v1"));
    }
    #[test]
    fn test_truncate() {
    ...
    #[test]
    fn is_ai_process_self_true_impossible_pid_false() {
        assert!(is_ai_process(std::process::id()));
        assert!(!is_ai_process(u32::MAX));
    }
    #[test]
    fn is_ai_process_strict_self_true_impossible_pid_false() {
        assert!(is_ai_process_strict(std::process::id()));
        assert!(!is_ai_process_strict(u32::MAX));
    }
    #[cfg(unix)]
    #[test]
