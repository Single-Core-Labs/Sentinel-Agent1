...
//! persisted to `~/.ai/tip_cursor.json`.
...
fn cursor_path(ai_home: &Path) -> PathBuf {
    ai_home.join(CURSOR_FILE)
}

/// Load the cursor from `~/.ai/tip_cursor.json`. Returns 0 on any error.
fn load_cursor(ai_home: &Path) -> u64 {
    let text = match std::fs::read_to_string(cursor_path(ai_home)) {
        Ok(t) => t,
        Err(_) => return 0,
    };
...
/// Save the cursor to `~/.ai/tip_cursor.json`. Silently ignores write errors.
fn save_cursor(ai_home: &Path, cursor: u64) {
    if let Ok(text) = serde_json::to_string(&TipState { cursor }) {
        let _ = std::fs::write(cursor_path(ai_home), text);
    }
}
...
/// cursor in `~/.ai/tip_cursor.json`, so every session sees the next tip
...
pub fn pick_and_advance(tips: &[String], ai_home: &Path) -> Option<String> {
    if tips.is_empty() {
        return None;
    }
    let cursor = load_cursor(ai_home);
    let tip = tips[cursor as usize % tips.len()].clone();
    save_cursor(ai_home, cursor + 1);
    Some(tip)
}
