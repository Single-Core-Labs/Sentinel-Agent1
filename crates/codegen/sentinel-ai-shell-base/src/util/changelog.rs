...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$AI_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `ai_home()` OnceLock). A seeded `$AI_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("AI_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::ai_home::ai_home);
        Self {
    ...
    ///
    /// When `AI_CHANGELOG_OFFLINE` is set (PTY / integration tests), skip
    ...
    /// `$AI_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`AI_HOME` /
    /// `AI_CHANGELOG_OFFLINE`), which races across the parallel test
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$AI_HOME` even when offline mode was not
...
fn changelog_offline() -> bool {
    std::env::var_os("AI_CHANGELOG_OFFLINE").is_some_and(|v| !v.is_empty() && v != "0")
}
...
mod tests {
    ...
    /// `$AI_HOME` env so tests never race the parallel harness.
    ...
    fn cdn_miss_falls_back_to_env_home_disk_cache() {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().join("ai-home-fallback");
        std::fs::create_dir_all(&home).unwrap();
