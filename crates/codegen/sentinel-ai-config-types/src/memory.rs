//! Memory-system configuration value types, extracted from sentinel-ai-shell
...
//! `sentinel-ai-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.ai/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.ai/memory/` are scanned:
