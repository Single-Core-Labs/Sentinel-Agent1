...
use sentinel_ai_announcements::RemoteAnnouncement;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-ai-doom-loop-check` header, parse the reported
...
pub struct WorktreeAutoGcSettings {
    ...
    /// Optional discovery rebuild + ai-scoped stale `.git/worktrees/` scrub (default off).
    #[serde(
        default,
        deserialize_with = "de_opt_bool_tolerant",
        skip_serializing_if = "Option::is_none"
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `ai_build_settings`
    ...
    /// default (on). Sits below env `AI_FOLDER_TRUST`, user
    ...
    /// remote settings `ai_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `ai_build_settings` remote settings key) to enable it. See
    ...
    /// `sentinel_ai_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `AI_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `ai_build_settings`).
    ...
    /// remote settings gate for first-run auto-registration of the official AI
    ...
    /// ghost text), from the `ai_build_settings` remote settings flag. Sits below
    /// env (`AI_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `ai-build-0.1` default. The effective model is catalog-guarded: when
    ...
    /// (e.g. `ai-imagine-image`) instead of the default quality model
    /// (`ai-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `AI_VOICE_MODE`. Free-tier SuperAi upsell
    ...
    /// remain coding-data opted-out. From `ai_build_settings`.
    ...
    /// User's subscription tier from remote settings `ai_build_access_gate`.
    /// E.g. "free", "premium", "superai", "superai_heavy".
    ...
    /// Whether the user is allowed to use Ai Build. Set by remote settings
    /// `ai_build_access_gate` targeting rules. `None` = no server response
    ...
    /// (e.g. "SuperAi", "X Premium+", "Free", "API Key"). Set by CCP
    ...
    /// Server-controlled via the remote settings `ai_build_usage_redirect_url`
    ...
    /// `ai_build_settings`. Per-model override on `ModelInfo`
    /// (`ai_build_models`) takes precedence; user config and env var
    /// further override per the resolver chain.
    #[serde(default)]
    pub auto_compact_threshold_percent: Option<u8>,
    /// Max subagent nesting depth (`ai_build_settings.subagents_max_depth`).
    #[serde(default)]
    ...
    /// `0` disables. Env (`AI_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`AI_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`AI_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`AI_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `ai workspace` CLI command (Computer Hub
    /// workspace exposure), from `ai_build_settings.workspace_command_enabled`.
...
pub struct ContextualHintsRemote {
    ...
    /// SSH wrap session-load tip (recommend `ai wrap ssh` for remote sessions).
    #[serde(default)]
...
/// only guaranteed to work with a compatible harness (cursor vs ai-build).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoalRoleModel {
    /// Model id, e.g. "ai-4". Resolved against available models at
    /// spawn time; unknown/unauthorized ⇒ fail-open to current model.
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "ai-build-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-ai-build toolset), applied REGARDLESS of the
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "ai-build"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("ai-build"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "ai-build-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("ai-build-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_goal_planner_model_round_trip() {
        let json =
            r#"{"goal_planner_model": {"model": "ai-4", "agent_type": "general-purpose"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_strategist_model_round_trip() {
        let json = r#"{"goal_strategist_model": {"model": "ai-4.5", "agent_type": "cursor"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_fully_valid_pool_round_trips() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "ai-4", "agent_type": "general-purpose"},
            {"model": "ai-3", "agent_type": "cursor"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_extra_unknown_fields_kept() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "ai-4", "agent_type": "general-purpose", "reasoning_effort": "high"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_skeptic_models,
    ...
    fn remote_settings_goal_role_models_malformed_pair_does_not_drop_other_fields() {
        let json = r#"{
            ...
            "goal_strategist_model": {"model": "ai-4.5", "agent_type": "cursor"},
            "default_model": "ai-4"
        }"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.goal_planner_model, None);
        ...
        assert_eq!(s.default_model.as_deref(), Some("ai-4"));
    }
    #[test]
    fn remote_settings_goal_role_model_extra_unknown_fields_kept_single_pair() {
        let json = r#"{"goal_planner_model": {"model": "ai-4", "agent_type": "general-purpose", "future": true}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_planner_model,
            Some(GoalRoleModel {
