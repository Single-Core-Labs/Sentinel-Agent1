...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"AiBuild:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"AiBuild:run_terminal_cmd"`).
...
/// between namespaces (e.g. `AiBuild:run_terminal_cmd` vs.
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("AiBuild:run_terminal_cmd", "legacy-0.4.10"),
            ("AiBuild:read_file", "legacy-0.4.10"),
            ("AiBuild:search_replace", "legacy-0.4.10"),
            ("AiBuild:get_task_output", "legacy-0.4.10"),
            ("AiBuild:kill_task", "legacy-0.4.10"),
            ("AiBuild:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("AiBuild:run_terminal_cmd", "current"),
            ("AiBuild:read_file", "current"),
            ("AiBuild:search_replace", "current"),
            ("AiBuild:list_dir", "current"),
            ("AiBuild:grep", "current"),
            ("AiBuild:kill_task", "current"),
            ("AiBuild:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "AiBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "AiBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "AiBuild:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "AiBuild:web_search", None).unwrap();
        assert_eq!(v, None);
    }
    ...
    fn override_on_unmanaged_tool_errors() {
        let err =
            resolve_version("current", "AiBuild:web_search", Some("legacy-0.4.10")).unwrap_err();
        assert!(err.contains("unmanaged tool"));
    }

    #[test]
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "AiBuild:run_terminal_cmd", None).unwrap_err();
    ...
    fn concise_namespace_not_managed() {
        // AiBuildConcise tools should NOT be version-managed.
        assert!(!is_version_managed("AiBuildConcise:run_terminal_cmd"));
        let v = resolve_version("current", "AiBuildConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("AiBuild:run_terminal_cmd"));
        assert!(is_version_managed("AiBuild:read_file"));
        assert!(is_version_managed("AiBuild:search_replace"));
        assert!(is_version_managed("AiBuild:list_dir"));
        assert!(is_version_managed("AiBuild:grep"));
        assert!(is_version_managed("AiBuild:kill_task"));
        // Not managed:
        assert!(!is_version_managed("AiBuild:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res =
            resolve_version_with_warnings("current", "AiBuild:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) =
            validate_and_resolve("AiBuild:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("AiBuild:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res = resolve_version_with_warnings("current", "AiBuild:web_search", None).unwrap();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err = resolve_version_with_warnings("current", "AiBuild:grep", Some("legacy-0.4.10"))
            .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),
