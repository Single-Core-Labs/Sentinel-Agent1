...
pub use sentinel_ai_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__AI_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    sentinel_ai_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = sentinel_ai_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias ai_alias_probe='echo ALIAS_OK'\nai_fn_probe() { echo FN_OK; }\n",
            "ai_alias_probe && ai_fn_probe",
        )
        .await;
