...
struct LocalTerminalActor {
    ...
    /// Linux-only: capture the login shell's env snapshot.
    #[allow(dead_code)]
...
impl LocalTerminalActor {
    ...
    async fn spawn_static_command(
        ...
        unsafe {
            cmd.pre_exec(sentinel_tty_utils::detach_pre_exec_hook());
        }

        #[cfg(target_os = "linux")]
        if sentinel_ai_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| sentinel_ai_sandbox::child_net::install_child_network_filter());
            }
        }
                ...
                && !process.lifecycle.has_exited()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::ai_build::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
#[cfg(unix)]
const ENV_LOGIN_ENV: &str = "AI_LOGIN_ENV";
...
/// active policy filters out and those already set in ai's own environment.
#[cfg(unix)]
fn layer_login_env_vars(
    ...
    if let Some(login) = login_env {
        for (key, value) in login {
            // `var_os` reads ai's own process env (not the possibly cleared
            // child env): a login var already present in ai's environment is
...
/// policy base, login-shell capture, ai control vars, request env, pager
...
/// name cannot re-enter; ai's own control vars, login `PATH`, and the marker
...
/// path must inherit ai's environment untouched (including non-UTF-8 vars).
...
fn apply_child_env(
    ...
    // 2. Login-shell capture (filtered). 3. Ai control vars. 4. Request env
    ...
    crate::util::apply_ai_agent_marker(cmd);
}
...
fn spawn_shell_command(
    ...
    let mut cmd = {
        ...
        // The parent (ai) process retains network for the LLM API.
    ...
    let build_cmd = |with_breakaway: bool| {
        ...
        let inv = sentinel_ai_config::shell::shell_command_argv(command);
        ...
        // `inv.env` is ai's trusted shell setup, so it is not filtered.
        ...
        cmd.envs(crate::util::pager_env());
        crate::util::apply_ai_agent_marker(&mut cmd);
        ...
        //     have JOB_OBJECT_LIMIT_BREAKAWAY_OK set — common when ai-agent
...
mod tests {
    ...
    fn apply_child_env_layers_in_fixed_order() {
        ...
        let policy = ShellEnvironmentPolicy {
            ...
            set: HashMap::from([("AI_TEST_BASE".to_string(), "1".to_string())]),
            ..Default::default()
        };
        let login = HashMap::from([
            ("AI_TEST_LOGIN".to_string(), "l".to_string()),
            ("PATH".to_string(), "/login/bin".to_string()),
        ]);
        let request = HashMap::from([
            ("AI_TEST_REQ".to_string(), "r".to_string()),
            ("PATH".to_string(), "/req/bin".to_string()),
            ("AI_TEST_SECRET".to_string(), "s".to_string()),
        ]);
        ...
        assert_eq!(env.get("AI_TEST_BASE").map(String::as_str), Some("1"));
        assert_eq!(env.get("AI_TEST_LOGIN").map(String::as_str), Some("l"));
        assert_eq!(env.get("AI_TEST_REQ").map(String::as_str), Some("r"));
        // Request env is filtered by the policy.
        assert!(!env.contains_key("AI_TEST_SECRET"));
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export AI_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $AI_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
    ...
    async fn test_persistent_shell_clears_gpg_tty() {
        ...
        req.env
            .insert("GPG_TTY".to_string(), "/ai-sentinel-tty".to_string());
