...
const BASH_STATE_START_MARKER: &str = "__AI_BASH_STATE_START__";
const BASH_STATE_END_MARKER: &str = "__AI_BASH_STATE_END__";
const ZSH_STATE_START_MARKER: &str = "__AI_ZSH_STATE_START__";
const ZSH_STATE_END_MARKER: &str = "__AI_ZSH_STATE_END__";

/// Marker emitted by the init path to separate login-shell noise (MOTD, etc.)
/// from the actual state dump on stdout.
const INIT_STATE_MARKER: &str = "__AI_INIT_STATE_MARKER__";
...
pub fn shell_env_overrides() -> HashMap<String, String> {
    HashMap::from([
        ...
        // Re-applied last via [`crate::util::apply_ai_agent_marker`] so request
        // env cannot clear it.
        (
            crate::util::AI_AGENT_ENV.to_string(),
            crate::util::AI_AGENT_ENV_VALUE.to_string(),
        ),
    ])
}
...
dump_bash_state() {
  ...
  _emit_encoded() {
    ...
    local var_name="$2"
    if [[ -n "$content" ]]; then
      builtin printf 'ai_snap_%s=$(command base64 -d <<'"'"'AI_SNAP_EOF_%s'"'"'\n' "$var_name" "$var_name"
      command base64 <<<"$content" | command tr -d '\n'
      builtin printf '\nAI_SNAP_EOF_%s\n' "$var_name"
      builtin printf ')\n'
      builtin printf 'eval "$ai_snap_%s"\n' "$var_name"
    fi
  }

  _emit "__AI_BASH_STATE_START__"

  _emit "$PWD"

  local env_vars
  env_vars=$(builtin export -p 2>/dev/null | command grep -viE '_proxy=|AI_SANDBOX|AI_AGENT=|SUDO_ASKPASS|AI_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
...
function dump_zsh_state() {
  ...
  _emit "__AI_ZSH_STATE_START__"

  _emit "$PWD"

  local env_vars
  env_vars=$(builtin typeset -xp 2>/dev/null | command grep -viE '_proxy=|AI_SANDBOX|AI_AGENT=|SUDO_ASKPASS|AI_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
  _emit_encoded "$env_vars" "ENV_VARS_B64"
...
impl ShellKind {
    ...
    pub fn detect() -> Self {
        match sentinel_ai_config::shell::detect_unix_shell_kind() {
            sentinel_ai_config::shell::UnixShellKind::Bash => Self::Bash,
            sentinel_ai_config::shell::UnixShellKind::Zsh => Self::Zsh,
        }
    }

    /// Resolved absolute path to the shell binary. Falls back from `$SHELL` →
    /// `which` → common dirs → `/bin/<name>`. Result is cached process-wide
    /// in `sentinel_ai_config::shell::unix_shell_path`. See that function for
    /// the full cascade. Returns `&'static str`.
    pub fn binary_path(&self) -> &'static str {
        let kind = match self {
            Self::Bash => sentinel_ai_config::shell::UnixShellKind::Bash,
            Self::Zsh => sentinel_ai_config::shell::UnixShellKind::Zsh,
        };
        sentinel_ai_config::shell::unix_shell_path(kind)
    }
    ...
    pub fn prepare_command(
        ...
        let wrapper = match self.shell {
            ShellKind::Bash => format!(
                ...
                // Re-export AI_AGENT=1 after snapshot eval so agent-definition
                ...
                "{dump_script} \
                 ...
                 builtin export AI_AGENT=1; \
                 builtin export PWD=\"$(builtin pwd)\"; \
                 builtin shopt -s expand_aliases 2>/dev/null; {sudo_inject}{search_inject}\
                 builtin printf '%s' \"${{2:-}}\"; \
                 __ai_user_cmd=\"$1\"; builtin declare +x __ai_user_cmd 2>/dev/null; builtin set --; \
                 builtin eval \"$__ai_user_cmd\" 2>&1; }}; \
                 COMMAND_EXIT_CODE=$?; builtin unset __ai_user_cmd 2>/dev/null; {dump_fn} >&4; builtin exit $COMMAND_EXIT_CODE"
            ),
            ...
            ShellKind::Zsh => format!(
                "{dump_script} \
                 ...
                 builtin printf '%s' \"${{2:-}}\"; \
                 __ai_user_cmd=\"$1\"; builtin typeset +x __ai_user_cmd 2>/dev/null; builtin set --; \
                 builtin eval \"$__ai_user_cmd\" 2>&1; }}; \
                 COMMAND_EXIT_CODE=$?; builtin unset __ai_user_cmd 2>/dev/null; {dump_fn} >&4; builtin exit $COMMAND_EXIT_CODE"
            ),
        };
...
mod tests {
    ...
    fn shell_env_overrides_marks_agent_terminal() {
        ...
        assert_eq!(
            env.get(crate::util::AI_AGENT_ENV).map(String::as_str),
            Some(crate::util::AI_AGENT_ENV_VALUE)
        );
    ...
    fn parse_dump_valid_bash() {
        let raw = "__AI_BASH_STATE_START__\n\
                    /home/user/project\n\
                    export FOO=bar\n\
                    # end of bash state dump\n\
                    __AI_BASH_STATE_END__\n";
    ...
    fn parse_dump_valid_zsh() {
        let raw = "__AI_ZSH_STATE_START__\n\
                    /tmp\n\
                    typeset -x FOO=bar\n\
                    # end of zsh state dump\n\
                    __AI_ZSH_STATE_END__\n";
    ...
    fn parse_after_marker_found() {
        let output = "Welcome to Ubuntu\nMOTD line\n__AI_INIT_STATE_MARKER__\nactual data\n";
        let result = parse_after_marker(output, "__AI_INIT_STATE_MARKER__");
    ...
    fn shell_state_update_from_dump() {
        ...
        let dump = "__AI_BASH_STATE_START__\n\
                     /new/dir\n\
                     export X=1\n\
                     # end of bash state dump\n\
                     __AI_BASH_STATE_END__\n";
    ...
    async fn test_init_bash() {
        ...
        assert!(
            state.snapshot.contains("ai_snap_") || state.snapshot.is_empty(),
    ...
    async fn test_prepare_command_and_roundtrip() {
        ...
        // Run "export AI_TEST_VAR=hello" and capture the new state
        let prep = state
            .prepare_command(
                "export AI_TEST_VAR=hello",
                None,
    ...
    async fn test_gpg_tty_excluded_from_snapshot_bash() {
        ...
        let (code, _) = run_command(&mut state, "export GPG_TTY=/ai-sentinel-tty").await;
        assert_eq!(code, 0);

        let (code, stdout) = run_command(&mut state, "echo \"[$GPG_TTY]\"").await;
        assert_eq!(code, 0);
        assert!(
            !stdout.contains("/ai-sentinel-tty"),
    ...
    /// after the model runs `set -a`), the wrapper's `__ai_user_cmd` temp
    ...
    async fn test_user_cmd_var_not_exported_under_allexport_bash() {
        ...
        // This command's wrapper assigns __ai_user_cmd under allexport.
        ...
        let (code, stdout) = run_command(
            &mut state,
            "printenv __ai_user_cmd >/dev/null 2>&1 && echo LEAKED_TO_ENV || echo ENV_CLEAN",
        )
        .await;
        assert_eq!(code, 0);
        assert!(
