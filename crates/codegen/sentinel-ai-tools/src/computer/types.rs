...
/// (`sentinel-ai-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (sentinel-ai-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in sentinel-ai-tools, spawns processes)
/// - `AcpTerminalBackend` (in sentinel-ai-shell, calls ACP protocol)
#[async_trait::async_trait]
