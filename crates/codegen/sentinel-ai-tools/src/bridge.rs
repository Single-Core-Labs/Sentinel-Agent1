//! ToolBridge: adapter that wraps `sentinel-ai-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// `Some("get_task_output")` for the ai_build agent and `None` for
    ...
    pub async fn kill_background_task(
        &self,
        task_id: &str,
    ) -> Result<KillOutcome, sentinel_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
    ...
    pub async fn list_scheduled_tasks(
        &self,
    ) -> Vec<crate::implementations::ai_build::scheduler::types::ScheduledTask> {
        use crate::implementations::ai_build::scheduler::types::{
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, sentinel_tool_runtime::ToolError> {
        use crate::implementations::ai_build::scheduler::types::{
            SchedulerCommand, SchedulerHandle,
        };
        let sender = {
            ...
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    sentinel_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
        ...
        reply_rx
            ...
            .map_err(crate::implementations::ai_build::scheduler::types::scheduler_tool_error)
    }
...
mod tests {
    ...
    impl sentinel_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> sentinel_tool_protocol::ToolId {
            sentinel_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::sentinel_tool_runtime::ListToolsContext,
        ) -> sentinel_tool_types::ToolDescription {
            sentinel_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + ai_build's snake_case in one registry
