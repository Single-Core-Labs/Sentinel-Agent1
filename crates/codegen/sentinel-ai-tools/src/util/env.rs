...
pub use sentinel_tty_utils::{detach_from_tty, pager_env};
...
/// Note: the CLI also uses `AI_AGENT` as an
/// optional agent-definition selector for launching `ai` itself; child terminal
/// processes only need the sentinel value `"1"`.
pub const AI_AGENT_ENV: &str = "AI_AGENT";

/// Sentinel value for [`AI_AGENT_ENV`] on agent tool terminals.
pub const AI_AGENT_ENV_VALUE: &str = "1";

/// Force `AI_AGENT=1` on an agent terminal child so request/login env cannot
/// clear the agent marker.
pub fn apply_ai_agent_marker(cmd: &mut tokio::process::Command) {
    cmd.env(AI_AGENT_ENV, AI_AGENT_ENV_VALUE);
}

/// Expand the four plugin-path tokens (`${CLAUDE_PLUGIN_ROOT}` / `${AI_PLUGIN_ROOT}`
/// and `${CLAUDE_PLUGIN_DATA}` / `${AI_PLUGIN_DATA}`) in `s`. Each pair is expanded
...
mod tests {
    ...
    fn expands_only_root_when_data_none() {
        ...
        assert_eq!(
            out,
            "/root/a /root/b ${CLAUDE_PLUGIN_DATA}/c ${AI_PLUGIN_DATA}/d"
        );
    }

    #[test]
    fn agent_marker_constants_match_cursor_parity() {
        assert_eq!(AI_AGENT_ENV, "AI_AGENT");
        assert_eq!(AI_AGENT_ENV_VALUE, "1");
    }
}
