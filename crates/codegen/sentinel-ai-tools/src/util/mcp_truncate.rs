...
//!    [`ENV_AI_MAX_MCP_OUTPUT_BYTES`] / [`ENV_MAX_MCP_OUTPUT_BYTES`]
//! 4. Built-in default
...
/// Ai-native env override for the MCP inline output cap (bytes).
pub const ENV_AI_MAX_MCP_OUTPUT_BYTES: &str = "AI_MAX_MCP_OUTPUT_BYTES";
...
/// Env tier: `AI_MAX_MCP_OUTPUT_BYTES` then `MAX_MCP_OUTPUT_BYTES`.
///
/// Ai-native wins when both are set. Positive integers only. Used by the
...
pub fn mcp_max_output_bytes_from_env() -> Option<usize> {
    parse_positive_bytes_env(ENV_AI_MAX_MCP_OUTPUT_BYTES)
...
mod tests {
    ...
    fn env_parser_rejects_zero_and_junk() {
        with_mcp_limit_lock(|| {
            ...
            // AI_* wins over MAX_* when both set.
            unsafe { std::env::set_var(ENV_AI_MAX_MCP_OUTPUT_BYTES, "99999") };
