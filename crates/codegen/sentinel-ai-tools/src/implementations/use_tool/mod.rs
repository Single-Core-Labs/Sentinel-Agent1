...
crate::register_resource!("ai_build", "UseTool", UseToolParams);
...
fn is_local_tool_id_rejection(err: &sentinel_tool_runtime::ToolError, tool_name: &str) -> bool {
    err.kind == sentinel_tool_runtime::ToolErrorKind::InvalidArguments
    ...
    if let Some(source) = gateway_source {
            ...
            && let Some(dispatch) = dispatch.clone()
        {
            match dispatch_local_mcp(dispatch, tool_name, tool_input.clone(), ctx.clone()).await {
                ...
                Err(err)
                    if err.kind != sentinel_tool_runtime::ToolErrorKind::NotFound
...
impl sentinel_tool_runtime::Tool for UseTool {
    type Args = UseToolInput;
    type Output = ToolOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("use_tool").expect("valid tool id")
    }
...
mod tests {
    ...
    impl sentinel_tool_runtime::ToolDispatch for MockToolDispatch {
        async fn call(
            &self,
    ...
    #[async_trait::async_trait]
    impl sentinel_tool_runtime::ToolDispatch for CapturingDispatch {
        async fn call(
            &self,
            tool_id: sentinel_tool_protocol::ToolId,
            ...
            if !matches!(
                ...
                return sentinel_tool_runtime::terminal_only(Err(
                    sentinel_tool_runtime::ToolError::not_found(tool_id, "Tool not found"),
                ));
            }
    ...
    fn ctx_capturing() -> (sentinel_tool_runtime::ToolCallContext, SharedArgs) {
    ...
    #[async_trait::async_trait]
    impl sentinel_tool_runtime::ToolDispatch for NotFoundDispatch {
        async fn call(
            &self,
            tool_id: sentinel_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            sentinel_tool_runtime::terminal_only(Err(sentinel_tool_runtime::ToolError::not_found(
                tool_id,
                "Tool not found",
            )))
        }
    ...
    #[async_trait::async_trait]
    impl sentinel_tool_runtime::ToolDispatch for InvalidArgumentsDispatch {
        async fn call(
            &self,
            _tool_id: sentinel_tool_protocol::ToolId,
            _args: serde_json::Value,
            _ctx: sentinel_tool_runtime::ToolCallContext,
        ) -> sentinel_tool_runtime::ToolStream<sentinel_tool_runtime::TypedToolOutput> {
    ...
    #[async_trait::async_trait]
    impl sentinel_tool_runtime::ToolDispatch for ErrorToolDispatch {
        async fn call(
            &self,
            _tool_id: sentinel_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            let tid = sentinel_tool_protocol::ToolId::new(&self.error)
    ...
    fn new_ctx() -> sentinel_tool_runtime::ToolCallContext {
        let call_id = sentinel_tool_protocol::ToolCallId::new_v7();
        sentinel_tool_runtime::ToolCallContext::new(call_id)
    }

    fn ctx_with_dispatch(
        dispatch: impl sentinel_tool_runtime::ToolDispatch + 'static,
    ...
    impl crate::types::resources::ManagedGatewayToolCaller for MockGatewayCaller {
        async fn call_tool(
            ...
            sentinel_tool_runtime::ToolError,
        > {
    ...
    fn ctx_capturing_with_resources(
        resources: crate::types::resources::SharedResources,
    ) -> (sentinel_tool_runtime::ToolCallContext, SharedArgs) {
        let args: SharedArgs = Arc::new(std::sync::Mutex::new(None));
        let mut ctx = new_ctx();
        ctx.extensions
