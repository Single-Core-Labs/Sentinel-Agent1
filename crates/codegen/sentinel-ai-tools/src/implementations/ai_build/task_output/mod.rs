...
use crate::implementations::ai_build::task::TaskTool;
use crate::implementations::ai_build::task::backend::SubagentBackendResource;
use crate::implementations::ai_build::task::types::{SubagentSnapshot, SubagentSnapshotStatus};
use crate::implementations::ai_build_concise::BashConciseTool;
...
/// The blocking-wait ceiling: `AI_MAX_WAIT_BLOCK_MS`, else 10 min.
///
...
pub(crate) fn max_wait_block() -> Duration {
    Duration::from_millis(sentinel_tool_types::max_wait_block_ms())
}
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let ai_build_concise_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashConciseTool).to_string(),
        id: sentinel_tool_runtime::Tool::id(&BashConciseTool)
            .as_str()
            .to_string(),
        if_params: Some(Expr::Value(ToolParamsRequirement {
            key: "enabled_background".to_string(),
    ...
    let opencode_bash = Expr::Value(ToolRequirement::Tool {
        ...
        id: sentinel_tool_runtime::Tool::id(&OpenCodeBashTool)
            .as_str()
            .to_string(),
        if_params: None,
    });
    vec![ai_build_bash, ai_build_concise_bash, opencode_bash]
}
...
impl TaskOutputTool {
    async fn run_single_task(
    ...
    ) -> Result<TaskOutputOutput, sentinel_tool_runtime::ToolError> {
        let contract_version = ctx
            .extensions
            .get::<sentinel_tool_runtime::BehaviorVersion>()
            .map(|v| v.0.clone());
        ...
        let waits = sentinel_tool_types::task_output_waits(timeout_ms);
...
pub(crate) use sentinel_tool_types::MAX_MULTI_WAIT_IDS;
...
fn format_subagent_snapshot(snap: &SubagentSnapshot, wait_hint: WaitHint) -> TaskOutputOutput {
    ...
    match &snap.status {
        ...
        SubagentSnapshotStatus::Completed {
            ...
            output.push_str("\n\n");
            output.push_str(&sentinel_tool_types::format_resume_footer(
                &snap.subagent_id,
                &snap.subagent_type,
...
/// [`sentinel_tool_types::build_task_output_description`] builder so the CLI and
...
impl sentinel_tool_runtime::Tool for TaskOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("get_task_output").expect("valid tool id")
    }
...
mod tests {
    ...
    fn capped_wait_timeout_clamps_and_defaults() {
        let cap = Duration::from_millis(sentinel_tool_types::MAX_WAIT_BLOCK_MS_DEFAULT);
    ...
    fn tool_name_and_description() {
        ...
        // The static fallback is the shared builder's default ai-build
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::ai_build::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::ai_build::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::ai_build::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::ai_build::task::types::SubagentEvent,
    ) -> crate::implementations::ai_build::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::ai_build::task::types::SubagentEvent::Query(q) => q,
