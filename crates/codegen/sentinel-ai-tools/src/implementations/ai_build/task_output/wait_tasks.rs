...
use crate::implementations::ai_build::task_output::{
...
use sentinel_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
/// [`sentinel_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl sentinel_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout = crate::implementations::ai_build::task_output::capped_wait_timeout(
            input.timeout_ms,
            crate::implementations::ai_build::task_output::max_wait_block(),
        );
