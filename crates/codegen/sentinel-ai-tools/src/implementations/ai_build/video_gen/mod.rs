...
//! the AI Video Generation API and save them to the local filesystem so the
...
const AI_VIDEO_BASE_MODEL: &str = "ai-imagine-video";
const AI_VIDEO_QUALITY_MODEL: &str = "ai-imagine-video-1.5-preview";
...
const DEFAULT_ZDR_VIDEO_KEY_PREFIX: &str = "ai-videos/";
...
pub use sentinel_ai_tools_api::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> sentinel_file_utils::s3::S3StaticCredentials {
        sentinel_file_utils::s3::S3StaticCredentials {
...
/// HTTP client for AI Video Generation API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
pub struct VideoGenClient {
    ...
    /// and return the SuperAi upsell prose. See [`VideoGenClient::is_tier_restricted`].
...
impl VideoGenClient {
    pub fn new(
        ...
        api_key_provider: Option<SharedApiKeyProvider>,
    ) -> Result<Self, sentinel_tool_runtime::ToolError> {
        ...
        extra_headers.into_iter().try_for_each(|(key, value)| {
            ...
            Ok::<(), sentinel_tool_runtime::ToolError>(())
        })?;

        let http = sentinel_ai_extra_ca::with_extra_root_certificates(
        ...
        let download_http = sentinel_ai_extra_ca::with_extra_root_certificates(
    ...
    /// SuperAi upsell instead of issuing a doomed request.
    ...
    pub async fn generate_with_images(
    ...
    ) -> Result<VideoOutcome, sentinel_tool_runtime::ToolError> {
        ...
        if !status.is_success() {
            ...
            return Err(sentinel_tool_runtime::ToolError::new(
                sentinel_tool_runtime::ToolErrorKind::Custom,
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, sentinel_tool_runtime::ToolError> {
        ...
        let upload_url = sentinel_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, sentinel_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = sentinel_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
pub enum VideoGenConfig {
    ...
    Enabled {
        ...
        /// at call time with the SuperAi upsell prose. Set by the host from
...
/// the deliberate `/imagine-video` slash command shows the SuperAi upsell
/// modal instead.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Video generation is a SuperAi feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperAi: https://ai.com/superai?referrer=ai-build. Do not retry this tool.";
...
async fn resolve_image_reference(value: &str) -> Result<String, sentinel_tool_runtime::ToolError> {
...
) -> Result<(VideoGenClient, std::path::PathBuf), sentinel_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, sentinel_tool_runtime::ToolError> {
...
) -> Result<MediaGenOutput, sentinel_tool_runtime::ToolError> {
    match outcome {
...
impl sentinel_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
    ...
    fn capabilities(&self) -> sentinel_tool_protocol::ToolCapabilities {
        sentinel_tool_protocol::ToolCapabilities {
            is_read_only: false,
...
impl sentinel_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
