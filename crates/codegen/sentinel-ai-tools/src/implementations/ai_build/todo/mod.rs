...
crate::register_resource!("ai_build", "Todo", TodoState);
...
impl sentinel_tool_runtime::Tool for TodoWriteTool {
    ...
    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("todo_write").expect("valid tool id")
    }
...
mod tests {
    ...
    async fn state_serialization_roundtrip() {
        ...
        assert!(
            state_map.get("ai_build.Todo").is_some(),
            "TodoState should serialize under 'ai_build.Todo'"
        );
