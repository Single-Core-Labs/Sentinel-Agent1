...
impl SubagentLimits {
    ...
    fn from_lookup(lookup: impl Fn(&str) -> Option<String>) -> Self {
        ...
        let max_concurrent = parse_positive_env(
            "AI_MAX_CONCURRENT_SUBAGENTS",
            lookup("AI_MAX_CONCURRENT_SUBAGENTS"),
        )
        .unwrap_or(default.max_concurrent);
        let behavior = match lookup("AI_SUBAGENT_LIMIT_BEHAVIOR") {
            ...
            Some(value) => {
                tracing::warn!(
                    %value,
                    "AI_SUBAGENT_LIMIT_BEHAVIOR is neither `queue` nor `fail`; keeping `queue`"
                );
                LimitBehavior::Queue
            }
        };
...
impl AdmissionError {
    ...
    /// into `AI_SUBAGENT_LIMIT_BEHAVIOR=fail`.
