...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"ai-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`sentinel_tool_types::is_not_sentinel`] for existing call sites.
pub use sentinel_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `sentinel-ai-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "AiBuild:run_terminal_cmd" | "AiBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
register_resource!("ai_build", "SubagentEventSender", SubagentEventSender);
...
register_resource!("ai_build", "SubagentDepthCounter", SubagentDepthCounter);

/// Host-injected max nesting depth; absent → [`super::MAX_SUBAGENT_DEPTH`].
#[derive(Debug, Clone, Copy)]
pub struct MaxSubagentDepth(pub u32);

register_resource!("ai_build", "MaxSubagentDepth", MaxSubagentDepth);
...
register_resource!("ai_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("ai_build", "SessionIdResource", SessionIdResource);
...
///
/// Set by sentinel-ai-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by sentinel-ai-shell at turn start.
...
register_resource!("ai_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("AiBuild:run_terminal_cmd", ToolKind::Execute),
                tc("AiBuild:read_file", ToolKind::Read),
                tc("AiBuild:list_dir", ToolKind::List),
                tc("AiBuild:grep", ToolKind::Search),
                tc("AiBuild:kill_task", ToolKind::KillTaskAction),
                tc("AiBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("AiBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
