...
use regex::Regex;
use sentinel_tool_types::{SubagentCompletedOutput, SubagentIsolationMode, TaskToolInput};
...
async fn detect_continue_parent_work(
    ...
    sentinel_tool_types::should_continue_parent_work(&asks, description, prompt)
}
...
impl crate::types::tool_metadata::ToolMetadata for TaskTool {
    ...
    fn description_template(&self) -> &str {
        // Ai Build normally supplies the description via
        // `ToolConfig::with_description(...)` using `build_task_description()`
        // in sentinel-ai-agent/src/builder.rs (live subagent roster). But a
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            let subagents: Vec<sentinel_tool_types::SubagentDescriptor> =
                sentinel_tool_types::BUILTIN_SUBAGENTS
                    .iter()
                    .map(|b| sentinel_tool_types::SubagentDescriptor {
                        name: b.name.to_owned(),
                        description: b.description.to_owned(),
                        tools: Some(guard_kind_tokens(b.tools_template)),
                    })
                    .collect();
            sentinel_tool_types::build_task_description(
                &subagents,
...
impl sentinel_tool_runtime::Tool for TaskTool {
    type Args = TaskToolInput;
    type Output = ToolOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("task").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let tool_cancellation = ctx
            .get::<sentinel_tool_runtime::Cancellation>()
        ...
        let model = sentinel_tool_types::sanitize_optional_arg(input.model);
            ...
            && resume_from.is_none()
        {
            ...
            if !p.is_dir() {
                ...
                return Err(sentinel_tool_runtime::ToolError::invalid_arguments(detail));
            }
        }
        ...
        if let Some(ref requested) = model {
            ...
            if let Some(error) = validator.error_for(requested) {
                return Err(sentinel_tool_runtime::ToolError::invalid_arguments(error));
            }
        }
        ...
        if input.run_in_background {
            ...
            return Ok(ToolOutput::Text(
                sentinel_tool_types::format_subagent_started_background(
                    &id,
                    &input.subagent_type,
                    &input.description,
                    &task_output_name,
        ...
        if result.backgrounded {
            ...
            if detect_continue_parent_work(&resources, &input.description, &input.prompt).await {
                text.push_str("\n\n");
                text.push_str(sentinel_tool_types::BACKGROUND_SUBAGENT_CONTINUE_PARENT_WORK);
            }
...
mod tests {
    ...
    async fn auto_backgrounded_result_returns_task_id_text() {
        ...
        match result {
            ToolOutput::Text(text) => {
                ...
                assert!(
                    text.text
                        .contains(sentinel_tool_types::BACKGROUND_SUBAGENT_CONTINUE_PARENT_WORK),
    ...
    async fn model_threads_to_runtime_overrides() {
        ...
        let result = sentinel_tool_runtime::Tool::run(&TaskTool, test_ctx(shared), input)
            .await
            .unwrap();
        handle.await.unwrap();
        match result {
    ...
    async fn model_whitespace_is_trimmed() {
        ...
        let mut input = task_input("general-purpose", false);
        input.model = Some("  test-model  ".into());
        let _ = sentinel_tool_runtime::Tool::run(&TaskTool, test_ctx(shared), input)
            .await
            .unwrap();
        handle.await.unwrap();
    }
