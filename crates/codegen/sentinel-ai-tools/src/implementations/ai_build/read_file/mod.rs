...
mod versions;
use crate::types::schema::AiIntegerSchema;
...
crate::register_resource!("ai_build", "ReadFile", ReadFileParams);
...
/// `PlainText` / `Append`.
static READ_FILE_CAPABILITIES: LazyLock<sentinel_tool_protocol::ToolCapabilities> =
...
async fn handle_pptx(
    file_bytes: Vec<u8>,
    path: &std::path::Path,
) -> Result<ReadFileOutput, sentinel_tool_runtime::ToolError> {
...
/// an empty window (harness-aligned; not a Ai-line clamp).
...
impl sentinel_tool_runtime::Tool for ReadFileTool {
    type Args = ReadFileInput;
    type Output = ReadFileOutput;
    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("read_file").expect("valid tool id")
    }
    fn description(
        &self,
    ...
    async fn execute(
        ...
        input: ReadFileInput,
    ) -> sentinel_tool_runtime::ToolStream<ReadFileOutput> {
        ...
        Box::pin(async_stream::stream! {
            ...
            match ReadFileTool::read_with_streamability(&ctx, input).await {
                Ok((output, streamable)) => {
                        ...
                        && !fc.content.is_empty()
                    {
                        ...
                        while window_start < content.len() {
                            ...
                            if let Some(p) = sentinel_tool_runtime::stream_chunk(
                                spec,
                                &content[..window_end],
                                window_end as u64,
                                &mut last_total,
                ...
                Err(e) => yield sentinel_tool_runtime::ToolStreamItem::Terminal(Err(e)),
            }
        })
    }
...
impl ReadFileTool {
    ...
    async fn read_with_streamability(
        ctx: &sentinel_tool_runtime::ToolCallContext,
        input: ReadFileInput,
    ) -> Result<(ReadFileOutput, bool), sentinel_tool_runtime::ToolError> {
        let resources = crate::types::tool_metadata::shared_resources(ctx)?;
        let cwd_override = ctx
            .extensions
            .get::<sentinel_tool_runtime::Cwd>()
            .map(|c| c.0.clone());
...
mod tests {
    ...
    async fn read_file_basic() {
        ...
        let result = sentinel_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
            .await
            .unwrap();
        match result {
    ...
    async fn missing_skill_read_suggests_registered_path() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".ai/skills/code-review");
    ...
    async fn missing_skill_read_uses_display_path() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".ai/skills/review");
        std::fs::create_dir_all(&skill_dir).unwrap();
        let skill_path = skill_dir.join("SKILL.md");
        ...
        assert_eq!(
            ...
            "Error: /wrong/root/review/SKILL.md does not exist.\n\
             The skill you are looking for is registered at:\n\
             /display/project/.ai/skills/review/SKILL.md"
        );
    }
    #[tokio::test]
    ...
    async fn missing_skill_read_omits_stale_registered_path() {
        let tmp = TempDir::new().unwrap();
        let stale_path = tmp.path().join(".ai/skills/review/SKILL.md");
        let mut resources = test_resources(tmp.path());
        resources.insert(PathNotFoundHints(true));
        resources.insert(seeded_manager(vec![SkillInfo {
            name: "review".to_owned(),
...
";
    ...
    async fn skill_file_ignores_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".ai/skills/commit");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".ai/skills/commit/SKILL.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn md_in_skills_dir_ignores_model_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".ai/skills/my-skill");
        std::fs::create_dir_all(&skill_dir).unwrap();
        let content = (1..=1200)
            .map(|n| format!("line{n}"))
            .collect::<Vec<_>>()
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".ai/skills/my-skill/reference.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn execute_collect(
        ...
        let mut stream = sentinel_tool_runtime::Tool::execute(&ReadFileTool, ctx, input).await;
    ...
    /// phantom field only (start past any `split_inclusive` line), so Ai
