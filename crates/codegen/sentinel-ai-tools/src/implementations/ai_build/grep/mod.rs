...
// Re-export the shared AiIntegerSchema from types module
pub use crate::types::AiIntegerSchema;
use ripgrep::rg_path;
...
crate::register_resource!("ai_build", "Grep", GrepParams);
...
fn grep_timeout() -> Duration {
    Duration::from_secs(grep_timeout_secs(sentinel_tty_utils::is_wsl()))
}
...
static GREP_CAPABILITIES: LazyLock<sentinel_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| sentinel_tool_protocol::ToolCapabilities {
        is_read_only: true,
        tool_scope: Some(sentinel_tool_protocol::ToolScope::Read),
...
impl sentinel_tool_runtime::Tool for GrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("grep").expect("valid tool id")
    }
    ...
    async fn execute(
    ...
    ) -> sentinel_tool_runtime::ToolStream<GrepSearchOutput> {
        ...
        let Some(spec) = admitted_spec else {
            return sentinel_tool_runtime::terminal_only(self.run(ctx, input).await);
        };
    ...
    async fn run(
        ...
        ctx: sentinel_tool_runtime::ToolCallContext,
        input: GrepSearchInput,
    ) -> Result<GrepSearchOutput, sentinel_tool_runtime::ToolError> {
...
fn grep_progress_stream(
    ctx: sentinel_tool_runtime::ToolCallContext,
    input: GrepSearchInput,
    spec: &'static sentinel_tool_protocol::StreamingSpec,
    span: tracing::Span,
) -> sentinel_tool_runtime::ToolStream<GrepSearchOutput> {
    Box::pin(async_stream::stream! {
        ...
        let GrepReady {
            ...
            Ok(GrepStep::Early(out)) => {
                ...
                yield sentinel_tool_runtime::ToolStreamItem::Terminal(Ok(out));
                return;
            }
            Err(e) => {
        ...
        if timed_out {
            ...
            if stdout_buf.is_empty() {
                yield sentinel_tool_runtime::ToolStreamItem::Terminal(Ok(grep_timeout_output(secs)));
            } else {
                ...
                output.exit_code = -1;
                yield sentinel_tool_runtime::ToolStreamItem::Terminal(Ok(output));
            }
            return;
        }
...
async fn prepare_grep(
    ctx: &sentinel_tool_runtime::ToolCallContext,
    input: &GrepSearchInput,
) -> Result<GrepStep, sentinel_tool_runtime::ToolError> {
...
struct BodyStreamer<'a> {
    spec: &'a sentinel_tool_protocol::StreamingSpec,
...
impl<'a> BodyStreamer<'a> {
    fn new(spec: &'a sentinel_tool_protocol::StreamingSpec, config: &'a GrepFormatConfig) -> Self {
        Self {
            spec,
            config,
            body: String::new(),
    ...
    fn feed(&mut self, bytes: &[u8]) -> Vec<sentinel_tool_runtime::ToolProgress> {
    ...
    fn finish(&mut self) -> Option<sentinel_tool_runtime::ToolProgress> {
    ...
    fn push_line(&mut self, line: &[u8]) -> Option<sentinel_tool_runtime::ToolProgress> {
        ...
        sentinel_tool_runtime::stream_chunk(
            self.spec,
            self.body.as_bytes(),
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        let tool = GrepTool;
        assert_eq!(sentinel_tool_runtime::Tool::id(&tool).as_str(), "grep");
    ...
    fn read_grep_delta(p: &sentinel_tool_runtime::ToolProgress) -> String {
        match p {
    ...
    async fn grep_streaming_suppressed_when_gate_off() {
        ...
        let tool = GrepTool;
        let mut stream =
            sentinel_tool_runtime::Tool::execute(&tool, ctx, make_grep_input("findme")).await;

        let mut progress = 0usize;
        let mut terminal: Option<Result<GrepSearchOutput, sentinel_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                sentinel_tool_runtime::ToolStreamItem::Progress(_) => progress += 1,
                sentinel_tool_runtime::ToolStreamItem::Terminal(r) => {
                    assert!(terminal.is_none(), "more than one Terminal yielded");
                    terminal = Some(r);
                }
