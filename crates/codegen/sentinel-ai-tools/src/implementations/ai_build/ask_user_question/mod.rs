...
/// params are injected, `AI_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `AI_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "AI_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("ai_build", "AskUserQuestion", AskUserQuestionParams);
...
pub struct QuestionOption {
    ...
    /// Opaque id; hidden from the model. Ai callers leave it `None`.
...
/// session-owned coordinator (in sentinel-ai-shell), which performs an ACP
...
impl AskUserQuestionTool {
    ...
    ) -> Result<AskUserQuestionOutput, sentinel_tool_runtime::ToolError> {
...
impl sentinel_tool_runtime::Tool for AskUserQuestionTool {
    type Args = AskUserQuestionInput;
    type Output = AskUserQuestionOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("ask_user_question").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        if sender.0.send(request).is_err() {
            return Err(sentinel_tool_runtime::ToolError::execution(
                sentinel_tool_protocol::ToolId::new("ask_user_question").expect("valid"),
        ...
        match result {
            ...
            Err(UserQuestionError::TransportError(msg)) => {
                Err(sentinel_tool_runtime::ToolError::execution(
                    sentinel_tool_protocol::ToolId::new("ask_user_question").expect("valid"),
...
mod tests {
    ...
    fn tool_is_read_only() {
        assert!(sentinel_tool_runtime::Tool::capabilities(&AskUserQuestionTool).is_read_only);
    }
    ...
    async fn fallback_sends_notification() {
        ...
        sentinel_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "call-q"), input)
            .await
            .unwrap();
