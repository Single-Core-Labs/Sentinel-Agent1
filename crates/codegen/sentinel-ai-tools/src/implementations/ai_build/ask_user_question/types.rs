...
//!
//! - **`sentinel-ai-tools`** — tool blocks on a oneshot, formats the result.
//! - **`sentinel-ai-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`sentinel-ai-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `sentinel-ai-tools`.
...
register_resource!("ai_build", "UserQuestionSender", UserQuestionSender);
