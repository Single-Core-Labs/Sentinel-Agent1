...
impl sentinel_tool_runtime::ToolOutput for SchedulerCreateOutput {}
...
impl sentinel_tool_runtime::Tool for SchedulerCreateTool {
    type Args = SchedulerCreateInput;
    type Output = SchedulerCreateOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new(SCHEDULER_CREATE_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<SchedulerCreateOutput, sentinel_tool_runtime::ToolError> {
...
mod tests {
    use super::*;
    use crate::implementations::ai_build::scheduler::actor::SchedulerActor;
