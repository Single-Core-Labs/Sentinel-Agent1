...
//! `LspTool` (Tool trait impl) under the `AiBuild` namespace.
...
impl sentinel_tool_runtime::ToolOutput for LspToolOutput {}
...
impl sentinel_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, sentinel_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    sentinel_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.ai/lsp.json or <cwd>/.ai/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
