//! `image_edit` tool — edits or transforms images via the AI Imagine
...
use crate::implementations::ai_build::image_gen::{ImageGenClient, ImageGenResponse};
...
pub(crate) const AI_IMAGINE_EDIT_MODEL: &str = "ai-imagine-image-quality";
...
fn compress_reference(
    raw_bytes: Vec<u8>,
) -> Result<(Vec<u8>, &'static str), sentinel_tool_runtime::ToolError> {
    ...
    let img = ImageReader::new(Cursor::new(&raw_bytes))
        ...
        .ok_or_else(|| {
            sentinel_tool_runtime::ToolError::invalid_arguments("failed to decode image reference")
        })?;
...
async fn resolve_to_data_url(value: &str) -> Result<String, sentinel_tool_runtime::ToolError> {
...
impl sentinel_tool_runtime::Tool for ImageEditTool {
    type Args = ImageEditInput;
    type Output = ToolOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("image_edit").expect("valid tool id")
    }
