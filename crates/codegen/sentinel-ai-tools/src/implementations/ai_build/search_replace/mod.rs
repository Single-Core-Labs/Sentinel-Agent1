...
register_resource!("ai_build", "SearchReplace", SearchReplaceParams);
            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(sentinel_tool_runtime::ToolError::execution(
                        sentinel_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl sentinel_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = sentinel_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = sentinel_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),
