//! LSP server configuration from `.ai/lsp.json`.
...
/// This is the canonical merge function — both session startup and `ai inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::ai_home::ai_home().join("lsp.json");
    let project_path = cwd.join(".ai").join("lsp.json");
...
/// Load LSP server configs from `~/.ai/lsp.json` and `<cwd>/.ai/lsp.json`.
