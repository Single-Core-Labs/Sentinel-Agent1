...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.ai/skills/ or .ai/skills/)
${%- endif %}
</available_skills>"#;
...
impl sentinel_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, sentinel_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(sentinel_tool_runtime::Tool::id(&tool).as_str(), "skill");
