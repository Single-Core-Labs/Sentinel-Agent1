...
use crate::implementations::ai_build::search_replace::CONTEXT_LINES;
use crate::implementations::ai_build::search_replace::helpers::{
...
// the snake_case `params.edit.old_string` keys of the ai_build twin resolve
...
impl sentinel_tool_runtime::Tool for EditTool {
    type Args = EditInput;
    type Output = SearchReplaceOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("edit").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: EditInput,
    ) -> Result<SearchReplaceOutput, sentinel_tool_runtime::ToolError> {
        ...
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            sentinel_tool_runtime::ToolError::execution(
                sentinel_tool_protocol::ToolId::new("edit").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
// are imported from `ai_build::search_replace::helpers` — shared across
// both the ai_build and opencode edit tools.
...
mod tests {
    ...
    fn tool_id_and_kind() {
        ...
        let tool = EditTool;
        assert_eq!(sentinel_tool_runtime::Tool::id(&tool).as_str(), "edit");
