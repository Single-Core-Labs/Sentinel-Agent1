...
//! ai-tools-server serves this tool on the stateless local backend
...
impl sentinel_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(sentinel_tool_runtime::ToolError::execution(
                    sentinel_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = sentinel_tool_protocol::ToolCallId::new(call_id).unwrap();
    ...
    async fn tool_metadata() {
        use crate::types::tool_metadata::ToolMetadata;
        let tool = BashTool;
        assert_eq!(sentinel_tool_runtime::Tool::id(&tool).as_str(), "bash");
    ...
    //
    // Same shape as ai_build/bash: the opencode tool inherits the same
