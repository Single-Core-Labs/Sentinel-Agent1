...
impl sentinel_tool_runtime::Tool for BashConciseTool {
    ...
    async fn run(
        ...
        let result = sentinel_tool_runtime::Tool::run(&BashTool, ctx, input).await?;
...
mod tests {
    ...
    fn no_double_header_after_default_prebake() {
        ...
        bash.output_for_prompt =
            crate::implementations::ai_build::bash::format_default_prompt(&bash);
