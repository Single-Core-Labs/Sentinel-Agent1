...
use crate::implementations::ai_build::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl sentinel_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = sentinel_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(sentinel_tool_runtime::Cwd(cwd));
        let mut result = sentinel_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                sentinel_tool_runtime::ToolError::execution(
                    sentinel_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
