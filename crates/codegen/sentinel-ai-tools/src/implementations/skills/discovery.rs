...
///
/// `skills` is the standard layout (`.ai/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Ai Build sessions.
...
/// name (e.g. `~/.ai/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Ai Build.
...
/// `cwd` (exclusive). At each directory, checks for `.ai/skills/`,
...
/// visits known config roots (`.ai`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.ai` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".ai", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.ai is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.ai/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.ai/ → kept (user content).
        let ai_shell = tmp.path().join(".ai").join("skills").join("shell");
        std::fs::create_dir_all(&ai_shell).unwrap();
        std::fs::write(
            ai_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (ai_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.ai/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .ai skill in an intermediate dir.
        ...
        .unwrap();
        let ai_skill = sub.join(".ai").join("skills").join("ai-dyn");
        std::fs::create_dir_all(&ai_skill).unwrap();
        std::fs::write(ai_skill.join("SKILL.md"), "---\nname: ai-dyn\n---\n").unwrap();
        ...
        // claude.skills OFF → only ai-dyn discovered.
        ...
        assert!(
            names_off.contains(&"ai-dyn"),
            "ai-dyn missing: {names_off:?}"
        );
        assert!(
