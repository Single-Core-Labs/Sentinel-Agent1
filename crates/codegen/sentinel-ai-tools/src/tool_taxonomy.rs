...
//! `ToolInput`, proto, or runtime). A future `sentinel-tool-taxonomy` leaf crate
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::AiBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::AiBuild => ("ai_build", "AiBuild"),
                ToolNamespace::AiBuildConcise => ("ai_build_concise", "AiBuildConcise"),
                ToolNamespace::AiBuildHashline => ("ai_build_hashline", "AiBuildHashline"),
    ...
    fn canonical_meta_wire_shape_round_trips() {
        ...
        assert_eq!(t["namespace"], "ai_build");
