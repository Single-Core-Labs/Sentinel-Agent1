...
crate::register_resource!("ai_build", "TaskWakeSuppressed", TaskWakeSuppressed);
...
pub fn format_monitor_events(
    events: &[crate::implementations::ai_build::monitor::types::MonitorEventNotification],
    task_output_name: Option<&str>,
) -> Option<String> {
    use std::fmt::Write as _;
...
/// scope only when it has no recorded owner (legacy / non-ai-build backends)
...
/// - `sentinel-ai-shell`'s `SessionActor` to sweep matching synthetic
    ...
    async fn collect_reminders(
        ...
        if let Some(terminal) = terminal {
            ...
            let goal_loop_active = res
                .get::<crate::implementations::ai_build::task::types::GoalLoopActive>()
                .is_some_and(|g| g.0);
            let surface_reminders = !goal_loop_active
                && res
                    .get::<crate::types::resources::Params<
                        crate::implementations::ai_build::bash::BashParams,
                    >>()
...
mod tests {
    ...
    fn shared_with_reminders_disabled(tasks: Vec<TaskSnapshot>) -> SharedResources {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        let params = crate::implementations::ai_build::bash::BashParams {
    ...
    async fn completions_suppressed_when_goal_loop_active() {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        res.insert(crate::implementations::ai_build::task::types::GoalLoopActive(true));
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        res.insert(SubagentEventSender(tx));
        tokio::spawn(async move {
        ...
        shared
            .lock()
            .await
            .insert(crate::implementations::ai_build::task::types::GoalLoopActive(false));
        let second = reminder.collect_reminders(shared, &output).await;
        assert!(
            second.is_empty(),
    ...
    async fn reminder_pipeline_ignores_monitor_event_buffer() {
        use crate::implementations::ai_build::monitor::types::{
    ...
    fn format_monitor_events_single_vs_batched() {
        use crate::implementations::ai_build::monitor::types::MonitorEventNotification;
        ...
        let bare = crate::implementations::ai_build::monitor::types::MonitorEventNotification {
            task_id: "task-9".into(),
    ...
    fn wrap_monitor_event_round_trips_through_split() {
        use crate::implementations::ai_build::monitor::event::wrap_monitor_event;
