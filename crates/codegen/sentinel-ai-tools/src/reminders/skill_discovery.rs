...
//! after every tool call to check for SKILL.md files in `.ai/skills/`,
...
/// Directories that contain skill definitions (`.ai/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".ai", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.ai/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.ai/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.ai/) are outside the git root so the walk breaks early.
