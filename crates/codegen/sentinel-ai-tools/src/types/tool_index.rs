...
//!
//! `ToolSearchIndex` is defined in `sentinel-ai-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `sentinel-ai-shell`
...
/// in `sentinel-ai-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `sentinel-ai-shell` after MCP initialization.
#[derive(Clone)]
