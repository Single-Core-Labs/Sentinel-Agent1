...
/// `"namespace.Name"` (e.g., `"ai_build.ReadFile"`). The ID is used as
...
///
/// ```ignore
/// register_resource!("ai_build", "ReadFile", ReadHistory);
/// ```
///
/// This generates:
/// ```ignore
/// impl ResourceType for ReadHistory {
///     const ID: &'static str = "ai_build.ReadFile";
/// }
/// ```
#[macro_export]
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"ai_build.ReadFile"`).
    id: String,
...
/// tool falls back to `Cwd/.ai/plan.md`.
#[derive(Debug, Clone)]
...
pub const PLAN_FILE_RELATIVE_PATH: &str = ".ai/plan.md";
...
/// [`Cwd`]`/.ai/plan.md`, else the bare relative `.ai/plan.md`.
...
pub(crate) fn require_plan_file_path(
    res: &Resources,
) -> Result<(PathBuf, String), sentinel_tool_runtime::ToolError> {
...
/// Newtype wrapper for `Arc<dyn sentinel_tool_runtime::ToolDispatch>` so it can
...
#[derive(Clone)]
pub struct InnerDispatch(pub std::sync::Arc<dyn sentinel_tool_runtime::ToolDispatch>);
#[derive(Debug, Clone)]
...
pub trait ManagedGatewayToolCaller: Send + Sync {
    async fn call_tool(
        ...
        caller: &str,
    ) -> Result<ManagedGatewayToolCallResponse, sentinel_tool_runtime::ToolError>;
}
#[derive(Clone)]
...
/// `AI_SCHEDULER_BACKGROUND_LOOPS` env var, or the
...
///
/// Stamped onto [`sentinel_tool_runtime::ToolCallContext::extensions`] by
...
register_resource!("ai_build", "WebCitation", WebCitationCounter);
...
/// depending on `sentinel-ai-mcp` directly.  Follows the same pattern as
...
mod tests {
    ...
    register_resource!("ai_build", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
    ...
    register_resource!("ai_build", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("ai_build.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "ai_build.ReadFile".to_string(),
            serde_json::json!({"files_read": ["loaded.rs"]}),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "ai_build.Edit".to_string(),
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "ai_build.Edit").unwrap();
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "ai_build.Edit").is_none());
    }
    #[test]
