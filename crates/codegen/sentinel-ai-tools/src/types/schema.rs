...
///
/// Use with `#[schemars(with = "AiIntegerSchema")]` on `Option<usize>` fields.
pub struct AiIntegerSchema;
impl schemars::JsonSchema for AiIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "ai_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `sentinel-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use sentinel_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
