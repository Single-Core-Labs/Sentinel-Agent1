...
//!
//! `MemoryBackend` is defined in `sentinel-ai-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `sentinel-ai-tools`.
/// Mirrors `sentinel_ai_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "sentinel_memory";
