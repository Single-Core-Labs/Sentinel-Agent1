...
//!
//! The tool runtime contract (`Tool` trait) lives in `sentinel_tool_runtime`.
...
///
/// Serializes to snake_case (`ai_build`, `mcp`, …) for the
...
/// (e.g. `"AiBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "AiBuild")]
    AiBuild,
    #[serde(alias = "AiBuildConcise")]
    AiBuildConcise,
    #[serde(alias = "AiBuildHashline")]
    AiBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
