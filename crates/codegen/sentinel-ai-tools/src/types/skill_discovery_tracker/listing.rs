...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (ai build harness) ─────────────
...
///
/// `"/path/.ai/skills/my-skill/SKILL.md"` -> `"/path/.ai/skills/"`
...
/// Used by the budgeted (ai build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Ai build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the ai
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // ai-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: ai build harness (budgeted XML) ───────────
