...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.ai/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.ai/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.ai/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.ai/ or ~/.claude/.
    User { path: PathBuf },
