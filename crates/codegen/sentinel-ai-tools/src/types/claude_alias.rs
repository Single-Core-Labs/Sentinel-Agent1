//! Canonical external-settings tool name ↔ Ai tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`sentinel-ai-hooks`) needs the
//! Ai tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`sentinel-ai-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `ai` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Ai, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Ai [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Ai tool names this Claude tool maps to (empty when there is no direct
    /// Ai tool — the entry then only contributes a `kind`).
    ai: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, ai: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        ai,
...
const fn match_only(claude: &'static str, ai: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        ai,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (ai-build reads SKILL.md)
...
/// The Ai [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Ai tool names a Claude matcher term fires on.
pub fn ai_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.ai)
...
/// The Claude names that map to `ai_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(ai_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.ai.contains(&ai_name))
        .map(|t| t.claude)
}

/// Every distinct Ai name the table references, for the `sentinel-ai-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn ai_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.ai.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Ai name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.ai.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
