//! `ToolMetadata` — ai-tools-specific metadata for tools.
//!
//! Each tool implements two traits:
//! 1. `sentinel_tool_runtime::Tool` — typed Args/Output, `run()` with actual logic
...
//! ## Context helpers
//!
//! Tools access session state through `sentinel_tool_runtime::ToolCallContext`
...
/// Ai-tools-specific metadata trait.
///
/// Each tool struct implements this alongside `sentinel_tool_runtime::Tool`.
...
pub trait ToolMetadata: Send + Sync {
    ...
    /// Namespace grouping (AiBuild, Cursor, OpenCode, ...).
    ...
    /// Model-safe fallback description for `sentinel_tool_runtime::Tool::description()`
...
) -> Result<SharedResources, sentinel_tool_runtime::ToolError> {
    ctx.extensions
        .get::<SharedResources>()
        .map(|arc| (*arc).clone())
        .ok_or_else(|| {
...
) -> Result<PathBuf, sentinel_tool_runtime::ToolError> {
    if let Some(cwd) = ctx.extensions.get::<sentinel_tool_runtime::Cwd>() {
...
pub fn test_ctx(resources: SharedResources) -> sentinel_tool_runtime::ToolCallContext {
    ...
    ctx.extensions
        .insert(sentinel_tool_runtime::WorkspaceViewerContext {
...
pub fn test_ctx_with_call_id(
    ...
    let id = sentinel_tool_protocol::ToolCallId::new(call_id)
        .unwrap_or_else(|_| sentinel_tool_protocol::ToolCallId::new_v7());
    let mut ctx = sentinel_tool_runtime::ToolCallContext::new(id);
    ctx.extensions.insert(resources);
    ctx.extensions
        .insert(sentinel_tool_runtime::WorkspaceViewerContext {
            stream_tool_progress: true,
...
pub fn behavior_version(ctx: &sentinel_tool_runtime::ToolCallContext) -> Option<String> {
    ctx.extensions
