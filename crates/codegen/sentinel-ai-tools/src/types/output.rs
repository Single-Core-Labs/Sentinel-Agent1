...
use crate::implementations::ai_build::todo::{TodoItem, TodoState};
...
pub(crate) fn typed_tool_output_preserving_cco(
    ...
    source: &impl sentinel_tool_runtime::ToolOutput,
) -> sentinel_tool_runtime::TypedToolOutput {
...
/// protocol boundary in `sentinel-ai-shell`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the ai_build `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the ai_build backend.
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when AiBuild persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
impl sentinel_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<sentinel_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => sentinel_tool_runtime::ToolOutput::chat_completion_output(bash),
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::ai_build::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::ai_build::scheduler::create::SchedulerCreateOutput),
    SchedulerDelete(crate::implementations::ai_build::scheduler::delete::SchedulerDeleteOutput),
    SchedulerList(crate::implementations::ai_build::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::ai_build::update_goal::UpdateGoalOutput),
    Workflow(crate::implementations::ai_build::workflow::WorkflowToolOutput),
...
impl sentinel_tool_runtime::ToolOutput for ToolOutput {
    fn chat_completion_output(&self) -> Option<sentinel_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => sentinel_tool_runtime::ToolOutput::chat_completion_output(bash),
            _ => None,
...
impl sentinel_tool_runtime::ToolOutput for BashOutput {
    fn chat_completion_output(&self) -> Option<sentinel_tool_runtime::ToolChatCompletionResponse> {
        ...
        Some(sentinel_tool_runtime::ToolChatCompletionResponse {
            result: Some(sentinel_tool_runtime::ToolChatCompletion {
                sender: "assistant".into(),
                message_tag: Some("raw_function_result".into()),
                code_execution_result: Some(sentinel_tool_runtime::ToolCodeExecutionResult {
                    stdout,
                    stderr: String::new(),
...
impl sentinel_tool_runtime::ToolOutput for GrepSearchOutput {}
impl sentinel_tool_runtime::ToolOutput for ReadFileOutput {}
impl sentinel_tool_runtime::ToolOutput for ListDirOutput {}
impl sentinel_tool_runtime::ToolOutput for SearchReplaceOutput {}
impl sentinel_tool_runtime::ToolOutput for TodoWriteOutput {}
impl sentinel_tool_runtime::ToolOutput for WebSearchOutput {}
impl sentinel_tool_runtime::ToolOutput for WebFetchOutput {}
impl sentinel_tool_runtime::ToolOutput for SkillOutput {}
impl sentinel_tool_runtime::ToolOutput for ApplyPatchOutput {}
impl sentinel_tool_runtime::ToolOutput for CodexGrepFilesOutput {}
impl sentinel_tool_runtime::ToolOutput for SearchToolOutput {}
impl sentinel_tool_runtime::ToolOutput for EnterPlanModeOutput {}
impl sentinel_tool_runtime::ToolOutput for ExitPlanModeOutput {}
impl sentinel_tool_runtime::ToolOutput for AskUserQuestionOutput {}
impl sentinel_tool_runtime::ToolOutput for MCPOutput {}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn subagent_completed_prompt_format_with_worktree() {
        let output = ToolOutput::SubagentCompleted(SubagentCompletedOutput {
            ...
            turns: 1,
            duration_ms: 2000,
            worktree_path: Some("/tmp/ai-worktree/wt-agent".into()),
            persona: None,
        ...
        let rendered = output.to_prompt_format();
        assert!(
            rendered.contains("<worktree_path>/tmp/ai-worktree/wt-agent</worktree_path>"),
    ...
    fn assert_cer(
        resp: &sentinel_tool_runtime::ToolChatCompletionResponse,
        stdout: &str,
        exit_code: i32,
        timed_out: bool,
    ) {
    ...
    fn bash_output_chat_completion_carries_exit_and_stdout() {
        let resp = sentinel_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(
            0, b"hello\n", false,
        ))
        .unwrap();
    ...
    fn bash_output_chat_completion_empty_stdout_still_emits() {
        let resp =
            sentinel_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(0, b"", false))
                .unwrap();
    ...
    fn bash_output_chat_completion_truncation_marker_and_extra() {
        ...
        let resp = sentinel_tool_runtime::ToolOutput::chat_completion_output(&bash).unwrap();
        let result = resp.result.as_ref().unwrap();
    ...
    fn bash_tool_output_foreground_delegates_background_skips() {
        ...
        assert!(
            sentinel_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn aggregate_tool_output_bash_delegates_background_skips() {
        let resp = sentinel_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Bash(
        ...
        assert!(
            sentinel_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Text(
    ...
    fn bash_tool_id() -> sentinel_tool_protocol::ToolId {
        sentinel_tool_protocol::ToolId::new("bash").unwrap()
    }
    #[test]
