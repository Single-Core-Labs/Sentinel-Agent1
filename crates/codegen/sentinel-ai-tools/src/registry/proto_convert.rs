//! Conversion from the gRPC wire config types (`sentinel-ai-tools-api`) to the
//! runtime registry types ([`ToolConfig`] / [`ToolServerConfig`]).
//!
//! The `params_json` parse/validation contract lives in
//! [`sentinel_ai_tools_api::config_validation`] so every consumer (this
...
pub fn tool_config_from_entry(
    index: usize,
    entry: sentinel_ai_tools_api::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let sentinel_ai_tools_api::ToolConfigEntry {
        id,
        params_json,
        name_override,
    ...
    let params = sentinel_ai_tools_api::config_validation::parse_params_json(
        index,
        &id,
        params_json.as_deref(),
    )?;
    sentinel_ai_tools_api::config_validation::validate_name_override(
        index,
        &id,
...
pub fn tool_server_config_from_entries(
    entries: Vec<sentinel_ai_tools_api::ToolConfigEntry>,
...
mod tests {
    ...
    fn entry(id: &str) -> sentinel_ai_tools_api::ToolConfigEntry {
        sentinel_ai_tools_api::ToolConfigEntry {
            id: id.to_owned(),
            params_json: None,
            name_override: None,
    ...
    fn minimal_entry_converts_with_defaults() {
        let cfg = tool_config_from_entry(0, entry("AiBuild:read_file")).unwrap();
        assert_eq!(cfg.id, "AiBuild:read_file");
    ...
    fn name_override_valid_tool_id_charset_is_accepted() {
        for name in ["search", "AiBuild:grep", "a-b_C9"] {
            let mut e = entry("AiBuild:grep");
    ...
    fn name_override_outside_tool_id_charset_is_rejected() {
        for name in ["has space", "", "a:b:c", "emoji✨", "dot.name"] {
            ...
            assert_eq!(err.tool_id, "AiBuild:grep");
    ...
    fn server_config_conversion_rejects_invalid_name_override_entry() {
        let mut bad = entry("AiBuild:grep");
    ...
    fn server_config_conversion_preserves_valid_name_overrides() {
        let mut a = entry("AiBuild:grep");
        a.name_override = Some("search".to_owned());
        let cfg = tool_server_config_from_entries(vec![a, entry("AiBuild:bash")]).unwrap();
