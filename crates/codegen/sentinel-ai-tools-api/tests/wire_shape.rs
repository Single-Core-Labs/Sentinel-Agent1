...
//! field rename/retype in `ai-tools.proto` cannot silently break those
//! wire contracts (the producer and consumer live in separate services).

use sentinel_ai_tools_api::ToolConfigEntry;

fn full_entry() -> ToolConfigEntry {
    ToolConfigEntry {
        id: "AiBuild:grep".to_owned(),
...
fn finalize_request_minimal() -> sentinel_ai_tools_api::FinalizeToolServerConfigRequest {
    sentinel_ai_tools_api::FinalizeToolServerConfigRequest {
        tools: vec![],
        truncation: None,
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry =
        serde_json::from_value(serde_json::json!({"id": "AiBuild:read_file"}))
            .expect("deserialize minimal");
    assert_eq!(back.id, "AiBuild:read_file");
...
fn finalize_request_callback_fields_default_when_absent() {
    let back: sentinel_ai_tools_api::FinalizeToolServerConfigRequest =
