//! Managed MCP gateway catalog and tool calls via the Ai API.
//!
...
async fn get_authenticated_json<T: serde::de::DeserializeOwned>(
    ...
    let resp = match sentinel_ai_http::shared_client()
        .get(url)
...
/// Fetch the managed MCP gateway tool catalog from the Ai API
