...
const PRODUCTION_ENDPOINTS: AiBuildEndpoints = AiBuildEndpoints {
    cli_chat_proxy_base_url: "https://cli-chat-proxy.ai.com/v1",
    asset_server_url: "https://assets.ai.com",
    relay_ws_url: "wss://code.ai.com/ws/code-agent",
    gateway_ws_url: "wss://ai.com/ws/gw/",
    ws_origin: "https://ai.com",
};
...
pub enum AiBuildEnvironment {
    #[default]
    Production,
}
impl AiBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        AiBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            AiBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, AiBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            AiBuildEnvironment::Production => "AI_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> AiBuildEndpoints {
        match self {
            AiBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// The relay WebSocket URL (Web Frontend at `ai.com/code` driving a
    ...
    /// `AI_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for AiBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AiBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "AI_AI_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    /// loop mistakenly connecting to `wss://ai.com/ws/gw/`).
    #[test]
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            AiBuildEnvironment::Production.relay_ws_url(),
            AiBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            AiBuildEnvironment::from_flags(false, false),
            AiBuildEnvironment::Production
        );
    }
}
