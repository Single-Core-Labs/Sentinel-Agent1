//! Pure resolution logic for `ai plugin install <name>` marketplace refs.
...
mod tests {
    ...
    fn parse_name_with_owner_repo_qualifier() {
        assert_eq!(
            parse_marketplace_ref("sentry@sentinel-org/plugin-marketplace"),
            Some(MarketplaceRef {
                name: "sentry".into(),
                qualifier: Some("sentinel-org/plugin-marketplace".into()),
            })
        );
    }
    ...
    fn parse_rejects_fragment() {
        ...
        assert_eq!(
            parse_marketplace_ref("sentry@sentinel-org/marketplace#sub"),
            None
        );
    }
    ...
    fn slugify_lowercases_and_hyphenates_spaces() {
        ...
        assert_eq!(slugify("AI Official"), "sentinel-official");
    }
    ...
    fn addressable_qualifier_git_and_local() {
        assert_eq!(
            ...
            "sentinel-org/plugin-marketplace"
        );
        assert_eq!(
    ...
    fn resolve_qualifier_matches_git_owner_repo_across_url_forms() {
        for url in [
            ...
            assert_eq!(
                resolve_qualified_source("sentinel-org/plugin-marketplace", &sources),
                Ok(0),
                "url: {url}"
            );
        }
    ...
    fn resolve_qualifier_normalizes_dot_git_in_qualifier() {
        ...
        assert_eq!(
            resolve_qualified_source("sentinel-org/plugin-marketplace.git", &sources),
            Ok(0)
        );
    }
    ...
    fn resolve_qualifier_name_vs_other_source_owner_repo_is_ambiguous() {
        let sources = [
            ...
            git_source(
                "sentinel-org/plugin-marketplace",
                "git@github.example.com:mirror/sentinel.git",
            ),
        ];
        assert_eq!(
            resolve_qualified_source("sentinel-org/plugin-marketplace", &sources),
