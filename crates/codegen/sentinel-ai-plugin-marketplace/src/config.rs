//! Parse marketplace sources from `~/.ai/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
//! name = "AI Official"
//! git = "https://github.com/sentinel-org/sentinel-plugin-marketplace.git"
//!
//! [[marketplace.sources]]
//! name = "Local Dev"
//! path = "~/dev/my-plugins"
...
/// `AI_MARKETPLACE_REQUIRE_SHA=1`. Tighten-only: either source can enable,
...
pub fn env_require_sha() -> bool {
    sentinel_ai_config::env_bool("AI_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// instead of `~/.ai`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn parse_git_source() {
        ...
        let sources = load_sources(&config);
        assert_eq!(sources.len(), 1);
        assert_eq!(sources[0].name, "AI Official");
        assert!(
            matches!(&sources[0].kind, SourceKind::Git { url, branch } if url.contains("sentinel-org") && branch.as_deref() == Some("main"))
        );
    }
