...
//!
//! Catalog file lookup, in order: `.ai-plugin/marketplace.json` (preferred),
//! `.ai-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.ai-plugin/marketplace.json` (preferred AI convention)
/// 2. `.ai-plugin/plugin.json`
...
mod tests {
    ...
    fn load_index_valid_ai_dir() {
        ...
        assert_eq!(result.unwrap().name, "ai");
    }

    #[test]
    fn load_index_ai_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".ai-plugin", "ai"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "ai");
    }
