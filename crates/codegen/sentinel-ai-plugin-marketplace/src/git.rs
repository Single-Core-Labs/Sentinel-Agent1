...
//! Cache root: `~/.ai/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = sentinel_ai_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(sentinel_ai_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    sentinel_ai_config::ai_home().join("marketplace-cache")
}
...
mod tests {
    ...
    fn cache_hash_is_deterministic() {
        let url = "https://github.com/sentinel-org/sentinel-plugin-marketplace.git";
    ...
    #[test]
    fn default_cache_root_under_ai() {
