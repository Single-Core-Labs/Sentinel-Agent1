...
/// Display name of the official AI marketplace source.
pub const OFFICIAL_SOURCE_NAME: &str = "AI Official";

/// Git URL of the official AI marketplace source. Auto-registered on first run.
pub const OFFICIAL_SOURCE_GIT_URL: &str = "https://github.com/sentinel-org/plugin-marketplace.git";

/// Whether `url` is the official AI marketplace source, normalizing case, a
/// `www.` prefix, a trailing `/` or `.git`, and HTTPS/SSH forms before comparing.
pub fn is_official_source_url(url: &str) -> bool {
    canonical_github_owner_repo(url).as_deref() == Some("sentinel-org/plugin-marketplace")
}
...
mod tests {
    ...
    fn is_official_matches_ssh_form() {
        assert!(is_official_source_url(
            "git@github.com:sentinel-org/plugin-marketplace.git"
        ));
        assert!(is_official_source_url(
            "git@github.com:sentinel-org/plugin-marketplace"
        ));
        assert!(is_official_source_url(
            "ssh://git@github.com/sentinel-org/plugin-marketplace.git"
        ));
        assert!(is_official_source_url(
            "ssh://git@github.com/sentinel-org/plugin-marketplace"
        ));
    }
    ...
    fn is_official_rejects_unrelated_urls() {
        ...
        assert!(!is_official_source_url(
            "https://github.com/sentinel-org/some-other-repo.git"
        ));
        assert!(!is_official_source_url(""));
    }
    ...
    fn is_official_matches_noncanonical_forms() {
        assert!(is_official_source_url(
            "https://GitHub.com/AI-org/Plugin-Marketplace"
        ));
        assert!(is_official_source_url(
            "https://github.com/sentinel-org/plugin-marketplace/"
        ));
        ...
        assert!(is_official_source_url(
            "http://github.com/sentinel-org/plugin-marketplace"
        ));
        assert!(is_official_source_url(
            "https://www.github.com/sentinel-org/plugin-marketplace.git"
        ));
        assert!(is_official_source_url(
            "git@github.com:AI-org/plugin-marketplace.git"
        ));
    }
}
