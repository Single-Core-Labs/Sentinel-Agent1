...
use sentinel_ai_agent::plugins::manifest::{ManifestLoadResult, load_manifest, name_from_dirname};
...
fn clone_repo_to_path(
    ...
    let mut cmd = sentinel_tty_utils::git_command();
...
mod tests {
    ...
    fn with_test_registry<T>(f: impl FnOnce(&mut InstallRegistry) -> T) -> T {
        ...
        // process-global `ai_home()` `OnceLock` (first-write-wins). A parallel
        // test in this binary can cache the real `~/.ai` before this runs,
    ...
    fn transactional_update_save_failure_rolls_back_install_and_disk_registry() {
        with_test_registry(|registry| {
            ...
            unsafe { std::env::set_var("AI_AI_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE", "1") };
            let result = update_from_marketplace_entry_transactional(
                marketplace.path(),
                &entry,
                provenance(marketplace.path(), "plugins/demo"),
            ...
            unsafe { std::env::remove_var("AI_AI_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE") };
