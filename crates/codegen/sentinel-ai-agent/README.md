...
This crate extracts a first-class `Agent` type from `sentinel-ai-shell`.
...
`sentinel-ai-shell`, another in-process host, or a headless batch runner.
...
in `.ai/agents/` (project-level) or `~/.ai/agents/` (user-level).

```rust
use sentinel_ai_agent::{AgentDefinition, AgentBuilder};
use sentinel_ai_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".ai/agents/code-reviewer.md")?;
...
```rust
use sentinel_ai_agent::discovery;

// Find all .md files in .ai/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  sentinel-ai-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`sentinel-ai-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `sentinel-ai-agent` depends on it for tool setup.
- **`sentinel-ai-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `sentinel-ai-agent` (AGENTS.md
...
|---|---|---|
| `ai-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p sentinel-ai-agent

# Test
cargo test -p sentinel-ai-agent

# Clippy
cargo clippy -p sentinel-ai-agent --fix --allow-dirty
