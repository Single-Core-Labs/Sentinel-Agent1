...
//! `sentinel-ai-hooks`' parser and inject plugin env vars. Not a second engine.

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use sentinel_ai_hooks::config::{HookSpec, parse_hook_file};
...
fn process_hooks_content(
    ...
    // Native `AI_PLUGIN_*` vars plus their vendor-compat aliases.
    let plugin_env: HashMap<String, String> = HashMap::from([
        ("AI_PLUGIN_ROOT".to_string(), plugin_root.to_string()),
        ("CLAUDE_PLUGIN_ROOT".to_string(), plugin_root.to_string()),
        ("AI_PLUGIN_DATA".to_string(), plugin_data.to_string()),
    ...
    for spec in &mut specs {
        ...
        spec.layer = sentinel_ai_hooks::config::HookProvenance::Plugin;
        spec.name = format!(
            "{}{}/{}",
            sentinel_ai_hooks::config::PLUGIN_HOOK_PREFIX,
            plugin_name,
            spec.name
        );
        ...
        if let Some(cmd) = &spec.command {
            ...
            let expanded = sentinel_ai_config::expand_env_vars_in_string(&substituted);
            if expanded != cmd_str {
...
mod tests {
    ...
    fn parse_plugin_hooks_from_file() {
        ...
        assert_eq!(
            specs[0].extra_env.get("AI_PLUGIN_ROOT").unwrap(),
            "/path/to/plugin"
        );
        assert_eq!(
        ...
        assert_eq!(
            specs[0].extra_env.get("AI_PLUGIN_DATA").unwrap(),
            "/path/to/data"
        );
    ...
    fn parse_plugin_hooks_substitutes_plugin_root_in_command() {
            ...
            "hooks": {
                "PreToolUse": [
                    {"hooks": [
                        ...
                        {"type": "command", "command": "${AI_PLUGIN_ROOT}/hooks/alias.sh"},
        ...
        assert!(
            raws.contains(&"${AI_PLUGIN_ROOT}/hooks/alias.sh"),
            "command_raw must preserve the source string verbatim, got {raws:?}"
        );
        assert!(
    ...
    fn parse_plugin_hooks_user_env_merged_with_plugin_precedence() {
                    ...
                    {"hooks": [
                        {
                            ...
                            "env": {
                                ...
                                "AI_PLUGIN_ROOT": "/user/wins?",
                                "CLAUDE_PLUGIN_DATA": "/user/wins?",
                                "AI_PLUGIN_DATA": "/user/wins?"
                            }
                        }
                    ]}
                ]
        ...
        for (key, expected) in [
            ...
            ("AI_PLUGIN_ROOT", "/actual/plugin/root"),
            ("CLAUDE_PLUGIN_DATA", "/actual/plugin/data"),
            ("AI_PLUGIN_DATA", "/actual/plugin/data"),
        ] {
            assert_eq!(
