...
//!
//! Plugins from project directories (`.ai/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.ai/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.ai/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.ai/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_ai_home() so a project's `.ai/trusted-plugins` is never
        // read as the user trust store when neither AI_HOME nor a home dir resolves.
        let Some(ai) = sentinel_ai_config::user_ai_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = ai.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.ai/trusted-plugins`.
    ...
    /// rewrites `~/.ai/trusted-plugins` without the revoked entry.
