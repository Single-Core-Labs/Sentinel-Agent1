...
/// Default install directory name under `~/.ai/`.
...
/// Lives here (not in sentinel-ai-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MarketplaceProvenance {
    ...
    /// Plugin subdirectory within marketplace (e.g., "plugins/sentinel-code-review").
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("AI_AI_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
    ...
    /// 2. Default: `~/.ai/installed-plugins/`
    pub fn resolve_install_dir() -> PathBuf {
        if let Some(dir) = Self::read_install_dir_from_config() {
            return dir;
        }

        sentinel_ai_config::ai_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
...
pub enum InstallError {
    ...
    #[error(
        "refusing unpinned remote plugin code for '{plugin}' from {url}: \
         marketplace.require_sha / AI_MARKETPLACE_REQUIRE_SHA is enabled and \
