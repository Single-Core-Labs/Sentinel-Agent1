...
//!
//! Searches from cwd to repo root, plus `~/.ai/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.ai/rules/`,
...
//! (`~/.ai/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).

use std::path::{Path, PathBuf};

use crate::prompt::ignore::{build_gitignore, is_ignored};

use sentinel_ai_tools::types::compat::CompatConfig;
...
/// Find `*.md` files in `.ai/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.ai/, git repo root, and session cwd.
...
async fn read_agents_config_with_roots(
    ...
    add_discovery_root(&mut home_roots, ai_home, true, HOME_RULES_DIRS);
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                sentinel_ai_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
/// Used by `sentinel-ai-shell` to structurally detect legacy untagged AGENTS.md
...
/// Open/close `system-reminder` (Ai) or `system_reminder` (Cursor/IDE), case-insensitive.
...
mod tests {
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        ...
        for path in [
            ai_home.join(".ai/rules/doubled.md"),
    ...
    async fn vendor_home_agents_and_rules_cells_are_independent() {
        let tmp = tempfile::tempdir().unwrap();
        let ai_home = tmp.path().join("ai-home");
        let home = tmp.path().join("home");
    ...
    #[tokio::test]
    async fn nested_ai_home_keeps_project_role_in_repo_order() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        let nested = repo.join("nested");
        fs::create_dir_all(nested.join("rules")).unwrap();
        fs::create_dir_all(nested.join(".ai/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(nested.join(".ai/rules/project.md"), "nested-project-rule").unwrap();
    ...
    #[tokio::test]
    async fn overlapping_ai_home_and_project_root_merges_roles() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        ...
        fs::write(repo.join(".ai/rules/project.md"), "project-ai-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".ai/.ai/rules")).unwrap();
        fs::write(repo.join(".ai/.ai/rules/doubled.md"), "doubled").unwrap();
        ...
        .await;
        for expected in ["home-rule", "project-ai-rule", "project-claude-rule"] {
            assert_eq!(
                configs
                    .iter()
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            (ai_home.join("rules/global.md"), "custom-home-body"),
            (home.join(".claude/rules/global.md"), "claude-body"),
            (home.join(".cursor/rules/global.md"), "cursor-body"),
            (repo.join(".ai/rules/project.md"), "ai-project-body"),
