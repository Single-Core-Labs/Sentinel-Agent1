...
//! - `Default`: the legacy Ai Build prefix (built by the shell layer).
...
pub enum UserMessageTemplate {
    /// Legacy Ai Build prefix (`<user_info>` + optional `<git_status>`), built directly by the
...
/// One discovered rule file (AGENTS.md / Claude.md / .ai/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `sentinel_ai_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.ai/, ~/.claude/).
    ...
    /// (`~/.ai/projects/<encoded-cwd>/mcps`). Surfaced in
