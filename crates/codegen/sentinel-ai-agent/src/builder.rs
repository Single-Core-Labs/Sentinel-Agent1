...
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    sentinel_ai_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.ai/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<sentinel_ai_tools::implementations::ai_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn sentinel_ai_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: sentinel_ai_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: sentinel_ai_tools::implementations::ai_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn sentinel_ai_tools::implementations::lsp::LspBackend>>,
    image_gen_config: sentinel_ai_tools::implementations::ai_build::image_gen::ImageGenConfig,
    video_gen_config: sentinel_ai_tools::implementations::ai_build::video_gen::VideoGenConfig,
    ...
    compat: sentinel_ai_tools::types::compat::CompatConfig,
    ...
    api_key_provider: Option<sentinel_ai_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<sentinel_ai_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in sentinel-ai-shell).
    ...
    preloaded_skills: Option<Vec<sentinel_ai_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut sentinel_ai_tools::registry::types::ToolServerConfig) {
    use sentinel_ai_tools::implementations::ai_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("AiBuild:enter_plan_mode");
    let missing_exit = !existing.contains("AiBuild:exit_plan_mode");
    let missing_ask = !existing.contains("AiBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
    ...
    if missing_ask {
        tool_config
            .tools
            .push((&ai_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut sentinel_ai_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(sentinel_ai_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<sentinel_ai_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn sentinel_ai_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: sentinel_ai_tools::implementations::ai_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: sentinel_ai_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `AI_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: sentinel_ai_tools::implementations::ai_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn sentinel_ai_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    /// allowing image generation via the AI Imagine API with session
    /// credentials. When `Disabled` (default), the tool is not registered.
    pub fn with_image_gen_config(
        mut self,
        config: sentinel_ai_tools::implementations::ai_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    /// allowing video generation via the AI Video Generation API with
    /// session credentials. When `Disabled` (default), the tool is not
    /// registered.
    pub fn with_video_gen_config(
        mut self,
        config: sentinel_ai_tools::implementations::ai_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: sentinel_ai_tools::implementations::ai_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: sentinel_ai_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the AiBuild Task description.
    ...
    ///
    /// When disabled, `AiBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.ai/skills, ~/.ai/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use sentinel_ai_tools::implementations::ai_build;
                tool_config.tools.push((&ai_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use sentinel_ai_tools::implementations::ai_build;
                tool_config.tools.push((&ai_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&sentinel_ai_tools::implementations::ai_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&sentinel_ai_tools::implementations::ai_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&sentinel_ai_tools::implementations::ai_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&sentinel_ai_tools::implementations::ai_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&sentinel_ai_tools::implementations::ai_build::ReferenceToVideoTool).into(),
                );
            }
        ...
        if self.memory_backend.is_none() {
            let ai_build_ns = sentinel_ai_tools::types::tool::ToolNamespace::AiBuild.to_string();
            let mem_search_id = format!(
                "{ai_build_ns}:{}",
                sentinel_ai_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{ai_build_ns}:{}",
                sentinel_ai_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                sentinel_ai_tools::types::tool::ToolNamespace::AiBuild,
            );
        ...
        if task_stripped {
            use sentinel_ai_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::AiBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::AiBuildConcise, "run_terminal_cmd", true)
        ...
        if let sentinel_ai_tools::implementations::ai_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["AiBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "AiBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if self.is_non_interactive {
            ...
            merge_tool_params(&mut tool_config, &["AiBuild:ask_user_question"], &ni);
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    "tools allowlist had unmappable entries; keeping full ai toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                sentinel_ai_tools::types::resources::TruncationCfg(
                    sentinel_ai_tools::types::context::TruncationConfig {
...
/// CLI [`sentinel_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: sentinel_tool_types::SubagentToolNaming<'static> =
    sentinel_tool_types::SubagentToolNaming {
...
/// shared [`sentinel_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => sentinel_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => sentinel_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => sentinel_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`sentinel_tool_types::SubagentDescriptor`] and defers to
/// [`sentinel_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<sentinel_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            sentinel_tool_types::SubagentDescriptor {
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        sentinel_ai_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: sentinel_ai_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(sentinel_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(sentinel_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: sentinel_ai_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("ai", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use sentinel_ai_tools::types::template_renderer::TemplateRenderer;
        use sentinel_ai_tools::types::tool::ToolKind;
    ...
    async fn discovery_snapshot_records_gated_and_preloaded_skills() {
        ...
        let write_skill = |dir: &str, content: &str| {
            let d = tmp.path().join(".ai/skills").join(dir);
        ...
        let mut definition = crate::config::AgentDefinition::default_ai_build();
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "ai-build / subagents+ask_user",
                profile: AgentDefinition::default_ai_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ai-build / subagents / no-ask-user",
                profile: AgentDefinition::default_ai_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "ai-build / no-subagents / ask_user",
                profile: AgentDefinition::default_ai_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ai-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_ai_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "ai-build-ask-user / subagents",
                profile: AgentDefinition::ai_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ai-build-ask-user / no-subagents",
                profile: AgentDefinition::ai_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ai-build-plan",
                profile: AgentDefinition::ai_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ai-build-plan / no-ask-user",
                profile: AgentDefinition::ai_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "ai-build-plan-no-subagents",
                profile: AgentDefinition::ai_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "ai-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::ai_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use sentinel_ai_tools::computer::local::LocalTerminalBackend;
        use sentinel_ai_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_ai_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use sentinel_ai_tools::computer::local::LocalTerminalBackend;
        use sentinel_ai_tools::implementations::ai_build::ask_user_question::AskUserQuestionParams;
        use sentinel_ai_tools::notification::ToolNotificationHandle;
        use sentinel_ai_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_ai_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "AiBuild:ask_user_question"),
    ...
    async fn non_interactive_build_stamps_ask_user_question_params() {
        ...
        .from_definition(crate::config::AgentDefinition::default_ai_build())
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use sentinel_ai_tools::computer::local::LocalTerminalBackend;
        use sentinel_ai_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_ai_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("AiBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use sentinel_ai_tools::computer::local::LocalTerminalBackend;
        use sentinel_ai_tools::implementations::ai_build::bash::BashParams;
        use sentinel_ai_tools::notification::ToolNotificationHandle;
        use sentinel_ai_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_ai_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their Ai
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_ai_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their ai
    ...
    /// treat `mcp__*` as always-on, so ai keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        ...
        use sentinel_ai_tools::implementations::web_search::WebSearchConfig;
        use sentinel_ai_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_ai_build();
    ...
    /// ai-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to ai's `search_tool` (MCP
    ...
    async fn build_with_web_search(
        ...
        tool_overrides: Option<sentinel_ai_sampling_types::ToolOverrides>,
    ) -> crate::agent::Agent {
        use sentinel_ai_tools::computer::local::LocalTerminalBackend;
        use sentinel_ai_tools::implementations::web_search::WebSearchConfig;
        use sentinel_ai_tools::notification::ToolNotificationHandle;
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, sentinel_ai_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, sentinel_ai_sampling_types::HostedTool::XSearch { .. })),
    ...
    async fn hosted_tools_bake_definition_tool_overrides_into_options() {
        let x_search = sentinel_ai_sampling_types::XSearchOptions {
            date_bound: Some(
                sentinel_ai_sampling_types::SearchDateBound::new(None, Some("2024-03-15".into()))
                    .unwrap(),
            ),
        };
        let agent = build_with_web_search(
            ...
            true,
            &[],
            Some(sentinel_ai_sampling_types::ToolOverrides {
        ...
        assert!(
            agent
                .hosted_tools()
                .contains(&sentinel_ai_sampling_types::HostedTool::XSearch {
