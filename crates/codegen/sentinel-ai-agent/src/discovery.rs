...
//!
//! Searches `.ai/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.ai/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.ai/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".ai/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.ai/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.ai/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), ai.as_deref())
}
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, ai_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.ai/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.ai/agents/` walking from `cwd` up to repo root
/// 2. `~/.ai/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.ai/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user ai agents, `.claude`
/// compat agents, then bundled. `.ai` dirs resolve from `ai_home`
/// (AI_HOME-aware) plus the legacy literal `~/.ai` when AI_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    ai_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.ai, included only when it differs from ai_home
    // (i.e. AI_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_ai = home
        .map(|h| h.join(".ai"))
        .filter(|legacy| ai_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = ai_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_ai {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let ai = sentinel_ai_config::user_ai_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), ai.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, ai_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let ai = sentinel_ai_config::user_ai_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), ai.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, ai_home);
...
///
/// Project-level `.ai/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let ai = sentinel_ai_config::user_ai_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), ai.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, ai_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, ai_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Ai
...
/// Load project agent definitions from every `.ai/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectAi,
            PluginScope::User => PluginOrigin::UserAi,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_ai_when_ai_home_differs() {
        let home = Path::new("/home/u");
        let ai = Path::new("/custom/aihome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(ai))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&ai.join("agents")));
        assert!(paths.contains(&home.join(".ai").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&ai.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".ai").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_ai_home_is_dot_ai() {
        let home = Path::new("/home/u");
        let ai = home.join(".ai");
        let count = user_agent_dirs(Some(home), Some(&ai))
            .into_iter()
    ...
    #[test]
    fn test_by_name_builtin_ai_build() {
        let def = by_name("ai-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "ai-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".ai").join("agents");
        let agents_dir_2 = inner_dir.join(".ai").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".ai").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".ai")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".ai")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".ai").join("agents");
        let bundled_dir = home.join(".ai").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".ai")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".ai").join("agents");
        let bundled_dir = home.join(".ai").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "ai-build");
        assert_eq!(def.description, "Custom ai-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .ai/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("ai-build-orchestrator")
            .expect("from_str must resolve ai-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::AiBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "ai-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("ai-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find ai-build-orchestrator");
        assert_eq!(def.name, "ai-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.ai/agents/) agent named "explore" should NOT shadow
