...
//! primitive consumed cross-crate by `sentinel-ai-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.ai/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.ai`/`.mcp.json`/plugins would look repo-local. Drop
