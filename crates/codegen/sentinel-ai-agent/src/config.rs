//! Agent definition types — parsed from `.ai/agents/*.md` files.
...
/// Orchestrator-specific prompt body appended to the standard AiBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&ai_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&ai_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&ai_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&ai_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&ai_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_ai_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_ai_build_toolset() -> ToolServerConfig {
    let mut tools = default_ai_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&ai_build::EnterPlanModeTool).into());
    tools.push((&ai_build::ExitPlanModeTool).into());
    tools.push((&ai_build::AskUserQuestionTool).into());
    tools.push((&ai_build::WebSearchTool).into());
    tools.push((&ai_build::ImageGenTool).into());
    tools.push((&ai_build::ImageToVideoTool).into());
    tools.push((&ai_build::ReferenceToVideoTool).into());
    tools.push((&ai_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&ai_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `ai-computer` (workspace/sandbox) preset.
fn ai_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&ai_build::ReadFileTool).into(),
        (&ai_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&ai_build::ListDirTool).into(),
        (&ai_build::GrepTool).into(),
        (&ai_build::KillTerminalCommandTool).into(),
        (&ai_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("ai-build", workspace_ai_build_toolset()),
        ("ai-build-concise", ai_build_concise_toolset()),
        ("ai-build-plan", ai_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("ai-computer", ai_computer_toolset()),
    ]
}
...
fn default_ai_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ai_build::ReadFileTool).into(),
            ...
            (&ai_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&ai_build::SchedulerCreateTool).into(),
            (&ai_build::SchedulerDeleteTool).into(),
            (&ai_build::SchedulerListTool).into(),
            (&ai_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&ai_build::UpdateGoalTool).into(),
            (&ai_build::WorkflowTool).into(),
        ],
        behavior_preset: None,
    }
}
fn ai_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&ai_build_concise::BashConciseTool).into(),
            (&ai_build_concise::ReadFileConciseTool).into(),
            (&ai_build_concise::SearchReplaceConciseTool).into(),
            (&ai_build::ListDirTool).into(),
            (&ai_build::GrepTool).into(),
            kill_task_tool_config(),
            (&ai_build::TodoWriteTool).into(),
...
pub fn ai_build_hashline_toolset(
    hashline_tools: Vec<sentinel_ai_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<sentinel_ai_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
/// Ai Build + plan mode toolset.
///
/// Extends the default `ai-build` toolset with plan mode tools:
...
fn ai_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard ai-build tools
            bash_tool_config(),
            (&ai_build::ReadFileTool).into(),
            (&ai_build::SearchReplaceTool).into(),
            (&ai_build::ListDirTool).into(),
            ...
            (&ai_build::WorkflowTool).into(),
            // Plan mode tools
            (&ai_build::EnterPlanModeTool).into(),
            (&ai_build::ExitPlanModeTool).into(),
            (&ai_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            // Web tools
            (&ai_build::WebSearchTool).into(),
            (&ai_build::WebFetchTool).into(),
            // Imagine
            (&ai_build::ImageGenTool).into(),
            (&ai_build::ImageToVideoTool).into(),
            (&ai_build::ReferenceToVideoTool).into(),
            // Memory
...
/// Ai Build + plan mode toolset WITHOUT subagent tools.
///
/// Same as `ai_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn ai_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard ai-build tools (minus TaskTool only — KillTaskTool and
...
/// Default Ai Build toolset + `ask_user_question`.
///
/// Same as `default_ai_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn ai_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&ai_build::ReadFileTool).into(),
...
/// - `model: ai-3-fast` → `Override("ai-3-fast")`
...
pub enum BuiltinAgentName {
    ...
    Plan,
    BrowserUse,
    #[strum(serialize = "ai-build-orchestrator")]
    AiBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::AiBuild => AgentDefinition::default_ai_build(),
            Self::AiBuildConcise => AgentDefinition::ai_build_concise(),
            Self::AiBuildPlan => AgentDefinition::ai_build_plan(),
            Self::AiBuildPlanNoSubagents => AgentDefinition::ai_build_plan_no_subagents(),
            Self::AiBuildAskUser => AgentDefinition::ai_build_ask_user(),
            ...
            Self::AiBuildOrchestrator => AgentDefinition::ai_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .ai/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_ai_build_toolset")]
    ...
    pub capability_mode: Option<sentinel_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<sentinel_ai_sampling_types::ToolOverrides>,
...
pub enum AgentScope {
    /// .ai/agents/ (project-level, highest priority)
    Project,
    /// ~/.ai/agents/ (user-level)
    User,
    /// ~/.ai/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_ai_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.ai/agent-memory/<name>/`
    User,
    /// `<project>/.ai/agent-memory/<name>/`
    Project,
    /// `<project>/.ai/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: sentinel_ai_config::ai_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".ai/agent-memory").join(agent_name),
...
/// these into `sentinel_ai_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let ai = sentinel_ai_config::user_ai_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), ai.as_deref()) {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `ai-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<sentinel_ai_tools::registry::types::ToolConfig>,
    ) {
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_ai_build_toolset(),
    ...
    pub fn default_ai_build() -> Self {
        Self::base(
            BuiltinAgentName::AiBuild,
            "Ai Build agent for software engineering tasks.",
        )
    }
    /// Ai Build Concise agent definition — concise output format for SFT/RL.
    pub fn ai_build_concise() -> Self {
        Self {
            tool_config: ai_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::AiBuildConcise,
                "Ai Build agent with concise output format.",
            )
        }
    }
    /// Ai Build agent with plan mode tools.
    pub fn ai_build_plan() -> Self {
        Self {
            tool_config: ai_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::AiBuildPlan,
                "Ai Build agent with plan mode support.",
            )
        }
    }
    /// Ai Build + plan mode WITHOUT subagent tools.
    pub fn ai_build_plan_no_subagents() -> Self {
        Self {
            tool_config: ai_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::AiBuildPlanNoSubagents,
                "Ai Build agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Ai Build agent with the `ask_user_question` tool.
    pub fn ai_build_ask_user() -> Self {
        Self {
            tool_config: ai_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::AiBuildAskUser,
                "Ai Build agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: sentinel_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: sentinel_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// Ai Build Orchestrator — GBL model with full AiBuild tools
    ...
    pub fn ai_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::AiBuildOrchestrator,
                "AiBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_ai_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("ai-build").unwrap();
    ...
    fn ai_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&ai_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&ai_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn ai_computer_preset_is_curated_ai_build_subset() {
        let gc = toolset_for_preset("ai-computer").unwrap();
        let gb = toolset_for_preset("ai-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = ai_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "ai-computer tool `{}` must also ship in the ai-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "ai-computer should be a curated subset of ai-build"
        );
    }
    #[test]
    fn ai_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("ai-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&ai_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&ai_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&ai_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&ai_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&ai_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&ai_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&ai_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The ai-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the ai-build
    ...
    #[test]
    fn ai_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("ai-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "ai-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn ai_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("ai-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&ai_build::LspTool).id,
            ToolConfig::from(&ai_build::EnterPlanModeTool).id,
            ToolConfig::from(&ai_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "ai-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_ai_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_ai_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::AiBuildOrchestrator => true,
            BuiltinAgentName::AiBuild
            | BuiltinAgentName::AiBuildConcise
            | BuiltinAgentName::AiBuildPlan
            | BuiltinAgentName::AiBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "ai-build-orchestrator"] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("ai-3-fast".to_string()).to_string(),
            "ai-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("ai-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.ai/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.ai/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_ai_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have ai_build tools"
        );
    }
    #[test]
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"ai-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("ai-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("ai-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"ai-3-fast\"");
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!({
            "name": "test",
            "description": "Test",
            "model": "ai-code-fast-1"
        });
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("ai-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("ai-build", BuiltinAgentName::AiBuild),
            ("ai-build-concise", BuiltinAgentName::AiBuildConcise),
            ("ai-build-ask-user", BuiltinAgentName::AiBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_ai_build();
