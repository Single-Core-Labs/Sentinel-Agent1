...
//! `sentinel_ai_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `AI_POOL_*` /
//! `AI_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `AI_SAMPLER_SHARED_CLIENT=0` falls back to
...
//! `AI_EXTRA_CA_BUNDLE` via `sentinel_ai_extra_ca` (see env-var registry).

use std::sync::OnceLock;

use sentinel_ai_workspace::permission::ClientType;
...
///
/// Replaces `sentinel_ai_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `sentinel_ai_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use sentinel_ai_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `sentinel-ai-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `sentinel-ai-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `sentinel-ai-telemetry` consumes
// the same type via `sentinel_ai_sampler::OriginClientInfo`. The shell-specific
...
/// `AI_CLIENT_VERSION` env vars. Returns `None` when
/// `AI_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`AI_CLIENT_NAME` env var, default `"ai-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("AI_CLIENT_NAME").unwrap_or_else(|_| "ai-shell".to_string())
}

/// Header telling cli-chat-proxy whether this process is a single-prompt
/// (`ai -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-ai-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `ai -p`, `run_headless_inner` for
/// `ai agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn sentinel_ai_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(sentinel_ai_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("ai-desktop/1.2.3 ai-shell/"));
        ...
        assert!(without_version.starts_with("ai-web ai-shell/"));
        assert!(!without_version.starts_with("ai-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "ai-shell/0.1.171 (macos; aarch64)");
    }
}
