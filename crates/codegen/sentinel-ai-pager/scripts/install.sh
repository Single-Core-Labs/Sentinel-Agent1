#!/bin/bash
#
# Ai CLI installer — https://x.ai/cli/install.sh
#
# Auth: AI_DEPLOYMENT_KEY (takes precedence) or ~/.ai/auth.json from `ai login`.
# Env: AI_CHANNEL (stable|alpha|enterprise, default: stable), AI_BIN_DIR, AI_PROXY_URL
#
# Usage:
#   curl -fsSL https://x.ai/cli/install.sh | bash            # latest stable
#   curl -fsSL https://x.ai/cli/install.sh | bash -s 0.1.42  # specific version
#   AI_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/install.sh)
#
...
# Read a token from ~/.ai/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_ai_token() {
    local auth_file="$HOME/.ai/auth.json"
    local scope="$1"
...
# Resolve auth: AI_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://auth.x.ai::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.x.ai/sign-in"
AUTH_SOURCE=""

if [ -n "$AI_DEPLOYMENT_KEY" ]; then
    AUTH_SOURCE="deployment key"
    echo "Auth: using deployment key." >&2
else
    OIDC_TOKEN=$(read_ai_token "$OIDC_SCOPE" 2>/dev/null) || true
    LEGACY_TOKEN=$(read_ai_token "$LEGACY_SCOPE" 2>/dev/null) || true
    if [ -n "$OIDC_TOKEN" ]; then
        AUTH_SOURCE="auth.json (oidc)"
        echo "Auth: using OIDC token from ~/.ai/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.ai/auth.json." >&2
    fi
fi
...
BASE_URL_FALLBACK="https://storage.googleapis.com/ai-build-public-artifacts/cli"
DOWNLOAD_DIR="$HOME/.ai/downloads"
BIN_DIR="${AI_BIN_DIR:-$HOME/.ai/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${AI_CHANNEL:-stable}"
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing Ai $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing Ai $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/ai-$platform"
artifact_base="${BASE_URL}/ai-${version}-${platform}"
    ...
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: Ai is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in ai.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/ai.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded ai failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.ai/bin and ~/.ai/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/ai"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/ai and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.ai/completions/bash" "$HOME/.ai/completions/zsh"
"$BIN_DIR/ai" completions bash > "$HOME/.ai/completions/bash/ai.bash" 2>/dev/null || true
"$BIN_DIR/ai" completions zsh  > "$HOME/.ai/completions/zsh/_ai"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/ai" completions fish > "$HOME/.config/fish/completions/ai.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.ai/config.toml"
...
if [ -n "$AI_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${AI_PROXY_URL:-https://cli-chat-proxy.ai.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$AI_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.ai/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.ai/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.ai/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.ai/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "Ai $version installed to $BIN_DIR/ai.exe" >&2
else
    echo "Ai $version installed to $BIN_DIR/ai" >&2
fi
...
# Try to symlink into a directory already on PATH so ai works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/ai" "$candidate/ai"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/ai -> $BIN_DIR/ai" >&2
...
# Also update shell config so ~/.ai/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> ai installer >>>
fish_add_path $HOME/.ai/bin
# <<< ai installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> ai installer >>>
export PATH="$HOME/.ai/bin:$PATH"
fpath=(~/.ai/completions/zsh $fpath)
...
# <<< ai installer <<<'
    ...
    if grep -qs "ai installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> ai installer >>>/ { skip=1; next }
            /# <<< ai installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'ai' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'ai' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'ai' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.ai/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use ai from cmd.exe or PowerShell, add %USERPROFILE%\\.ai\\bin to your PATH." >&2
fi
