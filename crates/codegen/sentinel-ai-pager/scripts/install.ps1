#
# Ai CLI installer for PowerShell — https://x.ai/cli/install.ps1
#
# Auth: AI_DEPLOYMENT_KEY env var (takes precedence) or ~/.ai/auth.json from `ai login`.
...
#   $env:AI_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex           # specific version (alt)
#   $env:AI_DEPLOYMENT_KEY="<key>"; irm https://x.ai/cli/install.ps1 | iex
#
...
if (-not $Version -and $env:AI_VERSION) {
    $Version = $env:AI_VERSION
}
...
function Read-AiToken([string]$Scope) {
    $authFile = Join-Path $AiDir 'auth.json'
...
if ($env:AI_DEPLOYMENT_KEY) {
    $AuthSource = 'deployment key'
    Write-Host 'Auth: using deployment key.' -ForegroundColor DarkGray
} else {
    $oidcToken = Read-AiToken $OidcScope
    $legacyToken = Read-AiToken $LegacyScope
    if ($oidcToken) {
        $AuthSource = 'auth.json (oidc)'
        Write-Host 'Auth: using OIDC token from ~/.ai/auth.json.' -ForegroundColor DarkGray
    } elseif ($legacyToken) {
        $AuthSource = 'auth.json (legacy)'
        Write-Host 'Auth: using legacy token from ~/.ai/auth.json.' -ForegroundColor DarkGray
    }
}
...
$BaseUrlFallback = 'https://storage.googleapis.com/ai-build-public-artifacts/cli'
$DownloadDir = Join-Path $AiDir 'downloads'
$BinDir = if ($env:AI_BIN_DIR) { $env:AI_BIN_DIR } else { Join-Path $AiDir 'bin' }

New-Item -ItemType Directory -Path $DownloadDir -Force | Out-Null
New-Item -ItemType Directory -Path $BinDir -Force | Out-Null

$Channel = if ($env:AI_CHANNEL) { $env:AI_CHANNEL } else { 'stable' }
...
if ($AuthSource) {
    Write-Host "Installing Ai $resolvedVersion ($platform, $AuthSource)..." -ForegroundColor Cyan
} else {
    Write-Host "Installing Ai $resolvedVersion ($platform)..." -ForegroundColor Cyan
}

# --- Download binary ---

$binaryPath = Join-Path $DownloadDir "ai-$platform.exe"
$artifactBase = "$BaseUrl/ai-$resolvedVersion-$platform"
...
foreach ($binName in @('ai.exe', 'agent.exe')) {
...
Write-Host "  Installed to $BinDir\ai.exe and $BinDir\agent.exe." -ForegroundColor DarkGray

# --- Generate completions (best-effort) ---

$completionsDir = Join-Path (Join-Path $AiDir 'completions') 'powershell'
try {
    New-Item -ItemType Directory -Path $completionsDir -Force | Out-Null
    & (Join-Path $BinDir 'ai.exe') completions powershell 2>$null |
        Set-Content (Join-Path $completionsDir 'ai.ps1') -ErrorAction SilentlyContinue
} catch {}

# --- Persist installer config ---

$ConfigFile = Join-Path $AiDir 'config.toml'
...
if ($env:AI_DEPLOYMENT_KEY) {
    $ProxyUrl = if ($env:AI_PROXY_URL) { $env:AI_PROXY_URL } else { 'https://cli-chat-proxy.ai.com/v1' }
    Write-Host '  Fetching deployment config...' -ForegroundColor DarkGray
    try {
        $headers = @{ 'Authorization' = "Bearer $($env:AI_DEPLOYMENT_KEY)" }
    ...
    if ($deployResponse) {
        ...
        $managedConfigPath = Join-Path $AiDir 'managed_config.toml'
        $requirementsPath = Join-Path $AiDir 'requirements.toml'
...
Write-Host "Ai $resolvedVersion installed to $BinDir\ai.exe" -ForegroundColor Green
...
if ($pathEntries -notcontains $BinDir) {
    ...
    # Update current session so ai works immediately.
...
Write-Host ''
Write-Host "Run 'ai' or 'agent' to get started!" -ForegroundColor Cyan
