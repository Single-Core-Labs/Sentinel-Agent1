...
//   1. Brotli-compresses the built binary into `../ai-<platform>/bin/<bin>.br`
...
// (`@sentinel-official/ai`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw ai binary is
...
const sentinelRoot = process.env.AI_ROOT || path.resolve(__dirname, '..', '..', '..', '..', '..');
const npmRoot = path.resolve(__dirname, '..', '..');

const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'sentinel-ai-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `ai-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] ai-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'ai',
            envVar: 'AI_DARWIN_ARM64',
            defaultSource: path.join(sentinelRoot, 'target', 'release', 'sentinel-ai-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'ai',
            envVar: 'AI_DARWIN_X64',
            defaultSource: path.join(sentinelRoot, 'target', 'x86_64-apple-darwin', 'release', 'sentinel-ai-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'ai',
            envVar: 'AI_LINUX_X64',
            defaultSource: path.join(sentinelRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'sentinel-ai-pager'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'ai',
            envVar: 'AI_LINUX_ARM64',
            defaultSource: path.join(sentinelRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'sentinel-ai-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'ai.exe',
            envVar: 'AI_WIN32_X64',
            defaultSource: path.join(sentinelRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'sentinel-ai-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'ai.exe',
            envVar: 'AI_WIN32_ARM64',
            defaultSource: path.join(sentinelRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'sentinel-ai-pager.exe'),
        },
    ];
