...
// postinstall.js and the bin/ai trampoline.
//
...
function makeTmpDir() {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'ai-test-'));
}
...
// ─── Extracted logic (mirrors postinstall.js and bin/ai exactly) ─────
...
function installVersionedBinary(vendoredBinPath, version, canonicalDir) {
    const canonicalPath = path.join(canonicalDir, 'ai');
    fs.mkdirSync(canonicalDir, { recursive: true });

    const versionedName = `ai-${version}`;
...
function cleanupOldVersions(canonicalDir, currentVersionedName) {
    ...
    const versionedBinaries = entries
        .filter(e => e.startsWith('ai-') && !e.includes('.tmp.') && !e.includes('.link.') && e !== currentVersionedName)
        .sort(byVersionDescending('ai-'));
...
/** Ai bin dir resolution (mirrors postinstall.js and bin/ai). */
function resolveAiBinDir(env, homedir) {
    const aiHome = env.AI_HOME ?? path.join(homedir, '.ai');
    return path.join(aiHome, 'bin');
}
...
/** Bootstrap canonical from vendored (same as bin/ai trampoline). */
...
test('creates versioned binary and symlink on fresh install', () => {
    ...
    try {
        const vendored = path.join(dir, 'vendored-ai');
        ...
        assert.strictEqual(target, 'ai-0.1.140');
...
test('upgrade swaps symlink and preserves old binary', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'ai-0.1.141');
        ...
        const oldBinary = path.join(binDir, 'ai-0.1.140');
...
test('idempotent: reinstalling same version does not re-copy', () => {
    ...
    try {
        ...
        const versionedPath = path.join(binDir, 'ai-0.1.140');
...
test('symlink swap is atomic (no intermediate missing state)', () => {
    ...
    try {
        const binDir = path.join(dir, 'bin');
        const canonicalPath = path.join(binDir, 'ai');
...
test('handles upgrade from old-style regular file to versioned symlink', () => {
    ...
    try {
        ...
        // Simulate old installation: ai is a regular file
...
test('handles broken symlink (target deleted externally)', () => {
    ...
    try {
        ...
        const canonicalPath = path.join(binDir, 'ai');
        fs.symlinkSync('ai-0.1.99', canonicalPath);
...
test('three sequential upgrades: v1 -> v2 -> v3 all coexist', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ai')), 'ai-0.1.3');

        // All three versioned binaries still exist (no cleanup yet)
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.1')));
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.2')));
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.3')));
    } finally {
        cleanup(dir);
    }
});
...
test('cleanup keeps N-1 version and removes older ones', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ai-0.1.138'), 'v138');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.139'), 'v139');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.140'), 'v140');
        // ai-0.1.141 is the current version (excluded from cleanup)
        fs.writeFileSync(path.join(binDir, 'ai-0.1.141'), 'v141');

        cleanupOldVersions(binDir, 'ai-0.1.141');

        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.141')), 'current should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.140')), 'N-1 should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.1.139')), 'N-2 should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.1.138')), 'N-3 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('cleanup with only one old version keeps it', () => {
    ...
    try {
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.140')), 'single old version should be kept');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.141')), 'current should exist');
    } finally {
        cleanup(dir);
    }
...
test('cleanup with no old versions is a no-op', () => {
    ...
    try {
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.141')), 'current should still exist');
        const entries = fs.readdirSync(binDir).filter(e => e.startsWith('ai-'));
...
test('cleanup ignores .tmp. and .link. files', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ai-0.1.141'), 'current');
        // Leftover temp files from a crashed install
        fs.writeFileSync(path.join(binDir, 'ai-0.1.140.tmp.12345'), 'crashed-tmp');
        fs.writeFileSync(path.join(binDir, 'ai.link.12345'), 'crashed-link');

        cleanupOldVersions(binDir, 'ai-0.1.141');

        // Temp files should not be touched by cleanup (they're filtered out)
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.140.tmp.12345')), 'tmp file should not be touched');
        assert.ok(fs.existsSync(path.join(binDir, 'ai.link.12345')), 'link file should not be touched');
    } finally {
        cleanup(dir);
    }
});
...
test('semver sort: 0.1.9 vs 0.1.10 (digit boundary)', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ai-0.1.8'), 'v8');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.9'), 'v9');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.10'), 'v10');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.11'), 'v11');

        cleanupOldVersions(binDir, 'ai-0.1.11');

        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.11')), 'current should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.10')), '0.1.10 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.1.9')), '0.1.9 should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.1.8')), '0.1.8 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('semver sort: major version boundary (0.x vs 1.x)', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ai-0.9.99'), 'old');
        fs.writeFileSync(path.join(binDir, 'ai-1.0.0'), 'v1');
        fs.writeFileSync(path.join(binDir, 'ai-1.0.1'), 'current');

        cleanupOldVersions(binDir, 'ai-1.0.1');

        assert.ok(fs.existsSync(path.join(binDir, 'ai-1.0.0')), '1.0.0 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.9.99')), '0.9.99 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('semver sort: minor version boundary (0.1.x vs 0.2.x)', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'ai-0.1.999'), 'old');
        fs.writeFileSync(path.join(binDir, 'ai-0.2.0'), 'v2');
        fs.writeFileSync(path.join(binDir, 'ai-0.2.1'), 'current');

        cleanupOldVersions(binDir, 'ai-0.2.1');

        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.2.0')), '0.2.0 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.1.999')), '0.1.999 should be removed');
    } finally {
        cleanup(dir);
    }
});
...
test('byVersionDescending: unit test comparator directly', () => {
    const input = ['ai-0.1.9', 'ai-0.1.10', 'ai-0.1.2', 'ai-1.0.0', 'ai-0.2.0'];
    const sorted = [...input].sort(byVersionDescending('ai-'));
...
test('bootstrapCanonical creates versioned binary from vendored', () => {
    ...
    try {
        ...
        assert.strictEqual(result, path.join(binDir, 'ai'));
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.140')), 'versioned binary should exist');
...
test('bootstrapCanonical is idempotent', () => {
    ...
    try {
        ...
        assert.strictEqual(
            fs.readFileSync(path.join(binDir, 'ai-0.1.140'), 'utf8'),
            'original-content',
...
test('bootstrapCanonical works when canonical already exists (different version)', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(result), 'ai-0.1.141');
        // v1 should still exist
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.140')), 'old version should still exist');
    } finally {
        cleanup(dir);
    }
});
...
test('full lifecycle: install, upgrade, cleanup', () => {
    ...
    try {
        ...
        cleanupOldVersions(binDir, 'ai-0.1.142');

        // Current (v3) + N-1 (v2) should exist; v1 removed
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.142')), 'v3 should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.141')), 'v2 should be kept (N-1)');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.1.140')), 'v1 should be removed');

        // Canonical symlink points to v3
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ai')), 'ai-0.1.142');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'ai'), 'utf8'), 'v3');
    } finally {
        cleanup(dir);
    }
});
...
test('downgrade: installing older version than current', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ai')), 'ai-0.1.140');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'ai'), 'utf8'), 'v1');

        // v2 should still exist (never delete old binaries during install)
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.141')), 'v2 should still exist');
    } finally {
        cleanup(dir);
    }
});

test('non-ai files in bin dir are not touched by cleanup', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        // Ai versions
        fs.writeFileSync(path.join(binDir, 'ai-0.1.138'), 'old1');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.139'), 'old2');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.140'), 'current');

        cleanupOldVersions(binDir, 'ai-0.1.140');

        assert.ok(fs.existsSync(path.join(binDir, 'other-tool')), 'non-ai file should not be touched');
        assert.ok(fs.existsSync(path.join(binDir, 'README.md')), 'non-ai file should not be touched');
    } finally {
        cleanup(dir);
    }
});
...
// ═══════════════════════════════════════════════════════════════════════
// ai vs ai-pager Isolation Tests
// ═══════════════════════════════════════════════════════════════════════
 ...
 * Uses prefix + leading digit to avoid ai-* matching ai-pager-*.
 */
...
test('installing both ai and ai-pager creates independent symlinks', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored');
        fs.writeFileSync(vendored, 'ai-binary');
        const vendoredPager = path.join(dir, 'vendored-pager');
        fs.writeFileSync(vendoredPager, 'pager-binary');

        installNamedBinary(vendored, 'ai', '0.1.141', binDir);
        installNamedBinary(vendoredPager, 'ai-pager', '0.1.141', binDir);

        // Both symlinks exist and point to correct targets
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ai')), 'ai-0.1.141');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ai-pager')), 'ai-pager-0.1.141');

        // Both versioned files exist with correct content
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'ai-0.1.141'), 'utf8'), 'ai-binary');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'ai-pager-0.1.141'), 'utf8'), 'pager-binary');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of ai-* does not remove ai-pager-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        // Old ai versions
        fs.writeFileSync(path.join(binDir, 'ai-0.1.138'), 'old-ai-1');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.139'), 'old-ai-2');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.140'), 'old-ai-3');
        // Current ai
        fs.writeFileSync(path.join(binDir, 'ai-0.1.141'), 'current-ai');

        // ai-pager versions (should not be touched)
        fs.writeFileSync(path.join(binDir, 'ai-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'ai-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'ai-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'ai', '0.1.141');

        // ai cleanup: current + N-1 kept, older removed
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.141')), 'current ai should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.140')), 'N-1 ai should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.1.139')), 'N-2 ai should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.1.138')), 'N-3 ai should be removed');

        // ALL ai-pager versions must be untouched
        assert.ok(fs.existsSync(path.join(binDir, 'ai-pager-0.1.138')), 'ai-pager-0.1.138 must survive ai cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-pager-0.1.139')), 'ai-pager-0.1.139 must survive ai cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-pager-0.1.141')), 'ai-pager-0.1.141 must survive ai cleanup');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of ai-pager-* does not remove ai-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });

        // ai versions (should not be touched)
        fs.writeFileSync(path.join(binDir, 'ai-0.1.138'), 'old-ai-1');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.139'), 'old-ai-2');
        fs.writeFileSync(path.join(binDir, 'ai-0.1.141'), 'current-ai');

        // Old ai-pager versions
        fs.writeFileSync(path.join(binDir, 'ai-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'ai-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'ai-pager-0.1.140'), 'old-pager-3');
        // Current pager
        fs.writeFileSync(path.join(binDir, 'ai-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'ai-pager', '0.1.141');

        // ai-pager cleanup: current + N-1 kept, older removed
        assert.ok(fs.existsSync(path.join(binDir, 'ai-pager-0.1.141')), 'current pager should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-pager-0.1.140')), 'N-1 pager should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-pager-0.1.139')), 'N-2 pager should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-pager-0.1.138')), 'N-3 pager should be removed');

        // ALL ai versions must be untouched
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.138')), 'ai-0.1.138 must survive pager cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.139')), 'ai-0.1.139 must survive pager cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.141')), 'ai-0.1.141 must survive pager cleanup');
    } finally {
        cleanup(dir);
    }
});
...
test('full dual-binary lifecycle: install, upgrade, cleanup both', () => {
    ...
    try {
        ...
        // v1
        fs.writeFileSync(vendored, 'ai-v1');
        fs.writeFileSync(vendoredPager, 'pager-v1');
        installNamedBinary(vendored, 'ai', '0.1.140', binDir);
        installNamedBinary(vendoredPager, 'ai-pager', '0.1.140', binDir);

        // v2
        fs.writeFileSync(vendored, 'ai-v2');
        fs.writeFileSync(vendoredPager, 'pager-v2');
        installNamedBinary(vendored, 'ai', '0.1.141', binDir);
        installNamedBinary(vendoredPager, 'ai-pager', '0.1.141', binDir);

        // v3
        fs.writeFileSync(vendored, 'ai-v3');
        fs.writeFileSync(vendoredPager, 'pager-v3');
        installNamedBinary(vendored, 'ai', '0.1.142', binDir);
        installNamedBinary(vendoredPager, 'ai-pager', '0.1.142', binDir);

        // Cleanup both independently
        cleanupOldVersionsNamed(binDir, 'ai', '0.1.142');
        cleanupOldVersionsNamed(binDir, 'ai-pager', '0.1.142');

        // Current + N-1 for each
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.142')));
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.141')));
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-0.1.140')));

        assert.ok(fs.existsSync(path.join(binDir, 'ai-pager-0.1.142')));
        assert.ok(fs.existsSync(path.join(binDir, 'ai-pager-0.1.141')));
        assert.ok(!fs.existsSync(path.join(binDir, 'ai-pager-0.1.140')));

        // Symlinks correct
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ai')), 'ai-0.1.142');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ai-pager')), 'ai-pager-0.1.142');
    } finally {
        cleanup(dir);
    }
});
...
test('ai installs normally regardless of platform key', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored-ai');
        ...
        for (const platform of ['darwin-arm64', 'linux-x64', 'linux-arm64']) {
            const result = installNamedBinary(vendored, 'ai', '0.1.150', binDir);
            assert.ok(fs.existsSync(result.versionedPath), `ai should install for ${platform}`);
            assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'ai-0.1.150');
        }
    } finally {
        cleanup(dir);
    }
...
test('ai-pager installs when vendored binary exists (darwin-arm64 path)', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendoredPager = path.join(dir, 'vendored-pager');
        ...
        const result = installNamedBinary(vendoredPager, 'ai-pager', '0.1.150', binDir);
        assert.ok(fs.existsSync(result.versionedPath), 'pager versioned binary should exist');
        assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'ai-pager-0.1.150');
...
test('Linux pager vendor files are not required for ai install', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        ...
        fs.writeFileSync(path.join(vendorBase, 'darwin-arm64', 'ai-pager'), 'mac-pager');
        ...
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-x64', 'ai-pager')));
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-arm64', 'ai-pager')));

        // ai install should succeed independently
        const aiVendored = path.join(dir, 'vendored-ai');
        fs.writeFileSync(aiVendored, 'ai-linux');
        const result = installNamedBinary(aiVendored, 'ai', '0.1.150', binDir);
        assert.ok(fs.existsSync(result.versionedPath), 'ai should install without Linux pager');
    } finally {
        cleanup(dir);
    }
});

test('skipping pager install on Linux does not affect ai cleanup', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored');

        // Install ai across two versions
        fs.writeFileSync(vendored, 'ai-v1');
        installNamedBinary(vendored, 'ai', '0.1.149', binDir);
        fs.writeFileSync(vendored, 'ai-v2');
        installNamedBinary(vendored, 'ai', '0.1.150', binDir);

        // Simulate Linux: only run ai cleanup, skip pager entirely
        cleanupOldVersionsNamed(binDir, 'ai', '0.1.150');

        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.150')), 'current ai should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'ai-0.1.149')), 'N-1 ai should be kept');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'ai')), 'ai-0.1.150');
...
test('canonical pager from non-npm install is preserved on Linux', () => {
    ...
    try {
        ...
        // Simulate pager installed by install-ai.sh (not npm)
        const pagerVersioned = path.join(binDir, 'ai-pager-0.1.150');
        fs.writeFileSync(pagerVersioned, 'installer-pager-binary');
        fs.chmodSync(pagerVersioned, 0o755);
        const pagerCanonical = path.join(binDir, 'ai-pager');
        fs.symlinkSync('ai-pager-0.1.150', pagerCanonical);

        // Run ai-only install + cleanup (simulating Linux postinstall)
        const vendored = path.join(dir, 'vendored');
        fs.writeFileSync(vendored, 'ai-binary');
        installNamedBinary(vendored, 'ai', '0.1.150', binDir);
        cleanupOldVersionsNamed(binDir, 'ai', '0.1.150');
        ...
        assert.strictEqual(fs.readlinkSync(pagerCanonical), 'ai-pager-0.1.150');
    } finally {
        cleanup(dir);
    }
});

console.log('\nai home + brotli install tests\n');

test('resolveAiBinDir honors $AI_HOME, else falls back to <home>/.ai/bin', () => {
    assert.strictEqual(
        resolveAiBinDir({ AI_HOME: '/fast/local/.ai' }, '/home/alice'),
        path.join('/fast/local/.ai', 'bin'),
    );
    assert.strictEqual(
        resolveAiBinDir({}, '/home/alice'),
        path.join('/home/alice', '.ai', 'bin'),
    );
    assert.strictEqual(resolveAiBinDir({ AI_HOME: '' }, '/home/alice'), path.join('', 'bin'));
});

test('writeVendorBinary returns false (not true) when the destination cannot be written', () => {
    const dir = makeTmpDir();
    try {
        const brPath = path.join(dir, 'ai.br');
...
test('decompresses brotli into the canonical dir without duplicating into node_modules', () => {
    ...
    try {
        ...
        assert.ok(!fs.existsSync(path.join(vendorBin, 'ai')), 'no uncompressed binary in node_modules');
