...
MCP (Model Context Protocol) servers extend Ai with external tool integrations. They let Ai interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Ai over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Ai's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.ai/config.toml` under `[mcp_servers.<name>]` sections.
...
Ai spawns a local process and communicates over stdin/stdout:
...
> `AI_MCP_STARTUP_TIMEOUT_SECS` (seconds). A per-server `startup_timeout_sec`
...
>
> - env `AI_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Ai-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.ai/config.toml`) **or repo-level**
>   (`.ai/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.ai/config.toml` >
...
ai mcp list
ai mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by ai.
ai mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
ai mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
ai mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
ai mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
ai mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
ai mcp remove github

# Enable or disable a local/TOML (or compat-sourced) server
ai mcp enable github
ai mcp disable github

# Diagnose a server's configuration and connectivity
ai mcp doctor               # Check every configured server
ai mcp doctor github        # Check one server
ai mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `ai mcp add` writes to `~/.ai/config.toml` (`--scope user`). Use `--scope project` to write to `.ai/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `ai mcp list` shows servers from both scopes, marking project-scoped ones with `(project)` and disabled ones with `(disabled)`.

`ai mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.

`ai mcp enable` / `disable` persist the personal on/off state to user `~/.ai/config.toml` (`disabled_mcp_servers`, and `[mcp_servers.<name>].enabled` when that entry exists). Scope:

- **Known names:** user/project Ai TOML, names already on the disabled list, compat sources (`.mcp.json`, Claude, Cursor), and **plugin** MCP servers (same discovery as doctor/`/mcps`).
...
MCP servers can be configured per-project by placing a `.ai/config.toml` in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Ai handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Ai's own OAuth client with the provider.

Ai walks from the current directory up to the git repo root, loading `.ai/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.ai/config.toml` | All projects | Lowest |
| `<repo-root>/.ai/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Ai reads most other config sections only from `~/.ai/config.toml`.
...
You can enable or disable MCP servers without restarting Ai (TUI `/mcps` or CLI — see [CLI Management](#cli-management)).
...
Ai loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Ai config | `~/.ai/config.toml`, `.ai/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.ai/config.toml` or the corresponding environment variable (`AI_CURSOR_MCPS_ENABLED`, `AI_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `ai inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, Ai handles the credential flow automatically. When an MCP server requests OAuth credentials, Ai opens a browser-based authorization flow and stores the resulting tokens for future use.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Ai expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
1. The parent session actually connected the server (see Extensions / `ai inspect`)
...
For stdio servers, Ai captures the process's standard error to `~/.ai/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.ai/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `ai inspect` to see all loaded MCP servers and their sources:

```bash
ai inspect          # Human-readable
ai inspect --json   # Machine-readable
```

### Debug Logging

```bash
RUST_LOG=debug AI_LOG_FILE=/tmp/ai.log ai
tail -f /tmp/ai.log
```
