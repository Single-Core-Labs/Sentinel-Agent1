...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.ai/agents/` or `~/.ai/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.ai/personas/`; applied during subagent resolution |
...
| **Examples** | `ai-build`, `explore`, `plan` | `researcher`, `concise` |
...
Ai Build discovers file-based personas from these locations, in priority order:

- `.ai/personas/*.toml` (project)
- `~/.ai/personas/*.toml` (user)
...
> **Note:** Ai Build applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Ai Build resolves the effective model and reasoning effort in this order, highest priority first:
...
Ai Build manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "ai-build"               # route explore to a specific model
```
...
# instructions_file = ".ai/personas/concise.md"  # or load from a file
```

Ai Build also discovers roles from `.ai/roles/*.toml` and personas from `.ai/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Ai Build shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · ai-3) — Thinking`
