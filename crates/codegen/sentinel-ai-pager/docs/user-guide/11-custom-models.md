...
Ai connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.

---

## Default Models

By default, Ai uses models hosted by SpaceAI, and new sessions start with `ai-4.5`. Default models require no configuration. Authenticate with `ai login` or an API key, then start a session.
...
Set a persistent default in `~/.ai/config.toml`:
...
Ai supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, Ai uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. Ai sends those headers verbatim with every request to the endpoint.

---

## Configuring Custom Models

Add custom model endpoints in `~/.ai/config.toml` under `[model.<name>]` sections:
...
env_key = "AI_API_KEY"                   # Env var holding the API key (optional; string or array)
...
Ai resolves the API key in this order:
...
3. Your signed-in session token (from `ai login`), for a model with no `api_key`/`env_key` of its own
4. The `AI_API_KEY` environment variable (global fallback; Ai also accepts `AI_CODE_AI_API_KEY` for backward compatibility)

### Context Window

The `context_window` value tells Ai when to trigger auto-compaction. When you override a known model, Ai inherits that model's context window. When you define a new model and omit `context_window`, Ai defaults to 200,000 tokens, so set it explicitly to match your provider.
...
Some gateways route or version on the query string. `query_params` appends percent-encoded query parameters to every request Ai makes for a model. For example, a gateway that selects an API version this way:
...
When you override a built-in model, Ai starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which Ai sends verbatim.
...
Point Ai at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `AI_MODELS_BASE_URL` | Yes | Base URL for inference. Ai fetches the model list from `{base_url}/models`. |
| `AI_API_KEY` | Yes | API key sent as `Authorization: Bearer`. Ai also accepts `AI_CODE_AI_API_KEY`. |
| `AI_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, Ai inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, Ai uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `ai login` -- the API key is enough.
...
```bash
export AI_WEB_SEARCH_MODEL="ai-4.5"
```

If you point web search at a custom model, you also need a `[model.*]` entry so Ai can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:
