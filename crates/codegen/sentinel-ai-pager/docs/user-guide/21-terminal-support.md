...
Ai Build runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in Ai to check the current session and see available fixes. If
Ai cannot start, run `ai doctor` in your shell. Use `ai doctor --json`
...
`ai doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `ai doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. Ai preserves the file's
...
Ai deliberately does **not** run `tmux source-file` or change the live tmux
...
Ai detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach Ai.
...
Inside tmux there are two separate questions: what color Ai emits, and what
...
Ai writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when Ai runs inside tmux.
...
terminal focused. Older compositors may require Ai to remain focused until
the copy message appears. Ai shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`AI_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

Ai emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `AI_CLIPBOARD_NO_OSC52=1` before starting Ai to
...
A remote Ai process normally cannot read the local terminal's selection. Use
...
When Ai cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.ai/last-copy.txt` by
default; override with `AI_COPY_FILE`); the toast names that path when delivery
...
through `ai wrap`, for example `ai wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `ai wrap`, Ai shows the one-time tip
...
`[ui.contextual_hints]` in `$AI_HOME/config.toml`. This setting does not hide
the Doctor recommendation.

For repeated SSH use, Doctor offers `ai doctor fix ssh-wrap`. It also shows
...
> **Warning**: `ai wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. Ai normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.ai/pager.toml`, or run
`ai --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with Ai

Zellij can intercept Ctrl/Alt keys before they reach Ai. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach Ai, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in Ai.
...
keys. Ai therefore does not negotiate the protocol there, and Shift+Enter can
...
If Ai stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, Ai stops capture and shows
...
to the terminal hosting Ai. Open **System Settings → Privacy & Security →
...
Run `ai doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone Ai would use. If no input device is available, Doctor
...
problem, set `AI_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
