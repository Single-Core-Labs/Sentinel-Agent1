...
> **Status: alpha.** The schema below is versioned (`ai_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

Ai CLI can export usage **metrics** and **events** to your organization's
...
any data flowing through SpaceAI.
...
|---------|---------------|
| Telemetry master switch | `[features] telemetry` / `AI_TELEMETRY_ENABLED` |
| Coding data, retention, and training | Settings — `/privacy` opens the row |
| Trace upload | `[telemetry] trace_upload` / `AI_TELEMETRY_TRACE_UPLOAD` |
| External OpenTelemetry | `AI_EXTERNAL_OTEL` / `[telemetry] otel_*` (this guide) |
...
- **Structurally separate** from SpaceAI-internal telemetry: its exporters carry
  only the headers you configure, never SpaceAI credentials.
- **Independent of SpaceAI data-retention opt-outs**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams. Those
  settings govern SpaceAI-side retention; the external stream is governed solely
  by your own OTEL configuration.

## Quick start

```bash
export AI_EXTERNAL_OTEL=1                  # master switch
...
`AI_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `AI_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `AI_TELEMETRY_ENABLED`, which controls SpaceAI-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `ai_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `AI_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`AI_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `ai setup` managed config /
...
Because AI can force-disable this stream fleet-wide, the CLI holds emission
...
**The wait is bounded**, so a deployment that cannot reach AI still exports:

- If no fleet policy can apply at all — `[features] remote_fetch = false`, or
  `[endpoints] cli_chat_proxy_base_url` points somewhere other than AI — the
...
(`ai --debug`) for `external otel:` lines — they record whether the stream
...
## Metrics (meter scope `ai.sentinel.ai_code`)

| Metric | Unit | Attributes |
|---|---|---|
| `ai_code.session.count` | `{session}` | base attrs only |
| `ai_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `ai_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `ai_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `ai_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `ai_code.error.count` | `{error}` | `error_category`, `model` |
| `ai_code.startup.total` | `ms` | `outcome` = `ok` \| `timeout` \| `error`; `auth_mode` |
| `ai_code.startup.phase_duration` | `ms` | `phase`, `outcome`, `auth_mode` |
| `ai_code.startup.timeout` | `{timeout}` | `stuck_in`, `auth_mode` |
...
There is no `cost.usage` metric: join `ai_code.token.usage` with your own
...
|---|---|
| `ai_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `ai_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `ai_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `ai_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `ai_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `ai_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `ai_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `ai_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `ai_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `ai_code.permission_mode_changed` | `to_mode`, `trigger` |
| `ai_code.skill_activated` | `skill_source`, `trigger` = `slash_command` \| `skill_md_read` \| `skill_tool`; `skill.name` (**details**) |
| `ai_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `ai_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `ai_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `ai_code.auth` | `auth_method` |
| `ai_code.internal_error` | `error_type` (class only — no message, no location) |
| `ai_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(ai_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(ai_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(ai_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(ai_code_tool_decision_total[1h]))
```
