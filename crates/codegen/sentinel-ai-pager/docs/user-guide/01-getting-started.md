...
Ai Build is a terminal-based AI coding assistant from SpaceAI. It runs as a TUI (Terminal User Interface) that understands your codebase, executes shell commands, edits files, searches the web, and manages tasks.
...
```powershell
$env:AI_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex
```

The PowerShell installer automatically adds `%USERPROFILE%\.ai\bin` to your User PATH. Alternatively, install via [Git for Windows](https://gitforwindows.org/) (Git Bash) or MSYS2 using the bash script above. WSL users get the Linux binary automatically.
...
On first launch, Ai opens your browser to authenticate with ai.com. After you sign in, Ai stores your credentials in `~/.ai/auth.json`, where they persist across sessions. Ai refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.

If you prefer API key authentication (e.g., for CI/CD or environments without a browser), set the `AI_API_KEY` environment variable instead:
...
Once authenticated, Ai presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, Ai's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. Ai reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, Ai asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `ai --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.ai/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `ai -c`
...
- **Agent messages** -- Ai's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- Ai's reasoning process (collapsible)
...
ai --resume <session-id>

# Continue the most recent session
ai -c

# Experimental scrollback-native render mode. Sticky: plain `ai` reopens in
...
Run Ai non-interactively for scripting, CI/CD, and automation:
...
```bash
ai -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. Ai reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.ai/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. Ai also reads `CLAUDE.md` files for compatibility.
