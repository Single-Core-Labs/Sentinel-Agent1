...
Hooks let you run a script or send an HTTP request at key moments in a Ai session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that Ai calls when a specific lifecycle event occurs. Hooks can:
...
2. Create a hook file, e.g. `~/.ai/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'Ai session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a Ai session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.ai/hooks/*.json` | Always | Personal hooks |
...
| Project | `<project>/.ai/hooks/*.json` | Requires trust | Per-repo automation |
...
| Config | `~/.ai/config.toml` | Always | Your hooks alongside the rest of your config |
| Config | `managed_config.toml` (`$AI_HOME` and `/etc/ai`) | Always | Organization-distributed hooks (server-synced and on-device) |
...
Config-file hooks live in the same TOML your organization already controls; see [Hooks in Config Files](#hooks-in-config-files) for the format. The compatible vendor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.ai/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run -- until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.ai/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.ai/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.

Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and cascades to subdirectories. Conversely, disabling folder-trust (`AI_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
Ai accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). Ai skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, Ai maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
Hooks can also live directly in your Ai config, so a team can distribute them with the rest of their configuration instead of shipping separate JSON files. The same `hooks` object is read from three TOML files:

| File | Tier | Who sets it |
|------|------|-------------|
| `~/.ai/config.toml` | User | You |
| `managed_config.toml` (`$AI_HOME`, `/etc/ai`) | Managed / system | Your organization |
...
- **camelCase input**: ai's stdin envelope uses camelCase keys throughout where Claude uses snake_case. A script reading `.stop_hook_active`, `.hook_event_name`, or `.background_tasks[].agent_type` must switch to `.stopHookActive`, `.hookEventName`, and `.backgroundTasks[].agentType` (the event value is `"stop"`). Hooks registered through the ai-agent-sdk convert both the top-level keys and the `backgroundTasks`/`sessionCrons` entry keys to snake_case, so the wire's `.backgroundTasks[].agentType` reads as `.background_tasks[].agent_type` in the SDK.
...
- **StopFailure classes**: the emitted set is Claude Code's vocabulary — `rate_limit`, `authentication_failed`, `invalid_request`, `server_error`, `max_output_tokens`, `unknown`. ai emits a subset: capacity errors (503/529) fold into `rate_limit` as in Claude, and `billing_error` is never emitted (no signal), so a `billing_error` matcher will not fire.
- **permission_mode values**: ai emits `default`, `auto`, `plan`, or `bypassPermissions`. Claude's `acceptEdits`/`dontAsk` have no ai equivalent (ai's `auto` is the nearest), so a check like `permission_mode === "acceptEdits"` never matches.
...
- **`/goal`**: ai's goal loop is a separate feature that runs before the stop gate; it is not a prompt-type Stop hook.
...
Ai sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `AI_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `AI_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `AI_SESSION_ID`     | The unique identifier of the current Ai session. |
| `AI_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `AI_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, Ai additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `AI_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `AI_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`AI_PLUGIN_ROOT`, `AI_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
```json
{ "type": "http", "url": "https://hooks.example.com/ai-event", "timeout": 15 }
```
...
Press `r` in the Hooks tab to reload all hooks from disk. Ai re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.ai/hooks/`) run with your user permissions -- treat them like shell scripts.
...
5. **Version control project hooks** -- commit `.ai/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug AI_LOG_FILE=/tmp/ai.log ai`, then check `/tmp/ai.log`.
