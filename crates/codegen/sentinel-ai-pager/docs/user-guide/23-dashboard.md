...
- **`ai dashboard`** — launch the TUI into the dashboard.
- **`/dashboard`** (aliases **`/agents-dashboard`**, **`/sessions`**) — open
  from inside a session.
- **`Ctrl+\`** — same view as the slash command.

Hidden in minimal mode. Set `AI_AGENT_DASHBOARD=0` or
...
```
 Ai Build · Dashboard — 4 agents · 2 awaiting
...
State icons match other session lists in Ai Build:
...
Per-user preferences under `[dashboard]` in `~/.ai/config.toml`:
