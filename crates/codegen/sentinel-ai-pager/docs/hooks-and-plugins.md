...
Ai Build supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.ai/hooks/`
- **Project hooks** — from `.ai/hooks/` in your repo
...
Plugin hooks automatically receive `AI_PLUGIN_ROOT` and `AI_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.ai/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `ai plugin marketplace add <source>`)
...
Sources land in `~/.ai/config.toml`:
...
Or in `~/.ai/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../sentinel-ai-hooks/examples/README.md) — ready-to-use sample hooks
