...
Ai asks before doing anything risky — and can plan before it codes.

## Permissions

When Ai wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: Ai explores the
...
`s` requests changes — Ai iterates until you're happy, then implements.
...
background — Ai keeps working and you're notified when it finishes
