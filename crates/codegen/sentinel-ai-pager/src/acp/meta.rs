...
//! ai-shell hasn't been updated or meta is absent.
...
pub struct NotificationMeta {
    ...
    /// (`"{sessionId}-{counter}"`, see `sentinel-ai-shell util::event_id`). The
...
/// replayed notifications (`_meta.isReplay`, stamped by sentinel-ai-shell's
...
pub mod user_prompt_meta {
    ...
    /// See [`sentinel_prompt_queue::COMBINED_DISPLAY_TEXTS_META`].
    pub const COMBINED_DISPLAY_TEXTS: &str = sentinel_prompt_queue::COMBINED_DISPLAY_TEXTS_META;
}
...
pub mod user_message_chunk_meta {
    ...
    /// ([`sentinel_ai_shell::session::PromptOrigin::hide_user_echo_from_scrollback`]).
...
mod tests {
    ...
    fn parse_missing_new_fields() {
        // Simulate old ai-shell that doesn't send streamStartMs/turnStartMs
        let meta_json = json!({
            "totalTokens": 1000u64,
            "agentTimestampMs": 1700000000000i64,
        });
