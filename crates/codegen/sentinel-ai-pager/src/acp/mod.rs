...
use sentinel_ai_telemetry::startup;
pub use sentinel_ai_telemetry::startup::{
...
use sentinel_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
use sentinel_ai_shell::agent::auth_method::AuthMethodKind;
use sentinel_ai_shell::agent::config::Config as AgentConfig;
use sentinel_ai_shell::sampling::types::ReasoningEffort;
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a ai-shell instance.
    pub is_ai_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<sentinel_ai_announcements::RemoteAnnouncement>>
    /// Whether interactive login is required (deferred auth for `ai.com`).
    ...
    pub auth_manager: std::sync::Arc<sentinel_ai_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<sentinel_ai_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_ai_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    startup::set_auth_mode(sentinel_ai_shell::managed_config::classify_auth_mode());
    let mut agent_config = AgentConfig::new_from_toml_cfg(raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    let auth_manager = std::sync::Arc::new(sentinel_ai_shell::auth::AuthManager::new(
    ...
    sentinel_ai_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = sentinel_ai_shell::util::ai_home::ai_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a ai-shell agent
    let is_ai_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("aiShell"))
...
///
/// Matches TUI startup behavior: if the first method is `ai.com`, defer auth
...
/// for a `ai.com` or `oidc` method — these are the ones that can trigger
...
/// When `sentinel.api_key` was advertised, non-interactive credentials were
...
mod tests {
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "aiShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "aiShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    #[test]
    fn startup_auth_ai_com_no_provider_needs_login_pending() {
        let methods = vec![make_auth_method("ai.com", "ai.com", None)];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
        assert_eq!(label.as_deref(), Some("ai.com"));
        assert_eq!(method_id.as_ref().unwrap().0.as_ref(), "ai.com");
        assert_eq!(mode, AuthStartMode::Pending);
    }

    #[test]
    fn startup_auth_ai_com_with_external_provider_command() {
        let meta = serde_json::json!({ "external_provider": true });
        let methods = vec![make_auth_method("ai.com", "Acme Corp", Some(meta))];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
    ...
    #[test]
    fn startup_auth_non_ai_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.ai/config.toml` with a
    ...
    /// change re-orders `build_auth_methods()` to put `sentinel.api_key` anywhere
    ...
    /// (`agent::auth_method::tests::enterprise_byok_first_method_is_sentinel_api_key`
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use sentinel_ai_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};

        let built = build_auth_methods(AuthMethodsBuildInputs {
            // enterprise-style: model has `env_key` set and the env var resolves,
            // so the shell-side predicate returns true.
            has_external_api_key: true,
            // Realistic enterprise user: no cached session token, default `ai.com`
        ...
        assert!(
            ...
            "shell built auth_methods for a BYOK user, but the pager still \
             ...
             sentinel.api_key first or the pager stopped treating sentinel.api_key as \
             a no-login method.",
        );
        assert!(label.is_none());
        assert!(method_id.is_none());
    ...
    /// Inverse direction: when `sentinel.api_key` is NOT in the list, the pager
    /// MUST show the login screen. We assert this with `sentinel.api_key` present
    ...
    /// `startup_auth_ai_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_sentinel_api_key_not_first_still_requires_login() {
        use sentinel_ai_shell::agent::auth_method::{AI_COM_METHOD_ID, AI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(AI_COM_METHOD_ID, "Ai", None),
            make_auth_method(AI_API_KEY_METHOD_ID, "sentinel.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with ai.com first, the pager must require login -- pinning \
             the BAD-ordering failure mode (sentinel.api_key not first)",
        );
    }

    #[test]
    fn startup_auth_method_id_is_copied_not_synthesized() {
        let methods = vec![make_auth_method("ai.com", "My Login", None)];
    ...
    fn startup_auth_external_provider_false_is_pending() {
        ...
        let methods = vec![make_auth_method("ai.com", "ai.com", Some(meta))];
