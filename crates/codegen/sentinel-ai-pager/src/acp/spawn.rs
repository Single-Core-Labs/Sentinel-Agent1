...
//!
//! Simplified to only support AiShell (in-process) mode.
...
///
/// Hold one from every site that calls [`spawn_ai_shell`] (headless, the TUI,
...
/// Spawn a AiShell agent in a background thread.
pub async fn spawn_ai_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<sentinel_ai_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    sentinel_ai_shell::agent::app::apply_otel_config(&auth_manager, &agent_config.ai_com_config);
    ...
    sentinel_ai_shell::managed_config::ensure_managed_policy_present(&auth_manager).await;
    ...
    let (agent_config, models_manager) =
        sentinel_ai_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
            ...
            local.block_on(&rt, async move {
                ...
                let _skills_watcher = {
                    ...
                    let workspace_user_dir =
                        sentinel_ai_agent::prompt::workspace_user::optional_workspace_user_dir();
                    sentinel_ai_shell::config::watcher::SkillsFileWatcher::start(
                        Some(cwd.as_path()),
                    ...
                    .map(|(mut watcher, mut skills_rx)| {
                        ...
                        tokio::task::spawn_local(async move {
                            while let Some(change) = skills_rx.recv().await {
                                ...
                                match change {
                                    sentinel_ai_shell::config::watcher::DiscoveryChange::Skills => {
                                        tracing::info!(
                                    ...
                                    sentinel_ai_shell::config::watcher::DiscoveryChange::Workflows => {
                                        tracing::info!(
