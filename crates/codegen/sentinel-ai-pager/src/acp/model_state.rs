...
impl ModelState {
    ...
    /// Machine-readable model ID string for the current model (e.g. "ai-4.5").
    ...
    /// correct today (all current Ai models accept images, so nothing is
    ...
    /// not offer (e.g. `none` on ai-4.5) are rejected so we fail in the TUI
...
mod tests {
    ...
    fn update_catalog_preserves_user_effort_when_model_unchanged() {
        let id = acp::ModelId::new(Arc::from("ai-build"));
        let mut state = ModelState::default();
        state.available.insert(
            id.clone(),
            model_with_effort("ai-build", "Ai Build", "high"),
        );
