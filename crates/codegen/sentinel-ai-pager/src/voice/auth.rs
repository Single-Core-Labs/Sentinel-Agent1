...
use sentinel_ai_tools::types::SharedApiKeyProvider;
use sentinel_ai_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
///
/// Works for every auth method: OAuth / ai.com / OIDC session tokens and
/// `AI_API_KEY` / per-model BYOK keys.
pub fn build_voice_auth(auth_manager: Arc<sentinel_ai_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        sentinel_ai_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
