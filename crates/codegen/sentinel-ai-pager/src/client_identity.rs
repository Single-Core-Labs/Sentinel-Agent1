pub const PAGER_CLIENT_TYPE: &str = "ai-pager";
pub const HEADLESS_CLIENT_TYPE: &str = "ai-shell";

pub const PAGER_CLIENT_VERSION: &str = sentinel_ai_version::VERSION;

/// `User-Agent` for pager-owned direct-to-`api.x.ai` clients (voice STT).
///
/// Matches the sampler's `ai-shell/<version> (os; arch)` shape so server-side
...
mod tests {
    ...
    fn client_user_agent_has_expected_shape() {
        // e.g. "ai-shell/1.2.3 (macos; aarch64)". The pieces are wire
