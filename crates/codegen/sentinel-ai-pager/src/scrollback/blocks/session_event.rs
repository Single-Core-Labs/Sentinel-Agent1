...
pub enum SessionEvent {
    ...
    /// Message comes from agent via AiSessionUpdate::HookAnnotation.
    HookAnnotation {
        /// The hook message
        message: String,
    },
    ...
    pub fn message(&self) -> String {
        match self {
            ...
            SessionEvent::DiskFull => {
                sentinel_ai_shell::extensions::notification::DISK_FULL_USER_MESSAGE.to_string()
            }
...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "ai-4.5".into(),
            new_model_id: "ai-build".into(),
            reason: "Model \"ai-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"ai-4.5\" is no longer available. Switched to \"ai-build\"."
        );
    }
