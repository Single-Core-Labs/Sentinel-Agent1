...
//! the generic [`CodeBlockSpan`](sentinel_ai_markdown::CodeBlockSpan) API) and
...
use sentinel_ai_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](sentinel_ai_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](sentinel_ai_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a sentinel_ai_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `AiDay` is the only light theme; every other concrete theme (and the
/// `AiNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `sentinel_ai_mermaid::MermaidTheme`; it
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_aiday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::AiDay),
            "AiDay is the light theme"
        );
        for dark in [
            ThemeKind::AiNight,
