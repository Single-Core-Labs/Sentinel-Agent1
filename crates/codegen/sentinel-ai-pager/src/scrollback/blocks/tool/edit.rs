...
mod tests {
    ...
    fn workflow_script_header_hides_rhai_path() {
        let theme = Theme::current();
        let block = EditToolCallBlock::new(".ai/workflows/cc-deep-research.rhai", vec![]);
        let header = block.header_line(
            &theme,
            false,
            false,
        ...
        let block =
            EditToolCallBlock::new(".ai/workflows/triage.rhai", vec![]).with_prefix("Creating ");
        let header = block.header_line(
            &theme,
            false,
            false,
    ...
    fn test_diff_reflow_preserves_banded_syntect_styles_across_wrap() {
        let _guard = pin_ainight_syntect();
    ...
    /// Pins AiNight via the shared test-lock guard; hold it for the whole
    /// test so a concurrent theme flip can't skew the compared highlighter walks.
    fn pin_ainight_syntect() -> std::sync::MutexGuard<'static, ()> {
        let guard = crate::theme::cache::pin_theme();
        assert!(
            !Theme::ainight().diff_uses_line_fg(),
            "AiNight must be banded for Equal-line syntect path in render"
        );
        guard
    }
    ...
    /// Caller pins the theme via `pin_ainight_syntect` (lock is not reentrant).
