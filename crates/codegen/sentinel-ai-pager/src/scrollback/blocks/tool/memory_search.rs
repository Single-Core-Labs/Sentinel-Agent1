...
fn shorten_path(path: &str) -> &str {
    let memory_root = sentinel_ai_config::ai_home().join("memory");
...
**File:** /root/.ai/memory/MEMORY.md (lines 0-10)
```
## Project Conventions
...
"#;
        ...
        assert_eq!(results[0].path, "/root/.ai/memory/MEMORY.md");
...
**File:** /root/.ai/memory/ws/MEMORY.md (lines 1-5)
```
workspace content
```

### Result 2 (score: 0.42, source: session)
**File:** /root/.ai/memory/ws/sessions/2026-05-01.md (lines 10-20)
```
session content
```
"#;
    ...
    fn shorten_memory_path() {
        // Paths under the configured ai memory root keep one trailing segment group.
        let memory_root = sentinel_ai_config::ai_home().join("memory");
        let session = memory_root.join("sentinel-50aa78f0/sessions/2026-05-01.md");
