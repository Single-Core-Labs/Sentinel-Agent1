...
use sentinel_fast_worktree::WorktreeRecord;

use agent_client_protocol as acp;
use sentinel_acp_lib::acp_send;
...
pub async fn run(args: WorktreeArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    sentinel_ai_telemetry::startup::mark_utility_process();
    let spawned = crate::acp::spawn::spawn_ai_shell(agent_config.clone(), &cancel, None).await?;
...
async fn dispatch(command: WorktreeCommand, tx: &sentinel_acp_lib::AcpAgentTx) -> Result<()> {
    match command {
...
async fn cmd_show(tx: &sentinel_acp_lib::AcpAgentTx, id_or_path: &str) -> Result<()> {
...
async fn cmd_db(tx: &sentinel_acp_lib::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
...
mod tests {
    ...
    fn ext_request_builds_list_with_filters() {
        ...
        assert_eq!(params["repo"], "sentinel");
    ...
    fn ext_envelope_unwraps_success_result() {
        let json = r#"{"result": {"path": "/home/user/.ai/worktrees.db"}, "error": null}"#;
        #[derive(serde::Deserialize)]
        struct PathResp {
            path: String,
        }
        ...
        assert_eq!(inner.path, "/home/user/.ai/worktrees.db");
    }
