...
//! render mode.
//!
//! `sentinel-ai-pager` cannot depend on `sentinel-ai-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`sentinel-ai-pager-bin`) wires it up once at
//! startup by calling `sentinel_ai_pager_minimal::install()`. When nothing has
...
/// (`sentinel_ai_pager_minimal::draw`).
