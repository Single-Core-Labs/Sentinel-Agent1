//! Streaming output filter for `ai wrap`: OSC 52 clipboard interception,
...
///
/// Delegates to [`sentinel_ai_shell::util::clipboard::set_text`] which uses
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = sentinel_ai_shell::util::clipboard::set_text(text) {
...
pub(crate) fn host_clipboard_image_frame() -> Vec<u8> {
    let image = sentinel_ai_pager_render::clipboard::system_clipboard_get_image();
