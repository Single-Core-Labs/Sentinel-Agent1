...
use sentinel_ai_shell::auth::{AuthManager, try_ensure_fresh_auth};
use sentinel_ai_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // deployments that only configure a deployment_key + custom sentinel_api_base_url.
    // If the user has previously run the interactive `ai` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.ai_com_config).await;
    ...
    let client = sentinel_ai_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = sentinel_ai_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                sentinel_ai_shell::session::merge::CwdScope::WithSiblings,
                None,
                limit,
            )
            .await;
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use sentinel_ai_shell::session::merge::REMOTE_TIMEOUT;
            use sentinel_ai_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = sentinel_ai_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
