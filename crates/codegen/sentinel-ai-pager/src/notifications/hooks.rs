...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("AI_EVENT", event_str)
        .env("AI_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("AI_SESSION_ID", sid);
    }
...
fn attach_to_global_scope(child: &std::process::Child) -> Option<Arc<sentinel_tty_utils::ProcessGroup>> {
    ...
    sentinel_tty_utils::global_process_scope()
        .register(&group)
        .then_some(group)
}
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'AI_EVENT=%s\\nAI_MESSAGE=%s\\nAI_SESSION_ID=%s\\n' \
             \"$AI_EVENT\" \"$AI_MESSAGE\" \"$AI_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("AI_EVENT=Turn complete"),
            "missing AI_EVENT: {content}"
        );
        assert!(
            content.contains("AI_MESSAGE=hello world"),
            "missing AI_MESSAGE: {content}"
        );
        assert!(
            content.contains("AI_SESSION_ID=sess-42"),
            "missing AI_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("AI_SESSION_ID"),
            "AI_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("AI_EVENT=Turn complete"));
        assert!(content.contains("AI_MESSAGE=test body payload"));
        assert!(content.contains("AI_SESSION_ID=test-session-123"));
    }
}
