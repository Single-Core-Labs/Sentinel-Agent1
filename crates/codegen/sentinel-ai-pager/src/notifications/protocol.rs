...
pub fn select_protocol(ctx: &TerminalContext) -> NotificationProtocol {
    ...
    match ctx.brand {
        ...
        TerminalName::AiDesktop => NotificationProtocol::None,
...
fn notification_sequence(
    ...
    Some(match protocol {
        ...
        // OSC 777 already uses the tab title as subtitle, so keep "Ai".
        NotificationProtocol::Osc9 => format!("\x1b]9;{body} \u{b7} {title}\x07").into(),
        NotificationProtocol::Osc99 => format!("\x1b]99;i=ai;{body} \u{b7} {title}\x1b\\").into(),
        NotificationProtocol::Osc777 => format!("\x1b]777;notify;Ai;{body}\x1b\\").into(),
...
mod tests {
    ...
    #[test]
    fn select_ai_desktop_uses_none() {
        assert_eq!(
            select_protocol(&ctx_with_brand(TerminalName::AiDesktop)),
    ...
    fn emit_none_is_noop() {
        let ctx = ctx_with_brand(TerminalName::AiDesktop);
