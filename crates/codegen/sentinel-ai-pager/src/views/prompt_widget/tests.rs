#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/ai-test-image.png"));
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/ai-test-image.png"),
    ...
    fn accept_inside_command_with_args_absorbs_existing_separator() {
        ...
        pw.textarea.insert_str("/mod ai-4");
        pw.textarea.set_cursor(3);
        pw.refresh_slash(&models);
    ...
    fn accept_arg_completion_replaces_arg_range() {
        ...
        let model_id = agent_client_protocol::ModelId::new(Arc::from("ai-4.5"));
        models.available.insert(
            model_id.clone(),
            agent_client_protocol::ModelInfo::new(model_id, "Ai 4.5".to_string()),
        );
        ...
        // Accept arg completion → should replace "gr" with "Ai 4.5".
        pw.accept_slash_completion(&models);
        let text = pw.textarea.text().to_string();
        assert!(
            text.contains("Ai 4.5"),
    ...
    fn test_image() -> PastedImage {
        PastedImage {
            element_id: sentinel_ratatui_textarea::ElementId::from_raw(0), // overwritten by insert_image
            display_number: 0,
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> sentinel_ai_workspace::file_system::FuzzyMatchResult {
        sentinel_ai_workspace::file_system::FuzzyMatchResult {
