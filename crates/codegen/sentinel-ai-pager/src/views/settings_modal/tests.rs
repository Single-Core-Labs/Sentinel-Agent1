...
fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
    ...
    let snapshot = PagerLocalSnapshot {
        available_models: vec![(
            "Ai Test".to_string(),
            acp::ModelId::new(Arc::from("ai-test")),
        )],
        ..PagerLocalSnapshot::default()
    };
...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("Ai Test", 4); // cursor between "Ai" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn picking_enum_esc_returns_to_browse() {
    let mut s = make_state();
    s.transition_to_picking_enum("theme", 0, SettingValue::Enum("ainight"), true);
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"ainight\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode(), SettingsModalMode::Browse));
}
...
fn fork_secondary_model_picker_opens_on_persisted_model() {
    ...
    let slug = "ai-4.5-fast";
    assert_ne!(slug, sentinel_ai_shell::models::default_model());
    let snapshot = PagerLocalSnapshot {
        available_models: vec![
            ("Ai 3".to_string(), acp::ModelId::new(Arc::from("ai-3"))),
            (
                "Ai 4.5 Fast".to_string(),
    ...
    assert_eq!(
        ...
        Some(SettingValue::String("Ai 4.5 Fast".to_string())),
    );
    ...
    match s.mode() {
        SettingsModalMode::PickingEnum {
            ...
            // Choices: [(no override), Ai 3, Ai 4.5 Fast] → idx 2.
            assert_eq!(
                choices_idx, 2,
            ...
            assert_eq!(
                original_value,
                &SettingValue::String("Ai 4.5 Fast".to_string()),
...
fn picker_visual_smoke_debug() {
    let entries = vec![SettingMeta {
        ...
        description: "Controls whether SpaceAI may retain and train on coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-out",
            choices: &[
                EnumChoice {
                    canonical: "opt-in",
                    display: "Opt in",
                    description: "Allow SpaceAI to retain and use coding session data for training and product improvement.",
                },
                EnumChoice {
                    canonical: "opt-out",
                    display: "Opt out",
...
fn picker_multi_line_choice_hit_rect_spans_all_lines() {
    ...
    let entries = vec![SettingMeta {
        ...
        owner: SettingOwner::Shared,
        label: "Coding data sharing",
        description: "Controls whether SpaceAI may retain coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-in",
            choices: &[
...
fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
    ...
    assert!(
        ...
        "validation_error must be Some for unknown model 'a' \
         (catalog has 'Ai 4 Fast' only)",
    );

    // Enter on a buffer that fails the KnownModel validator
    // (catalog has 'Ai 4 Fast'; "a" doesn't match) is
...
fn string_editor_uses_canonical_edits_policy_and_live_validation() {
    ...
    let mut state = editor_render_fixture("Ai Tes", "Ai Tes".len());
    ...
    assert_eq!(state.editing_buffer(), Some("Ai Test"));
...
//    the "Tip · Ask Ai…" docs footer and the keybindings hints.
//
...
fn string_editor_paste_sanitizes_validates_and_consumes_rejected_text() {
    let mut state = editor_render_fixture("Ai Tst", "Ai T".len());
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip · Ask Ai to change a setting" (34 cells);
    ...
    assert!(
        !row_short.contains("aiday"),
        "width=40 must NOT render LONG path (no `aiday`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask Ai") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // `"ainight"`. Tightened from the previous `Action(_) |
    ...
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"ainight\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `ainight`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `ainight`; entering the picker
            // captures `original_value = current value = ainight`,
...
fn consent_chooser_drops_tip_and_reset() {
    ...
    assert!(
        text.contains("d reset") && text.contains("Ask Ai"),
...
/// misleading: on dark themes (AiNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the AiDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::AiNight => crate::theme::Theme::ainight(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::AiDay => crate::theme::Theme::aiday(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::ainight(),
    };
    assert_ne!(
