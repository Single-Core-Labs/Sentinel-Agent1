...
pub struct UsageInfoContext {
    ...
    /// Plan name for the allowance header (e.g. "SuperAi").
...
fn session_info_content(state: &UsageInfoModalState, theme: &Theme) -> TabContent {
    ...
    for row in text.lines() {
        ...
        // The auth method (and its `ai login` upsell) is deliberately
        // not part of this surface.
        if trimmed.is_empty()
            || trimmed.starts_with("Auth method:")
            || trimmed.starts_with("Run `ai login`")
        {
            continue;
        }
...
mod tests {
    ...
    fn state_with_session() -> UsageInfoModalState {
        UsageInfoModalState::new(
            ...
            UsageInfoContext {
                ...
                subscription_tier: Some("SuperAi".to_string()),
            },
        )
    }
    ...
    fn usage_limit_tab_shows_allowance_and_payg() {
        ...
        assert_eq!(text[0], "Weekly limit (SuperAi)");
