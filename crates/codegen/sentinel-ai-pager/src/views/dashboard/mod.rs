...
///
/// Order: env override (`AI_AGENT_DASHBOARD=0` → off) wins, else the
...
pub fn dashboard_enabled() -> bool {
    if std::env::var_os("AI_AGENT_DASHBOARD")
        .as_deref()
...
mod tests {
    ...
    /// `AI_AGENT_DASHBOARD` env-mutating tests.
    ...
    fn switch_hint_minimal_is_resume_even_with_dashboard_disabled() {
        ...
        // `AI_AGENT_DASHBOARD` key reads it concurrently.
    ...
    fn switch_hint_non_minimal_follows_dashboard_flag() {
        // SAFETY: see above — serialized on the AI_AGENT_DASHBOARD key.
        unsafe { std::env::set_var("AI_AGENT_DASHBOARD", "0") };
