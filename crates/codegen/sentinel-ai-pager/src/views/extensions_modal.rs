...
pub fn fuzzy_matches_hook(hook: &sentinel_hooks_plugins_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
fn hook_row_label(hook: &sentinel_hooks_plugins_types::HookInfo) -> String {
    let matcher = hook
        .matcher
        .as_deref()
        .map(|m| format!(" /{m}"))
...
fn is_official_marketplace_source(source: &sentinel_hooks_plugins_types::MarketplaceScanResult) -> bool {
    source.source_name == sentinel_ai_plugin_marketplace::OFFICIAL_SOURCE_NAME
        || sentinel_ai_plugin_marketplace::is_official_source_url(&source.source_url_or_path)
}
...
pub(crate) fn ordered_marketplace_view(
    sources: &[sentinel_hooks_plugins_types::MarketplaceScanResult],
...
fn skill_group(skill: &SkillInfo) -> SkillGroup {
    use sentinel_ai_tools::implementations::skills::types::SkillScope;
    use sentinel_ai_tools::types::config_source::ConfigSource;
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<sentinel_hooks_plugins_types::PluginOrigin>,
) -> sentinel_hooks_plugins_types::PluginInfo {
    sentinel_hooks_plugins_types::PluginInfo {
        name: name.to_string(),
        id: format!("user/abcd1234/{name}"),
        root: format!("/tmp/{name}"),
        scope: sentinel_hooks_plugins_types::PluginScope::User,
        trusted: true,
        enabled: true,
        version: None,
        description: None,
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: sentinel_hooks_plugins_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
        mcp_status: sentinel_hooks_plugins_types::McpStatus::None,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    PluginGroupSortKey,
    Vec<(usize, &'a sentinel_hooks_plugins_types::PluginInfo)>,
>;
...
/// sensible groups.
pub fn plugin_group(plugin: &sentinel_hooks_plugins_types::PluginInfo) -> PluginGroup {
    use sentinel_hooks_plugins_types::{PluginOrigin, PluginScope};

    match &plugin.origin {
        Some(PluginOrigin::ProjectAi) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserAi) => PluginGroup::new(2, "origin:user", "User"),
...
fn build_hook_groups<'a>(
    hooks: &'a [sentinel_hooks_plugins_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<HookGroupView<'a>> {
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> sentinel_ai_telemetry::events::ExtensionsModalTab {
        use sentinel_ai_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    HooksAction(sentinel_hooks_plugins_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(sentinel_hooks_plugins_types::PluginsAction),
    ...
    AddMcpServer {
        name: String,
        config: Box<sentinel_ai_shell::util::config::McpServerConfig>,
    },
    ...
    RefreshMcpList,
    /// Open ai.com connectors page (MCP tab: press `o`).
    ...
    MarketplaceAction(sentinel_hooks_plugins_types::MarketplaceAction),
...
impl ModalInputField {
    ...
    pub(crate) fn viewport(&self, width: usize) -> sentinel_ratatui_textarea::SingleLineViewport {
...
pub enum ConfirmationAction {
    ...
    Hooks(sentinel_hooks_plugins_types::HooksAction),
    ...
    Plugins(sentinel_hooks_plugins_types::PluginsAction),
    /// Replay a marketplace action (uninstall plugin or remove source).
    Marketplace(sentinel_hooks_plugins_types::MarketplaceAction),
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use sentinel_hooks_plugins_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use sentinel_hooks_plugins_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            sentinel_hooks_plugins_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use sentinel_ai_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<sentinel_hooks_plugins_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<sentinel_hooks_plugins_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<sentinel_hooks_plugins_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<sentinel_hooks_plugins_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[sentinel_hooks_plugins_types::PluginInfo]) {
...
fn classify_hook_source(source_dir: &str) -> HookSourceMeta {
    let ai = sentinel_ai_config::ai_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user ai home (AI_HOME-aware)
    // or a project-scoped `{cwd}/.ai/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User ai home (AI_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(ai.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.ai/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".ai" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    ...
    // Global hooks under $AI_HOME/hooks
    let global_hooks = ai.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.ai/hooks") || source_dir.contains("/.ai/hooks/") {
        return HookSourceMeta {
    ...
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&ai) {
            let prefix = crate::util::display_ai_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            sentinel_ai_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_ai_home(path) {
                    crate::util::display_user_ai_path("skills")
            ...
            sentinel_ai_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            sentinel_ai_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &sentinel_hooks_plugins_types::PluginInfo) -> Vec<String> {
    use sentinel_hooks_plugins_types::McpStatus;
...
fn component_categories(
    components: &sentinel_hooks_plugins_types::PluginComponents,
) -> [(&'static str, &[sentinel_hooks_plugins_types::ComponentItem]); 6] {
    use sentinel_hooks_plugins_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &sentinel_hooks_plugins_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.ai/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // ai-home branch is AI_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.ai/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.ai/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        let servers = vec![
            make_mcp_server_for_rows("ai_com_linear", McpWireSource::Managed, vec![]),
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by ai.com")),
            "managed section header must appear"
        );
        assert!(
            !rows.labels.iter().any(|l| l == "ai_com_linear"),
    ...
    fn make_skill(
        name: &str,
        desc: &str,
    ) -> sentinel_ai_tools::implementations::skills::types::SkillInfo {
        sentinel_ai_tools::implementations::skills::types::SkillInfo {
            name: name.to_string(),
            display_name: None,
            ...
            metadata: None,
            path: "test".to_string(),
            scope: sentinel_ai_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = sentinel_ai_tools::implementations::skills::types::SkillScope::Plugin;
    ...
    fn make_plugin(name: &str) -> sentinel_hooks_plugins_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: sentinel_hooks_plugins_types::PluginOrigin,
    ) -> sentinel_hooks_plugins_types::PluginInfo {
    ...
    fn make_plugin_with_enabled(name: &str, enabled: bool) -> sentinel_hooks_plugins_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(sentinel_hooks_plugins_types::PluginsListResponse {
            plugins: vec![
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    sentinel_ai_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    sentinel_ai_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                sentinel_hooks_plugins_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(
        ...
        source_dir: &str,
        disabled: bool,
    ) -> sentinel_hooks_plugins_types::HookInfo {
        sentinel_hooks_plugins_types::HookInfo {
            name: name.to_string(),
            event: sentinel_hooks_plugins_types::HookEvent::PreToolUse,
            handler_type: sentinel_hooks_plugins_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn superpowers_source() -> sentinel_hooks_plugins_types::MarketplaceScanResult {
        sentinel_hooks_plugins_types::MarketplaceScanResult {
    ...
    struct TestPlugin {
        ...
        components: Option<sentinel_hooks_plugins_types::PluginComponents>,
    }
    ...
    impl TestPlugin {
        fn build(self) -> sentinel_hooks_plugins_types::MarketplacePluginEntry {
            ...
            sentinel_hooks_plugins_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                sentinel_hooks_plugins_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                sentinel_hooks_plugins_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        ...
        state.marketplace_data =
            TabDataState::Loaded(sentinel_hooks_plugins_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_renders_header_with_error_badge() {
        let error_source = sentinel_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> sentinel_hooks_plugins_types::ComponentItem {
        sentinel_hooks_plugins_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> sentinel_hooks_plugins_types::PluginComponents {
        sentinel_hooks_plugins_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(sentinel_hooks_plugins_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = sentinel_hooks_plugins_types::PluginComponents {
            skills: (0..12)
    ...
    fn marketplace_modal_state(
        source: sentinel_hooks_plugins_types::MarketplaceScanResult,
    ...
    fn plugins_modal_state(
        plugins: Vec<sentinel_hooks_plugins_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
            TabDataState::Loaded(sentinel_hooks_plugins_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugin_group_maps_each_origin_variant() {
        use sentinel_hooks_plugins_types::PluginOrigin;
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectAi, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserAi, 2, "origin:user", "User"),
            (
            ...
            (
                PluginOrigin::MarketplaceInstall {
                    source_name: Some("AI Official".into()),
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = sentinel_hooks_plugins_types::PluginScope::Project;
        ...
        cli.scope = sentinel_hooks_plugins_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = sentinel_hooks_plugins_types::PluginScope::Config;
        assert_eq!(plugin_group(&config).key, "origin:config");

        let mut mp = make_plugin("mp-tool");
        mp.marketplace_source = Some("AI Official".into());
        let group = plugin_group(&mp);
        assert_eq!(group.key, "ai-mp:AI Official");
        assert_eq!(group.label, "AI Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            sentinel_hooks_plugins_types::PluginOrigin::Unknown,
        );
        assert_eq!(plugin_group(&unknown).key, "origin:user");

        unknown.marketplace_source = Some("AI Official".into());
        assert_eq!(plugin_group(&unknown).key, "ai-mp:AI Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserAi),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use sentinel_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserAi),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use sentinel_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserAi);
    ...
    fn plugins_fallback_grouping_without_origin() {
        ...
        assert_eq!(buffer_count(&buf, "User (1 plugin)"), 1);
        assert_eq!(buffer_count(&buf, "AI Official (1 plugin)"), 1);
    ...
    fn marketplace_placeholders_render_only_when_expanded() {
        ...
        source.plugins[0].components = Some(sentinel_hooks_plugins_types::PluginComponents::default());
    ...
    fn plugins_sort_az_by_name_within_group() {
        use sentinel_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("Zebra", PluginOrigin::UserAi),
            make_plugin_with_origin("alpha", PluginOrigin::UserAi),
            make_plugin_with_origin("MID", PluginOrigin::UserAi),
        ]);
    ...
    fn ordered_marketplace_view_pins_official_then_az() {
        ...
        let sources = vec![
            ...
            mp(
                "AI Official",
                sentinel_ai_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
                None,
                &["zeta", "alpha"],
            ),
        ...
        let names: Vec<_> = view
            ...
            .collect();
        assert_eq!(names, ["AI Official", "alpha-mp", "zeta-mp"]);
    ...
    fn skills_groups_then_az_by_label() {
        ...
        project_z.scope = sentinel_ai_tools::implementations::skills::types::SkillScope::Local;
        project_z.config_source = Some(
            sentinel_ai_tools::types::config_source::ConfigSource::Project {
                path: std::path::PathBuf::from("/repo/.ai/skills/zzz"),
            },
        );
        project_z.display_name = Some("zeta-proj".into());

        let mut project_a = make_skill("aaa-proj", "other project");
        project_a.scope = sentinel_ai_tools::implementations::skills::types::SkillScope::Repo;
        project_a.config_source = Some(
            sentinel_ai_tools::types::config_source::ConfigSource::Project {
                path: std::path::PathBuf::from("/repo/.ai/skills/aaa"),
            },
        );
    ...
    fn hooks_group_and_row_order() {
        ...
        h_stop.event = sentinel_hooks_plugins_types::HookEvent::Stop;
        let mut h_pre = make_hook("pre-hook", "/tmp/hooks-src", false);
        h_pre.event = sentinel_hooks_plugins_types::HookEvent::PreToolUse;
        h_pre.matcher = Some("Bash".into());
        let mut h_notify = make_hook("notify-hook", "/tmp/hooks-src", false);
        h_notify.event = sentinel_hooks_plugins_types::HookEvent::Notification;

        let hooks = vec![
            make_hook("c", "/zzz/custom", false),
            h_stop,
            make_hook("a", "/repo/.ai/hooks", false),
            h_pre,
            h_notify,
