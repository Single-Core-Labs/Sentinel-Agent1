...
/// Env override for the whole feature: `AI_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "AI_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call:
/// `AI_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "AI_PROMPT_SUGGESTIONS_MODEL";
...
pub const PREFERRED_SUGGESTION_MODEL: &str = "ai-build-0.1";
...
pub struct PromptSuggestionController {
    ...
    /// `AI_PROMPT_SUGGESTIONS` env var, falling back to the persisted
...
pub fn resolve_enabled() -> bool {
    ...
    ENV_OVERRIDE
        .get_or_init(|| sentinel_ai_config::env_bool(PROMPT_SUGGESTIONS_ENV))
...
/// env override > `ai-build-0.1` when the catalog offers it > `None`.
///
/// The hint is one tier of the shell-side resolution (env > config.toml >
/// remote settings > this hint > `ai-build-0.1` default): the shell
