...
/// only one normal component (e.g., `/sentinel`), returns that component alone.
/// Does not perform tilde expansion — callers provide absolute paths.
/// Returns `"unknown"` for empty input.
///
/// Examples: `/home/user/fw/1` → `"fw-1"`, `/sentinel` → `"sentinel"`, `/` → `"/"`.
///
...
pub(crate) fn pending_delete_from_selection(
    ...
    content_results: Option<&[sentinel_ai_shell::extensions::session_search::SearchSessionHit]>,
...
/// matters because the fast foreign scan can land rows the default Ai view
...
///
/// Default is [`Self::Ai`]: native Ai sessions only (local / remote /
/// conversation), so `/resume` does not mix Claude/Codex/Cursor foreign
/// sessions into the list. `f` cycles Ai → External → All → Local →
/// Remote — External first so one press from the default reveals foreign
/// sessions.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SourceFilter {
    /// Native Ai sessions only — excludes Claude/Codex/Cursor foreign rows.
    #[default]
    Ai,
    Local,
    Remote,
...
impl SourceFilter {
    ...
    ///
    /// ai.com conversations carry `source == "conversation"` and live remotely,
    /// so they pass the `Remote` filter (and `Ai` / `All`) but not `Local`.
    /// Foreign sources (`claude` / `codex` / `cursor`) only pass `External` and
    /// `All`.
    pub fn matches(self, source: &str) -> bool {
        match self {
            Self::Ai => !crate::app::is_foreign_picker_source(source),
...
pub(crate) fn build_content_entry_data(
    hits: &[sentinel_ai_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
/// Hint shown on the default `Ai` view when the foreign-session scan loaded
/// Claude/Codex/Cursor entries it hides. Ai-only: `next(Ai) == External`
...
pub(crate) fn hidden_external_hint(
    ...
    if source_filter != SourceFilter::Ai {
        return None;
    }
    let hidden = entries?
        .iter()
...
mod tests {
    ...
    fn repo_name_from_cwd_standard_path() {
        assert_eq!(repo_name_from_cwd("/home/user/sentinel"), "user-sentinel");
    }
    ...
    fn repo_name_from_cwd_single_dir() {
        assert_eq!(repo_name_from_cwd("/sentinel"), "sentinel");
    }
    ...
    fn source_filter_matches() {
        // Default Ai filter: native only (not Claude/Codex/Cursor).
        assert!(SourceFilter::Ai.matches("local"));
        assert!(SourceFilter::Ai.matches("remote"));
        assert!(SourceFilter::Ai.matches("both"));
        assert!(SourceFilter::Ai.matches("conversation"));
        assert!(!SourceFilter::Ai.matches("claude"));
        assert!(!SourceFilter::Ai.matches("codex"));
        assert!(!SourceFilter::Ai.matches("cursor"));
        ...
        // ai.com conversations are remote: visible under Ai + All + Remote, not Local.
    ...
    fn source_filter_cycles() {
        ...
        assert_eq!(SourceFilter::Ai.next(), SourceFilter::External);
        ...
        assert_eq!(SourceFilter::Remote.next(), SourceFilter::Ai);
        assert_eq!(SourceFilter::Ai.label(), "Ai");
        assert_eq!(SourceFilter::External.label(), "External");
        assert_eq!(SourceFilter::default(), SourceFilter::Ai);
    }
    ...
    fn source_filter_filters_entries() {
        ...
        let ai = filter_session_entries(Some(&entries), "", SourceFilter::Ai);
        assert_eq!(ai, vec![0, 1, 2]); // local + remote + both, no foreign
    ...
    fn source_filter_empty_and_unknown_source() {
        ...
        // foreign, so it passes Ai + All but never Local, Remote, or External.
        assert!(SourceFilter::Ai.matches(""));
        ...
        assert!(SourceFilter::Ai.matches("unknown"));
    ...
    fn source_filter_is_active() {
        assert!(!SourceFilter::Ai.is_active());
    ...
    fn hidden_external_hint_visibility() {
        ...
        // Only the default Ai view surfaces the hint (with the count).
        assert_eq!(
            hidden_external_hint(Some(&entries), SourceFilter::Ai).as_deref(),
        ...
        assert_eq!(
            hidden_external_hint(Some(&one), SourceFilter::Ai).as_deref(),
        ...
        assert!(hidden_external_hint(Some(&native), SourceFilter::Ai).is_none());
        assert!(hidden_external_hint(None, SourceFilter::Ai).is_none());
    }
