...
use sentinel_ai_shell::claude_import::{ImportPlan, ImportableItem, PathKind, find_project_root};
use sentinel_ai_workspace::permission::types::RuleAction;
...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.ai/config.toml".to_string();
    ...
    if !plan.project_items.is_empty() {
        ...
        let project_config = find_project_root(cwd).join(".ai").join("config.toml");
...
mod tests {
    use super::*;
    use sentinel_ai_shell::claude_import::PathKind;
    use sentinel_ai_workspace::permission::types::{PatternMode, PermissionRule, ToolFilter};
    ...
    fn filtered_plan_respects_per_item_selection_after_grouping() {
        use sentinel_ai_shell::claude_import::ImportableItem;
