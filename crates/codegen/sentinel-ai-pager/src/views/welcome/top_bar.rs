...
mod tests {
    ...
    fn format_cwd_plain_repo() {
        assert_eq!(format_cwd_parts("~/sentinel", None), "~/sentinel");
    }
    ...
    fn format_cwd_worktree_shows_main_repo() {
        assert_eq!(
            format_cwd_parts("~/wt/session-1", Some("~/sentinel")),
            "~/wt/session-1 (worktree of ~/sentinel)"
        );
    }
    ...
    fn format_cwd_display_shows_subdir_not_repo_root() {
        ...
        assert_eq!(
            format_cwd_display(Path::new("/work/sentinel/frontend/apps"), Some(&info)),
            "/work/sentinel/frontend/apps",
        );
    }
    ...
    fn format_cwd_display_cache_miss_shows_raw_cwd() {
        assert_eq!(
            format_cwd_display(Path::new("/work/sentinel/frontend/apps"), None),
            "/work/sentinel/frontend/apps",
        );
    }
}
