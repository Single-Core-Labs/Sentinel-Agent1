...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a sentinel_ai_announcements::RemoteAnnouncement>,
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Ai Build** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Ai Build**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    let channel = sentinel_ai_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", sentinel_ai_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
        }
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a sentinel_ai_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a sentinel_ai_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [sentinel_ai_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a sentinel_ai_shell::auth::GateInfo>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("ai.com");
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Ai Build is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that Ai Build
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Ai Build may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &sentinel_ai_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn render_welcome_done(
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("SuperAi subscription required");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
...
fn masked_auth_token_view(input: &str, cursor_byte: usize, width: usize) -> (String, usize) {
    ...
    let buffer =
        sentinel_ratatui_textarea::EditBuffer::from_parts(masked.display.as_str(), masked.cursor_byte);
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("Ai Build"), "full badge: {full:?}");
        assert!(inline.contains("Ai Build"), "inline badge: {inline:?}");
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use sentinel_ai_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = sentinel_ai_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    /// Ai view when scanned foreign rows exist — even when the native list
    ...
    fn hidden_external_hint_renders_outside_chat_mode() {
        ...
        assert!(
            ...
            "default Ai filter must pin the hidden-external hint:\n{build_mode}"
        );
        assert!(
    ...
    fn grouped_entries_insert_headers() {
        let entries = vec![
            make_entry("s1", "Fix auth", "sentinel"),
            make_entry("s2", "Add streaming", "sentinel"),
        ...
        // Groups are sorted alphabetically: fw-1 before sentinel.
        // Header positions: 0 (fw-1), 2 (sentinel)
        ...
        assert!(
            matches!(&result[2], crate::views::picker::PickerEntry::Header { label } if label == &"sentinel")
        );
    }
    ...
    fn grouped_entries_rows_are_indented() {
        let entries = vec![make_entry("s1", "Fix auth", "sentinel")];
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.x.ai/oauth2/authorize?client_id=ai";
    ...
    fn long_ann() -> sentinel_ai_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = sentinel_ai_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
