...
pub const WORKSPACE_MODE_LOG: &str = "ai.pager.workspace_mode";
...
mod apply_tests {
    use super::*;
    use crate::app::session_startup::{
        AI_CHAT_LOCAL_WORKSPACE_ACK_ENV, LocalWorkspaceMode, set_active_local_workspace,
    };
    ...
    fn welcome_local_one_shot_only_when_agents_alive() {
        let _ack = sentinel_ai_test_support::EnvGuard::set(AI_CHAT_LOCAL_WORKSPACE_ACK_ENV, "1");
        set_active_local_workspace(None).unwrap();
        let tmp = tempfile::tempdir().unwrap();
        let out = prepare_welcome_workspace_for_new_session(
            WelcomeWorkspaceMode::LocalWorkspace,
