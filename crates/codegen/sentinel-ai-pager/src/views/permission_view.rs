...
pub struct PermissionViewState {
    ...
    pub request: sentinel_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `sentinel-ai-shell`, NOT duplicated locally.
...
pub(crate) fn preview_command_text(state: &PermissionViewState) -> String {
    match state.bash_highlights.as_ref() {
        Some(h) => sentinel_ai_workspace::permission::bash_command_splitting::unwrap_command_wrappers(
            &h.highlighted_words,
        )
        .join(" "),
...
fn render_pattern_preview_line(
    ...
    match edit.trimmed() {
        ...
        Some(pattern) => {
            if sentinel_ai_workspace::permission::bash_pattern_matches_command(pattern, command) {
                spans.push(Span::styled(
            ...
            if sentinel_ai_workspace::permission::bash_pattern_is_broad(pattern) {
...
/// ```text
///  3 [ ] Tell Ai what to do differently
...
mod tests {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        assert!(
            text.contains("\"body\": \"hello from ai\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p sentinel-ai-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/sentinel-ai-pager
