...
//! loop. Recipe: `AI_FPS=1 AI_SCROLL_DEBUG=1 ai --resume <session>`,
//! then flip `scroll_mode` / `scroll_lines` / `invert_scroll` /
//! `scroll_speed` in `/settings` to compare variants live. For event-exact
//! capture beyond this per-frame sampling, add `AI_SCROLL_LOG=1` — the
...
/// `AI_SCROLL_DEBUG` (nonempty and not `"0"`) enables at startup, and the
...
impl ScrollDebugHud {
    pub fn new() -> Self {
        let env_on = std::env::var("AI_SCROLL_DEBUG").is_ok_and(|v| !v.is_empty() && v != "0");
        Self { enabled: env_on }
    }
...
pub struct ScrollDebugPanel {
    ...
    /// Rows left free for FPS overlays stacked above (the dev `AI_FPS`
