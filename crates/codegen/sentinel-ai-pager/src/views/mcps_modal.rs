...
/// Display label for a section header, e.g. `"Managed by ai.com (3)"`.
pub fn section_label(section: &McpSectionId, count: usize) -> String {
    match section {
        McpSectionId::Managed => format!("Managed by ai.com ({count})"),
        McpSectionId::Plugin(name) => format!("Plugin: {name} ({count})"),
        McpSectionId::Local => format!("Local ({count})"),
    }
}

/// Base ai.com connectors URL (no team). Prefer [`managed_connectors_url`] when opening.
pub const MANAGED_SECTION_CONNECTORS_URL: &str = "https://ai.com/connectors";
...
mod tests {
    ...
    fn section_description_lines_managed_includes_connectors_url() {
        ...
        assert_eq!(lines[1], "[ai.com/connectors]");
        assert!(
        ...
        assert_eq!(with_team[1], "[ai.com/connectors?teamId=team-1]");
    }

    #[test]
    fn managed_connectors_url_display_strips_scheme() {
        assert_eq!(managed_connectors_url_display(None), "ai.com/connectors");
        assert_eq!(
            managed_connectors_url_display(Some("team-uuid-1")),
            "ai.com/connectors?teamId=team-uuid-1"
        );
    }
    ...
    #[test]
    fn section_for_ai_com_local_name_is_local() {
        let server = server_from_wire("ai_com_linear", Some("local"), None);
    ...
    #[test]
    fn is_removable_allows_ai_com_local_name() {
        let server = server_from_wire("ai_com_slack", Some("local"), None);
        assert!(is_removable(&server));
    }
    ...
    fn convert_list_response_classifies_managed_gateway_only_for_gateway_rows() {
        ...
        let legacy_managed = server_from_wire("ai_com_slack", Some("managed"), None);
