//! `ai wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `ai ssh` wrapper: spawns an arbitrary command inside a
...
//! stamps `LC_AI_APPEARANCE` from the local OS theme so a remote
//! `theme = "auto"` can resolve over SSH + tmux.
//!
//! Resolvable programs spawn directly. On Unix, commands a direct spawn cannot
//! run — a single shell-quoted string (`ai wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("ai wrap: no command given"))?;
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = sentinel_ai_config::shell::is_command_available(program);
        (
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("ai wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `ai wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`ai wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`ai wrap "$PROG" ...` with `$PROG` unset)
...
/// deliberately not `sentinel_ai_config::shell::unix_shell_path`, which coerces
...
///
/// Unlike `ai ssh`, `ai wrap` does not gate on the terminal brand: the user
