...
/// Ai home for the replay path. In production this is just `ai_home()`; the
/// whole test override below is `#[cfg(test)]`, so no thread-local or dead
/// always-false branch ships in release.
#[cfg(not(test))]
fn effective_ai_home() -> std::path::PathBuf {
    sentinel_ai_shell::util::ai_home::ai_home()
}
#[cfg(test)]
thread_local! {
    static REPLAY_AI_HOME: std::cell::RefCell<Option<std::path::PathBuf>> =
        const { std::cell::RefCell::new(None) };
}
/// Override ai home for disk-replay unit tests (thread-local).
#[cfg(test)]
pub(crate) fn set_replay_ai_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_AI_HOME.with(|h| *h.borrow_mut() = home);
}
#[cfg(test)]
fn effective_ai_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_AI_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
    sentinel_ai_shell::util::ai_home::ai_home()
}
...
pub(crate) fn enrich_from_meta(
    ...
    enrich_from_meta_with_home(info, &effective_ai_home(), parent_cwd, parent_session_id);
}
...
mod tests {
    ...
    fn ensure_subagent_child_replayed_releases_retained_memory_once() {
        ...
        set_replay_ai_home_for_tests(Some(home.path().to_path_buf()));
        let mut parent = make_min_child_view();
        parent
            .subagent_views
        ...
        set_replay_ai_home_for_tests(None);
    }
    #[test]
    ...
    fn replay_inherited_updates_uses_child_cwd_hint() {
        ...
        let session_dir = home
            .path()
            .join("sessions")
            .join(sentinel_ai_config::encode_cwd_dirname(child_cwd))
            .join(child_sid);
        std::fs::create_dir_all(&session_dir).unwrap();
        std::fs::write(session_dir.join("summary.json"), "{}").unwrap();
        let user = format!(
    ...
    fn subagent_meta_all_fields() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), Some("analyst"), Some("ai-3")),
            " (researcher \u{00b7} analyst \u{00b7} ai-3)"
        );
    }
    #[test]
    fn subagent_meta_partial_skips_nones() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), None, Some("ai-3")),
            " (researcher \u{00b7} ai-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_collapses_duplicate_persona_role() {
        assert_eq!(
            format_subagent_meta(Some("reviewer"), Some("reviewer"), Some("ai-3")),
            " (reviewer \u{00b7} ai-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_drops_both_empty_persona_role() {
        assert_eq!(
            format_subagent_meta(Some(""), Some(" "), Some("ai-3")),
            " (ai-3)"
        );
    }
    #[test]
