...
pub enum Command {
    /// Run Ai without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Ai discovers for this directory
    Inspect {
        /// Emit machine-readable JSON output.
        #[arg(long)]
        json: bool,
    },
    /// Check terminal, clipboard, color, and input support without starting Ai
...
Examples:
  ai wrap docker exec -it my-container bash
  ai wrap kubectl exec -it my-pod -- bash

See ~/.ai/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// Show what the ai home (~/.ai) uses on disk
    ...
    /// `AI_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.ai/config.toml` or when the `AI_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by `ai leader` / `ai workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `ai leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
...
pub struct AgentArgs {
    ...
    /// Override the public AI API base URL.
    #[arg(long = "sentinel-api-base-url")]
    pub sentinel_api_base_url: Option<String>,
    /// Agent runtime mode
    #[command(subcommand)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "ai: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("ai: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the Ai WebSocket relay
...
pub struct HeadlessArgs {
    #[arg(long = "ai-ws-origin")]
    pub ai_ws_origin: Option<String>,
    #[arg(long = "ai-ws-url")]
...
pub struct LeaderArgs {
    ...
    /// Defer the ai.com relay WebSocket until the first headless IPC client
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.ai/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `AI_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            sentinel_ai_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `ai -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "AI_SANDBOX", value_name = "PROFILE")]
    ...
    /// Session-scoped only — does not write config. To default plain `ai` to
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.ai/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.ai/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "AI_LOG_SAMPLING", hide = true)]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `ai "fix the bug"` or `ai --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`AI_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                sentinel_ai_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                sentinel_ai_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<sentinel_ai_sandbox::ProfileName>().ok()
                    != s.parse::<sentinel_ai_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `ai "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_flags_parse_as_early_intent_without_exiting() {
        for flag in ["--version", "-v", "-V"] {
            let args = PagerArgs::try_parse_from(["ai", flag]).expect("version flag parses");
    ...
    fn ordinary_and_doctor_parsing_do_not_set_version_intent() {
        assert!(!PagerArgs::try_parse_from(["ai"]).unwrap().version);
        assert!(
            !PagerArgs::try_parse_from(["ai", "doctor"])
                .unwrap()
                .version
        );
        assert!(matches!(
            PagerArgs::try_parse_from(["ai", "version"])
                .unwrap()
                .command,
    ...
    fn doctor_accepts_report_and_explicit_fix_forms() {
        let bare = PagerArgs::try_parse_from(["ai", "doctor"]).expect("bare doctor parses");
        assert!(matches!(
            bare.command,
        ...
        let json =
            PagerArgs::try_parse_from(["ai", "doctor", "--json"]).expect("doctor --json parses");
        assert!(matches!(
            json.command,
            Some(Command::Doctor(crate::doctor_cmd::DoctorArgs {
                json: true,
        ...
        for id in [
            ...
            let fix = PagerArgs::try_parse_from(["ai", "doctor", "fix", id, "--yes"])
        ...
        let list = PagerArgs::try_parse_from(["ai", "doctor", "fix"])
        ...
        for unsupported in [
            vec!["ai", "doctor", "all"],
            vec!["ai", "doctor", "fix", "ssh-wrap", "extra"],
            vec!["ai", "doctor", "fix", "--yes"],
            vec!["ai", "doctor", "--json", "fix", "terminal.ssh-wrap"],
        ] {
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["ai"]).unwrap().resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ai", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ai", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ai", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ai", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ai", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["ai", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["ai", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["ai", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["ai", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ai", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
        assert_eq!(
            PagerArgs::try_parse_from(["ai"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["ai", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["ai"]).expect("bare ai parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["ai", "leader", "list", "--json"])
            .expect("ai leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["ai", "leader", "info", "--pid", "42"])
            .expect("ai leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill = PagerArgs::try_parse_from(["ai", "leader", "kill"]).expect("ai leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["ai", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["ai", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["ai", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["ai", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["ai"]).expect("bare ai parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["ai", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias =
            PagerArgs::try_parse_from(["ai", "--trust-folder"]).expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["ai", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias =
            PagerArgs::try_parse_from(["ai", "--effort", "high"]).expect("--effort alias parses");
        assert_eq!(alias.reasoning_effort.as_deref(), Some("high"));
    }
    #[test]
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["ai", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep =
            PagerArgs::try_parse_from(["ai", "--reasoning-effort", "deep"]).expect("deep parses");
        assert_eq!(deep.reasoning_effort.as_deref(), Some("deep"));
    }
    #[test]
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["ai", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["ai", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["ai", "agent", "--effort", "max", "stdio"])
