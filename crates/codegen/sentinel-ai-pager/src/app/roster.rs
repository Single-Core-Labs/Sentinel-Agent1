...
/// `sentinel-ai-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
mod tests {
    ...
    fn agent_entry() -> sentinel_ai_shell::agent::roster::RosterEntry {
        use sentinel_ai_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        assert_eq!(e.model_id.as_deref(), Some("ai-4"));
        assert!(e.yolo);
