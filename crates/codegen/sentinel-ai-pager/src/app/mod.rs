...
pub use sentinel_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
/// (`[ui] mouse_reporting_toggle` / `AI_MOUSE_REPORTING_TOGGLE`). Seeded once
...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    sentinel_ai_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn voice_mode_config_value() -> Option<bool> {
    sentinel_ai_shell::config::load_effective_config()
        .ok()
        .and_then(|cfg| {
            cfg.get("features")
                .and_then(|f| f.get("voice_mode"))
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use sentinel_ai_shell::agent::config::{BoolFlag, ConfigSource};
    let resolved = BoolFlag::env("AI_VOICE_MODE")
...
///
/// Uses [`sentinel_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `sentinel-ai-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`sentinel-ai-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_leader_mode<'p>(
    ...
    _remote_settings: Option<&sentinel_ai_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<sentinel_ai_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<sentinel_ai_shell::util::config::RemoteSettings> {
    let handle = handle?;
            ...
            Err(ConnectFailure {
                ...
                error: anyhow::anyhow!(
                    "timed out after {}s connecting to {target}\n  \
                     ...
                     startup log: {}",
                    timeout.as_secs(),
                    sentinel_ai_telemetry::unified_log::path().display()
                ),
...
/// should print a message telling the user to relaunch `ai`.
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<sentinel_ai_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    sentinel_tty_utils::redirect_native_stderr();
    ...
    let ai_com_config = match sentinel_ai_shell::agent::config::Config::new_from_toml_cfg(
        &raw_config,
    ) {
        Ok(c) => c.ai_com_config,
        Err(e) => {
            tracing::warn!(error = %e, "failed to parse config for auth refresh, using defaults");
            sentinel_ai_shell::auth::AiComConfig::default()
        }
    };
    let refreshed_auth = tokio::time::timeout(
        sentinel_ai_shell::http::STARTUP_AUTH_REFRESH_TIMEOUT,
        sentinel_ai_shell::auth::try_ensure_fresh_auth(&ai_com_config),
    )
    .await
    .unwrap_or(None);
    let early_prefetch = match refreshed_auth {
        Some(auth) => sentinel_ai_shell::agent::models::start_early_prefetch_with_auth(Some(auth)),
        None => sentinel_ai_shell::agent::models::start_early_prefetch(Some(ai_com_config.clone())),
    };
    ...
    sentinel_ai_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(sentinel_ai_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = sentinel_ai_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("AI_HUNK_TRACKER").ok();
    ...
    let launch_auto = sentinel_ai_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    let pending_startup = sentinel_ai_telemetry::startup::PendingStartup::new();
    let timer = sentinel_ai_telemetry::startup::begin(crate::acp::Owner::Client);
    let connect_result =
    ...
    drop(agent_guard);
    sentinel_tty_utils::global_process_scope().kill_all();
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  ai --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  ai --resume {}", info.session_id);
    }
}
...
fn disable_mouse_paste_raw() {
    sentinel_ai_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(sentinel_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    sentinel_crash_handler::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        sentinel_ai_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(sentinel_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                sentinel_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = sentinel_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let term = sentinel_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        sentinel_crash_handler::disable_terminal_escape_restore();
    })
}
...
/// BEL/ESC (titles can arrive from ai.com conversation metadata) would
...
mod tests {
    ...
    fn restore_runs_teardown_even_when_writer_failed() {
        ...
        let terminal = sentinel_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    async fn bounded_connect_times_out_when_the_target_stalls() {
        sentinel_ai_telemetry::unified_log::redirect_to_temp_for_tests();
        let cancel = CancellationToken::new();
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - ai"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "ai");
        assert_eq!(terminal_title_string(""), "ai");
        assert_eq!(terminal_title_string("My chat"), "My chat - ai");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["ai-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["ai-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["ai-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["ai-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["ai-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["ai-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = sentinel_ai_shell::util::config::RemoteSettings {
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["ai-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["ai-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["ai-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_chat_flag_rejected_without_feature() {
        assert!(try_parse_pager(&["ai-pager", "--chat"]).is_err());
    }
    #[cfg(feature = "local-workspace")]
    #[test]
    fn cli_local_workspace_attach_requires_chat() {
        assert!(
            try_parse_pager(&["ai-pager", "--local-workspace-attach=srv"]).is_err(),
            "attach without --chat must clap-error"
        );
        let args =
            try_parse_pager(&["ai-pager", "--chat", "--local-workspace-attach=srv"]).unwrap();
    ...
    fn cli_local_workspace_cwd_requires_chat() {
        assert!(try_parse_pager(&["ai-pager", "--local-workspace-cwd=/tmp/a"]).is_err());
    ...
    fn cli_local_workspace_flags_rejected_without_feature() {
        assert!(try_parse_pager(&["ai-pager", "--local-workspace-attach=srv"]).is_err());
        assert!(try_parse_pager(&["ai-pager", "--local-workspace"]).is_err());
        assert!(try_parse_pager(&["ai-pager", "--local-workspace-cwd=/tmp"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["ai-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["ai-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["ai-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["ai-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["ai-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["ai-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["ai-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["ai-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["ai-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["ai-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_resume_and_fork_ok() {
        let args =
            try_parse_pager(&["ai-pager", "-s", "a", "--resume", "b", "--fork-session"]).unwrap();
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["ai-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_ai() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "ai");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "Ai Build TUI",
                "",
                "Usage: ai [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["ai-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["ai-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  ai --resume sess-abc\n"));
    }
    #[test]
