...
use sentinel_ai_shell::extensions::notification::GoalClassifierVerdict;
...
impl BgTaskState {
    ...
    pub fn tombstone_from_snapshot(
        snapshot: &sentinel_ai_tools::types::TaskSnapshot,
        status: BgTaskStatus,
        ...
        let is_monitor = matches!(
            snapshot.kind,
            sentinel_ai_tools::computer::types::TaskKind::Monitor
        ) || snapshot
            .display_command
            .as_deref()
...
pub struct AgentSession {
    ...
    pub restore_degree: Option<sentinel_ai_workspace::session::git::RestoreDegree>,
...
pub struct ChipElement {
    ...
    pub kind: sentinel_ratatui_textarea::ElementKind,
...
impl AgentSession {
    ...
    /// [`sentinel_prompt_queue::combine_prefix_len`]. `editing_id` is held out of the
    ...
    pub fn dequeue_combined_prompt(&mut self, editing_id: Option<u64>) -> Option<QueuedPrompt> {
        use sentinel_prompt_queue::{CombineGate, TEXT_SEPARATOR, combine_prefix_len, join_texts};
