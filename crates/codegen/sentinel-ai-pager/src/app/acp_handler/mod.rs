...
//! and AI session extension notifications (`x.ai/session_notification` and
...
use sentinel_acp_lib::AcpClientMessage;

use super::actions::Effect;
use sentinel_ai_shell::extensions::notification::{
    SessionNotification, SessionUpdate as AiSessionUpdate, is_reauthable_failure,
};
use sentinel_ai_shell::tools::todo::todo_item_from_plan_entry;
use sentinel_ai_workspace::permission::bash_command_splitting::BashCommandHighlights;
        ...
        AcpClientMessage::SessionNotification(notif) => {
            ...
            let affected = match find_session_match(app, &notif.request.session_id) {
                Some(SessionMatch::Root(id)) => {
                    ...
                    // time and drained FIFO. The AI stream is direct-emitted
                    // and keeps a SEPARATE highwater (see the AI dedup in
...
/// Handle an AI extension notification.
...
fn handle_ext_method(ext: sentinel_acp_lib::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
