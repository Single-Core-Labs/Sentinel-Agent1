...
pub(super) fn handle_permission_request(
    perm: sentinel_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    app: &mut AppView,
) -> bool {
...
fn enqueue_permission(
    ...
    // 1. Parse bash highlights from request meta (imported from sentinel-ai-shell).
    ...
    let bash_selection_count = bash_highlights
        .as_ref()
        .map(|h| sentinel_ai_workspace::permission::default_always_allow_scope(&h.highlighted_words))
        .unwrap_or(0);
    ...
    let mcp_scope = perm
        ...
        .and_then(|m| {
            serde_json::from_value::<sentinel_ai_workspace::permission::McpToolPermission>(
...
/// `sentinel-ai-tools` for typed access to `command` and `description`.
...
fn build_permission_display(
    ...
    let bash_input = req.tool_call.fields.raw_input.as_ref().and_then(|v| {
        serde_json::from_value::<sentinel_ai_tools::implementations::BashToolInput>(v.clone()).ok()
    });
    ...
    let title = if is_execute {
        ...
        if let Some(path) = file_path {
            ...
            format!(
                "Allow {}?",
                sentinel_ai_workspace::permission::mcp_pretty_name_if_qualified(t)
            )
        } else {
...
fn protected_edit_description(req: &acp::RequestPermissionRequest) -> Option<String> {
    ...
    let protected: sentinel_ai_workspace::permission::ProtectedEditPermission =
...
fn cancel_permission(perm: sentinel_acp_lib::AcpArgs<acp::RequestPermissionRequest>) {
    perm.response_tx
        .send(Ok(acp::RequestPermissionResponse::new(
