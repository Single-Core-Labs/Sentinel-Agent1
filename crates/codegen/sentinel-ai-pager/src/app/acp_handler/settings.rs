...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if let Some(campaigns) = update.campaigns.clone() {
        ...
        sentinel_ai_shell::util::config::set_remote_campaigns_from_settings(Some(&rs));
    }
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        sentinel_ai_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = sentinel_ai_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = sentinel_ai_shell::config::load_effective_config().ok();
    ...
    if let Some(v) = update.privacy_notice_rollout {
        app.privacy_notice_rollout =
            sentinel_ai_config::env_bool("AI_PRIVACY_NOTICE_ROLLOUT").unwrap_or(v);
    }
    if let Some(v) = update.privacy_banner_reshow_days {
        app.privacy_banner_reshow_days = Some(
            std::env::var("AI_PRIVACY_BANNER_RESHOW_DAYS")
                .ok()
    ...
    // Always recompute is_api_key_auth from the tier so a later Free/SuperAi
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(sentinel_ai_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("AI_SESSION_PICKER_GROUPED")
            .ok()
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(sentinel_ai_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        sentinel_ai_shell::config::load_merged_requirements(),
        sentinel_ai_shell::config::load_from_disk().ok(),
        sentinel_ai_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = sentinel_ai_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = sentinel_ai_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = sentinel_ai_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use sentinel_ai_shell::util::config::resolve_tips;
    ...
    if let Some(remote_tags) = update.slash_command_tags.as_ref() {
        use sentinel_ai_shell::util::config::resolve_slash_command_tags;
        let effective_config = sentinel_ai_shell::config::load_effective_config().ok();
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = sentinel_ai_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        sentinel_ai_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}
...
pub(super) fn notify_sessions_leave_auto(app: &AppView, session_ids: &[acp::SessionId]) {
    ...
    let args = sentinel_acp_lib::AcpArgs {
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<sentinel_ai_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
/// `resolve_announcements` honors `AI_ANNOUNCEMENTS_OVERRIDE` first, so a
/// backend push can't reintroduce announcements when the override is set.
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[sentinel_ai_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = sentinel_ai_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = sentinel_ai_announcements::filter_expired(merged);
    ...
    if sentinel_ai_announcements::prune_hidden_announcement_ids(
...
) -> Option<sentinel_ai_announcements::RemoteAnnouncement> {
...
/// `sentinel-ai-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `sentinel-ai-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
...
pub(super) struct PagerSettingsUpdate {
    ...
    #[serde(default)]
    campaigns: Option<Vec<sentinel_ai_shell::util::config::CampaignOverride>>,
    #[serde(default)]
