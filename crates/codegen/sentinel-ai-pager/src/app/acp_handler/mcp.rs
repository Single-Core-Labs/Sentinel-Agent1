...
/// `AI_MCP_PUSH_SERVER_STATUS=0` to disable.
pub(super) fn push_server_status_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        sentinel_ai_shell::util::config::resolve_mcp_push_server_status(
...
/// `sentinel-ai-shell/src/session/acp_session.rs` — toggle-tool ~L6661,
...
///    [`sentinel_ai_shell::extensions::mcp::McpToolsChanged`] struct), and
...
/// ([`sentinel_ai_shell::extensions::mcp::McpServerStatusPayload`] +
/// [`sentinel_ai_shell::extensions::mcp::McpServerStatus`]) instead of
...
pub(super) fn handle_mcp_server_status(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    use sentinel_ai_shell::extensions::mcp::{McpServerStatus, McpServerStatusPayload, McpToolEntry};
...
/// on config reload (`crates/codegen/sentinel-ai-shell/src/agent/mvp_agent.rs`
