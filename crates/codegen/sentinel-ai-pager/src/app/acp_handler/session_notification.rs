use super::*;
use sentinel_ai_shell::sampling::error::format_rate_limited_user_message;
...
/// Replay gate shared by the ACP and AI session-update paths. Returns `true`
...
pub(super) fn handle_session_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    match &session_notif.update {
        AiSessionUpdate::TaskBackgrounded { .. } => {
            return handle_task_backgrounded(notif, app);
        }
        AiSessionUpdate::TaskCompleted { .. } => {
            return handle_task_completed(notif, app);
        }
        AiSessionUpdate::ScheduledTaskCreated { .. } => {
            return handle_scheduled_task_created(notif, app);
        }
        AiSessionUpdate::ScheduledTaskDeleted { .. } => {
    ...
    let is_workflow_update = matches!(
        session_notif.update,
        AiSessionUpdate::WorkflowUpdated { .. }
    );
    if !is_workflow_update
        && !meta.is_replay
        && meta.event_seq.is_some_and(|seq| {
            agent
                .last_applied_sentinel_event_seq
        ...
        })
    {
        tracing::debug!(
            ...
            last_applied = agent.last_applied_sentinel_event_seq,
    ...
    let changed = match session_notif.update {
        ref update @ (AiSessionUpdate::AutoCompactStarted { .. }
        | AiSessionUpdate::AutoCompactCompleted { .. }
        | AiSessionUpdate::AutoCompactFailed { .. }
        | AiSessionUpdate::AutoCompactCancelled { .. }
        | AiSessionUpdate::RetryState(_)
        | AiSessionUpdate::ImageDropped { .. }
        | AiSessionUpdate::MemoryFlushCompleted { .. }
        | AiSessionUpdate::MemoryDreamCompleted { .. }
        | AiSessionUpdate::MemorySessionSaved { .. }) => {
            ...
            if let AiSessionUpdate::AutoCompactCompleted { tokens_after, .. } = update {
                refresh_context_used(agent, *tokens_after);
                agent.todo.update_todos(Vec::new());
            }
            changed
        }
        AiSessionUpdate::ImageCompressed {
            ref images,
            ref message,
        } => apply_image_compressed(agent, images, message),
        AiSessionUpdate::TurnCompleted {
            prompt_id,
            stop_reason,
            agent_result,
            ..
        ...
        AiSessionUpdate::SubagentSpawned {
            subagent_id,
            child_session_id,
            subagent_type,
            description,
        ...
        AiSessionUpdate::SubagentProgress {
            child_session_id,
            duration_ms,
            turn_count,
            tool_call_count,
        ...
        AiSessionUpdate::SubagentFinished {
            child_session_id,
            status,
            error,
            tool_calls,
        ...
        AiSessionUpdate::HookAnnotation { message } => {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        sentinel_ai_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        sentinel_ai_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        sentinel_ai_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
                            blocked: true,
        ...
        AiSessionUpdate::HooksChanged {
            ...
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(sentinel_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        AiSessionUpdate::PluginsChanged { plugins } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                use crate::views::extensions_modal::TabDataState;
        ...
        AiSessionUpdate::SessionRecap { summary, auto } => {
        ...
        AiSessionUpdate::SessionRecapUnavailable => {
            if meta.is_replay {
                false
        ...
        AiSessionUpdate::ModelAutoSwitched {
            previous_model_id,
            new_model_id,
            reason,
        } => {
        ...
        AiSessionUpdate::ModelChanged {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if sentinel_ai_shell::agent::chat_modes::process_chat_mode_enabled() {
        ...
        AiSessionUpdate::MemoryFiles { files } => {
        ...
        update @ AiSessionUpdate::WorkflowUpdated { .. } => ingest_workflow_update(agent, update),
        AiSessionUpdate::GoalUpdated {
            goal_id,
            objective,
            status,
            phase,
        ...
        AiSessionUpdate::InteractionResolved { tool_call_id } => {
    ...
    if let Some(agent) = app.agents.get_mut(&parent_id) {
        if let Some(seq) = meta.event_seq
            ...
            && !is_workflow_update
        {
            agent.last_applied_sentinel_event_seq = Some(seq);
        }
...
/// Handle an AI session notification that targets a child (subagent) session.
///
...
pub(super) fn handle_child_session_notification(
    ...
    match update {
        AiSessionUpdate::AutoCompactStarted { .. }
        | AiSessionUpdate::AutoCompactCompleted { .. }
        | AiSessionUpdate::AutoCompactFailed { .. }
        | AiSessionUpdate::AutoCompactCancelled { .. }
        | AiSessionUpdate::RetryState(_) => {
            let compact_tokens = match &update {
                AiSessionUpdate::AutoCompactCompleted { tokens_after, .. } => Some(*tokens_after),
                _ => None,
            };
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct =
                        Some(sentinel_token_estimation::usage_percentage_u8(tokens_after, cw));
                }
            }
            changed
        }
        ref update @ (AiSessionUpdate::MemoryFlushCompleted { .. }
        | AiSessionUpdate::MemoryDreamCompleted { .. }
        | AiSessionUpdate::MemorySessionSaved { .. }) => {
            if let Some(child_view) = agent.subagent_views.get_mut(child_sid) {
                apply_session_event(
...
pub(super) fn apply_session_event(
    ...
    match update {
        AiSessionUpdate::AutoCompactStarted { percentage, .. } => {
        ...
        AiSessionUpdate::AutoCompactFailed { error } => {
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[sentinel_ai_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &sentinel_ai_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
    ...
    use sentinel_ai_shell::extensions::notification::RetryState;
    match retry {
        RetryState::Retrying {
            attempt,
            max_retries,
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                sentinel_ai_telemetry::session_ctx::log_event(
                    sentinel_ai_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
            ...
            let is_free_usage = *rate_limited
                && sentinel_ai_shell::sampling::error::is_free_usage_exhausted_error(reason);
            if is_credit_limit {
    ...
    if is_credit_limit {
        sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use sentinel_ai_tools::types::SessionMode;
