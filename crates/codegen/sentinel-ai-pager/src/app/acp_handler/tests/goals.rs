#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn goal_updated_ignores_unknown_json_fields_via_serde() {
        ...
        let raw_payload = serde_json::json!({
            ...
            "update": {
                ...
                "live_tokens_by_model": [["ai-4", 6_000], ["ai-3", 4_000]],
                "live_context_pct": 42,
                "live_turn_count": 7,
        ...
        let msg = AcpClientMessage::ExtNotification(sentinel_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
        ...
        assert_eq!(
            ...
            vec![("ai-4".to_owned(), 6_000), ("ai-3".to_owned(), 4_000)],
    ...
    #[test]
    fn workflow_updates_bypass_global_sentinel_highwater_and_use_run_revision() {
        let mut app = make_app_with_agent("sess-A");
        let id = AgentId(0);
        app.agents.get_mut(&id).unwrap().last_applied_sentinel_event_seq = Some(100);
        ...
        assert_eq!(app.agents[&id].last_applied_sentinel_event_seq, Some(100));
