...
use sentinel_ai_shell::extensions::notification::SessionUpdate as AiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> sentinel_ai_shell::extensions::notification::ImageCompressedEntry {
    sentinel_ai_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn make_exit_plan_ext(
    ...
    sentinel_acp_lib::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<sentinel_acp_lib::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(sentinel_acp_lib::AcpArgs {
...
pub(super) fn make_fired_notif(
    ...
    let notif = SessionNotification {
        ...
        update: AiSessionUpdate::ScheduledTaskFired {
            task_id: task_id.into(),
            prompt: prompt.into(),
...
pub(super) fn announcements_update_notif(
    r#gen: u64,
    announcements: &[sentinel_ai_announcements::RemoteAnnouncement],
) -> acp::ExtNotification {
    acp::ExtNotification::new(
...
pub(super) fn make_created_ext_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::ScheduledTaskCreated {
            task_id: task_id.into(),
            prompt: prompt.into(),
            human_schedule: human_schedule.into(),
            next_fire_at: next_fire_at.map(str::to_string),
...
pub(super) fn make_deleted_ext_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::ScheduledTaskDeleted {
            task_id: task_id.into(),
        },
        meta: None,
    };
...
pub(super) fn sentinel_model_switch_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::ModelAutoSwitched {
...
pub(super) fn sentinel_unhandled_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::MemoryFlushStarted,
        meta: Some(serde_json::json!({ "eventId": event_id })),
    };
    acp::ExtNotification::new(
        "x.ai/session/update",
...
pub(super) fn sentinel_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    is_replay: bool,
    ...
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::TurnCompleted {
...
pub(super) fn sentinel_turn_completed_notif_with_cancel_trigger(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    cancel_trigger: &str,
...
pub(super) fn sentinel_wake_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
...
pub(super) fn sentinel_hook_execution_notif_for_prompt(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
) -> acp::ExtNotification {
    use sentinel_ai_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    sentinel_hook_execution_notif_with_runs(
        session_id,
        event_name,
        prompt_id,
        is_replay,
...
pub(super) fn sentinel_hook_execution_notif_with_runs(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
    runs: Vec<sentinel_ai_shell::extensions::notification::HookRunEntryDto>,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::HookExecution {
...
pub(super) fn sentinel_hook_execution_notif(
    session_id: &str,
    event_name: &str,
    is_replay: bool,
) -> acp::ExtNotification {
    sentinel_hook_execution_notif_for_prompt(session_id, event_name, None, is_replay)
}
...
/// Build an `ExtNotification` envelope with an explicit AI session method.
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(sentinel_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
pub(super) fn test_subagent_finished(child_sid: &str) -> AiSessionUpdate {
...
/// Shared temp `AI_HOME` for disk-replay tests. `ai_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `AI_HOME`
...
/// Runs `f` with a thread-local ai home override so disk replay tests do not
/// depend on process-wide `ai_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_ai_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_ai_home_for_tests(None);
    out
}
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(sentinel_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
...
pub(super) fn interaction_resolved_ext(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::InteractionResolved {
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = sentinel_ai_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_task_backgrounded_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::TaskBackgrounded {
            tool_call_id: tool_call_id.into(),
            task_id: task_id.into(),
            command: command.into(),
            cwd: "/tmp".into(),
...
pub(super) fn task_completed_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::TaskCompleted {
...
pub(super) fn make_monitor_event_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::MonitorEvent {
            task_id: task_id.into(),
...
pub(super) fn model_changed_ext(
    ...
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: AiSessionUpdate::ModelChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: sentinel_ai_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use sentinel_ai_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = sentinel_ai_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "ai_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName": "ai_com_linear", "tools": [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("x.ai/mcp/tools_changed", std::sync::Arc::from(raw))
}
