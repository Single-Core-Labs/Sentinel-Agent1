#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn ext_session_update_replay_restores_bg_task() {
        ...
        let update = AiSessionUpdate::TaskBackgrounded {
    ...
    ) -> sentinel_ai_tools::types::TaskSnapshot {
        sentinel_ai_tools::types::TaskSnapshot {
    ...
    fn completed_notif_from_snapshot(
        session_id: &str,
        task_snapshot: sentinel_ai_tools::types::TaskSnapshot,
        replayed: bool,
    ) -> acp::ExtNotification {
        let notif = SessionNotification {
    ...
    fn unknown_completed_monitor_kind_marks_is_monitor() {
        ...
        snapshot.kind = sentinel_ai_tools::computer::types::TaskKind::Monitor;
        let done = completed_notif_from_snapshot("sess-1", snapshot, false);
        assert!(handle_task_completed(&done, &mut app));
