#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn reconnect_reload_failure_restores_pre_outage_transcript() {
        ...
        let id = AgentId(0);
        {
            ...
            agent.last_applied_sentinel_event_seq = Some(5);
            agent.begin_session_reload(1);
        }
        ...
        let _ = handle_ext_notification(&sentinel_model_switch_notif("sess-rc", "sess-rc-30"), &mut app);
        assert_eq!(
        ...
        assert_eq!(
            app.agents[&id].last_applied_sentinel_event_seq,
            Some(30),
            "the live AI line advanced its highwater in-window"
        );
        ...
        assert_eq!(
            agent.last_applied_sentinel_event_seq,
            Some(5),
            "the AI highwater reverts with the transcript — left at 30 it would \
    ...
    /// AI updates dedup on their OWN per-session `eventId` highwater: a
    ...
    /// AI arms have no other dedup. Replay stays exempt.
    #[test]
    fn sentinel_session_update_dedup_drops_already_applied_event() {
        let mut app = make_app_with_agent("sess-xdup");
        let id = AgentId(0);

        assert!(handle_ext_notification(
            &sentinel_model_switch_notif("sess-xdup", "sess-xdup-10"),
            &mut app
        ));
        assert_eq!(app.agents[&id].scrollback.len(), 1);
        assert_eq!(app.agents[&id].last_applied_sentinel_event_seq, Some(10));
        ...
        assert_eq!(
            ...
            1,
            "a duplicate AI event must not push a second block"
        );
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
            Some("sess-xdup-10")
        ...
        assert!(handle_ext_notification(
            &sentinel_model_switch_notif("sess-xdup", "sess-xdup-11"),
            &mut app
        ));
        assert_eq!(app.agents[&id].scrollback.len(), 2);
        assert_eq!(app.agents[&id].last_applied_sentinel_event_seq, Some(11));

        // Lower-stale re-delivery (an already-applied lower id re-sent by
        // the cursor tail, e.g. goal mode) is dropped too — `<=`, not just
        // equality.
        assert!(!handle_ext_notification(
            &sentinel_model_switch_notif("sess-xdup", "sess-xdup-9"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].scrollback.len(),
            2,
            "a stale lower-id AI event must not push a block"
        );
        assert_eq!(app.agents[&id].last_applied_sentinel_event_seq, Some(11));
    }

    /// An unhandled AI kind (the default `_` arm) leaves no trace, so it must
    ...
    #[test]
    fn unhandled_sentinel_update_does_not_advance_cursor_or_highwater() {
        let mut app = make_app_with_agent("sess-ig");
        let id = AgentId(0);

        assert!(!handle_ext_notification(
            &sentinel_unhandled_notif("sess-ig", "sess-ig-7"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id, None,
            "an unhandled AI update must not advance the reconnect cursor"
        );
        assert_eq!(
            app.agents[&id].last_applied_sentinel_event_seq, None,
            "an unhandled AI update must not advance the dedup highwater"
        );

        // An applied kind (ModelAutoSwitched) advances both.
        assert!(handle_ext_notification(
            &sentinel_model_switch_notif("sess-ig", "sess-ig-8"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
        ...
        assert_eq!(app.agents[&id].last_applied_sentinel_event_seq, Some(8));
    }

    /// Split-highwater regression: a fresh direct-emitted AI id must NOT
    /// make a queued lower-id ACP chunk look stale. AI lines bypass the
    ...
    #[test]
    fn direct_sentinel_event_does_not_shoot_down_delayed_acp_chunk() {
        let mut app = make_app_with_agent("sess-split");
        let id = AgentId(0);

        // Direct AI emission stamped N+1 arrives first.
        assert!(handle_ext_notification(
            &sentinel_model_switch_notif("sess-split", "sess-split-21"),
            &mut app
        ));
        assert_eq!(app.agents[&id].last_applied_sentinel_event_seq, Some(21));
        ...
        assert_eq!(
            app.agents[&id].last_applied_sentinel_event_seq,
            Some(21),
            "…and the ACP apply must not clobber the AI highwater either"
        );
    }
    ...
    /// AI extension session updates: replay-stamped ones are gated like ACP
    /// updates, and applied ones advance the reconnect cursor.
    #[test]
    fn sentinel_session_update_replay_gating_and_cursor() {
        fn model_switch_notif(meta: Option<serde_json::Value>) -> acp::ExtNotification {
            let payload = SessionNotification {
                session_id: acp::SessionId::new("sess-sentinel"),
        ...
        let mut app = make_app_with_agent("sess-sentinel");
        let id = AgentId(0);

        // Replay-stamped with no load in flight → dropped, nothing pushed.
        let replay_meta = serde_json::json!({ "isReplay": true, "eventId": "sess-sentinel-7" });
        assert!(!handle_ext_notification(
        ...
        {
            let agent = app.agents.get_mut(&id).unwrap();
            assert!(agent.scrollback.is_empty(), "unexpected AI replay dropped");
        ...
        assert_eq!(
            agent.last_seen_event_id.as_deref(),
            Some("sess-sentinel-7"),
            "applied AI updates advance the reconnect cursor"
        );
        assert!(agent.finish_session_reload(1, true));
        assert!(
            !scrollback_has_system_text(agent, "pre-outage content"),
            "an AI replay line counts as replay for the swap decision"
        );
        assert_eq!(
            agent.scrollback.len(),
            1,
            "the staged AI block is the new transcript"
        );
    }
    ...
    /// Apply-only cursor rule (AI path): a `ModelChanged` the catalog can't
    ...
    fn ignored_model_changed_does_not_advance_cursor_applied_one_does() {
        ...
        {
            let agent = app.agents.get_mut(&id).unwrap();
            seed_models(agent, "ai-3", &["ai-3", "ai-4"]);
        }

        // Unknown model → ignored → both markers untouched.
        assert!(!handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "ai-99-unknown", "sess-1-7"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id, None,
        ...
        assert!(handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "ai-4", "sess-1-8"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
