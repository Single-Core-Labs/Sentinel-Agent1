...
/// Inclusive bounds for motion cadence env knobs (`AI_MIN_DRAW_MS`,
/// `AI_SCROLL_CADENCE_MS`).
...
pub fn start(
    ...
    let (min_draw_env_set, min_draw_env_ms) =
        cadence_ms_from_env("AI_MIN_DRAW_MS", default_cadence_ms);
    let (scroll_env_set, scroll_env_ms) =
        cadence_ms_from_env("AI_SCROLL_CADENCE_MS", default_cadence_ms);
...
fn spawn_terminal_and_display_refresh_telemetry(tel: StartupTel) {
    tokio::task::spawn_blocking(move || {
        ...
        sentinel_ai_telemetry::session_ctx::log_event(t.clone());
        ...
        sentinel_ai_telemetry::session_ctx::log_event(
            sentinel_ai_telemetry::events::DisplayRefreshProbe {
                terminal: t,
