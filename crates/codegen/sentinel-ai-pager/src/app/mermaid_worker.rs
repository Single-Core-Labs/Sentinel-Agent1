...
//! inside [`sentinel_ai_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `sentinel-ai-pager __mermaid-render` (see
...
//!    via [`sentinel_ai_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`sentinel_ai_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `sentinel-ai-mermaid`). The parent also rejects obviously-oversized
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, sentinel_ai_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `AI_HOME` (whose `ai_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite — PNGs could land in the
    /// real `~/.ai`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_AI_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `AI_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    fn is_render_subcommand_matches_only_argv1() {
        ...
        assert!(!is_render_subcommand(&argv(&["ai"])));
        assert!(!is_render_subcommand(&argv(&["ai", "chat"])));
        assert!(!is_render_subcommand(&argv(&[])));
    ...
    fn per_theme_renders_land_in_distinct_files() {
        ...
        let light_key =
            MermaidCacheKey::derive(src, ThemeKind::AiDay, 80, MermaidRenderQuality::Terminal);
        assert_ne!(
    ...
    /// hermetic, with no process-global `AI_HOME` mutation. The `TempDir` lives
    ...
    /// so there is no cross-test contamination and no `ai_home()` cache race.
    ...
    fn agent_with_session(name: &str) -> AgentView {
        ...
        let cwd = PathBuf::from("/ai-mermaid-test").join(name);
    ...
    fn mermaid_is_rendering_matches_by_source_across_theme_width_change() {
        ...
        let click_key =
            MermaidCacheKey::derive(src, ThemeKind::AiNight, 80, MermaidRenderQuality::Open);
        let mut rt = MermaidRuntime::new();
        rt.pending.push(PendingMermaidAction {
            key: click_key.clone(),
            action: MermaidClickAction::Open,
        ...
        let live_key =
            MermaidCacheKey::derive(src, ThemeKind::AiDay, 240, MermaidRenderQuality::Open);
        assert_ne!(
            click_key, live_key,
