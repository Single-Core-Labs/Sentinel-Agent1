...
use sentinel_acp_lib::{AcpAgentTx, acp_send};
use super::actions::{PermissionModePersist, SubagentKillOutcome, TaskResult};
use super::agent::AgentId;
use crate::unified_log as ulog;
use sentinel_ai_shell::sampling::error::{
    RATE_LIMITED_ERROR_CODE, error_detail_from_data, format_rate_limited_user_message,
    http_status_from_error,
};
use sentinel_ai_shell::session::ExtMethodResult;
use sentinel_ai_shell::session::unified_list::ListScope;
...
/// before the agent's own deadline. Reads `AI_ENVRC_TIMEOUT_SECS` in this
/// process; the agent inherits the same environment.
pub(super) fn session_rpc_timeout() -> std::time::Duration {
    SESSION_RPC_FLOOR.max(sentinel_ai_workspace::envrc::loader_budget() + SESSION_RPC_SLACK)
}
...
where
    T: sentinel_acp_lib::AcpRequest,
    R: From<sentinel_acp_lib::AcpArgs<T>> + std::fmt::Debug,
{
...
/// rewrite when the body pushes personal SuperAi), else auth-aware fallback
...
) -> (bool, Option<String>, Option<sentinel_ai_workspace::session::git::RestoreDegree>) {
...
pub(crate) fn parse_session_scheduler_background_loops(
    ...
    resp_meta
        .and_then(|m| {
            m.get(sentinel_ai_shell::session::SCHEDULER_BACKGROUND_LOOPS_META_KEY)
        })
        .and_then(|v| v.as_bool())
}
/// Whether `raw` is (or wraps) a disk-full / ENOSPC failure.
pub(crate) fn is_disk_full_error(raw: &str) -> bool {
    raw.contains(sentinel_fast_worktree::OUT_OF_DISK_CONTEXT)
        || raw.contains(sentinel_fast_worktree::ENOSPC_OS_MESSAGE)
...
pub(crate) fn sanitize_user_error(raw: &str) -> String {
    if is_disk_full_error(raw) {
        return sentinel_fast_worktree::ENOSPC_OS_MESSAGE.to_string();
    }
...
/// |-------|-----------|----------|--------------------------------|--------------------|
/// | false | false     | false    | `ai-build` (default)         | `false`            |
/// | false | true      | false    | `ai-build` (default)         | `false`            |
/// | false | false     | true     | `ai-build-ask-user`          | omitted (shell gate) |
/// | false | true      | true     | `ai-build-ask-user`          | omitted (shell gate) |
/// | true  | false     | false    | `ai-build-plan-no-subagents` | `false`            |
/// | true  | true      | false    | `ai-build-plan`              | `false`            |
/// | true  | false     | true     | `ai-build-plan-no-subagents` | omitted (shell gate) |
/// | true  | true      | true     | `ai-build-plan`              | omitted (shell gate) |
///
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `ai-build` profile (no `_meta`
    ...
    pub(super) fn agent_profile(&self) -> Option<&'static str> {
        ...
        match (self.plan_mode, self.subagents, self.ask_user) {
            (true, true, _) => Some("ai-build-plan"),
            (true, false, _) => Some("ai-build-plan-no-subagents"),
            (false, _, true) => Some("ai-build-ask-user"),
            (false, _, false) => None,
        }
    }
    ...
    /// `AI_AGENT` — the env var chooses the *agent*, but the tool-strip is
    ...
    pub(super) fn to_meta(&self) -> Option<acp::Meta> {
        ...
        if self.chat_mode {
            if self.plan_mode || self.agent_override.is_some()
                || std::env::var("AI_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
            {
                tracing::warn!(
        ...
        } else if std::env::var("AI_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
...
pub(crate) fn reject_non_fs_only_advertised_tools(
    ...
    let Some(ids) = advertised_tool_ids else {
        return Err(
            "operator attestation AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS is unset \
    ...
    if ids.is_empty() {
        return Err(
            "operator attestation AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS is empty \
    ...
    if forbidden.is_empty() {
        ...
        Err(
            ...
            "operator attestation lists tools outside the FS-only allowlist: {}. \
             ...
             AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS or restart workspace_server \
...
pub(super) fn extract_first_user_prompt(
    info: &sentinel_ai_shell::session::info::Info,
) -> Option<String> {
    use std::io::BufRead;
    let history_path = sentinel_ai_shell::session::persistence::session_dir(info)
...
pub(super) fn count_chat_history_stats(history_path: &Path) -> (usize, usize) {
    use std::io::BufRead;
    use sentinel_ai_shell::sampling::{AssistantItem, ConversationItem, UserItem};
...
pub(super) fn parse_session_picker_entries(
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            use sentinel_ai_tools::implementations::skills::skill::extract_skill_display_text;
            let display = if let Some(ref fp) = first_prompt {
                ...
                let info_cwd = v
                    ...
                    .unwrap_or_default()
                    .to_string();
                let info = sentinel_ai_shell::session::info::Info {
        ...
        .filter_map(|mut e| {
            ...
            if e.source == "remote"
                && sentinel_ai_shell::session::resolve_local_session_any_cwd(&e.id)
                    .is_some()
            {
...
pub(crate) async fn persist_setting(
    ...
    match key {
        "compact_mode" => {
            ...
            sentinel_ai_shell::util::config::set_compact_mode(b)
                .await
        ...
        "show_timestamps" => {
            ...
            sentinel_ai_shell::util::config::set_show_timestamps(b)
                .await
                .map_err(|e| e.to_string())
        }
        "page_flip_on_send" => {
            ...
            sentinel_ai_shell::util::config::set_page_flip_on_send(b)
                .await
                .map_err(|e| e.to_string())
        }
        "confirm_before_rewind" => {
            ...
            sentinel_ai_shell::util::config::set_confirm_before_rewind(b)
                .await
                .map_err(|e| e.to_string())
        }
        "combine_queued_prompts" => {
            ...
            sentinel_ai_shell::util::config::set_combine_queued_prompts(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_timeline" => {
            ...
            sentinel_ai_shell::util::config::set_show_timeline(b)
                .await
                .map_err(|e| e.to_string())
        }
        "simple_mode" => {
            ...
            sentinel_ai_shell::util::config::set_simple_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.undo" => {
            ...
            sentinel_ai_shell::util::config::set_contextual_hint_undo(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.plan_mode" => {
            ...
            sentinel_ai_shell::util::config::set_contextual_hint_plan_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.image_input" => {
            ...
            sentinel_ai_shell::util::config::set_contextual_hint_image_input(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.send_now" => {
            ...
            sentinel_ai_shell::util::config::set_contextual_hint_send_now(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.small_screen" => {
            ...
            sentinel_ai_shell::util::config::set_contextual_hint_small_screen(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.word_select" => {
            ...
            sentinel_ai_shell::util::config::set_contextual_hint_word_select(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.ssh_wrap" => {
            ...
            sentinel_ai_shell::util::config::set_contextual_hint_ssh_wrap(b)
                .await
                .map_err(|e| e.to_string())
        }
        "theme" => {
            ...
            sentinel_ai_shell::util::config::set_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_dark_theme" => {
            ...
            sentinel_ai_shell::util::config::set_auto_dark_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_light_theme" => {
            ...
            sentinel_ai_shell::util::config::set_auto_light_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "default_model" => {
            ...
            sentinel_ai_shell::util::config::set_default_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_speed" => {
            ...
            sentinel_ai_shell::util::config::set_scroll_speed(i)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_mode" => {
            ...
            sentinel_ai_shell::util::config::set_scroll_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "invert_scroll" => {
            ...
            sentinel_ai_shell::util::config::set_invert_scroll(b)
                .await
                .map_err(|e| e.to_string())
        }
        "display_refresh_auto_cadence" => {
            ...
            sentinel_ai_shell::util::config::set_display_refresh_auto_cadence(b)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_lines" => {
            ...
            sentinel_ai_shell::util::config::set_scroll_lines(i)
                .await
                .map_err(|e| e.to_string())
        }
        "default_selected_permission" => {
            ...
            sentinel_ai_shell::util::config::set_default_selected_permission(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "cancel_subagents_on_turn_cancel" => {
            ...
            sentinel_ai_shell::util::config::set_cancel_subagents_on_turn_cancel(
                    s.to_string(),
                )
                .await
                .map_err(|e| e.to_string())
        ...
        "vim_mode" => {
            ...
            sentinel_ai_shell::util::config::set_vim_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "remember_tool_approvals" => {
            ...
            sentinel_ai_shell::util::config::set_remember_tool_approvals(b)
                .await
                .map_err(|e| e.to_string())
        }
        "toolset.ask_user_question.timeout_enabled" => {
            ...
            sentinel_ai_shell::util::config::set_ask_user_question_timeout_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_thinking_blocks" => {
            ...
            sentinel_ai_shell::util::config::set_show_thinking_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "group_tool_verbs" => {
            ...
            sentinel_ai_shell::util::config::set_group_tool_verbs(b)
                .await
                .map_err(|e| e.to_string())
        }
        "collapsed_edit_blocks" => {
            ...
            sentinel_ai_shell::util::config::set_collapsed_edit_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "prompt_suggestions" => {
            ...
            sentinel_ai_shell::util::config::set_prompt_suggestions(b)
                .await
                .map_err(|e| e.to_string())
        }
        "keep_text_selection" => {
            ...
            sentinel_ai_shell::util::config::set_keep_text_selection(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "render_mermaid" => {
            ...
            sentinel_ai_shell::util::config::set_render_mermaid(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "hunk_tracker_mode" => {
            ...
            sentinel_ai_shell::util::config::set_hunk_tracker_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "screen_mode" => {
            ...
            sentinel_ai_shell::util::config::set_screen_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_keybind_enabled" => {
            ...
            sentinel_ai_shell::util::config::set_voice_keybind_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "voice_capture_mode" => {
            ...
            sentinel_ai_shell::util::config::set_voice_capture_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_stt_language" => {
            ...
            sentinel_ai_shell::util::config::set_voice_stt_language(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
            ...
            sentinel_ai_shell::util::config::set_max_thoughts_width(i)
                .await
                .map_err(|e| e.to_string())
        }
        "show_tips" => {
            ...
            sentinel_ai_shell::util::config::set_show_tips(b)
                .await
                .map_err(|e| e.to_string())
        }
        "auto_update" => {
            ...
            sentinel_ai_shell::util::config::set_auto_update(b)
                .await
                .map_err(|e| e.to_string())
        }
        "fork_secondary_model" => {
            ...
            sentinel_ai_shell::util::config::set_fork_secondary_model(s)
                .await
                .map_err(|e| e.to_string())
        }
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    let disk_result = sentinel_ai_shell::util::config::update_config(|cfg| {
...
pub(super) fn marketplace_outcome_succeeded(
    outcome: &sentinel_hooks_plugins_types::ActionOutcome,
) -> bool {
    outcome.status == sentinel_hooks_plugins_types::OutcomeStatus::Success
}
...
/// serializes (`sentinel_ai_shell::extensions::task::KillTaskResponse` +
/// `sentinel_ai_shell::session::result::ExtMethodResult`) so the contract stays
...
pub(super) fn parse_kill_outcome(
    resp: &str,
) -> Option<sentinel_ai_tools::types::KillOutcome> {
    use sentinel_ai_shell::extensions::task::KillTaskResponse;
    use sentinel_ai_shell::session::result::ExtMethodResult;
...
pub(super) fn parse_subagent_kill_outcome(resp: &str) -> SubagentKillOutcome {
    use sentinel_ai_shell::extensions::task::{
...
pub(super) fn credit_balance_from_config(
    c: sentinel_ai_shell::extensions::billing::BillingConfig,
...
pub(super) async fn fetch_auto_topup_info(
    tx: &sentinel_acp_lib::AcpAgentTx,
...
pub(super) fn parse_auto_topup_response(
    ...
    use sentinel_ai_shell::extensions::billing::GetAutoTopupRuleResponse;
...
/// A blocking flock on the shared, possibly-network `~/.ai` lock must never
...
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match sentinel_ai_shell::active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
