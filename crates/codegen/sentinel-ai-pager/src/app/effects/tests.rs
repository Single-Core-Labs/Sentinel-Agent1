...
use super::*;
use sentinel_ai_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn format_acp_error_rate_limit_surfaces_detail_or_fallback() {
    ...
    let rpm_body = "You are sending requests too quickly. Please slow down, or upgrade to a Ai subscription for higher limits: https://ai.com/superai";
    let rpm = acp::Error::new(RATE_LIMITED_ERROR_CODE, "Rate limited")
        .data(format!("API error (status 429 Too Many Requests): {rpm_body}"));
    assert!(format_acp_error(&rpm, false).contains("ai.com/superai"));
...
/// Untitled ai.com chats must stay listed, rendered as "Untitled".
#[test]
...
fn parse_kill_outcome_reads_result_envelope() {
    use sentinel_ai_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use sentinel_ai_workspace::session::git::RestoreDegree;
...
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<sentinel_acp_lib::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let sentinel_acp_lib::AcpAgentMessage::ExtNotification(args) = msg {
...
/// Redirect `AI_HOME` to a tempdir for test isolation.
fn setup_ai_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        unsafe { std::env::set_var("AI_HOME", tmp.path()) };
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use sentinel_ai_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            sentinel_ai_shell::active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_ai_home_in_tempdir();
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use sentinel_acp_lib::AcpAgentMessage;
    use sentinel_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "x.ai/marketplace/action" => {
                        ...
                        let req: sentinel_hooks_plugins_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: sentinel_ai_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            ai_home: PathBuf::from("/path/that/must/not/be-read"),
...
fn agent_profile_names_are_valid_builtins() {
    use std::str::FromStr;
    use sentinel_ai_agent::config::BuiltinAgentName;
...
/// Default flags produce no agent profile (uses ai-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (ai-build already has TaskTool).
#[test]
...
/// Neutralize `AI_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(AI_AGENT)]` (process-global env).
fn without_ai_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("AI_AGENT", "")
}
...
#[serial_test::serial(AI_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_ai_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "ai-build-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "ai-build-plan-no-subagents");
...
/// --ask-user alone selects the ai-build-ask-user profile.
#[serial_test::serial(AI_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "ai-build-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `ai-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce ai-build-plan
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> sentinel_ai_shell::session::SessionInfoResponse {
    use sentinel_ai_shell::session::acp_types::{ContextInfo, SessionInfoData};
    sentinel_ai_shell::session::SessionInfoResponse {
...
fn format_session_info_session_auth_ignores_api_key_env() {
    ...
    assert!(!text.contains("Also present: AI_API_KEY"), "{text}");
    assert!(!text.contains("console.x.ai"), "{text}");
    assert!(!text.contains("ai login"), "{text}");
}
#[test]
fn format_session_info_api_key_without_env() {
    ...
    assert!(!text.contains("AI_API_KEY"), "{text}");
    assert!(!text.contains("Manage account and credits"), "{text}");
    assert!(
            text.contains("Run `ai login` to use your SuperAi subscription instead."),
            "{text}"
        );
    assert!(!text.contains("ai.com"), "{text}");
}
#[test]
fn format_session_info_api_key_auth_suggests_ai_login() {
    let info = make_session_info("auto", None, 1000, 10000);
    let text = format_session_info(&info, None, false, true, true);
    assert!(text.contains("Auth method: API key (AI_API_KEY)"), "{text}");
    assert!(!text.contains("Manage account and credits"), "{text}");
    assert!(
            text.contains("Run `ai login` to use your SuperAi subscription instead."),
            "{text}"
...
fn format_session_info_shows_resolved_when_enabled_and_different() {
    let info = make_session_info("ai-4.5", Some("ai-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: ai-4.5 (ai-4.3)"));
}
#[test]
fn format_session_info_hides_resolved_when_disabled() {
    let info = make_session_info("ai-4.5", Some("ai-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model: ai-4.5"));
    assert!(!text.contains("ai-4.3"));
}
#[test]
fn format_session_info_no_parens_when_resolved_matches_requested() {
    let info = make_session_info("ai-4.5", Some("ai-4.5"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: ai-4.5"));
    assert!(!text.contains("(ai-4.5)"));
}
#[test]
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("ai-build", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model Hash: abc123"));
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    assert_eq!(roster.model_id.as_deref(), Some("ai-4"));
