...
impl WireErrorType {
    pub(crate) fn parse(raw: Option<&str>) -> Self {
        match raw {
            ...
            Some(s) if s == sentinel_ai_shell::extensions::notification::DISK_FULL_ERROR_TYPE => {
                Self::DiskFull
            }
            Some("api") => Self::Api,
            Some("http") => Self::Http,
...
mod tests {
    ...
    fn formats_401_dump_without_triggering_reauth_copy() {
        ...
        let formatted = format_request_failure(
            Some(401),
            Some("api"),
            r#"Unauthorized (401) from https://cli-chat-proxy.ai.com/v1/responses: {"error":"Invalid or expired credentials (auth_kind=bearer)"}"#,
        );
    ...
    fn strips_reqwest_url_from_connection_error() {
        let formatted = format_request_failure(
            None,
            Some("http"),
            "error sending request for url (https://server.ai.com/v1/responses)",
        );
        assert!(
