...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain Ai session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: sentinel_ai_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = sentinel_ai_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = sentinel_ai_shell::util::ai_home::ai_home().join("sessions");
    let encoded = sentinel_ai_shell::util::ai_home::encode_cwd_dirname(&cwd_str);
    ...
    for dir in cwd.ancestors() {
        ...
        if git.is_dir() {
            return std::fs::read_to_string(git.join("ai-worktree-source"))
...
/// unless `AI_CHAT_LOCAL_WORKSPACE_MODE` / attach server id is set.
#[cfg(feature = "local-workspace")]
pub const AI_CHAT_LOCAL_WORKSPACE_ENV: &str = "AI_CHAT_LOCAL_WORKSPACE";
#[cfg(feature = "local-workspace")]
pub const AI_CHAT_LOCAL_WORKSPACE_CWD_ENV: &str = "AI_CHAT_LOCAL_WORKSPACE_CWD";
#[cfg(feature = "local-workspace")]
pub const AI_CHAT_LOCAL_WORKSPACE_MODE_ENV: &str = "AI_CHAT_LOCAL_WORKSPACE_MODE";
#[cfg(feature = "local-workspace")]
pub const AI_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV: &str = "AI_CHAT_LOCAL_WORKSPACE_SERVER_ID";
#[cfg(feature = "local-workspace")]
pub const AI_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV: &str = "AI_CHAT_LOCAL_WORKSPACE_ALLOW_HOME";
/// Skip interactive first-run confirm (still prints the banner).
#[cfg(feature = "local-workspace")]
pub const AI_CHAT_LOCAL_WORKSPACE_ACK_ENV: &str = "AI_CHAT_LOCAL_WORKSPACE_ACK";
...
pub const LOCAL_WORKSPACE_ATTACH_NEEDS_SERVER_ID: &str = "local-workspace attach requires --local-workspace-attach=<server_id> \
     (or AI_CHAT_LOCAL_WORKSPACE_SERVER_ID)";
#[cfg(feature = "local-workspace")]
...
pub const LOCAL_WORKSPACE_HOME_DENIED: &str =
    "local-workspace cwd may not be / or $HOME unless AI_CHAT_LOCAL_WORKSPACE_ALLOW_HOME=1";
#[cfg(feature = "local-workspace")]
...
pub const LOCAL_WORKSPACE_ACK_REQUIRED: &str =
    "local-workspace requires interactive confirm, AI_CHAT_LOCAL_WORKSPACE_ACK=1, or an ack file";
/// Declared advertised tool ids for attach FS-only check (comma-separated).
#[cfg(feature = "local-workspace")]
pub const AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV: &str =
    "AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS";
#[cfg(feature = "local-workspace")]
...
pub fn resolve_local_workspace_config(
    ...
    let env_enable = env_truthy(AI_CHAT_LOCAL_WORKSPACE_ENV);
    let env_mode = env_nonempty(AI_CHAT_LOCAL_WORKSPACE_MODE_ENV);
    let env_server_id = env_nonempty(AI_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    let env_cwd = env_nonempty(AI_CHAT_LOCAL_WORKSPACE_CWD_ENV).map(std::path::PathBuf::from);
    ...
    let mode = if cli_attach.is_some() {
        ...
        match m.as_str() {
            ...
            other => {
                anyhow::bail!(
                    "invalid {AI_CHAT_LOCAL_WORKSPACE_MODE_ENV}={other:?}; expected own|attach"
                )
            }
        }
...
pub fn validate_local_workspace_cwd(path: &std::path::Path) -> anyhow::Result<std::path::PathBuf> {
    ...
    if env_truthy(AI_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV) {
        return Ok(canon);
    }
...
///
/// Skip confirm only with `AI_CHAT_LOCAL_WORKSPACE_ACK=1` or a prior ack file.
...
pub fn local_workspace_ack_satisfied() -> bool {
    if env_truthy(AI_CHAT_LOCAL_WORKSPACE_ACK_ENV) {
        return true;
    }
...
/// via `AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS` (comma-separated ids).
/// Unset / empty → refuse.
#[cfg(feature = "local-workspace")]
...
pub fn probe_advertised_tool_ids() -> Option<Vec<String>> {
    let raw = env_nonempty(AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV)?;
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    sentinel_ai_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub struct MaterializeCtx {
    ...
    /// Process-wide flag: resume targets are ai.com conversations, not
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = sentinel_ai_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'ai' to start a new session."
        )
    })?;
    Ok((first.info.id.to_string(), first.display_title_opt()))
}
/// `AuthManager` for direct ai.com calls made outside the agent (pre-ACP
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &sentinel_ai_shell::agent::config::Config,
) -> std::sync::Arc<sentinel_ai_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(sentinel_ai_shell::auth::AuthManager::new(
        &sentinel_ai_shell::util::ai_home::ai_home(),
        agent_config.ai_com_config.clone(),
    ));
    auth.configure_refresher(
        agent_config.ai_com_config.auth_provider_command.clone(),
        None,
    );
    auth
}
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if sentinel_ai_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = sentinel_ai_shell::session::resolve_local_session(session_id, cwd) {
    ...
    if let Some(original_cwd) = sentinel_ai_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
...
async fn restore_session_from_remote(
    ...
    if let Some((false, source)) =
        sentinel_ai_shell::util::config::session_registry_local_override_sourced(Some(&raw_config))
    {
        anyhow::bail!(
    ...
    let agent_config = sentinel_ai_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use sentinel_ai_shell::agent::session_registry_client::SessionRegistryClient;
    use sentinel_ai_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    use sentinel_ai_shell::session::restore::{RestoreSessionOpts, restore_session_with_storage};
    use sentinel_ai_shell::util::ai_home::ai_home;
    ...
    let storage_client = sentinel_ai_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: sentinel_ai_shell::session::restore::ProgressCallback = Box::new(move |event| {
    ...
    let recovered_local_id = sentinel_ai_shell::session::find_local_child_for_remote(session_id, cwd);
...
mod tests {
    ...
    fn parent_session_is_worktree_summary_session_kind() {
        let mut fx = crate::test_util::AiHomeFixture::new();
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["ai"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["ai", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["ai", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["ai", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["ai", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["ai"])).chat_mode);
    }
    /// hardcoded `false` here once disabled it everywhere.
    #[test]
    fn remote_restore_follows_compiled_restore_stack() {
        assert_eq!(
            MaterializeCtx::from_pager_args(&parse(&["ai"])).allow_remote_restore,
            false
        );
    }
    #[test]
    fn from_pager_args_does_not_probe_tty_for_progress() {
        assert!(
            !MaterializeCtx::from_pager_args(&parse(&["ai"])).restore_progress_on_stdout,
            "stdout vs stderr is decided at the composition root, not from_pager_args"
        );
    }
    #[test]
    fn materialize_ctx_restore_code_follows_cli_flag() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["ai"])).restore_code);
        assert!(!MaterializeCtx::from_pager_args(&parse(&["ai", "-r", "abc"])).restore_code);
        assert!(
            MaterializeCtx::from_pager_args(&parse(&["ai", "-r", "abc", "--restore-code"]))
                .restore_code
        );
    ...
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        ...
        unsafe { std::env::set_var("AI_HOME", home.path()) };
        let cwd = tempfile::tempdir().expect("cwd tempdir");
        let cwd_str = cwd.path().to_string_lossy().to_string();
        let id = "aaaaaaaa-1111-2222-3333-444444444444";
        let encoded = sentinel_ai_shell::util::ai_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = sentinel_ai_shell::util::ai_home::ai_home()
            .join("sessions")
            .join(&encoded);
    ...
    mod resume_by_title {
        use super::*;
        use crate::test_util::AiHomeFixture;
    ...
    #[cfg(feature = "local-workspace")]
    fn advertised_tools_env() -> sentinel_ai_test_support::EnvGuard {
        sentinel_ai_test_support::EnvGuard::set(
            AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV,
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[test]
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE_SERVER_ID)]
    #[test]
    fn resolve_local_workspace_empty_cli_attach_falls_back_to_env() {
        let _env = advertised_tools_env();
        let _sid = sentinel_ai_test_support::EnvGuard::set(
            AI_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV,
            "srv-from-env",
        );
        let tmp = tempfile::tempdir().unwrap();
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE_CWD)]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE)]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE_MODE)]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE_SERVER_ID)]
    #[test]
    fn resolve_local_workspace_cwd_only_is_not_a_request() {
        let tmp = tempfile::tempdir().unwrap();
        let _cwd = sentinel_ai_test_support::EnvGuard::set(
            AI_CHAT_LOCAL_WORKSPACE_CWD_ENV,
            tmp.path().to_str().unwrap(),
        );
        let _enable = sentinel_ai_test_support::EnvGuard::unset(AI_CHAT_LOCAL_WORKSPACE_ENV);
        let _mode = sentinel_ai_test_support::EnvGuard::unset(AI_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = sentinel_ai_test_support::EnvGuard::unset(AI_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_own_env_defaults() {
        let _env = advertised_tools_env();
        let _enable = sentinel_ai_test_support::EnvGuard::set(AI_CHAT_LOCAL_WORKSPACE_ENV, "1");
        let _mode = sentinel_ai_test_support::EnvGuard::unset(AI_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = sentinel_ai_test_support::EnvGuard::unset(AI_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE_ALLOW_HOME)]
    #[serial_test::serial(HOME)]
    #[serial_test::serial(USERPROFILE)]
    #[test]
    fn resolve_local_workspace_defaults_cwd_and_denies_home() {
        let _tools = sentinel_ai_test_support::EnvGuard::set(
            AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV,
            "workspace.fs_list",
        );
        let _allow =
            sentinel_ai_test_support::EnvGuard::unset(AI_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV);
        let home = tempfile::tempdir().unwrap();
        let home_str = home.path().to_str().unwrap();
        let _home = sentinel_ai_test_support::EnvGuard::set("HOME", home_str);
        let _userprofile = sentinel_ai_test_support::EnvGuard::set("USERPROFILE", home_str);
        let err =
    ...
    fn resolve_local_workspace_refuses_uncheckable_toolset() {
        let _tools =
            sentinel_ai_test_support::EnvGuard::unset(AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV);
        let tmp = tempfile::tempdir().unwrap();
        let err =
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(AI_CHAT_LOCAL_WORKSPACE_ACK)]
    #[serial_test::serial(AI_HOME)]
    #[test]
    fn local_workspace_non_tty_requires_ack() {
        let _ack = sentinel_ai_test_support::EnvGuard::unset(AI_CHAT_LOCAL_WORKSPACE_ACK_ENV);
        let home = tempfile::tempdir().unwrap();
        let _home =
            sentinel_ai_test_support::EnvGuard::set("AI_HOME", home.path().to_str().unwrap());
