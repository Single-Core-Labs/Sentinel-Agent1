...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: sentinel_ai_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open ai.com in the browser for SuperAi subscription upsell.
    OpenSuperaiUrl,
    ...
    /// Open ai.com managed connectors, appending session teamId when set.
    ...
    OpenTranscriptPager,
    /// Minimal mode (`ai --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: sentinel_ai_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(sentinel_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(sentinel_hooks_plugins_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(sentinel_hooks_plugins_types::MarketplaceAction),
    ...
    AnnouncementsOpenCta(sentinel_ai_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"ainight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `ai dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.ai/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: sentinel_ai_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.ai/config.toml`.
    ...
    /// Persist a typed setting to `~/.ai/config.toml`. On failure,
    ...
    MarketplaceAction {
        agent_id: AgentId,
        session_id: acp::SessionId,
        action: sentinel_hooks_plugins_types::MarketplaceAction,
    },
    ...
    /// registry (`~/.ai/active_sessions.json`).
    ...
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`ai-build-0.1` when the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<sentinel_ai_workspace::session::git::RestoreDegree>,
    ...
    SessionListLoaded {
        ...
        scope: sentinel_ai_shell::session::unified_list::ListScope,
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<sentinel_ai_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<sentinel_ai_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<sentinel_ai_shell::util::changelog::ChangelogEntry>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<sentinel_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<sentinel_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<sentinel_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<sentinel_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<sentinel_ai_tools::implementations::skills::types::SkillInfo>, String>,
    },
    WorkflowsListLoaded {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        info: Box<sentinel_ai_shell::session::SessionInfoResponse>,
        text: String,
        nonce: u64,
    },
    ...
    SessionUsageComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        usage: Box<sentinel_ai_shell::extensions::notification::PromptUsage>,
        nonce: u64,
    },
    ...
    DeepSearchResults {
        results: Vec<sentinel_ai_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    ...
    GateRefreshed {
        settings: Option<sentinel_ai_shell::util::config::RemoteSettings>,
    },
