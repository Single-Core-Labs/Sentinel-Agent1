...
use sentinel_ai_workspace::foreign_sessions::{
...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, ai_home: &Path) -> [PathBuf; 2] {
        ...
        [
            ai_home
                ...
                .join(skill)
                .join("SKILL.md"),
            ai_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(ai_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, ai_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, ai_home, metadata_exists).await;
...
mod tests {
    ...
    use sentinel_ai_workspace::foreign_sessions::ForeignSessionSource;
    ...
    async fn async_gate_supports_bundled_and_user_skill_locations() {
        let enabled = gated_sources_async_with(compat_all(), Path::new("/ai"), |path| {
