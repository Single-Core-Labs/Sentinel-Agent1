...
impl AgentView {
    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    sentinel_ai_telemetry::session_ctx::log_event(
                        sentinel_ai_telemetry::events::PromptSuggestion {
                            action: sentinel_ai_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                sentinel_ai_telemetry::session_ctx::log_event(
                    sentinel_ai_telemetry::events::PromptSuggestion {
                        action: sentinel_ai_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
    ...
    /// The pty-only `AI_ESC_DOUBLE_PRESS_MS` override can exceed this; no
