...
pub struct BannerSlotParams<'a> {
    ...
    pub(crate) announcements: &'a [sentinel_ai_announcements::RemoteAnnouncement],
...
pub(crate) struct SessionReload {
    ...
    /// Live dedup highwaters (ACP + AI) as of window open (same restore
    /// rationale).
    last_applied_event_seq: Option<u64>,
    last_applied_sentinel_event_seq: Option<u64>,
...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<sentinel_hooks_plugins_types::MarketplacePluginEntry>,
...
pub struct AgentView {
    ...
    ///
    /// ACP stream only — the AI stream keeps its own highwater
    /// ([`Self::last_applied_sentinel_event_seq`]) because the two streams are not
    /// delivered in one id order: ACP lines ride the agent's FIFO event
    /// pipeline while AI lines are emitted direct-to-gateway, so a fresh AI
    ...
    /// AI-stream sibling of [`Self::last_applied_event_seq`] (see there for
    /// why the highwaters are split). Same drop rule, replay-exempt.
    pub last_applied_sentinel_event_seq: Option<u64>,
    /// Raw `eventId` of the most recent update APPLIED to this root session —
    /// replay or live, on both the ACP and AI paths; dropped updates (dedup,
    ...
    pub context_state: Option<sentinel_ai_shell::session::ContextInfo>,
    ...
    pub(crate) deferred_session_mode: Option<sentinel_ai_tools::types::SessionMode>,
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(sentinel_ai_telemetry::events::CreditLimitChoice::PayAsYouGo);
            sentinel_ai_telemetry::session_ctx::log_event(
                sentinel_ai_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            sentinel_ai_telemetry::session_ctx::log_event(
                sentinel_ai_telemetry::events::SuperAiUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn remember_mode_enabled() -> bool {
    let path = sentinel_ai_tools::util::ai_home::ai_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &sentinel_ai_tools::implementations::ai_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(crate) mod test_fixtures {
    ...
    -> crate::views::permission_view::PermissionViewState {
        ...
        let perm = sentinel_acp_lib::AcpArgs {
            request,
            response_tx,
        };
        crate::views::permission_view::PermissionViewState {
