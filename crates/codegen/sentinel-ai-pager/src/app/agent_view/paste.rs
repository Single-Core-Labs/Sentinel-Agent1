                ...
                if let Err(msg) = self.prompt.insert_image(pasted) {
                    ...
                    if ctx.source.tip_showing() {
                        sentinel_ai_telemetry::session_ctx::log_event(
                            sentinel_ai_telemetry::events::ContextualTip {
...
pub(super) mod paste_key_tests {
    ...
    -> crate::views::question_view::QuestionViewState {
        let question = sentinel_ai_tools::implementations::ai_build::ask_user_question::Question {
            question: "Pick one?".to_string(),
            options: vec![
                sentinel_ai_tools::implementations::ai_build::ask_user_question::QuestionOption {
                    label: "A".to_string(),
    ...
    fn agent_completion_inserts_unreadable_file_url_as_path_text() {
        ...
        assert_eq!(
            agent.prompt.text(),
            "/definitely/missing/sentinel-primary-paste.png "
        );
        assert!(agent.prompt.images.is_empty());
    }
    #[test]
