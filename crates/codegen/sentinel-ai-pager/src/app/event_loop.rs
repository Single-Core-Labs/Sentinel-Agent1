...
use sentinel_acp_lib::{AcpClientMessage, acp_send};
...
/// [`decide`](sentinel_ai_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    pending_startup: sentinel_ai_telemetry::startup::PendingStartup,
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("AI_LOG_SAMPLING", "1") };
    }
    ...
    sentinel_ai_telemetry::startup::enter(sentinel_ai_telemetry::startup::StartupPhase::AppInit);
    ...
    let launch_effective_ui = sentinel_ai_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(sentinel_ai_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = sentinel_ai_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match sentinel_ai_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.privacy_notice_rollout = sentinel_ai_config::env_bool("AI_PRIVACY_NOTICE_ROLLOUT")
        .or_else(|| {
            remote_settings
                .as_ref()
        ...
        .unwrap_or(false);
    app.privacy_banner_reshow_days = std::env::var("AI_PRIVACY_BANNER_RESHOW_DAYS")
        .ok()
    ...
    app.privacy_banner_acked = sentinel_ai_shell::config::load_from_disk()
        .ok()
        .and_then(|root| {
            sentinel_ai_shell::util::config::load_config_from_toml(&root)
                .privacy
                .privacy_banner_acked
        });
    app.plugin_cta_enabled = sentinel_ai_config::env_bool("AI_PLUGIN_CTA")
        .or_else(|| remote_settings.as_ref().and_then(|s| s.plugin_cta))
        .unwrap_or(false);
    // Voice is applied after auth_meta so API-key detection is accurate.
    app.session_picker_grouped = std::env::var("AI_SESSION_PICKER_GROUPED")
        .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: find the ai.com method from the advertised list
            let ai_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "ai.com");
            if let Some(method) = ai_com {
                ...
                // No ai.com method available, use the first method as fallback
        ...
        // fail-closed: do not invent ai.com / auto-start OIDC.
        tracing::info!(
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    sentinel_ai_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<sentinel_ai_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == sentinel_ai_shell::agent::auth_method::AI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = sentinel_ai_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = sentinel_ai_shell::config::load_merged_requirements();
    let user_config = sentinel_ai_shell::config::load_from_disk().ok();
    let managed_config = sentinel_ai_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match sentinel_ai_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = sentinel_ai_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > AI_AI_API_BASE_URL env > default).
            let endpoints_base =
                sentinel_ai_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .sentinel_api_base_url;
            app.voice_config =
                sentinel_ai_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to ai-cli server-side (mirrors sampler / imagine). Done after
    ...
    app.zdr_access_enabled = sentinel_ai_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        sentinel_ai_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        sentinel_ai_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        sentinel_ai_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    app.scheduler_background_loops_seed =
        sentinel_ai_shell::util::config::resolve_scheduler_background_loops(
            remote_settings
                .as_ref()
...

    {
        ...
        app.active_announcements = sentinel_ai_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = sentinel_ai_shell::util::tips::pick_and_advance(&app.tips, &ai_home);
        }
    ...
    let hints = sentinel_ai_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.ai/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        sentinel_ai_shell::util::config::clamped_display_permission_mode(
            sentinel_ai_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        sentinel_ai_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = sentinel_ai_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = sentinel_ai_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // AI_SCROLL_SPEED/_MODE/_LINES + AI_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<sentinel_ai_voice::VoiceEvent>>;
    ...
    if let Some(action) = startup_action {
        ...
        app.finish_startup(sentinel_ai_telemetry::startup::StartupOutcome::Ok);
    }

    // Initial prompt from the CLI positional (`ai "fix the bug"`). When
    ...
    // `ai dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `AI_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("AI_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let Err(e) = run_pending_suspends(
            ...
            app.finish_startup(sentinel_ai_telemetry::startup::StartupOutcome::Error);
            return Err(e);
        }
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(sentinel_ai_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice could not start. Restart Ai.");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.ai/config.toml [ui]`
                // rather than `~/.ai/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = sentinel_ai_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> sentinel_ai_shell::agent::config::UiConfig {
    use sentinel_ai_shell::agent::config::UiConfig;
    let Ok(root) = sentinel_ai_shell::config::load_effective_config() else {
...
async fn drain_and_process(
    ...
    let mut handle_one = |routed: &RoutedInputEvent| -> bool {
        ...
        if let Event::Key(ke) = ev
            ...
            && sentinel_ai_voice::AUDIO_SUPPORTED
            && is_voice_chord(ke)
