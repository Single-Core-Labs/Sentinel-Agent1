...
use sentinel_acp_lib::AcpAgentTx;
...
/// [`sentinel_ai_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<sentinel_ai_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: sentinel_ai_shell::util::config::WorktreeHintMode) -> Self {
        use sentinel_ai_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        sentinel_ai_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            sentinel_ai_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`sentinel_ai_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `AI_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `AI_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("AI_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`sentinel_ai_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => sentinel_ai_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    pub pending_startup: Option<sentinel_ai_telemetry::startup::PendingStartup>,
    ...
    /// stays sans-IO.
    pub current_ui: sentinel_ai_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    ...
    /// Current appearance config (hot-reloadable from ~/.ai/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`AI_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `AI_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<sentinel_ai_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<sentinel_ai_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `AI_PLUGIN_CTA`
    ...
    /// No ai.com billing session exists; `/usage` and credit UI stay off.
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `ai dashboard`, the
    ...
    /// non-selectable headers. Gated by `AI_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `AI_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `sentinel-ai-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<sentinel_ai_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        sentinel_ai_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `sentinel_ai_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `AI_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: sentinel_ai_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<sentinel_ai_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `ai wrap` tip: set after the first
    ...
    /// Login button label from `AuthMethod.name` (e.g., "ai.com", "Acme Corp").
    ...
    /// `ai_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, sentinel_ai_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `ai_build_access_gate`. `Some` = blocked.
    pub gate: Option<sentinel_ai_shell::auth::GateInfo>,
    /// User-friendly subscription tier name (e.g. "SuperAi", "Free").
    ...
    pub pending_gate_verification: Option<sentinel_ai_shell::auth::GateInfo>,
    ...
    /// `~/.ai/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `AI_VOICE_MODE=0` — the STT pipeline is not started and
    ...
    pub voice_config: sentinel_ai_voice::VoiceConfig,
    /// Auth for STT (OAuth session via shell `AuthManager`, or `AI_API_KEY`).
    /// `None` until the pipeline is first started (lazy on `/voice`).
    pub voice_auth: Option<sentinel_ai_voice::SharedVoiceAuth>,
    /// Commands into the voice pipeline (start/stop capture — toggle, not hold).
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<sentinel_ai_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub(crate) fn finish_startup(&mut self, outcome: sentinel_ai_telemetry::startup::StartupOutcome) {
        sentinel_ai_telemetry::startup::PendingStartup::finish_held(
    ...
    pub fn gate_from_settings(
        rs: &sentinel_ai_shell::util::config::RemoteSettings,
    ) -> Option<sentinel_ai_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(sentinel_ai_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &sentinel_ai_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            sentinel_ai_telemetry::session_ctx::log_event(
                sentinel_ai_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: sentinel_ai_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: sentinel_ai_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: sentinel_ai_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && sentinel_ai_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: sentinel_ai_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(sentinel_ai_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(sentinel_ai_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `AI_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<sentinel_ai_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    sentinel_ai_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(rect) = ctx.gate_url_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::OpenSuperaiUrl);
                }
                if let Some(rect) = ctx.upgrade_cta_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        sentinel_ai_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
fn dispatch_access_gate_menu_action(index: usize) -> InputOutcome {
    match index {
        0 => InputOutcome::Action(Action::OpenSuperaiUrl),
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                sentinel_ai_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            sentinel_ai_telemetry::session_ctx::log_event(
                                sentinel_ai_telemetry::events::SuperAiUpsellShown {
                                    source:
                                        sentinel_ai_telemetry::events::SuperAiUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`sentinel_ai_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use sentinel_ai_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = sentinel_ai_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                sentinel_ai_telemetry::session_ctx::log_event(
                    sentinel_ai_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: sentinel_ai_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `ai wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::ContextualTip {
                tip: sentinel_ai_telemetry::events::ContextualTipKind::ImageInput,
                action: sentinel_ai_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = sentinel_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    /// including after the fast foreign scan lands rows the default Ai
    ...
    fn tick_demand_fast_while_modal_session_picker_loads() {
        ...
        assert_eq!(
            app.tick_demand(),
            TickDemand::Fast,
            "foreign rows hidden by the Ai filter must not end the loading spinner"
        );
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![sentinel_ai_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![sentinel_ai_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: sentinel_ai_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn external_auth_provider_keeps_billing_off_after_auth_meta() {
        ...
        app.apply_auth_meta(&sentinel_ai_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_disables_billing_surface_for_team_users() {
        let mut app = test_app();
        assert!(app.usage_visible);
        let meta = sentinel_ai_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_enables_billing_surface_for_personal_users() {
        let mut app = test_app();
        app.usage_visible = false;
        let meta = sentinel_ai_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&sentinel_ai_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_lifts_restrictions_for_paid_tiers_and_teams() {
        ...
        advertise_media_tools(&mut app);
        let meta = sentinel_ai_shell::auth::AuthMeta {
            subscription_tier: Some("SuperAi".into()),
            ..Default::default()
        };
        app.apply_auth_meta(&meta);
        assert!(app.tier_restricted_commands.is_empty());
        ...
        app.apply_auth_meta(&sentinel_ai_shell::auth::AuthMeta::default());
        assert!(!app.tier_restricted_commands.is_empty());
        app.subscription_tier = Some("SuperAi".into());
    ...
    fn is_restricted_tier_classification() {
        ...
        assert!(!is_restricted_tier(Some("SuperAi")));
        assert!(!is_restricted_tier(Some("SuperAi Heavy")));
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(sentinel_ai_shell::auth::GateInfo {
            message: "Subscribe to use Ai Build".into(),
            url: Some("https://ai.com/superai?referrer=ai-build".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = sentinel_ai_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(sentinel_ai_workspace::foreign_sessions::RecentForeignSession {
                tool: sentinel_ai_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn esc_owned_before_agent_covers_app_level_owners() {
        ...
        app.import_claude_modal = Some(
            crate::views::import_claude_modal::ImportClaudeModalState::new(
                sentinel_ai_shell::claude_import::ImportPlan::default(),
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![sentinel_ai_workspace::file_system::FuzzyMatchResult {
    ...
    fn install_question_overlay(
        ...
        use sentinel_ai_tools::implementations::ai_build::ask_user_question::{
    ...
    fn welcome_picker_f_cycle_disabled_under_chat_mode() {
        ...
        assert_eq!(
            ...
            crate::views::session_picker::SourceFilter::Ai,
