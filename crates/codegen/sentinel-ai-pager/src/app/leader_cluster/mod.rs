...
//!
//! Env sandboxing follows this crate's `serial(AI_HOME)` idiom; note
//! `ai_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_ai_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `AI_API_KEY`,
//! `AI_LEADER_SOCKET`, `AI_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `ai_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p sentinel-ai-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
use sentinel_acp_lib::{AcpClientRx, acp_send};
...
use sentinel_ai_test_support::MockInferenceServer;
...
/// The ai home the agent actually persisted under: `ai_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_ai_home() -> PathBuf {
    sentinel_ai_config::ai_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same AI_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(AI_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = ai_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("AI_HOME", ai_home.path()),
            crate::test_util::EnvVarGuard::set("AI_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("AI_AI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("AI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("AI_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("AI_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("AI_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (sentinel_ai_version), not
        ...
        generation_tasks.push(tokio::task::spawn_local(async move {
            let _ = run_leader_server(
                ...
                sentinel_ai_shell::agent::activity::AgentActivity::default(),
                tokio::sync::watch::channel(true).1,
                tokio::sync::watch::channel(false).0,
                tokio::sync::watch::channel(sentinel_ai_shell::leader::ShutdownReason::Manual).0,
                None,
                control_state,
            )
            .await;
        ...
        generation_tasks.extend(sentinel_ai_shell::leader::in_process::spawn_agent(
            acp_rx,
            response_tx,
        ));
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same AI_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `AI_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    ai_ws_url: String::new(),
                    ai_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
        ...
        if !self.authenticated {
            let _: acp::AuthenticateResponse = bounded(
                "authenticate",
                acp_send(
                    acp::AuthenticateRequest::new(acp::AuthMethodId::new("sentinel.api_key"))
