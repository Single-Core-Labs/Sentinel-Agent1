...
fn plugins_list_response() -> sentinel_hooks_plugins_types::PluginsListResponse {
    use crate::views::extensions_modal::test_plugin_info;
    sentinel_hooks_plugins_types::PluginsListResponse {
        plugins: vec![
            test_plugin_info(
                "user-tool",
                Some(sentinel_hooks_plugins_types::PluginOrigin::UserAi),
            ),
            test_plugin_info(
                "claude-tool",
                Some(sentinel_hooks_plugins_types::PluginOrigin::UserClaude),
            ),
        ],
    }
}
