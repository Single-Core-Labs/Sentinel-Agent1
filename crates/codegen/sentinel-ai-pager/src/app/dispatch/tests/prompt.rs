...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `ai-build-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `ai-build-0.1`
...
fn prompt_response_formatted_402_takes_credit_limit_path() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PromptResponse {
            agent_id: id,
            result: Err(
                "Request failed (402) \u{2014} Ai Build usage balance exhausted".to_string(),
            ),
            http_status: None,
            prompt_id: None,
        }),
...
fn prompt_response_disk_full_suppresses_turn_failed_and_toast() {
    fn run(pre_seed_disk_full: bool, with_user_echo: bool) -> (bool, bool, usize) {
        ...
        dispatch(
            Action::TaskComplete(TaskResult::PromptResponse {
                agent_id: id,
                result: Err(sentinel_fast_worktree::ENOSPC_OS_MESSAGE.to_string()),
                http_status: None,
                prompt_id: None,
            }),
            &mut app,
...
/// The always-on pipeline end to end, with `AI_SUGGESTIONS` semantics
