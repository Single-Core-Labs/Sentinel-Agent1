...
fn voice_final_replaces_whitespace_only_draft() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        sentinel_ai_voice::VoiceEvent::UtteranceFinal { text: "hi".into() },
    );
    let p = &app.agents.get(&id).unwrap().prompt;
    assert_eq!(p.text(), "hi");
...
/// A free-tier user hitting the voice keybinding gets the SuperAi upsell
...
fn voice_keybinding_on_restricted_tier_opens_upsell() {
    ...
    assert!(
        ...
        "restricted-tier voice keybinding must open the SuperAi upsell"
    );
    assert!(
        !app.voice_listening(),
...
fn voice_interim_sets_then_error_clears_state() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        sentinel_ai_voice::VoiceEvent::InterimTranscript {
            text: "partial".into(),
        },
    );
    ...
    crate::voice::handle_voice_event(
        &mut app,
        sentinel_ai_voice::VoiceEvent::Error {
            message: "boom".into(),
            hint: None,
        },
    );
...
fn voice_toggle_starts_and_stops() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(sentinel_ai_voice::VoiceCommand::PttPress)
    ));
...
fn voice_stt_language_change_recycles_pipeline() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(sentinel_ai_voice::VoiceCommand::Shutdown)
    ));
    assert!(matches!(
        effects.as_slice(),
