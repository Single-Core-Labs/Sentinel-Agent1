...
use super::*;
use sentinel_ai_shell::sampling::error::is_free_usage_exhausted_error;
...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("superai_heavy")));
    assert!(is_max_tier(Some("SuperAi Heavy")));
    assert!(is_max_tier(Some("SUPERAI_HEAVY")));
}

#[test]
fn is_max_tier_non_max_and_unknown() {
    assert!(!is_max_tier(Some("superai")));
...
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("superai_heav")));
    assert!(!is_max_tier(Some("superai_heavy_plus")));
...
#[test]
fn upsell_non_max_upgrade_url_is_superai() {
    let mut app = test_app_with_agent();
    open_upsell_qa(
        &mut app,
        CreditLimitUpsellMode::LegacyPayg { enabled: false },
    ...
    let url = agent_qv(&app).questions[0].options[0]
        ...
        .as_deref()
        .unwrap();
    assert!(url.contains("superai"), "got: {url}");
    assert!(url.contains("referrer=ai-build"), "got: {url}");
}
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): Ai Build usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn complete_session_usage(
    app: &mut AppView,
    session_id: &str,
    usage: sentinel_ai_shell::extensions::notification::PromptUsage,
) -> Vec<Effect> {
    dispatch(
...
fn session_usage_complete_pushes_block_and_chains_billing() {
    ...
    app.screen_mode = crate::app::ScreenMode::Minimal;
    let before = agent_scrollback_len(&app);
    let usage = sentinel_ai_shell::extensions::notification::PromptUsage {
        totals: sentinel_ai_shell::extensions::notification::PromptUsageModel {
            input_tokens: 1_000,
            output_tokens: 100,
            total_tokens: 1_100,
            model_calls: 3,
...
fn session_usage_complete_drops_stale_session() {
    ...
    let effects = complete_session_usage(
        &mut app,
        "old-session",
        sentinel_ai_shell::extensions::notification::PromptUsage {
            totals: sentinel_ai_shell::extensions::notification::PromptUsageModel {
                model_calls: 99,
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("superai_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("superai_heavy"));
}
...
fn free_usage_upsell_shows_two_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: sentinel_ai_telemetry::events::SuperAiUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        ...
        (
            "Upgrade to SuperAi Heavy",
            "Get the most out of Ai Build. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use sentinel_ai_shell::extensions::notification::{RetryState, SessionUpdate};
...
/// Submitting a tier-restricted command opens the two-option SuperAi
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: sentinel_ai_telemetry::events::SuperAiUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
    assert_eq!(q.question, "Unlock all features with SuperAi.");
    assert_eq!(q.options.len(), 2);
    assert_eq!(q.options[0].label, "Upgrade to SuperAi");
    assert_eq!(q.options[0].id.as_deref(), Some(UPSELL_URL_UPGRADE));
    assert_eq!(q.options[1].label, "Upgrade to SuperAi Heavy");
...
/// `AI_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "ai-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("AI_TEST_OPEN_URL_FILE", &bad) };
    ...
    unsafe { std::env::remove_var("AI_TEST_OPEN_URL_FILE") };
}
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("ai-open-url-ok-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("AI_TEST_OPEN_URL_FILE", &url_file) };
...
fn open_url_welcome_toasts_single_line_url_when_browser_unavailable() {
    let bad = std::env::temp_dir().join(format!(
        "ai-open-url-welcome-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("AI_TEST_OPEN_URL_FILE", &bad) };
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "ai-open-url-upsell-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("AI_TEST_OPEN_URL_FILE", &bad) };
...
fn billing_fetched_clears_usage_modal_loading() {
    ...
    assert_eq!(state.ctx.subscription_tier.as_deref(), Some("SuperAi"));
