...
fn cta_entry(name: &str, status: &str) -> sentinel_hooks_plugins_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: sentinel_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
/// Plant a Build session under the process `ai_home()` (OnceLock-cached;
/// do not rely on setting `AI_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = sentinel_ai_shell::util::ai_home::ai_home();
    let encoded = sentinel_ai_shell::util::ai_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: sentinel_ai_tools::implementations::ai_build::ask_user_question::AskUserQuestionMode::Default,
...
fn enqueue_permission_with_enable_always_approve(
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    sentinel_ai_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
    ...
    agent.permission_queue.push_back(PermissionViewState {
        request: sentinel_acp_lib::AcpArgs {
            request,
            response_tx,
        },
        id: 1,
...
const POLICY_WARNING: &str =
    sentinel_ai_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = sentinel_acp_lib::AcpArgs {
