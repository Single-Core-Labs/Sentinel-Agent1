...
fn e2e_compact_auth_failure_holds_prompt_and_resubmits_after_login() {
    ...
    use sentinel_ai_shell::extensions::notification::{RetryState, SessionUpdate as AiSessionUpdate};
    ...
    let id = AgentId(0);
    {
        ...
        apply_session_event_for_test(
            &AiSessionUpdate::RetryState(RetryState::Failed {
...
/// `ai.com` or start an OIDC flow the agent did not advertise.
#[test]
...
fn auth_complete_preserves_show_resolved_model_when_absent() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::AuthComplete {
            request_seq: 1,
            meta: Some(serde_json::to_value(sentinel_ai_shell::auth::AuthMeta::default()).unwrap()),
        }),
        &mut app,
    );
