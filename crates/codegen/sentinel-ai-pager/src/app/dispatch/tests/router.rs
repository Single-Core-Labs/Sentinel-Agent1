...
fn auth_copy_dispatch_preserves_all_delivery_states() {
    for delivery in [
        ...
        let effects = crate::app::dispatch::router::dispatch_copy_auth_url(&mut app, |url| {
            assert_eq!(url, "https://ai.com/auth");
            delivery
        });
...
fn critical_announcement(id: &str) -> sentinel_ai_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> sentinel_ai_announcements::RemoteAnnouncement {
    sentinel_ai_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(sentinel_ai_announcements::AnnouncementCta {
            label: Some("Go".into()),
...
/// the promo url (observed via the `AI_TEST_OPEN_URL_FILE` seam, from every
...
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    ...
    let url_file = std::env::temp_dir().join(format!("ai-cta-open-{}.txt", std::process::id()));
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!({
        "scope": "user",
        "path": "/home/user/.ai/skills/pick-best/SKILL.md",
    });
