...
fn dispatch_confirm_reset_setting_reset_dispatches_typed_setter_for_shared_enum() {
    ...
    with_theme_test_env(|| {
        ...
        // Reset → SetTheme("ainight") (the registered default).
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting { key, value, .. } => {
                assert_eq!(*key, "theme");
        ...
        assert_eq!(app.current_ui.theme.as_deref(), Some("ainight"));
    });
}
...
fn context_info_response() -> sentinel_ai_shell::session::SessionInfoResponse {
