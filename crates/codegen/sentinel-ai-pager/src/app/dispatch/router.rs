...
use super::billing::dispatch_open_superai_url;
use super::ctx::{
    ...
    let effects = match action {
        Action::Quit | Action::QuitConfirmed => {
            if let Some(tx) = &app.voice_cmd_tx {
                let _ = tx.try_send(sentinel_ai_voice::VoiceCommand::Shutdown);
            }
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, sentinel_ai_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(sentinel_ai_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(sentinel_ai_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
                open_url_or_show(app, &url);
        ...
        Action::SetCodingDataSharing { opted_in } => set_coding_data_sharing(
            app,
            opted_in,
            sentinel_ai_telemetry::events::CodingDataConsentSource::Settings,
        ),
        ...
        Action::OpenSuperaiUrl => dispatch_open_superai_url(app),
        Action::OpenUrl(url) => {
                ...
                if let Some(ref mut modal) = agent.extensions_modal {
                    if !outcome.message.trim().is_empty() && modal.result_notice.is_none() {
                        let entry_index = match modal.last_plugins_action {
                            Some(sentinel_hooks_plugins_types::PluginsAction::Uninstall { .. }) => None,
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = modal.last_plugins_action.as_ref().map(|a| {
                        ...
                        if let sentinel_hooks_plugins_types::PluginsAction::Uninstall {
                            ref mut confirmed,
                            ..
                        } = action
                        {
