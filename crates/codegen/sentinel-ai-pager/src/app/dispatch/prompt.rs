...
/// Start (if needed) and submit the initial prompt from `ai "<prompt>"`.
///
...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sentinel_ai_telemetry::events::ContextualTip {
            tip: sentinel_ai_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sentinel_ai_telemetry::events::ContextualTip {
            tip: sentinel_ai_telemetry::events::ContextualTipKind::SmallScreen,
            action: sentinel_ai_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(in crate::app) fn show_ssh_wrap_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sentinel_ai_telemetry::events::ContextualTip {
            tip: sentinel_ai_telemetry::events::ContextualTipKind::SshWrap,
            action: sentinel_ai_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_plan_nudge(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sentinel_ai_telemetry::events::ContextualTip {
            tip: sentinel_ai_telemetry::events::ContextualTipKind::PlanMode,
            action: sentinel_ai_telemetry::events::ContextualTipAction::Shown,
        });
    }
    vec![]
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sentinel_ai_telemetry::events::ContextualTip {
            tip: sentinel_ai_telemetry::events::ContextualTipKind::WordSelect,
            action: sentinel_ai_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
pub(super) fn dispatch_accept_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    log_event(sentinel_ai_telemetry::events::ContextualTip {
        tip: sentinel_ai_telemetry::events::ContextualTipKind::WordSelect,
        action: sentinel_ai_telemetry::events::ContextualTipAction::Accepted,
    });
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sentinel_ai_telemetry::events::ContextualTip {
            tip: sentinel_ai_telemetry::events::ContextualTipKind::SendNow,
            action: sentinel_ai_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_send_prompt_inner(
            ...
            .is_restricted(invocation.token)
    {
        ...
        // another question modal already up, `open_superai_upsell` would
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        let exec_result = {
            ...
            if let Some(invocation) = parse_invocation(trimmed) {
                ...
                {
                    use sentinel_ai_telemetry::events::{PagerCommandSource, PagerSlashCommand};
                    use sentinel_ai_telemetry::session_ctx::log_event;
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        let free_usage_blocked = agent.session.free_usage_blocked
            || result
                .as_ref()
                .err()
                .is_some_and(|e| sentinel_ai_shell::sampling::error::is_free_usage_exhausted_error(e));
