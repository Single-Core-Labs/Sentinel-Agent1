...
pub(crate) const FEEDBACK_QUESTION_LABEL: &str = "How can we improve Ai Build?";
...
pub(super) fn dispatch_send_feedback(app: &mut AppView, text: String) -> Vec<Effect> {
    ...
    agent.scrollback.push_block(RenderBlock::system(
        "Thanks for the feedback! The Ai Build team is on it.".to_string(),
    ));
            ...
            Ok(()) => {
                agent
                    ...
                    .push_block(crate::scrollback::block::RenderBlock::system(format!(
                        "Memory saved to {}",
                        crate::util::display_user_ai_path("memory/MEMORY.md")
                    )));
            }
            Err(error) => {
                agent
