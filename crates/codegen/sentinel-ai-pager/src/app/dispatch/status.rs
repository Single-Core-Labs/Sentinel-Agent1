...
fn log_coding_data_consent_selected(
    source: sentinel_ai_telemetry::events::CodingDataConsentSource,
    opted_in: bool,
    previous_opted_in: bool,
) {
    use sentinel_ai_telemetry::events::{CodingDataConsentChoice, CodingDataConsentSelected};
    sentinel_ai_telemetry::session_ctx::log_event(CodingDataConsentSelected {
        source,
...
/// (persists via ACP ext-request, NOT `~/.ai/config.toml`).
...
pub(super) fn append_consumer_billing_surface(app: &mut AppView, agent_id: AgentId) -> Vec<Effect> {
    ...
    // Remote-settings kill switch (`ai_build_usage_redirect_url`): link out
...
pub(super) fn dispatch_manage_billing(app: &mut AppView) -> Vec<Effect> {
    ...
    super::router::dispatch(
        crate::app::actions::Action::OpenUrl("https://ai.com/?_s=usage".to_string()),
        app,
    )
}
...
pub(in crate::app::dispatch) fn dispatch_privacy_banner_opt_in(app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = set_coding_data_sharing(
        app,
        true,
        sentinel_ai_telemetry::events::CodingDataConsentSource::PrivacyBanner,
    );
