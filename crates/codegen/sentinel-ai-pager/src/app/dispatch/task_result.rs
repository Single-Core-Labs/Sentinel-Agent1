...
/// Whether a completed clipboard probe should fall through to the `ai wrap`
/// host-image request. A clean `FullMiss` always qualifies; a remote read
/// *error* (`AttachmentRead`) also qualifies because inside `ai wrap` the
...
/// `osc52_sink_active()`, so this is inert outside `ai wrap`.
    ...
    match result {
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    ai_home: sentinel_ai_tools::util::ai_home::ai_home(),
                    launch_token,
                }]
            } else {
                vec![]
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                sentinel_ai_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
