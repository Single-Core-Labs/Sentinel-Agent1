...
use sentinel_ai_telemetry::events::{SuperAiUpsell, SuperAiUpsellClicked};
...
/// Whether the user is at the highest subscription tier (SuperAi Heavy).
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("superai_heavy") and CCP display names ("SuperAi Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "superai_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://ai.com/superai?referrer=ai-build";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://ai.com?_s=usage";
...
///
/// **`max_tier = true`** (positively identified as SuperAi Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        sentinel_ai_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using Ai Build",
            CreditLimitCardAction::PurchaseCredits,
            sentinel_ai_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            sentinel_ai_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(sentinel_ai_telemetry::events::CreditLimitUpsellShown {
            surface: sentinel_ai_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(sentinel_ai_telemetry::events::CreditLimitUpsellShown {
        surface: sentinel_ai_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            sentinel_ai_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
/// `SuperAiUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_superai_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the SuperAi upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_superai_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}

/// Which situation opened the SuperAi upsell modal. Controls the heading
...
fn open_superai_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            SuperAiUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with SuperAi.",
            SuperAiUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(sentinel_ai_telemetry::events::SuperAiUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        QuestionOption {
            label: "Upgrade to SuperAi".into(),
            description: "For everyday coding and productivity tasks".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to SuperAi Heavy".into(),
            description: "Get the most out of Ai Build. Highest usage limits.".into(),
            preview: None,
            // No Heavy-specific URL exists; the /superai page lists
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<sentinel_ai_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<sentinel_ai_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_superai_url(app: &mut AppView) -> Vec<Effect> {
    log_event(SuperAiUpsellClicked {
        source: SuperAiUpsell::WelcomeScreen,
    ...
    let url = app
        ...
        .unwrap_or("https://ai.com/superai?referrer=ai-build");
    // Funnel attribution: tag CLI-originated SuperAi upsell clicks
    // with `referrer=ai-build`, matching the OAuth consent flow and
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "ai-build");
