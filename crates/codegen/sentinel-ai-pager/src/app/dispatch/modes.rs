...
/// from `~/.ai/sessions/<urlencoded_cwd>/<session_id>/plan.md`.
...
pub(super) fn set_plan_mode(
    ...
    let mode_id = acp::SessionModeId::new(if new {
        sentinel_ai_tools::types::SessionMode::Plan.as_id()
    } else {
        sentinel_ai_tools::types::SessionMode::Default.as_id()
    });
...
pub(super) fn set_yolo_mode_inner(app: &mut AppView, new: bool) {
    ...
    if previous_state != new {
        sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::YoloToggled {
            enabled: new,
            previous_state,
            trigger: sentinel_ai_telemetry::events::YoloTrigger::Pager,
        });
...
fn dispatch_cycle_mode_inner(app: &mut AppView) -> Vec<Effect> {
    ...
    let Some(session_id) = agent.session.session_id.clone() else {
        ...
        let persist_canonical: Option<&'static str> = match (in_plan, in_auto, in_yolo) {
            ...
            (false, false, false) => {
                agent.plan_mode_pending = Some(true);
                agent.deferred_session_mode = Some(sentinel_ai_tools::types::SessionMode::Plan);
    ...
    match (in_plan, in_auto, in_yolo) {
        ...
        (false, false, false) => {
            ...
            vec![Effect::SetSessionMode {
                session_id,
                mode_id: acp::SessionModeId::new(sentinel_ai_tools::types::SessionMode::Plan.as_id()),
            }]
        }
