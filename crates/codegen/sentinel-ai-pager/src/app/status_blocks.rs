...
fn format_cost(m: &sentinel_ai_shell::extensions::notification::PromptUsageModel) -> String {
    use sentinel_ai_shell::extensions::notification::ticks_to_usd;
...
mod tests {
    use super::*;
    use sentinel_ai_shell::extensions::notification::{PromptUsage, PromptUsageModel};
    ...
    fn session_usage_block_lists_models_when_multiple() {
        ...
        usage
            .model_usage
            .insert("ai-build".into(), model_row(100, 10, None));
        usage
            .model_usage
            .insert("ai-4".into(), model_row(50, 5, None));
        let text = session_usage_block_text(&usage);
        assert!(text.contains("By model:"), "{text}");
        assert!(text.contains("ai-build — 100 in / 10 out"), "{text}");
        assert!(text.contains("ai-4 — 50 in / 5 out"), "{text}");
    }
