...
pub struct CwdGitInfo {
    ...
    /// (linked `git worktree`, ai standalone clone, or worktree DB hit).
...
fn compute_snapshot(cwd: &Path) -> GitSnapshot {
    ...
    // Standalone ai worktrees are CoW clones (`.git` is a directory), so
    // `path != commondir` is false; the source marker is the back-pointer.
    let mut marker_main_repo = None;
    for ancestor in cwd.ancestors() {
        let git = ancestor.join(".git");
        if let Ok(contents) = std::fs::read_to_string(git.join("ai-worktree-source"))
...
pub fn worktree_label_index() -> std::collections::HashMap<PathBuf, String> {
    ...
    let Ok(db) = sentinel_fast_worktree::db::WorktreeDb::open_default() else {
        return map;
    };
...
/// match `/Users/user/sentinel`, and trailing slashes on HOME do not break the join.
...
///
/// `AI_NERD_FONTS=1` forces Powerline; `AI_NERD_FONTS=0` forces the fallback.
pub(crate) fn branch_icon() -> &'static str {
    static ICON: OnceLock<&str> = OnceLock::new();
    ICON.get_or_init(|| {
        decide_branch_icon(
            std::env::var("AI_NERD_FONTS").ok().as_deref(),
            HostOs::current(),
...
///
/// An explicit `AI_NERD_FONTS` override (`0`/`false` → off, anything else →
...
mod tests {
    ...
    fn cwd_git_info_lazy_non_repo_is_none() {
        let p = Path::new("/nonexistent-sentinel-git-info-lazy-test-zzz");
    ...
    fn collapse_home_requires_whole_path_component() {
        ...
        assert_eq!(
            collapse_home_path(Path::new("/Users/user/sentinel"), Some(home)),
            "/Users/user/sentinel"
        );
        assert_eq!(
            collapse_home_path(Path::new("/Users/u/sentinel"), Some(home)),
            "~/sentinel"
        );
        assert_eq!(
    ...
    #[test]
    fn compute_cwd_git_info_standalone_ai_worktree() {
    ...
    fn compute_cwd_git_info_nested_repo_does_not_inherit_db_record() {
        ...
        // serial(AI_HOME) orders peers; EnvVarGuard restores on drop so
        // later `open_default()` callers do not see a deleted temp home.
        let _ai_home = crate::test_util::EnvVarGuard::set("AI_HOME", home.path());
        let _ = sentinel_fast_worktree::db::WorktreeDb::open(home.path());

        let wt = crate::test_util::TempGitRepo::init("wt-branch");
        let canon_wt = dunce::canonicalize(&wt.path).unwrap_or_else(|_| wt.path.clone());
        let db = sentinel_fast_worktree::db::WorktreeDb::open(home.path()).unwrap();
        db.register(&sentinel_fast_worktree::db::WorktreeRecord {
            id: "db-wt".into(),
            path: canon_wt,
            source_repo: PathBuf::from("/src/main-repo"),
            repo_name: "main-repo".into(),
            kind: sentinel_fast_worktree::db::WorktreeKind::Session,
            ...
            status: sentinel_fast_worktree::db::WorktreeStatus::Alive,
    ...
    fn update_from_notification_ors_cached_label_into_is_worktree() {
        let dir = PathBuf::from("/nonexistent-sentinel-notif-label-wt");
        {
