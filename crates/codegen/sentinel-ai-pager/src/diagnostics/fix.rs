...
use sentinel_ai_config::managed_text::{
...
pub const SSH_WRAP_FIX_COMMAND: &str = "ai doctor fix terminal.ssh-wrap";
pub const SSH_WRAP_ONE_OFF: &str = "ai wrap ssh <host>";

const MANAGED_NAMESPACE: &str = "ai doctor";
const SSH_WRAP_ALIAS_POSIX: &str = "alias ssh='ai wrap ssh'";
const SSH_WRAP_ALIAS_FISH: &str = "alias ssh 'ai wrap ssh'";
const TMUX_SCANNER_CAVEAT: &str = "Ai checks this file for direct global assignments of this option. Review sourced files, conditionals, plugins, and generated tmux setup yourself.";
...
pub enum FixError {
    ...
    Managed(sentinel_ai_config::managed_text::ManagedConfigError),
    TmuxManaged(sentinel_ai_config::managed_text::ManagedConfigError),
    PostconditionFailed,
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnknownId(id) => write!(
                formatter,
                "`{id}` is not an available Doctor fix. Run `ai doctor fix` to list available fixes."
            ),
            Self::PlatformUnsupported => write!(
                formatter,
                "Automatic SSH setup is not available on Windows. Run `{SSH_WRAP_ONE_OFF}` when needed."
            ),
            Self::HomeUnavailable => formatter.write_str("Ai could not find your home directory."),
            ...
            Self::ByobuConfigUnavailable => formatter.write_str(
                "Ai could not determine Byobu's effective config directory. Keep `BYOBU_CONFIG_DIR` set in this session, then run the fix again.",
            ),
            Self::UnsafeDirectory { label, path } => write!(
                formatter,
                "Ai refused unsafe {label} `{}`. Use a non-root absolute directory without control characters, `~`, `.` or `..` components.",
                path.display()
            ),
                    ...
                    || detail.contains("`ssh` fish function") =>
            {
                write!(
                    formatter,
                    "Ai found an existing SSH alias or function in {} and did not change it: {detail}",
                    path.display()
                )
            }
            Self::ExistingCustomization { path, detail } => write!(
                formatter,
                "Ai found an existing customization in {} and did not change it: {detail}",
                path.display()
            ),
            ...
            Self::PostconditionFailed => formatter
                .write_str("The configuration changed, but Ai could not verify the SSH alias."),
            Self::TmuxPostconditionFailed => formatter.write_str(
                "The configuration changed, but Ai could not verify the managed tmux option.",
            ),
        }
    }
}
...
impl From<sentinel_ai_config::managed_text::ManagedConfigError> for FixError {
    fn from(error: sentinel_ai_config::managed_text::ManagedConfigError) -> Self {
        Self::Managed(error)
    }
}
...
/// existing line elsewhere in the config can defeat Ai's managed block.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum TmuxRemedy {
    ...
    Assignment,
    /// `set -as <option> …`: tmux accumulates these and Ai appends its block
...
const FIX_REGISTRY: &[FixSpec] = &[
    ...
    FixSpec {
        ...
        command: "ai doctor fix terminal.tmux-clipboard",
    ...
    FixSpec {
        ...
        command: "ai doctor fix terminal.dcs-passthrough",
    ...
    FixSpec {
        ...
        command: "ai doctor fix terminal.tmux-extended-keys",
    ...
    FixSpec {
        ...
        command: "ai doctor fix terminal.tmux-truecolor",
...
pub(crate) fn human_fix_command(id: DiagnosticId) -> Option<String> {
    fix_spec(id).map(|spec| format!("ai doctor fix {}", spec.handle))
}
    ...
    for (id, handle, availability) in fixes {
        ...
        match availability {
            AutomaticFixAvailability::Here => output.push_str(&format!(
                "    Run: ai doctor fix {handle}\n    In Ai: /doctor fix {handle}\n"
            )),
            AutomaticFixAvailability::RunLocally => output.push_str(&format!(
                "    On your local computer, run: ai doctor fix {handle}\n"
            )),
        }
    }
    output
...
pub(crate) fn format_fix_preview(plan: &FixPlan) -> String {
    ...
    match &change.backup_path_hint {
        Some(path) => {
            let _ = writeln!(
                output,
                "\nBackup will be saved to: {}\nIf that file exists, Ai will choose a unique name.",
                preview_path(path)
            );
        }
    ...
    match &plan.payload {
        FixPayload::SshWrap(_) => {
            output.push_str(
                "\nWhat this changes:\n  In new interactive shells, `ssh ...` runs as `ai wrap ssh ...`.\n",
            );
            let _ = writeln!(
                output,
...
fn plan_ssh_wrap(
    ...
    Ok(FixPlan {
        ...
        caveats: vec![
            ...
            "`ai wrap` starts the SSH process directly, so the alias does not loop.",
            "Ai checks this file for direct SSH aliases and functions. Review sourced files, plugins, and generated shell setup yourself.",
        ],
