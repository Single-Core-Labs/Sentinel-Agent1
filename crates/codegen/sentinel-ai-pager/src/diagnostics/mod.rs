...
/// Passive input-device probe for `ai doctor` / `/doctor`.
///
...
pub fn apply_voice_probe(report: &mut DiagnosticReport, emit_missing_issue: bool) {
    if !sentinel_ai_voice::AUDIO_SUPPORTED {
        return;
    }
    match sentinel_ai_voice::input_device_info() {
        Ok(device) => {
        ...
        Err(err) => {
            let error = match err {
                sentinel_ai_voice::VoiceError::Config(message) => message,
    ...
    DiagnosticFinding {
        ...
        note: Some(
            "Connect or select a microphone in your system sound settings. On Linux, install a \
             supported audio recorder if none was found on PATH. Then run `/doctor` or `ai \
...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `ai wrap` on the local end, so
...
pub(crate) fn collect_startup_warnings_from(
    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "Ai also saves each copy to the backup file shown in the copy message. To copy \
             directly, run `ai wrap ssh <host>` on your local computer or use a terminal that \
    ...
    if ctx.is_tmux_backed() && matches!(tmux.control_mode, probes::TmuxProbeResult::Available(true))
    {
        let message = match fullscreen_active {
            ...
            Some(false) => "Ai is using inline mode because tmux control mode limits fullscreen",
...
pub(crate) fn wezterm_kitty_keyboard_warning_from(
    ...
    if shape == WezTermShape::SshXtversion {
        ...
        warning.note = Some(
            "For this session, type `\\` and then press Enter. Ai can't negotiate the Kitty \
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(sentinel_ai_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        config_path: None,
        note: Some(format!(
            "Ai is using the user profile. Compare `.ai/sandbox.toml` with {}, then rename \
             or remove the conflicting project profile. Project settings can add profile names \
             but can't redefine a user profile.",
            crate::util::display_user_ai_path("sandbox.toml")
        )),
    })
}

/// Pure SSH `ai wrap` recommendation — suggests launching the session
/// through `ai wrap ssh <host>` on the user's local machine, which gives a
...
/// - `!osc52_sink_active` — no wrap is already capturing our output. `ai
...
pub(crate) fn collect_notification_warnings_with_method(
        ...
        && ctx.brand == TerminalName::Unknown
    {
        let mut warning = TerminalWarning::new(
            ...
            "Ai is using the terminal bell because the terminal was not recognized",
            None,
            None,
        );
        warning.note = Some(format!(
            ...
            crate::util::display_user_ai_path("config.toml")
        ));
        warnings.push(warning);
    }
    ...
    if condition == NotificationCondition::Unfocused && !supports_focus_tracking(ctx.brand) {
        let mut warning = TerminalWarning::new(
            ...
            Some(&crate::util::display_user_ai_path("config.toml")),
        );
        warning.note = Some(
...
pub fn color_support_warning(
    ...
    if level == probes::RuntimeEvidence::Available(ColorLevel::None) {
        ...
        warning.note = Some("Unset `NO_COLOR`, then restart Ai.".to_string());
        return Some(warning);
    }

    // Checked before the detected level is consulted at all: the level says
    // what Ai emits, which is a different question from what survives tmux.
    // A truecolor detection is not evidence that truecolor reaches the
    // terminal, and a session with no color evidence (piped `ai doctor`)
    ...
    if color_passthrough == TmuxColorPassthrough::Reduced {
        ...
        warning.note = Some(format!(
            "Run `tmux source-file {tmux_config_path}`, then detach and reattach: the server \
             ...
             If Ai still reports less than truecolor afterwards, also add `set -g \
    ...
    if is_tmux_backed {
        ...
        warning.note = Some(format!(
            "In the same tmux config, also add `set -g default-terminal \"tmux-256color\"`. Add \
             ...
             `tmux source-file {tmux_config_path}`, then detach and reattach, and restart Ai."
        ));
        return Some(warning);
    }
...
mod tests {
    ...
    fn sandbox_profile_conflict_warning_reports_conflicts() {
        ...
        assert!(w.note.as_deref().is_some_and(|note| {
            note.contains("rename or remove")
                && note.contains(".ai/sandbox.toml")
                && note.contains(&crate::util::display_user_ai_path("sandbox.toml"))
    ...
    // -- ssh_wrap_hint: `ai wrap ssh` recommendation --------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("ai wrap ssh <host>"));
        assert!(
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        ...
        // `ai wrap` — adoption silences the hint by itself.
        ...
        assert!(findings[1].remediation.as_ref().is_some_and(|remediation| {
            remediation.fix.contains("condition = \"always\"")
                && remediation.config_path.as_deref()
                    == Some(crate::util::display_user_ai_path("config.toml").as_str())
        }));
    }
    ...
    fn supports_focus_tracking_known_terminals() {
        ...
        assert!(supports_focus_tracking(TerminalName::AiDesktop));
    ...
    /// Doctor whenever Ai's own detection reported truecolor, so a session
    ...
    /// Piped `ai doctor` has no color evidence, but the tmux client is still
