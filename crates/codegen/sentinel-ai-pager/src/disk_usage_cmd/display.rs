//! Pure renderer over [`DiskUsageReport`]. `sentinel_ai_config::ai_home()`,
...
use sentinel_fast_worktree::WorktreeStatus;
...
pub fn print_report(
    ...
    let home_label = home_prefix_label(&report.ai_home);
    ...
    match report.registry {
        ...
        RegistryState::Unopenable => {
            writeln!(
                ...
                abbreviate(&report.registry_path, &report.ai_home, &home_label)
            )?;
        }
        RegistryState::Corrupt => {
            writeln!(
                out,
                "  Worktree registry is damaged; rows show as untracked. Remove {} and run `ai worktree db rebuild` to recreate it.",
                abbreviate(&report.registry_path, &report.ai_home, &home_label)
            )?;
        }
    }
    if report.worktrees.is_empty() {
        ...
        for (wt, kind) in report.worktrees.iter().zip(&kind_cells) {
            ...
            writeln!(
                ...
                abbreviate(&wt.path, &report.ai_home, &home_label),
            )?;
        }
    }
...
pub fn print_missing_home(ai_home: &str, out: &mut impl Write) -> std::io::Result<()> {
    writeln!(
        out,
        "Nothing on disk yet at {}.",
        home_prefix_label(ai_home)
    )
}
...
fn home_prefix_label(ai_home: &str) -> String {
    crate::util::display_ai_home_prefix_for(Path::new(ai_home))
}
