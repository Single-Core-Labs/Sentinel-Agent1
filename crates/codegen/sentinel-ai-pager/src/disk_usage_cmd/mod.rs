//! `ai du`: what the user ai home uses on disk. It creates no ai home,
...
use sentinel_fast_worktree::{
    ...
    managed_worktree_roots, path_under_worktree_roots, resolve_ai_home,
};
...
#[command(
    after_help = "Lists every top-level directory in the ai home, largest first, then every \
worktree under `worktrees/` and `worktree_pool/` with its size, age, and label. To reclaim space, preview a sweep with \
`ai worktree gc --max-age 7d --dry-run`: without `--max-age`, gc expires nothing, and it \
...
pub fn run(args: DiskUsageArgs) -> Result<()> {
    // The registry's own resolution, unlike sentinel_ai_config::ai_home().
    let ai_home = resolve_ai_home()?;
    let mut out = std::io::stdout().lock();
    let present = ai_home
        .try_exists()
        .with_context(|| format!("cannot stat {}", ai_home.display()))?;
    if !present {
        if args.json {
            return write_report(&empty_report(&ai_home), args.json, &mut out);
        }
        let written = display::print_missing_home(&ai_home.to_string_lossy(), &mut out);
        return Ok(crate::util::ignore_broken_pipe(written)?);
    }
    // Rows store canonical paths, so the home must match to strip-prefix.
    let ai_home = dunce::canonicalize(&ai_home).unwrap_or(ai_home);
    write_report(&collect_report(&ai_home)?, args.json, &mut out)
}
...
fn empty_report(ai_home: &Path) -> DiskUsageReport {
    DiskUsageReport {
        ai_home: ai_home.to_string_lossy().into_owned(),
...
fn collect_report(ai_home: &Path) -> Result<DiskUsageReport> {
    let mut top_level_dirs = Vec::new();
    let mut root_files_bytes = 0u64;
    let mut issues = WalkIssues::default();
    let mut unfollowed_dir_symlinks = 0u64;
    let volume = Volume::of(ai_home);
    let mut worktree_sizes: HashMap<PathBuf, Measure> = HashMap::new();
    let children = std::fs::read_dir(ai_home)
        .with_context(|| format!("cannot read {}", ai_home.display()))?;
    for child in children {
        let child = match child {
            Ok(child) => child,
            Err(e) => {
                tracing::debug!(path = %ai_home.display(), error = %e, "du: home entry could not be read");
    ...
    let (registry, records, registry_path) = load_registry(ai_home);
    let rows = collect_worktrees(ai_home, records, &worktree_sizes, volume);
    issues.merge(rows.issues);
    let (volume_capacity_bytes, volume_available_bytes) = volume_bytes(ai_home).unzip();
    Ok(DiskUsageReport {
        schema_version: SCHEMA_VERSION,
        ai_home: ai_home.to_string_lossy().into_owned(),
        total_bytes,
...
fn load_registry(ai_home: &Path) -> (RegistryState, Vec<WorktreeRecord>, PathBuf) {
    classify(WorktreeDb::open_read_only(ai_home))
}
...
fn collect_worktrees(
    ...
    let roots = managed_worktree_roots(ai_home);
    ...
    for found in discover_worktrees(ai_home).found {
