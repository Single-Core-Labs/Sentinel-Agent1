...
fn untracked_row(bytes: u64) -> WorktreeUsage {
    WorktreeUsage {
        ...
        path: "/wt-home/worktrees/sentinel/wt-1".into(),
    }
}
...
fn collect_report_joins_registry_and_flags_untracked() {
    ...
    let home = base.join("ai-home");
    let tracked = home.join("worktrees/sentinel/wt-tracked");
    let untracked = home.join("worktrees/sentinel/wt-untracked");
...
fn record_registered_via_symlinked_home_joins_as_one_row() {
    ...
    let wt = real_home.join("worktrees/sentinel/wt-a");
    ...
    db.register(&make_record(
        "wt-a",
        &link_home.join("worktrees/sentinel/wt-a"),
        "via-link",
    ))
    .unwrap();
...
fn duplicate_discovered_dirs_size_once() {
    let tmp = tempfile::TempDir::new().unwrap();
    let home = dunce::canonicalize(tmp.path()).unwrap().join("ai-home");
    let wt = home.join("worktrees/sentinel/wt-a");
    std::fs::create_dir_all(&wt).unwrap();
    std::fs::write(wt.join("f.bin"), vec![b'x'; 4096]).unwrap();
    std::os::unix::fs::symlink(&wt, home.join("worktrees/sentinel/wt-alias")).unwrap();
...
fn escape_symlink_is_counted_not_sized() {
    ...
    let base = dunce::canonicalize(tmp.path()).unwrap();
    let home = base.join("ai-home");
    std::fs::create_dir_all(home.join("worktrees/sentinel")).unwrap();
    let external = base.join("external");
    std::fs::create_dir_all(&external).unwrap();
    std::fs::write(external.join("huge.bin"), vec![b'x'; 65536]).unwrap();
    std::os::unix::fs::symlink(&external, home.join("worktrees/sentinel/escape")).unwrap();
...
fn record_at_missing_path_is_omitted() {
    ...
    db.register(&make_record(
        "wt-gone",
        &home.join("worktrees/sentinel/wt-gone"),
        "gone",
    ))
    .unwrap();
...
fn a_row_off_the_anchor_reports_no_size() {
    let tmp = tempfile::TempDir::new().unwrap();
    let wt = tmp.path().join("worktrees/sentinel/wt-a");
    std::fs::create_dir_all(&wt).unwrap();
...
fn every_open_outcome_maps_to_its_state() {
    ...
    db.register(&make_record(
        "wt-a",
        &home.join("worktrees/sentinel/wt-a"),
        "lbl",
    ))
    .unwrap();
...
fn json_shape_is_frozen() {
    let report = DiskUsageReport {
        ...
        registry: RegistryState::Read,
        registry_path: "/home/user/.ai/worktrees.db".into(),
        worktrees: vec![
            WorktreeUsage {
                last_modified_at: Some(1_700_005_000),
                path: "/home/user/.ai/worktrees/sentinel/wt-1".into(),
                ..tracked_row(
                    90,
                    TrackedRow {
                        last_accessed_at: Some(1_700_009_999),
                        label: Some("my-feature".into()),
                        repo_name: "sentinel".into(),
            ...
            WorktreeUsage {
                ...
                path: "/home/user/.ai/worktree_pool/inst/wt-2".into(),
                ..untracked_row(10)
            },
        ],
    };
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "worktrees_outside_managed_roots": 0,
            "registry": "read",
            "registry_path": "/home/user/.ai/worktrees.db",
            "worktrees": [
                {
                    "bytes": 90,
                    "kind": "session",
                    ...
                    "repo_name": "sentinel",
                    "git_ref": "brian/fix",
                    "path": "/home/user/.ai/worktrees/sentinel/wt-1",
                },
                {
                    "bytes": 10,
                    "kind": "pool",
                    ...
                    "repo_name": null,
                    "git_ref": null,
                    "path": "/home/user/.ai/worktree_pool/inst/wt-2",
                },
            ],
        })
    );
...
fn missing_home_json_is_valid_and_empty() {
    let mut out = Vec::new();
    write_report(
        &empty_report(Path::new("/nonexistent/.ai")),
        /*json*/ true,
        &mut out,
    )
    .unwrap();
...
fn print_report_truncates_long_labels_and_keeps_columns_aligned() {
    ...
    let report = DiskUsageReport {
        ...
        worktrees: vec![
            WorktreeUsage {
                path: "/wt-home/worktrees/sentinel/wt-long".into(),
                ..tracked_row(
                    150,
                    TrackedRow {
            ...
            WorktreeUsage {
                path: "/wt-home/worktrees/sentinel/wt-dead".into(),
                ..tracked_row(
                    100,
                    TrackedRow {
            ...
            WorktreeUsage {
                path: "/wt-home/worktrees/sentinel/wt-short".into(),
                ..untracked_row(50)
            },
        ],
        ..DiskUsageReport::default()
    ...
    crate::test_util::assert_path_column_aligned(&text, "worktrees/sentinel/wt-");
}
...
fn print_report_renders_registry_notices() {
    ...
    let cases = [
        ...
        Case {
            ...
            absent: &["db rebuild", "damaged", "Remove $AI_HOME/worktrees.db"],
        },
        Case {
...
fn reclaim_hint_names_a_sequence_that_frees_space() {
    const AGE: &str = "run `ai worktree gc --max-age 7d --dry-run`";
    const RM: &str = "Remove one with `ai worktree rm --dry-run <path>`";
...
fn symlinked_worktrees_dir_is_surfaced_not_silently_dropped() {
    ...
    let wt = elsewhere.join("sentinel/wt-a");
    std::fs::create_dir_all(&wt).unwrap();
...
fn symlinked_default_home_keeps_home_label() {
    ...
    let real_ai = tmp.path().join("ai-on-disk");
    std::fs::create_dir_all(&fake_home).unwrap();
    std::fs::create_dir_all(&real_ai).unwrap();
    std::os::unix::fs::symlink(&real_ai, fake_home.join(".ai")).unwrap();
    let _home = crate::test_util::EnvVarGuard::set("HOME", &fake_home);

    let resolved = dunce::canonicalize(&fake_home).unwrap().join(".ai");
    let canonical = dunce::canonicalize(&resolved).unwrap();
    assert_ne!(canonical, resolved, "the symlink must actually resolve");
    assert_eq!(
        crate::util::display_ai_home_prefix_for(&canonical),
        "~/.ai"
    );
}
