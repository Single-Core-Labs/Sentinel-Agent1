//! Local PTY wrapper: the engine behind `ai wrap` (see [`crate::wrap_cmd`]).
//!
...
fn apply_wrap_child_env(
    ...
    cmd.env("AI_OSC52_SINK", "1");
    cmd.env("LC_AI_OSC52_SINK", "1");
    if let Some(appearance) = appearance {
        let value = appearance.as_env_value();
        cmd.env("AI_APPEARANCE", value);
        cmd.env("LC_AI_APPEARANCE", value);
    }
}

/// Run an arbitrary command inside a local PTY with OSC 52 output filtering.
///
/// This is the engine behind `ai wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // detached — `ai wrap` is short-lived and exits with the child.
...
mod tests {
    ...
    fn apply_wrap_child_env_dark_overrides_parent_light_on_both_names() {
        ...
        cmd.env("AI_APPEARANCE", "light");
        cmd.env("LC_AI_APPEARANCE", "light");
        apply_wrap_child_env(&mut cmd, Some(SystemAppearance::Dark));
        assert_eq!(env_str(&cmd, "AI_APPEARANCE").as_deref(), Some("dark"));
        assert_eq!(env_str(&cmd, "LC_AI_APPEARANCE").as_deref(), Some("dark"));
        assert_eq!(env_str(&cmd, "AI_OSC52_SINK").as_deref(), Some("1"));
        assert_eq!(env_str(&cmd, "LC_AI_OSC52_SINK").as_deref(), Some("1"));
    }

    #[test]
    fn apply_wrap_child_env_light_overrides_parent_dark_on_both_names() {
        let mut cmd = portable_pty::CommandBuilder::new("true");
        cmd.env("AI_APPEARANCE", "dark");
        cmd.env("LC_AI_APPEARANCE", "dark");
        apply_wrap_child_env(&mut cmd, Some(SystemAppearance::Light));
        assert_eq!(env_str(&cmd, "AI_APPEARANCE").as_deref(), Some("light"));
        assert_eq!(
            env_str(&cmd, "LC_AI_APPEARANCE").as_deref(),
            Some("light")
        );
    }
    ...
    fn apply_wrap_child_env_none_does_not_stamp_from_parent_snapshot() {
        let mut cmd = portable_pty::CommandBuilder::new("true");
        cmd.env("AI_APPEARANCE", "dark");
        cmd.env_remove("LC_AI_APPEARANCE");
        apply_wrap_child_env(&mut cmd, None);
        assert_eq!(env_str(&cmd, "AI_APPEARANCE").as_deref(), Some("dark"));
        assert_eq!(env_str(&cmd, "LC_AI_APPEARANCE"), None);
        assert_eq!(env_str(&cmd, "AI_OSC52_SINK").as_deref(), Some("1"));
    }
}
