...
fn numeric_task_id_is_decoded_tracked_and_reaped() {
    ...
    let notif = sentinel_acp_lib::AcpArgs {
...
fn drain_records_task_backgrounded_delivered_at_exit() {
    let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<sentinel_acp_lib::AcpClientMessage>();
    let payload = serde_json::json!({
        "sessionId": "sess-1",
    ...
    tx.send(sentinel_acp_lib::AcpClientMessage::ExtNotification(
...
use super::*;
use sentinel_ai_workspace::permission::types::{RuleAction, ToolFilter};
...
fn domain_mode_web_fetch() {
    ...
    assert_eq!(
        ...
        sentinel_ai_workspace::permission::types::PatternMode::Domain
    );
...
fn handler_answers_ext_method_instead_of_dropping() {
    ...
    let msg = sentinel_acp_lib::AcpClientMessage::ExtMethod(sentinel_acp_lib::AcpArgs {
