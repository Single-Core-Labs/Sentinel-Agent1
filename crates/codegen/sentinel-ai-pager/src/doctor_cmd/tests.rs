...
fn mixed_report() -> DiagnosticReport {
    ...
    report.facts.color = ColorFacts {
        ...
        available_themes: vec![ThemeKind::AiNight, ThemeKind::AiDay],
        total_themes: ThemeKind::ALL.len(),
    };
...
fn human_mixed_fixture_is_exact() {
    assert_eq!(
        ...
        concat!(
            ...
            "    → Automatic setup: `ai doctor fix tmux-clipboard`\n",
            "    → Add `set -g set-clipboard on` to ~/.tmux.conf\n",
            "      Reload tmux after editing.\n",
            "  i terminal.ssh-wrap            Use local SSH wrapping\n",
            "    → Automatic setup: `ai doctor fix ssh-wrap`\n",
            "    → One-off: `ai wrap ssh <host>`\n",
            "\n",
            ...
            "  Some checks only run in Ai. Start Ai and run /doctor.\n",
            "\n",
...
fn fix_preview_contains_exact_change_and_caveats() {
    ...
    assert!(preview.contains("To use once without changing config: `ai wrap ssh <host>`"));
...
fn json_contract_is_structural_stable_ordered_and_ansi_free() {
        ...
        serde_json::json!({
            ...
            "findings": [
                ...
                },
                {
                    ...
                    "remediation": {"fix": "ai wrap ssh <host>", "configPath": null},
                    "automaticRemediation": {
                        "fixId": "terminal.ssh-wrap",
                        "command": "ai doctor fix terminal.ssh-wrap"
                    },
                    "note": null
                }
            ],
    ...
    assert!(!text.contains("Ai Doctor"));
}
...
fn new_named_findings_extend_json_without_schema_changes() {
    ...
    report.facts.clipboard.fix = Some("ai wrap <ssh command> or /minimal".to_owned());
    report.findings.push(DiagnosticFinding {
    ...
    assert_eq!(
        ...
        "ai wrap <ssh command> or /minimal"
    );
