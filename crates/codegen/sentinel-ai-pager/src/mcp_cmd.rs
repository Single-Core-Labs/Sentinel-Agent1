//! `ai mcp` — manage MCP server configurations from the command line.
...
Examples:
  ...
  # Add to the project config (./.ai/config.toml) instead of ~/.ai/config.toml
  ai mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.ai/config.toml`, available in all your projects
    User,
    /// `./.ai/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// flags such as `-y` are passed to the server instead of ai.
    #[arg(value_name = "ARGS")]
    args: Vec<String>,
    ...
    /// Config to write to: user (~/.ai/config.toml) or project (./.ai/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = sentinel_ai_shell::util::config::load_mcp_server_configs_with_project(&cwd);
    let disabled = sentinel_ai_shell::util::config::disabled_mcp_server_names(&cwd);
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `ai mcp add --help` to get started.");
    } else {
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    sentinel_ai_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: ai mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: ai mcp add --transport {label} <name> <url>"
                );
            };
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => sentinel_ai_shell::util::config::user_config_path(),
        McpScope::Project => {
            sentinel_ai_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn scope_display(scope: McpScope, path: &Path) -> String {
    match scope {
        McpScope::User => display_user_ai_path("config.toml"),
...
fn select_remove_site(
    ...
    use sentinel_ai_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    sentinel_ai_shell::util::config::user_config_path(),
                )
            })
        })
}
...
fn mcp_server_is_known(name: &str, cwd: &Path) -> bool {
    sentinel_ai_shell::util::config::cli_known_mcp_server_names(cwd).contains(name)
}
...
fn available_mcp_server_names(cwd: &Path) -> Vec<String> {
    let mut names: Vec<String> = sentinel_ai_shell::util::config::cli_known_mcp_server_names(cwd)
        .into_iter()
        .collect();
    names.sort();
    names
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    if !mcp_server_is_known(name, &cwd) {
        ...
        if !available.is_empty() {
            ...
            eprintln!("No MCP servers configured. Run `ai mcp add --help` to get started.");
        }
        std::process::exit(1);
    }

    let was_disabled = sentinel_ai_shell::util::config::disabled_mcp_server_names(&cwd).contains(name);

    let modified =
        sentinel_ai_shell::util::config::save_mcp_server_enabled_in(name, enabled, &cwd).await?;

    let now_disabled = sentinel_ai_shell::util::config::disabled_mcp_server_names(&cwd).contains(name);
    ...
    let user_config = sentinel_ai_shell::util::config::user_config_path();
    for path in &modified {
        if path == &user_config {
            println!("File modified: {}", display_user_ai_path("config.toml"));
        } else {
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        sentinel_ai_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!("  user: {}", display_user_ai_path("config.toml"));
            eprintln!("  project: {}", project_path.display());
            eprintln!("Specify which one to remove, e.g.: ai mcp remove {name} --scope project");
            std::process::exit(1);
        }
    };
    ...
    // ancestor .ai/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = sentinel_ai_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        sentinel_ai_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_hyphen_flag_without_double_dash_is_rejected() {
        let err = PagerArgs::try_parse_from(["ai", "mcp", "add", "fs", "npx", "-y"])
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        let add = parse_add(&["ai", "mcp", "add", "api", "https://mcp.example.com/mcp"]);
        ...
        let add = parse_add(&["ai", "mcp", "add", "local", "localhost:3000"]);
    ...
    fn add_scope_project_parses_and_invalid_scope_is_rejected() {
        let add = parse_add(&[
            "ai", "mcp", "add", "-s", "project", "fs", "--", "npx", "pkg",
        ]);
        assert_eq!(add.scope, McpScope::Project);

        let err = PagerArgs::try_parse_from([
            "ai", "mcp", "add", "-s", "local", "fs", "--", "npx", "pkg",
        ])
        .expect_err("local is not a ai scope");
    ...
    fn add_legacy_multi_value_env_is_rejected() {
        ...
        let add = parse_add(&[
            "ai", "mcp", "add", "pg", "--env", "A=1", "B=2", "--", "npx", "-y", "server",
        ]);
    ...
    fn add_validates_name_env_and_headers() {
        let add = parse_add(&["ai", "mcp", "add", "fs", "-e", "NOT_A_PAIR", "--", "npx"]);
        ...
        let add = parse_add(&["ai", "mcp", "add", "bad name!", "--", "npx"]);
    ...
    fn add_rejects_mismatched_options_per_transport() {
        let add = parse_add(&["ai", "mcp", "add", "fs", "-H", "X: y", "--", "npx"]);
    ...
    fn add_requires_a_source_for_each_transport() {
        let add = parse_add(&["ai", "mcp", "add", "fs"]);
        ...
        let add = parse_add(&["ai", "mcp", "add", "--transport", "http", "api"]);
    ...
    fn remove_accepts_optional_scope() {
        let args = PagerArgs::try_parse_from(["ai", "mcp", "remove", "fs", "-s", "project"])
    ...
    fn gateway_cli_toggle_names_are_rejected() {
        ...
        assert!(!is_gateway_cli_toggle_name("ai_com_slack"));
        assert!(!is_gateway_cli_toggle_name("user-slack"));
    }

    #[test]
    fn ai_com_known_only_with_toml_definition() {
        // Unique name: `ai_home()` is process-wide OnceLock, so AI_HOME
        // EnvGuard is a no-op if another test already resolved it. A leftover
        // `ai_com_*` in the real ~/.ai disabled list would fail an orphan
        // assertion on a well-known name.
        let name = format!("ai_com_orphan_{}", uuid::Uuid::new_v4().as_simple());

        let orphan = tempfile::tempdir().unwrap();
        git2::Repository::init(orphan.path()).unwrap();
        assert!(
            !mcp_server_is_known(&name, orphan.path()),
            "orphan ai_com_* must not be known by prefix"
        );

        let defined = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(defined.path().join(".ai")).unwrap();
        std::fs::write(
            defined.path().join(".ai").join("config.toml"),
            format!(
                r#"
[mcp_servers.{name}]
...
"#
    ...
    fn enable_and_disable_parse_name() {
        let args = PagerArgs::try_parse_from(["ai", "mcp", "enable", "user-grafana"])
        ...
        let args = PagerArgs::try_parse_from(["ai", "mcp", "disable", "user-slack"])
    ...
    fn enable_disable_require_name() {
        let err = PagerArgs::try_parse_from(["ai", "mcp", "enable"])
            .expect_err("enable without name must fail");
        assert_eq!(err.kind(), clap::error::ErrorKind::MissingRequiredArgument);

        let err = PagerArgs::try_parse_from(["ai", "mcp", "disable"])
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = sentinel_ai_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.ai/config.toml");
