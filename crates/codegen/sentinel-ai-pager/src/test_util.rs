...
pub fn make_worktree_record(
    ...
    path: &std::path::Path,
    label: &str,
) -> sentinel_fast_worktree::WorktreeRecord {
    use sentinel_fast_worktree::{WorktreeKind, WorktreeRecord, WorktreeStatus};
    WorktreeRecord {
        id: id.to_owned(),
...
/// Shared AI_HOME boundary fixture for the resume-by-title startup and
/// pre-sandbox tests.
///
/// `ai_home()` is OnceLock-cached process-wide, so summaries land under the
/// *resolved* home (possibly the real `~/.ai` when another test pinned the
...
impl Default for AiHomeFixture {
    fn default() -> Self {
        Self::new()
    }
}
...
impl TempGitRepo {
    ...
    /// CoW-style standalone clone: `.git` is a directory plus `ai-worktree-source`.
    pub fn standalone_clone(&self, branch: &str) -> Self {
        ...
        std::fs::write(
            path.join(".git").join("ai-worktree-source"),
