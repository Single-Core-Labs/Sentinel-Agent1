...
) -> sentinel_acp_lib::AcpArgsBox<acp::ExtNotification> {
    make_raw_ext_notif(
        method,
        serde_json::json!({
            "sessionId": "sess-1",
...
fn headless_response_started_parses_per_response_fields() {
    ...
    assert_eq!(message_id.as_deref(), Some("msg_01"));
    assert_eq!(model.as_deref(), Some("ai-4"));
...
fn headless_version_mismatch_logs_warn_with_both_versions() {
    ...
    let banner = crate::glyphs::sanitize_toast_message(
        "⚠ Version mismatch: client 0.1.157, leader 0.1.150 — restart ai to match",
    );
    assert!(
...
fn ext_method_reply(
    method: &str,
    params: serde_json::Value,
) -> sentinel_acp_lib::AcpResult<acp::ExtResponse> {
    let raw = serde_json::value::to_raw_value(&params).unwrap();
