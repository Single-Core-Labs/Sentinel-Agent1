...
pub(crate) struct ToolCallEvent {
    ...
    content: Value,
    locations: Value,
    /// True for Ai's backend `web_search`, which folds inline instead of the client split.
...
/// True iff `tc` is Ai's backend `web_search` (`_meta.backend` and `raw_input.variant == "WebSearch"`).
