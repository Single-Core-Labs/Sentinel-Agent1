...
fn messages_model_usage_maps_and_zero_fills() {
    let rows = json!({
        "ai-4": {"inputTokens": 90, "outputTokens": 7, "cacheReadInputTokens": 10, "cacheCreationInputTokens": 25, "costUSD": 0.02},
    });
    let out = messages_model_usage(Some(&rows), Some("ai-4"), 0, Some(131_072));
    let mu = &out["ai-4"];
...
fn messages_model_usage_attributes_web_search_to_current_model() {
    let rows = json!({
        "ai-4": {"inputTokens": 90, "outputTokens": 7, "costUSD": 0.02},
        "ai-mini": {"inputTokens": 5, "outputTokens": 1},
    });
    let out = messages_model_usage(Some(&rows), Some("ai-4"), 3, Some(131_072));
    assert_eq!(out["ai-4"]["webSearchRequests"], 3);
    assert_eq!(out["ai-mini"]["webSearchRequests"], 0);
    assert_eq!(out["ai-4"]["contextWindow"], 131_072);
    assert!(out["ai-mini"]["contextWindow"].is_null());
}
