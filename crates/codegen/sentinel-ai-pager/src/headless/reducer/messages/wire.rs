...
use sentinel_ai_shell::extensions::notification::ResponseUsage;
...
/// Map a Ai permission mode to the Messages `permissionMode` enum, clamping Ai-only values to `default`.
...
/// The `usage.server_tool_use` sub-object (Ai counts backend `web_search` only).
#[derive(Clone, Default, Serialize, Deserialize)]
