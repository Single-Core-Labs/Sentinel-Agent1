...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<sentinel_ai_workspace::permission::types::PermissionRule>> {
...
pub fn parse_permission_rules_lenient(
    allow: &[String],
    deny: &[String],
) -> Vec<sentinel_ai_workspace::permission::types::PermissionRule> {
    let (rules, errors) = parse_permission_rules_inner(allow, deny);
...
pub(crate) fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<sentinel_ai_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = sentinel_ai_shell::agent::config::AgentDefinition::from_json(&value)
...
pub(crate) fn apply_agent_flag(
    agent: &Option<String>,
    config: &mut sentinel_ai_shell::agent::config::Config,
) {
