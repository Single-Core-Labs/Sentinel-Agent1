...
use sentinel_acp_lib::{AcpArgsBox, AcpResult};
...
pub(crate) fn reply_headless_ext_method(args: AcpArgsBox<acp::ExtRequest>) {
    use sentinel_ai_tools::implementations::ai_build::ask_user_question::AskUserQuestionExtResponse;
    use sentinel_ai_tools::implementations::ai_build::exit_plan_mode::ExitPlanModeExtResponse;
...
pub(crate) fn handle_ext_notification(
    notif: &sentinel_acp_lib::AcpArgsBox<acp::ExtNotification>,
) -> ExtEvent {
...
fn decode_session_notification(method: &str, params: &str) -> ExtEvent {
    ...
    enum AiUpdate {
        ...
        ResponseCompleted {
            ...
            #[serde(default)]
            usage: Option<sentinel_ai_shell::extensions::notification::ResponseUsage>,
            #[serde(default)]
            signature: Option<String>,
            #[serde(default)]
    ...
    let sentinel_notif = match serde_json::from_str::<AiNotif>(params) {
        Ok(n) => n,
        Err(e) => {
            tracing::warn!(
                method,
    ...
    match sentinel_notif.update {
        AiUpdate::AutoCompactStarted { percentage } => {
            ExtEvent::Lifecycle(Lifecycle::CompactStarted { percentage })
        }
        AiUpdate::AutoCompactCompleted { tokens_before } => {
            ExtEvent::Lifecycle(Lifecycle::CompactCompleted {
                pre_tokens: tokens_before.unwrap_or(0),
            })
        }
        AiUpdate::AutoCompactFailed { error } => {
            ExtEvent::Lifecycle(Lifecycle::CompactFailed { error })
        }
        AiUpdate::AutoCompactCancelled {} => ExtEvent::Lifecycle(Lifecycle::CompactCancelled),
        AiUpdate::AutoContinueCompleted { total_tokens } => {
            ExtEvent::Lifecycle(Lifecycle::AutoContinue { total_tokens })
        }
        AiUpdate::ImageCompressed { message } => {
            ExtEvent::Lifecycle(Lifecycle::ImageCompressed { message })
        }
        AiUpdate::SubagentSpawned { subagent_id } => ExtEvent::SubagentSpawned { subagent_id },
        AiUpdate::SubagentFinished { subagent_id, .. } => {
            ExtEvent::SubagentFinished { subagent_id }
        }
        AiUpdate::ResponseStarted {
            message_id,
            model,
            input_tokens,
