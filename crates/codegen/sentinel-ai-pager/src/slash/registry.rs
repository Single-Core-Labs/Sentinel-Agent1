...
/// plus `/help`). Must stay covered by [`sentinel_ai_shell::session::PAGER_COMMAND_KEYS`]
...
impl CommandRegistry {
    ...
    /// but disappear from `get()` — invoking one shows the SuperAi upsell
    ...
    fn rebuild_triggers(&mut self) {
        ...
        for (idx, command) in self.commands.iter().enumerate() {
            ...
            // invoking one shows the SuperAi upsell instead.
