...
pub struct SuggestionRow {
    /// Display text (e.g., "/model" or "Ai 4 Fast").
    pub display: String,
...
pub struct SlashInvocation<'a> {
    /// Command token (e.g., "model" for "/model ai-4").
    pub token: &'a str,
...
mod tests {
    ...
    fn parses_invocation_with_args() {
        let inv = parse_invocation("/model ai-code-fast-1").expect("parsed");
        assert_eq!(inv.token, "model");
        assert_eq!(inv.args, "ai-code-fast-1");
    }
    ...
    fn required_arg_command_blocks_without_args() {
        ...
        assert!(!is_command_complete("/model ", &reg));
        assert!(is_command_complete("/model ai-4", &reg));
    }
    ...
    fn cursor_inside_leading_command_with_args_filters_by_prefix() {
        ...
        // Cursor 3 in "/mod ai-4" clamps the query to "mo".
        ctrl.refresh(&state, "/mod ai-4", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
    ...
    fn cursor_inside_recognized_command_with_args_opens_menu() {
        ...
        ctrl.refresh(&state, "/model ai-4", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
    ...
    /// though `get()` blocks execution — invoking one shows the SuperAi
    ...
    fn acp_and_skill_commands_tag_from_map() {
        ...
        let skill_meta = serde_json::json!({
            "scope": "local",
            "path": "/home/user/.ai/skills/skill-cmd/SKILL.md",
        })
        .as_object()
        .cloned()
