...
mod tests {
    ...
    fn with_test_env(f: impl FnOnce()) {
        ...
        theme_cache::set(ThemeKind::AiNight);
        f();
        system_appearance::clear_mock();
        theme_cache::reset_for_test();
    }
    ...
    fn suggest_args_explicit_active_when_not_auto() {
        with_test_env(|| {
            ...
            let ainight = items
                .iter()
                .find(|i| i.insert_text == "ainight")
                .expect("ainight should be in list");
            assert!(
                ainight.description.contains("(active)"),
    ...
    fn run_explicit_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            let result = cmd.run(&mut ctx, "ainight");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "ainight");
                }
                other => panic!("expected Action::SetTheme(\"ainight\"), got {other:?}"),
            }
        });
    }
    ...
    fn run_toggle_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    // available[0] = AiNight; next is available[1].
    ...
    fn run_alias_normalises_to_canonical() {
        with_test_env(|| {
            ...
            // "dark" is an alias for AiNight.
            let result = cmd.run(&mut ctx, "dark");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "ainight", "alias must normalise to canonical");
                }
                other => panic!("expected Action::SetTheme(\"ainight\"), got {other:?}"),
            }
        });
    ...
    fn preview_auto_applies_resolved_theme() {
        with_test_env(|| {
            ...
            // Default auto config maps Light -> AiDay.
            assert_eq!(Theme::current_kind(), ThemeKind::AiDay);
        });
    }
    ...
    fn cancel_preview_unknown_theme_is_no_op() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::AiDay);
            let cmd = ThemeCommand;
