...
mod tests {
    ...
    fn sample_models() -> ModelState {
        ...
        let id_fast = acp::ModelId::new(Arc::from("ai-4.5"));
        models.available.insert(
            id_fast.clone(),
            acp::ModelInfo::new(id_fast.clone(), "Ai 4.5".to_string()),
        );
        let id_pro = acp::ModelId::new(Arc::from("ai-4.3"));
        models.available.insert(
            id_pro.clone(),
            acp::ModelInfo::new(id_pro.clone(), "Ai 4.3".to_string()),
        );
    ...
    fn model_resolves_by_display_name() {
        ...
        let result = cmd.run(&mut ctx, "Ai 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(id)) => {
                assert_eq!(id.0.as_ref(), "ai-4.5");
            }
    ...
    fn model_resolves_by_model_id() {
        ...
        let mut ctx = make_ctx(&models);
        let cmd = model::ModelCommand;
        let result = cmd.run(&mut ctx, "ai-4.3");
        match result {
            CommandResult::Action(Action::SetDefaultModel(id)) => {
                assert_eq!(id.0.as_ref(), "ai-4.3");
            }
            other => panic!("expected Action(SetDefaultModel), got {other:?}"),
        }
    }
    ...
    fn model_suggest_args_returns_available_models() {
        ...
        assert!(
            items
                .iter()
                .any(|i| i.display.starts_with("Ai 4.5") && i.insert_text == "Ai 4.5")
        );
        assert!(
            items
                .iter()
                .any(|i| i.display == "Ai 4.3" && i.insert_text == "Ai 4.3")
        );
    }
    #[test]
    ...
    fn pager_builtin_triggers_are_reserved_in_shell() {
        let reserved: std::collections::HashSet<&str> = sentinel_ai_shell::session::PAGER_COMMAND_KEYS
            .iter()
            .copied()
            .collect();
        ...
        assert!(
            ...
            "pager builtin trigger keys missing from the shell's \
             PAGER_COMMAND_KEYS (sentinel-ai-shell/src/session/slash_commands.rs); \
