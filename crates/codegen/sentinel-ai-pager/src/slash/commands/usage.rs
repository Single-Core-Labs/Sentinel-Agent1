...
//!
//! External-auth deployments (`auth_provider_command`) never reach ai.com
...
fn auth_provider_env_set() -> bool {
    std::env::var("AI_AUTH_PROVIDER_COMMAND")
        .ok()
        .is_some_and(|s| !s.trim().is_empty())
}

fn auth_provider_config_set() -> bool {
    let Ok(raw) = sentinel_ai_shell::config::load_effective_config() else {
        return false;
    };
    let Ok(cfg) = sentinel_ai_shell::agent::config::Config::new_from_toml_cfg(&raw) else {
        return false;
    };
    cfg.ai_com_config
