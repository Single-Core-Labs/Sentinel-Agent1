//! `ai plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `sentinel_ai_shell::plugin` (shared orchestration)
//! and lower crates (`sentinel-ai-agent`, `sentinel-ai-plugin-marketplace`). This
...
use sentinel_ai_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use sentinel_ai_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use sentinel_ai_plugin_marketplace::SourceKind;
use sentinel_ai_plugin_marketplace::git::SourceCacheLease;
use sentinel_ai_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `ai plugin list --json`. The `status` field acts as a
...
/// Typed entry for `ai plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `ai plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  ai plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `ai plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(
        &str,
        &sentinel_ai_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = sentinel_ai_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = sentinel_ai_plugin_marketplace::load_sources(&config);
    sources.extend(sentinel_ai_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in sentinel_ai_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                sentinel_ai_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &sentinel_ai_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = sentinel_ai_plugin_marketplace::git::default_cache_root();
            sentinel_ai_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> sentinel_ai_telemetry::events::InstallKind {
    if is_git {
        sentinel_ai_telemetry::events::InstallKind::Git
    } else {
        sentinel_ai_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: sentinel_ai_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = sentinel_ai_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use sentinel_ai_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &sentinel_ai_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `ai plugin update {}` to update it.",
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            sentinel_ai_telemetry::session_ctx::log_event(
                sentinel_ai_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               ai plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `ai plugin list` to see installed plugins."
        );
    }
    if let Err(e) = sentinel_ai_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    sentinel_ai_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = sentinel_ai_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    sentinel_ai_shell::config::add_disabled_plugin(name)
...
fn cmd_validate(path: &str) -> Result<()> {
    ...
    match load_manifest(&root) {
        ...
        Ok(ManifestLoadResult::NotFound) => {
            println!(
                "No plugin.json found. Ai discovers skills, agents, and hooks \
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `ai plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `ai plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use sentinel_ai_shell::plugin::MarketplaceAddInput;
    ...
    if !force && let MarketplaceAddInput::GitUrl(git_url) = &input {
        sentinel_ai_plugin_marketplace::git::probe_git_remote(git_url).map_err(|e| {
            anyhow::anyhow!(
...
fn find_removal_source<'a>(
    sources: &'a [sentinel_ai_plugin_marketplace::MarketplaceSource],
    input: &str,
    cwd: &Path,
) -> Result<&'a sentinel_ai_plugin_marketplace::MarketplaceSource, String> {
    ...
    let local_input = match plugin::classify_marketplace_add_input(input, cwd) {
        sentinel_ai_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &sentinel_ai_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match sentinel_ai_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use sentinel_ai_plugin_marketplace::MarketplaceSource;
    ...
    fn removal_fixture() -> Vec<MarketplaceSource> {
        vec![
            ...
            MarketplaceSource {
                name: "official".into(),
                kind: SourceKind::Git {
                    url: "https://github.com/sentinel-org/plugin-marketplace.git".into(),
                    branch: None,
                },
            },
            MarketplaceSource {
    ...
    fn find_removal_source_matches_by_url_ignoring_git_suffix() {
        ...
        let found = find_removal_source(
            &sources,
            "https://github.com/sentinel-org/plugin-marketplace",
            Path::new("/"),
        )
        .unwrap();
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        let msg = trust_prompt(
            "\"sentry\" from marketplace \"AI Official\"",
            "sentry@sentinel-org/plugin-marketplace",
        );
        assert!(
            msg.starts_with(
                "Installing \"sentry\" from marketplace \"AI Official\" requires confirmation."
            ),
            "{msg}"
        );
        assert!(msg.contains("hooks, MCP servers, and skills"));
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  ai plugin install sentry@sentinel-org/plugin-marketplace --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  ai plugin install u/r --trust"), "{git}");
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = sentinel_ai_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = sentinel_ai_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
