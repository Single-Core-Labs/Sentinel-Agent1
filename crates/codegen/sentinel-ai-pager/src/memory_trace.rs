...
//!   `AI_MEMTRACE_INTERVAL_SECS` (default 30s) from a detached thread.
...
//!   (`AI_MEMTRACE_THRESHOLD_MB`, default 1 GiB, then doubling), a full
...
//! ## Files
//!
//! `$AI_HOME/memtrace/<start-ts>-<pid>.jsonl` (+ `.1` after 4 MiB
...
//!
//! The lib cannot depend on jemalloc; `sentinel-ai-pager-bin` installs:
...
/// Test-only: `pthread_t` of the live `ai-memtrace` sampler, published when
...
pub use sentinel_tty_utils::{ProcessResources, sample_process_memory};
...
/// Whether a trace sink is installed ([`start`] ran and `AI_MEMTRACE` is
...
/// Env: disable with `AI_MEMTRACE=0|false|off`.
fn enabled_by_env() -> bool {
    !matches!(
        std::env::var("AI_MEMTRACE").ok().as_deref(),
        Some("0") | Some("false") | Some("off")
    )
}

fn interval_from_env() -> Duration {
    let secs = std::env::var("AI_MEMTRACE_INTERVAL_SECS")
        .ok()
...
fn first_threshold_from_env() -> u64 {
    std::env::var("AI_MEMTRACE_THRESHOLD_MB")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(1024)
        .saturating_mul(1 << 20)
...
/// `dir` (e.g. `$AI_HOME/memtrace/`) and spawn the detached sampler
...
/// don't trace. Inert when `AI_MEMTRACE=0`.
///
...
/// (`ai --version`, `ai trace …`) leave no files behind.
...
pub fn default_dir() -> PathBuf {
    sentinel_ai_shell::util::ai_home::ai_home().join("memtrace")
}
