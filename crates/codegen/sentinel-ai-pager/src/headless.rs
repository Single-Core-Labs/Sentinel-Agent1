//! Headless single-turn mode (`ai -p "prompt"`).
//!
//! Runs the agent in-process via `spawn_ai_shell`, drives the ACP lifecycle
...
use sentinel_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use sentinel_ai_shell::agent::auth_method::AuthMethodKind;
use sentinel_ai_shell::agent::config::Config as AgentConfig;
use sentinel_ai_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use sentinel_ai_shell::util::config as cli_config;
use sentinel_ai_telemetry::startup::PendingStartup;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::{AgentShutdownGuard, spawn_ai_shell};
...
pub(crate) fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    sentinel_ai_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn mcp_server_names(cwd: &Path) -> Vec<McpServer> {
    let servers =
        cli_config::load_mcp_servers(cwd, &sentinel_ai_tools::types::compat::CompatConfig::default());
    servers
        .iter()
        .filter_map(|s| {
            let name = match s {
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `ai login` to authenticate \
         (or `ai login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         ai login --device-code\n\n\
         Alternatively, set the AI_API_KEY environment variable \
         or run `ai login` on a machine with a browser."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !sentinel_ai_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !sentinel_ai_shell::util::clipboard::is_remote_session();
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'ai models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    sentinel_ai_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = sentinel_ai_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = sentinel_ai_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        sentinel_ai_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }
    ...
    let spawned = match spawn_ai_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    sentinel_ai_telemetry::startup::enter(crate::acp::StartupPhase::AcpInitialize);
    ...
    sentinel_ai_telemetry::startup::enter(crate::acp::StartupPhase::EagerAuth);
    ...
    sentinel_ai_telemetry::startup::enter(crate::acp::StartupPhase::SessionCreate);
    ...
    let track_active = std::env::var("AI_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = sentinel_ai_shell::active_sessions::register(
            sentinel_ai_shell::active_sessions::ActiveSession {
    ...
    let session_models = if needs_fresh_catalog {
        match sentinel_ai_shell::cli_models::fetch_model_state(&acp_tx).await {
    ...
    if track_active {
        // Non-blocking flock so a slow/network ~/.ai can't hang exit.
        let _ = sentinel_ai_shell::active_sessions::try_unregister(&session_id);
    }
    ...
    let outcome: Result<()> = match prompt_result {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = sentinel_ai_shell::sampling::error::prompt_usage_from_error(&err) {
            ...
            let stop_reason_override =
                (sentinel_ai_shell::sampling::error::stop_reason_for_turn_error(&err) == "MaxTokens")
