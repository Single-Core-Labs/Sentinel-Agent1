...
use sentinel_ai_test_support::{TestProcess, TestProcessConfig, TestSandbox};

const CHILD_ENV: &str = "AI_MEMTRACE_SIGNAL_TOPOLOGY_CHILD";
const CHILD_SUCCESS_MARKER: &str = "AI_MEMTRACE_SIGNAL_TOPOLOGY_OK";
...
async fn memtrace_wait_survives_outer_sigchld_errno_clobber() {
    ...
    let mut config = TestProcessConfig::new()
        ...
        .env(CHILD_ENV, "1")
        .env("AI_MEMTRACE", "1")
        .env("AI_MEMTRACE_INTERVAL_SECS", SAMPLE_INTERVAL_SECS);
    if let Some(scale) = std::env::var_os("AI_TEST_TIMEOUT_SCALE") {
        config = config.env("AI_TEST_TIMEOUT_SCALE", scale);
    }

    let mut child =
        TestProcess::spawn(command, &sandbox, config).expect("spawn isolated signal topology test");
    let child_deadline = sentinel_ai_test_support::scaled(CHILD_DEADLINE);
...
fn run_isolated_topology() {
    ...
    let notifier = std::thread::spawn(move || {
        ...
        let handler_deadline = sentinel_ai_test_support::scaled(HANDLER_DEADLINE);
        while !required_properties_observed() {
            // SAFETY: sampler_pthread is the live ai-memtrace thread published
    ...
    let sample_deadline = sentinel_ai_test_support::scaled(CHILD_DEADLINE);
...
fn wait_for_sampler_pthread() -> usize {
    let deadline = sentinel_ai_test_support::scaled(HANDLER_DEADLINE);
