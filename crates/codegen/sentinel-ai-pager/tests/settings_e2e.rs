...
use sentinel_ai_pager::app::actions::Action;
use sentinel_ai_pager::settings::{
    EnumChoice, PagerLocalSnapshot, SettingCategory, SettingKind, SettingMeta, SettingOwner,
    SettingValue, SettingsRegistry,
};
use sentinel_ai_pager::views::settings_modal::{
...
fn make_state() -> SettingsModalState {
    ...
    sentinel_ai_pager::app::set_voice_mode_enabled_for_test(true);
...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    sentinel_ai_pager::views::settings_modal::render_settings_modal(
...
fn defaults_round_trip_through_registry() {
    use sentinel_ai_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    sentinel_ai_pager::appearance::cache::set_keep_text_selection(
        sentinel_ai_pager::appearance::TextSelection::Flash,
    );
    sentinel_ai_pager::appearance::cache::set_show_thinking_blocks(true);
    sentinel_ai_pager::appearance::cache::set_prompt_suggestions(true);
    sentinel_ai_pager::appearance::cache::set_group_tool_verbs(true);
    sentinel_ai_pager::appearance::cache::set_page_flip_on_send(true);
    sentinel_ai_pager::appearance::cache::set_combine_queued_prompts(false);
    sentinel_ai_pager::appearance::cache::set_scroll_mode(
        sentinel_ai_pager::appearance::ScrollMode::Auto,
    );
    sentinel_ai_pager::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    sentinel_ai_pager::appearance::cache::set_scroll_lines(3);
    ...
    let expected = |key: &str| -> SettingValue {
        match key {
            ...
            "theme" => SettingValue::Enum("ainight"),
            "auto_dark_theme" => SettingValue::Enum("ainight"),
            "auto_light_theme" => SettingValue::Enum("aiday"),
    ...
    for meta in reg.all() {
        ...
        let default_value = sentinel_ai_pager::settings::default_value_for(meta);
...
// `crates/codegen/sentinel-ai-pager/src/app/dispatch.rs::tests` (next to
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use sentinel_ai_pager::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use sentinel_ai_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn reset_overlay_dims_all_rows_except_target() {
    ...
    use sentinel_ai_pager::views::settings_modal::ResetConfirmOverlay;
...
/// "Ask Ai" tip footer at the bottom of the content area in
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask Ai"),
            "[{fixture_label}] docs footer (`Ask Ai`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
            all_text.contains("change theme to aiday"),
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    sentinel_ai_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use sentinel_ai_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use sentinel_ai_shell::agent::config::UiConfig;
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use sentinel_ai_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use sentinel_ai_pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use sentinel_ai_shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr14_default_model_picker_commits_resolved_model_id() {
    let snapshot = PagerLocalSnapshot {
        available_models: vec![
            (
                "Ai 4.5".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("ai-4.5")),
            ),
            (
                "Ai 3".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("ai-3")),
            ),
        ],
        ..PagerLocalSnapshot::default()
    };
    ...
    // Walk down past row 0 ("(no override)") to row 1 ("Ai 4.5").
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Down));
    assert!(
        matches!(outcome, SettingsKeyOutcome::Changed),
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        sentinel_ai_pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        sentinel_ai_pager::settings::default_value_for(mt_meta),
...
fn pr9_privacy_slash_command_takes_no_arguments() {
    use sentinel_ai_pager::slash::commands::builtin_commands;
    use sentinel_ai_pager::slash::registry::CommandRegistry;
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use sentinel_ai_pager::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use sentinel_ai_pager::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use sentinel_ai_pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use sentinel_ai_pager::appearance::TextSelection;
    ...
    sentinel_ai_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    sentinel_ai_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn scroll_mode_picker_enter_dispatches_set_commit() {
    use sentinel_ai_pager::appearance::ScrollMode;

    // Pin the cache-backed live value so the picker seeds at auto (idx 0)
    // regardless of sibling tests on this thread.
    sentinel_ai_pager::appearance::cache::set_scroll_mode(ScrollMode::Auto);
    let mut s = make_state();
    navigate_to(&mut s, "scroll_mode");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
    assert!(
...
fn invert_scroll_enter_dispatches_typed_setter() {
    sentinel_ai_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    sentinel_ai_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    sentinel_ai_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    sentinel_ai_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    sentinel_ai_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    sentinel_ai_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
