...
//!
//! These tests exercise the real `sentinel-ai-pager` binary through a PTY using
...
/// enterprise deploy report: with `AI_GOAL=1`, `/goal` must
...
/// explicitly off (`AI_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `AI_FOLDER_TRUST=1` and a git repo that
...
/// also pins `AI_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
