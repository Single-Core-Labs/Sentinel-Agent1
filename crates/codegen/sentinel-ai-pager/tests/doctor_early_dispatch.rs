...
fn pager_binary() -> Result<std::path::PathBuf, String> {
    for key in ["PAGER_BINARY", "CARGO_BIN_EXE_sentinel-ai-pager"] {
    ...
    Err("PAGER_BINARY/CARGO_BIN_EXE_sentinel-ai-pager not set".to_owned())
}
...
fn doctor_json_bypasses_unrelated_startup_state() {
    ...
    let ai_home = temp.path().join("ai-home");
    std::fs::create_dir_all(&home).expect("create HOME");
    std::fs::create_dir_all(&ai_home).expect("create AI_HOME");
    ...
    let before = directory_entries(&ai_home);
    ...
    assert!(!String::from_utf8_lossy(&output.stdout).contains("Ai Doctor"));

    let after = directory_entries(&ai_home);
...
fn doctor_fix_without_id_lists_tmux_fixes_from_current_probe_evidence() {
    ...
    let home = temp.path().join("home");
    let ai_home = temp.path().join("qhome");
...
fn doctor_hostile_home_and_byobu_create_no_config_files() {
    ...
    for (key, value) in [("HOME", "."), ("BYOBU_CONFIG_DIR", "relative")] {
        let mut command = base_pager_command(&binary, &home, &ai_home, "/bin/bash");
        command
...
fn doctor_fix_without_id_lists_only_applicable_automatic_fixes() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(
        stdout.contains("On your local computer, run: ai doctor fix ssh-wrap"),
        "{stdout}"
    );
    assert!(!home.join(".bashrc").exists());
...
fn doctor_tmux_fix_yes_writes_only_actual_home_tmux_config() {
    ...
    assert!(!ai_home.join(".tmux.conf").exists());
}
...
fn doctor_fix_yes_writes_only_actual_home_shell_rc() {
    ...
    assert_eq!(
        ...
        "# >>> ai doctor >>>\n# >>> terminal.ssh-wrap >>>\nalias ssh='ai wrap ssh'\n# <<< terminal.ssh-wrap <<<\n# <<< ai doctor <<<"
    );
    assert!(!ai_home.join(".bashrc").exists());
}
...
fn doctor_fix_safety_boundaries_are_process_isolated() {
    ...
    assert!(
        stderr.contains("Ai found an existing SSH alias or function")
...
fn run_pager(
    ...
    let mut command = base_pager_command(binary, home, ai_home, shell);
