...
//!
//! Uses `sentinel-ai-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.ai/auth.json` from `AI_AUTH_JSON` (path) or the
//! developer's `~/.ai/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p sentinel-ai-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::PathBuf;
use std::time::Duration;

use sentinel_ai_pager_pty_harness::{PtyHarness, pager_binary};
use sentinel_ai_test_support::TestSandbox;
...
/// Prefer explicit path, else the user's real `~/.ai/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("AI_AUTH_JSON") {
...
fn prepare_sandbox(sandbox: &mut TestSandbox, gate_on: bool) -> Vec<(String, String)> {
    ...
    sandbox.remove_env("AI_API_KEY");

    let home = sandbox.home();
    let ai = sandbox.ai_home();
    let _ = std::fs::create_dir_all(ai);
    if let Some(src) = auth_json_source() {
        let dest = ai.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.ai/auth.json — may hit device login");
    }
    ...
    // of the developer's shell. `AI_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (sentinel_ai_config::
...
fn require_auth() -> bool {
    std::env::var("AI_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness =
        PtyHarness::new_in_sandbox(&binary, ROWS, COLS, &[], &sandbox, &env_refs, None)
            .expect("spawn pager in PTY (sentinel-ai-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "AI_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
