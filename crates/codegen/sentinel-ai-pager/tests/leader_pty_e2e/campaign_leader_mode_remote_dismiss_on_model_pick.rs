...
#[ignore = "PTY e2e; run with cargo test -p sentinel-ai-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared AI_HOME so every spawn elects/attaches to the
    ...
    let socket = ai_home.join("leader-e2e.sock");
    let socket = socket.to_str().expect("socket path is utf-8").to_owned();

    // Session (OAuth) auth, not the harness's default AI_API_KEY: the
    ...
    let state_path = ai_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(ai_home.join("config.toml")).expect("read config.toml");
    assert!(
