...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party AI OAuth (`is_sentinel_auth()`:
    // AuthMode::Oidc + AI issuer); the harness's AI_API_KEY is ApiKey mode
    ...
    let overrides = [
        ...
        EnvOp::set("AI_TRACE_UPLOAD", "true"),
        EnvOp::set("AI_TELEMETRY_TRACE_UPLOAD", "true"),
        EnvOp::set("AI_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"),
    ];
    ...
    //
    // Accounting (sentinel-file-utils upload queue):
    ...
    //   not fire inside this 3s window; with `AI_UPLOAD_QUEUE_AUTH_PROBE_SECS=2`
