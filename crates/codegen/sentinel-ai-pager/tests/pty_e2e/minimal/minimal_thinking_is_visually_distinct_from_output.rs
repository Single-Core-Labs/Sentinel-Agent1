...
async fn run_reasoning_turn(collapse_thinking: bool) -> Turn {
    ...
    std::fs::create_dir_all(content.home().join(".ai")).expect("mk .ai");
    std::fs::write(
        content.home().join(".ai/config.toml"),
        "[ui]\nshow_thinking_blocks = true\n",
    )
    .expect("write config");
    if collapse_thinking {
        let ai_home = content.sandbox().ai_home().to_path_buf();
        std::fs::create_dir_all(&ai_home).expect("mk ai home");
        std::fs::write(
