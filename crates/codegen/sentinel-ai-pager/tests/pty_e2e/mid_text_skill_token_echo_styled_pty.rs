...
use sentinel_ai_pager_pty_harness::StyledLine;
...
/// Seed a user-invocable skill under `dir/.ai/skills`. The workspace must
/// be a git repo: the live session advertises workspace-local skills only for
/// git workspaces (offline `ai inspect` scans plain dirs too) — same shape
/// as the mid_text_skill_token_echo.yaml scenario's `git_init: true`.
fn seed_test_skill(dir: &Path) {
    let skill_dir = dir.join(".ai").join("skills").join("test-skill");
