...
//!
//! The stack is `tmux -> nvim :terminal -> ai`: nvim/tmux repaint ai's
//! pane out-of-band (no ai PTY resize), and ai's diff renderer only
...
//! into the virtual terminal, bypassing ai) and then check whether ai
//! heals it. `NVIM` is set in ai's env so it sees the embedded-editor
//! context the fix keys off (mirroring a real nvim `:terminal`).
//!
//! The fix: ai forces a full clear+repaint on `FocusGained` in
//! editor/multiplexer contexts, so the injected row is gone after refocus.
//! The final assertion guards that heal.

use super::common::*;

/// Unique sentinel that ai would never render on its own.
...
async fn out_of_band_stale_row_heals_on_focus_gained() {
    ...
    let overrides: Vec<(String, String)> =
        vec![("NVIM".into(), "/tmp/ai-pty-harness-fake-nvim.sock".into())];
    ...
    // virtual screen at col 1 — the static left margin ai's diff renderer
    ...
    // ai must not self-heal out-of-band content via ordinary diff redraws:
    // it only rewrites cells whose own model changed, so this row is stranded.
    h.update(Duration::from_millis(300));
    assert!(
        h.contains_text(STALE_MARKER),
        "stale row should survive a normal redraw (ai's diff renderer doesn't own it)\nscreen:\n{}",
        h.screen_contents()
    );

    // A FocusGained (CSI I) forces a full clear+repaint that re-asserts ai's
