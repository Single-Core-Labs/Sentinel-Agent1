...
/// backend, often with the ai.com proxy blocked. A TCP listener that accepts
...
async fn embedded_mode_boots_without_hanging_on_blocked_backend() {
    ...
    let env = [
        ...
        ("AI_HOME", ai_home.to_str().unwrap()),
        ("AI_API_KEY", "test-key-for-ci"),
        ("AI_CLI_CHAT_PROXY_BASE_URL", base.as_str()),
        ("AI_AI_API_BASE_URL", base.as_str()),
