...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("ai-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.ai/config.toml` with a per-model reasoning-effort menu.
    let ai_home = content.home().join(".ai");
    std::fs::create_dir_all(&ai_home).expect("create .ai");
    // Quote the dotted model id: bare `[model.ai-4.5]` is TOML key-path syntax (model.ai-4.5), not the id "ai-4.5".
    std::fs::write(
