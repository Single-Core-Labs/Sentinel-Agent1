...
pub(crate) use sentinel_ai_pager_pty_harness::{
...
pub(crate) const CONTEXTUAL_HINTS_ENV: &[(&str, &str)] = &[("AI_CONTEXTUAL_HINTS", "1")];
...
// `sentinel_ai_pager_pty_harness::flows` (re-exported above).
...
/// the harness's default `AI_API_KEY` (ApiKey/BYOK mode, no auth.json entry)
/// would never fetch `/v1/settings`. Spawns WITHOUT `AI_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    overrides.push(EnvOp::set("AI_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// Explicit overrides for a folder-trust run. A self-built ai auto-trusts,
/// so `AI_TEST_VERSION` simulates a release; the gate is pinned both ways.
pub(crate) fn trust_env(feature_on: bool) -> [(&'static str, &'static str); 2] {
    [
        ("AI_TEST_VERSION", "0.0.0-sim"),
        ("AI_FOLDER_TRUST", if feature_on { "1" } else { "0" }),
    ]
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".ai")
        .join(sentinel_ai_workspace::trust::TRUST_FILE_NAME);
    let store = sentinel_ai_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&sentinel_ai_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `sentinel_ai_pager_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let ai_home = content.home().join(".ai");
    std::fs::create_dir_all(&ai_home).expect("create fake AI_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(ai_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in sentinel-ai-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.ai/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{AI_HOME|HOME}/.ai/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let ai_home = content.home().join(".ai");
    std::fs::create_dir_all(&ai_home).expect("create .ai");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let ai_home = content.home().join(".ai");
    std::fs::create_dir_all(&ai_home).expect("create .ai");
    // Minimal opt-in only — matches load_config's `{AI_HOME|HOME}/.ai/config.toml`.
...
pub(crate) fn spawn_mouse_toggle_pager(content: &ContentController) -> PtyHarness {
    ...
    PtyHarness::spawn_with_content_env_ops(
        ...
        content,
        &[],
        &[EnvOp::set("AI_MOUSE_REPORTING_TOGGLE", "true")],
    )
    .expect("spawn pager")
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "AI_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = sentinel_ai_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
/// Locate `<ai_home>/sessions/<encoded cwd>/<session id>/`, where the shell
...
pub(crate) fn session_dir(content: &ContentController, harness: &mut PtyHarness) -> PathBuf {
    let sessions = content.home().join(".ai").join("sessions");
    for _ in 0..100 {
...
/// `ai wrap` run budget. Same contention math as the requirements-version
...
/// Run `ai wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `AI_HOME`, returning the exit code (`None` only while still running at
...
pub(crate) fn run_wrap_driving(
    ...
    let code = match wait_for_exit_status(&mut harness, WRAP_TIMEOUT) {
        ...
        Ok(PtyExitPoll::Running) => {
            harness.quit().expect("kill ai wrap after timeout");
            None
        }
        Ok(PtyExitPoll::PendingStatus) => {
            panic!("ai wrap exited but portable status remained unavailable for {WRAP_TIMEOUT:?}")
        }
        Err(error) => panic!("poll ai wrap exit: {error:#}"),
    };
...
/// `ai wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$AI_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("AI_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
/// `$AI_PTY_CAST_DIR/<file_stem>.{txt,html}` when the env var is set.
...
pub(crate) use sentinel_ai_pager_pty_harness::host_clipboard::{
...
pub(crate) use sentinel_ai_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
