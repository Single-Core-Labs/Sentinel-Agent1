...
//! Dark themes (AiNight, TokyoNight) share `ai-night.tmTheme`;
//! AiDay uses `ai-day.tmTheme` with deepened colors for light backgrounds.
//!
...
//! `current_kind()` is a nominal `AiNight` (so leftover kind-keyed paths
...
pub use sentinel_ai_markdown::Syntect;
...
static SYNTECT_AINIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_TOKYONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_AIDAY: OnceLock<Syntect> = OnceLock::new();
...
/// reports a nominal `AiNight`, so this returns the night theme. Token
...
pub fn get_syntect() -> &'static Syntect {
    match crate::theme::Theme::current_kind() {
        ...
        | ThemeKind::Auto => SYNTECT_AINIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/ai-night.tmTheme"))),
        ThemeKind::TokyoNight => SYNTECT_TOKYONIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/tokyo-night.tmTheme"))),
        ThemeKind::AiDay => SYNTECT_AIDAY
            .get_or_init(|| Syntect::new(include_bytes!("../assets/ai-day.tmTheme"))),
    }
}
