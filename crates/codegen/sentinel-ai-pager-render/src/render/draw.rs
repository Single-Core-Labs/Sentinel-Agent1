...
//! [`sentinel_ratatui_inline::Terminal`] whose `flush()` returns `bool` indicating
...
use sentinel_ratatui_inline::LinkSpan;
...
pub type PagerTerminal = sentinel_ratatui_inline::Terminal<CrosstermBackend<TermWriter>>;
#[derive(Debug)]
...
pub fn spawn_writer_thread() -> (
    ...
    let test_delay = std::env::var("AI_TEST_FRAME_WRITE_DELAY_MS")
        .ok()
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || -> std::io::Result<()> {
            ...
            let mut writer: Box<dyn std::io::Write> = {
                let tui_out = sentinel_tty_utils::dup_tui_stderr().unwrap_or_else(|_| {
            ...
            while let Ok(payload) = rx.recv() {
                ...
                let result = {
                    let _guard = sentinel_ai_shared::stderr::stderr_lock();
