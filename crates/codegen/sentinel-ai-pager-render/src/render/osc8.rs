...
//! frame diff (see `sentinel_ratatui_inline::Terminal::flush_with_links`).
...
/// One path segment without spaces (`main.rs`, `.ai`, `@scope`). Leading `.`
/// matches dot-directories and `%` matches percent-encoded segments — ai
/// session media lives under `~/.ai/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.ai/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.ai/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    #[test]
    fn scan_detects_ai_session_media_path() {
        // Dot-directory (`.ai`), percent-encoded session segment, and a
        // trailing sentence period — the shape of `image_gen` output prose.
        let line = make_line("Saved to /Users/alice/.ai/sessions/%2Fabc/00000000/images/1.jpg.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.ai/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 =
            make_line("Image generated and saved to /Users/alice/.ai/sessions/%2FUsers%2Fali");
        let row1 = make_line("ce%2Fcode%2Fsentinel/00000000-0000-0000-0000-000000000001/images/1.jpg");
        let rows: Vec<(u16, &Line<'static>, Option<&str>)> =
        ...
        let expected_url = "file:///Users/alice/.ai/sessions/%252FUsers%252Fali\
                            ce%252Fcode%252Fsentinel/00000000-0000-0000-0000-000000000001/images/1.jpg";
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.ai/sessions/%2FUsers%2Fali"
            ) as u16
        );
        ...
        assert_eq!(
            l1.col_end,
            2 + UnicodeWidthStr::width(
                "ce%2Fcode%2Fsentinel/00000000-0000-0000-0000-000000000001/images/1.jpg"
            ) as u16
        );
    }
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.ai/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.ai/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        let line = make_line("Reading /tmp/ai-impl-summary.md now.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/ai-pager-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/sentinel-ai-pager/src/render` should
        // NOT produce a link for the `/sentinel-ai-pager/src/render` substring.
        let line = make_line("find crates/codegen/sentinel-ai-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
