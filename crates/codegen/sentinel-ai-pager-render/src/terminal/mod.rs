...
pub enum TerminalName {
    ...
    JetBrains,
    /// Ai Desktop (Electron app).
    #[strum(to_string = "Ai Desktop")]
    AiDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` ai is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches ai, which
    ...
    pub fn telemetry_snapshot(&self) -> sentinel_ai_telemetry::events::TerminalTelemetry {
        ...
        sentinel_ai_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: sentinel_ai_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> sentinel_ai_shared::session::FeedbackTerminalInfo {
        use sentinel_ai_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(sentinel_ai_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `ai ssh` can
    // forward env vars.
    let mut ctx = build_terminal_context_from_env(&env);
...
/// `sentinel-ai-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "aidesktop" => Some(TerminalName::AiDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.ai/pager.toml` and
