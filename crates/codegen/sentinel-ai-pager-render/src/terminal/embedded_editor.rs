//! Detects whether ai is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` ai is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `sentinel-ai-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
