...
//!
//! Extracted from the `OpenSuperaiUrl` dispatch handler so that any
...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("AI_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "AI_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    sentinel_ai_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// The returned command is TTY-guarded via [`sentinel_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    sentinel_tty_utils::detach_std_command(&mut command);
    command
}
...
///
/// Used by the SuperAi upsell flow to attribute clicks to `referrer=ai-build`,
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/ai session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://ai.com/superai", "referrer", "ai-build");
        assert_eq!(out, "https://ai.com/superai?referrer=ai-build");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://ai.com/superai?referrer=other",
            "referrer",
            "ai-build",
        );
        assert_eq!(out, "https://ai.com/superai?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://ai.com/superai?heavy=1",
            "referrer",
            "ai-build",
        );
        assert_eq!(
            out,
            "https://ai.com/superai?heavy=1&referrer=ai-build"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`ai.com/#superai`); we still want the referrer attached.
        let out = ensure_query_param("https://ai.com/#superai", "referrer", "ai-build");
        assert_eq!(out, "https://ai.com/?referrer=ai-build#superai");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "ai-build");
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://ai.com/superai?referrer=ai-build";
        assert_eq!(
