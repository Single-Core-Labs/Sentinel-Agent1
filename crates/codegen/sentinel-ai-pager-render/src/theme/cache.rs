...
//! lookup must be cheaper than re-loading from `~/.ai/config.toml`.
...
//! `sentinel_ai_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::AiNight as u8);
...
/// back to `AiNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::AiNight as u8 => ThemeKind::AiNight,
        x if x == ThemeKind::AiDay as u8 => ThemeKind::AiDay,
...
/// When `None`, `to_theme_kind()` defaults to `AiNight` / `AiDay`.
...
///
/// On the first call, reads from `~/.ai/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    sentinel_ai_markdown::set_color_level_cap(if locked {
        sentinel_ai_markdown::ColorLevel::Basic
    } else {
        sentinel_ai_markdown::ColorLevel::TrueColor
    });
    sentinel_ai_markdown::set_polarity_safe_syntax(locked);
}
...
///
/// Precedence:
/// 1. Environment variable (`AI_THEME` / `LC_AI_THEME`)
...
fn env_theme_name_from(env: &HashMap<String, String>) -> Option<&str> {
    for key in ["AI_THEME", "LC_AI_THEME"] {
        let Some(raw) = env
            .get(key)
            .map(String::as_str)
...
/// and the user's dark/light theme mapping. Falls back to `AiNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `sentinel_ai_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = sentinel_ai_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = sentinel_ai_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::AiNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            ...
            assert_eq!(current_kind(), ThemeKind::AiNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::AiDay);
            assert_eq!(applied, ThemeKind::AiNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::AiNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::aiday().text_primary,
                "must not serve the cached (AiDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!sentinel_ai_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                sentinel_ai_markdown::polarity_safe_syntax(),
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // aiday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::AiDay), false),
                ThemeKind::AiDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::AiNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_ainight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::AiNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_aiday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::AiDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_ainight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_ainight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::AiDay), true);
            assert_eq!(result, ThemeKind::AiDay);
            assert!(
                !is_auto_mode(),
    ...
    fn env_theme_overrides_config() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(Some("aiday"), Some(ThemeKind::TokyoNight), false),
                ThemeKind::AiDay
            );
            assert!(!is_auto_mode());
        });
    ...
    fn unknown_env_theme_falls_through_to_config() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(Some("not-a-theme"), Some(ThemeKind::AiDay), false),
                ThemeKind::AiDay
            );
        });
    }
    ...
    #[test]
    fn ai_theme_wins_over_lc_and_config() {
        with_test_env(|| {
            let env = theme_env(&[("AI_THEME", "aiday"), ("LC_AI_THEME", "tokyonight")]);
            assert_eq!(
    ...
    #[test]
    fn empty_or_absent_ai_theme_falls_through_to_lc() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("LC_AI_THEME", "aiday")])),
                    Some(ThemeKind::TokyoNight),
                    true,
                ),
                ThemeKind::AiDay
    ...
    #[test]
    fn unknown_ai_theme_falls_through_to_valid_lc() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[
                        ("AI_THEME", "not-a-theme"),
                        ("LC_AI_THEME", "aiday"),
                    ])),
                    Some(ThemeKind::TokyoNight),
                    false,
    ...
    fn unknown_or_empty_theme_env_falls_through_to_config() {
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("AI_THEME", ""), ("LC_AI_THEME", "")])),
                    Some(ThemeKind::AiDay),
                    true,
                ),
                ThemeKind::AiDay
    ...
    fn env_theme_system_alias_arms_auto_on_both_resolve_paths() {
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("AI_THEME", "system")])),
                    Some(ThemeKind::TokyoNight),
                    false,
                ),
                ThemeKind::AiDay
        ...
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("LC_AI_THEME", "auto")])),
                    Some(ThemeKind::TokyoNight),
                    true,
                ),
                ThemeKind::AiNight
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::AiDay);
            assert_eq!(current_kind(), ThemeKind::AiDay);
        });
    }
