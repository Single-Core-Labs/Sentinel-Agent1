...
//!
//! `LC_AI_APPEARANCE` is the SSH-surviving alias (`AcceptEnv LC_*`).
...
/// Ordered lookup: `AI_APPEARANCE`, then `LC_AI_APPEARANCE`, then `COLORFGBG`.
#[must_use]
...
pub fn detect_explicit_from_env_map(env: &HashMap<String, String>) -> Option<SystemAppearance> {
    parse_appearance_var(env_nonempty(env, "AI_APPEARANCE"))
        .or_else(|| parse_appearance_var(env_nonempty(env, "LC_AI_APPEARANCE")))
}
...
mod tests {
    ...
    #[test]
    fn ai_appearance_dark_and_light() {
        assert_eq!(
            detect_from_env_map(&env(&[("AI_APPEARANCE", "dark")])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("AI_APPEARANCE", "Light")])),
            Some(SystemAppearance::Light)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("AI_APPEARANCE", "night")])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("AI_APPEARANCE", "day")])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    fn lc_alias_used_when_canonical_absent() {
        assert_eq!(
            detect_from_env_map(&env(&[("LC_AI_APPEARANCE", "light")])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    fn unknown_or_empty_appearance_falls_through_to_colorfgbg() {
        assert_eq!(
            detect_from_env_map(&env(&[
                ("AI_APPEARANCE", "solarized"),
                ("COLORFGBG", "15;0"),
            ])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("AI_APPEARANCE", ""), ("COLORFGBG", "0;15"),])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    #[test]
    fn unknown_or_empty_lc_falls_through_to_colorfgbg_when_ai_absent() {
        assert_eq!(
            detect_from_env_map(&env(&[
                ("LC_AI_APPEARANCE", "solarized"),
                ("COLORFGBG", "15;0"),
            ])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("LC_AI_APPEARANCE", ""), ("COLORFGBG", "0;15"),])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    fn colorfgbg_map_ignores_explicit_hints() {
        ...
        assert_eq!(
            detect_colorfgbg_from_env_map(&env(&[("AI_APPEARANCE", "dark")])),
            None
        );
    }
}
