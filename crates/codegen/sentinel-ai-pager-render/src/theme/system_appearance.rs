...
//! 2. Explicit env stamps — `AI_APPEARANCE` / `LC_AI_APPEARANCE`
...
/// 1. Desktop APIs
/// 2. Explicit wrap/SSH stamps (`AI_APPEARANCE` / `LC_AI_APPEARANCE`)
...
///
/// Used by `ai wrap` to stamp the *local* OS theme into the child env
...
/// in `config.toml`. When `None`, defaults to `AiNight` / `AiDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::AiDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::AiNight),
    }
}
