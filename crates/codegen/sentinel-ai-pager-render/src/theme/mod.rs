...
//! The default theme is AiNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! AiNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::AiNight, ThemeKind::AiDay];
    ...
    /// when quantized to 256 or 16 colors. AiNight uses neutral grays
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "ainight" | "ai-night" | "dark" => Some(Self::AiNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "aiday" | "ai-day" | "light" | "day" => Some(Self::AiDay),
...
/// `"ainight"` → `"Ai Night"`). Falls back to `value` verbatim.
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.ai/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::AiNight => Self::ainight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::AiDay => Self::aiday(),
            ...
            // if reached, fall back to AiNight.
            ThemeKind::Auto => Self::ainight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `AI_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on ainight, erasing every state signal). We pin each
    ...
    /// theme (e.g., AiNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // AiNight bg field quantizes to Color::Black and these
            // become invisible.
            ...
            // Markdown content: naive Basic quantize lands AiNight
...
pub fn apply_cursor_color() {
    ...
    sentinel_ai_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::AiNight.is_auto());
        assert!(!ThemeKind::AiDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        // — see `ansi16_overrides_gray_hierarchy_collapses_to_two_slots`.
        use ratatui::style::Color;
        let t = Theme::aiday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::ainight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::aiday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. AiNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::AiNight => Theme::ainight(),
                ThemeKind::AiDay => Theme::aiday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_ainight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark AiNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::ainight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("ainight"),
            Some(ThemeKind::AiNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::AiNight));
        assert_eq!(ThemeKind::from_name("aiday"), Some(ThemeKind::AiDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::AiDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("ainight", ThemeKind::AiNight),
            ("ai-night", ThemeKind::AiNight),
            ("dark", ThemeKind::AiNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("aiday", ThemeKind::AiDay),
            ("ai-day", ThemeKind::AiDay),
            ("light", ThemeKind::AiDay),
            ("day", ThemeKind::AiDay),
