...
pub use sentinel_ai_config::ai_home;
pub use sentinel_ai_tools::util::format_bytes;

/// A closed stdout (`ai du | head`) is a clean stop, not a failure.
...
/// Path to `$AI_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    ai_home().join("pager.toml")
}

/// `~/.ai` or `$AI_HOME`, decided by the resolved home rather than by
/// whether `AI_HOME` is set in the environment.
pub fn display_ai_home_prefix() -> String {
    display_ai_home_prefix_for(&ai_home())
}

pub fn display_ai_home_prefix_for(home: &Path) -> String {
    let default = sentinel_ai_config::default_ai_home();
...
/// User-facing path under [`ai_home()`], e.g. ``~/.ai/config.toml``.
pub fn display_user_ai_path(relative: impl AsRef<Path>) -> String {
    display_user_ai_path_for(&ai_home(), relative)
}

fn display_user_ai_path_for(home: &Path, relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_ai_home_prefix_for(home);
...
/// Abbreviate an absolute path for display: prefer [`ai_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let ai = ai_home();
    if let Ok(rest) = path_buf.strip_prefix(&ai) {
        let prefix = display_ai_home_prefix();
...
/// True when `path` is under user [`ai_home()`] (not project `{cwd}/.ai`).
pub fn is_under_user_ai_home(path: &Path) -> bool {
    path.starts_with(ai_home())
}
...
mod tests {
    ...
    #[test]
    fn display_ai_home_prefix_default_install() {
        if std::env::var("AI_HOME").is_ok() {
            return;
        }
        assert_eq!(display_ai_home_prefix(), "~/.ai");
    }

    #[test]
    fn display_user_ai_path_joins_relative() {
        let path = display_user_ai_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".ai") || path.contains("$AI_HOME"));
    }

    #[test]
    fn display_user_ai_path_for_custom_home_uses_override_label() {
        let custom = std::env::temp_dir().join("ai-home-display-regression");
        assert_eq!(
            display_user_ai_path_for(&custom, "config.toml"),
            "$AI_HOME/config.toml"
        );
        assert_eq!(
            display_user_ai_path_for(&custom, "sandbox.toml"),
            "$AI_HOME/sandbox.toml"
        );
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_ai() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.ai/memory/MEMORY.md");
