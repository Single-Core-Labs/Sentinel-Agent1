...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("AI_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = sentinel_ai_config::shell::shell_command_argv(&command_str);
    ...
    // plugin) can spoof `AI_HOOK_EVENT`, `AI_HOOK_NAME`, `AI_SESSION_ID`,
    // `AI_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR`, which are the
    ...
    let mut child = match cmd
        ...
        .env("AI_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("AI_HOOK_NAME", &spec.name)
        .env("AI_SESSION_ID", ctx.session_id)
        .env("AI_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `AI_WORKSPACE_ROOT`; native `.ai` hooks should use
        // `AI_WORKSPACE_ROOT`.
...
///   hooks),
/// * the Ai process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn find_unresolved_skips_resolvable_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${AI_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${AI_SESSION_ID}/${CLAUDE_PLUGIN_ROOT}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.ai-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".ai-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.ai-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.ai-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.ai-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
    ...
    async fn command_hook_session_close_reaps_whole_group() {
        ...
        let envelope = make_envelope();
        let scope = sentinel_ai_tools::util::ProcessScope::new();
