...
pub use sentinel_ai_config::HookProvenance;
...
pub fn parse_hooks_from_config_layers(
    layers: &[sentinel_ai_config::HookConfigLayer],
) -> (Vec<HookSpec>, Vec<HookError>) {
    let home = sentinel_ai_config::user_ai_home();
...
mod tests {
    ...
    fn config_layer(source_name: &str, toml_src: &str) -> sentinel_ai_config::HookConfigLayer {
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let hooks = value.get("hooks").cloned().unwrap();
        sentinel_ai_config::HookConfigLayer::new(
            sentinel_ai_config::HookProvenance::Managed,
            source_name,
            hooks,
        )
    }
    ...
    fn config_layers_additive_and_dedup_keeps_higher_authority() {
        ...
        use sentinel_ai_config::HookProvenance::{Managed, User};
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.ai/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.ai/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "AI_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "AI_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "AI_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "AI_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "AI_HOOKS_PARSE_TEST_MATCHER_VAR";
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "AI_HOOK_EVENT": "spoofed",
                                    "AI_HOOK_NAME": "spoofed",
                                    "AI_SESSION_ID": "spoofed",
                                    "AI_WORKSPACE_ROOT": "/etc",
