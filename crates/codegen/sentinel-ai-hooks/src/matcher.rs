use regex::Regex;
use sentinel_ai_tools::types::{claude_names_for, ai_names_for};
...
///   **exact** match against each name (after external→Ai alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the Ai tool `run_terminal_command`).
///
...
/// plus any Ai tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to Ai registry in
/// `sentinel-ai-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for ai_name in ai_names_for(term) {
            push(ai_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_ai_tool() {
        let m = HookMatcher::new("Bash").unwrap();
        assert!(m.is_match("Bash")); // external alias name
        assert!(m.is_match("run_terminal_command")); // Ai name
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_edit_write_matches_ai_tool_exactly() {
        let m = HookMatcher::new("Edit|Write").unwrap();
        assert!(m.is_match("Edit"));
        assert!(m.is_match("Write"));
        assert!(m.is_match("search_replace")); // Ai equivalent
        assert!(m.is_match("hashline_edit")); // second Ai alias
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn regex_against_claude_alias_matches_ai_tool() {
        // A regex written against an external alias still matches the Ai tool
