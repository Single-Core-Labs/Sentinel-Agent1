...
// (`~/.ai/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_ai_home>/trusted-hook-projects`), or `None` when no user ai home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(sentinel_ai_config::user_ai_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user ai home (set $AI_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$AI_HOME/disabled-hooks`, or `None` when no user ai
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(sentinel_ai_config::user_ai_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        ...
        ai_dir.join("trusted-hook-projects")
    }
