//! Integration tests for sentinel-ai-hooks.
//!
...
use sentinel_ai_hooks::dispatcher;
use sentinel_ai_hooks::event::*;
use sentinel_ai_hooks::result::HookDecision;
use sentinel_ai_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        sentinel_ai_hooks::result::HookRunResult::Success { .. }
    ));
}
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: sentinel_ai_hooks::event::StopFailureKind::RateLimit,
...
/// runner-reserved keys (`AI_HOOK_EVENT`, `AI_HOOK_NAME`,
/// `AI_SESSION_ID`, `AI_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$AI_HOOK_EVENT" > {f}; echo "NAME=$AI_HOOK_NAME" >> {f}; echo "SESSION=$AI_SESSION_ID" >> {f}; echo "ROOT=$AI_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "AI_HOOK_EVENT": "spoofed_event",
                                "AI_HOOK_NAME": "spoofed_name",
                                "AI_SESSION_ID": "spoofed_session",
                                "AI_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "AI_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed AI_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "AI_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed AI_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "AI_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed AI_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use sentinel_ai_hooks::result::HookRunResult;
    let info = match run {
