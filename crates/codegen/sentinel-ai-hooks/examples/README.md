...
Sample hooks for Ai. Copy to `~/.ai/hooks/` to enable globally, or to `<project>/.ai/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.ai/hooks/bin
cp examples/hooks/safe-shell.json ~/.ai/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.ai/hooks/bin/
chmod +x ~/.ai/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.ai/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.ai/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.ai/hooks/bin/
chmod +x ~/.ai/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.ai/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.ai/hooks/bin
cp examples/hooks/session-log.json ~/.ai/hooks/
cp examples/hooks/bin/session-log.sh ~/.ai/hooks/bin/
chmod +x ~/.ai/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.ai/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.ai/hooks/bin
cp examples/hooks/tool-logger.json ~/.ai/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.ai/hooks/bin/
chmod +x ~/.ai/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.ai/hooks/bin
cp examples/hooks/stop-verify.json ~/.ai/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.ai/hooks/bin/
chmod +x ~/.ai/hooks/bin/stop-verify.sh
```
...
- **Event names:** `SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStop`, `SessionEnd` (see the [user guide](../../sentinel-ai-pager/docs/user-guide/10-hooks.md) for the full set)
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Ai names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.ai/hooks/`. The hook stops running on the next session.