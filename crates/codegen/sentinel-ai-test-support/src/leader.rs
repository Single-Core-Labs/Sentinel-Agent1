//! Leader-mode (`ai agent --leader stdio`) test harness.
//!
...
/// (version-skew) test. Falls back to [`ai_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "AI_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`ai_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "AI_BINARY_CLIENT";
...
/// A `ai agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderFixture {
    ...
    pub async fn start(
        ...
        Self::start_with_binary(&ai_binary(), server, cwd, sandbox).await
    }
    ...
    async fn start_with_binary_base_url_timeout(
        ...
        let socket = sandbox.ai_home().join("leader.sock");
        let lock = sandbox.ai_home().join("leader.lock");
        ...
        cmd.envs(sentinel_tty_utils::pager_env())
            .env("AI_CLI_CHAT_PROXY_BASE_URL", base_url)
            .env("AI_AI_API_BASE_URL", base_url)
            .env("AI_MODELS_BASE_URL", base_url)
            .env("AI_FEEDBACK_BASE_URL", base_url)
            .env("AI_TRACE_UPLOAD_URL", base_url)
            .env("AI_API_KEY", "test-key-for-ci")
            .env("AI_LEADER_SOCKET", &socket)
            .env("RUST_LOG", "sentinel_ai_shell=debug");
        let log_path = sandbox.ai_home().join("leader.log");
        ...
        let pid = child.id();
        let tree = match TestProcessTree::try_attach(pid, "persistent ai test leader") {
            Ok(tree) => tree,
            Err(error) => {
                let _ = child.kill();
...
impl LeaderStdioClient {
    async fn spawn_with_binary_and_socket(
        ...
        let mut process = TestProcess::spawn(
            ...
            TestProcessConfig::new()
                ...
                .env("AI_TRACE_UPLOAD_URL", base_url)
                .env("AI_API_KEY", "test-key-for-ci")
                .env("AI_LEADER_SOCKET", leader_socket)
                .env("RUST_LOG", "sentinel_ai_shell=debug"),
        )
        .map_err(|error| {
            io::Error::new(
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init
            .auth_methods
            .iter()
            .find(|method| &*method.id().0 == "sentinel.api_key")
            .expect("sentinel.api_key auth method");
        self.conn
            .authenticate(
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".ai").join("leader.log")).unwrap_or_default()
}
