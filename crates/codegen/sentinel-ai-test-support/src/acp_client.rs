//! ACP stdio clients for testing ai sessions end-to-end: the typed
//! [`AiStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
/// Spawn `ai agent stdio` with the sandbox's canonical hermetic environment.
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init_resp
            ...
            .unwrap_or_else(|| {
                ...
                panic!(
                    "expected auth method 'sentinel.api_key' but got: {ids:?}\n\
...
/// Drives `ai agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`AiStdioClient`] (`ClientSideConnection`,
