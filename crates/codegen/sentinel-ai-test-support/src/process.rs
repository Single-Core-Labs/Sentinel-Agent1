//! Shared subprocess lifecycle ownership for ai-build test harnesses.
//!
...
pub struct TestProcessTree {
    pid: u32,
    label: String,
    group: Option<sentinel_tty_utils::ProcessGroup>,
...
impl TestProcessTree {
    ...
    pub fn attach(pid: u32, label: impl Into<String>) -> Self {
        let label = label.into();
        let (group, attachment_error) = match sentinel_tty_utils::ProcessGroup::new() {
    ...
    fn from_group(pid: u32, label: String, group: sentinel_tty_utils::ProcessGroup) -> Self {
        Self {
            pid,
            label,
            group: Some(group),
...
impl TestProcess {
    ...
    pub fn spawn(
        ...
        cmd.envs(sentinel_tty_utils::pager_env())
            ...
            .stderr(Stdio::piped())
            .kill_on_drop(true);
        sentinel_tty_utils::detach_command(&mut cmd);
