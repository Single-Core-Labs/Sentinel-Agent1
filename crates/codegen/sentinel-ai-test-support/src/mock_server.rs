...
mod tests {
    ...
    fn explicit_request_headers_override_tool_count_heuristic() {
        ...
        headers.insert("x-ai-req-id", "title-request".parse().unwrap());
        assert!(
            !overrides
        ...
        headers.insert("x-ai-turn-idx", "1".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
        ...
        headers.insert("x-ai-req-id", "".parse().unwrap());
        headers.insert("x-ai-turn-idx", "".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
    ...
    async fn cancelling_replay_waits_for_primary_before_satisfaction() {
        ...
        let replay = tokio::spawn({
            ...
            async move {
                reqwest::Client::new()
                    .post(url)
                    .header("x-ai-req-id", "cancel-replay")
                    .header("x-ai-turn-idx", "1")
                    .json(&body)
                    .send()
                    .await
