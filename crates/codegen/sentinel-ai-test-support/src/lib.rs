...
//! Shared test utilities for ai-build crates: mock inference server, SSE
...
//! - [`AiStdioClient`] — ACP client that drives `ai agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `ai agent --leader stdio` (unix)
...
//! - [`run_headless`] — Run `ai -p` against the mock server and capture output
//! - [`git_workdir`] — Create a git-initialized [`TestSandbox`]
//! - [`ai_binary`] — Resolve the ai binary path (AI_BINARY env or cargo_bin)
...
/// Multiply a harness timeout by `AI_TEST_TIMEOUT_SCALE` (positive integer,
/// default 1). CI lanes on shared runner pools raise it so pool load slows
/// tests instead of failing them (see the Ai Build merge CI workflow).
pub fn scaled(base: std::time::Duration) -> std::time::Duration {
    let scale = std::env::var("AI_TEST_TIMEOUT_SCALE")
        .ok()
