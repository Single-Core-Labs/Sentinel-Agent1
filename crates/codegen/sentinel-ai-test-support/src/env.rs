...
fn local_ai_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("sentinel-ai-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_ai_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = cmd
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build sentinel-ai-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build sentinel-ai-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "sentinel-ai-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve ai binary: `AI_BINARY` env (CI) or a locally built `sentinel-ai-pager` binary.
pub fn ai_binary() -> PathBuf {
    if let Ok(path) = std::env::var("AI_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "AI_BINARY does not exist: {}", p.display());
        // Bazel's AI_BINARY is runfiles-relative; the harness spawns the child
        // with a different cwd, so absolutize against the (runfiles-root) cwd now.
        return std::path::absolute(&p).unwrap_or(p);
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_sentinel-ai-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_ai_binary_path();
    ensure_local_ai_binary(&binary);
    binary
}
