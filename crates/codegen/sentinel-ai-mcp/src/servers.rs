...
use sentinel_ai_tools::types::{
    output::{MCPOutput, MCPOutputDetails, ToolOutput},
    tool::{ToolKind, ToolNamespace},
    tool_metadata::ToolMetadata,
};
use sentinel_ai_tools::util::{ProcessGroup, ProcessScope};

/// MCP tool name delimiter: server names are qualified as `"server__tool"`.
/// Canonical definition lives in `sentinel_ai_workspace_types`; re-exported here
/// for callers that historically imported it from this module.
pub use sentinel_ai_workspace_types::MCP_TOOL_NAME_DELIMITER;

/// Reqwest 0.13 adapter over `sentinel_ai_extra_ca::extra_root_ders` (DER is version-neutral).
fn with_extra_root_certificates(mut builder: reqwest::ClientBuilder) -> reqwest::ClientBuilder {
    for der in sentinel_ai_extra_ca::extra_root_ders() {
        match reqwest::Certificate::from_der(der) {
            Ok(cert) => builder = builder.add_root_certificate(cert),
            Err(e) => tracing::warn!(
                error = %e,
                "AI_EXTRA_CA_BUNDLE: validated DER rejected by reqwest 0.13; skipping cert"
            ),
        }
    }
    builder
...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"ai_com_linear"`).
...
pub struct McpState {
    ...
    /// Persisted to `~/.ai/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    event_writer: sentinel_file_utils::events::EventWriter,
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: sentinel_file_utils::events::EventWriter::noop(),
    ...
    /// `AiClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: sentinel_file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &sentinel_file_utils::events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `sentinel-ai-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use sentinel_ai_telemetry::enums::McpInitStrategy;

/// Parse a non-empty `server__tool` ID with one overlap-aware delimiter and
/// valid [`sentinel_tool_protocol::ToolId`] syntax.
pub fn parse_mcp_qualified_name(name: &str) -> Option<(sentinel_tool_protocol::ToolId, &str, &str)> {
    ...
    Some((sentinel_tool_protocol::ToolId::new(name).ok()?, server, tool))
}
...
impl McpError {
    ...
    pub fn error_category(&self) -> sentinel_file_utils::events::McpErrorCategory {
        use sentinel_file_utils::events::McpErrorCategory;
        match self {
...
/// `sentinel_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl sentinel_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> sentinel_tool_protocol::ToolId {
        ...
        sentinel_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| sentinel_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        sentinel_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(sentinel_file_utils::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(sentinel_file_utils::events::Event::McpAuthRetry {
                    server_name: server.clone(),
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(sentinel_file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(sentinel_file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &sentinel_file_utils::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, sentinel_tool_runtime::ToolError> {
        ...
        match result {
            ...
            Ok(Err(e)) => Err(sentinel_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
                *is_timeout = true;
                ...
                Err(sentinel_tool_runtime::ToolError::custom(
                    "process_manager",
                    format!(
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(sentinel_file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(sentinel_file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
            ...
            Err(e) => {
                ...
                return Err(sentinel_tool_runtime::ToolError::custom(
                    "process_manager",
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(sentinel_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(sentinel_file_utils::events::Event::McpTransportDecodeError {
...
/// Uses [`AiClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, AiClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`AiClientHandler`] when the server pushes a notification we
...
pub struct McpClient {
    ...
    /// [`AiClientHandler`]** constructed by
...
/// and the [`AiClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to sentinel-ai-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: sentinel_ai_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, AiClientHandler>, McpError> {
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("ai-shell-{server_name}"),
                sentinel_ai_version::VERSION.to_string(),
            ),
        )
    ...
    /// Build the [`AiClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> AiClientHandler {
        AiClientHandler {
    ...
    /// references (the [`AiClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            sentinel_ai_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = sentinel_ai_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`ai_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "ai_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("ai-cli"),
    );
}
...
pub struct McpSpawnCtx<'a> {
    ...
    pub(crate) event_writer: &'a sentinel_file_utils::events::EventWriter,
...
impl<'a> McpSpawnCtx<'a> {
    pub fn for_session(
        session_id: &'a str,
        event_writer: &'a sentinel_file_utils::events::EventWriter,
        mode: OauthInteractivity,
    ...
    pub fn session_less(event_writer: &'a sentinel_file_utils::events::EventWriter) -> Self {
        Self {
            session_id: None,
            event_writer,
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = sentinel_ai_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = sentinel_ai_telemetry::instrumentation::timer("mcp_stdio_spawn");
            ...
            .map_err(|e| {
                ...
                sentinel_ai_telemetry::session_ctx::log_event(
                    sentinel_ai_telemetry::events::McpServerFailed {
                        server_name: name.clone(),
                        error_type: sentinel_ai_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    sentinel_ai_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        ctx.event_writer.emit(
                            sentinel_file_utils::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = sentinel_ai_telemetry::instrumentation::timer("mcp_start_servers");
...
impl ClientHandler for AiClientHandler {
    ...
    // See the [`AiClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            sentinel_file_utils::events::EventWriter::noop(),
        );
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("ai_com_figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("AI_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("ai_com_FIGMA", "https://example.com/mcp"));
        ...
        assert!(!is_figma_mcp("ai_com_linear", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_ai_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    fn test_mcp_erased_tool_id_is_qualified() {
        use sentinel_tool_runtime::Tool;
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use sentinel_computer_hub_sdk::LocalRegistry;
        use sentinel_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = sentinel_file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = sentinel_file_utils::events::EventWriter::open(tmp.path());
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        ...
        assert!(is_auth_rejection_message(
            "MCP server 'ai_com_notion' handshake failed: Auth required, when send initialize request"
        ));
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            sentinel_file_utils::events::McpErrorCategory::AuthRequired
        ));
    }
