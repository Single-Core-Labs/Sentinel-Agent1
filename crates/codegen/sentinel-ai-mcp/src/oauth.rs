...
/// ai.com login's 10-minute callback budget. Without a bound, an abandoned
...
// mode disabled, multiple `ai` invocations).
//
// Layer 1 (cross-process): filesystem lock at $AI_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    sentinel_ai_config::ai_home().join(format!("mcp_auth_{safe}.lock"))
}
...
mod tests {
    ...
    async fn manager_ready_for_exchange(token_endpoint: String) -> (AuthorizationManager, String) {
        ...
        mgr.configure_client(
            OAuthClientConfig::new("ai-test-client", "http://127.0.0.1:0/callback")
