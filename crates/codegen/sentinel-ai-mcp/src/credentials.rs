...
//!
//! Credentials are stored in `$AI_HOME/mcp_credentials.json`, keyed by a
//! composite key derived from the server name and URL. This keeps MCP OAuth
//! tokens isolated from the user's AI auth (`auth.json`).
//!
...
///
/// Local helper (not shell-base): `sentinel-ai-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$AI_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$AI_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$AI_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user ai home (set $AI_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$AI_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(sentinel_ai_config::user_ai_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$AI_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("ai-mcp-credentials-test");
