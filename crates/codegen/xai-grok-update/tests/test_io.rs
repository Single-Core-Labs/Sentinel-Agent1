...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        unsafe { std::env::set_var("GROK_TEST_VERSION", "9.9.9") };
    }
    let v = xai_grok_update::version::get_installed_grok_version();
    assert_eq!(v, "9.9.9");
    unsafe {
        unsafe { std::env::remove_var("GROK_TEST_VERSION") };
    }
}
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    unsafe {
        unsafe { std::env::set_var("GROK_TEST_VERSION", "0.0.0-test") };
    }
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        unsafe { std::env::set_var("GROK_TEST_VERSION", "0.1.200-alpha.5") };
    }
    let v = xai_grok_update::version::get_installed_grok_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        unsafe { std::env::set_var("GROK_TEST_VERSION", "not-a-version") };
    }
    let v = xai_grok_update::version::get_installed_grok_version();
