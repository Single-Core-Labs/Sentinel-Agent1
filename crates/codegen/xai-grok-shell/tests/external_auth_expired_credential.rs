...
fn expired_external_credential_routes_to_the_provider_login_flow() {
    ...
    unsafe {
        ...
        unsafe { std::env::set_var("GROK_CLI_CHAT_PROXY_BASE_URL", server.url()) };
        unsafe { std::env::set_var("GROK_XAI_API_BASE_URL", server.url()) };
        unsafe { std::env::set_var("GROK_MODELS_BASE_URL", server.url()) };
        unsafe { std::env::set_var("GROK_AUTH_PROVIDER_COMMAND", &provider) };
        unsafe { std::env::set_var("GROK_AUTH_PROVIDER_LABEL", PROVIDER_LABEL) };
        // An API key would be advertised first and mask the session-auth path.
        unsafe { std::env::remove_var("XAI_API_KEY") };
        unsafe { std::env::remove_var("GROK_CODE_XAI_API_KEY") };
        ...
        unsafe { std::env::set_var("GROK_TELEMETRY_ENABLED", "false") };
        unsafe { std::env::set_var("GROK_FEEDBACK_ENABLED", "false") };
        unsafe { std::env::set_var("GROK_TRACE_UPLOAD", "false") };
    }
