...
async fn full_session_load_e2e() {
    ...
    unsafe {
        ...
        unsafe { std::env::set_var("GROK_INSTRUMENTATION", "log") };
        unsafe { std::env::set_var("GROK_INSTRUMENTATION_LOG", &instr_log) };
        unsafe { std::env::set_var("GROK_CLI_CHAT_PROXY_BASE_URL", server.url()) };
        unsafe { std::env::set_var("GROK_XAI_API_BASE_URL", server.url()) };
        unsafe { std::env::set_var("XAI_API_KEY", "test-key-for-ci") };
