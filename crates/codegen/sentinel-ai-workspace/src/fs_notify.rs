...
//! FsNotify adapter functions bridging [`sentinel_fsnotify`] events to
...
//! sent to the [`IndexManagerHandle`](sentinel_codebase_graph::IndexManagerHandle).
//! The [`refresh_codebase_graph_after_head_change`] helper handles
//! git HEAD changes by diffing `ORIG_HEAD..HEAD`.

use std::path::{Path, PathBuf};

use sentinel_fsnotify::{FsEvent, FsEventKind};
...
pub(crate) fn fs_event_to_codebase_graph_event(
    paths: &[PathBuf],
    kind: FsEventKind,
) -> sentinel_codebase_graph::FileEvent {
    use sentinel_codebase_graph::{FileEvent, FileEventKind};
...
/// Convert [`sentinel_fsnotify::FsEventKind`] to the wire-type
/// [`sentinel_ai_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> sentinel_ai_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => sentinel_ai_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => sentinel_ai_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => sentinel_ai_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => sentinel_ai_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => sentinel_ai_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](sentinel_ai_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<sentinel_ai_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    sentinel_ai_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
fn parse_diff_name_status_line(
    line: &str,
    repo_root: &Path,
) -> Option<sentinel_codebase_graph::FileEvent> {
    let mut parts = line.splitn(3, '\t');
    let status = parts.next()?.trim();
    let path = parts.next()?;

    match status.chars().next()? {
        'A' => Some(sentinel_codebase_graph::FileEvent::created(repo_root.join(path))),
        'D' => Some(sentinel_codebase_graph::FileEvent::removed(repo_root.join(path))),
        'R' | 'C' => {
            let new_path = parts.next()?;
            Some(sentinel_codebase_graph::FileEvent::renamed(
                repo_root.join(path),
                repo_root.join(new_path),
            ))
        }
        _ => Some(sentinel_codebase_graph::FileEvent::modified(
            repo_root.join(path),
        )),
    }
}
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    idx: &sentinel_codebase_graph::IndexManagerHandle,
    repo_root: &Path,
    events_tx: &tokio::sync::broadcast::Sender<sentinel_ai_workspace_types::WorkspaceEvent>,
) {
    ...
    sentinel_ai_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(sentinel_ai_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            sentinel_ai_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: sentinel_ai_workspace_types::FsEventKind,
) -> sentinel_codebase_graph::FileEvent {
    use sentinel_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        sentinel_ai_workspace_types::FsEventKind::Created => FileEventKind::Created,
        sentinel_ai_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        sentinel_ai_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        sentinel_ai_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.ai/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use sentinel_ai_workspace_types::FsEventKind as WsKind;
        assert_eq!(
    ...
    fn codebase_graph_event_mapping() {
        use sentinel_codebase_graph::FileEventKind;
