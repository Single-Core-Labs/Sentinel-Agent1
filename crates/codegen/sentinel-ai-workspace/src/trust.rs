...
//!
//! Persists per-folder trust decisions to `~/.ai/trusted_folders.toml`.
...
//!
//! The store is rooted at [`sentinel_ai_config::user_ai_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.ai`) when
//! neither `$AI_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.ai/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.ai/`.
...
impl TrustStore {
    /// Load the trust store from `<user_ai_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_ai_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`sentinel_ai_config::user_ai_home`], never
    /// [`sentinel_ai_config::ai_home`], so it never falls back to a cwd-relative
    /// `./.ai` — that fallback would let an untrusted cloned repo's `.ai`
    /// masquerade as the user-global store and self-trust the checkout.
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(sentinel_ai_config::user_ai_home())
    }

    /// Map a resolved user-ai-home to the store path, preserving "no home" as
    ...
    fn default_path_in(user_ai_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_ai_home?.join(TRUST_FILE_NAME))
    }
    ...
    fn record_decision(&mut self, workspace_key: &Path, trusted: bool) -> io::Result<()> {
        ...
        let Some(path) = self.path.as_deref() else {
            tracing::warn!(
                path = %canonical.display(),
                trusted,
                "folder trust: no user ai home resolved; trust decision not recorded"
            );
            return Ok(());
        };
...
///
/// A ai-managed worktree first collapses onto its recorded source repo's git
/// ROOT (via the `~/.ai/worktrees.db` registry), so every `ai -w` worktree
...
/// `ai -w` was launched from (the recorded source repo may be a repo subdir).
...
/// `ai -w` worktree of a repo shares one trust key. The collapse fires ONLY
...
fn git_derived_workspace_key(cwd: &Path) -> PathBuf {
    // A ai-managed worktree (any creation mode, incl. standalone clones git
    ...
    if let Some(source_repo) = crate::worktree::source_repo_for_cwd(&cwd.to_string_lossy()) {
        ...
        // ONE key regardless of the subdir ai -w was launched from (parity with
...
/// sharing `~/.ai/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.ai/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = sentinel_ai_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
/// is testable without the process-global ai-home cache. Returns the number
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match sentinel_ai_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.ai");
        assert_eq!(
        ...
        // cwd-relative `./.ai` that ai_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.ai/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_ai_home() {
        // Thin source-pin: the production accessor reads user_ai_home()
        // (Option, no cwd fallback), not ai_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            sentinel_ai_config::user_ai_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    fn workspace_key_collapses_linked_worktrees_onto_main_checkout() {
        // Every linked `ai -w` worktree of a repo must share ONE trust key:
    ...
    // ── workspace_key registry collapse (ai-managed worktrees) ─────────
    ...
    /// Point `AI_HOME` at an isolated tempdir and register one ai-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `AI_HOME` on
    /// drop (before releasing the lock), so the caller may bind it any way.
    fn register_ai_worktree(
        temp: &tempfile::TempDir,
        name: &str,
        source_repo: &Path,
        creation_mode: &str,
    ) -> (LockedTestEnv, PathBuf) {
        use sentinel_fast_worktree::{WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
    ...
    #[test]
    fn workspace_key_collapses_standalone_ai_worktree_onto_source_repo() {
        ...
        let (_env, wt) = register_ai_worktree(&temp, "wt", &source_repo, "standalone");

        let expected = canonicalize_or_owned(&source_repo);
        assert_eq!(
            workspace_key(&wt),
            expected,
            "a standalone ai worktree must collapse onto its recorded source repo"
        );
    ...
    fn workspace_key_collapses_worktree_onto_source_repo_git_root() {
        ...
        let (_env, wt) = register_ai_worktree(&temp, "wt", &subdir, "standalone");
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<ai_home>/worktrees`: `worktree_record_for_cwd` skips the registry
        ...
        // worktree cwd, and `outside` is its OWN git repo (under ai HOME but not
        ...
        let (_env, _wt) = register_ai_worktree(&temp, "wt", &source_repo, "standalone");

        // Under ai HOME but NOT under `<home>/worktrees`, and its own git repo.
        let outside = root.join("ai-home").join("not-worktrees").join("proj");
        ...
        assert_eq!(
            ...
            "a cwd outside <ai_home>/worktrees keys on its own repo root"
        );
        assert_ne!(
            key,
