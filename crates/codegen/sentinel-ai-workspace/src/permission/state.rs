...
pub struct PermissionState {
    ...
    /// Exact MCP tool names (e.g. `"ai_com_notion__notion-fetch"`)
    /// the user has granted "always allow" for. Lookup is exact.
    pub allowed_mcp_tools: HashSet<String>,
    /// Server components of valid qualified MCP IDs (e.g. `"ai_com_notion"`)
...
fn state_dir_for_cwd(cwd: &AbsPathBuf) -> std::path::PathBuf {
    sentinel_ai_config::sessions_cwd_dir(cwd.as_str())
}
...
async fn try_load_state(path: &std::path::Path) -> Option<PermissionState> {
    try_load_state_with_writer(path, |path, contents| {
        sentinel_ai_config::fs_atomic::write_atomically(path, contents, None)
    })
    .await
}
...
pub async fn cleanup_stale_permission_state(max_age: std::time::Duration) {
    let sessions_dir = ai_home().join("sessions");
...
mod tests {
    ...
    fn roundtrip_with_allowed_mcp_tools() {
        ...
        state
            .allowed_mcp_tools
            .insert("ai_com_notion__notion-fetch".to_string());
        state
            .allowed_mcp_tools
        ...
        assert!(
            restored
                .allowed_mcp_tools
                .contains("ai_com_notion__notion-fetch")
        );
    ...
    fn roundtrip_with_allowed_mcp_servers() {
        ...
        assert!(restored.allowed_mcp_servers.contains("ai_com_slack"));
