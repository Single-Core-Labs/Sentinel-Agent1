...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "ai-shell", alias = "ai_shell")]
    Generic,
    /// Ai TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "ai-tui", alias = "ai_tui")]
    AiTUI,
    /// Ai Web client - identified by clientIdentifier "ai-web"
    #[serde(rename = "ai_web")]
    AiWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "ai-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// Ai Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to AiTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"ai-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"ai_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "ai-pager", alias = "ai_pager")]
    AiPager,
    /// Ai Desktop (Electron) client - identified by clientIdentifier "ai-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "ai_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `ai-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "ai-shell",
            Self::AiTUI => "ai-tui",
            Self::AiWeb => "ai-web",
            Self::Nebula => "nebula",
            Self::Extension => "ai-code-extension",
            Self::AiPager => "ai-pager",
            Self::Desktop => "ai-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"ai-web"`, `"ai-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("ai-web") => Self::AiWeb,
            Some("nebula") => Self::Nebula,
            Some("ai-code-extension") => Self::Extension,
            Some("ai-desktop") => Self::Desktop,
            Some("ai-pager") => Self::AiPager,
            _ => Self::Generic,
        }
    }
...
impl From<&sentinel_ai_tools::types::ToolInput> for AccessKind {
    fn from(input: &sentinel_ai_tools::types::ToolInput) -> Self {
        use sentinel_ai_tools::types::ToolInput;
        match input {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.ai/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use sentinel_ai_tools::implementations::ai_build_hashline::edit::types::HashlineEditInput;
        use sentinel_ai_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use sentinel_ai_tools::implementations::ai_build::bash::BashToolInput;
        use sentinel_ai_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use sentinel_ai_tools::implementations::use_tool::UseToolInput;
        use sentinel_ai_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use sentinel_ai_tools::implementations::ai_build::monitor::types::MonitorInput;
        use sentinel_ai_tools::types::ToolInput;
    ...
    fn search_replace_maps_to_edit_access() {
        use sentinel_ai_tools::implementations::ai_build::search_replace::SearchReplaceInput;
        use sentinel_ai_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use sentinel_ai_tools::implementations::ai_build::web_fetch::WebFetchInput;
        use sentinel_ai_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use sentinel_ai_tools::implementations::ai_build::web_search::WebSearchInput;
        use sentinel_ai_tools::types::ToolInput;
    ...
    fn apply_patch_maps_to_edit_access() {
        use sentinel_ai_tools::implementations::codex::apply_patch::ApplyPatchInput;
        use sentinel_ai_tools::types::ToolInput;
    ...
    fn write_tool_maps_to_edit_access() {
        use sentinel_ai_tools::implementations::opencode::write::WriteInput;
        use sentinel_ai_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_ai_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("ai-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("ai_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
