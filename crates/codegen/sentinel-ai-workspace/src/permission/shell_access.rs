...
impl ProtectedEditReason {
    ...
    pub fn description(self) -> Option<&'static str> {
        match self {
            ...
            Self::AiConfig => Some(
                "Note: This edit contains changes to Ai config, which can alter permissions, tools, and other behavior in later sessions.",
            ),
            Self::AiSandbox => Some(
                "Note: This edit contains changes to the Ai sandbox config, which can loosen filesystem and network restrictions on commands.",
            ),
...
pub(crate) fn edit_target_protection(path: &Path) -> Option<ProtectedEditReason> {
    ...
    let lexical = sentinel_ai_paths::normalize_lexically(path);
...
fn protected_edit_reason(path: &Path) -> Option<ProtectedEditReason> {
    ...
    if protected_ai_hook_root(path, &string_components) {
    ...
    if let Some(reason) = protected_ai_config_file(path, &string_components) {
        return Some(reason);
    }
...
/// Ai config files that alter permissions (`config.toml`, the
...
/// inside any `.ai` dir (user-global default and workspace overlays) and
/// directly under a custom `$AI_HOME`, which the component match cannot see.
fn protected_ai_config_file(path: &Path, components: &[&str]) -> Option<ProtectedEditReason> {
    protected_ai_config_file_with_home(
        path,
        components,
        sentinel_ai_config::user_ai_home().as_deref(),
    )
}

fn protected_ai_config_file_with_home(
    path: &Path,
    components: &[&str],
    user_ai_home: Option<&Path>,
) -> Option<ProtectedEditReason> {
    let reason = match components.last().copied() {
        Some(
            sentinel_ai_config::USER_CONFIG_FILENAME
            | sentinel_ai_config::MANAGED_CONFIG_FILENAME
            | sentinel_ai_config::REQUIREMENTS_FILENAME,
        ) => ProtectedEditReason::AiConfig,
        Some("sandbox.toml") => ProtectedEditReason::AiSandbox,
        _ => return None,
    };
    let in_dot_ai = components.len() >= 2 && components[components.len() - 2] == ".ai";
    let in_ai_home = || ai_home_matches(user_ai_home, |home| path.parent() == Some(home));
    (in_dot_ai || in_ai_home()).then_some(reason)
}

/// True when `pred` holds for the user ai home in either its lexical or
...
fn ai_home_matches(home: Option<&Path>, pred: impl Fn(&Path) -> bool) -> bool {
    home.is_some_and(|home| {
        let lexical = sentinel_ai_paths::normalize_lexically(home);
        pred(&lexical)
            || resolve_following_symlinks(&lexical, 0).is_some_and(|resolved| pred(&resolved))
    })
}

fn path_is_under_user_ai_hook_root(path: &Path, ai_home: &Path) -> bool {
    path.starts_with(ai_home.join("hooks")) || path == ai_home.join("hooks-paths")
}

fn protected_ai_hook_root(path: &Path, components: &[&str]) -> bool {
    components.windows(2).any(|pair| pair == [".ai", "hooks"])
        || components.ends_with(&[".ai", "hooks-paths"])
        || ai_home_matches(sentinel_ai_config::user_ai_home().as_deref(), |home| {
            path_is_under_user_ai_hook_root(path, home)
        })
}
...
mod tests {
    ...
    fn sensitive_edit_targets_and_lexical_aliases_prompt() {
        for path in [
            ...
            "/home/user/.ai/sandbox.toml",
            "/work/project/.ai/sandbox.toml",
        ] {
            assert!(
        ...
        for path in [
            "/work/src/main.rs",
            "/work/project/.ai/config.toml/backup",
    ...
    fn edit_target_protection_classifies_reasons() {
        let cases = [
            ...
            (
                "/home/user/.ai/config.toml",
                ProtectedEditReason::AiConfig,
            ),
            (
                "/home/user/.ai/sandbox.toml",
                ProtectedEditReason::AiSandbox,
            ),
            (
                "/work/project/.ai/sandbox.toml",
                ProtectedEditReason::AiSandbox,
            ...
            (
                "/home/user/.ai/managed_config.toml",
                ProtectedEditReason::AiConfig,
            ),
            (
                "/home/user/.ai/requirements.toml",
                ProtectedEditReason::AiConfig,
            ),
            (
    ...
    fn sensitive_edit_targets_include_hook_roots() {
        for path in [
            "/home/user/.ai/hooks/evil.json",
            "/home/user/.ai/hooks/nested/deep.json",
            "/home/user/.ai/hooks-paths",
            "/home/user/.claude/settings.json",
            "/home/user/.claude/settings.local.json",
            "/home/user/.cursor/hooks.json",
            "/work/project/.ai/hooks/local.json",
            "/work/project/.ai/hooks-paths",
        ] {
            assert!(
                edit_target_protection(Path::new(path)).is_some(),
        ...
        for path in [
            "/home/user/.ai/hooks-disabled/note.json",
            "/home/user/.ai/hooks-evil/note.json",
    ...
    #[test]
    fn path_is_under_user_ai_hook_root_matches_relocated_home() {
        let home = Path::new("/custom/ai-home");
        for path in [
            "/custom/ai-home/hooks/x.json",
            "/custom/ai-home/hooks/nested/deep.json",
            "/custom/ai-home/hooks",
            "/custom/ai-home/hooks-paths",
        ] {
            assert!(
                path_is_under_user_ai_hook_root(Path::new(path), home),
                "must match under custom ai home: {path}"
            );
        }
        for path in [
            "/custom/ai-home/hooks-disabled/note.json",
            "/custom/ai-home/hooks-evil/note.json",
            "/custom/ai-home/config.toml",
            "/custom/other/hooks/x.json",
            "/custom/ai-home-extra/hooks/x.json",
        ] {
            assert!(
                !path_is_under_user_ai_hook_root(Path::new(path), home),
    ...
    fn sensitive_edit_targets_follow_symlinks() {
        ...
        .unwrap();
        let ai_hook = outside.path().join(".ai/hooks/evil.json");
        std::fs::create_dir_all(ai_hook.parent().unwrap()).unwrap();
        std::fs::write(&ai_hook, b"{}").unwrap();
        symlink(&ai_hook, ws.path().join("ai-hook-link")).unwrap();

        for path in [
            ws.path().join("file-link"),
            ws.path().join("hooks-link/new-hook"),
            ws.path().join("module-hooks-link/new-hook"),
            ws.path().join("ai-hook-link"),
        ] {
            assert!(
    ...
    /// A custom `$AI_HOME` has no `.ai` path component, so the live
    /// `config.toml` / `sandbox.toml` must be caught by the home-prefix branch.
    #[test]
    fn ai_config_files_under_custom_ai_home_are_protected() {
        let home = tempfile::tempdir().unwrap();
        let home_path = home.path();
        for (file, reason) in [
            ("config.toml", ProtectedEditReason::AiConfig),
            ("managed_config.toml", ProtectedEditReason::AiConfig),
            ("requirements.toml", ProtectedEditReason::AiConfig),
            ("sandbox.toml", ProtectedEditReason::AiSandbox),
        ] {
            let path = home_path.join(file);
            let components = [file];
            assert_eq!(
                protected_ai_config_file_with_home(&path, &components, Some(home_path)),
                Some(reason),
                "{file} directly under $AI_HOME must be protected"
            );
        }
    ...
    fn ai_config_under_symlinked_ai_home_is_protected() {
        ...
        assert_eq!(
            ...
            Some(ProtectedEditReason::AiSandbox)
        );
    }
    ...
    fn resolved_root_alias_matches_physical_destination() {
        ...
        assert!(resolved_path_is_within_root(
            &resolved_root.join("ai-test"),
            Path::new("/etc")
        ));
        assert!(!resolved_path_is_within_root(
            Path::new("/tmp/ai-test"),
            Path::new("/etc")
        ));
    }
