...
use sentinel_computer_hub_sdk::harness::PERMISSION_REQUEST_KIND;
use sentinel_computer_hub_sdk::{ToolServer, WeakToolServer};
...
static PERMISSION_TIMEOUT_TOTAL: LazyLock<IntCounter> = LazyLock::new(|| {
    register_int_counter!(
        "ai_workspace_permission_timeout_total",
        "permission_request hooks whose reply timed out (backstop deadline fired)"
    )
    .expect("ai_workspace_permission_timeout_total must register once")
});
...
pub const HITL_PERMISSION_LIVE_ENV: &str = "AI_HITL_PERMISSION_LIVE";
/// Whether the HITL-live permission path is enabled.
///
/// Intended long-term gate: the chat flag `ai_chat_enable_hitl_live_path`,
...
pub fn access_kind_for_hub_tool(tool_name: &str, args: &Value) -> Option<AccessKind> {
    ...
    let name = name.strip_prefix("AiBuild:").unwrap_or(name);
    match name {
