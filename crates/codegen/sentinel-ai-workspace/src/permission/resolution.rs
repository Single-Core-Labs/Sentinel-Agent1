//! Permission resolution engine: merges native `.ai/config.toml`,
...
/// (fail-safe) with a warn + skip record for `ai inspect`.
...
/// user `~/.ai` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    sentinel_ai_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Load `[permission]` rules from native Ai TOML config files:
///
///   * `~/.ai/config.toml` (lowest priority)
///   * Each `.ai/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.ai/config.toml` first (lowest priority within this layer).
    // Gated on user_ai_home() so a project's .ai/config.toml is never read as
    // global permissions when neither AI_HOME nor a home dir resolves.
    if let Some(global_path) = sentinel_ai_config::user_ai_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match sentinel_ai_config::load_config_file(&global_path) {
    ...
    // `.ai/config.toml` (same gate as project `.claude/settings.json`).
    if project_trusted {
        for path in crate::project_config::find_project_configs(cwd) {
            match sentinel_ai_config::load_config_file(&path) {
                Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[sentinel_ai_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native Ai and Claude sources.
...
/// `.ai/config.toml` permission rules (mirrors [`load_claude_env_with_project`]).
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `ai inspect`.
...
struct ResolveInputs<'a> {
    ...
    /// `.claude/settings.json` / `.ai/config.toml` permission rules are dropped
    ...
    fn live(project_trusted: bool) -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &sentinel_ai_config::managed_config_layers(),
            ),
            project_trusted,
        }
    }
...
/// bypass is pinned off via ai `requirements.toml`
...
///
/// `project_trusted` gates project-tier Claude settings and `.ai/config.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `ai inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in claude_settings_paths_for_trust(cwd, project_trusted) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `ai inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `ai inspect` via the outer resolver.
...
/// [`sentinel_ai_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = sentinel_ai_config::claude_managed_settings_path() else {
...
/// not consulted: ai must not inherit a host-wide always-approve lockdown from
/// that file. ai still honors that file's permission rules / MCP / marketplace
/// allowlists, and the user's own `--yolo` / `[ui] permission_mode` / runtime
/// toggle drive always-approve; to disable it in ai use a root-owned
/// `requirements.toml`. Fails open on user-writable layers.
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = sentinel_ai_config::requirements_layers();
...
/// without `~/.ai`); `path` only names the layer in a non-bool warning.
...
/// Namespace prefix for legacy injected MCP server names (`ai_com_*`).
/// Policy matching still uses this spelling.
pub const MANAGED_MCP_PREFIX: &str = "ai_com_";

/// Max `char` length of a managed runtime name (`ai_com_` + normalized display
...
///
/// Both sides reduce to one key (strip `ai_com_`, [`normalize_managed_name`],
...
mod tests {
    ...
    // Shared so `AI_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // AI_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("AI_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_AI_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate AI_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate AI_HOME so the claude-import marker reads clean (an
    ...
    fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
        ...
        assert!(mcp_name_matches("foo", "ai_com_foo"));
        assert!(mcp_name_matches("ai_com_foo", "foo"));
        assert!(mcp_name_matches("ai_com_foo", "ai_com_foo"));
        assert!(!mcp_name_matches("foo", "foobar"));
        assert!(!mcp_name_matches("foo", "ai_com_foobar"));
    ...
    fn mcp_name_matches_is_case_and_space_insensitive() {
        ...
        assert!(mcp_name_matches("Slack", "ai_com_slack"));
        assert!(mcp_name_matches("My Server", "ai_com_my_server"));
        assert!(mcp_name_matches("ai_com_my_server", "My Server"));
        ...
        assert!(!mcp_name_matches("ai_com_", "ai_com_anything"));
    }
    ...
    fn denied_by_server_name_matches_bare_and_managed_prefix() {
        ...
        for unrelated in ["foobar", "ai_com_foobar", "barfoo", "bar"] {
    ...
    fn allowed_by_server_name_restricts_across_transports() {
        ...
        assert!(al.is_server_allowed(&http_named("ai_com_foo", "https://evil.example.com/x")));
        assert!(al.is_server_allowed(&stdio_named("ai_com_foo", "/usr/bin/whatever")));
    ...
    fn server_name_deny_beats_allow() {
        ...
        for s in [
            ...
            http_named("ai_com_foo", "https://foo.example.com/x"),
        ] {
    ...
    fn server_name_prefix_edge_cases_vice_versa() {
        ...
        let al = allowlist_from(serde_json::json!({
            "deniedMcpServers": [ { "serverName": "ai_com_foo" } ]
        }));

        assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
        assert!(al.is_server_denied(&http_named("ai_com_foo", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("foobar", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("ai_com_foobar", "https://x.example.com/mcp")));
    }
    ...
    fn untrusted_project_claude_permissions_are_not_honored() {
        ...
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("HOME", home.path());
        let _ai_guard = EnvVarGuard::set("AI_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_AI_CLAUDE_MARKER_OVERRIDE");
    ...
    /// Untrusted clone must not contribute project `.ai/config.toml` [permission].
    ///
    /// Sync + `block_on` so `ENV_LOCK` is not held across `.await` (clippy
    /// `await_holding_lock`). Does not assert exact global rule counts:
    /// `sentinel_ai_config::ai_home()` is a process-wide `OnceLock`, so under
    /// single-process `cargo test` an earlier test may have already pinned
    /// `AI_HOME`. Project-rule filtering is independent of that; global
    /// survival is checked only when our temp home is the live `user_ai_home()`.
    #[test]
    fn untrusted_project_config_toml_permissions_are_not_honored() {
        ...
        // Global allow (survives untrusted project when AI_HOME resolves here).
        std::fs::write(
...
"#,
        ...
        let rt = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .expect("test runtime");
        // Untrusted may be None when no global rules load (AI_HOME OnceLock
        ...
        let global_live = sentinel_ai_config::user_ai_home()
            .is_some_and(|g| g == home.path() || g.starts_with(home.path()));
        if global_live {
            let untrusted = untrusted.expect("global rules present when AI_HOME is live");
            assert!(
                untrusted
                    .config
                    .rules
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/ai/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/ai/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.ai/requirements.toml` is not, despite its path.
        ...
        assert!(is_admin_source(&RequirementSource::SystemRequirements {
            path: "/etc/ai/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.ai/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/ai/managed_config.toml".into(),
        }));
    ...
    fn drop_untrusted_catchall_allows_is_source_aware() {
        ...
        let rules = vec![
            ...
            // Managed config: defaults tier, untrusted even from /etc/ai.
            sourced(
                allow_any(Some("*")),
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for ai inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            sentinel_ai_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            sentinel_ai_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
