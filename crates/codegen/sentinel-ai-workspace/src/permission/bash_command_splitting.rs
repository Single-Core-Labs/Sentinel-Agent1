...
///
/// ```bash
/// AI_API_KEY='sentinel-some-key' cargo run --bin sentinel-ai-pager
/// ```
///
...
///   prefix: ["AI_API_KEY=sentinel-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "sentinel-ai-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "AI_API_KEY='sentinel-some-key' cargo run --bin sentinel-ai-pager";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["AI_API_KEY=sentinel-some-key".to_owned()],
                highlighted_words: vec![
                    "cargo".to_owned(),
                    "run".to_owned(),
                    "--bin".to_owned(),
                    "sentinel-ai-pager".to_owned()
                ],
                suffix: vec![],
            })
        );
        ...
        let redirection_command = "cargo build --bin sentinel-ai-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package sentinel-ai-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
