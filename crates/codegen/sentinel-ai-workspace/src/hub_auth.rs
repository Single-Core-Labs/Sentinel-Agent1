//! Hub [`AuthProvider`] from `~/.ai/auth.json` for the standalone
...
//!
//! The in-leader `ai workspace` exposure does NOT use this path — it sources
...
pub fn default_auth_path() -> anyhow::Result<PathBuf> {
    let ai = sentinel_ai_config::user_ai_home()
        .ok_or_else(|| anyhow::anyhow!("no user ai home (set $AI_HOME or $HOME)"))?;
    Ok(ai.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `ai login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `ai login` first.",
                path.display()
            )
        })
}
...
/// ai-shell `auth.json` writer takes. Polling `try_lock` rather than a
...
/// the default credential path (`~/.ai/auth.json`).
pub fn provider(
    hub_url: &Url,
...
mod tests {
    ...
    fn read_auth_entry_picks_oidc_entry() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "legacy": { "key": "sentinel-plainkey", "user_id": "u1" },
            "oidc": {
    ...
    fn read_auth_entry_rejects_non_oidc() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "api_key": { "key": "sentinel-plainkey", "user_id": "u1" }
        }"#,
        );
    ...
    fn build_oidc_provider_succeeds_with_all_fields() {
        ...
        match cred {
            sentinel_computer_hub_sdk::AuthCredential::Bearer { token } => {
    ...
    fn write_refreshed_token_updates_jwt_entry() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "legacy": { "key": "sentinel-old", "user_id": "u1" },
        ...
        assert_eq!(updated["legacy"]["key"], "sentinel-old"); // untouched
    }

    /// With several OIDC entries (personal + enterprise login), the one with
    /// the latest `expires_at` wins — that's the entry the user's ai
