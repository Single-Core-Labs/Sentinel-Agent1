...
const ENVRC_TIMEOUT_ENV: &str = "AI_ENVRC_TIMEOUT_SECS"; // seconds; 0 disables
...
const OUTPUT_SENTINEL: &str = "__AI_ENVRC_COMPLETE__";
...
fn run_with_deadline(mut cmd: Command, deadline: Instant, label: &str) -> RunOutcome {
    ...
    let mut group = sentinel_ai_tools::util::ProcessGroup::new().ok();
    ...
    if let Some(g) = &group
        && !sentinel_ai_tools::util::global_process_scope().register(g)
    {
...
fn reap_with_timeout(child: &mut std::process::Child, label: &str) {
    if let Ok(None) = wait_timeout::ChildExt::wait_timeout(child, sentinel_tty_utils::KILL_REAP_TIMEOUT)
    {
        tracing::warn!(
            label,
            pid = child.id(),
...
mod tests {
    ...
    fn sentinel_named_variable_does_not_truncate_env() {
        ...
        fs::write(
            dir.path().join(".envrc"),
            "export __AI_ENVRC_COMPLETE__=decoy\nexport ZZ_AFTER_DECOY=survives\n",
        )
        .unwrap();

        let env = load_envrc_with_timeout(dir.path(), Duration::from_secs(10)).unwrap();
        assert_eq!(
            env.get("__AI_ENVRC_COMPLETE__"),
