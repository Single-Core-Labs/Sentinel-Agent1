...
//! spawns the unchanged `/usr/local/bin/sentinel-ai-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/sentinel-ai-preview-proxy";
...
/// `ai_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "ai_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    ...
    /// `sentinel-ai-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`AI_SERVER_KEY` /
/// `AI_SESSION_ID`) reach the proxy by env inheritance — never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT sentinel_tty_utils::detach_command: the proxy must stay in
...
/// `sentinel-ai-preview-proxy`'s `/__control/activity` route).
...
/// the default in `sentinel-ai-preview-proxy/src/cli.rs`).
...
pub async fn supervise_preview_metrics(control_port: Option<u16>, shutdown: watch::Receiver<bool>) {
    scrape_metrics_loop(
        ...
        shutdown,
        |body| {
            if let Some(sink) = sentinel_computer_hub_sdk::metric_donate::active_metrics_sink() {
...
mod tests {
    ...
    fn sample_cfg() -> PreviewArgs {
        PreviewArgs {
            ...
            auth_redirect: Some("https://ai.com/preview-auth".to_owned()),
            allow_public: true,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match sentinel-ai-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "AI_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
