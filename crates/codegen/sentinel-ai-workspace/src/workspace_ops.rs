...
use sentinel_computer_hub_sdk::ToolHarness;
use sentinel_ai_tools::types::output::ToolRunResult;
use sentinel_ai_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use sentinel_ai_workspace_types::rpc::WorkspaceRpc;
pub use sentinel_ai_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use sentinel_ai_workspace_types::rpc::code_nav::{
...
pub use sentinel_ai_workspace_types::rpc::export_github::ExportGithubReq;
pub use sentinel_ai_workspace_types::rpc::fs::{
...
pub use sentinel_ai_workspace_types::rpc::git::{
...
pub use sentinel_ai_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use sentinel_ai_workspace_types::rpc::hunks::{
...
pub use sentinel_ai_workspace_types::rpc::repos::{
    ProvisionedRepo, RepoManifest, ReposListReq, ReposListResponse,
};
pub use sentinel_ai_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use sentinel_ai_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use sentinel_ai_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use sentinel_ai_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use sentinel_ai_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &sentinel_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: sentinel_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use sentinel_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &sentinel_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: sentinel_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use sentinel_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: sentinel_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: sentinel_hunk_tracker::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &sentinel_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: sentinel_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: sentinel_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> sentinel_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => sentinel_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => sentinel_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<sentinel_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
/// Ancestor hop budget when locating `.ai/repos.json`.
///
...
/// bounds are the sandbox root (`/workspace`) and the user-global ai home.
...
///
/// Does not escape the sandbox workspace or load `~/.ai/repos.json` /
/// `$AI_HOME/repos.json` (user-global, not a provisioned workspace).
fn repos_manifest_search_dirs(start: &std::path::Path) -> Vec<std::path::PathBuf> {
    let rel = sentinel_ai_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH;
    #[allow(deprecated)]
    let home = std::env::home_dir();
    let mut global_manifests = Vec::with_capacity(2);
    if let Some(v) = std::env::var_os("AI_HOME")
        && !v.is_empty()
    {
        global_manifests.push(std::path::PathBuf::from(v).join("repos.json"));
    }
    if let Some(user_home) = sentinel_ai_config::user_ai_home() {
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = sentinel_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                sentinel_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &sentinel_ai_hooks::discovery::HookRegistry,
...
fn wire_to_hook_registry(
...
) -> WorkspaceResult<sentinel_ai_hooks::discovery::HookRegistry> {
    ...
    let mut registry: sentinel_ai_hooks::discovery::HookRegistry =
        serde_json::from_value(value).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<sentinel_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<sentinel_codebase_graph::QueryResult, sentinel_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<sentinel_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](sentinel_ai_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: sentinel_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<sentinel_ai_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](sentinel_ai_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(sentinel_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = sentinel_tool_protocol::ToolId::new(name).map_err(|e| {
                    sentinel_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                        format!("invalid tool name: {e}"),
                    )
                })?;
                let mut ctx = sentinel_tool_runtime::ToolCallContext::default();
                ctx.call_id =
                    sentinel_tool_protocol::ToolCallId::new(call_id.to_owned()).unwrap_or(ctx.call_id);
...
mod tests {
    ...
    /// request types are defined in `sentinel-ai-workspace-types` (and re-exported
    ...
    fn git_op_cwd_uses_explicit_git_root_per_window() {
        ...
        let window_a = std::path::PathBuf::from("/repos/sentinel-main");
        let window_b = std::path::PathBuf::from("/repos/sentinel-main-2");
        assert_eq!(
    ...
    async fn repos_list_reads_real_manifest_empty_one_and_many() {
        ...
        assert_eq!(
            empty.version,
            sentinel_ai_workspace_types::rpc::repos::REPOS_MANIFEST_VERSION
        );
        ...
        std::fs::write(
            tmp.path()
                .join(sentinel_ai_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH),
    ...
    async fn repos_list_walks_up_from_rewritten_cwd() {
        ...
        std::fs::create_dir_all(sandbox_ws.join(".ai")).unwrap();
        std::fs::write(
            sandbox_ws.join(sentinel_ai_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH),
            one.to_json_bytes().unwrap(),
        )
        .unwrap();
    ...
    fn repos_manifest_search_dirs_stops_at_sandbox_workspace_root() {
        ...
        let _unset_ai = crate::TestEnvGuard::unset("AI_HOME");
    ...
    #[test]
    fn repos_manifest_search_dirs_skips_user_global_ai_home() {
        let _lock = crate::ENV_TEST_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        ...
        assert!(
            ...
            "must not probe $HOME/.ai/repos.json: {dirs:?}"
        );
    }
    ...
    fn repos_list_does_not_load_user_global_manifest() {
        ...
        std::fs::create_dir_all(home.path().join(".ai")).unwrap();
        std::fs::write(
            home.path().join(".ai").join("repos.json"),
        ...
        assert!(
            ...
            "missing workspace manifest must not fall back to ~/.ai/repos.json: {:?}",
            listed.repos
        );
    }
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset = std::sync::Arc::new(
            sentinel_ai_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
    ...
    fn hunk_to_wire_serializes_identically() {
        use sentinel_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use sentinel_hunk_tracker::FileContentEntry;
        use sentinel_hunk_tracker::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use std::sync::Arc;
        use sentinel_hunk_tracker::SessionSummary;
        use sentinel_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = sentinel_ai_hooks::config::HookSpec {
            ...
            event: sentinel_ai_hooks::event::HookEventName::PreToolUse,
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use sentinel_ai_hooks::event::HookEventName as E;
    ...
    /// can't depend on `sentinel-ai-hooks`, and `hook_registry_to_wire` only couples
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use sentinel_ai_hooks::config::HookSpec;
        ...
        let spec = HookSpec {
            ...
            source_dir: std::path::PathBuf::from("/home/u/.ai/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
            layer: sentinel_ai_hooks::config::HookProvenance::Managed,
        };
        assert_eq!(
