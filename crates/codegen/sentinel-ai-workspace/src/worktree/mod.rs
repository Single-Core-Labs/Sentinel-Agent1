...
//!
//! Moved from `sentinel-ai-shell/src/session/worktree.rs` into the workspace
...
use sentinel_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
// Canonical in sentinel-ai-workspace-types; re-exported for existing paths.
...
const WORKTREE_LOG: &str = "sentinel_worktree";

/// Map a [`WorktreeType`] to the fast-worktree crate's `CreationMode`.
pub(crate) fn to_creation_mode(t: WorktreeType) -> sentinel_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => sentinel_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => sentinel_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => sentinel_fast_worktree::CreationMode::GitCheckout,
    }
}
...
pub use sentinel_fast_worktree::META_KEY_LABEL;
...
/// Resolve the ai home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`sentinel_fast_worktree::resolve_ai_home`), so checkout dirs and
/// the metadata DB always live under the same `.ai` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `sentinel_ai_config::ai_home()`,
/// so worktree paths also agree with trust/hooks and other ai-home paths.
fn ai_home() -> std::path::PathBuf {
    sentinel_fast_worktree::resolve_ai_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.ai/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    ai_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.ai/worktrees/<repo_name>`)
/// for a given source path, correctly handling ai-managed worktrees.
///
/// When `source_path` is already under `~/.ai/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the ai worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = ai_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.ai/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the ai-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.ai/worktrees/`.
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to sentinel_fast_worktree::WorkingTreeMode
    ...
    // Use sentinel-fast-worktree for high-performance worktree creation
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(sentinel_fast_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        sentinel_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        sentinel_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        sentinel_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        sentinel_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        sentinel_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
///
/// When the source path is already inside `~/.ai/worktrees/<repo>/`, the
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        sentinel_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use sentinel-fast-worktree -- it handles copying from any worktree path
    ...
    let builder_result = {
        ...
        tokio::task::spawn_blocking(move || {
            let mut builder = WorktreeBuilder::new(&source_worktree_path, &dest_path)
                ...
                .ignored_files_mode(IgnoredFilesMode::Skip)
                .creation_mode(to_creation_mode(creation_mode))
                .worktree_kind(sentinel_fast_worktree::WorktreeKind::Fork)
                .session_id(session_id_for_builder)
                .metadata(label_metadata);
...
///
/// ACP equivalent of `ai -w -r <session_id>` (optionally with `--ref`).
...
use sentinel_fast_worktree::{
    ...
    rebuild_worktree_db, resolve_ai_home, resolve_worktree_auto_gc_from_layers,
};
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &sentinel_ai_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use std::collections::BTreeMap;
    use sentinel_ai_config_types::WorktreeKindMaxAge;
...
/// Env + `$AI_HOME/config.toml` only — **`remote=None` is intentional**.
///
...
/// processes; use `AI_WORKTREE_AUTO_GC=0` / `AI_WORKTREE_AUTO_GC_DRY_RUN=1`
/// or local TOML until remote is plumbed into `make_workspace_handle`.
fn resolve_worktree_auto_gc_local() -> sentinel_fast_worktree::ResolvedWorktreeAutoGc {
    use sentinel_ai_config_types::WorktreeAutoGcSettings;

    let local = if let Ok(home) = resolve_ai_home() {
...
pub fn worktree_db_rebuild() -> Result<sentinel_fast_worktree::RebuildReport> {
    let home = resolve_ai_home()?;
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = sentinel_fast_worktree::ListFilter {
...
/// Scan `~/.ai/worktrees/<repo_name>/` for subdirectories not tracked
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[sentinel_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/ai/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/ai/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/ai/subagents/standalone-1";
    ...
    /// Point `AI_HOME` at an isolated tempdir (`resolve_ai_home` re-reads
    ...
    /// and restores `AI_HOME` on drop (before releasing the lock), so the
    ...
    fn worktree_db_fixture(
        ...
        let env = LockedTestEnv::lock().set("AI_HOME", &home);
        ...
        let record = WorktreeRecord {
            ...
            status: sentinel_fast_worktree::WorktreeStatus::Alive,
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (AiHomeFixture is not visible across crates).
