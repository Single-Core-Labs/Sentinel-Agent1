...
use sentinel_computer_hub_sdk::ToolServerHandler;
use sentinel_ai_tools::computer::types::TaskKind;
use sentinel_ai_tools::implementations::ai_build::scheduler::interval::interval_to_human;
use sentinel_ai_tools::implementations::ai_build::scheduler::types::{
    SchedulerCommand, SchedulerHandle,
};
use sentinel_ai_tools::registry::types::FinalizedToolset;
use sentinel_ai_tools::types::resources::Terminal;
use sentinel_ai_workspace_types::rpc::workspace::{
    BackgroundTaskSnapshotWire, ScheduledTaskSnapshotWire, TasksSnapshotResponse,
};
use sentinel_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
...
static WORKSPACE_RPC_CALLER_MISMATCH_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ai_workspace_rpc_caller_mismatch_total",
...
static WORKSPACE_RPC_MUTATION_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ai_workspace_rpc_mutation_total",
...
static WORKSPACE_RPC_REQUESTS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "ai_workspace_rpc_requests_total",
...
static WORKSPACE_RPC_DURATION_SECONDS: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "ai_workspace_rpc_duration_seconds",
...
) -> Vec<sentinel_ai_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use sentinel_ai_tools::computer::types::TaskKind;
    use sentinel_ai_tools::types::resources::Terminal;
    use sentinel_ai_tools::types::tool::ToolKind;
    use sentinel_ai_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &sentinel_ai_tools::registry::types::FinalizedToolset,
) -> Vec<sentinel_ai_workspace_types::rpc::workspace::TodoSummaryWire> {
    use sentinel_ai_tools::implementations::ai_build::todo::{TodoState, TodoStatus};
    use sentinel_ai_tools::types::resources::State;
    use sentinel_ai_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use sentinel_ai_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use sentinel_ai_workspace_types::rpc::search::FuzzyStatusReq;
        use sentinel_ai_workspace_types::rpc::skills::DiscoverPluginsReq;
        use sentinel_ai_workspace_types::rpc::workspace::{
        ...
        use sentinel_ai_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<sentinel_tool_runtime::SessionContext>();
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use sentinel_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    ) -> Option<sentinel_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use sentinel_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use sentinel_ai_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
        ...
        let read_only = ToolServerConfig {
            tools: vec![tc(
                "AiBuild:read_file",
                Some(sentinel_ai_tools::types::tool::ToolKind::Read),
            )],
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(sentinel_tool_runtime::SessionContext("main".to_owned()));
    ...
    async fn handle_call_records_rpc_metrics_and_collapses_unknown_method() {
        ...
        let has_bogus_series = prometheus::gather()
            .iter()
            .filter(|mf| mf.name() == "ai_workspace_rpc_requests_total")
