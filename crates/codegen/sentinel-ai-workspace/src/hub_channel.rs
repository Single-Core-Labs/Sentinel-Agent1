...
pub use sentinel_ai_workspace_client::consume_stream_terminal;

/// Extract a `WorkspaceEvent` from a custom `ToolNotificationFrame`.
pub fn extract_workspace_event(
    frame: &sentinel_tool_protocol::ToolNotificationFrame,
) -> Option<WorkspaceEvent> {
    use sentinel_tool_protocol::WireToolNotification;
...
mod tests {
    use super::*;
    use sentinel_tool_protocol::{ToolId, ToolNotificationFrame, WireToolNotification};
    ...
    // consume_stream_terminal tests live in sentinel-ai-workspace-client.
}
