...
//! access token from `~/.ai/auth.json` does (same identity).
...
use url::Url;
use uuid::Uuid;
use sentinel_computer_hub_sdk::pool::HubConnectionPool;
use sentinel_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &sentinel_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<sentinel_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "AiBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
