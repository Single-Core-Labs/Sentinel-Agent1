...
//!
//! Reads OIDC credentials from `~/.ai/auth.json`, connects to a
...
use url::Url;
use sentinel_ai_workspace::config::WorkspaceServerMetadata;
use sentinel_ai_workspace::daemonize;
use sentinel_ai_workspace::diag_server::{self, DiagHandle, ErrorClass};
use sentinel_ai_workspace::error::WorkspaceError;
use sentinel_ai_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_ai_workspace";
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<sentinel_tool_protocol::ServerId>()
        .err()
...
#[derive(Parser)]
#[command(name = "sentinel-workspace-server")]
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.ai.com/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `AI_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "AI_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.ai/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "AI_WORKSPACE_PROJECT_LSP_TRUSTED",
    ...
    /// Override with `AI_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "AI_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/sentinel-ai-preview-proxy` child (see `cli.rs` for the proxy's
...
fn main() -> anyhow::Result<()> {
    ...
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(sentinel_tty_utils::runtime::capped_worker_threads().get())
        .enable_all()
        .build()?
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = sentinel_computer_hub_sdk::DonatingLogLayer::new_inert();
    tracing_subscriber::registry()
        .with(env_filter)
        .with(tracing_subscriber::fmt::layer())
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("AI_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match sentinel_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use sentinel_ai_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("AI_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(value = %val,
                        "Unrecognized AI_SANDBOX_PROFILE, defaulting to workspace");
                    ProfileName::Workspace
                } else {
                    parsed
                }
            }
            None if sentinel_ai_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
            None => ProfileName::Workspace,
        };
        let profile_name = profile.to_string();
        if profile == ProfileName::Off {
            tracing::info!(profile = %profile_name, "Sandbox explicitly disabled via AI_SANDBOX_PROFILE=off");
        } else {
            ...
            sandbox.install();
            let active = sentinel_ai_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = sentinel_ai_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let auth_provider = sentinel_ai_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(
        hub_url = %url,
        cwd = %cwd.display(),
    ...
    let session_id = std::env::var("AI_SESSION_ID").ok();
    ...
    sentinel_ai_workspace::init_metrics();
    let ws_handle = match sentinel_ai_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = sentinel_ai_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            sentinel_ai_workspace::handle::DrainReason::Sigterm,
        )
        .await;
    ...
    sentinel_computer_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn classify_from_client_error_display_round_trip() {
        let handshake = WorkspaceError::HubError(
            sentinel_computer_hub_sdk::ClientError::HandshakeAuthFailed { status: 401 }.to_string(),
        );
        ...
        let auth = WorkspaceError::HubError(
            sentinel_computer_hub_sdk::ClientError::AuthError("token rejected".into()).to_string(),
        );
        assert_eq!(
        ...
        let network = WorkspaceError::HubError(
            sentinel_computer_hub_sdk::ClientError::NetworkError("connection refused".into())
                .to_string(),
        );
        assert_eq!(
    ...
    fn capabilities_flag_parses_and_defaults_off() {
        let args = Args::try_parse_from(["sentinel-workspace-server"]).unwrap();
        assert!(!args.capabilities);
        let args = Args::try_parse_from(["sentinel-workspace-server", "--capabilities"]).unwrap();
        assert!(args.capabilities);
    }
    #[test]
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("AI_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["sentinel-workspace-server"]).unwrap();
        assert!(!args.project_lsp_trusted);
        let args = Args::try_parse_from(["sentinel-workspace-server", "--project-lsp-trusted", "true"])
            .unwrap();
        assert!(args.project_lsp_trusted);
    }
    #[test]
    ...
    fn capabilities_probe_of_legacy_binary_exits_nonzero() {
        ...
        let err = LegacyArgs::try_parse_from(["sentinel-workspace-server", "--capabilities"])
    ...
    fn ready_file_is_accepted_as_a_deprecated_no_op() {
        let args =
            Args::try_parse_from(["sentinel-workspace-server", "--ready-file", "/tmp/x.ready"]).unwrap();
    ...
    fn argv_rejection_exit_code_is_distinct_from_server_id_exit_code() {
        let err = Args::try_parse_from(["sentinel-workspace-server", "--flag-from-the-future"])
            .err()
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        ...
        assert_eq!(
            ...
            Some("https://ai.com/preview-auth")
        );
