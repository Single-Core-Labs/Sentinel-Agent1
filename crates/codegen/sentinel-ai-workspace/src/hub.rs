...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        session_handler_resolver: Option<sentinel_computer_hub_sdk::SessionHandlerResolver>,
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(sentinel_ai_version::VERSION)
...
struct CallCompletedGuard {
    ...
    outcome: sentinel_file_utils::events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: sentinel_file_utils::events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: sentinel_file_utils::events::ToolOutcome) {
...
impl ToolServerHandler for SessionRoutedToolHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let hub_session = ctx
            .extensions
            .get::<sentinel_tool_runtime::SessionContext>()
            .map(|s| s.0.clone());
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        tool = %self.name(),
                        session = %session_id,
                        "AI_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
            ...
            while let Some(item) = inner.next().await {
                match item {
                    ...
                    ToolStreamItem::Terminal(Ok(run_result)) => {
                        ...
                        _guard.set_outcome(sentinel_file_utils::events::ToolOutcome::Success);
                    ...
                    ToolStreamItem::Terminal(Err(e)) => {
                        ...
                        _guard.set_outcome(sentinel_file_utils::events::ToolOutcome::Error);
...
fn parse_server_id(id: &str) -> Result<sentinel_tool_protocol::ServerId, ClientError> {
    sentinel_tool_protocol::ServerId::new(id)
...
mod tests {
    ...
    use futures::StreamExt;
    use sentinel_tool_runtime::{SessionContext, ToolCallId};
    ...
    async fn handle_call_terminal_matches_non_streaming_call() {
        ...
        match reference {
            Ok(run_result) => {
                ...
                assert_eq!(
                    typed.model_output,
                    vec![sentinel_tool_runtime::ContentBlock::Text {
    ...
    use sentinel_ai_tools::types::tool_metadata::ToolMetadata as AiToolMetadata;
    #[derive(Debug)]
    struct GateStreamingStub;
    impl AiToolMetadata for GateStreamingStub {
    ...
    impl sentinel_tool_runtime::Tool for GateStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> sentinel_tool_protocol::ToolId {
            sentinel_tool_protocol::ToolId::new("gate_streaming_stub").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::sentinel_tool_runtime::ListToolsContext,
        ) -> sentinel_tool_types::ToolDescription {
            sentinel_tool_types::ToolDescription::new("gate_streaming_stub", "gate streaming stub")
        }
        async fn run(
            &self,
        ...
        async fn execute(
            ...
            use sentinel_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    async fn drain_counts<T>(mut stream: sentinel_tool_runtime::ToolStream<T>) -> (usize, usize, bool) {
        let mut progress = 0;
        let mut terminal = 0;
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("AiBuild:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    async fn wait_until(
        ...
        pred: impl Fn(&sentinel_tool_protocol::ToolServerStatusPayload) -> bool,
        timeout: Duration,
    ) -> sentinel_tool_protocol::ToolServerStatusPayload {
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use sentinel_ai_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use sentinel_ai_tools::computer::types::{TaskKind, TaskSnapshot};
