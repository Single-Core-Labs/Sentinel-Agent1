...
pub trait AuthCredentialProvider: HttpAuth + Send + Sync + 'static {
    ...
    /// (`ai-desktop`, `ai login`). The `token` field MUST mirror
    ...
    /// Whether `X-AI-Token-Auth` should be sent with the bearer token.
    /// `false` for deployment keys (bare Bearer), `true` for user/OAuth tokens.
    /// See `AiAuthCredentials::apply()` for the wire format contract.
