//! `pty-bench` — PTY benchmark CLI for `sentinel-ai-pager`.
//!
...
//! ```bash
//! cargo bench -p sentinel-ai-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/ai-${VERSION}-linux-x86_64 \
//!   cargo bench -p sentinel-ai-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for sentinel-ai-pager",
    long_about = None,
)]
struct Cli {
