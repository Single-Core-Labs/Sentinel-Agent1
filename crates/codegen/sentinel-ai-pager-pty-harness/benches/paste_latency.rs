//! `paste-latency` — clipboard paste latency benchmark for `sentinel-ai-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p sentinel-ai-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p sentinel-ai-pager-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/ai-old --mode text --json /tmp/paste-old.json
//! ```
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for sentinel-ai-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
