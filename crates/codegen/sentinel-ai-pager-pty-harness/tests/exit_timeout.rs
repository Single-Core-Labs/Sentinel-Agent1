...
//! `AI_TEST_HOLD_TEARDOWN_SECS` supplies the hang; a real `SessionEnd`
//! hook cannot hold teardown past `SESSION_FLUSH_GRACE`.
//!
//! ```bash
//! cargo test -p sentinel-ai-pager-pty-harness --test exit_timeout -- --ignored
//! ```

use std::time::Duration;

use anyhow::{Context, Result};
use sentinel_ai_pager_pty_harness::{ContentController, PtyExitPoll, PtyHarness, keys, pager_binary};
...
async fn spawn_pager_with_teardown_hold() -> Result<(PtyHarness, tempfile::TempDir)> {
    ...
    let mut pager = PtyHarness::spawn_with_content_env_in_dir(
        ...
        &[],
        &[
            ("AI_EXIT_TIMEOUT_SECS", TIMEOUT_SECS),
            ("AI_TEST_HOLD_TEARDOWN_SECS", HOLD_SECS),
        ],
        Some(project.path()),
    )?;
