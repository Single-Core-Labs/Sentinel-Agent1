...
//!
//! ```bash
//! cargo test -p sentinel-ai-pager-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
/// The per-CWD history file lives at `<home>/.ai/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".ai").join("sessions");
