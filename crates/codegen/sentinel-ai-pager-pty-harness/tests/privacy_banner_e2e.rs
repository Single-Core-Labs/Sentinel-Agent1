...
//! as the active auth (`AI_API_KEY` removed) and the rollout forced on via
//! `AI_PRIVACY_NOTICE_ROLLOUT=1`.
//!
//! ```bash
//! cargo test -p sentinel-ai-pager-pty-harness --test privacy_banner_e2e \
//!   -- --ignored --nocapture
//! ```
...
/// sandbox's fake `AI_API_KEY` removed so the seeded opted-out OAuth entry
...
/// Poll `<home>/.ai/config.toml` for the async `privacy_banner_acked`
...
fn wait_for_ack_on_disk(pager: &mut PtyHarness, home: &Path, timeout: Duration) -> Result<()> {
    let path = home.join(".ai").join("config.toml");
