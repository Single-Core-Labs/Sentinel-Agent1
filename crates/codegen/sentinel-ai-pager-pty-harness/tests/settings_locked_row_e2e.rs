...
//! `AiAuth` → auth meta → `AppView::coding_data_sharing_lock()` →
...
//! `AI_PRIVACY_NOTICE_ROLLOUT=1` — asserted on the authenticated welcome
//! screen before opening settings. Row/input details are unit-tested in
//! `sentinel-ai-pager` (`views/settings_modal/tests.rs`, `locked_coding_*`);
//! this suite covers the auth-to-render pipeline.
//!
//! ```bash
//! cargo test -p sentinel-ai-pager-pty-harness --test settings_locked_row_e2e \
//!   -- --ignored --nocapture
//! ```
...
use sentinel_ai_pager_pty_harness::{
...
const ROWS: u16 = 50;
const COLS: u16 = 120;
const BANNER_TITLE: &str = "Help improve Ai";
...
const DESCRIPTION_PREFIX: &str = "Opt-in to provide SpaceAI";
...
/// the sandbox's fake `AI_API_KEY` removed so the seeded team OAuth entry is
...
fn locked_row_env_ops() -> [EnvOp<'static>; 3] {
    [
        EnvOp::set("AI_PRIVACY_NOTICE_ROLLOUT", "1"),
        EnvOp::set("AI_ZDR_ACCESS_ENABLED", "1"),
        EnvOp::remove("AI_API_KEY"),
    ]
}
