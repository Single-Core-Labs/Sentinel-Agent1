//! `scroll-matrix` — scroll validation matrix sweep for sentinel-ai-pager.
//!
...
use sentinel_ai_pager_pty_harness::pager_binary;
use sentinel_ai_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against sentinel-ai-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_sentinel-ai-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves AI_SCROLL_LOG against ITS cwd (the
