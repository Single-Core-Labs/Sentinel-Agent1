//! Unified PTY harness for sentinel-ai-pager.
//!
...
//!    `sentinel-ai-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `AI_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// (XTVERSION probes and ai wrap). Content-backed launches must use
    /// [`Self::new_in_sandbox`].
    ...
    /// Spawn from a canonical [`sentinel_ai_test_support::TestSandbox`] baseline
    ...
    pub fn new_in_sandbox(
        ...
        cols: u16,
        args: &[&str],
        sandbox: &sentinel_ai_test_support::TestSandbox,
        env: &[(&str, &str)],
        cwd: Option<&Path>,
    ) -> Result<Self> {
    ...
    /// # use sentinel_ai_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through ai's stdout. Used to reproduce the doubled-line class of bugs
    /// where ai's diff renderer never re-asserts a region it didn't write
