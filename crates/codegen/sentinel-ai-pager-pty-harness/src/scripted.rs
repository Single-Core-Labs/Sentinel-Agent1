//! Scripted TUI scenario runner for sentinel-ai-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$AI_HOME`
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&ai_home)
                .with_context(|| format!("create scenario AI_HOME {}", ai_home.display()))?;
            fs::write(ai_home.join("config.toml"), config_toml)
...
mod tests {
    ...
    fn workspace_rejects_paths_outside_the_tempdir() {
        ...
        let sandbox = sentinel_ai_test_support::TestSandbox::new();
