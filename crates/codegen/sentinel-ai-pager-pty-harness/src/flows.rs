...
/// Seed a fake AI OAuth entry into the isolated home's `auth.json` so the
/// shell has session auth (the harness's `AI_API_KEY` is ApiKey/BYOK mode
...
/// the shell's `AiAuth::is_zdr_team` trigger) — locks the settings modal's
...
/// names must match the shell's `AiAuth` serde names in
/// `sentinel-ai-shell/src/auth/model.rs`).
