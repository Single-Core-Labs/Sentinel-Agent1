...
//! [`MockInferenceServer`] from `sentinel-ai-test-support` and provides the
...
use sentinel_ai_test_support::{MockInferenceServer, TestSandbox};

pub use sentinel_ai_test_support::mock_server::LogEntry;
pub use sentinel_ai_test_support::mock_server::MockModelEntry as MockModel;
pub use sentinel_ai_test_support::mock_server::StorageUpload;
pub use sentinel_ai_test_support::sse;
pub use sentinel_ai_test_support::{
...
impl ContentController {
    ...
    pub async fn start_with_models(models: Vec<MockModel>) -> Result<Self> {
        ...
        sandbox.set_env("AI_PROMPT_SUGGESTIONS", "false");
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.ai
...
mod tests {
    ...
    async fn read_foreground(url: String, endpoint: InferenceEndpoint) -> String {
        ...
        reqwest::Client::new()
            ...
            .header("x-ai-turn-idx", "1")
            .header("x-ai-req-id", format!("direct-{endpoint:?}"))
            .json(&body)
            .send()
            .await
    ...
    /// on the SuperAi upsell screen and breaks every PTY test.
    #[tokio::test]
