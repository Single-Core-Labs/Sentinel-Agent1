...
//! `off`. Custom profiles via `~/.ai/sandbox.toml` or `.ai/sandbox.toml`.
...
/// Load sandbox config from `~/.ai/sandbox.toml` and `.ai/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.ai/sandbox.toml
    let global_path = ai_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.ai/sandbox.toml (additive only)
    let project_path = workspace.join(".ai").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&ai_home().join("sandbox.toml")).unwrap_or_default();
    let project = load_config_file(&workspace.join(".ai").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.ai/ that may not exist on first run.
        ...
        if !profile.write_deny.is_empty() {
            ...
            {
                let files =
                    sentinel_ai_config::validated_hook_json_files_for_sources(&profile.write_deny)
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.ai/sandbox.toml or .ai/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
...
mod tests {
    ...
    /// Hosts with a retargetable `$AI_HOME/hooks` symlink (fail-closed under
...
"#;
    ...
    fn enxio_device_file_is_skipped_but_directory_is_not() {
        ...
        let dir = std::env::temp_dir().join(format!("ai-sbx-test-{}", std::process::id()));
