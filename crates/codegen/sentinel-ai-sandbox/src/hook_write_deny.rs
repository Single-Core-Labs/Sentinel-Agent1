//! Ai-owned hook write-deny: plan, identity revalidation, and post-reexec checks.
...
#[cfg(target_os = "linux")]
use sentinel_ai_config::{ensure_ai_hook_slots, unique_ancestors_rootward};
...
impl From<sentinel_ai_config::GlobalHookSourceError> for HookWriteDenyError {
    fn from(e: sentinel_ai_config::GlobalHookSourceError) -> Self {
...
pub fn resolve_hook_write_deny_snapshot() -> Result<Vec<GlobalHookSource>, HookWriteDenyError> {
    let ai = ai_home();
    let resolved =
        resolve_global_hook_sources(Some(ai.as_path()), /* reject_symlinks */ true)?;
...
pub fn prepare_hook_write_deny(
    ...
    let ai = ai_home();
    ensure_ai_hook_slots(ai.as_path())?;
...
fn capture_dir_json_snapshot(dir: &Path) -> Result<DirJsonSnapshot, HookWriteDenyError> {
    use sentinel_ai_config::{list_direct_hook_json_files, validate_direct_hook_json_file};
