...
//! OS-level sandboxing for Ai Build via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use sentinel_ai_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
const BWRAP_ENV_VAR: &str = "__AI_INSIDE_BWRAP";
...
impl SandboxManager {
    ...
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        ...
        if requires_hook_write_deny(&self.profile, workspace) {
            sentinel_ai_config::ensure_ai_hook_slots(paths::ai_home().as_path())
...
/// Zero-permission placeholder (file or dir) under `ai_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent ai
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::ai_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    fn bwrap_hook_plan_binds_ancestors_then_leaves() {
        ...
        let sources = [sentinel_ai_config::GlobalHookSource {
            path: leaf.clone(),
            kind: sentinel_ai_config::GlobalHookSourceKind::ConfiguredSource,
        }];
    ...
    /// Create a temp workspace whose `.ai/sandbox.toml` contains `toml_body`.
    ...
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let ws = std::env::temp_dir().join(format!("ai-{tag}-{}-{nanos}", std::process::id()));
        let ai = ws.join(".ai");
        std::fs::create_dir_all(&ai).unwrap();
        std::fs::write(ai.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("ai-deny-dir-{}", std::process::id()));
