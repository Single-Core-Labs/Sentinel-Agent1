//! Integration tests for sentinel-ai-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use sentinel_ai_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use sentinel_ai_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!sentinel_ai_sandbox::should_restrict_child_network());
}
