//! E2E path-deny and Ai hook write-deny (subprocess; arm64-tagged).
...
const AI_HOME_ENV: &str = "SANDBOX_E2E_AI_HOME";
...
fn apply_fixture_env(cmd: &mut Command, home: &Path, ai_home: &Path, workspace: &Path) {
    cmd.env(WORKSPACE_ENV, workspace.as_os_str())
        .env(HOME_ENV, home.as_os_str())
        .env(AI_HOME_ENV, ai_home.as_os_str())
        .env("HOME", home.as_os_str())
        .env("AI_HOME", ai_home.as_os_str());
}
...
fn run_scenario(
    ...
    apply_fixture_env(&mut cmd, home, ai_home, workspace);
    let output = cmd
...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = sentinel_ai_sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> sentinel_ai_sandbox::ProfileName {
    sentinel_ai_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    // Isolate HOME/AI_HOME before any config OnceLock init.
    let home = PathBuf::from(std::env::var(HOME_ENV).expect(HOME_ENV));
    let ai_home = PathBuf::from(std::env::var(AI_HOME_ENV).expect(AI_HOME_ENV));
    ...
    match scenario.as_str() {
        ...
        "hook_write_deny_marker_spoof" => subprocess_hook_write_deny_marker_spoof(&ai_home),
        other => {
...
fn subprocess_profile_and_bwrap_reexec(profile: &sentinel_ai_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !sentinel_ai_sandbox::is_inside_bwrap() {
            ...
            match sentinel_ai_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = sentinel_ai_sandbox::SandboxManager::new(profile, workspace);
...
fn subprocess_hook_write_deny_marker_spoof(_ai_home: &Path) {
    #[cfg(not(target_os = "linux"))]
    {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        unsafe {
            unsafe { std::env::set_var("__AI_INSIDE_BWRAP", "1") };
        }
        match sentinel_ai_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
/// Workspace-profile Ai-owned hook write-deny probes (existing sources + first-run).
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    let home = PathBuf::from(std::env::var(AI_HOME_ENV).expect(AI_HOME_ENV));

    let profile = sentinel_ai_sandbox::ProfileName::Workspace;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
    ...
    if first_run {
        ...
        eprintln!("OK: first-run Ai hook slots denied");
    } else {
    ...
    // Parent grants remain creatable: Ai runtime sibling, workspace, temp.
    assert_write_ok(
        "ai runtime sibling",
...
/// Create isolated HOME + AI_HOME fixture dirs for a scenario.
fn fixture_homes(
    tag: &str,
) -> (
    PathBuf,
    ...
    let ai = unique_temp_dir(&format!("{tag}-ai"));
    let workspace = unique_temp_dir(&format!("{tag}-ws"));
    // Empty global sandbox config under fixture AI_HOME so generic tests do
    // not inherit the developer/runner's ~/.ai/sandbox.toml.
    fs::write(ai.join("sandbox.toml"), "").expect("empty global sandbox.toml");
    (
        home.clone(),
        ai.clone(),
        workspace.clone(),
...
fn run_deny_case(
    ...
    let (home, ai, tmp, _ch, _cg, _cw) = fixture_homes(tag);
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".ai")).expect("mkdir .ai");
    fs::write(
        tmp.join(".ai").join("sandbox.toml"),
        format!("[profiles.{profile}]\nextends = \"workspace\"\ndeny = [{deny_list}]\n"),
    )
    .expect("write sandbox.toml");

    // Ensure Ai fixed slots exist so workspace-based custom profiles can
    // resolve hook write-deny without depending on the real user tree.
    fs::create_dir_all(ai.join("hooks")).expect("mkdir fixture hooks");
    fs::write(ai.join("hooks-paths"), b"").expect("write fixture hooks-paths");
    ...
    let (status, stderr) = run_scenario(&home, &ai, &tmp, profile, targets, controls, postlaunch);
    assert!(
        status.success(),
...
fn hardlinked_hooks_paths_refuses_startup() {
    ...
    let (home, ai, workspace, _ch, _cg, _cw) = fixture_homes("hook-hl");
    fs::create_dir_all(ai.join("hooks")).unwrap();
    let reg = ai.join("hooks-paths");
    let alias = ai.join("hooks-paths-alias");
...
/// Workspace profile: Ai-owned direct hook sources are write-denied but readable;
/// create / overwrite / unlink / rename / mkdir fail; absolute hooks-paths
/// targets are denied; parent rename is blocked; Ai/CWD/temp siblings stay writable.
#[test]
fn workspace_protects_direct_hook_sources() {
    ...
    for needle in [
        ...
        "OK: ai runtime sibling writable",
...
fn hardlinked_hooks_json_refuses_startup() {
    ...
    let (home, ai, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-hl");
    fs::create_dir_all(ai.join("hooks")).unwrap();
    fs::write(ai.join("hooks-paths"), b"").unwrap();
    let active = ai.join("hooks").join("active.json");
    let alias = ai.join("hooks").join("active-alias.json");
...
fn symlinked_hooks_json_refuses_startup() {
    ...
    let (home, ai, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-sym");
    fs::create_dir_all(ai.join("hooks")).unwrap();
    fs::write(ai.join("hooks-paths"), b"").unwrap();
    let real = ai.join("real-active.json");
    let active = ai.join("hooks").join("active.json");
...
/// First-run: missing fixed slots are created as real Ai state before apply,
...
fn workspace_protects_direct_hook_sources_first_run() {
    ...
    let (home, ai, workspace, _ch, _cg, _cw) = fixture_homes("hook-fr");
    // Intentionally leave hooks/ and hooks-paths absent (first-run ensure path).

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &ai, &workspace, "hook_write_deny_first_run");
    assert!(
        status.success(),
    ...
    for needle in [
        "OK: first-run Ai hook slots denied",
    ...
    // Post-exit host: Ai slots exist and are valid; no vendor artifacts.
    assert!(
        ai.join("hooks").is_dir(),
        "post-exit: hooks dir must exist as a real directory"
    );
    assert!(
        ai.join("hooks-paths").is_file(),
        "post-exit: hooks-paths must exist as a real file"
    );
    assert_eq!(
        fs::read(ai.join("hooks-paths")).expect("read hooks-paths"),
        b"",
...
fn hook_write_deny_refuses_marker_spoof() {
    ...
    #[cfg(target_os = "linux")]
    {
        let (home, ai, workspace, _ch, _cg, _cw) = fixture_homes("hook-spoof");
        fs::create_dir_all(ai.join("hooks")).unwrap();
        fs::write(ai.join("hooks").join("x.json"), b"{}").unwrap();
        fs::write(ai.join("hooks-paths"), b"").unwrap();
        let (status, stderr) =
            run_hook_write_deny_scenario(&home, &ai, &workspace, "hook_write_deny_marker_spoof");
        assert!(
            status.success(),
