//! Shared announcement types, persistence, and formatting for Ai CLI apps.
//!
//! This crate provides the common logic used by `sentinel-ai-shell` and
//! `sentinel-ai-pager` for handling announcements (banner notifications).
...
/// Read hidden announcement ids from `~/.ai/announcements.json`.
...
/// Write hidden announcement ids to `~/.ai/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    sentinel_ai_tools::util::ai_home::ai_home().join("announcements.json")
}
...
///
/// Precedence: `AI_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("AI_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid AI_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            unsafe { std::env::set_var("AI_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#) };
        }
        ...
        // SAFETY: test-only
        unsafe {
            unsafe { std::env::remove_var("AI_ANNOUNCEMENTS_OVERRIDE") };
        }
    }
    ...
    fn cta_parses_nested_partial_and_absent() {
        let full: RemoteAnnouncement = serde_json::from_str(
            r#"{"id":"p","severity":"promo","cta":{"label":"Get SuperAi","url":"https://x.ai/ai","caption":"or use Ctrl+O"}}"#,
        )
        .unwrap();
        let cta = full.cta.as_ref().expect("cta present");
        assert_eq!(cta.label.as_deref(), Some("Get SuperAi"));
        assert_eq!(cta.url.as_deref(), Some("https://x.ai/ai"));
