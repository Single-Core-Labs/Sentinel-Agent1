...
- **Using Ai**
  ...
  - [SSH Passthrough](#ssh-passthrough-ai-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`ai inspect`)](#introspection)
...
- [Building with Ai](#building-with-ai) — headless API, ACP SDK integration
...
On first launch, Ai opens your browser to authenticate with ai.com:

```bash
ai
```

Credentials are stored in `~/.ai/auth.json` and persist across sessions. Tokens expire after 7 days; Ai will prompt you to re-authenticate when needed.
...
export AI_OIDC_ISSUER="https://acme.okta.com"
export AI_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export AI_CLI_CHAT_PROXY_BASE_URL="https://ai-proxy.acme.com/v1"
```

**3. Run `ai`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.ai/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Ai is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.
...
1. Ai runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by Ai and saved to `~/.ai/auth.json` as the access token
5. exit 0 → success; exit non-zero → Ai falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | Ai (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Ai reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Ai to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Ai when to proactively refresh. Without it, Ai assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Ai captures this)
...
```bash
# Or via environment variables
export AI_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export AI_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export AI_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Ai cannot detect expiry proactively and will only refresh after a 401.
...
> **Windows:** write the path as a TOML *literal* string (single quotes) so backslashes survive: `auth_provider_command = 'C:\corp\ai-auth.exe'`. Inside a double-quoted TOML string `\t`, `\n`, `\r`, `\b` and `\f` are escape sequences, so `"C:\temp\auth.exe"` parses into a path containing a tab character and the provider fails to start — after which Ai falls back to browser login as if the setting were ignored.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with ai.com". In headless mode (`ai -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=ai-cli")
...
Ai runs your binary on two different contracts, and `AI_AUTH_EXPIRED` is how it tells them apart:

| | `AI_AUTH_EXPIRED=1` | unset |
|---|---|---|
| **What it is** | A headless refresh over a credential Ai already holds — near-expiry rotation, or a token the server rejected | A sign-in: `ai login`, the sign-in screen, or the escalation after a headless run couldn't mint |
...
| **Budget** | A few seconds (7s), then Ai kills the process | 300s — enough for a browser round trip or a device code |
| **So your binary should** | Mint silently, or **exit non-zero**. Never block | Do the full SSO flow, and always mint fresh |

```bash
#!/bin/sh
if [ "$AI_AUTH_EXPIRED" = "1" ]; then
    # Headless: silent refresh only. If that can't work — the SSO session
    # lapsed, say — exit non-zero rather than block. Ai then shows the
...
Exiting promptly on `AI_AUTH_EXPIRED=1` is what makes the handover to the sign-in screen fast: a binary that blocks instead pays the whole refresh timeout on every start with an expired token.

One case stays ambiguous, and only in **leader mode** (`--leader`, or `[cli] use_leader = true`; off by default): with no credential at all, the leader makes one extra attempt in the background just after startup, and that run has the variable unset, like a sign-in. A binary that mints without help (service account, keytab, mounted token) succeeds there and the session heals itself. One that must prompt just sits, up to the 300s sign-in ceiling — nothing waits on it, the sign-in screen is already up, and its stderr goes to `~/.ai/leader.log` rather than to the user.

`AI_AUTH_EXPIRED` is optional — if your binary ignores it, Ai still works. It just runs the same flow for both login and refresh, and a flow that prompts will be killed on the headless run before it can finish.

### Automatic Credential Refresh

Ai supports automatic credential refresh for external auth providers and OIDC. When Ai detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Ai handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Ai re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Ai re-runs the binary and retries the request once.
- **When the refresh run can't mint:** refreshes are headless (no stdin, short timeout), so a binary that needs you to complete an SSO flow cannot succeed there. Ai then stops treating the stored credential as usable and runs your binary in its interactive mode instead — at startup that is the same sign-in flow a machine with no credentials gets; mid-session the turn fails with a re-auth prompt and `/login` re-runs the binary.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Ai silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Ai refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export AI_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `ai login` before starting — Ai runs your binary automatically on first launch. You _can_ run `ai login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Ai tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
ai --debug-file /tmp/ai-auth.log -p "hello"
tail -f /tmp/ai-auth.log
```
...
|-------------|---------------|
| `auth: running external auth provider (headless refresh)` | Your binary is being called with `AI_AUTH_EXPIRED=1` and a few seconds to work |
| `auth: running external auth provider (interactive login)` | Your binary is being called on the sign-in contract: no `AI_AUTH_EXPIRED`, stderr shown, 300s |
...
| `auth: external auth provider timed out (likely needs interactive auth), killing` | Binary didn't exit before the 7s headless-refresh timeout and was killed. Exiting non-zero on `AI_AUTH_EXPIRED=1` avoids this wait entirely |
| `auth: failed to start external auth provider` | The command couldn't be spawned (e.g. binary not found) |

### Per-Model Auth Providers

`auth_provider_command` above replaces Ai's *session* auth: it mints the token sent to AI's backend. If you instead want AI models on normal AI login while **other models** route through a gateway (LiteLLM, corporate proxy) whose bearer tokens rotate, use a named auth provider — the rotating-token analogue of a per-model `api_key`/`env_key`.
...
- `AI_AUTH_EXPIRED=1` is set whenever Ai re-mints over a token still cached in memory, whether from near-expiry rotation or a rejection. The first mint on a cold cache runs without it.

**Token lifecycle:**

- Tokens are cached in memory per provider and shared by every model referencing the provider; nothing is written to disk. The command is a credential helper: it owns durable storage and OAuth2 refresh (keychain, its own dotdir, etc.), exactly like `gcloud auth print-access-token` or a git credential helper. On an in-session re-mint the last credential is handed back via `AI_AUTH_PROVIDER_ACCESS_TOKEN` (and, when present, `AI_AUTH_PROVIDER_REFRESH_TOKEN` / `AI_AUTH_PROVIDER_EXPIRES_AT`), so a refresh-grant command can refresh instead of re-authenticating. The command must be non-interactive and fast; do any interactive login out of band, and Ai re-runs the command on restart to re-mint.
- Ai runs the command before a chat turn when the token is missing or within about a minute of expiring, and once more after the server rejects a token. A token rejected within 30 seconds of being fetched is not refetched again, so a broken helper surfaces one clear error instead of looping.
...
**Interaction with other credentials:** a literal `api_key`/`env_key` on the model wins over its `auth_provider`. Provider-backed models are BYOK: your AI session token is never sent to their endpoints, and a failing provider command fails the request rather than falling back to the session token.

**Security:** provider commands execute code, so they are honored only from trusted config layers (`~/.ai/config.toml`, managed config, requirements). A project's `.ai/config.toml` can never define one. Whatever layer sets a model's `base_url` decides where that model's minted token is sent, and `base_url` (unlike the provider table) is not stripped from remote or campaign patches, the same as for a static `env_key`. Keep provider tables and the model `base_url` in layers you trust. The command inherits Ai's environment (so it sees `PATH`, `HOME`, and any other secrets there), but Ai's own first-party credentials (`AI_API_KEY`, `AI_DEPLOYMENT_KEY`, and related keys) are removed so a BYOK helper never receives them; write helpers that read only what they need, and prefer the `AI_AUTH_PROVIDER_*` handback for the prior credential.

### Using auth.json for API Access

If you've authenticated with `ai login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the ai CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.ai.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.ai/auth.json)" \
  -H "X-AI-Token-Auth: sentinel-ai-cli" \
  -H "x-ai-model-override: ai-build" \
  -d '{
    "model": "ai-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.ai/auth.json` (set by `ai login`)                                                                                                                              |
| `X-AI-Token-Auth: sentinel-ai-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-ai-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `ai-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `ai-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `ai login` to refresh.
...
ai --cwd ~/projects/my-app

# Start with an initial task
ai --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
ai --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
ai --always-approve --prompt "Format all files"
```
...
Run Ai non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `ai-build`)               |
...
ai -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
ai -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
ai -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
ai -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
ai -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
ai -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
ai -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
ai -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
ai -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
ai --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
ai -p "What does this project do?"

# Use a specific model
ai -p "Optimize this function" -m ai-build

# Get JSON output for parsing
ai -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
ai -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
ai -p "Remember: the secret number is 42" --output-format json
ai -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
ai -p "Continue where we left off" -c

# Run in a different directory
ai -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
ai -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
ai -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
ai -p "Generate a README" > README.md

# Parse JSON output with jq
ai -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
ai -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | ai -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  ai -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
ai -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run Ai as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `ai-build`)                           |
...
```bash
ai agent headless --ai-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`ai ssh`)

Use `ai ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
ai ssh -t user@host
ai ssh -L 8080:localhost:8080 user@host

# With remote command
ai ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `ai ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Ai can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `ai -p` and parses JSON output.
...
class AiChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["ai", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="ai-build", stream=False):
...
class AiChat {
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("ai", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await AiACPChat(cwd=".").init()
...
class AiACPChat {
  ...
  async init() {
    this.proc = spawn("ai", ["agent", "stdio"]);
...
// Usage
const client = await new AiACPChat(".").init();
...
Ai implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Ai reads configuration from `~/.ai/config.toml`. If the file doesn't exist, Ai uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "ai-4.5"                   # model used for new sessions
web_search = "ai-4.5"                # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: AI_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: AI_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: AI_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: AI_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: AI_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: AI_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: AI_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: AI_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `AI_TELEMETRY_BUILD_EVENTS_URL`, `AI_TELEMETRY_BUILD_EVENTS_API_KEY`, and `AI_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Ai can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Ai language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Ai can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Ai can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Ai looks for server definitions in:

- project config: `<repo>/.ai/lsp.json`
- user config: `~/.ai/lsp.json`
...
- LSP tools are enabled (`AI_LSP_TOOLS=1` or `[features] lsp_tools = true`)
...
If LSP tools are enabled but no usable server config is found, Ai emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Ai does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `AI_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
With this config, `ai` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Ai reads these files and appends their contents to the system prompt.

Ai scans for agent rules in this order:
...
Within each directory, Ai checks for these filenames:
...
Skills are reusable prompt packages that extend Ai with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

Ai discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.ai/skills/`           | Local | Highest  |
| `<repo_root>/.ai/skills/` | Repo  | Medium   |
| `~/.ai/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.ai/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Ai decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Ai invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Ai automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Ai discovers agent definitions from `.ai/agents/` (project), `~/.ai/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `AI_AGENT` env var
4. Default `ai-build` agent
...
```bash
ai --agent-profile ./my-agent.md
# or
export AI_AGENT="my-custom-agent"
```
...
```bash
export AI_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "ai-build"              # route explore to a lighter model
```
...
model = "ai-build"
prompt_file = ".ai/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".ai/personas/concise.md"  # or load from file
```

Both are also discovered from `.ai/roles/*.toml` and `.ai/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Ai with additional tools, skills, and MCP servers from external packages.
...
Ai discovers hooks from `.ai/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → cached `auth_provider` token (terminal: a cache miss resolves to no credential, never the session token) → session token → `AI_API_KEY`. See [Per-Model Auth Providers](#per-model-auth-providers).

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Ai falls back to built-in defaults for known models.
...
[model.ai-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, Ai starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `AI_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `ai models` output). You also need a `[model.*]` entry so Ai knows
...
>
> [model.my-custom-model]              # 2. tell Ai how to reach it
...
Point Ai at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `AI_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `AI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `AI_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export AI_MODELS_BASE_URL="https://api.acme.com/v1"
export AI_API_KEY="sentinel-..."
ai
```

Ai fetches the model list from `{AI_MODELS_BASE_URL}/models` on startup and sends inference requests to `AI_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `AI_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.ai/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Ai uses API key auth (`Authorization: Bearer`) instead of session auth. `ai login` is not required — only the API key.

---

## MCP Servers

Extend Ai's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.ai/config.toml`:
...
MCP servers can also be configured per-project in `.ai/config.toml`. Ai walks from the current directory up to the git repo root, loading `.ai/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.ai/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.ai/config.toml` | This repository   | ↑        |
| `<cwd>/.ai/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.ai/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.ai/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.ai/config.toml`. Other config sections (models, etc.) are only read from `~/.ai/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `AI_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets Ai remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.ai/memory/`:
- **Global** (`~/.ai/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.ai/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.ai/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries

Workspace directories are suffixed with a short hash for uniqueness (e.g. `sentinel-a3f7b2c9/`). The hash is derived from the git remote URL so all clones and worktrees of the same repository share the same memory directory.
...
At the end of each session, Ai saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.ai/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Ai searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.ai/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.ai/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Ai emits the `ai-shell-memory_injection`
...
Ai can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.ai/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.ai/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.ai/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.ai/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.ai/sandbox.toml` (global) or `.ai/sandbox.toml` (per-project):
...
The sandbox is applied to the **entire ai process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Ai logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.ai/sandbox-events.jsonl`
...
Use `ai inspect` to see everything Ai discovers in the current directory:

```bash
ai inspect          # human-readable output
ai inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.ai/skills/`, `~/.ai/skills/`, plugins, and config paths
...
Ai automatically discovers configuration from Claude Code directories alongside native `.ai/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Ai uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.ai/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Ai at runtime.

---

## Built-in Tools

Ai includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `AI_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (SpaceAI, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `x.ai/company`).

---

## Session Persistence

Ai automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.ai/sessions/`, organized by URL-encoded working directory:

```
~/.ai/sessions/<encoded-cwd>/<session-id>/
...
ai -p "Remember: X=42" -s my-session
ai -p "What is X?" -s my-session

# Resume existing session (errors if not found)
ai -p "Continue" -r my-session

# Continue most recent session in current directory
ai -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
ai -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.ai/config.toml` | Configuration file                                  |
| `~/.ai/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.ai/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.ai/memory/`     | Cross-session memory files and index                |
| `~/.ai/skills/`     | User-scoped skill definitions                       |
| `~/.ai/plugins/`    | User-scoped plugins                                 |
| `~/.ai/agents/`     | User-scoped agent definitions                       |
| `.ai/config.toml`   | Project-scoped config (MCP servers)                 |
| `.ai/skills/`       | Project-scoped skill definitions                    |
| `.ai/plugins/`      | Project-scoped plugins                              |
| `.ai/agents/`       | Project-scoped agent definitions                    |
| `.ai/hooks/`        | Project-scoped hooks                                |
| `.ai/lsp.json`      | LSP server configuration                            |
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `AI_API_KEY`         | API key from [console.x.ai](https://console.x.ai). Used for custom endpoint auth and API key login      |
| `AI_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.ai.com/v1`)                          |
| `AI_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `AI_MODELS_LIST_URL`          | Override the model list URL if it differs from `{AI_MODELS_BASE_URL}/models`                                              |
| `AI_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `AI_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `AI_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `AI_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `AI_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `AI_HOME`                     | Override config directory (default: `~/.ai`)                                                           |
| `AI_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `AI_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `AI_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `AI_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `AI_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `AI_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `AI_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `AI_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `AI_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `AI_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.ai/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `AI_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `ai` commands and flags.
...
ai completions bash > ~/.local/share/bash-completion/completions/ai
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Ai-managed location):

```bash
mkdir -p ~/.ai/completions/bash
ai completions bash > ~/.ai/completions/bash/ai.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.ai/completions/bash/ai.bash" ]] && source "$HOME/.ai/completions/bash/ai.bash"
```
...
```bash
mkdir -p ~/.ai/completions/zsh
ai completions zsh > ~/.ai/completions/zsh/_ai
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.ai/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `ai` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `ai --debug` or `AI_LOG_FILE` instead:

```bash
# Per-session debug log (~/.ai/debug/<sessionId>.txt)
ai --debug

# Log to a custom path
AI_LOG_FILE=/tmp/ai-debug.log ai

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.ai/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `AI_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
AI_LOG_FILE=/tmp/ai-debug.log RUST_LOG="info,sentinel_ai_shell::auth=debug" ai
```
...
cat ~/.ai/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.ai/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.ai/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
