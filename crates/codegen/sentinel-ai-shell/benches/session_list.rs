...
//!
//! Run: `cargo bench -p sentinel-ai-shell --bench session_list`
...
use sentinel_fast_worktree::{ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
use sentinel_ai_shell::session::info::Info;
use sentinel_ai_shell::session::persistence::Summary;
use sentinel_ai_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use sentinel_ai_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = sentinel_ai_shell::util::ai_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = sentinel_ai_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    repo.remote(
        "origin",
        "https://github.com/sentinel-org/session-list-benchmark.git",
    )
    ...
    let worktree_base = sentinel_ai_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(sentinel_ai_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn write_summary(
    ...
    let summary = Summary {
        ...
        git_remotes: vec!["git@github.com:sentinel-org/benchmark.git".to_owned()],
...
fn bench_session_list(c: &mut Criterion) {
    ...
    assert_eq!(sentinel_ai_shell::util::ai_home::ai_home(), home.path());
