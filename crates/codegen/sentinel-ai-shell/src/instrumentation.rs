//! Shim — see `sentinel_ai_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `sentinel_ai_shell::instrumentation_timer!`). Keeping the macro here
...
pub use sentinel_ai_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `sentinel_ai_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `sentinel_ai_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `sentinel_ai_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `sentinel_ai_telemetry::instrumentation`.
#[macro_export]
