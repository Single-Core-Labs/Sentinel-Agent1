...
pub mod ai_auth_credentials;
pub mod hooks;
pub mod limits;
pub(crate) mod subprocess;
pub(crate) mod user_identity;

// The foundation utilities live in `sentinel-ai-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `sentinel_ai_shell::util::…` users compile
// unchanged.
pub use sentinel_ai_shell_base::util::*;
...
pub(crate) fn is_user_instruction_path(
    ...
    let ai_rules = ai_home.join("rules");
    let is_exact_home_surface = parent
        .is_some_and(|parent| parent == ai_home || parent == ai_rules)
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn ai_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_ai_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(
            Path::new("/custom/ai/worktrees/repo/src/AGENTS.md"),
            Path::new("/custom/ai"),
            &[],
            Some(Path::new("/custom/ai/worktrees/repo")),
        ));
    }
}
