...
use sentinel_ai_config::resolve_global_hook_sources;
use sentinel_ai_hooks::discovery::HookSource;
use sentinel_ai_hooks::error::HookError;
...
fn include_claude_hooks(compat: &sentinel_ai_tools::types::compat::CompatConfig) -> bool {
    compat.claude.hooks
        && !crate::claude_import::is_claude_import_marked_with_log("discover_hook_source_paths")
}

fn include_cursor_hooks(compat: &sentinel_ai_tools::types::compat::CompatConfig) -> bool {
    compat.cursor.hooks
}
...
pub(crate) fn discover_hook_source_paths(
    ...
    // Soft hooks-paths I/O keeps fixed slots; hard resolve omits Ai globals.
    let mut global: Vec<PathBuf> =
        match resolve_global_hook_sources(ai.as_deref(), /* reject_symlinks */ false) {
            Ok(resolved) => {
                if let Some(e) = &resolved.configured_error {
                    tracing::warn!(
                        error = %e,
                        "hooks-paths unreadable; retaining fixed Ai hook discovery sources only"
                    );
                }
                resolved
                    .discovery_sources()
            ...
            Err(e) => {
                tracing::warn!(
                    error = %e,
                    "global hook source resolve hard-failed; omitting Ai global sources"
                );
                Vec::new()
            }
        };
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".ai").join("hooks"));
        if include_cursor {
...
pub(crate) fn discover_hooks(
    ...
    compat: &sentinel_ai_tools::types::compat::CompatConfig,
    trusted: bool,
) -> (sentinel_ai_hooks::discovery::HookRegistry, Vec<HookError>) {
...
/// dedup in [`sentinel_ai_hooks::discovery::registry_from_specs_deduped`], a config
...
pub(crate) fn assemble_hooks(
    config_layers: &[sentinel_ai_config::HookConfigLayer],
    git_root: Option<&Path>,
    compat: &sentinel_ai_tools::types::compat::CompatConfig,
    trusted: bool,
) -> (sentinel_ai_hooks::discovery::HookRegistry, Vec<HookError>) {
    let (mut specs, mut errors) =
        sentinel_ai_hooks::config::parse_hooks_from_config_layers(config_layers);
    ...
    let (file_specs, file_errors) =
        sentinel_ai_hooks::discovery::collect_specs_from_sources(&global_sources, &project_sources);
    specs.extend(file_specs);
    errors.extend(file_errors);

    (
        sentinel_ai_hooks::discovery::registry_from_specs_deduped(specs),
        errors,
    )
}
