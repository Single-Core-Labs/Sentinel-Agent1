...
use std::sync::Arc;
/// Credentials for authenticating with ai backend services.
///
...
/// User token (AI users) sends `Bearer` + `X-AI-Token-Auth: sentinel-ai-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct AiAuthCredentials {
...
impl std::fmt::Debug for AiAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AiAuthCredentials")
            .field(
                "user_token",
...
impl AiAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your AI_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `ai login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> AiAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> AiAuthCredentials {
    ...
    pub fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        let builder = if let Some(ref key) = self.deployment_key {
            ...
            builder
                ...
                .header(
                    obfstr::obfstr!("X-AI-Token-Auth"),
                    obfstr::obfstr!("sentinel-ai-cli"),
                )
        } else {
            builder
        };
...
impl sentinel_ai_auth::HttpAuth for AiAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        AiAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = AiAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = AiAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = AiAuthCredentials::new(Some("static-token".into()));
