...
use sentinel_ai_config_types::DisplayRefreshSettings;

pub const ENV_DISPLAY_REFRESH_PROBE_ENABLED: &str = "AI_DISPLAY_REFRESH_PROBE_ENABLED";
pub const ENV_DISPLAY_REFRESH_AUTO_CADENCE: &str = "AI_DISPLAY_REFRESH_AUTO_CADENCE";
...
/// Same band as env cadence knobs (`AI_MIN_DRAW_MS` / `AI_SCROLL_CADENCE_MS`).
...
mod tests {
    ...
    fn guard() -> std::sync::MutexGuard<'static, ()> {
        ...
        unsafe {
            unsafe { std::env::remove_var(ENV_DISPLAY_REFRESH_PROBE_ENABLED) };
            unsafe { std::env::remove_var(ENV_DISPLAY_REFRESH_AUTO_CADENCE) };
        }
        g
    }
    ...
    fn env_overrides_probe_and_auto() {
        let _g = guard();
        unsafe {
            unsafe { std::env::set_var(ENV_DISPLAY_REFRESH_PROBE_ENABLED, "0") };
            unsafe { std::env::set_var(ENV_DISPLAY_REFRESH_AUTO_CADENCE, "1") };
        }
    ...
    fn requirement_beats_env() {
        ...
        unsafe {
            unsafe { std::env::set_var(ENV_DISPLAY_REFRESH_PROBE_ENABLED, "0") };
            unsafe { std::env::set_var(ENV_DISPLAY_REFRESH_AUTO_CADENCE, "0") };
        }
