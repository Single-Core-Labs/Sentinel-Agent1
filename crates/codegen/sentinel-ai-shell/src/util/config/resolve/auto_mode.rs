...
pub(crate) const ENV_AUTO_PERMISSION_MODE: &str = "AI_AUTO_PERMISSION_MODE";
...
/// `AI_AUTO_PERMISSION_MODE`. Every test reading the gate (here and in
...
/// two can't drift. Precedence: requirement > env (`AI_AUTO_PERMISSION_MODE`)
...
///
/// requirements > env (`AI_AUTO_PERMISSION_MODE`) > `[auto_mode] enabled` in
...
pub(crate) fn auto_mode_classifier_defaults(
    ...
    sentinel_ai_workspace::permission::ClassifierPromptType,
    Option<sentinel_ai_sampling_types::ReasoningEffort>,
) {
    let prompt_type = cfg
        .prompt_type
        .unwrap_or(sentinel_ai_workspace::permission::ClassifierPromptType::Full);
    let reasoning_effort = cfg.reasoning_effort.or_else(|| {
        effective_supports_reasoning_effort.then_some(sentinel_ai_sampling_types::ReasoningEffort::Low)
    });
...
mod auto_permission_mode_gate_tests {
    ...
    // `AI_AUTO_PERMISSION_MODE` is process-global; serialize every test that
