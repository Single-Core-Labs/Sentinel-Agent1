...
/// [`sentinel_ai_config::env_bool`] parser (`AI_TOOLS_FIND_BFS` /
/// `AI_TOOLS_GREP_UGREP`, plus the `AI_FIND_BFS` / `AI_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.ai/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`sentinel_ai_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub(crate) fn resolve_search_tools_enabled(
    ...
    let disable = sentinel_ai_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = sentinel_ai_config::env_bool(primary).or_else(|| sentinel_ai_config::env_bool(alias));
    ...
    (
        resolve("AI_TOOLS_FIND_BFS", "AI_FIND_BFS", "find_bfs"),
        resolve("AI_TOOLS_GREP_UGREP", "AI_GREP_UGREP", "grep_ugrep"),
    )
}
...
) -> Option<sentinel_ai_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<sentinel_ai_tools::util::ShellEnvironmentPolicy>() {
...
const ENV_LOGIN_SHELL_CAPTURE: &str = "AI_LOGIN_ENV";
...
const ENV_SCHEDULER_BACKGROUND_LOOPS: &str = "AI_SCHEDULER_BACKGROUND_LOOPS";
...
///
/// Precedence: requirements > env (`AI_SCHEDULER_BACKGROUND_LOOPS`) > user
...
/// the shared [`sentinel_ai_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "AI_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`AI_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            sentinel_ai_tools::implementations::ai_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`AI_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        sentinel_ai_tools::implementations::ai_build::ask_user_question::response_timeout_env_secs(),
...
) -> sentinel_ai_tools::implementations::ai_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use sentinel_ai_tools::implementations::ai_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = sentinel_ai_tools::implementations::ai_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes AI_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
...
mod shell_env_policy_tests {
    use super::*;
    use sentinel_ai_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
