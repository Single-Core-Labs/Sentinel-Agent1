...
/// Process-wide write lock for `~/.ai/config.toml`.
///
...
"#;
        ...
        assert_eq!(cfg.ui.auto_light_theme.as_deref(), Some("aiday"));
...
"#;
        ...
        let mut cfg = load_config_from_toml(&root);
        cfg.models.default = Some("ai-4".to_string());
        let mut table = root.as_table().unwrap().clone();
    ...
    fn models_config_serializes_only_some_fields() {
        ...
        if let TomlValue::Table(t) = v {
            ...
            assert_eq!(t.get("default").and_then(|x| x.as_str()), Some("ai-3"));
        } else {
    ...
    fn merge_section_models_only_updates_set_fields_preserves_others() {
        ...
        let cfg = crate::agent::config::ModelsConfig {
            default: Some("ai-new".to_string()),
            ..Default::default()
        };
        merge_section(&mut table, "models", &cfg);
        let m = table.get("models").unwrap().as_table().unwrap();
        assert_eq!(m.get("default").and_then(|v| v.as_str()), Some("ai-new"));
        assert_eq!(
    ...
    fn persist_preferred_model_flow_roundtrips_via_load_and_new_from_toml_cfg() {
        ...
        let root: TomlValue = toml::from_str(original).unwrap();
        let mut cfg = load_config_from_toml(&root);
        cfg.models.default = Some("ai-persisted".to_string());
        ...
        assert_eq!(reloaded.models.default.as_deref(), Some("ai-persisted"));
        let cfg2 = crate::agent::config::Config::new_from_toml_cfg(&reloaded_root)
            .expect("new_from_toml_cfg");
        assert_eq!(cfg2.models.default.as_deref(), Some("ai-persisted"));
    }
    #[test]
    ...
    mod resolve_auto_compact {
        ...
        use std::sync::Mutex;
        const TEST_MODEL: &str = "ai-4.5";
        const OTHER_MODEL: &str = "ai-4.3";
        /// Serialize tests that mutate `AI_AUTO_COMPACT_THRESHOLD_PERCENT`.
    ...
    fn settings_helpers_target_correct_ui_fields() {
        ...
        let cfg = apply(|cfg| cfg.ui.auto_light_theme = Some("aiday".to_string()));
        assert_eq!(cfg.ui.auto_light_theme, Some("aiday".to_string()));
