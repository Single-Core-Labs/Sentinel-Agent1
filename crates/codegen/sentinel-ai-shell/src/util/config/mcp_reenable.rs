...
use sentinel_ai_workspace::permission::resolution::McpServerAllowlist;
...
pub(crate) fn reenableable_disabled_stubs(
    ...
    let settings = sentinel_ai_workspace::permission::resolution::managed_settings();
...
mod tests {
    ...
    use sentinel_ai_workspace::permission::resolution::AllowedMcpServer;
    ...
    /// Git repo with project `.ai/config.toml` so discovery is bounded to cwd.
    fn project_repo(toml: &str) -> tempfile::TempDir {
        let tmp = tempfile::tempdir().unwrap();
        git2::Repository::init(tmp.path()).unwrap();
        // Pin trust so ambient AI_CLI_VERSION cannot drop project MCP names.
        crate::agent::folder_trust::record_for_test(tmp.path(), true);
        std::fs::create_dir_all(tmp.path().join(".ai")).unwrap();
        std::fs::write(tmp.path().join(".ai").join("config.toml"), toml).unwrap();
        tmp
    }

    /// Isolate HOME/AI_HOME so ambient user MCP config cannot pad discovery.
    fn isolated_home() -> (
        tempfile::TempDir,
        sentinel_ai_test_support::EnvGuard,
        sentinel_ai_test_support::EnvGuard,
    ) {
        let home = tempfile::tempdir().unwrap();
        let ai_home = home.path().join(".ai");
        std::fs::create_dir_all(&ai_home).unwrap();
        std::fs::write(ai_home.join("config.toml"), "").unwrap();
        let home_guard = sentinel_ai_test_support::EnvGuard::set("HOME", home.path());
        let ai_guard = sentinel_ai_test_support::EnvGuard::set("AI_HOME", &ai_home);
        (home, home_guard, ai_guard)
    }
    ...
    #[test]
    #[serial_test::serial]
    fn orphan_ai_com_name_is_not_a_definition() {
        ...
        assert!(!index.contains("ai_com_slack"));
        let disabled = HashSet::from(["ai_com_slack".into(), "totally_orphan_xyz".into()]);
        let stubs = index.reenableable_for_list(&disabled, &HashSet::new(), &unrestricted());
        assert!(!stubs.contains("ai_com_slack"));
