...
use sentinel_ai_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use sentinel_ai_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `sentinel-ai-config-types` (config
...
pub use sentinel_ai_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use sentinel_ai_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.ai/config.toml` at each level.
/// Project-scoped `.ai/config.toml` entries override global `~/.ai/config.toml`
...
/// `.ai/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.ai/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.ai/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.ai/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .ai/config.toml"
                );
...
pub(crate) fn mcp_preferences_path() -> PathBuf {
    sentinel_ai_config::ai_home().join("mcp_preferences.json")
}
...
pub(crate) fn collect_mcp_setup_configs(
    cwd: &std::path::Path,
    plugin_registry: Option<&sentinel_ai_agent::plugins::PluginRegistry>,
    compat: &CompatConfig,
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    sentinel_ai_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
/// Upsert an MCP server entry in `~/.ai/config.toml`.
///
...
/// config file, e.g. a project-scoped `.ai/config.toml`.
...
/// Delete an MCP server entry from `~/.ai/config.toml`.
///
...
/// config file, e.g. a project-scoped `.ai/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = sentinel_ai_mcp::credentials::McpCredentialStore::load_default() {
...
/// transport-less case is steered to `disabled_mcp_servers`, Ai's real
/// disable mechanism.
    ...
    let message = if !has_command && !has_url {
        format!(
            "`mcp_servers.{name}` has no transport. To run it, set `command = \"...\"` or \
             ...
             See ~/.ai/docs/user-guide/07-mcp-servers.md"
        )
    } else {
        format!(
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` has an unrecognized field `{field}`; it is \
                             ignored. See ~/.ai/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                }
                if config.enabled
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` is enabled but its `{field}` is blank. \
                             ...
                             off. See ~/.ai/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                    continue;
                }
...
// `.mcp.json` discovery moved to `sentinel-ai-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use sentinel_ai_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.ai/config.toml` files from `cwd` up to the
...
/// [`load_mcp_server_configs_with_project`], for `ai inspect`.
...
/// Names `ai mcp enable`/`disable` may target: user/project TOML (including
...
/// `ai_com_*` is known only when a TOML / disabled / compat / plugin
...
fn load_cli_plugin_registry(cwd: &std::path::Path) -> sentinel_ai_agent::plugins::PluginRegistry {
    ...
    let discovered = sentinel_ai_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    sentinel_ai_agent::plugins::PluginRegistry::from_discovered(
        discovered,
...
fn config_path() -> PathBuf {
    crate::util::ai_home::ai_home().join("config.toml")
}

/// Path to the user-level config file (`~/.ai/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.ai/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".ai").join("config.toml")
}
...
/// Overrides `[cli] session_registry`; usable before `~/.ai/config.toml` exists.
pub const SESSION_REGISTRY_ENV_VAR: &str = "AI_SESSION_REGISTRY";

pub(crate) fn session_registry_from_env_opt() -> Option<bool> {
    sentinel_ai_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = sentinel_ai_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = sentinel_ai_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = sentinel_ai_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.ai/skills", "~/.ai/skills/special/SKILL.md"]
ignore = ["~/.ai/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.ai/skills", "~/.ai/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.ai/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${AI_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
...
"#,
    ...
    fn apply_mcp_server_enabled_managed_name_only_updates_array() {
        ...
        apply_mcp_server_enabled(table, "ai_com_slack", false);
        let disabled = table
            .get("disabled_mcp_servers")
            .and_then(|v| v.as_array())
            .expect("disabled_mcp_servers array");
        assert_eq!(disabled.len(), 1);
        assert_eq!(disabled[0].as_str(), Some("ai_com_slack"));
        assert!(table.get("mcp_servers").is_none());

        apply_mcp_server_enabled(table, "ai_com_slack", true);
        assert!(table.get("disabled_mcp_servers").is_none());
        assert!(table.get("mcp_servers").is_none());
    }
...
"#,
    ...
    async fn enable_unstick_only_touches_nearest_project_definition() {
        ...
        std::fs::create_dir_all(nested.join(".ai")).unwrap();
...
enabled = false
"#;
        let ancestor = tmp.path().join(".ai").join("config.toml");
        let nearer = nested.join(".ai").join("config.toml");
    ...
    async fn restore_mcp_server_enabled_after_enable_scopes_tiers() {
        ...
        // save_mcp_server_enabled_in (that reads ambient config_path / ai_home).
