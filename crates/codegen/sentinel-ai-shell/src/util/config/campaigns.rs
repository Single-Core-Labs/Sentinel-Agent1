...
pub(crate) fn dismiss_campaign_ids(ids: impl IntoIterator<Item = String>) {
    let Some(home) = user_ai_home() else {
        return;
    };
...
fn dismiss_campaign_ids_at(
    ...
    // several ai processes share `$AI_HOME`; the in-process mutex alone would
...
/// `AI_CAMPAIGNS_OVERRIDE` JSON array replaces all sources (`[]` = none; beats
...
pub(crate) fn campaigns_override() -> Option<Vec<CampaignEntry>> {
    let json = std::env::var("AI_CAMPAIGNS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<CampaignOverride>>(&json) {
        ...
        Err(e) => {
            tracing::warn!(error = %e, "invalid AI_CAMPAIGNS_OVERRIDE JSON; suppressing all campaigns");
            Some(Vec::new())
        }
    }
}
...
/// The single campaign-resolution path: `AI_CAMPAIGNS_OVERRIDE` (replaces all
...
/// `AI_CAMPAIGNS_OVERRIDE`. For one-shot CLI entrypoints that never fetch
...
/// without touching `AI_HOME` / the process-global cache).
...
pub async fn persist_models_default(
    value: Option<String>,
    reasoning_effort: Option<sentinel_ai_sampling_types::ReasoningEffort>,
) -> anyhow::Result<()> {
...
mod tests {
    ...
    use sentinel_ai_config::ConfigLayers;
    ...
    /// `AI_CAMPAIGNS_OVERRIDE` applies despite the kill switch; without it the
    ...
    fn override_beats_kill_switch() {
        ...
        let _env = EnvGuard::unset("AI_CAMPAIGNS_OVERRIDE");
        let active = resolve_active_campaigns_from_layers(&layers, &base, &[], &HashSet::new());
        assert!(
            active.is_empty(),
    ...
    /// Invalid `AI_CAMPAIGNS_OVERRIDE` JSON fails toward *no campaigns*: the
    ...
    fn invalid_override_json_suppresses_all_campaigns() {
        let _env = EnvGuard::set("AI_CAMPAIGNS_OVERRIDE", "{ not json");
    ...
    /// made while `AI_CAMPAIGNS=0` must still record the dismissal, or a
    /// later re-enabled campaign would override the user's explicit choice.
    #[test]
    #[serial]
    fn dismiss_resolution_ignores_kill_switch() {
        let _over = EnvGuard::unset("AI_CAMPAIGNS_OVERRIDE");
        let _kill = EnvGuard::set("AI_CAMPAIGNS", "0");
    ...
    fn campaign_driven_models_default_tracks_remote_and_dismissals() {
        let _over = EnvGuard::unset("AI_CAMPAIGNS_OVERRIDE");
        let _kill = EnvGuard::unset("AI_CAMPAIGNS");
    ...
    /// `AI_CAMPAIGNS_OVERRIDE="[]"` replaces all sources with nothing — even
    /// layer + remote campaigns resolve to empty.
    #[test]
    #[serial]
    fn override_empty_means_none() {
        let _env = EnvGuard::set("AI_CAMPAIGNS_OVERRIDE", "[]");
