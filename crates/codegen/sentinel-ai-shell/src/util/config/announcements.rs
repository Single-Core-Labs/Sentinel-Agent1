...
/// Re-exported from `sentinel-ai-announcements` for backward compatibility.
pub use sentinel_ai_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `AI_ANNOUNCEMENTS_OVERRIDE` (a JSON
...
pub(crate) fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("AI_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid AI_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// `AI_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev-only escape hatch).
