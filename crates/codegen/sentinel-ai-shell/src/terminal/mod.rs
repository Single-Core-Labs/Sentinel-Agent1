...
/// Resolved absolute path to bash. On Unix uses the `sentinel_ai_config` shell
/// resolution cascade (`$AI_SHELL` > `$SHELL` > `which` > common dirs >
...
pub(crate) fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        sentinel_ai_config::shell::unix_shell_path(sentinel_ai_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `sentinel-ai-tools`.
pub use sentinel_ai_tools::util::pager_env;
...
/// sentinel-ai-pager which renders its own UI and doesn't need raw ANSI codes).
///
