...
/// after we stop waiting. Mirrors the bound the `sentinel-ai-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("AI_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for sentinel_acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
