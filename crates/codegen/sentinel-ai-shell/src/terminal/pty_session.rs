...
enum Shell {
    Running {
        ...
        group: Option<Arc<sentinel_tty_utils::ProcessGroup>>,
    },
...
impl Shell {
    ...
    fn attach_group(&mut self, enrolled: Arc<sentinel_tty_utils::ProcessGroup>) {
    ...
    fn reap_now(&mut self) {
        if self.hangup() && self.wait_exit(sentinel_tty_utils::HANGUP_GRACE) {
            return;
        }
        self.kill();
...
pub(crate) async fn create_pty(
    ...
    if let Some(pid) = shell.pid() {
        ...
        let enrolled = tokio::task::spawn_blocking(move || {
            sentinel_tty_utils::global_process_scope().enroll_terminal_pid(pid)
        })
        .await
    ...
    if sentinel_tty_utils::global_process_scope().is_closed() {
...
fn reap(entry: &Arc<Mutex<PtySession>>) {
    ...
    if hung_up && wait_for_exit(entry, sentinel_tty_utils::HANGUP_GRACE) {
        return;
    }
...
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use sentinel_ai_config::shell::{WindowsShell, detect_windows_shell};
    ...
    async fn scope_teardown_kills_a_background_grandchild() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let scope = sentinel_tty_utils::ProcessScope::new();
