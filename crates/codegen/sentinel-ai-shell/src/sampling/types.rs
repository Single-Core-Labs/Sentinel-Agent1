// Re-export all types from the standalone sentinel-ai-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use sentinel_ai_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `sentinel-ai-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `sentinel-ai-sampler` crate can reference them without a circular
// dep on `sentinel-ai-shell`.

// Tests for the types now live in sentinel-ai-sampling-types crate.

use sentinel_ai_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `sentinel-ai-sampling-types` or
/// `sentinel-ai-tools`) so neither of those crates needs to depend on
