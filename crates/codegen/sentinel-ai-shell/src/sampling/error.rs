...
//!
//! The canonical error types now live in `sentinel_ai_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a ai-shell dependency).

// Re-export everything from the standalone crate.
pub use sentinel_ai_sampling_types::error::*;
...
/// API key / team rate-limit copy. Personal ai.com upgrades do not raise API
...
pub const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Ai Build usage limit for now. Get SuperAi for much higher limits, or try again later: https://ai.com/superai?referrer=ai-build";
...
/// API-key + personal SuperAi upsell → team credits copy. Else the body
...
/// IC sometimes reuses OAuth free-tier upsell copy on 429s ("upgrade to a Ai
/// subscription" / ai.com/superai). That is wrong for API-key / team auth:
/// higher limits come from credits and spend-based rate-limit tiers, not a
/// personal SuperAi plan.
fn pushes_consumer_subscription_upsell(detail: &str) -> bool {
    let d = detail.to_ascii_lowercase();
    d.contains("ai.com/superai") || d.contains("upgrade to a ai subscription")
}
...
/// This stays in sentinel-ai-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a Ai subscription")
                    && crate::agent::auth_method::has_sentinel_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (AI_API_KEY). \
                         Your cached OAuth session is being used instead. \
                         To use your API key, run `ai logout` or type /logout in the TUI."
                    )
                } else {
                    message
                };
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                sentinel_ai_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub(crate) fn terminal_error_data(
    message: String,
    http_status: Option<u16>,
    kind: sentinel_ai_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != sentinel_ai_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == sentinel_ai_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = sentinel_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &sentinel_ai_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn format_rate_limited_surfaces_nonempty_server_detail() {
        ...
        // Team console rate-limit copy has no personal SuperAi upsell — surface as-is.
    ...
    fn format_rate_limited_api_key_rewrites_consumer_subscription_upsell() {
        let body = "Some resource has been exhausted: You are sending requests too quickly. \
             Please slow down, or upgrade to a Ai subscription for higher limits: \
             https://ai.com/superai";
        let wire = format!("API error (status 429 Too Many Requests): {body}");
        // OAuth keeps the IC body (personal plan upgrade is correct).
        assert_eq!(format_rate_limited_user_message(Some(&wire), false), body);
        // API key must not push ai.com SuperAi — team credits / rate-limit tiers.
        assert_eq!(
    ...
    /// Helper: run a closure with AI_API_KEY temporarily set (or cleared).
    /// Cleans up even if the closure panics.
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        let prev = std::env::var("AI_API_KEY").ok();
        let prev_legacy = std::env::var("AI_CODE_AI_API_KEY").ok();
        // SAFETY: serial_test ensures no concurrent env mutation.
        unsafe {
            unsafe { std::env::remove_var("AI_API_KEY") };
            unsafe { std::env::remove_var("AI_CODE_AI_API_KEY") };
            if let Some(k) = key {
                unsafe { std::env::set_var("AI_API_KEY", k) };
            }
        }
        ...
        unsafe {
            ...
            unsafe { std::env::remove_var("AI_CODE_AI_API_KEY") };
            if let Some(v) = prev {
                unsafe { std::env::set_var("AI_API_KEY", v) };
            }
            if let Some(v) = prev_legacy {
                unsafe { std::env::set_var("AI_CODE_AI_API_KEY", v) };
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("sentinel-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
                message: "The model 'ai-build' requires a Ai subscription.".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry: None,
            };
            ...
            assert!(
                msg.contains("ai logout"),
                "should suggest ai logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
