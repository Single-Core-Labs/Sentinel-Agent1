...
pub use self::types::*;
pub use sentinel_ai_sampler::ApiBackend;
pub use sentinel_ai_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// sentinel-ai-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `sentinel-ai-sampler` crate. We re-export the public surface here so
...
// `sentinel_ai_sampler::*`. The shell-side `sampling::client::Config`
