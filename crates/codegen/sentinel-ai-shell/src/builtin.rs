//! Built-in files extracted to `~/.ai/` on startup.

const BUILTIN_FILES: &[(&str, &str)] = &[("README.md", include_str!("../README.md"))];

/// Extract built-in metadata files to `~/.ai/` on startup.
///
/// User skills under `~/.ai/skills/` are never managed here. Platform skills
/// are delivered separately through the bundled skill cache.
pub fn extract_builtin_files(ai_home: &std::path::Path) {
    let version = sentinel_ai_version::VERSION;
    let marker = ai_home.join(".metadata_version");
    ...
    for stale in &["CHANGELOG.json", "CHANGELOG.md"] {
        let _ = std::fs::remove_file(ai_home.join(stale));
    }

    for &(filename, content) in BUILTIN_FILES {
        if let Err(e) = std::fs::write(ai_home.join(filename), content) {
...
/// `$AI_HOME/skills/`; `help` rows hash the pre-substitution bytes.
...
/// Remove platform-skill leftovers extracted into `$AI_HOME/skills/` by
...
pub fn purge_stale_extracted_skills(ai_home: &std::path::Path) {
    purge_skill_dirs_matching(ai_home, FORMER_PLATFORM_SKILL_HASHES);
}

fn purge_skill_dirs_matching(ai_home: &std::path::Path, known: &[(&str, &str)]) {
    // For reversing the extract-time `~/.ai/` -> home rewrite (help only).
    let home_prefix = format!("{}/", ai_home.to_string_lossy());

    let mut names: Vec<&str> = known.iter().map(|&(name, _)| name).collect();
    names.dedup();

    for name in names {
        let dir = ai_home.join("skills").join(name);
        ...
        let unrewritten = sha256_hex(content.replace(&home_prefix, "~/.ai/").as_bytes());
        let managed = known
            .iter()
...
mod tests {
    ...
    fn same_version_does_not_restore_missing_or_delete_legacy_skills() {
        ...
        std::fs::write(home.join(".metadata_version"), sentinel_ai_version::VERSION).unwrap();
    ...
    fn purge_reverses_home_rewrite_before_hashing() {
        ...
        let raw = EXTRACTED_BODY.replace("platform", "read ~/.ai/docs/user-guide then platform");
        let body = raw.replace("~/.ai/", &format!("{}/", home.to_string_lossy()));
