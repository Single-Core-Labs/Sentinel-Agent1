...
/// Env var that, when set, advertises `sentinel.api_key` as a viable auth method.
///
/// Kept as a constant so test code and the production check stay in sync.
pub const AI_API_KEY_ENV_VAR: &str = "AI_API_KEY";

/// Legacy env var name. Checked as a fallback when `AI_API_KEY` is not set,
/// so existing deployments that use the old name keep working.
pub const LEGACY_AI_API_KEY_ENV_VAR: &str = "AI_CODE_AI_API_KEY";

/// Read the API key from the environment.
///
/// Checks `AI_API_KEY` first, then falls back to the legacy
/// `AI_CODE_AI_API_KEY` for backward compatibility.
pub(crate) fn read_sentinel_api_key_env() -> Result<String, std::env::VarError> {
    std::env::var(AI_API_KEY_ENV_VAR).or_else(|_| std::env::var(LEGACY_AI_API_KEY_ENV_VAR))
}

/// Returns `true` if either `AI_API_KEY` or `AI_CODE_AI_API_KEY` is set.
pub fn has_sentinel_api_key_env() -> bool {
    read_sentinel_api_key_env().is_ok()
}

/// Whether `sentinel.api_key` should be advertised (and pushed FIRST) when building
/// the `auth_methods` list at `initialize()` time.
///
/// Regression: `sentinel.api_key` must stay first when only per-model credentials
/// exist (no global `AI_API_KEY`). Deferring it made BYOK users hit the login
...
///
/// `disable_api_key_auth` (`[ai_com_config] disable_api_key_auth` /
/// `AI_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
/// method is never advertised, regardless of available credentials, so
/// `AI_API_KEY` can't bypass a deployment's forced IdP login.
///
/// Presence-only for the first-party env key (treats it as usable). Login
/// paths that have run the validity probe should call
/// [`should_advertise_sentinel_api_key_with_env_ok`] with the probe result instead.
pub(crate) fn should_advertise_sentinel_api_key<'a, I>(disable_api_key_auth: bool, models: I) -> bool
where
    I: IntoIterator<Item = &'a ModelEntry>,
{
    should_advertise_sentinel_api_key_with_env_ok(disable_api_key_auth, models, true)
}

/// Single advertise policy for `sentinel.api_key`: kill switch, BYOK, and first-party
/// env gated by `first_party_env_ok` (probe result, or `true` for presence-only).
/// BYOK still advertises without a probe.
pub(crate) fn should_advertise_sentinel_api_key_with_env_ok<'a, I>(
    ...
    I: IntoIterator<Item = &'a ModelEntry>,
{
    ...
    has_byok || (has_sentinel_api_key_env() && first_party_env_ok)
}
...
pub struct AuthMethodsBuildInputs<'a> {
    /// True if `sentinel.api_key` should be advertised AT ALL. Login/initialize
    /// callers compute via [`should_advertise_sentinel_api_key_with_env_ok`] after
    /// the validity probe; presence-only paths may use
    /// [`should_advertise_sentinel_api_key`]. When `preferred_method` is `Oidc`,
    ...
    /// Optional display label for the login method (`ai.com` or `oidc`).
    pub login_label: Option<&'a str>,
    /// True if `ai_com_config.auth_provider_command` is configured (sets
    /// `meta.external_provider = true` on the `ai.com` method).
...
pub struct BuiltAuthMethods {
    ...
    /// `cached_token` wins over `sentinel.api_key` when both are present. When
...
/// `has_external_api_key` is true, the **first** entry MUST be `sentinel.api_key`.
...
/// 1. `sentinel.api_key`     (if `has_external_api_key`)
...
/// - `sentinel.api_key`  else if `has_external_api_key`
/// - `None`         otherwise
///
/// Pinned (`preferred_method`):
/// - `ApiKey`: only `sentinel.api_key` if available; else empty list + `None` (fail).
/// - `Oidc`: `cached_token` (if any) + interactive login; never `sentinel.api_key`.
...
fn build_pinned_api_key(has_external_api_key: bool) -> BuiltAuthMethods {
    if !has_external_api_key {
        sentinel_ai_telemetry::unified_log::warn(
    ...
    BuiltAuthMethods {
        methods: vec![sentinel_api_key_auth_method()],
        default_auth_method_id: Some(acp::AuthMethodId::new(AI_API_KEY_METHOD_ID)),
    }
}
...
fn build_unpinned(
    ...
    if has_external_api_key {
        methods.push(sentinel_api_key_auth_method());
        default_auth_method_id = Some(acp::AuthMethodId::new(AI_API_KEY_METHOD_ID));
    }

    if has_cached_token {
        methods.push(cached_token_auth_method());
        // cached_token wins over sentinel.api_key for default_auth_method_id so
        // is_session_based_auth() returns true and OIDC refresh stays alive.
        let overrode_api_key = default_auth_method_id.is_some();
        default_auth_method_id = Some(acp::AuthMethodId::new(CACHED_TOKEN_AUTH_METHOD_ID));
        if overrode_api_key {
            sentinel_ai_telemetry::unified_log::info(
                "auth method priority: cached_token overrides sentinel.api_key for default_auth_method_id",
                None,
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.ai_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.ai_com_config.oidc.as_ref().unwrap()`
        ...
        methods.push(ai_com_auth_method(login_label, has_auth_provider_command));
    }
}
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            AI_API_KEY_METHOD_ID => Self::AiApiKey,
            CACHED_TOKEN_AUTH_METHOD_ID => Self::CachedToken,
            AI_COM_METHOD_ID => Self::AiCom,
    ...
    /// `true` for session-based methods (cached_token, ai.com, oidc).
    pub(crate) fn is_session_based(self) -> bool {
        matches!(self, Self::CachedToken | Self::AiCom | Self::Oidc)
    }

    /// Requires user interaction (browser, OIDC redirect, or external auth command).
    pub fn needs_interactive_login(self) -> bool {
        matches!(self, Self::AiCom | Self::Oidc)
    }
}

/// `true` for session-based ACP methods (cached_token, ai.com, oidc).
...
/// `AI_API_KEY` is set, which demoted live OIDC sessions to non-refreshable
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `ai login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `ai login`, set AI_API_KEY, or add api_key to ~/.ai/config.toml.";

/// Next ACP method id when `cached_token` cannot proceed (missing / expired /
/// legacy WebLogin), or `None` when fallthrough is forbidden.
///
/// Unpinned: prefer non-interactive `sentinel.api_key` when advertiseable, else
/// interactive `ai.com`.
///
...
pub const PREFERRED_API_KEY_UNAVAILABLE: &str = "preferred_method=api_key but no API key is configured (set AI_API_KEY or model api_key/env_key in config.toml).";

/// Error when `preferred_method=oidc` but the session path cannot proceed.
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `ai login` to authenticate.";

pub const AI_API_KEY_METHOD_ID: &str = "sentinel.api_key";
pub(crate) fn sentinel_api_key_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(
            acp::AuthMethodId::new(AI_API_KEY_METHOD_ID),
            "sentinel.api_key".to_string(),
        )
        .description(Some(format!(
            "{AI_API_KEY_ENV_VAR} or api_key/env_key in config.toml"
        ))),
    )
}
...
pub(crate) fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.ai/auth.json".to_string())),
    )
}

pub const AI_COM_METHOD_ID: &str = "ai.com";

/// AI OAuth2/OIDC auth. Method id `"ai.com"` kept for ACP wire-compat.
pub(crate) fn ai_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("Ai");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(AI_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    /// `cached_token` to non-interactive `sentinel.api_key` (not browser OAuth).
    ...
    /// `sentinel.api_key` first, while `default_auth_method_id` prefers session;
    /// after session fails, this helper must still pick `sentinel.api_key`.
    #[test]
    ...
    /// No advertiseable API-key credentials → interactive `ai.com`.
    #[test]
    fn after_cached_token_unavailable_falls_to_ai_com_without_api_key() {
        assert_eq!(
    ...
    fn auth_method_kind_classifier_matrix() {
        ...
        let api_id = acp::AuthMethodId::new(AI_API_KEY_METHOD_ID);
    ...
    // Reordering so `sentinel.api_key` is after login methods must fail the tests below.

    /// BYOK with only per-model `env_key` must list `sentinel.api_key` first.
    #[test]
    fn enterprise_byok_first_method_is_sentinel_api_key() {
        ...
        assert_eq!(
            ...
            Some(AuthMethodKind::AiApiKey),
            "BYOK enterprise-style: auth_methods.first() MUST be sentinel.api_key \
        ...
        assert!(
            ...
            "first method MUST NOT need interactive login when sentinel.api_key is available",
        );
    }

    /// BYOK + cached session token: sentinel.api_key stays first in the methods
    /// list (skips login screen), but `default_auth_method_id` is
    /// `cached_token` (keeps OIDC refresh alive).
    #[test]
    fn byok_with_cached_token_keeps_sentinel_api_key_first() {
        let inputs = AuthMethodsBuildInputs {
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::AiApiKey),
            "sentinel.api_key MUST precede cached_token in advertised order",
        );
    ...
    /// `ai.com` — `auth_methods.first()` does NOT need interactive login,
    ...
    /// Brand-new user (no API key, no cached token): only `ai.com` is
    ...
    #[test]
    fn fresh_user_only_advertises_ai_com_and_requires_login() {
        let built = build_auth_methods(default_inputs());

        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::AiCom));
        assert!(built.default_auth_method_id.is_none());
        assert_eq!(built.methods.len(), 1);
    }

    /// Enterprise OIDC replaces `ai.com` (mutually exclusive). sentinel.api_key,
    /// when present, still leads.
    #[test]
    fn enterprise_oidc_replaces_ai_com_but_sentinel_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
        ...
        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::AiApiKey));
        assert!(
            built
                .methods
                .iter()
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::AiCom),
            "ai.com and oidc are mutually exclusive",
        );
    }

    /// `has_auth_provider_command` is plumbed through to the `ai.com` method
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let ai = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::AiCom)
            .expect("ai.com must be advertised");
        assert_eq!(ai.name(), "Acme Corp");
        let meta = ai.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.ai/config.toml` skeleton from the bug report, walks it through
    /// the same predicate (`should_advertise_sentinel_api_key`) and the same
    /// list-builder (`build_auth_methods`) that `MvpAgent::initialize()` uses
    /// in production, and asserts that `auth_methods.first()` is `sentinel.api_key`
    /// (which causes the pager to skip the login screen).
    ///
    /// This is the test that *would have caught* that regression -- if you mentally
    /// re-introduce that bug (push sentinel.api_key LAST when has_external_api_key
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        let _global = EnvGuard::unset(AI_API_KEY_ENV_VAR);
        ...
        // login method. Confirms the predicate isn't trivially true.
        {
            ...
            let has_external_api_key = should_advertise_sentinel_api_key(false, models.values());
            ...
            assert_ne!(
                first_kind(&built.methods),
                Some(AuthMethodKind::AiApiKey),
                "without env_key resolved, sentinel.api_key must NOT be advertised first",
            );
        }

        // With the env var present (the actual enterprise scenario), the predicate
        // returns true and the builder MUST put `sentinel.api_key` first so the
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            let built = build_auth_methods(AuthMethodsBuildInputs {
                ...
                // ai.com login (no enterprise OIDC).
                has_cached_token: false,
                ..default_inputs()
            });
            assert_eq!(
                first_kind(&built.methods),
                Some(AuthMethodKind::AiApiKey),
                "BYOK: sentinel.api_key must be auth_methods.first(); deferred-to-last \
    ...
    /// `AI_API_KEY` alone (no per-model creds) also triggers
    /// advertising `sentinel.api_key` as the first method. Historical "external
    /// key" path; covered here so the predicate keeps treating env-var-only
    /// users the same as per-model users.
    #[test]
    #[serial]
    fn global_external_api_key_advertises_sentinel_api_key_first() {
        let _set = EnvGuard::set(AI_API_KEY_ENV_VAR, "sentinel-external-key");
    ...
    /// AND per-model env_key), so the builder never advertises `sentinel.api_key`
    /// and the pager sends the user to the deployment's login method instead.
    #[test]
    #[serial]
    fn disable_api_key_auth_suppresses_sentinel_api_key_method() {
        let _set = EnvGuard::set(AI_API_KEY_ENV_VAR, "sentinel-external-key");
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);

        // Flag off: today's behavior (advertised first).
        assert!(should_advertise_sentinel_api_key(false, models.values()));

        // Flag on: never advertised, regardless of credentials.
        let has_external_api_key = should_advertise_sentinel_api_key(true, models.values());
        assert!(!has_external_api_key);
        let built = build_auth_methods(AuthMethodsBuildInputs {
            has_external_api_key,
            ..default_inputs()
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::AiApiKey),
            "sentinel.api_key must not be advertised when disable_api_key_auth is set",
        );
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::AiCom),
    ...
    fn env_key_probe_unusable_suppresses_advertise_without_byok() {
        let _set = EnvGuard::set(AI_API_KEY_ENV_VAR, "sentinel-dead-key");
        let _legacy = EnvGuard::unset(LEGACY_AI_API_KEY_ENV_VAR);
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);
        assert!(
            should_advertise_sentinel_api_key(false, models.values()),
            "presence-only helper still sees the env key"
        );
        assert!(
            !should_advertise_sentinel_api_key_with_env_ok(false, models.values(), false),
    ...
    fn env_key_probe_ok_still_advertises() {
        let _set = EnvGuard::set(AI_API_KEY_ENV_VAR, "sentinel-live-key");
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);
        assert!(should_advertise_sentinel_api_key_with_env_ok(
            false,
            models.values(),
            true
        ));
    ...
    fn byok_advertises_even_when_env_probe_unusable() {
        ...
        let _unset = EnvGuard::unset(AI_API_KEY_ENV_VAR);
        let _legacy = EnvGuard::unset(LEGACY_AI_API_KEY_ENV_VAR);
    ...
    /// Legacy `AI_CODE_AI_API_KEY` env var is accepted as a fallback
    /// when `AI_API_KEY` is not set, ensuring existing deployments keep working.
    #[test]
    #[serial]
    fn legacy_env_var_fallback_advertises_sentinel_api_key() {
        let _unset_new = EnvGuard::unset(AI_API_KEY_ENV_VAR);
        let _set_legacy = EnvGuard::set(LEGACY_AI_API_KEY_ENV_VAR, "sentinel-legacy-key");
        assert!(has_sentinel_api_key_env());
        assert_eq!(read_sentinel_api_key_env().unwrap(), "sentinel-legacy-key");
    ...
    /// When both `AI_API_KEY` and `AI_CODE_AI_API_KEY` are set,
    /// the new name takes precedence.
    #[test]
    #[serial]
    fn new_env_var_takes_precedence_over_legacy() {
        let _new = EnvGuard::set(AI_API_KEY_ENV_VAR, "new-key");
        let _legacy = EnvGuard::set(LEGACY_AI_API_KEY_ENV_VAR, "old-key");
        assert_eq!(read_sentinel_api_key_env().unwrap(), "new-key");
    }

    // -- ai login --legacy regression coverage ------------------------
    //
    // `ai login --legacy` produces a AiAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `AI_AUTH` env var (or via legacy
    ...
    // to-end. A regression in AI_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `AI_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn ai_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, AiAuth, AiComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("AI_AUTH_PATH");
        let _g2 = EnvGuard::unset(AI_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `ai login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = AiAuth {
        ...
        // Provide it via AI_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("AI_AUTH", &legacy_json);

        // AuthManager picks it up from the env var directly (no file needed).
        let dir = tempfile::tempdir().unwrap();
        let cfg = AiComConfig::default();
        let mgr = AuthManager::new(dir.path(), cfg);
        let current = mgr.current();
        assert!(
            current.is_some(),
            "legacy token in AI_AUTH env MUST be loaded directly -- if this fails, \
        ...
        // With only this legacy token (no sentinel api key), first method must be
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("AI_AUTH");
        let _g2 = EnvGuard::unset("AI_AUTH_PATH");
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::AiCom),
            "no cached token AND no api key: pager must show login (ai.com first)",
        );
    }
    ...
    fn pin_api_key_with_key_only_advertises_api_key() {
        ...
        assert_eq!(method_ids(&built), vec![AI_API_KEY_METHOD_ID]);
        assert_eq!(default_id(&built), Some(AI_API_KEY_METHOD_ID));
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, AI_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![AI_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
