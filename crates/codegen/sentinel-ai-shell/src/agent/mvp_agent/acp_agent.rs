...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        sentinel_ai_telemetry::unified_log::info("agent initialized", None, None);
        sentinel_ai_telemetry::startup::mark_agent_serving();
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let opts = sentinel_fast_worktree::AutoGcOptions::from_resolved(auto_gc_policy);
            if let Err(e) = sentinel_fast_worktree::WorktreeDb::open_default()
                .and_then(|db| sentinel_fast_worktree::maybe_auto_gc(&db, &opts))
            {
        ...
        {
            let root = crate::util::ai_home::ai_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    sentinel_ai_workspace::permission::cleanup_stale_permission_state(
        ...
        sentinel_ai_workspace::trust::migrate_legacy_hook_trust();
        if let Some(auth) = self.auth_manager.current() {
            ...
            sentinel_ai_telemetry::unified_log::info(
                ...
                Some(
                    ...
                    "key_prefix": sentinel_ai_auth::bearer_suffix(&auth.key),
                    "rt_prefix": auth.refresh_token.as_deref().map(sentinel_ai_auth::bearer_suffix),
                }),
                ),
            );
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_ai_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("ai-web") => client_type = ClientType::AiWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("ai-code-extension") => client_type = ClientType::Extension,
                Some("ai-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let pre = self
            ...
            .current()
            .map(|a| (
                sentinel_ai_auth::bearer_suffix(&a.key).to_owned(),
                a
                    .refresh_token
                    .as_deref()
                    .map(|t| sentinel_ai_auth::bearer_suffix(t).to_owned()),
            ));
        ...
        if !self.cfg.borrow().ai_com_config.api_key_auth_disabled()
            && auth_method::read_sentinel_api_key_env().is_err()
            && let Some(api_key) = crate::auth::read_api_key(
                &crate::util::ai_home::ai_home(),
            )
        {
            unsafe { std::env::set_var("AI_API_KEY", &api_key) };
            tracing::info!("auth: loaded API key from auth.json (sentinel::api_key scope)");
            sentinel_ai_telemetry::unified_log::info(
                "auth: loaded API key from auth.json (sentinel::api_key scope)",
                None,
                None,
            );
        }
        ...
        let preferred_method_early = self.cfg.borrow().ai_com_config.preferred_method;
        let sentinel_api_base_url = self.cfg.borrow().endpoints.sentinel_api_base_url.clone();
        let has_byok = self
            .models_manager
            .models()
            .values()
        ...
        let first_party_env_ok = if crate::auth::should_probe_first_party_env_key(
            ...
            has_byok,
            auth_method::has_sentinel_api_key_env(),
        ...
        let has_external_api_key = auth_method::should_advertise_sentinel_api_key_with_env_ok(
            disable_api_key_auth,
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.ai_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.ai_com_config.auth_provider_label.clone(),
                cfg.ai_com_config.auth_provider_command.is_some(),
                cfg.ai_com_config.oidc.is_some(),
                issuer,
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                label = ?login_label,
                has_auth_provider,
                "auth: advertising ai.com auth method",
            );
        }
        ...
        sentinel_ai_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!({
                "ai_home": crate::util::ai_home::ai_home().display().to_string(),
        ...
        debug_assert!(
            !has_external_api_key
                || matches!(
                    ...
                    Some(auth_method::AuthMethodKind::AiApiKey)
                ),
            "BYOK invariant violated: sentinel.api_key MUST be auth_methods.first() \
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("AI_AGENT_METADATA");
                    serde_json::json!({
                    "aiShell": true,
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().ai_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::AI_API_KEY_METHOD_ID => {
                if self.cfg.borrow().ai_com_config.api_key_auth_disabled() {
                ...
                if sampling_config.api_key.is_none() {
                    if let Ok(api_key) = auth_method::read_sentinel_api_key_env() {
                        ...
                        .any(|m| m.has_own_credentials())
                    {
                        ...
                        return Err(
                            acp::Error::auth_required()
                                .data(
                                    "Set AI_API_KEY or add api_key/env_key to config.toml.",
                                ),
                        );
                    }
                }
                ...
                log_event(sentinel_ai_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().ai_com_config.preferred_method,
            ...
            auth_method::AI_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let ai_ctx = self.auth_manager.ai_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_ai_oauth(cli_oauth);
                    ...
                    })?;
                {
                    ...
                    tracing::debug!("auth: ai.com/oidc handler set api_key (SessionToken)");
                    sentinel_ai_telemetry::unified_log::debug(
                        "auth: ai.com/oidc handler set api_key (SessionToken)",
                        None,
                        None,
                    );
                }
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            sentinel_file_utils::trace_context::link_current_span_to_meta(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match sentinel_ai_sampling_types::ToolOverridesUpdate::parse(value) {
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: sentinel_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                admission: None,
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<sentinel_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                        ...
                        && let Some(client) = self.session_registry_client()
                    {
                        ...
                        tokio::spawn(async move {
                            let git_out = |args: &[&str]| -> Option<String> {
                                sentinel_tty_utils::git_command()
                                    .current_dir(&cwd_str)
                                    .args(args)
                                    .output()
                                    .ok()
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = sentinel_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = sentinel_ai_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = sentinel_ai_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            sentinel_file_utils::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            ...
            "x.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_sentinel_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `ai login` to authenticate.",
                )?;
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "_x.ai/session/update" {
            ...
            >(args.params.get()) {
                tracing::info!(
                    "Storing AI session notification: session_id={}",
                    notification.session_id.0
                );
                if let Some(handle) = self.resident_handle(&notification.session_id) {
                    let _ = handle
                        .cmd_tx
                        .send(crate::session::SessionCommand::AiSessionNotification {
                            notification,
                        });
                } else {
                    tracing::warn!(
                        "Received AI session notification for unknown session: {}",
                        notification.session_id.0
                    );
                }
            } else {
                tracing::warn!("Failed to parse AI session notification params");
            }
        }
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| sentinel_ai_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| sentinel_ai_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == sentinel_ai_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                sentinel_ai_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            sentinel_ai_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
