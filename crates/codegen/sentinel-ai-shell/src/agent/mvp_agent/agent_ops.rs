...
use sentinel_ai_tools::implementations::ai_build::task::backend::SubagentBackend;
use sentinel_tty_utils::ProcessScope;
...
impl MvpAgent {
    ...
    fn has_proxy_credentials(&self) -> bool {
        self.cfg.borrow().endpoints.deployment_key.is_some()
            || self.auth_manager.current_or_expired().is_some_and(|a| a.is_sentinel_auth())
    }
    ...
    pub(crate) fn sync_process_static_api_key(&self, preferred_model_id: Option<&str>) {
        if self.cfg.borrow().ai_com_config.api_key_auth_disabled() {
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::AiAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
    ...
    /// must not block the ACP response (ai-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled ai-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_sentinel_auth() || a.auth_mode == crate::auth::AuthMode::ApiKey
                }) else {
                return;
            };
            ...
            let (user_id, team_id) = if auth.is_sentinel_auth() {
                (Some(auth.user_id), auth.team_id)
            } else {
                (None, auth.team_id)
            };
            sentinel_ai_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                sentinel_ai_version::VERSION.to_owned(),
                subscription_tier,
    ...
    /// Return the current AiAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::AiAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &sentinel_ai_agent::plugins::SharedPluginRegistryHandle {
    ...
    fn arm_local_workspace_watcher(
        ...
        let auth_path = sentinel_ai_workspace::hub_auth::default_auth_path().ok();
    ...
    fn ensure_attach_fs_only_advertised_tools() -> Result<(), String> {
        const ENV: &str = "AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS";
        const ALLOW: &[&str] = &[
            "workspace.fs_list",
            "workspace.fs_exists",
        ...
        let Some(raw) = std::env::var(ENV)
            ...
            return Err(
                "attached workspace_server advertised toolset is uncheckable; refuse attach \
                 (set AI_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS to a comma-separated FS-only catalog)"
                    .into(),
            );
        };
        let ids: Vec<&str> = raw
    ...
    /// discovery on ai-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<sentinel_ai_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    sentinel_ai_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    sentinel_ai_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match sentinel_ai_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => sentinel_ai_workspace::WorkspaceOps::local(handle),
            Err(e) => {
                tracing::error!(
                    error = %e,
    ...
    ///     `ai.com`, `oidc`) -- even if the in-memory token is currently
    ///     expired or missing.
    ///
    /// Returns `ApiKey` only when the auth method is BYOK (`sentinel.api_key`) or
    ...
    pub(crate) fn auth_type(&self) -> sentinel_chat_state::AuthType {
    ...
    /// Fall through to `sentinel.api_key` if the startup probe still allows it,
    /// else `ai.com`. `None` when `preferred_method` is pinned.
    pub(super) fn cached_token_fallthrough_method_id(
        &self,
    ) -> Option<acp::AuthMethodId> {
        let preferred = self.cfg.borrow().ai_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_sentinel_api_key_with_env_ok(
                self.cfg.borrow().ai_com_config.api_key_auth_disabled(),
    ...
    /// `use_oauth` only when the target is interactive `ai.com`. When
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let meta = if method_id.0.as_ref() == auth_method::AI_COM_METHOD_ID {
    ...
    /// `resolve_config` gate: only upgrades from `Local`, writeback needs sentinel auth.
    fn reapply_storage_mode(&self) {
        ...
        let resolved_mode = {
            ...
            let has_sentinel_auth = self
                .auth_manager
                .current_or_expired()
                .is_some_and(|a| a.is_sentinel_auth());
            StorageMode::from_remote_gated(cfg.remote_settings.as_ref(), has_sentinel_auth)
        };
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::AiAuth) {
        if !crate::util::config::resolve_remote_fetch_enabled() {
            tracing::debug!("post-auth settings refresh skipped: remote_fetch disabled");
            return;
        }
        let is_sentinel = auth.is_sentinel_auth();
        ...
        let (
            ...
            let ai_user_id = is_sentinel.then(|| user_id.clone());
            let ai_team_id = is_sentinel.then(|| team_id.clone()).flatten();
        ...
        sentinel_ai_telemetry::client::init(
            telemetry_config,
            telemetry_mode,
            ai_user_id,
            ai_team_id,
    ...
    pub(super) fn spawn_post_auth_settings(&self, auth: crate::auth::AiAuth) {
        let agent_ref = LocalRef::new(self);
        let _spawned = self
            .spawn_coalesced_settings_task(
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<sentinel_ai_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if prefers_oidc && !model.has_own_credentials()
            && credentials.auth_type == sentinel_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = sentinel_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == sentinel_chat_state::AuthType::ApiKey
    ...
    /// Whether the current session is a personal ai.com account on a gated
    ...
    /// SuperAi upsell prose (see `ImageGenConfig`/`VideoGenConfig`'s
    /// `tier_restricted`).
    ///
    ...
    fn is_tier_restricted_capability(&self) -> bool {
        ...
        if !auth.is_sentinel_auth() || auth.team_id.is_some() {
            return false;
        }
        let tier = self
            .cfg
    ...
    ///
    /// Both BYOK and session (OAuth) users go direct to `sentinel_api_base_url`.
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> sentinel_ai_tools::implementations::ai_build::image_gen::ImageGenConfig {
        ...
        let cfg = self.cfg.borrow();
        let base_url = cfg.endpoints.sentinel_api_base_url.clone();
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| sentinel_ai_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("sentinel-ai-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        &self,
    ) -> sentinel_ai_tools::implementations::ai_build::deploy_app::AppBuilderDeployerConfig {
        use sentinel_ai_tools::implementations::ai_build::deploy_app::AppBuilderDeployerConfig;
        AppBuilderDeployerConfig::Disabled
    }
    /// Build video generation config. Video tools call the AI API directly.
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> sentinel_ai_tools::implementations::ai_build::video_gen::VideoGenConfig {
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `AI_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `AI_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    /// - `allow_local`: `[toolset.web_fetch] allow_local` > `AI_WEB_FETCH_ALLOW_LOCAL` > false
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> sentinel_ai_tools::implementations::ai_build::web_fetch::WebFetchConfig {
        use sentinel_ai_tools::implementations::ai_build::web_fetch::WebFetchConfig;
        let cfg = self.cfg.borrow();
    ...
    pub fn with_models(
        ...
        if !cfg.ai_com_config.api_key_auth_disabled() {
        ...
        let has_sentinel_auth = auth_manager
            .current_or_expired()
            .is_some_and(|a| a.is_sentinel_auth());
        let relay_sync_enabled = tui_mode && relay_config_enabled && has_sentinel_auth;
        ...
        if relay_sync_enabled {
            tracing::info!("[ai] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_sentinel_auth {
            tracing::info!("[ai] Relay sync: DISABLED (no auth - run 'ai login' first)");
        ...
        let instance = Self {
            ...
            plugin_registry_handle: sentinel_ai_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: sentinel_ai_tools::implementations::ai_build::monitor::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().ai_com_config.auth_provider_command.clone(),
    ...
    pub(crate) async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<sentinel_ai_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub(crate) async fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> sentinel_ai_tools::implementations::ai_build::task::types::SubagentCancelOutcome {
        sentinel_ai_tools::implementations::ai_build::task::backend::ChannelBackend::new(
    ...
    pub(crate) async fn list_running_subagents(
        ...
        sentinel_ai_tools::implementations::ai_build::task::types::SubagentInspection,
    > {
        sentinel_ai_tools::implementations::ai_build::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub(crate) async fn query_subagent(
        ...
        sentinel_ai_tools::implementations::ai_build::task::types::SubagentSnapshot,
    > {
        sentinel_ai_tools::implementations::ai_build::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<sentinel_ai_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or AI_RELAY_SYNC_ENABLED)
    ...
    pub(crate) async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<sentinel_hooks_plugins_types::HooksListResponse> {
    ...
    pub(crate) async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: sentinel_hooks_plugins_types::HooksAction,
    ) -> Option<sentinel_hooks_plugins_types::ActionOutcome> {
        if matches!(action, sentinel_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&sentinel_ai_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub(crate) async fn execute_plugins_action(
        &self,
        session_id: &acp::SessionId,
        action: sentinel_hooks_plugins_types::PluginsAction,
    ) -> Option<sentinel_hooks_plugins_types::ActionOutcome> {
        let is_reload = matches!(action, sentinel_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(|o| &o.status),
            Some(sentinel_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    pub(crate) fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<sentinel_ai_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn trace_upload_config_snapshot(
        ...
        let auth_token = if cfg.endpoints.deployment_key.is_none() {
            self.auth_manager
                .current_or_expired()
                .filter(|auth| auth.is_sentinel_auth())
                .map(|auth| auth.key)
        } else {
            None
        };
    ...
    pub(super) fn insert_session_config_meta(
        ...
        let detail = session_config::AiSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<sentinel_ai_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<sentinel_ai_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, sentinel_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(sentinel_ai_version::VERSION.to_string()),
                workspace_type: None,
                sandbox: local_sandbox_telemetry(),
            };
            let capture = sentinel_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                sentinel_ai_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            .upload_queue
            .get_or_init(|| {
                let ai_home = crate::util::ai_home::ai_home();
    ...
    ///
    /// `AI_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`sentinel_ai_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<sentinel_ai_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> sentinel_ai_agent::AgentDefinition {
        use sentinel_ai_agent::AgentDefinition;
        let ai_agent_env_set = std::env::var("AI_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(sentinel_ai_agent::config::is_strict_harness_agent_type);
        if !ai_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = sentinel_ai_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
                agent_name = %def.name,
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = sentinel_ai_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = %name,
        ...
        let agent_name = std::env::var("AI_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("ai-build-concise") | Some("ai_build_concise") => {
                AgentDefinition::ai_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::ai_build_plan()
                    }
                }
            }
            Some(name) => {
                sentinel_ai_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::ai_build_plan)
            }
            None => AgentDefinition::ai_build_plan(),
        };
        if !ai_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = sentinel_ai_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
                model_agent_type = %required,
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let envrc = envrc
            .unwrap_or_else(|| sentinel_ai_workspace::envrc::spawn_envrc_load(
        ...
        let fs_notify_config = init
            ...
            .and_then(|v| {
                ...
                use sentinel_fsnotify::FsConfig;
        ...
        let fs: Arc<dyn sentinel_ai_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (sentinel_hunk_tracker::HunkTrackerHandle::noop(), None),
        };
        let has_sentinel_auth = self.auth_manager.current().is_some_and(|a| a.is_sentinel_auth());
        let loc_tracking_enabled = hunk_tracking_enabled && has_sentinel_auth
            && (self
                .cfg
                .borrow()
                .remote_settings
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = sentinel_hunk_tracker::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = sentinel_hunk_tracker::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
                ...
                tokio::spawn(
                    sentinel_hunk_tracker::run_loc_sink(
                        hunk_event_rx,
                        loc_writer,
                        loc_ctx,
                        loc_cancel,
        ...
        let mut session_env = sentinel_ai_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            sentinel_ai_agent::config::ModelOverride::Override(id) => {
            ...
            sentinel_ai_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = sentinel_ai_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = sentinel_ai_tools::util::ai_home::ai_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".ai").join("lsp.json");
                tracing::debug!(
                    cwd = %tool_ctx.cwd,
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = sentinel_chat_state::Credentials {
            ...
            let attribution_callback: Option<
                sentinel_ai_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = sentinel_ai_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                        &format!(
                        "{}{}",
                        sentinel_ai_hooks::config::AGENT_HOOK_PREFIX,
                    ...
                    let git_root = sentinel_ai_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    sentinel_ai_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        sentinel_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == sentinel_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        sentinel_hunk_tracker::LocAggregate::LinesReverted {
