...
type ReplayCompletionRx = tokio::sync::oneshot::Receiver<sentinel_acp_lib::AcpResult<()>>;
...
impl MvpAgent {
    ...
    /// - AI updates (`"_x.ai/session/update"`) → `ExtNotification`.
    ///
    ...
    pub(super) fn forward_raw_replay_line(
        ...
        let is_sentinel = method == "_x.ai/session/update";
        ...
        if is_sentinel {
            ...
            let Ok(mut params) = serde_json::from_str::<serde_json::Value>(raw_params.get()) else {
                tracing::debug!("replay: skipping AI update with unparseable params");
                return None;
            };
...
mod drain_tests {
    ...
    async fn completion_window_awaits_oldest_before_exceeding_cap() {
        ...
        let (overflow_tx, overflow_rx) =
            tokio::sync::oneshot::channel::<sentinel_acp_lib::AcpResult<()>>();
        {
