...
use sentinel_ai_telemetry::id::{agent_id, agent_instance_id};
use sentinel_ai_telemetry::session_ctx::log_event;
use sentinel_ai_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use sentinel_ai_workspace::permission::{ClientType, PermissionEvent};
...
use sentinel_ai_sampler::SamplerConfig as SamplingConfig;
...
use sentinel_ai_paths::AbsPathBuf;
use sentinel_ai_workspace::session::git::GitDiscoveryResult;
use sentinel_hunk_tracker::HunkTrackerActor;
...
/// `/user` already reports a newly qualifying tier (e.g. `SuperAiPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "AiPro" => jwt_claim == "superai",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "SuperAiPro" => jwt_claim == "superai_heavy",
        "SuperAiLite" => jwt_claim == "superai_lite",
        "SuperAiPlus" => jwt_claim == "superai_plus",
...
pub(crate) struct SessionSpawnOptions<'a> {
    ...
    pub envrc: Option<sentinel_ai_workspace::envrc::EnvrcLoad>,
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a sentinel_ai_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[sentinel_ai_announcements::RemoteAnnouncement]>,
    last_emitted: &[sentinel_ai_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<sentinel_ai_announcements::RemoteAnnouncement>> {
    let current = sentinel_ai_announcements::filter_expired_at(
...
/// Override with `AI_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("AI_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
struct ResidentResources {
    ...
    codebase_index: Option<std::sync::Arc<sentinel_codebase_graph::IndexManagerHandle>>,
...
pub struct MvpAgent {
    ...
    /// ai.com chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// list. Set by `enforce_ai_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.ai/`).
    ///
    ...
    /// Local session-registry override: `AI_SESSION_REGISTRY` env, else
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        sentinel_ai_tools::implementations::ai_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: sentinel_ai_tools::implementations::ai_build::monitor::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: sentinel_ai_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled ai-desktop's first
    ...
    workspace_ops: RefCell<Option<sentinel_ai_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<sentinel_ai_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &sentinel_ai_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<sentinel_ai_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !sentinel_ai_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = sentinel_ai_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `ai-build` →
/// `ai-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `ai-build` →
/// `ai-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use sentinel_ai_agent::config::is_strict_harness_agent_type;
    match (
...
use sentinel_chat_state::conversation_util::replace_or_insert_system_head;
...
///
/// Headers injected:
///  - `x-ai-client-version` -- required by the proxy's version-gate check.
///    Uses `client_version` when provided, otherwise falls back to cli-chat-proxy
///    compile-time `CARGO_PKG_VERSION`.
///  - `X-AI-Token-Auth` / `x-authenticateresponse` -- required by the
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-ai-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
    ...
    headers
        .entry("x-ai-client-identifier".to_string())
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<sentinel_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(sentinel_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<sentinel_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        session_id: &acp::SessionId,
        updates_file_path: &Option<PathBuf>,
    ) -> Vec<tokio::sync::oneshot::Receiver<sentinel_acp_lib::AcpResult<()>>> {
        ...
        for task in &orphaned {
            let snapshot = sentinel_ai_tools::types::TaskSnapshot {
    ...
    ///
    /// Non-AI auth (API keys, enterprise) always passes. For AI OAuth2
    ...
    pub(super) async fn enforce_ai_code_access(&self, auth: &crate::auth::AiAuth) {
        ...
        *self.allow_access_resolved_for.borrow_mut() = Some(auth.user_id.clone());
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings ai_build_access_gate)"
            );
    ...
    /// cached_token, ai.com/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.ai/bundled/` and the manifest.
