...
//! (`x.ai/folder_trust/request`) that asks a GUI client (ai-desktop) to decide
...
//! verdict is [`sentinel_ai_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.ai/lsp.json` servers start on the NEXT session open (the durable grant
...
impl MvpAgent {
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = sentinel_ai_workspace::trust::workspace_key(cwd);
        ...
        self.session_registry.for_each_resident(|_, h| {
            if sentinel_ai_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key {
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a sentinel_ai_agent::plugins::SharedPluginRegistryHandle,
    compat: &'a sentinel_ai_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.ai/hooks`, `.cursor/hooks.json`),
