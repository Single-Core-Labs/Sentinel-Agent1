...
//! The shared coordinator actor lives in `sentinel-ai-tools`; this module plugs
...
impl coordinator::ChildRunner for ShellChildRunner {
    ...
    type ValidateFuture = coordinator::LocalBoxFuture<
        sentinel_ai_tools::implementations::ai_build::task::types::SubagentValidateTypeOutcome,
    >;
    type DescribeFuture = coordinator::LocalBoxFuture<
        sentinel_ai_tools::implementations::ai_build::task::types::SubagentDescribeOutcome,
    >;
        ...
        Box::pin(async move {
            ...
            let Some(mut ctx) = this.try_build_subagent_spawn_context(&parent_sid) else {
                ...
                return coordinator::ChildRunOutput {
                    result: sentinel_ai_tools::implementations::ai_build::task::types::SubagentResult {
                        success: false,
                        error: Some(
        ...
        Box::pin(async move {
            ...
            match this.try_build_subagent_spawn_context(&parent_session_id) {
                ...
                None => {
                    ...
                    sentinel_ai_tools::implementations::ai_build::task::types::SubagentDescribeOutcome::Unavailable
                }
            }
        })
    }
...
fn log_limit_notice(notice: coordinator::SubagentLimitNotice) {
    ...
    sentinel_ai_telemetry::session_ctx::log_event(SubagentLimitHit::session_concurrent(
...
impl MvpAgent {
    ...
    /// [`SubagentCoordinator`](sentinel_ai_tools::implementations::ai_build::task::coordinator::SubagentCoordinator)
    ...
    pub(super) fn start_subagent_coordinator(&self) {
        ...
        let config = coordinator::CoordinatorConfig {
            foreground_budget:
                sentinel_ai_tools::implementations::ai_build::task::backend::env_duration_or(
                    "AI_SUBAGENT_AWAIT_BUDGET_MS",
                    std::time::Duration::from_secs(600),
                ),
            limits: sentinel_ai_tools::implementations::ai_build::task::admission::SubagentLimits {
    ...
    pub(super) fn try_build_subagent_spawn_context(
        ...
        let (
            ...
            (
                ...
                ps.as_ref()
                    ...
                    .unwrap_or_else(sentinel_hunk_tracker::HunkTrackerHandle::noop),
                ps.as_ref()
                ...
                ps.as_ref()
                    ...
                    .unwrap_or_else(|| {
                        let cwd = ps
                            ...
                            .map(|h| std::path::PathBuf::from(&h.info.cwd))
                            .unwrap_or_default();
                        std::sync::Arc::new(sentinel_ai_workspace::file_system::LocalFs::new(cwd))
                    }),
                ps.as_ref()
        ...
        Some(crate::agent::subagent::SubagentSpawnContext {
            ...
            task_output_tool_name: parent_handle
                ...
                .unwrap_or_else(|| {
                    sentinel_ai_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
                }),
