...
) -> Option<sentinel_ai_sampling_types::ToolOverrides> {
...
fn insert_applied_tool_overrides(
    ...
    echo: Option<&sentinel_ai_sampling_types::ToolOverrides>,
) {
...
struct RestoredSignals {
    ...
    turn_count: u64,
    tool_call_count: u64,
    plan_mode_state: sentinel_ai_telemetry::events::PlanModeState,
...
impl RestoredSignals {
    fn read(
        ...
        use sentinel_ai_telemetry::events::PlanModeState as Reported;
        Self {
...
fn begin_session(session_id: &acp::SessionId, cwd: &AbsPathBuf) -> SessionInfo {
    sentinel_ai_telemetry::session_ctx::log_session_event(
...
impl MvpAgent {
    pub(super) async fn new_session_inner(
        ...
        if product_analytics || sentinel_ai_telemetry::external::is_active() {
            ...
            let perm = if session_yolo_mode {
                sentinel_ai_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                sentinel_ai_telemetry::enums::PermissionMode::Auto
            } else {
                sentinel_ai_telemetry::enums::PermissionMode::Ask
            };
            tokio::spawn(async move {
                let git = sentinel_ai_telemetry::context::collect_git_context(&cwd_str);
                let ev = sentinel_ai_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                sentinel_ai_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) =
            match sentinel_ai_workspace::session::git::discover_git_root(cwd.as_path()) {
    ...
    async fn attach_session(
        ...
        let git_root =
            sentinel_ai_workspace::session::git::find_git_root_from_path(cwd.as_path()).ok();
        ...
        let envrc = if !load_envrc {
            Some(sentinel_ai_workspace::envrc::spawn_envrc_load(
        ...
        let (model_state, response_meta) = self
            ...
            .await;
        sentinel_ai_telemetry::unified_log::info("session loaded", Some(session_id.0.as_ref()), None);
        ...
        if self.product_analytics_enabled() {
            log_event(sentinel_ai_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn restore_session_code(
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            sentinel_ai_workspace::session::git::warn_registry_disabled_restore(session_id.0.as_ref());
        }
        let restore_checkout_allowed =
            sentinel_ai_workspace::session::git::restore_code_checkout_allowed(
                cwd.as_path(),
        ...
        {
            tracing::warn!(
                target: sentinel_ai_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
                supplied_cwd = %cwd.as_str(),
                persisted_cwd = %summary.info.cwd,
                target_sha = %target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a ai worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
        ...
        if restore_code_requested
            ...
            && let Some(ref target_sha) = summary.head_commit
        {
            use sentinel_ai_workspace::session::git::RestoreKind;
            let outcome = sentinel_ai_workspace::session::git::checkout_session_commit(
                cwd.as_path(),
                target_sha,
                true,
                session_id.0.as_ref(),
    ...
    async fn restore_persisted_model(
        ...
        let is_ai_build = persisted_model.0.starts_with("ai-build");
        let same_family_fallback = if is_ai_build {
            available
                .keys()
                .find(|id| id.0.starts_with("ai-build"))
                .cloned()
        } else {
            available
                .keys()
    ...
    async fn build_attach_response_meta(
        ...
        if summary.head_commit.is_some()
            ...
            && summary.git_root_dir.as_deref().is_none_or(|root| {
                sentinel_ai_workspace::session::git::find_git_root_from_path(std::path::Path::new(
                    cwd.as_str(),
                ))
                .ok()
            ...
            })
        {
            ...
            let current_head =
                sentinel_ai_workspace::session::git::git_cli(cwd_path, &["rev-parse", "HEAD"])
                    .await
                    .ok();
            if let Some(divergence) = sentinel_ai_workspace::session::git::detect_head_divergence(
