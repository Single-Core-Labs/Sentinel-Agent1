...
fn includes_baseline_keys_without_usage() {
    let meta = build_prompt_response_meta(args("sess-1", "prompt-1", 42_000, "ai-4.5"));
...
fn enriches_meta_with_camelcase_token_keys() {
    ...
    let meta = build_prompt_response_meta(PromptResponseMetaArgs {
        ...
        ..args("sess-1", "prompt-1", 1_700, "ai-4.5")
    });
...
fn tool_overrides_land_as_camelcase_meta_key() {
    let overrides = sentinel_ai_sampling_types::ToolOverrides {
        x_search: Some(sentinel_ai_sampling_types::XSearchOptions {
            date_bound: Some(
                sentinel_ai_sampling_types::SearchDateBound::new(None, Some("2024-03-15".to_string()))
                    .unwrap(),
            ),
        }),
        web_search: None,
