...
fn auth_with_mode(mode: crate::auth::AuthMode, key: &str) -> crate::auth::AiAuth {
    crate::auth::AiAuth {
        key: key.into(),
        auth_mode: mode,
...
fn resolve_subscription_tier_prefers_display_then_api_key_then_jwt() {
    ...
    let api = auth_with_mode(crate::auth::AuthMode::ApiKey, "sentinel-not-a-jwt");
    assert_eq!(
...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ...
        ("superai_heavy", "SuperAiPro"),
        ("9", "EnterpriseMystery"),
        ("superai_lite", "SuperAiLite"),
        ("superai_plus", "SuperAiPlus"),
    ];
...
fn jwt_claim_matches_user_subscription_tier_rejects_stale_and_unknown() {
    ...
    assert!(!jwt_claim_matches_user_subscription_tier("free", "AiPro"));
...
mod hunk_tracking_mode {
    ...
    use sentinel_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<sentinel_ai_sampling_types::conversation::ConversationItem> {
    use sentinel_ai_sampling_types::ToolCall;
    use sentinel_ai_sampling_types::conversation::ConversationItem;
    vec![
...
async fn upload_harness_trace_turns_numbers_siblings_and_persists_counter() {
    ...
    let queue = crate::upload::trace::spawn_upload_queue(
        queue_home.path(),
        &queue_cfg,
        Some(sentinel_ai_version::VERSION),
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_ai_build() {
    let prev = std::env::var("AI_AGENT").ok();
    unsafe {
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("AI_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return ai-build.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = sentinel_ai_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `ai-build-plan`, models in the catalog that still declare
/// `agent_type = "ai-build"` explicitly must NOT preempt an ACP
/// profile. Any value in the `ai-build*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_ai_build_family() {
    let prev = std::env::var("AI_AGENT").ok();
    unsafe {
        unsafe { std::env::remove_var("AI_AGENT") };
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["ai-build", "ai-build-plan", "ai-build-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for ai-build family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("AI_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use sentinel_ai_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".ai").join("agents");
    ...
    assert_eq!(
        def.model,
        sentinel_ai_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe {  std::env::set_var("AI_AGENT", v)  },
        None => unsafe {  std::env::remove_var("AI_AGENT")  },
    }
}
#[test]
...
/// flow: a zero-turn switch from `ai-build` (a client profile name) to
/// `ai-build-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("ai-build", "ai-build-plan"));
    assert!(harnesses_are_compatible("ai-build-plan", "ai-build"));
    assert!(harnesses_are_compatible("ai-build", "ai-build"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("ai-build-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "ai-build-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_ai_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "ai-build-plan"),
        "ai-build-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_ai_build() {
    assert_eq!(
        resolve_required_agent_type(None, "ai-build"),
        "ai-build"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "ai-build-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "ai-build-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "ai-build-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use sentinel_ai_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = sentinel_ai_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: sentinel_ai_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: sentinel_ai_tools::implementations::ai_build::image_gen::ImageGenConfig::default(),
        video_gen_config: sentinel_ai_tools::implementations::ai_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: sentinel_ai_tools::implementations::ai_build::deploy_app::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: sentinel_ai_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = sentinel_hunk_tracker::HunkTrackerActor::spawn(
        "test".to_string(),
        std::path::PathBuf::from("/tmp"),
        hunk_event_tx,
        sentinel_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        hunk_tracker_handle,
        chat_state_handle: sentinel_chat_state::ChatStateHandle::noop(),
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            sentinel_ai_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(sentinel_ai_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: sentinel_ai_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "ai-build".to_string(),
        managed_mcp_proxy_base_url: String::new(),
        session_default_agent_profile: None,
        allowed_subagent_types: None,
        hook_registry: None,
        workspace_ops: sentinel_ai_workspace::WorkspaceOps::for_test(),
...
async fn lookup_session_model_returns_per_session_model() {
    ...
    assert_eq!(
        lookup_session_model(Some(acp::ModelId::new("ai-3-fast")), &default_model)
            .0
            .as_ref(),
        "ai-3-fast"
    );
...
async fn lookup_session_model_fallback_no_session() {
    let default_model = acp::ModelId::new("ai-3");
    assert_eq!(
...
async fn set_session_model_does_not_cross_contaminate() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("ai-3", false, None)),
        (sid_b.clone(), make_test_handle("ai-3", false, None)),
    ]
    .into();
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use sentinel_ai_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use sentinel_ai_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("ai-3", false, Some("ai-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("ai-3", false, Some("ai-code-extension")),
        ),
    ]
    .into();
    let updated =
        apply_yolo_mode_to_matching_sessions(sessions.values_mut(), Some("ai-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("ai-3", true, Some("ai-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("ai-3", true, Some("ai-code-extension")),
        ),
    ]
    .into();
    let updated =
        apply_yolo_mode_to_matching_sessions(sessions.values_mut(), Some("ai-tui"), false);
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("ai-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("ai-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::AiWeb) {
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::AiAuth {
                key: "eligible".into(),
                auth_mode: crate::auth::AuthMode::WebLogin,
                ..crate::auth::AiAuth::test_default()
            });
            use acp::Agent as _;
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    use crate::auth::{AuthManager, AiComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), AiComConfig::default()));
...
fn build_agent_with_auth(auth: crate::auth::AiAuth) -> MvpAgent {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, AiComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let ai_home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("AI_HOME", ai_home.path());
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_home.path(), AiComConfig::default()));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    let gateway = GatewaySender::new(tx);
...
async fn resident_activity_reports_needs_input_when_pending() {
    ...
    let handle = make_test_handle("ai-3", false, None);
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<sentinel_acp_lib::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let sentinel_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
async fn push_roster_activity_delta_broadcasts_overridden_activity() {
    ...
    agent.insert_resident(&sid, make_test_handle("ai-3", false, None));
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("ai-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("ai-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("ai-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("ai-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("ai-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::AiWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::AiTUI, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::AiWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::AI_COM_METHOD_ID,
...
/// BYOK guard. Users with `sentinel.api_key` must continue to report `ApiKey`
...
#[tokio::test(flavor = "current_thread")]
async fn auth_type_sentinel_api_key_no_current_returns_api_key() {
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::AI_API_KEY_METHOD_ID,
    ));
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(
        agent.auth_type(),
        sentinel_chat_state::AuthType::ApiKey,
        "sentinel.api_key auth must report ApiKey -- BYOK has no session-token \
...
async fn auth_type_session_based_with_current_returns_session_token() {
    use crate::auth::AiAuth;
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::OIDC_METHOD_ID,
    ));
    agent.auth_manager.hot_swap(AiAuth::test_default());
    assert!(agent.auth_manager.current().is_some());
    assert_eq!(agent.auth_type(), sentinel_chat_state::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), sentinel_chat_state::AuthType::ApiKey,);
}
...
/// Minimal agent whose `ai_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.ai_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
/// Deployment-key / managed-config user: `AI_API_KEY` resolves and the kill
/// switch is off, so a dead `cached_token` MUST fall through to `sentinel.api_key`
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    use crate::agent::auth_method::{AI_API_KEY_ENV_VAR, AI_API_KEY_METHOD_ID};
    use sentinel_ai_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("AI_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(AI_API_KEY_ENV_VAR, "test-deployment-key");
    let agent = build_minimal_agent_for_tests();
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(AI_API_KEY_METHOD_ID),
        "deployment-key user (AI_API_KEY set, no kill switch) must fall \
         through to sentinel.api_key on a dead cached_token -- not interactive login",
    );
}
/// Forced-IdP deployment: even with `AI_API_KEY` present, the admin kill
/// switch keeps the fallthrough on interactive `ai.com` (api-key auth is
/// neither advertised nor an eligible fallthrough).
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{AI_COM_METHOD_ID, AI_API_KEY_ENV_VAR};
    use sentinel_ai_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("AI_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(AI_API_KEY_ENV_VAR, "test-deployment-key");
    ...
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(AI_COM_METHOD_ID),
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive ai.com so AI_API_KEY can't bypass forced IdP login",
    );
}
/// No advertiseable credentials at all (no env key, no kill switch): the user
/// genuinely needs to log in, so the fallthrough is interactive `ai.com`.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_ai_com_without_credentials() {
    use crate::agent::auth_method::{
        AI_COM_METHOD_ID, LEGACY_AI_API_KEY_ENV_VAR, AI_API_KEY_ENV_VAR,
    };
    use sentinel_ai_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("AI_DISABLE_API_KEY_AUTH");
    let _new = EnvGuard::unset(AI_API_KEY_ENV_VAR);
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(AI_COM_METHOD_ID),
        "no API-key creds and no kill switch -> interactive ai.com login",
    );
}
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use sentinel_ai_tools::implementations::ai_build::video_gen::{
...
async fn prepare_video_gen_config_respects_feature_flag() {
    use sentinel_ai_tools::implementations::ai_build::video_gen::VideoGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
    assert!(matches!(
        agent.prepare_video_gen_config(),
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use sentinel_ai_tools::implementations::ai_build::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
/// Build traffic via the `x-ai-client-identifier` header. If this header is
...
async fn data_collection_enabled_for_normal_user() {
    let agent = build_agent_with_auth(crate::auth::AiAuth::test_default());
    assert!(
...
fn chat_initial_model_matrix() {
    let cases: &[(&str, bool, Option<&str>, Option<&str>)] = &[
        ("chat_with_model", true, Some("ai-4.5"), Some("ai-4.5")),
        ("chat_without_model", true, None, None),
        ("build_with_model", false, Some("ai-4.5"), None),
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = sentinel_ai_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = sentinel_ai_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(sentinel_ai_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<sentinel_ai_workspace::permission::PermissionEvent>();
    agent
        .session_registry
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::AI_DISABLE_CUSTOM_BRIDGE_ENV);
    run_local_for_bridge_test(|| async {
        let agent = build_minimal_agent_for_tests();
...
/// `test_cancel_ends_in_flight_turn_and_frees_slot` (ai-agent-sdk).
#[test]
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("ai-tui"));
    ...
    for (method, params) in cases {
        ...
        match (method, cmd) {
            (
                ...
                assert_eq!(owner.as_deref(), Some("ai-tui"));
            }
            ...
            ("x.ai/queue/clear", SessionCommand::ClearQueue { owner }) => {
                assert_eq!(owner.as_deref(), Some("ai-desktop"));
            }
            (
                "x.ai/queue/edit",
                ...
                assert_eq!(editor.as_deref(), Some("ai-vscode"));
            }
            (
                "x.ai/queue/interject",
...
async fn storage_mode_self_corrects_to_writeback_when_settings_arrive() {
    let _env = crate::env::EnvVarGuard::remove("AI_STORAGE_MODE");
    let auth = crate::auth::AiAuth {
...
fn post_auth_settings_not_coalesced_by_in_flight_reapply() {
    run_local_for_bridge_test(|| async {
        ...
        agent.spawn_settings_reapply();
        assert!(agent.settings_reapply_in_flight.get());
        agent.spawn_post_auth_settings(crate::auth::AiAuth::test_default());
        assert_eq!(
...
fn build_agent_with_auth_and_proxy(
    auth: crate::auth::AiAuth,
    proxy_url: String,
    mode: crate::agent::config::AgentMode,
) -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<sentinel_acp_lib::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, AiComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
...
impl Drop for RestoreOtelGate {
    fn drop(&mut self) {
        sentinel_ai_telemetry::external::mark_external_otel_settings_resolved();
    }
}
...
async fn access_gate_does_not_leak_verdict_across_identities() {
    ...
    use crate::auth::{AiAuth, AI_OAUTH2_ISSUER};
    let auth_a = AiAuth {
        oidc_issuer: Some(AI_OAUTH2_ISSUER.to_string()),
        user_id: "user-a".into(),
        ..AiAuth::test_default()
    };
    let (agent, _rx) = build_agent_with_auth_and_proxy(
    ...
    *agent.allow_access_resolved_for.borrow_mut() = Some("user-a".to_string());
    ...
    assert!(auth_b.is_sentinel_auth(), "precondition: first-party AI auth");
    agent.enforce_ai_code_access(&auth_b).await;
    assert!(
        agent.tier_allowed.get(),
        "identity B must not inherit identity A's denied allow_access verdict",
    );
}
/// First-party AI auth + `writeback_enabled` settings → storage upgrades to
/// Writeback; the settings arrival also emits `x.ai/settings/update` and opens
/// the external-OTEL gate.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial_test::serial]
async fn post_auth_settings_sentinel_upgrades_writeback_emits_and_opens_gate() {
    use crate::agent::config::AgentMode;
    use crate::auth::{AiAuth, AI_OAUTH2_ISSUER};
    let _restore = RestoreOtelGate;
    let _storage_env = crate::env::EnvVarGuard::remove("AI_STORAGE_MODE");
    let server = sentinel_ai_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    let sentinel_auth = AiAuth {
        oidc_issuer: Some(AI_OAUTH2_ISSUER.to_string()),
        ..AiAuth::test_default()
    };
    assert!(sentinel_auth.is_sentinel_auth(), "precondition: first-party AI auth");
    let (agent, mut rx) =
        build_agent_with_auth_and_proxy(sentinel_auth, server.url(), AgentMode::Leader);
    assert_eq!(
        agent.storage_mode(),
        StorageMode::Local,
        "precondition: leader boots in Local storage mode"
    );
    sentinel_ai_telemetry::external::suppress_external_otel_until_settings();
    assert!(!sentinel_ai_telemetry::external::is_settings_gate_open());
    agent.maybe_fetch_post_auth_settings().await;
    assert_eq!(
        agent.storage_mode(),
        StorageMode::Writeback,
        "sentinel auth + writeback_enabled settings must upgrade storage to Writeback"
    );
    assert!(
        sentinel_ai_telemetry::external::is_settings_gate_open(),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial_test::serial]
async fn post_auth_settings_non_sentinel_keeps_local_but_still_emits() {
    use crate::agent::config::AgentMode;
    use crate::auth::{AuthMode, AiAuth};
    let _restore = RestoreOtelGate;
    let server = sentinel_ai_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    assert_eq!(
        agent.storage_mode(),
        StorageMode::Local,
        "non-sentinel auth must stay Local even when writeback is advertised remotely"
    );
    assert!(
        sentinel_ai_telemetry::external::is_settings_gate_open(),
    ...
    assert!(
        drained_settings_update(&mut rx),
        "settings arrival must push x.ai/settings/update for non-sentinel auth too"
    );
}
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial_test::serial]
async fn post_auth_settings_failure_resolves_gate_onto_local_policy() {
    ...
    let (agent, _rx) = build_agent_with_auth_and_proxy(sentinel_auth, server.url(), AgentMode::Leader);
    sentinel_ai_telemetry::external::suppress_external_otel_until_settings();
    assert!(!sentinel_ai_telemetry::external::is_settings_gate_open());
    agent.maybe_fetch_post_auth_settings().await;
    assert!(
...
async fn same_credential_refresh_does_not_flap_resolved_gate() {
    ...
    let (agent, _rx) =
        build_agent_with_auth_and_proxy(sentinel_auth.clone(), server.url(), AgentMode::Leader);
    agent.otel_gate.set_resolved_for(&sentinel_auth.user_id);
    sentinel_ai_telemetry::external::mark_external_otel_settings_resolved();
    assert!(sentinel_ai_telemetry::external::is_settings_gate_open());
    agent.refresh_remote_settings(&sentinel_auth).await;
    assert!(
        sentinel_ai_telemetry::external::is_settings_gate_open(),
...
async fn settings_self_heal_refetches_after_token_rotation() {
    ...
    use crate::auth::{AiAuth, AI_OAUTH2_ISSUER};
    let _restore = RestoreOtelGate;
    let server = sentinel_ai_test_support::MockInferenceServer::start_with_required_auth(
        vec![sentinel_ai_test_support::MockModelEntry::new("ai-build")],
        "rotated-key",
    )
    .await
    .unwrap();
    ...
    impl TokenRefresher for RotatingRefresher {
        async fn refresh(&self, _r: crate::auth::manager::RefreshReason) -> RefreshOutcome {
            RefreshOutcome::Success(Box::new(AiAuth {
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".ai/roles");
    let personas = cwd.join(".ai/personas");
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.insert_resident(&sid, handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                sentinel_ai_subagent_resolution::config::SubagentRole {
...
/// End-to-end gate wiring: project `.ai/roles` / `personas` alone must drive
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _env = EnvGuard::set("AI_HOME", home.path());
    let _sim = EnvGuard::set(sentinel_ai_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("AI_FOLDER_TRUST");
    let repo = tempfile::tempdir().unwrap();
    git2::Repository::init(repo.path()).unwrap();
    write_project_subagent_definitions(repo.path());
    run_local_for_bridge_test(|| async {
...
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<sentinel_acp_lib::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let sentinel_acp_lib::AcpClientMessage::ExtMethod(args) = msg else {
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use sentinel_ai_test_support::EnvGuard;
    use sentinel_ai_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("AI_HOME", home.path());
    let _sim = EnvGuard::set(sentinel_ai_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("AI_FOLDER_TRUST");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, sentinel_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(sentinel_acp_lib::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use sentinel_ai_test_support::EnvGuard;
    use sentinel_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("AI_HOME", home.path());
    let _sim = EnvGuard::set(sentinel_ai_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("AI_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(sentinel_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
...
fn ann(id: &str) -> sentinel_ai_announcements::RemoteAnnouncement {
    sentinel_ai_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<sentinel_ai_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = sentinel_ai_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = sentinel_ai_announcements::RemoteAnnouncement {
...
async fn emit_announcements_gate_emits_updates_baseline_and_bumps_gen() {
    ...
    let recv_gen =
        |rx: &mut tokio::sync::mpsc::UnboundedReceiver<sentinel_acp_lib::AcpClientMessage>| {
            ...
            let sentinel_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
