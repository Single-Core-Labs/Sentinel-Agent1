...
use sentinel_ai_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl ModelFetchAuth {
    ...
    pub(crate) fn resolve(endpoints: &config::EndpointsConfig, has_cached_session: bool) -> Self {
        if endpoints.has_custom_endpoint() {
            ...
            Self::Deployment
        } else if crate::agent::auth_method::has_sentinel_api_key_env() {
            Self::ApiKey
        } else {
            Self::Session
        }
...
struct Inner {
    ...
    gateway: RwLock<Option<sentinel_acp_lib::AcpAgentGatewaySender>>,
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&ai_home, AiComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: sentinel_acp_lib::AcpAgentGatewaySender) {
    ...
    /// Hot-reload the catalog from `~/.ai/models_cache.json` after an external write (config-watcher detected).
    ...
    async fn bounded_startup_auth(auth_manager: &Arc<AuthManager>) -> Option<AiAuth> {
    ...
    /// without a live IdP.
    async fn bounded_auth_refresh<F>(fut: F) -> Option<AiAuth>
    where
        F: std::future::Future<Output = Option<AiAuth>>,
    {
