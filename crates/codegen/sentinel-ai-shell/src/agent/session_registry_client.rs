...
impl SessionRegistryClient {
    pub fn new(base_url: impl Into<String>, user_token: impl Into<String>) -> Self {
        ...
        Self {
            ...
            credentials: crate::util::ai_auth_credentials::AiAuthCredentials::new(Some(
                user_token.into(),
            )),
            session_id: None,
        }
    ...
    async fn send_authed(
        ...
        reqwest::Response,
        Option<sentinel_ai_auth::StampedBearerSuffix>,
    )> {
        let builder = sentinel_file_utils::trace_context::inject_trace_context_into_request(builder);
        let request = builder.build().context(op)?;
        sentinel_ai_auth::execute_with_stamp(&self.client, request)
            .await
            .map_err(|e| match e {
    ...
    /// [`sentinel_ai_auth::StampedBearerSuffix`] for why never a re-resolution).
    fn record_401_attribution(&self, op: &str, stamp: Option<&sentinel_ai_auth::StampedBearerSuffix>) {
