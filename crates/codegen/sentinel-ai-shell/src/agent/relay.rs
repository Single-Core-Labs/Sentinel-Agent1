...
//! connection to the ai.com relay server with automatic reconnection.
...
/// Config for the ai.com WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a ai.com first-party session
    /// (`is_sentinel_auth`: x.ai-issuer OIDC or external credential) with a
    ...
    pub(crate) fn for_session(
        ...
        if !session.is_sentinel_auth() || session.key.is_empty() {
            return None;
        }
        let _ = alpha_test_key;
        Some(Self {
            ws_url: ctx.ai_ws_url.clone(),
            ws_origin: ctx.ai_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to ai.com with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `ai login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    match recovered {
        Ok(new_auth) if new_auth.key == config.auth.key => {
            ...
            sentinel_ai_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "key_prefix": sentinel_ai_auth::bearer_suffix(&new_auth.key),
                })),
            );
            false
        }
        Ok(new_auth) => {
            ...
            sentinel_ai_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "new_key_prefix": sentinel_ai_auth::bearer_suffix(&new_auth.key),
                })),
            );
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from Ai WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        "x-ai-client-version",
        axum::http::header::HeaderValue::from_static(sentinel_ai_version::VERSION),
    );
    req.headers_mut().insert(
...
mod tests {
    ...
    /// Helper to create a test AiAuth with the given key.
    fn test_auth(key: &str) -> AiAuth {
        AiAuth {
            key: key.to_string(),
    ...
    #[test]
    fn for_session_builds_only_for_sentinel_issuer() {
        use crate::auth::AI_OAUTH2_ISSUER;
        let cfg = AiComConfig::default();
        let builds = |a: &AiAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let sentinel = AiAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(AI_OAUTH2_ISSUER.to_string()),
            ..test_auth("sentinel-bearer")
        };
        assert!(sentinel.is_sentinel_auth(), "precondition: is_sentinel_auth");
        assert!(builds(&sentinel));
        let external_sentinel = AiAuth {
            auth_mode: AuthMode::External,
            oidc_issuer: Some(AI_OAUTH2_ISSUER.to_string()),
            ..test_auth("ext-bearer")
        };
        assert!(external_sentinel.is_sentinel_auth(), "precondition: is_sentinel_auth");
        assert!(builds(&external_sentinel));
        assert!(!builds(&AiAuth {
            key: String::new(),
            ..sentinel.clone()
        }));
        assert!(!builds(&AiAuth {
    ...
    /// Helper: write a AiAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &AiAuth) {
