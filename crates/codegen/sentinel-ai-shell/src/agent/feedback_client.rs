...
const CLIENT_VERSION_HEADER: &str = "x-ai-client-version";

// ============================================================================
// Turn delta wire types (local to sentinel-ai-shell until cli-chat-proxy catches up)
// ============================================================================
...
pub struct FeedbackClient {
    ...
    base_url: String,
    credentials: crate::util::ai_auth_credentials::AiAuthCredentials,
...
impl FeedbackClient {
    pub fn new(base_url: impl Into<String>, user_token: Option<String>) -> Self {
        ...
        let credentials = crate::util::ai_auth_credentials::AiAuthCredentials::new(user_token);
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(sentinel_ai_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::ai_auth_credentials::AiAuthCredentials,
    ) -> Arc<dyn sentinel_ai_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(sentinel_ai_auth::StaticAuthCredentialProvider::new(
    ...
    fn record_401_attribution_if_needed(
        ...
        stamp: Option<&sentinel_ai_auth::StampedBearerSuffix>,
        op: &str,
    ) {
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, sentinel_ai_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-AI-Token-Auth", "sentinel-ai-cli")
        } else {
            builder
        }
    }
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = sentinel_file_utils::trace_context::inject_trace_context_into_request(request);
        let req = request.build().context(context)?;
        let (response, stamp) = sentinel_ai_auth::execute_with_stamp(&self.client, req)
            .await
            .context(context)?;
...
mod tests {
    ...
    fn test_signals_to_update() {
        let signals = crate::session::signals::SessionSignals {
            ...
            models_used: vec!["ai-3".to_string()],
            primary_model_id: Some("ai-3".to_string()),
        ...
        assert_eq!(update.primary_model_id, Some("ai-3".to_string()));
