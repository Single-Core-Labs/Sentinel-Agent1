...
pub fn bootstrap(
    ...
    sentinel_ai_telemetry::startup::enter(sentinel_ai_telemetry::startup::StartupPhase::Bootstrap);
    ...
    sentinel_ai_telemetry::startup::enter(sentinel_ai_telemetry::startup::StartupPhase::ModelCatalog);
    let models_manager = {
...
fn ensure_remote_settings_side_effects(cfg: &mut AgentConfig, sync_managed: bool) {
    ...
    if cfg.remote_settings.is_none() {
        let handle = if sync_managed {
            crate::agent::models::start_early_prefetch(Some(cfg.ai_com_config.clone()))
        } else {
            crate::agent::models::start_early_prefetch_settings_only(Some(
                cfg.ai_com_config.clone(),
            ))
        };
...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (ai -p, subagents).
    let has_sentinel_auth = auth_manager.current().is_some_and(|a| a.is_sentinel_auth());
    ...
    // A CLI/env-set Writeback still requires ai.com auth.
    if cfg.storage_mode == StorageMode::Writeback && !has_sentinel_auth {
        tracing::info!("Writeback is disabled: requires auth with ai.com");
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_builtin_files(&ai_home);
        if !cfg!(test) {
            // Deletes dirs; must never touch a unit-test process's real home.
            crate::builtin::purge_stale_extracted_skills(&ai_home);
        }

        crate::extensions::marketplace::purge_default_skills_installs(&ai_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&ai_home);
        }
        ...
        sentinel_ai_telemetry::session_ctx::log_event(limits.into_event());
    });
}
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let ai_auth = auth_manager.current().filter(|a| a.is_sentinel_auth());
    let user_id = ai_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = ai_auth.as_ref().and_then(|a| a.team_id.clone());
