...
//! [`sentinel_ai_telemetry::external`]:
//!
...
pub(crate) fn suppress() {
    sentinel_ai_telemetry::external::set_settings_gate_max_wait(SETTINGS_GATE_MAX_WAIT);
    sentinel_ai_telemetry::external::suppress_external_otel_until_settings();
}

/// Whether an AI fleet policy can govern this process at all.
...
pub(crate) fn policy_channel(remote_fetch_enabled: bool, proxy_is_sentinel: bool) -> PolicyChannel {
    if !remote_fetch_enabled {
        return PolicyChannel::Unavailable(NoPolicy::RemoteFetchDisabled);
    }
    if !proxy_is_sentinel {
...
/// Returns whether a session-less startup is about to mint a ai.com session
pub(crate) fn is_session_pending(
    has_session: bool,
    ai_com_config: &crate::auth::AiComConfig,
) -> bool {
    !has_session
        && (ai_com_config.auth_provider_command.is_some()
...
mod tests {
    ...
    fn policy_channel_reports_every_structural_reason() {
        assert_eq!(
            ...
            "remote_fetch off: the deployment declared it never calls AI"
        );
        assert_eq!(
            policy_channel(true, false),
            PolicyChannel::Unavailable(NoPolicy::ProxyRepointed),
            "a non-AI proxy is not governed by AI fleet policy"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            PolicyChannel::Applies,
            "AI proxy + fetches allowed: a policy can arrive, so wait for it"
        );
    }
