//! ai.com chat-product model catalog: caches `/rest/modes` and maps modes to
...
/// ~54 min, matching ai-web's refetch cadence.
...
pub const AI_CHAT_MODE_ENV: &str = "AI_CHAT_MODE";
...
impl ChatModesManager {
    ...
    /// The active ai.com identity, or `None` when unauthenticated. Modes are
...
/// Maps ai.com modes → `SessionModelState`: keeps only `available` modes,
...
mod tests {
    ...
    fn name_falls_back_to_id_when_title_blank() {
        ...
        let state = modes_to_model_state(&resp);
        assert_eq!(state.available_models[0].name, "ai-4.5");
    }
}
