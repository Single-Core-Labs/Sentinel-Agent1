...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct AiSessionDetail {
...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("ai-build", "Ai Build"),
            model("ai-4.5", "Ai 4.5"),
        ];
        let current = acp::ModelId::from("ai-build");
        ...
        assert_eq!(selected_models[0].id, "ai-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("ai-build", "Ai Build")];
        let current = acp::ModelId::from("ai-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("ai-build", "")];
        let current = acp::ModelId::from("ai-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "ai-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "ai-build".to_string(),
            category: "model".to_string(),
            label: "Ai Build".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "ai-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "Ai Build");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn ai_session_detail_serializes_camel_case() {
        let detail = AiSessionDetail::build(
            "sess-1".to_string(),
            "/Users/me/sentinel".to_string(),
            "ai-build".to_string(),
            None,
        );
        ...
        assert_eq!(v["cwd"], "/Users/me/sentinel");
        assert_eq!(v["currentModelId"], "ai-build");
