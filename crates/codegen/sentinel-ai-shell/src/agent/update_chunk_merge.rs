...
impl ReplayBuffer {
    ...
    fn merge(
        ...
        match (prev, new) {
            ...
            (Some(SessionNotification::Ai(prev)), SessionNotification::Ai(new)) => {
                let (force, first, second) = merge_sentinel_chunks(*prev, *new);
                (
                    force,
                    SessionNotification::Ai(Box::new(first)),
                    second.map(|s| SessionNotification::Ai(Box::new(s))),
                )
            }
...
/// Merge two consecutive AI notifications.
fn merge_sentinel_chunks(
...
fn estimate_payload_bytes(n: &SessionNotification) -> u64 {
    match n {
        ...
        SessionNotification::Ai(n) => match &n.update {
...
mod tests {
    ...
    #[test]
    fn sentinel_same_id_deltas_merge_args_and_preserve_name() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn sentinel_different_tool_call_id_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        ...
        assert!(matches!(flushed, SessionNotification::Ai(_)));
        assert!(rest.is_some());
        assert!(buf.pending.is_none());
    }

    #[test]
    fn sentinel_tool_call_delta_is_bufferable() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn sentinel_after_acp_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        let acp_chunk = msg_chunk("s", 1, "thinking...");
        let sentinel_chunk = delta_chunk("s", Some("call_1"), 0, Some("bash"), Some("args"));

        assert!(buf.consume_chunk(acp_chunk).is_none());
        let (flushed, rest) = buf.consume_chunk(sentinel_chunk).expect("should force-flush");

        // Both emitted immediately — different kinds can't merge.
        assert!(matches!(flushed, SessionNotification::Acp(_)));
        assert!(matches!(rest, Some(SessionNotification::Ai(_))));
        assert!(buf.pending.is_none());
    }
}
