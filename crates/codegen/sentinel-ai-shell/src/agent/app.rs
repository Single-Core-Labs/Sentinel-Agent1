...
use crate::auth::{AuthManager, AiAuth, AiComConfig, run_auth_flow};
...
/// or live subagent). The second signal covers relay-driven (ai.com
...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(sentinel_file_utils::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = sentinel_ai_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// ([`sentinel_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    sentinel_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
pub async fn run_stdio_agent(
    ...
    sentinel_ai_telemetry::unified_log::set_version(sentinel_ai_version::VERSION);
    sentinel_file_utils::queue::cleanup_orphaned_uploads(
        &ai_home::ai_home(),
        sentinel_file_utils::queue::DEFAULT_MAX_AGE,
    );
    if let Ok(version) = std::env::var("AI_CLIENT_VERSION") {
    ...
    let result = local_set
        .run_until(async move {
            ...
            apply_otel_config(&auth_manager, &agent_config.ai_com_config);
...
pub async fn run_headless(
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a ai.com session. \
        Run `ai login` to sign in, or use `ai agent stdio` for API-key access.";
    sentinel_file_utils::queue::cleanup_orphaned_uploads(
        &ai_home::ai_home(),
        sentinel_file_utils::queue::DEFAULT_MAX_AGE,
    );
    ...
    let ctx = &agent_config.ai_com_config;
    let (mut auth, did_browser_flow) = if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&ai_home::ai_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
        ...
        let auth_manager = Arc::new(AuthManager::new(&ai_home::ai_home(), ctx.clone()));
        if crate::agent::auth_method::has_sentinel_api_key_env()
    ...
    let ai_code_url = format!("{}/build", ctx.ai_ws_origin);
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow {
            eprintln!();
            eprintln!(
                "Open Ai Build: {} (press Enter to open in browser)",
                ai_code_url
            );
            eprintln!();
            let url_for_open = ai_code_url.clone();
        ...
        .run_until(async move {
            let _agent_handle = tokio::task::spawn_local(async move {
                ...
                tokio::task::spawn_local(
                    GatewayReceiver::new(gw_rx, conn)
                        .with_on_meta(
                            sentinel_file_utils::trace_context::span_from_meta_traceparent,
                        )
                        .run(),
                );
...
fn should_seed_shared_session(existing: Option<&AiAuth>, session: &AiAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
/// Start the leader's ai.com relay connection according to the start policy,
/// parking the [`RelayHandle`](crate::agent::relay::RelayHandle) in `slot`
/// once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `ai agent leader`
...
///   duplication of mirroring every agent message to ai.com.
///
...
/// Everything needed to arm the leader's ai.com relay *after* startup.
///
...
struct DeferredRelayArm {
    ...
    slot: Rc<std::cell::RefCell<Option<crate::agent::relay::RelayHandle>>>,
    ai_com_config: crate::auth::AiComConfig,
...
impl DeferredRelayArm {
    ...
    /// [`spawn_leader_relay`]).
    fn arm_if_eligible(self, session: &AiAuth, auth_manager: &Arc<AuthManager>) -> Option<Self> {
        ...
        info!("Relay-eligible auth token appeared after startup — arming ai.com relay");
        spawn_leader_relay(
            self.slot,
            relay_config,
...
/// the agent boundary.
pub fn apply_otel_config(auth_manager: &AuthManager, ai_com_config: &AiComConfig) {
    suppress_otel();
    ...
    if crate::agent::otel_gate::should_open_at_startup(crate::agent::otel_gate::StartupGate {
        ...
        has_session,
        session_pending: crate::agent::otel_gate::is_session_pending(has_session, ai_com_config),
    }) {
        crate::agent::otel_gate::open_at_startup();
    }
}
/// Run the agent in leader mode, accepting IPC connections from multiple clients.
/// When a ai.com session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the ai.com relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: sentinel_ai_version::VERSION.to_string(),
    })
    ...
    let ctx = &agent_config.ai_com_config;
    suppress_otel();
    let auth: Option<AiAuth> = crate::auth::try_noninteractive_auth_no_mint(ctx).await;
    let has_session = auth.is_some()
        || agent_config
            ...
            .is_some();
    let session_pending =
        crate::agent::otel_gate::is_session_pending(has_session, &agent_config.ai_com_config);
    let policy_channel =
    ...
    local_set
        .run_until(async move {
                ...
                tokio::task::spawn_local(async move {
                    tokio::select! {
                        ...
                        minted = crate::auth::mint_session_noninteractive(&mint_auth_manager)
                            => match minted {
                            Some(session) => info!(
                                is_sentinel = session.is_sentinel_auth(),
            ...
            if let Some(relay_config) = relay_config {
                ...
                info!(
                    "Relay not started: no ai.com session token \
                ...
                deferred_relay_arm = Some(DeferredRelayArm {
                    ...
                    ai_com_config: agent_config.ai_com_config.clone(),
            ...
            let auth_scope = agent_config.ai_com_config.auth_scope();
            let initial_auth_key_hash = sentinel_ai_config::user_ai_home()
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> AiAuth {
        AiAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::AI_OAUTH2_ISSUER.to_string()),
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::AiComConfig {
            ai_ws_url: format!("ws://{addr}"),
            ai_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
    ...
    fn embedded_otel_gate_keeps_a_session_user_fail_closed() {
        ...
        use sentinel_ai_telemetry::external::{
        ...
        impl Drop for Restore {
            fn drop(&mut self) {
                unsafe {
                    set_or_clear(AI_API_KEY_ENV_VAR, self.key.take());
                    set_or_clear(LEGACY_AI_API_KEY_ENV_VAR, self.legacy.take());
                    set_or_clear(PROXY_ENV_VAR, self.proxy.take());
                }
                mark_external_otel_settings_resolved();
            }
        }
        const PROXY_ENV_VAR: &str = "AI_CLI_CHAT_PROXY_BASE_URL";
        let _restore = Restore {
            key: std::env::var_os(AI_API_KEY_ENV_VAR),
            legacy: std::env::var_os(LEGACY_AI_API_KEY_ENV_VAR),
            proxy: std::env::var_os(PROXY_ENV_VAR),
        };
        let cfg = AiComConfig::default();
        unsafe {
            unsafe { std::env::set_var(AI_API_KEY_ENV_VAR, "test-key") };
            unsafe { std::env::remove_var(LEGACY_AI_API_KEY_ENV_VAR) };
        ...
        let with_session = {
            ...
            let am = Arc::new(AuthManager::new(dir.path(), AiComConfig::default()));
            am.hot_swap(session);
            am
        };
    ...
    /// `ai agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the ai.com relay eagerly.
    ...
    async fn deferred_arm_connects_relay_when_auth_appears() {
        ...
        let (_demand_tx, demand_rx) = watch::channel(false);
        let slot = Rc::new(std::cell::RefCell::new(None));
        let ai_com_config = crate::auth::AiComConfig {
            ai_ws_url: format!("ws://{addr}"),
            ai_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
        let tmp = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(AuthManager::new(tmp.path(), ai_com_config.clone()));
        ...
        local
            .run_until(async {
                let ineligible = AiAuth::test_default();
                let arm = arm
