...
use sentinel_ai_tools::implementations::{ai_build, opencode};
pub(super) fn canonical_total_tokens(totals: &sentinel_chat_state::UsageTotals) -> u64 {
    totals.total_tokens()
}
...
pub(super) async fn record_subagent_usage(
    ...
    by_model: Option<Vec<(String, sentinel_chat_state::UsageTotals)>>,
...
/// `sentinel-ai-tools` coordinator actor and reached only through `reporter`.
#[tracing::instrument(
...
pub(crate) async fn run_shell_child(
    run: ai_build::task::coordinator::ChildRunRequest<ShellChildRuntime>,
    mut ctx: SubagentSpawnContext,
    gateway: &GatewaySender,
) -> ChildRunOutput<ShellCompletionData> {
    let ai_build::task::coordinator::ChildRunRequest {
        mut request,
    ...
    let mut effective_runtime = sentinel_ai_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = sentinel_ai_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != sentinel_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != sentinel_tool_types::SubagentIsolationMode::None {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(&source_cwd) {
            ...
            Err(e) => {
                ...
                std::env::temp_dir()
                    .join("ai-subagent-worktrees")
                    .join(&request.id)
            }
        };
        let source_clone = source_cwd;
        let subagent_id = request.id.clone();
        let creation_mode: sentinel_fast_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
            let mut builder = sentinel_fast_worktree::WorktreeBuilder::new(&source_clone, &dest)
                .working_tree_mode(sentinel_fast_worktree::WorkingTreeMode::PreserveWorkingTree)
                .creation_mode(creation_mode)
                .worktree_kind(sentinel_fast_worktree::WorktreeKind::Subagent)
    ...
    effective_runtime.capability_mode = sentinel_ai_subagent_resolution::intersect_capability_modes(
    ...
    sentinel_ai_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    {
        let reminder = sentinel_ai_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let covered_by_parent = sentinel_fsnotify::watch_root_covers(&ctx.parent_cwd, &child_cwd);
    ...
    let child_cwd_abs = sentinel_ai_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        sentinel_ai_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let parent_traceparent = sentinel_file_utils::trace_context::current_traceparent();
    let tracker_child_cwd = child_session_info.cwd.clone();
    let tracker_model_id = effective_model_id.0.to_string();
    let initial_child_tokens = sentinel_chat_state::estimate_conversation_tokens(&forked_conversation);
    ...
    let attribution_callback: Option<sentinel_ai_sampler::SharedAttributionCallback> =
    ...
    let yolo_policy_block = sentinel_ai_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        let memory_tools: Vec<sentinel_ai_tools::registry::types::ToolConfig> = vec![
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                sentinel_ai_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == sentinel_ai_hooks::event::HookEventName::Stop {
                            s.event = sentinel_ai_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if !agent_owned_mcp_servers_allowed(is_plugin_agent) {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    sentinel_ai_agent::config::McpServerRef::Named(name) => {
                    ...
                    sentinel_ai_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            sentinel_ai_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        sentinel_ai_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
        sentinel_chat_state::CompactionMode::Summary,
        ...
        std::sync::Arc::new(parking_lot::Mutex::new(
            sentinel_ai_workspace::file_system::CodebaseIndexManager::new(),
        )),
        false,
        subagent_fs_watch,
        None,
        ...
        if inherit_skills {
            ...
            sentinel_ai_agent::prompt::skills::SkillsConfig::default()
        },
        if inherit_skills {
        ...
        ctx.yolo_mode
            || matches!(
                ...
                sentinel_ai_agent::config::PermissionMode::BypassPermissions
            ),
        false,
        None,
        ...
        Vec::new(),
        sentinel_ai_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        sentinel_ai_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        sentinel_ai_telemetry::events::Outcome::Cancelled
    } else {
        sentinel_ai_telemetry::events::Outcome::Error
    };
    sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        owner: telemetry_owner_kind(&request),
        workflow_run_id: request.owner.workflow_run_id().map(str::to_string),
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/ai/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
    ...
    let level_fn = if success {
        ...
        sentinel_ai_telemetry::unified_log::error
    };
    level_fn(
        if success {
            "subagent completed"
