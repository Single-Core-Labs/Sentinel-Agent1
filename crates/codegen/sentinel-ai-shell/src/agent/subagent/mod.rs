...
//! `sentinel-ai-tools` coordinator actor. This module keeps shell-specific
...
use sentinel_acp_lib::AcpAgentGatewaySender as GatewaySender;
use sentinel_file_utils::events::types::CancellationCategory;
use sentinel_ai_agent::config::{McpInheritance, ModelOverride, PermissionMode};
use sentinel_ai_sampling_types::conversation::ConversationItem;
use sentinel_ai_subagent_resolution::ResumeSourceData;
use sentinel_ai_tools::implementations::ai_build::monitor::types::MonitorEventBuffer;
...
use sentinel_ai_tools::implementations::ai_build::task::types::*;
use sentinel_ai_tools::types::tool::ToolKind;
use sentinel_ai_workspace::file_system::AsyncFileSystem;
use sentinel_hunk_tracker::HunkTrackerHandle;
mod handle_request;
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn sentinel_ai_tools::implementations::lsp::LspBackend>>,
    ...
    /// (not an intermediate parent's) because sentinel-ai-tools task/coordinator.rs
    ...
    pub process_scope: Option<sentinel_tty_utils::ProcessScope>,
    ...
    pub sampling_config: sentinel_ai_sampler::SamplerConfig,
    ...
    pub auth: Option<crate::auth::AiAuth>,
    pub parent_cwd: PathBuf,
    pub parent_session_id: String,
    /// The parent's cutoff at spawn, applied to the child's first turn. `None` if unset.
    pub inherited_tool_overrides: Option<sentinel_ai_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
    ...
    pub parent_terminal_backend: Option<Arc<dyn sentinel_ai_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<sentinel_ai_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<sentinel_ai_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: sentinel_ai_tools::implementations::ai_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: sentinel_ai_tools::implementations::ai_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: sentinel_ai_tools::implementations::ai_build::video_gen::VideoGenConfig,
    ...
    pub parent_chat_state: Option<sentinel_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<sentinel_ai_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<sentinel_ai_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<sentinel_ai_hooks::discovery::HookRegistry>>,
    pub permission_handle: Option<sentinel_ai_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<sentinel_ai_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: sentinel_ai_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<sentinel_ai_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "ai-build").
    ...
    pub parent_tool_definitions: Option<Vec<sentinel_ai_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<sentinel_ai_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: sentinel_ai_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: sentinel_ai_tools::types::compat::CompatConfig,
    /// Shared completion reservations held by auto-wake prompts.
    pub task_completion_reservations:
        Option<sentinel_ai_tools::reminders::task_completion::TaskCompletionReservations>,
...
impl SubagentSpawnContext {
    ...
    /// key (env `AI_API_KEY`). Keyed on the resolved config key, not the
    ...
    fn apply_session_cli_overrides(&self, def: &mut sentinel_ai_agent::config::AgentDefinition) {
    ...
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("AI_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub(crate) fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("AI_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &sentinel_ai_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (sentinel_ai_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &sentinel_ai_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (sentinel_ai_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &sentinel_ai_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &sentinel_ai_sampler::SamplerConfig,
) {
...
async fn read_parent_sampling_config(
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let inherited = sentinel_ai_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(sentinel_ai_sampler::SamplerConfig, acp::ModelId)> {
    ...
    config.bearer_resolver = if !ctx.would_strip_fallback_key(config.api_key.as_deref())
        && resolved_auth_type == sentinel_chat_state::AuthType::SessionToken
    {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[sentinel_ai_sampling_types::conversation::ConversationItem],
) -> usize {
    conversation
        .iter()
...
struct InitialContext {
    ...
    conversation: Vec<sentinel_ai_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<sentinel_ai_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        sentinel_ai_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[sentinel_ai_sampling_types::conversation::ConversationItem],
) -> bool {
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<sentinel_ai_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = sentinel_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        sentinel_ai_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
    ...
    if let Some(source) = resume_source {
            ...
            .await
        {
            Ok(result) => {
                ...
                let estimated_tokens = sentinel_chat_state::estimate_conversation_tokens(&conversation);
...
fn resolve_inherited_mcp_pool(
    ...
    inheritance: &sentinel_ai_agent::config::McpInheritance,
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<sentinel_ai_agent::config::AgentDefinition> {
    let cli_agents = ctx
        .agent_config
        .as_ref()
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    let resolution_context = sentinel_ai_subagent_resolution::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
    ...
    let mut def = sentinel_ai_subagent_resolution::discover_agent_definition(
        subagent_type,
        &resolution_context,
    )?;
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    sentinel_ai_subagent_resolution::available_agent_names(
        &sentinel_ai_subagent_resolution::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<sentinel_ai_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let context = sentinel_ai_subagent_resolution::DefinitionValidationContext {
        cwd: &ctx.parent_cwd,
        plugins: ctx.plugin_registry.as_deref(),
    ...
    match sentinel_ai_subagent_resolution::validate_agent_name(subagent_type, &context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(sentinel_ai_subagent_resolution::ResolutionError::Unknown { available, .. }) => {
            SubagentValidateTypeOutcome::Unknown { available }
        }
        Err(sentinel_ai_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(sentinel_ai_subagent_resolution::ResolutionError::NotAllowed { allowed, .. }) => {
            SubagentValidateTypeOutcome::NotAllowed { allowed }
        }
        Err(
            sentinel_ai_subagent_resolution::ResolutionError::PersonaResolution(_)
            | sentinel_ai_subagent_resolution::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match sentinel_ai_subagent_resolution::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(sentinel_ai_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        ...
        Err(
            sentinel_ai_subagent_resolution::ResolutionError::Unknown { .. }
            | sentinel_ai_subagent_resolution::ResolutionError::PersonaResolution(_)
            | sentinel_ai_subagent_resolution::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    sentinel_ai_subagent_resolution::subagent_harness_flavor_is_representable(agent_type)
}
/// Apply the harness-dependent toolset/prompt re-selection to a resolved
/// agent definition.
///
/// The harness flavor (alternate vs ai-build) normally follows the PARENT
/// agent: `AiBuildOrchestrator` parents give children
...
/// (so a ai-build session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut sentinel_ai_agent::config::AgentDefinition,
) {
    let resolution_context = sentinel_ai_subagent_resolution::HarnessToolsetContext {
    ...
    sentinel_ai_subagent_resolution::apply_harness_toolset(
        subagent_type,
        &resolution_context,
        definition,
    );
...
fn summarize_tool_config(
    config: &sentinel_ai_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_permission_mode(
    requested: sentinel_ai_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> sentinel_ai_agent::config::PermissionMode {
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    sentinel_ai_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
fn cancellation_error_message(
    category: Option<sentinel_file_utils::events::types::CancellationCategory>,
...
fn inject_subagent_completed_prompt(
    ...
    task_completion_reservations: &Option<
        sentinel_ai_tools::reminders::task_completion::TaskCompletionReservations,
    >,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
    ...
    let summary =
        sentinel_ai_tools::implementations::ai_build::task::completion_summary(request, result);
    let message = sentinel_ai_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = sentinel_ai_tools::reminders::wrap_reminder(&message);
...
fn telemetry_owner_kind(
    request: &SubagentRequest,
) -> sentinel_ai_telemetry::events::SubagentOwnerKind {
    if request.owner.is_workflow() {
        sentinel_ai_telemetry::events::SubagentOwnerKind::Workflow
    } else if request.from_scheduler_loop() {
        sentinel_ai_telemetry::events::SubagentOwnerKind::SchedulerLoop
    } else {
        sentinel_ai_telemetry::events::SubagentOwnerKind::Task
    }
}
...
fn emit_subagent_notification(
    ...
    if let Some(cmd_tx) = parent_cmd_tx {
        let _ = cmd_tx.send(SessionCommand::AiSessionNotification {
    ...
    tokio::task::spawn_local(async move {
        ...
        loop {
            ...
            if let Some(ref cmd_tx) = parent_cmd_tx {
                let _ = cmd_tx.send(SessionCommand::AiSessionNotification { notification });
            }
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("x.ai/session_notification", params.into());
...
pub(crate) async fn reconcile_orphaned_subagents_with_backend(
    ...
    backend: &sentinel_ai_tools::implementations::ai_build::task::backend::ChannelBackend,
    session_dir: &Path,
    parent_session_id: &str,
    gateway: &GatewaySender,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
