...
use sentinel_ai_tools::implementations::ai_build::task::backend::ChannelBackend;
#[test]
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = sentinel_ai_subagent_resolution::context::normalize_forked_context(
        items,
    );
    if let ConversationItem::User(u) = &conv[1] {
        let text = u
            ...
            .filter_map(|p| match p {
                sentinel_ai_sampling_types::conversation::ContentPart::Text { text } => {
                    Some(text.as_ref())
                }
                _ => None,
            })
...
fn normalize_forked_context_consecutive_users() {
    ...
    let (conv, prefix_len) = sentinel_ai_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = sentinel_ai_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = sentinel_ai_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
        ...
        .find_map(|item| {
            if let ConversationItem::User(u) = item {
                let text: String = u
                    ...
                    .iter()
                    .filter_map(|p| match p {
                        sentinel_ai_sampling_types::conversation::ContentPart::Text {
                            text,
                        } => Some(text.as_ref()),
                        _ => None,
                    })
            ...
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                    p,
                    sentinel_ai_sampling_types::conversation::ContentPart::Text { text } if text.contains("<background_context>")
                )
                    })
            } else {
                false
...
fn snapshot_ref_field_in_meta_roundtrips() {
    let meta = SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/ai-wt/sa-snap".into()),
        snapshot_ref: Some("refs/ai/subagent-snapshots/sa-snap".into()),
        effective_model_id: None,
    };
    let json = serde_json::to_string(&meta).unwrap();
    assert!(json.contains("snapshot_ref"));
    assert!(json.contains("refs/ai/subagent-snapshots/sa-snap"));
    let parsed: SubagentMeta = serde_json::from_str(&json).unwrap();
    assert_eq!(
            parsed.snapshot_ref.as_deref(),
            Some("refs/ai/subagent-snapshots/sa-snap")
        );
}
#[test]
...
fn snapshot_test_meta(id: &str) -> SubagentMeta {
    SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/ai-wt/subagent-x".into()),
        snapshot_ref: None,
        effective_model_id: None,
    }
}
...
async fn completion_snapshot_sequence_persists_ref_then_removes_worktree() {
    sentinel_test_utils::require_git!();
    use sentinel_test_utils::git::{git_commit_all, init_git_repo};
    ...
    sentinel_fast_worktree::WorktreeBuilder::new(&repo, &wt)
        .standalone(true)
        .create()
        .unwrap();
    ...
    let ref_name = "refs/ai/subagents/glue-1";
...
fn subagent_session_metadata_roundtrip() {
    ...
    assert_eq!(session_meta.model_id.as_deref(), Some("ai-4.5"));
...
fn upload_lifecycle_spawn_then_completion_preserves_fields() {
    ...
    assert_eq!(spawn_gcs.model_id.as_deref(), Some("ai-4.5"));
    ...
    assert_eq!(completion_gcs.model_id.as_deref(), Some("ai-4.5"));
...
fn resume_source_worktree_reuse() {
    let source_with_worktree = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.ai/worktrees/myrepo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
    ...
    assert_eq!(
            worktree.as_deref(),
            Some(Path::new(
                "/home/user/.ai/worktrees/myrepo/subagent-sub-wt",
            )),
...
fn resume_inherited_cwd_requires_existing_non_worktree_dir() {
    ...
    let missing = ResumeSourceData {
        child_cwd: "/no/such/dir/ai-missing".into(),
        ..present.clone()
    };
...
fn select_override_cwd_resume_never_falls_through_to_request_cwd() {
    let source = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.ai/worktrees/repo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
...
fn token_estimation_for_window_safety() {
    ...
    let estimated = sentinel_chat_state::estimate_conversation_tokens(&conversation);
    assert!(estimated > 0, "should produce non-zero estimate");
    assert!(
            estimated < 100,
            "short conversation should have small token estimate"
        );
    assert_eq!(sentinel_chat_state::estimate_conversation_tokens(&[]), 0);
}
#[test]
fn token_estimation_accounts_for_images() {
    use sentinel_ai_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
    ...
    let text_tokens = sentinel_chat_state::estimate_conversation_tokens(&text_only);
    ...
    let image_tokens = sentinel_chat_state::estimate_conversation_tokens(&with_image);
    assert_eq!(
            image_tokens,
            text_tokens + 765,
...
fn durable_fallback_roundtrips_child_cwd_and_worktree() {
    ...
    let meta = SubagentMeta {
        ...
        worktree_path: Some("/tmp/ai-wt/sa-dur".into()),
        snapshot_ref: None,
        effective_model_id: Some("ai-3".into()),
    };
    ...
    assert_eq!(loaded.worktree_path.as_deref(), Some("/tmp/ai-wt/sa-dur"));
...
fn drain_cancelled_finish_cmds(
    ...
    while let Ok(cmd) = cmd_rx.try_recv() {
        if let SessionCommand::AiSessionNotification { notification } = cmd
...
async fn reconcile_reemits_shared_actor_terminal_outcome() {
    ...
    let finish = std::iter::from_fn(|| cmd_rx.try_recv().ok())
        .find_map(|command| {
            let SessionCommand::AiSessionNotification { notification } = command else {
                return None;
            };
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
            sentinel_ai_subagent_resolution::validate_resume_identity(
                "general-purpose",
                None,
                &source,
            )
...
async fn read_parent_sampling_config_keeps_auto_catalog_id_with_routing_slug() {
    ...
    models.insert("auto".to_string(), test_model_entry("ai-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "ai-4.5", "composer-2-fast", models);
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    assert_eq!(config.model, "ai-4.5");
    assert_eq!(model_id.0.as_ref(), "auto");
}
#[tokio::test]
async fn read_parent_sampling_config_keeps_auto_when_catalog_has_slug_key_only() {
    let mut models = indexmap::IndexMap::new();
    models.insert("ai-4.5".to_string(), test_model_entry("ai-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "ai-4.5", "auto", models);
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    assert_eq!(config.model, "ai-4.5");
    assert_eq!(model_id.0.as_ref(), "auto");
}
...
async fn read_parent_sampling_config_fallback_wires_bearer_resolver() {
    ...
    ctx.sampling_config.model = "ai-4.5".to_string();
...
async fn read_parent_sampling_config_live_never_strips_a_fallback_key() {
    ...
    ctx.auth = None;
    let chat = spawn_test_parent_chat_state("ai-4.5");
    chat.update_credentials(sentinel_chat_state::Credentials {
        api_key: Some("sentinel-env-fallback".to_string()),
        auth_type: sentinel_chat_state::AuthType::SessionToken,
        alpha_test_key: None,
        client_version: None,
    });
    ...
    assert_eq!(config.api_key.as_deref(), Some("sentinel-env-fallback"));
}
/// `would_strip_fallback_key` on the inherit-fallback path: the baseline
/// keeps the env `AI_API_KEY` even while `auth_type` flips to
...
async fn read_parent_sampling_config_fallback_never_strips_a_fallback_key() {
    ...
    ctx.sampling_config.model = "ai-4.5".to_string();
    ctx.sampling_config.base_url = "https://api.x.ai/v1".to_string();
    ctx.sampling_config.api_key = Some("sentinel-env-fallback".to_string());
    let (config, _) = read_parent_sampling_config(&ctx).await;
...
fn resolve_model_override_wires_resolver_for_fresh_and_hard_expired_session_keys() {
    for auth in [
        ...
        ctx.available_models
            .insert("ai-4.5".to_string(), test_model_entry("ai-4.5"));
        let (config, _) = resolve_model_override_to_config("ai-4.5", &ctx).unwrap();
        assert!(config.bearer_resolver.is_some(), "key={key}");
    }
}
/// `would_strip_fallback_key` on the override path. `AI_API_KEY`'s
...
fn resolve_model_override_to_config_never_strips_a_fallback_key() {
    ...
    ctx.auth = None;
    ctx.available_models.insert("ai-4.5".to_string(), test_model_entry("ai-4.5"));
    let (config, _) = resolve_model_override_to_config("ai-4.5", &ctx).unwrap();
    assert_eq!(
...
async fn read_parent_sampling_config_resolves_backend_search_from_catalog() {
    let mut entry = test_model_entry("ai-4.5");
    entry.info.supports_backend_search = true;
    let mut models = indexmap::IndexMap::new();
    models.insert("auto".to_string(), entry);
    let mut ctx = ctx_with_parent_chat_state("auto", "ai-4.5", "auto", models);
...
async fn runtime_override_wins_over_subagents_models_pin_in_precedence_path() {
    use sentinel_ai_agent::config::ModelOverride;
    let build_ctx = || {
        let mut models = indexmap::IndexMap::new();
...
async fn resolve_subagent_inherits_parent_model_without_pins() {
    use sentinel_ai_agent::config::ModelOverride;
    for parent_model in ["ai-4.5", "composer-2-fast", "my-custom-byok-model"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
...
/// (`ai-4.5`) and a custom parent (`composer-2-fast`)
...
async fn resolve_subagent_config_override_pin_applies_for_any_parent() {
    use sentinel_ai_agent::config::ModelOverride;
    for parent_model in ["ai-4.5", "composer-2-fast"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
        ctx.sampling_config.model = parent_model.to_string();
        ctx.model_id = acp::ModelId::new(parent_model);
        ctx.available_models
...
async fn resolve_subagent_agent_definition_pin_applies_for_light_parent() {
    ...
    let mut ctx = ctx_with_toggle(HashMap::new());
    ctx.sampling_config.model = "ai-4.5".to_string();
    ctx.model_id = acp::ModelId::new("ai-4.5");
    ctx.available_models
        .insert("pinned-model".to_string(), test_model_entry("pinned-model"));
...
async fn resolve_subagent_config_override_unknown_model_falls_through_to_inherit() {
    ...
    let (config, model_id) = resolve_subagent_sampling_config(
        ...
        .await;
    assert_eq!(config.model, "ai-4.5");
    assert_eq!(model_id.0.as_ref(), "ai-4.5");
}
...
async fn subagent_override_provider_model_spawns_cache_only_credentials() {
    ...
    ctx.model_id = acp::ModelId::new("ai-4.5");
    ctx.available_models = models;
    ctx.auth = Some(crate::auth::AiAuth {
...
fn non_cursor_persona_injected_as_system_reminder() {
    use sentinel_ai_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &sentinel_ai_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &sentinel_ai_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &sentinel_ai_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &sentinel_ai_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &sentinel_ai_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &sentinel_ai_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &sentinel_ai_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(&result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &sentinel_ai_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn resolve_inherited_pool_named_filters() {
    ...
    let result = super::resolve_inherited_mcp_pool(
            Some(pool),
            &sentinel_ai_agent::config::McpInheritance::Named(vec!["atlassian".into()]),
        )
...
fn make_test_skill(
    ...
    sentinel_ai_tools::implementations::skills::types::SkillInfo {
        ...
        scope: sentinel_ai_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
    ...
    local
        .run_until(async {
            ...
            let cmd = tokio::time::timeout(
                ...
                .expect("channel open");
            cancel.cancel();
            let SessionCommand::AiSessionNotification { notification } = cmd else {
                panic!("expected AiSessionNotification");
            };
