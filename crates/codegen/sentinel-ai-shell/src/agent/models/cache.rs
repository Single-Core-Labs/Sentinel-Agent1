...
pub(crate) struct ModelsCache {
    ...
    pub(crate) ai_version: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl ModelsCacheManager {
    pub(crate) fn new() -> Self {
        Self {
            path: crate::util::ai_home::ai_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    pub(crate) fn load_fresh(
        ...
        if cache.ai_version.as_deref() != Some(sentinel_ai_version::VERSION) {
    ...
    /// Per-writer temp path: `~/.ai` is shared across concurrent CLI
