...
fn test_manager() -> ModelsManager {
    let _ = tracing_subscriber::fmt()
        ...
        .with_test_writer()
        .try_init();
    let tmp = std::env::temp_dir().join("ai-test-models-manager");
    let auth_manager = Arc::new(AuthManager::new(&tmp, AiComConfig::default()));
...
fn cold_manager(cfg: config::Config, endpoint: Arc<dyn ModelsEndpoint>) -> ModelsManager {
    ...
    let auth_manager = Arc::new(AuthManager::new(tmp.path(), AiComConfig::default()));
    ModelsManagerBuilder::new(
        None,
        IndexMap::new(),
        acp::ModelId::new("default"),
...
async fn catalog_retry_recovers_after_endpoint_returns() {
    ...
    let tmp = std::env::temp_dir().join("ai-test-catalog-retry");
    let auth_manager = Arc::new(AuthManager::new(&tmp, AiComConfig::default()));
    ...
    assert!(mgr.models().contains_key("ai-4"));
    assert!(
...
async fn disk_cache_reload_applies_without_fetching() {
    ...
    assert!(mgr.models().contains_key("ai-4.5"));
    assert!(mgr.has_fetched_real_catalog());
    assert_eq!(
...
async fn auth_refresh_watcher_refetches_on_notify() {
    ...
    let calls = Arc::new(AtomicUsize::new(0));
    let tmp = std::env::temp_dir().join("ai-test-auth-refresh-watcher");
    let auth_manager = Arc::new(AuthManager::new(&tmp, AiComConfig::default()));
    let mgr = ModelsManagerBuilder::new(
        None,
        IndexMap::new(),
...
async fn first_catalog_wait_skips_doomed_signed_out_fetch() {
    let _no_key = EnvGuard::unset("AI_API_KEY");
    let _no_legacy_key = EnvGuard::unset("AI_CODE_AI_API_KEY");
    let mgr = cold_manager(config::Config::default(), Arc::new(HangingEndpoint));
    let start = tokio::time::Instant::now();
    mgr.spawn_fetch_inner(None, /*remote_fetch_enabled*/ true);
    assert!(
...
async fn set_current_model_id_change_fires_watch_to_all_subscribers() {
    ...
    mgr.set_current_model_id(acp::ModelId::new("ai-4"));
...
async fn model_switch_generation_snapshot_reflects_current_state() {
    ...
    mgr.set_current_model_id(acp::ModelId::new("ai-4"));
    assert_eq!(mgr.model_switch_generation(), start + 1);
    mgr.set_current_model_id(acp::ModelId::new("ai-3"));
...
fn first_catalog_reselect_bumps_model_switch_watch() {
    ...
    mgr.apply_refresh_result(&cfg, Some(make_prefetched(&["ai-4.5", "ai-4"])), None);
    assert_eq!(mgr.current_model_id().0.as_ref(), "ai-4.5");
    assert!(
...
fn reselect_missing_current_model_bumps_watch() {
    let mgr = test_manager();
    let cfg = config::Config::default();
    mgr.apply_refresh_result(&cfg, Some(make_prefetched(&["ai-4", "ai-3"])), None);
    mgr.set_current_model_id(acp::ModelId::new("ai-4"));
    let start = mgr.model_switch_generation();
    // A later catalog drops the current model → reselect_current_model_if_missing.
    mgr.apply_refresh_result(&cfg, Some(make_prefetched(&["ai-3"])), None);
...
fn current_reasoning_effort_seeded_from_config() {
    let tmp = std::env::temp_dir().join("ai-test-models-manager-seed");
    let auth_manager = Arc::new(AuthManager::new(&tmp, AiComConfig::default()));
...
fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
    use indexmap::IndexMap;
    use sentinel_ai_sampling_types::ReasoningEffortOption;
    ...
    let mut no_none = ModelEntry {
        info: config::ModelInfo::fallback("ai-4.5"),
        api_key: None,
        env_key: None,
        auth_provider: None,
        api_base_url: None,
    ...
    prefetched.insert("ai-4.5".to_string(), no_none);
    ...
    let catalog = resolve_model_catalog(&cfg, Some(prefetched));
    assert_eq!(
        catalog["ai-4.5"].info.reasoning_effort,
        Some(ReasoningEffort::High),
...
fn config_menu_only_model_derives_support_and_default() {
    ...
    let tmp = std::env::temp_dir().join("ai-test-models-manager-menu-only");
    let auth_manager = Arc::new(AuthManager::new(&tmp, AiComConfig::default()));
    let mgr = ModelsManager::new(
        None,
        catalog,
...
async fn spawn_background_refresh_never_blocks_on_a_hanging_endpoint() {
    ...
    let mgr = ModelsManagerBuilder::new(
        None,
        make_prefetched(&["ai-4", "ai-4.5"]),
        acp::ModelId::new("ai-4.5"),
        auth_manager,
        config_from_toml("[models]\ndefault = \"ai-4.5\""),
    )
...
fn first_apply_refresh_reselects_default_model() {
    let mgr = test_manager();
    let mut cfg = config::Config::default();
    cfg.models.default = Some("ai-3".to_string());

    assert!(!mgr.has_fetched_real_catalog());

    let prefetched = make_prefetched(&["ai-3", "ai-4"]);
    mgr.apply_refresh_result(&cfg, Some(prefetched), None);

    assert!(mgr.has_fetched_real_catalog());
    assert_eq!(mgr.current_model_id().0.as_ref(), "ai-3");
}
...
fn subsequent_refresh_reselects_when_model_removed() {
    ...
    let prefetched = make_prefetched(&["ai-3", "ai-4.5"]);
    mgr.apply_refresh_result(&cfg, Some(prefetched), None);
...
fn apply_config_honors_new_preferred_model() {
    ...
    *mgr.inner.cfg.write() = stale_cfg;
    ...
    new_cfg.models.default = Some("ai-3".to_string());
...
fn apply_config_falls_back_when_preferred_not_in_catalog() {
    ...
    let mut new_cfg = config::Config::default();
    new_cfg.models.default = Some("ai-nonexistent".to_string());
    mgr.apply_config(new_cfg);
...
fn auth_refresh_then_config_reload_preserves_user_model() {
    ...
    let prefetched = make_prefetched(&["ai-3", "ai-4"]);
    mgr.apply_refresh_result(&cfg, Some(prefetched), None);
    assert_eq!(mgr.current_model_id().0.as_ref(), "ai-4");

    let mut new_cfg = config::Config::default();
    new_cfg.models.default = Some("ai-4".to_string());
    mgr.apply_config(new_cfg);
    assert_eq!(mgr.current_model_id().0.as_ref(), "ai-4");
}
...
fn reload_from_disk_cache_applies_external_catalog() {
    ...
    assert!(mgr.has_fetched_real_catalog());
    assert!(mgr.models().contains_key("ai-4.5"));
    assert!(mgr.models().contains_key("ai-4.3"));
...
fn reload_from_disk_cache_ignores_stale_cache() {
    ...
    let stale = ModelsCache {
        ...
        ai_version: Some(sentinel_ai_version::VERSION.to_string()),
        auth_method: Some(auth_method),
        origin: Some(mgr.cache_origin()),
        etag: Some("etag-stale".into()),
        models: make_prefetched(&["ai-stale"]),
    };
    cache.atomic_write(&stale);

    mgr.reload_from_cache_manager(&cache);

    assert!(!mgr.models().contains_key("ai-stale"));
...
fn reload_from_disk_cache_ignores_auth_method_mismatch() {
    ...
    assert!(!mgr.models().contains_key("ai-other-auth"));
}
...
fn reload_from_disk_cache_ignores_origin_mismatch() {
    ...
    let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
    cache.persist(
        &make_prefetched(&["ai-other-origin"]),
        Some("etag-y"),
        auth_method,
    ...
    assert!(!mgr.models().contains_key("ai-other-origin"));
    assert!(mgr.inner.catalog.read().etag.is_none());
}
...
fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
    ...
    let legacy = ModelsCache {
        ...
        models: make_prefetched(&["ai-legacy"]),
    };
    cache.atomic_write(&legacy);

    mgr.reload_from_cache_manager(&cache);

    assert!(!mgr.models().contains_key("ai-legacy"));
}
...
fn clear_resets_has_fetched_real_catalog() {
    ...
    let prefetched = make_prefetched(&["ai-4.5", "ai-4.3"]);
    mgr.apply_refresh_result(&cfg, Some(prefetched), None);
    let first_available = mgr.available().keys().next().unwrap().clone();
    assert_eq!(
        mgr.current_model_id().0.as_ref(),
...
fn resolve_custom_endpoint_always_wins() {
    let _key = EnvGuard::set("AI_API_KEY", "test-key");
...
fn resolve_falls_back_to_session_when_nothing_set() {
    let _unset = EnvGuard::unset("AI_API_KEY");
    let _unset_legacy = EnvGuard::unset("AI_CODE_AI_API_KEY");
    let endpoints = config::EndpointsConfig::default();
    assert_eq!(
        ModelFetchAuth::resolve(&endpoints, false),
        ModelFetchAuth::Session,
...
fn resolve_deployment_key_outranks_ambient_api_key() {
    let _key = EnvGuard::set("AI_API_KEY", "stray-env-key");
    let endpoints = config::EndpointsConfig {
        deployment_key: Some("deploy-key".to_owned()),
        ..config::EndpointsConfig::default()
    };
    assert_eq!(
        ModelFetchAuth::resolve(&endpoints, false),
        ModelFetchAuth::Deployment,
        "managed deployment_key should outrank an ambient AI_API_KEY",
    );
    assert_eq!(
        ModelFetchAuth::resolve(&endpoints, true),
        ModelFetchAuth::Session,
...
fn prefetch_env_none_when_remote_fetch_disabled_despite_credentials() {
    ...
    assert!(
        resolve_prefetch_env_from_parts(Some(AiAuth::test_default()), endpoints.clone(), false,)
            .is_none(),
...
fn build_prefetched_map_distinct_ids_same_slug() {
    let entries = vec![
        make_entry_config_with_id(Some("auto"), "ai-build", Some("Auto")),
        make_entry_config_with_id(Some("ai-build"), "ai-build", Some("Ai Build")),
    ...
    assert!(map.contains_key("ai-build"));
    assert!(map.contains_key("experimental-fast"));
    assert_eq!(
        map["auto"].info.model, "ai-build",
        "auto entry should still route to ai-build"
    );
    assert_eq!(map["ai-build"].info.model, "ai-build");
}
...
fn build_prefetched_map_duplicate_id_overwrites() {
    let entries = vec![
        make_entry_config_with_id(Some("ai-build"), "ai-build", Some("First")),
        make_entry_config_with_id(Some("ai-build"), "ai-build", Some("Second")),
    ];
    let map = build_prefetched_map(entries, None);

    assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
    assert_eq!(map["ai-build"].info.name.as_deref(), Some("Second"));
}
...
fn resolve_default_model_prefers_id_over_model_slug() {
    ...
    catalog.insert(
        "auto-ai-build".to_string(),
        make_model_entry("ai-build"),
    );
    catalog.insert("ai-build".to_string(), make_model_entry("ai-build"));

    let mut cfg = config::Config::default();
    cfg.models.default = Some("ai-build".to_string());

    let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
    assert_eq!(key, "ai-build", "must match id, not first slug hit");
}
...
fn resolve_catalog_key_maps_routing_slug_to_config_key() {
    ...
    models.insert(
        "enterprise-ai-build".to_string(),
        make_model_entry("ai-4.5"),
    );
    models.insert("ai-4.3".to_string(), make_model_entry("ai-4.3"));

    let persisted = acp::ModelId::new("ai-4.5");
    let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
    assert_eq!(key.0.as_ref(), "enterprise-ai-build");
}

#[test]
fn resolve_catalog_key_prefers_exact_key_match() {
    let mut models = IndexMap::new();
    models.insert("ai-4.5".to_string(), make_model_entry("ai-4.5"));

    let persisted = acp::ModelId::new("ai-4.5");
    let key = resolve_catalog_key(&models, &persisted).expect("exact key must resolve");
    assert_eq!(key.0.as_ref(), "ai-4.5");
}
...
fn resolve_catalog_key_last_slug_match_wins() {
    let mut models = IndexMap::new();
    models.insert(
        "default-ai-build".to_string(),
        make_model_entry("ai-4.5"),
    );
    models.insert("user-ai-build".to_string(), make_model_entry("ai-4.5"));

    let persisted = acp::ModelId::new("ai-4.5");
    let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
    assert_eq!(key.0.as_ref(), "user-ai-build");
}
...
fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
    let mut models = IndexMap::new();
    models.insert("ai-build".to_string(), make_model_entry("ai-build"));
    models.insert(
        "enterprise-ai-build".to_string(),
        make_model_entry("ai-build"),
    );
    ...
    let available = test_available_keys(&["enterprise-ai-build", "ai-4.3"]);

    let persisted = acp::ModelId::new("ai-build");
    assert_eq!(
...
fn selectable_prefers_exact_key_over_later_slug_match() {
    let mut models = IndexMap::new();
    models.insert("ai-build".to_string(), make_model_entry("ai-4.5"));
    models.insert("other".to_string(), make_model_entry("ai-build"));

    let available = test_available_keys(&["ai-build", "other"]);

    let persisted = acp::ModelId::new("ai-build");
    let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
        .expect("exact selectable key must win");
    assert_eq!(key.0.as_ref(), "ai-build");
}
...
async fn bounded_auth_refresh_times_out_to_none() {
    ...
    let result =
        ModelsManager::bounded_auth_refresh(std::future::pending::<Option<AiAuth>>()).await;
...
async fn bounded_auth_refresh_passes_through_ready_value() {
    let result =
        ModelsManager::bounded_auth_refresh(async { Some(AiAuth::test_default()) }).await;
    assert!(
        result.is_some(),
