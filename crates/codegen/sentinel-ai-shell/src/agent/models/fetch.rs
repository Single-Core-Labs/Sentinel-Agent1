    ...
    match fetch_models_blocking(endpoints, auth, fetch_auth) {
        Ok(FetchModelsResult { models, etag }) if !models.is_empty() => {
            let api_base_url_override = match fetch_auth {
                ModelFetchAuth::ApiKey => Some(endpoints.sentinel_api_base_url.clone()),
                _ => None,
            };
...
pub fn start_early_prefetch_with_auth(auth: Option<AiAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(ai_com_config: Option<AiComConfig>) -> Option<EarlyPrefetchHandle> {
    start_early_prefetch_impl(ai_com_config, true)
}
...
pub fn start_early_prefetch_settings_only(
    ai_com_config: Option<AiComConfig>,
) -> Option<EarlyPrefetchHandle> {
    start_early_prefetch_impl(ai_com_config, false)
}
...
fn start_early_prefetch_impl(
    ...
    let auth = AuthManager::new(&ai_home, ai_com_config.unwrap_or_default()).current();
