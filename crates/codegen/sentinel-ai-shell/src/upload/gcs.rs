...
//! `StorageClient` constructed inside `sentinel_file_utils::gcs::*` helpers.
//!
...
//! to an `sentinel_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `sentinel_file_utils::gcs::*` helpers wire
...
///
/// ```ignore
/// sentinel_file_utils::gcs::upload_bytes(
...
/// `AI_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("AI_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match sentinel_file_utils::gcs::upload_bytes(
