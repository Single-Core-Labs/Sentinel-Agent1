...
/// Parse stdout into a session-credential `AiAuth`.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<AiAuth> {
...
pub(crate) async fn run_external_refresh(command: &str) -> Option<AiAuth> {
...
pub(crate) async fn refresh_with_command(command: &str, prev_auth: &AiAuth) -> Option<AiAuth> {
...
mod tests {
    ...
    #[test]
    fn parse_output_issuer_claim_enables_sentinel_auth() {
    ...
    #[tokio::test]
    async fn sets_ai_auth_expired_env_on_refresh() {
        let auth = run_external_refresh("echo $AI_AUTH_EXPIRED")
            .await
            .unwrap();
