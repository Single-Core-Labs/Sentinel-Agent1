...
use super::super::config::{ForceLoginTeam, AiComConfig, OAuth2ProviderConfig, OidcAuthConfig};
use super::super::{AuthMode, AiAuth};
use base64::Engine;
...
pub(crate) fn is_configured(config: &AiComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &AiComConfig) -> Option<ForceLoginTeam> {
...
async fn refresh_tokens_once(
    ...
    if !resp.status().is_success() {
        ...
        tracing::warn!(
            ...
            rt_prefix = sentinel_ai_auth::bearer_suffix(refresh_token),
            client_id = %client_id,
            principal_type = ?principal_type,
...
mod tests {
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "ai-cli:access".into()],
            audience: None,
        };
        ...
        assert!(url.contains("referrer=ai-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        assert!(url.contains("referrer=ai-desktop"));
        assert!(!url.contains("referrer=ai-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access ai-cli:access api:access",
            "principal_type": "Team",
