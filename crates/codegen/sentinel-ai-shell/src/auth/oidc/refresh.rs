...
use super::super::AiAuth;
use super::protocol::{OidcError, OidcUserInfo, build_ai_auth, discover, refresh_tokens};
...
pub(crate) async fn oidc_token_exchange(auth: &AiAuth) -> OidcRefreshResult {
                ...
                && let Some(reason) = classify_terminal(&error_code)
            {
                ...
                crate::unified_log::error(
                    ...
                    Some(serde_json::json!({
                        ...
                        "client_id": client_id,
                        "tried_rt_prefix": auth.refresh_token.as_deref().map(sentinel_ai_auth::bearer_suffix),
    ...
    let mut new_auth = build_ai_auth(tokens, user_info, issuer, client_id);
