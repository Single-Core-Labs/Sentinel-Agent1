...
use super::super::config::{AiComConfig, OidcAuthConfig};
use super::super::{AuthManager, AiAuth};
use super::protocol::{
    OidcError, build_authorize_url, build_ai_auth, discover, enforce_login_principal,
...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => (
            "Signed in",
            "You can close this window and return to Ai Build.",
        ),
...
pub async fn run_login_flow_with_config(
    ...
    let oauth2 = auth_manager.ai_com_config().oauth2.as_ref();
    ...
    if has_client_ui {
        ...
        eprintln!();
        let provider_label = if oidc.issuer == super::super::config::AI_OAUTH2_ISSUER {
            "Ai".to_owned()
        } else {
            oidc.issuer.clone()
        };
    ...
    let principal_policy = login_principal_policy(auth_manager.ai_com_config());
    ...
    let mut auth = build_ai_auth(tokens, user_info, &oidc.issuer, &oidc.client_id);
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        let auth_manager = Arc::new(
            AuthManager::new(temp_dir.path(), AiComConfig::default())
        ...
        .await
        .unwrap();
        let auth = build_ai_auth(tokens, user_info, &oidc_cfg.issuer, &oidc_cfg.client_id);
