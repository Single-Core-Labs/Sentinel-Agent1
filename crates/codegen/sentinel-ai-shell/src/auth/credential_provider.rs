...
use crate::util::ai_auth_credentials::AiAuthCredentials;
use reqwest::RequestBuilder;
use std::sync::Arc;
use sentinel_ai_auth::{
...
fn api_key_id_for(auth: Option<&crate::auth::AiAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| sentinel_ai_telemetry::config::deployment_id_from_key(&a.key))
}
/// Sampler [`BearerResolver`](sentinel_ai_sampler::BearerResolver) over a live
...
impl WireValidBearerResolver {
    ...
    pub(crate) fn shared(auth_manager: Arc<AuthManager>) -> sentinel_ai_sampler::SharedBearerResolver {
...
impl sentinel_ai_sampler::BearerResolver for WireValidBearerResolver {
...
pub(crate) struct ShellAuthCredentialProvider {
    ...
    static_credentials: AiAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = AiAuthCredentials::new(None);
...
/// Resolves the embedding credentials for `embed_base_url`, attaching the AI
/// session credential only to AI-operated endpoints over `https`.
pub(crate) fn embedding_session_credentials(
    embed_base_url: &str,
    auth_manager: Option<&Arc<AuthManager>>,
    api_key_provider: Option<sentinel_ai_tools::types::SharedApiKeyProvider>,
) -> sentinel_ai_memory::EndpointScopedCredentials {
    ...
    sentinel_ai_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
        crate::util::is_sentinel_api_bearer_url,
        auth_credentials,
        api_key_provider,
    )
}
...
///
/// - `"ai-shell"`   — classic Ai CLI / TUI (sentinel-ai-shell)
/// - `"ai-pager"`   — new Ai Pager / TUI (sentinel-ai-pager)
/// - `"ai-desktop"` — Ai Desktop app
/// - `"ai-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-ai-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-ai-client-identifier`
///
/// to the underlying `sentinel-file-utils::StorageClient` via
...
///   - `user_token` (normal OIDC users via com scope) → sends Bearer + `X-AI-Token-Auth`
...
pub fn build_storage_client_for_proxy(
...
) -> sentinel_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn sentinel_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        sentinel_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(sentinel_ai_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = AiAuthCredentials::new(user_token);
...
/// Bridge that lets `StorageClient` (which lives in sentinel-file-utils)
...
impl sentinel_file_utils::storage_client::Auth401AttributionCallback for StorageClientAttributionBridge {
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = AiAuthCredentials::new(None);
...
pub(crate) fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        sentinel_ai_telemetry::external::set_identity(
            sentinel_ai_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `sentinel-ai-pager` and `sentinel-ai-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> sentinel_ai_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let ai_com_config = crate::auth::AiComConfig::default();
    let exporter = sentinel_ai_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = ai_com_config.token_header.clone();
    let ai_home = crate::util::ai_home::ai_home();
    let bootstrap = Arc::new(AuthManager::new(&ai_home, ai_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    sentinel_ai_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("AI_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("AI_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => unsafe { std::env::set_var("AI_AUTH_EARLY_INVALIDATION_SECS", prev) },
                    None => unsafe { std::env::remove_var("AI_AUTH_EARLY_INVALIDATION_SECS") },
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> AiAuth {
        AiAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<AiAuth>) -> Arc<AuthManager> {
    ...
    /// pinned in sentinel-ai-sampler's resolver tests): over a real
    ...
    fn wire_valid_resolver_tracks_manager_across_expiry_and_refresh() {
        use sentinel_ai_sampler::BearerResolver;
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::AiComConfig::default(),
        ));
        mgr.hot_swap(AiAuth {
            key: "stale".into(),
        ...
        impl crate::auth::refresh::TokenRefresher for OkRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(AiAuth {
                    key: "fresh".into(),
                    auth_mode: crate::auth::AuthMode::Oidc,
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let mgr = make_manager(
            &dir,
            Some(make_auth("sentinel-session-token", ChronoDuration::hours(1))),
        );
        let api_key_provider: sentinel_ai_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use sentinel_ai_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let dep = ShellAuthCredentialProvider::new(
            make_manager(&dir, None),
            Some("sentinel-token-EX".into()),
            None,
        )
        .snapshot();
        assert_eq!(
            dep.deployment_id.as_deref(),
            Some(deployment_id_from_key("sentinel-token-EX").as_str())
        );
    ...
    fn otel_bootstrap_snapshot_picks_up_disk_writes() {
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let scope = crate::auth::AiComConfig::default().auth_scope();
    ...
    /// user token, matching the precedence in `AiAuthCredentials::apply`.
