...
pub(crate) trait ExternalCommandRunner: Send + Sync {
    ...
    async fn run_external_command(&self, command: &str) -> Option<AiAuth>;
}
...
impl RefreshOutcome {
    ...
    pub(crate) fn success(auth: AiAuth) -> Self {
    ...
    /// flow). Any refresher holding the [`AiAuth`] it sent must use
    ...
    ///
    /// Prefer this wherever the attempted [`AiAuth`] is in hand: it captures
...
mod tests {
    use super::*;
    use crate::auth::{AuthMode, AiAuth, AiComConfig};
