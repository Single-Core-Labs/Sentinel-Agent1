...
fn write_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &AiAuth) {
...
async fn oidc_refresher_e2e_full_refresh_cycle() {
    ...
    let mgr = Arc::new(
        AuthManager::new(dir.path(), AiComConfig::default()).with_proxy_base_url(&base_url),
    );
...
fn expired_oidc_for(base_url: &str) -> AiAuth {
    AiAuth {
        key: "old-expired-token".into(),
        create_time: Utc::now() - Duration::hours(2),
        user_id: "user-42".into(),
...
impl AuthSnapshot for EmptySnapshot {
    fn current(&self) -> Option<AiAuth> {
        None
    }
    fn expired_auth(&self) -> Option<AiAuth> {
        None
    }
    fn read_disk_auth(&self) -> Option<AiAuth> {
        None
    }
...
/// produce a `PermanentFailure` ("Run `ai login`") verdict, because they
