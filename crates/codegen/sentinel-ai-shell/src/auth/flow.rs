...
use crate::auth::{AuthManager, AiAuth, AiComConfig, parse_output};
use crate::http::TransportFailureKind;
use crate::util::ai_home;
use sentinel_ai_telemetry::events::{LoginFailed, LoginFailureKind};
...
/// session.
fn is_cached_credential_compatible(auth: &AiAuth, ai_com_config: &AiComConfig) -> bool {
    let expected_issuer = ai_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| ai_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(ai_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("AI_LOGIN_DEVICE_FLOW")
...
/// AI OAuth2 provider supports it. Enterprise OIDC (`oidc=Some`) always uses
...
/// Whether interactive AI OAuth2 login uses the RFC 8628 device flow (vs loopback).
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `AI_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `ai_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("AI_LOGIN_DEVICE_FLOW");
...
/// Sets no `AI_AUTH_EXPIRED`: operator binaries, which live outside this
...
async fn run_external_auth_provider(
    ...
    // Proper fix: pgid-kill via sentinel-tty-utils.
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.ai_com_config());
...
async fn run_auth_flow_inner(
    ...
    {
        sentinel_ai_telemetry::session_ctx::log_event(event);
    }
    result
}
...
async fn run_auth_flow_steps(
    ...
    tracing::info!(
        has_oidc = ai_com_config.oidc.is_some(),
        has_oauth2 = ai_com_config.oauth2.is_some(),
        has_external_auth = ai_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, ai_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, ai_com_config)
        }) {
            sentinel_ai_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = ai_com_config.auth_provider_command {
    ...
    // `ai login --devbox` uses run_devbox_login and is not gated here.
    if !ai_com_config.blocks_automatic_oidc()
    ...
    // AI OAuth2 also defaults to loopback; the device flow (robust on
    // remote/SSH where the loopback redirect can't reach the CLI) is opt-in via
    // --device-auth / AI_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(ai_com_config) {
        return crate::auth::oidc::run_login_flow(ai_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = ai_com_config.oauth2 {
    ...
    tracing::error!(
        "auth: no OAuth2 configuration available (neither enterprise OIDC nor AI OAuth2 configured)"
    );
    anyhow::bail!(
        "No OAuth2 configuration available. Run `ai login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(ai_com_config: &AiComConfig) -> Option<AiAuth> {
    try_ensure_fresh_auth_with(&build_startup_auth_manager(ai_com_config)).await
}

/// Builds and configures the startup `AuthManager`; the policy helpers below
/// take it injected so tests can substitute their own.
fn build_startup_auth_manager(ai_com_config: &AiComConfig) -> Arc<AuthManager> {
    let auth_manager = Arc::new(AuthManager::new(
        &ai_home::ai_home(),
        ai_com_config.clone(),
    ));
    // auth()'s OIDC/external refresh needs the refresher configured first.
    auth_manager.configure_refresher(ai_com_config.auth_provider_command.clone(), None);
    auth_manager
}

/// Policy: cached-valid creds, else silent refresh (no interactive login).
async fn try_ensure_fresh_auth_with(auth_manager: &Arc<AuthManager>) -> Option<AiAuth> {
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_noninteractive_auth_no_mint(
    ...
    try_noninteractive_auth_no_mint_with(&build_startup_auth_manager(ai_com_config)).await
}

/// Policy behind [`try_noninteractive_auth_no_mint`], with the `AuthManager`
/// injected for tests.
async fn try_noninteractive_auth_no_mint_with(auth_manager: &Arc<AuthManager>) -> Option<AiAuth> {
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<AiAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_sentinel_auth() && a.refresh_token.is_some())
}
...
pub(crate) async fn mint_session_noninteractive(
    ...
    let ai_com_config = auth_manager.ai_com_config();
    ...
    if let Some(cmd) = ai_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: AiAuth) -> AiAuth {
...
fn report_signed_in(auth: &AiAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_with_override(
    ...
    let ai_home = ai_home::ai_home();
    let auth_manager = Arc::new(AuthManager::new(&ai_home, ai_com_config.clone()));
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<AiAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(ai_com_config).await)
    } else {
        ensure_authenticated(ai_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `ai login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `AI_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // client, and `ai login` never boots an agent, so without this every
    ...
    sentinel_ai_telemetry::session_ctx::drain_pending(CLI_TELEMETRY_DRAIN).await;
    result
}
...
async fn run_cli_login_steps(
    ...
    // oauth2=None) always uses the loopback flow; only the AI OAuth2 provider
    // supports the device flow. Without this guard, `ai login` on an
    // enterprise-OIDC deployment would wrongly enter the device branch (which
    // requires `oauth2`) and error.
    let authenticated = if cli_should_use_device(&config.ai_com_config, login_override).await {
        if config.ai_com_config.oauth2.is_none() {
            // No OIDC and no oauth2 here, so `--oauth` can't help.
            anyhow::bail!("Sign-in is not available for this deployment. Set AI_API_KEY instead.");
        }
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.ai_com_config) {
            eprintln!(
...
/// config was actually applied; `ai setup` reports the no-config case.
async fn apply_post_login_config(authenticated: AiAuth) -> anyhow::Result<()> {
...
pub struct LogoutResult {
    ...
    /// `true` if `AI_API_KEY` / `AI_CODE_AI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        sentinel_ai_telemetry::external::set_identity(
            sentinel_ai_telemetry::external::IdentityAttrs::default(),
        );
        sentinel_ai_telemetry::external::flush();
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_sentinel_api_key_env(),
    })
}

/// `ai logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let ai_home = ai_home::ai_home();
    let auth_manager = AuthManager::new(&ai_home, config.ai_com_config.clone());
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via AI_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("AI_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::auth::config::AI_OAUTH2_ISSUER;
    use crate::env::EnvVarGuard;
    use chrono::Utc;

    /// `os_error` and the reqwest classification are covered in
    /// `sentinel-ai-http`; what's local is which `LoginFailureKind` each maps to,
    ...
    /// unrelated auth tests, so classification is covered in `sentinel-ai-http`.
    #[test]
    ...
    /// Run `f` with `AI_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("AI_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("AI_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("AI_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A ai.com first-party (x.ai-issuer) OIDC session — `is_sentinel_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> AiAuth {
        AiAuth {
            key: key.into(),
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), AiComConfig::default());
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only ai_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), AiComConfig::default()));
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        let dir = tempfile::tempdir().unwrap();
        let cfg = AiComConfig {
            auth_provider_command: Some("printf '%s' sentinel-ext-token".to_string()),
            ..AiComConfig::default()
        };
        let mgr = Arc::new(
        ...
        assert_eq!(auth.map(|a| a.key), Some("sentinel-ext-token".to_string()));
    }

    #[tokio::test]
    async fn interactive_login_carries_no_expired_flag_even_over_a_stale_credential() {
        let echo_env = "printf '%s' \"e=${AI_AUTH_EXPIRED:-unset}\"";
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), AiComConfig::default())
    ...
    async fn a_provider_written_to_the_published_contract_can_sign_in_after_an_expiry() {
        let conforming =
            r#"if [ "$AI_AUTH_EXPIRED" = "1" ]; then exit 1; else printf '%s' sso-token; fi"#;
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), AiComConfig::default())
                .with_proxy_base_url(&dead_proxy_url()),
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("AI_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("AI_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `ai login` must use loopback, not device —
        ...
        // The AI OAuth2 provider (oidc=None, oauth2=Some) does use device.
        let sentinel = AiComConfig::default();
        assert!(sentinel.oauth2.is_some() && sentinel.oidc.is_none());
        assert!(cli_should_use_device(&sentinel, LoginTransportOverride::ForceDevice).await);
    }
    ...
    fn oidc_auth(issuer: &str) -> AiAuth {
        AiAuth {
    ...
    fn pinned_cfg(team: &str) -> AiComConfig {
        AiComConfig {
    ...
    async fn run_auth_flow_uses_valid_disk_token_when_expired() {
        ...
        let valid_disk = AiAuth {
            key: "fresh-token-from-disk".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() + chrono::Duration::hours(1)),
            refresh_token: Some("new-rt".into()),
            oidc_issuer: Some(AI_OAUTH2_ISSUER.into()),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = AiComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = AiAuth {
        ...
        // AI_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into Ai: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `ai login` passes `on_stderr=None`; stderr must be inherited so
    ...
    async fn no_mint_readiness_auth_is_bounded() {
        ...
        assert!(
            result.is_none(),
            "a non-AI expired session is no first-party fallback and no mint              runs on this path, so no auth is produced"
        );
