...
//!
//! 1. [`sentinel_ai_telemetry::unified_log::warn`] for the local
//!    `~/.ai/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`sentinel_ai_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`sentinel_ai_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`sentinel_ai_sampler::SamplerConfig::attribution_callback`] at every
...
use sentinel_ai_sampler::{Auth401AttributionCallback, SamplingConsumer};
use sentinel_ai_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`sentinel_ai_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `sentinel_ai_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn sentinel_ai_tools::Auth401AttributionCallback>` for the
...
/// in `sentinel-ai-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `sentinel-ai-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.ai/logs/unified.jsonl) + scrubbed
...
fn compute_attribution_payload(
    ...
    // `ai_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    use crate::auth::{AuthManager, AiAuth, AiComConfig};

    use super::*;

    /// Test helper: build a fresh `AuthManager` rooted at a tempdir so
    /// nothing from a developer's actual `~/.ai/auth.json` leaks in.
    ...
    fn hard_expired_auth(key: &str) -> AiAuth {
        AiAuth {
            key: key.to_string(),
    ...
    /// `ShellAttribution` implements `sentinel_ai_tools::Auth401AttributionCallback`
