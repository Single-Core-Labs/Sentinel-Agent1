...
use super::is_sentinel_oauth2_issuer;
...
/// auth.json scope key for plain API key auth (desktop login, `ai login --api-key`).
pub(super) const API_KEY_SCOPE: &str = "sentinel::api_key";
...
/// server enrichment opts in. Single source for `AiAuth`, `AuthMeta`, and
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from ai-desktop login or `ai login --api-key`)
    ApiKey,
}
...
pub struct AiAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_ai_code_access: Option<bool>,
    ...
    /// credential first-party (`is_sentinel_auth`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl std::fmt::Debug for AiAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AiAuth")
...
impl AiAuth {
    ...
    /// `true` when the token comes from a first-party AI account —
    /// either an OIDC login against https://auth.x.ai (or the local-dev
    /// equivalent), or an external auth provider that declared an AI
    ...
    pub fn is_sentinel_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::Oidc | AuthMode::External => self
                .oidc_issuer
                .as_deref()
                .is_some_and(is_sentinel_oauth2_issuer),
            AuthMode::ApiKey | AuthMode::WebLogin => false,
        }
    }

    /// `true` when this auth can access ai.com managed MCP connectors.
    pub fn is_managed_mcp_eligible(&self) -> bool {
        self.is_sentinel_auth() || self.auth_mode == AuthMode::WebLogin
    }
    ...
    /// (`is_sentinel_auth`), matching the built-in devbox login they replace.
    /// Plain API keys never do.
    pub(crate) fn is_session_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::WebLogin | AuthMode::Oidc => true,
            AuthMode::External => self.is_sentinel_auth(),
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &AiAuth) {
...
#[cfg(test)]
impl AiAuth {
    /// Returns a `AiAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// AiAuth { key: "my-key".into(), ..AiAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, AiAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `ai login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<AiAuth> {
...
/// Early-invalidation buffer. Override with `AI_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("AI_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &AiAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &AiAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> AiAuth {
        AiAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    #[test]
    fn is_sentinel_auth_matrix() {
        use crate::auth::AI_OAUTH2_ISSUER;
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| AiAuth {
            oidc_issuer: issuer.map(str::to_owned),
            ..make_auth(mode)
        };

        // Only Oidc/External qualify, and only with an x.ai issuer.
        assert!(with_issuer(AuthMode::Oidc, Some(AI_OAUTH2_ISSUER)).is_sentinel_auth());
        assert!(with_issuer(AuthMode::External, Some(AI_OAUTH2_ISSUER)).is_sentinel_auth());
        assert!(!with_issuer(AuthMode::Oidc, None).is_sentinel_auth());
        assert!(!with_issuer(AuthMode::External, None).is_sentinel_auth());
        assert!(!with_issuer(AuthMode::Oidc, Some("https://idp.acme.example")).is_sentinel_auth());
        assert!(!with_issuer(AuthMode::External, Some("https://idp.acme.example")).is_sentinel_auth());

        // ApiKey / WebLogin stay false even with an x.ai issuer set.
        assert!(!with_issuer(AuthMode::ApiKey, Some(AI_OAUTH2_ISSUER)).is_sentinel_auth());
        assert!(!with_issuer(AuthMode::WebLogin, Some(AI_OAUTH2_ISSUER)).is_sentinel_auth());
    }
    ...
    fn is_session_auth_requires_first_party_for_external() {
        ...
        assert!(with_issuer(AuthMode::External, Some(AI_OAUTH2_ISSUER)).is_session_auth());
        ...
        // Plain API keys never do.
        assert!(!with_issuer(AuthMode::ApiKey, Some(AI_OAUTH2_ISSUER)).is_session_auth());
    }
    ...
    fn user_info_subscription_tier_present() {
        let json = r#"{
            "userId": "u1",
            "subscriptionTier": "SuperAiPro"
        }"#;
        let info: UserInfo = serde_json::from_str(json).unwrap();
        assert_eq!(info.subscription_tier.as_deref(), Some("SuperAiPro"));
    }
