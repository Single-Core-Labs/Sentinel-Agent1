...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> AiAuth {
    AiAuth {
...
fn auth_scope_uses_oauth2_when_present() {
    let cfg = AiComConfig::default();
    // Default config always has oauth2 set to the AI defaults.
    assert_eq!(
        cfg.auth_scope(),
        format!(
            "{}::{}",
            crate::auth::config::AI_OAUTH2_ISSUER,
...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older ai version).
...
fn manager_collection_predicates_fail_directions() {
    ...
    mgr.hot_swap(AiAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `ai login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn refresh_chain_demotes_when_attributed_tried_rt_differs_from_disk() {
    ...
    struct AttributedRejection(AiAuth);
    #[async_trait::async_trait]
    ...
    assert!(
        ...
        "demotion must not record a sticky verdict that locks out every \
         sibling process until the user re-runs `ai login`",
    );
}
...
async fn permanent_other_retains_credentials() {
    ...
    let mut store = AuthStore::new();
    store.insert(AiComConfig::default().auth_scope(), session);
    write_auth_json(&dir.path().join("auth.json"), &store).unwrap();
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `ai logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `ai login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
/// `has_ai_code_access`. The `&mut AiAuth` signature already
...
/// contributor adding a token-like field to both `AiAuth` and
...
fn apply_user_info_enrichment_preserves_token_fields() {
    ...
    assert_eq!(disk.has_ai_code_access, snapshot.has_ai_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    use sentinel_ai_tools::types::ApiKeyProvider;

    let _sentinel = EnvGuard::unset("AI_API_KEY");
    let _legacy = EnvGuard::unset("AI_CODE_AI_API_KEY");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), AiComConfig::default()));
    mgr.hot_swap(AiAuth {
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), AiComConfig::default()).with_proxy_base_url(&base);
...
fn oidc_session_for_team(principal_id: &str) -> AiAuth {
    AiAuth {
        key: team_jwt(principal_id),
        auth_mode: AuthMode::Oidc,
        refresh_token: Some("rt".into()),
        expires_at: Some(Utc::now() + Duration::hours(1)),
        oidc_issuer: Some(crate::auth::config::AI_OAUTH2_ISSUER.to_string()),
        oidc_client_id: Some("client".into()),
        ..AiAuth::test_default()
    }
}
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || AiAuth {
        key: "sentinel-cached-key".into(),
        auth_mode: AuthMode::ApiKey,
        expires_at: Some(Utc::now() + Duration::hours(1)),
        ..AiAuth::test_default()
    };
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), AiComConfig::default()));
    mgr2.hot_swap(api_key_session());
    assert_eq!(
        mgr2.current().map(|a| a.key),
        Some("sentinel-cached-key".to_string()),
...
/// No OAuth session → env or auth.json `sentinel::api_key` for voice/tools.
#[tokio::test]
#[serial_test::serial]
async fn shared_api_key_provider_static_fallthrough() {
    ...
    {
        let _legacy = EnvGuard::unset("AI_CODE_AI_API_KEY");
        let _key = EnvGuard::set("AI_API_KEY", "env-only-key");
        assert_eq!(
    ...
    {
        let _key = EnvGuard::set("AI_API_KEY", "env-should-lose");
        mgr.hot_swap(AiAuth {
...
async fn shared_api_key_provider_kill_switch_blocks_static() {
    ...
    let _key = EnvGuard::set("AI_API_KEY", "blocked");
    let dir = tempfile::tempdir().unwrap();
...
async fn shared_api_key_provider_oidc_preferred_blocks_static() {
    ...
    let _key = EnvGuard::set("AI_API_KEY", "should-not-use");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        AiComConfig {
...
async fn shared_api_key_provider_api_key_preferred_skips_session() {
    ...
    let _legacy = EnvGuard::unset("AI_CODE_AI_API_KEY");
    let _key = EnvGuard::set("AI_API_KEY", "static-preferred");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        AiComConfig {
...
async fn shared_api_key_provider_sync_falls_through_when_session_expired() {
    ...
    let _legacy = EnvGuard::unset("AI_CODE_AI_API_KEY");
    let _key = EnvGuard::set("AI_API_KEY", "static-after-expiry");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), AiComConfig::default()));
    mgr.hot_swap(AiAuth {
        key: "expired-oidc".into(),
...
async fn shared_api_key_provider_sync_buffered_session_beats_static() {
    ...
    let _legacy = EnvGuard::unset("AI_CODE_AI_API_KEY");
    let _key = EnvGuard::set("AI_API_KEY", "leftover-static");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), AiComConfig::default()));
...
async fn process_key_precedence() {
    ...
    {
        let _key = EnvGuard::set("AI_API_KEY", "env");
        assert_eq!(
            provider.current_api_key_async().await.as_deref(),
            Some("env")
        );
...
#[test]
#[serial_test::serial(force_dark_wake_env)] // reads AI_AUTH_FORCE_DARK_WAKE
fn is_dark_wake_false_when_power_listener_not_started() {
    let _unset = sentinel_ai_test_support::EnvGuard::unset("AI_AUTH_FORCE_DARK_WAKE");
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), AiComConfig::default());
    assert!(
        !mgr.is_dark_wake(),
...
/// `AI_AUTH_FORCE_DARK_WAKE` forces the dark-wake answer for manual and
...
fn is_dark_wake_env_override_forces_both_states() {
    ...
    {
        let _g = EnvGuard::set("AI_AUTH_FORCE_DARK_WAKE", "1");
        assert!(
            mgr.is_dark_wake(),
    ...
    {
        let _g = EnvGuard::set("AI_AUTH_FORCE_DARK_WAKE", "0");
    ...
    }
    {
        ...
        let _g = EnvGuard::set("AI_AUTH_FORCE_DARK_WAKE", "yes");
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use sentinel_ai_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use sentinel_ai_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, AiComConfig::default()));
    ...
    use sentinel_ai_telemetry::events::{
...
fn external_provider_config() -> AiComConfig {
    AiComConfig {
