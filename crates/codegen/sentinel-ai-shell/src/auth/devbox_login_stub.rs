...
//! points that can still be reached directly (`ai login --devbox`) return a
//! descriptive error.

use super::manager::AuthManager;
use super::model::AiAuth;
...
pub(crate) async fn mint_devbox_auth(_auth_manager: &AuthManager) -> anyhow::Result<AiAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// Unreachable in practice (guarded by [`is_devbox_environment`]); errors
/// defensively if called.
pub(super) async fn mint_devbox_auth_raw() -> anyhow::Result<AiAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// `ai login --devbox` entry point: always errors in this build.
