...
use crate::auth::model::AiAuth;
use crate::auth::token_type::TokenType;
use sentinel_ai_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&AiAuth>) -> Self {
        Self {
...
impl UnauthorizedRecovery {
    ...
    pub(crate) async fn next(&mut self) -> Result<AiAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<AiAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<AiAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.ai_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<AiAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            sentinel_ai_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
        if crate::auth::is_expired(&disk_auth) {
            ...
            sentinel_ai_telemetry::unified_log::debug(
                ...
                Some(serde_json::json!({
                    "disk_key_prefix": sentinel_ai_auth::bearer_suffix(&disk_auth.key),
        ...
        if self.is_different_token(&disk_auth) {
            ...
            sentinel_ai_telemetry::unified_log::info(
                ...
                None,
                Some(serde_json::json!({
                    "adopted_key_prefix": sentinel_ai_auth::bearer_suffix(&disk_auth.key),
                    "expires_at": disk_auth.expires_at.map(|e| e.to_rfc3339()),
                })),
            );
    ...
    fn fresh_mint_guard(&self) -> Option<AiAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<AiAuth, AuthError> {
                    ...
                    Ok(auth) => {
                        sentinel_ai_telemetry::unified_log::info(
                            ...
                            Some(serde_json::json!({
                                ...
                                "new_key_prefix": sentinel_ai_auth::bearer_suffix(&auth.key),
                                "expires_at": auth.expires_at.map(|e| e.to_rfc3339()),
                            })),
                        );
                    }
    ...
    fn is_different_token(&self, candidate: &AiAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, AiAuth};
    ...
    fn rejected_cred() -> Option<AiAuth> {
        Some(AiAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), AiComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<AiAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.ai_com_config().auth_scope();
        let fresh = AiAuth {
