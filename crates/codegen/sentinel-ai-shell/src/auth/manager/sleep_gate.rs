...
impl SleepGate {
    pub(super) fn raise(&self) {
        *self.raised_at.write() = Some(DualClock::now());
        sentinel_ai_telemetry::unified_log::warn("auth.sleep.gate_set", None, None);
    }
...
impl AuthManager {
    ...
    /// [`sentinel_system_power::PowerState`] for the canonical explanation of what a
    ...
    ///
    /// `AI_AUTH_FORCE_DARK_WAKE=1|0` forces the answer for testing (unset
    ...
    /// `AI_AUTH_EARLY_INVALIDATION_SECS` for a seconds-long repro.
    pub(crate) fn is_dark_wake(&self) -> bool {
        #[cfg(test)]
        if let Some(forced) = *self.dark_wake_override.lock() {
            return forced;
        }
        match std::env::var("AI_AUTH_FORCE_DARK_WAKE").ok().as_deref() {
            Some("1") => return true,
            Some("0") => return false,
            _ => {}
        }
        ...
        matches!(
            sentinel_system_power::current_power_state(),
            sentinel_system_power::PowerState::DarkWake
        )
    }
    ...
    pub fn start_system_power_listener(self: &Arc<Self>) {
        ...
        let listener = sentinel_system_power::SystemPowerListener::start(move |event| {
            if let Some(this) = weak.upgrade() {
                let imminent = matches!(event, sentinel_system_power::PowerEvent::WillSleep);
