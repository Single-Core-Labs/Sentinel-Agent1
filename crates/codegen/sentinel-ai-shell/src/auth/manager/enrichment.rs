...
use crate::auth::model::{AiAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: AiAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.ai_com_config.token_header;
    ...
    let response = http_client
        ...
        .header("X-AI-Token-Auth", token_header.as_str())
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut AiAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: AiAuth) {
    let started = std::time::Instant::now();
    ...
    if disk.key != auth.key || disk.refresh_token != auth.refresh_token {
        sentinel_ai_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                ...
                "written_key_prefix": sentinel_ai_auth::bearer_suffix(&auth.key),
                "disk_key_prefix": sentinel_ai_auth::bearer_suffix(&disk.key),
            })),
        );
        return;
    }
...
pub(super) fn apply_user_info_enrichment(disk: &mut AiAuth, user_info: UserInfo) {
