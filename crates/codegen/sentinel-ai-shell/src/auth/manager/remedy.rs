    ...
    pub(crate) fn auth_remedy(&self) -> AuthRemedy {
        ...
        match (user_must_act, provider_mints_sessions) {
            ...
            (true, true) => AuthRemedy::ProviderLogin {
                label: self.ai_com_config().auth_provider_label.clone(),
            },
...
mod tests {
    ...
    use crate::auth::{AiComConfig, error::RefreshTokenFailedReason};
    ...
    fn external_credential(expires_at: chrono::DateTime<Utc>) -> AiAuth {
        AiAuth {
            key: "external".into(),
    ...
    fn provider_manager(dir: &std::path::Path, credential: AiAuth) -> Arc<AuthManager> {
    ...
    fn expired_credential_without_a_provider_command_needs_a_manual_login() {
        let dir = tempfile::tempdir().unwrap();
        let manager = Arc::new(AuthManager::new(dir.path(), AiComConfig::default()));
