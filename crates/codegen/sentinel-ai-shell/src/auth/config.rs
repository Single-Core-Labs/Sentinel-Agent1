...
/// Default scopes for the AI OAuth2 provider. Includes `ai-cli:access`
...
pub enum PreferredAuthMethod {
    /// `AI_API_KEY` / auth.json `sentinel::api_key` / per-model BYOK (`sentinel.api_key`).
    ApiKey,
    /// OIDC / OAuth2 session (`cached_token`, interactive `ai.com` / `oidc`,
    /// including devbox-minted OIDC).
    Oidc,
}
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct AiComConfig {
    ...
    /// Login button label (env: `AI_AUTH_PROVIDER_LABEL`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// refresh works. Env: `AI_AUTH_TOKEN_TTL`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub auth_token_ttl: Option<u64>,
    /// Admin kill switch: when `Some(true)`, the `sentinel.api_key` auth method is
    /// neither advertised nor accepted, so `AI_API_KEY`/per-model credentials
    /// can't bypass the deployment's IdP login. Env: `AI_DISABLE_API_KEY_AUTH`.
...
/// Customer OIDC Identity Provider configuration (`[ai_com_config.oidc]`).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// OAuth2 provider configuration (`AI_OAUTH2_ISSUER` / `AI_OAUTH2_CLIENT_ID`).
///
...
pub const AI_OAUTH2_ISSUER: &str = "https://auth.x.ai";
...
const AI_OAUTH2_LOCAL_ISSUER: &str = "http://localhost:22255";
const DEFAULT_OAUTH2_REFERRER: &str = "ai-build";
/// Returns `true` when `AI_LOCAL_AUTH=1` is set,
/// indicating the local accounts-app should be used as the OAuth2 issuer.
pub(crate) fn use_local_auth() -> bool {
    std::env::var("AI_LOCAL_AUTH")
        .map(|v| !v.is_empty() && v != "0")
        .unwrap_or(false)
}
/// Returns the active AI OAuth2 issuer — the local-dev issuer when
/// `AI_LOCAL_AUTH=1` is set, otherwise the production issuer.
pub fn sentinel_oauth2_issuer() -> &'static str {
    if use_local_auth() {
        AI_OAUTH2_LOCAL_ISSUER
    } else {
        AI_OAUTH2_ISSUER
    }
}
/// Returns `true` if `issuer` is a recognised AI OAuth2 issuer
/// (production **or** local-dev). Use this instead of comparing against
/// [`AI_OAUTH2_ISSUER`] directly so that local-dev sessions are still
/// treated as first-party AI auth.
pub fn is_sentinel_oauth2_issuer(issuer: &str) -> bool {
    issuer == AI_OAUTH2_ISSUER || issuer == AI_OAUTH2_LOCAL_ISSUER
}
/// auth.json scope key used by the pre-OIDC `ai login --legacy` flow.
...
impl AiComConfig {
    /// Whether `sentinel.api_key` auth is disabled. Pinning a team
    ...
    /// `AI_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
    ...
    /// pin is fail-closed. Explicit `ai login --devbox` / `--api-key` bypass
    ...
    pub fn auth_scope(&self) -> String {
        if let Some(ref oidc) = self.oidc {
            ...
            unreachable!("oauth2 config is always present (AI default or env override)")
        }
    }
}
impl OAuth2ProviderConfig {
    ...
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("AI_OAUTH2_ISSUER").ok()?;
        let client_id = std::env::var("AI_OAUTH2_CLIENT_ID").ok()?;
        let principal_type = std::env::var("AI_OAUTH2_PRINCIPAL_TYPE").ok();
        let principal_id = std::env::var("AI_OAUTH2_PRINCIPAL_ID").ok();
        ...
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("AI_OAUTH2_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or(default_scopes),
            principal_type,
            principal_id,
            referrer: Some(
                std::env::var("AI_OAUTH2_REFERRER")
...
impl Default for AiComConfig {
    fn default() -> Self {
        ...
        let oauth2 = if oidc.is_some() {
            ...
            Some(
                OAuth2ProviderConfig::from_env().unwrap_or_else(|| OAuth2ProviderConfig {
                    issuer: sentinel_oauth2_issuer().to_owned(),
        ...
        Self {
            ai_ws_origin: std::env::var("AI_WS_ORIGIN")
                .unwrap_or_else(|_| PROD_WS_ORIGIN.to_owned()),
            ai_ws_url: std::env::var("AI_WS_URL")
                .unwrap_or_else(|_| PROD_RELAY_WS_URL.to_owned()),
            token_header: "sentinel-ai-cli".to_owned(),
            oidc,
            oauth2,
            auth_provider_command: std::env::var("AI_AUTH_PROVIDER_COMMAND").ok(),
            auth_provider_label: std::env::var("AI_AUTH_PROVIDER_LABEL").ok(),
            auth_token_ttl: std::env::var("AI_AUTH_TOKEN_TTL")
                .ok()
                .and_then(|v| v.parse().ok()),
            disable_api_key_auth: std::env::var("AI_DISABLE_API_KEY_AUTH")
                .ok()
...
/// Parse a boolean env-var value for ai's on/off flags. A bare presence
/// enables the flag, but the common falsy spellings (`0`, `false`, `off`,
/// `no`, empty) count as disabled — so e.g. `AI_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `AI_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("AI_DISABLE_API_KEY_AUTH")
        .ok()
        .is_some_and(|v| env_flag_enabled(&v))
}
impl OidcAuthConfig {
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("AI_OIDC_ISSUER").ok()?;
        let client_id = std::env::var("AI_OIDC_CLIENT_ID").ok()?;
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("AI_OIDC_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or_else(|_| default_oidc_scopes()),
            audience: std::env::var("AI_OIDC_AUDIENCE").ok(),
        })
    }
}
#[cfg(test)]
mod tests {
    ...
    /// FROZEN client contract: the 10 scopes the AI OAuth2 client requests.
    ...
    fn preferred_method_deserializes_from_toml() {
        let cfg: AiComConfig = toml::from_str(
            r#"
        ...
        let cfg: AiComConfig = toml::from_str("").expect("parse empty");
