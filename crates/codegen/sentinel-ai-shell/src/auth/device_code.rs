...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, AiAuth};
...
pub(crate) enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `ai login` or set AI_API_KEY instead."
    )]
    NotEnabled,
}
...
/// `x-ai-client-surface` header so device-flow metrics can separate logins a
...
pub(crate) async fn request_device_code(
    ...
    let resp = with_alpha_test_key(
        client
            ...
            .header("x-ai-client-surface", surface.as_str())
            .form(&[
                ("client_id", client_id),
...
///
/// On success, persists credentials to `~/.ai/auth.json` and returns
/// the authenticated `AiAuth`.
///
...
pub(crate) async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `ai login --device-auth` again.");
        }
...
pub(crate) mod tests {
    ...
    use crate::auth::{AuthMode, AiComConfig};
    ...
    fn auth_manager_with_ai_home(
        ai_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(ai_home, AiComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let ai_home = temp_dir.path().join(".ai");
        std::fs::create_dir_all(&ai_home).unwrap();
        let auth_manager = auth_manager_with_ai_home(&ai_home, "http://127.0.0.1:9");
        let tokens = super::TokenOk {
            access_token: "access-token".to_string(),
            refresh_token: Some("refresh-token".to_string()),
            expires_in: Some(900),
            scope: Some("openid email offline_access ai-cli:access".to_string()),
            id_token: Some(
    ...
    fn build_auth_seeds_team_metadata_from_access_token() {
        ...
        let claims = serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access ai-cli:access team:read",
            "principal_type": "Team",
        ...
        let tokens = super::TokenOk {
            ...
            expires_in: Some(900),
            scope: Some("offline_access ai-cli:access team:read".to_owned()),
            id_token: None,
        };
    ...
    /// persist nothing.
    fn assert_build_auth_rejected(cfg: AiComConfig, token_principal: &str, expected_err: &str) {
        ensure_crypto_provider();
        let temp_dir = tempfile::tempdir().unwrap();
        let ai_home = temp_dir.path().join(".ai");
        std::fs::create_dir_all(&ai_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&ai_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::AiAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_ai_home(temp_dir.path(), "http://127.0.0.1:9");
