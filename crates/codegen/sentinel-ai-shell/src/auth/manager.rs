...
use sentinel_ai_auth::bearer_suffix;
...
use crate::auth::config::AiComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use sentinel_ai_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, AiAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<AiAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    power_listener: parking_lot::Mutex<Option<sentinel_system_power::SystemPowerListener>>,
...
impl AuthManager {
    pub fn new(ai_home: &Path, ai_com_config: AiComConfig) -> Self {
        let scope = ai_com_config.auth_scope();
        let proxy_base_url =
        ...
        sentinel_ai_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "ai_home": ai_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "AI_HOME": std::env::var("AI_HOME").unwrap_or_else(|_| "(unset)".into()),
                "AI_AUTH_PATH": std::env::var("AI_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "AI_AUTH": std::env::var("AI_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // AI_AUTH_PATH: custom file path (overrides default $AI_HOME/auth.json).
        // Resolved before the AI_AUTH branch so inline-credential managers
        ...
        let path = std::env::var("AI_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| ai_home.join("auth.json"));

        // AI_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("AI_AUTH") {
            if let Ok(auth) = serde_json::from_str::<AiAuth>(&inline_json) {
            ...
            tracing::warn!("AI_AUTH set but failed to parse as JSON, falling back to file");
        }
    ...
    /// (inline `AI_AUTH` vs. on-disk `auth.json`), which differ only in the
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &AiAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.ai_com_config)?;
    ...
    fn vet_cached(&self, auth: AiAuth) -> Option<AiAuth> {
    ...
    pub(crate) fn current(&self) -> Option<AiAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<AiAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&AiAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<AiAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<AiAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<AiAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &AiAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &AiAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &AiAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.ai_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `AiAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: AiAuth) -> std::io::Result<AiAuth> {
        ...
        match &write_result {
            Ok(()) => sentinel_ai_telemetry::unified_log::info(
            ...
            Err(e) => sentinel_ai_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: AiAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut AiAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    /// Path to the `auth.json` this manager reads/writes (respects
    /// `AI_AUTH_PATH` / constructor home). Prefer this over
    /// `ai_home()/auth.json` so temp-home tests and custom stores stay isolated.
    pub(crate) fn auth_json_path(&self) -> &Path {
        &self.path
    }

    pub(crate) fn ai_com_config(&self) -> &AiComConfig {
        &self.ai_com_config
    }
    ...
    pub(crate) async fn run_external_refresh_command(&self, command: &str) -> Option<AiAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: AiAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<AiAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> AiAuth {
        self.inner.read().clone().unwrap_or_else(|| AiAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: AiAuth) -> Option<AiAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<AiAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<AiAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<AiAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in AI_AUTH mode).
            DiskAuthState::Ok => {
                sentinel_ai_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<AiAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<AiAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<AiAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.ai_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`ai login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.ai_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery(snapshot_key.as_deref()).await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(
        ...
        sentinel_ai_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    pub(crate) async fn refresh_chain(
        ...
        let outcome = {
            ...
            let _awake = if self.is_dark_wake() {
                ...
                sentinel_system_power::hold_awake("ai: OIDC token refresh")
            } else {
                None
            };
    ...
    pub(crate) fn is_different_token(&self, candidate: &AiAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    fn is_external_provider_refresh_authority(&self) -> bool {
        self.ai_com_config.auth_provider_command.is_some()
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<sentinel_ai_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[ai_com] auth_token_ttl` to drive a real schedule.
...
impl sentinel_ai_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.ai_com_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.ai_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.ai_com_config.preferred_method,
    ...
    non_empty_key(crate::agent::auth_method::read_sentinel_api_key_env().ok())
...
impl AuthManager {
    /// `sentinel::api_key` from this manager's auth file, memoized on
...
) -> sentinel_ai_tools::types::SharedApiKeyProvider {
