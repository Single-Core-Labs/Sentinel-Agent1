//! First-party env key probe (`GET {sentinel_api_base_url}/api-key`) before
//! advertising `sentinel.api_key`. BYOK is not probed.
//!
//! The base URL is the caller's effective `endpoints.sentinel_api_base_url` so the
//! probe hits the same host turn traffic uses (`AI_AI_API_BASE_URL` /
//! `[endpoints] sentinel_api_base_url`), not a hardcoded production host.
//!
...
/// (`endpoints.sentinel_api_base_url`), not a hardcoded public default.
async fn probe_sentinel_api_key(key: &str, api_base_url: &str, timeout: Duration) -> ApiKeyProbeVerdict {
    let url = api_key_info_url(api_base_url);
    probe_sentinel_api_key_at_url(key, &url, timeout).await
}

/// Injectable full URL for tests.
async fn probe_sentinel_api_key_at_url(key: &str, url: &str, timeout: Duration) -> ApiKeyProbeVerdict {
...
///
/// `api_base_url` is the caller's effective `endpoints.sentinel_api_base_url`, so the
/// probe follows the same endpoint as turn traffic — and, in tests, the mock
/// server fixtures already set via `AI_AI_API_BASE_URL`.
pub(crate) async fn first_party_env_key_allows_advertise(
    api_base_url: &str,
    timeout: Duration,
) -> bool {
    let Ok(key) = crate::agent::auth_method::read_sentinel_api_key_env() else {
        return false;
    };
    probe_sentinel_api_key(&key, api_base_url, timeout)
        .await
        .allows_advertise()
}
...
mod tests {
    ...
    async fn local_server_invalid_key_is_unusable() {
        ...
        let v = probe_sentinel_api_key_at_url("sentinel-bad", &url, Duration::from_secs(2)).await;
    ...
    async fn local_server_blocked_key_is_unusable() {
        ...
        let v = probe_sentinel_api_key_at_url("sentinel-blocked", &url, Duration::from_secs(2)).await;
        assert_eq!(v, ApiKeyProbeVerdict::Unusable);
    }
    ...
    async fn local_server_ok_key_is_usable() {
        ...
        let v = probe_sentinel_api_key_at_url("sentinel-good", &url, Duration::from_secs(2)).await;
    ...
    async fn probes_joined_base_url_path() {
        ...
        let v = probe_sentinel_api_key("sentinel-good", base, Duration::from_secs(2)).await;
        assert_eq!(v, ApiKeyProbeVerdict::Usable);
    }
    ...
    async fn retries_429_then_succeeds() {
        ...
        let v = probe_sentinel_api_key_at_url("sentinel-retry", &url, Duration::from_secs(2)).await;
        assert_eq!(v, ApiKeyProbeVerdict::Usable);
    }
    ...
    async fn timeout_is_unknown_fail_open() {
        ...
        let v = probe_sentinel_api_key_at_url("sentinel-slow", &url, Duration::from_millis(80)).await;
