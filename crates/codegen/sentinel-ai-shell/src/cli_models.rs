//! Data APIs for `ai models`. Clients own display.
...
/// Status for the `ai models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `ai_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    /// [`crate::agent::auth_method::should_advertise_sentinel_api_key`] so
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.ai_com_config.ai_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_sentinel_api_key(
            agent_config.ai_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    ///
    /// Uses `AI_AUTH_PATH` (not `AI_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            EnvGuard::unset(AI_API_KEY_ENV_VAR),
            EnvGuard::unset(LEGACY_AI_API_KEY_ENV_VAR),
            EnvGuard::unset("AI_AUTH"),
            EnvGuard::set("AI_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("AI_DEPLOYMENT_KEY"),
            EnvGuard::unset("AI_WS_ORIGIN"),
            EnvGuard::unset("AI_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_api_key_env() {
        ...
        let _key = EnvGuard::set(AI_API_KEY_ENV_VAR, "sentinel-test-key");
    ...
    fn resolve_legacy_api_key_env() {
        let (_dir, _g) = isolate_auth_sources();
        let _key = EnvGuard::set(LEGACY_AI_API_KEY_ENV_VAR, "legacy-key");
        assert_eq!(AuthStatus::resolve(&Config::default()), AuthStatus::ApiKey);
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("AI_AUTH", &json);

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("ai.com".to_owned())
        );
    }
    ...
    fn models_list_response_round_trips() {
        let state = acp::SessionModelState::new(
            acp::ModelId::new("ai-4"),
            vec![acp::ModelInfo::new(acp::ModelId::new("ai-4"), "Ai 4")],
        );
        let ok = crate::session::ExtMethodResult::success(state)
            .to_ext_response()
            .unwrap();
        let parsed = parse_models_list_response(ok.0.get()).unwrap();
        assert_eq!(parsed.current_model_id.0.as_ref(), "ai-4");
