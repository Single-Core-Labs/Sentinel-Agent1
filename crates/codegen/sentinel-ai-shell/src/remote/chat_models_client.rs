//! ai.com chat-product model catalog (`POST /rest/modes`) — the models
//! ai-web's chat picker shows, distinct from the CLI `/v1/models` build
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("AI_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
    ...
    /// Gated only on a valid ai.com bearer — deliberately NOT `is_sentinel_auth()`
