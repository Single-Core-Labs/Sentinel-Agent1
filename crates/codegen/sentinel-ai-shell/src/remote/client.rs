...
use crate::auth::{AiAuth, AiComConfig};
...
const AI_CODE_BACKEND_URL: &str = "https://code.ai.com";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const AI_CODE_WEB_URL: &str = "https://ai.com";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("AI_CODE_WEB_URL").unwrap_or_else(|_| AI_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-AI-Token-Auth", AiComConfig::default().token_header)
        .header("x-userid", &auth.user_id)
        .header("x-ai-client-version", sentinel_ai_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::ai_auth_credentials::AiAuthCredentials::new(
    ...
    sentinel_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn sentinel_ai_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    fn build_default_client() -> reqwest::Client {
        sentinel_ai_extra_ca::with_extra_root_certificates(
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("AI_CODE_BACKEND_URL")
                .unwrap_or_else(|_| AI_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&AiAuth`.
    pub(crate) fn with_auth_manager(
        mut self,
        manager: std::sync::Arc<crate::auth::AuthManager>,
    ) -> Self {
        let credentials: std::sync::Arc<dyn sentinel_ai_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<AiAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-AI-Token-Auth so nginx auth subrequest routes to authenticate_sentinel_ai_cli_token.
    /// See: crates/codegen/sentinel-ai-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-AI-Token-Auth",
            required(&AiComConfig::default().token_header, "X-AI-Token-Auth")?,
        );
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-ai-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-ai-client-version",
            HeaderValue::from_static(sentinel_ai_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = sentinel_file_utils::trace_context::inject_trace_context_into_request(
...
/// Fetch `ai_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-ai-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(sentinel_ai_telemetry::id::agent_id)
        .await
        .ok()?;
...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.sentinel_api_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_sentinel_api_key_env()
                .or_else(|_| {
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for custom models endpoint. Set AI_API_KEY.".into(),
                    )
                })?;
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-AI-Token-Auth", "sentinel-ai-cli")
                .header("x-userid", &auth.user_id)
                .header("x-ai-client-version", sentinel_ai_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub(crate) fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| sentinel_ai_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(sentinel_ai_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
fn parse_compaction_at_tokens(
    v: &serde_json::Value,
) -> Option<sentinel_ai_sampling_types::CompactionAtTokens> {
...
fn parse_compactions_remaining(
    v: &serde_json::Value,
) -> Option<sentinel_ai_sampling_types::CompactionsRemaining> {
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-ai-agent-id"),
                            client_identifier: header_str(&headers, "x-ai-client-identifier"),
                            client_version: header_str(&headers, "x-ai-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-ai-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-ai-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-ai-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::AiComConfig::default());
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("sentinel-ai-cli"));
    ...
    fn parse_openai_format_uses_id_field() {
        ...
        assert_eq!(result.model, "ai-3");
        assert_eq!(result.base_url, "https://api.x.ai/v1");
        assert_eq!(result.name.as_deref(), Some("ai-3"));
    }
    #[test]
    ...
    fn inference_url_defaults_to_proxy() {
        let ep = endpoints("https://proxy.ai.com/v1", None, None);
        assert_eq!(ep.resolve_inference_base_url(), "https://proxy.ai.com/v1");
    }
    #[test]
    fn inference_url_uses_models_base_url() {
        let ep = endpoints(
            "https://proxy.ai.com/v1",
    ...
    /// ApiKey → `sentinel_api_base_url` (ApiKey, public default when unset); a custom
    ...
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        assert_eq!(session.url, "https://cli-chat-proxy.ai.com/v1/models");
        ...
        assert_eq!(deployment.url, "https://cli-chat-proxy.ai.com/v1/models");
        assert_eq!(deployment.auth, EndpointAuth::Session);
        let api = ListModelsEndpoint::from_endpoints(&cfg, ModelFetchAuth::ApiKey);
        assert_eq!(api.url, "https://inference.acme-corp.example/sentinel/v1/models");
    ...
    /// REGRESSION: `ai setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("AI_DEPLOYMENT_KEY", "sentinel-token-ENTERPRISE") };
        ...
        assert_eq!(url, "https://cli-chat-proxy.ai.com/v1/deployment/config");
        assert!(
        ...
        unsafe { std::env::remove_var("AI_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
