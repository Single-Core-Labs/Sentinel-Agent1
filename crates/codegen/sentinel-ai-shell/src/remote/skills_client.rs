//! ai.com product Skills catalog — the same REST sources ai-web uses:
...
use sentinel_ai_tools::implementations::skills::skill::extract_skill_body;
use sentinel_ai_tools::implementations::skills::types::{SkillInfo, SkillScope};

use crate::auth::AuthManager;

const AI_WEB_URL: &str = "https://ai.com";
...
/// connect timeout; without this a hung ai.com stalls the session actor.
...
/// Combined product catalog matching ai-web's Skills menu composition.
...
impl ProductSkillsCatalog {
    ...
    ///
    /// Mirrors ai-web: enabled user skills first (and hide same-named
...
fn primary_is_tenant_tagged(auth: &crate::auth::AiAuth) -> bool {
    auth.team_id.is_some() || auth.organization_id.is_some()
}

fn entry_is_untagged(auth: &crate::auth::AiAuth) -> bool {
...
fn entry_matches_primary_tenant(
    primary: &crate::auth::AiAuth,
    entry: &crate::auth::AiAuth,
) -> bool {
...
fn skills_auth_alt_candidates<'a>(
    primary: &crate::auth::AiAuth,
    entries: impl IntoIterator<Item = &'a crate::auth::AiAuth>,
...
/// Stateless transport for product Skills REST (ai-web `skillsApi`).
...
impl SkillsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("AI_SKILLS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("AI_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("AI_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| AI_WEB_URL.to_string());
        Self {
    ...
    fn apply_auth_headers(
        ...
        let mut builder = builder
            ...
            .header(
                "X-AI-Token-Auth",
                self.auth.ai_com_config().token_header.clone(),
            )
    ...
    /// Ai.com product Skills require first-party session auth (same gate as
    /// managed MCP / sibling ai.com clients — not plain BYOK API keys).
    async fn require_skills_auth(&self) -> Result<crate::auth::AiAuth, SkillsError> {
    ...
    /// Credentials to try for ai.com product Skills REST.
    ///
    /// Primary first. When primary is OIDC on the default ai.com host, also
    ...
    fn skills_auth_candidates(&self, primary: &crate::auth::AiAuth) -> Vec<SkillsAuthCandidate> {
    ...
    /// feature-flag allow-list (same as ai-web attach menu).
    ///
...
mod tests {
    ...
    fn test_auth_manager() -> Arc<AuthManager> {
        use crate::auth::{AuthMode, AiAuth, AiComConfig, AI_OAUTH2_ISSUER};
