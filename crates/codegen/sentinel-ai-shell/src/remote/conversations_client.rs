...
/// Body for `PUT /rest/app-chat/conversations/{id}` (ai-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("AI_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
    ...
    async fn require_sentinel_auth(&self) -> Result<AiAuth, ConvError> {
    ...
    pub async fn list_conversations(
        ...
        let auth = self.require_sentinel_auth().await?;
