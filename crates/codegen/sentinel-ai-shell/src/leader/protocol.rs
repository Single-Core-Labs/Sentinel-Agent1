...
pub enum ClientMode {
    /// Headless mode (ai agent, ai agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (ai agent stdio, ai -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `ai update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `ai update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `ai update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["ai-a".into(), "ai-b".into()],
            pid: 4242,
        };
