//! Leader-follower IPC architecture for ai-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.ai/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use sentinel_ai_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
//! // Connect to existing leader or spawn a new one
//! let caps = ClientCapabilities {
//!     yolo_mode: true,
//!     default_model: Some("ai-3-fast".to_string()),
//! };
...
mod transport;
use crate::env::AiBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = sentinel_ai_version::VERSION;
...
/// How long the SAME live ai flock-holder may stay unconnectable before
...
/// Maximum reconnection attempts for bounded mode (headless/`ai -p`).
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<AiBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(AiBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<AiBuildEnvironment> {
    let environments: &[AiBuildEnvironment] = &[AiBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<AiBuildEnvironment> {
    let environments: &[AiBuildEnvironment] = &[AiBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::ai_home::ai_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!({
                            ...
                            "reason": reason,
                            "killer_ver": sentinel_ai_version::VERSION,
                        })),
                    );
                }
                Err(e) => {
...
impl From<&crate::auth::AiComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::AiComConfig) -> Self {
        Self {
            ai_ws_url: c.ai_ws_url.clone(),
            ai_ws_origin: c.ai_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`ai -p` where hanging forever is unacceptable.
...
///     "ai-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
enum ZombieAction {
    ...
    Clear,
    /// A live ai holder is still unconnectable; timer (re)armed, keep waiting.
    Wait,
...
/// The live *ai* PID that ACTUALLY holds the flock on the lock file, if any.
/// `None` for a dead / non-ai PID, OR when the file PID can't be confirmed to be
...
/// stricter (name-matching) ai check since this drives the auto-kill path.
///
...
fn live_ai_lock_holder(lock: &LeaderLock) -> Option<u32> {
    let file_pid = lock.read_pid()?;
    let pid = evictable_holder(file_pid, confirmed_flock_holder(lock.lock_path()))?;
    (crate::util::is_process_alive(pid) && crate::util::is_ai_process_strict(pid)).then_some(pid)
}
...
///
/// The `env_urls.ai_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "ai-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    if let Some(profile) = sentinel_ai_sandbox::requested_confinement_profile() {
        return Err(ConnectionError::SandboxConfinement(profile));
    }
    let start = std::time::Instant::now();
    let mut lock = LeaderLock::new(&env_urls.ai_ws_url);
    ...
    loop {
        ...
        match wait_for_socket_connectable(&sock_path, client_type, mode, capabilities.clone()).await
        {
            ...
            Err(e) if is_connect_level_failure(&e) => {
                let holder = live_ai_lock_holder(&lock);
...
///
/// For a **managed install** — the running binary lives under `ai_home`
/// (e.g. `~/.ai/...`) — prefer the managed `~/.ai/bin/ai` symlink. After an
/// auto-update or `ai update` atomically swaps that symlink, `current_exe()`
...
/// `sentinel_ai_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `ai_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.ai/bin/ai` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::ai_home::ai_home())
}
fn resolve_binary_with_home(ai_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(ai_home, std::env::current_exe().ok())
}
/// Binary file name for the managed ai install (`ai` / `ai.exe`).
fn managed_ai_bin_name() -> &'static str {
    if cfg!(windows) { "ai.exe" } else { "ai" }
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = ai_home.join("bin").join(managed_ai_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, ai_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--ai-ws-url").arg(&env_urls.ai_ws_url);
    cmd.arg("--ai-ws-origin").arg(&env_urls.ai_ws_origin);
    ...
    let log_path = crate::util::ai_home::ai_home().join("leader.log");
    ...
    let leader_log = std::env::var("AI_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "sentinel_ai_shell=info,sentinel_acp_lib=warn,sentinel_ai_mcp=warn".into());
...
mod tests {
    ...
    /// No live ai holder → `Clear`, and any pending timer is reset.
    #[test]
...
";
    ...
    /// `live_ai_lock_holder` returns `None` for a missing or dead PID, so the
    /// zombie net never times/kills a recycled or unrelated PID.
    #[test]
    fn live_ai_lock_holder_none_for_missing_or_dead_pid() {
        let temp = TempDir::new().unwrap();
        let lock = LeaderLock::from_paths(
            temp.path().join("leader.lock"),
            temp.path().join("leader.sock"),
        );
        assert_eq!(live_ai_lock_holder(&lock), None);
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            ai_ws_url: "wss://test.invalid".into(),
            ai_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("ai"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("ai-v2");
        std::fs::write(&target_v2, "new-binary").unwrap();
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("ai")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("ai-v2");
        std::fs::write(&new_target, "new-binary").unwrap();
        let managed = bin_dir.join("ai");
        std::os::unix::fs::symlink(&new_target, &managed).unwrap();
        let stale_target = bin_dir.join("ai-v1");
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_ai_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_ai_bin_name());
