...
use sentinel_computer_hub_sdk::{AuthCredential, AuthIdentity, AuthProvider};
use sentinel_ai_workspace::WorkspaceHandle;
...
struct ClientState {
    ...
    /// The client type string from IPC registration (e.g., "ai-tui", "ai-code-extension").
...
impl AuthProvider for LeaderAuthProvider {
    ...
    /// read. Mirrors the in-process path (`mvp_agent`): prefer `AiAuth.team_id`
...
/// notifications without an `eventId` (AI one-shots / older shell).
...
async fn handle_workspace_start(
    ...
    let status_config = sentinel_ai_workspace::StatusConfig::from_env();
    ...
    let upload_queue_enabled =
        std::env::var("AI_WORKSPACE_UPLOAD_QUEUE_ENABLED").as_deref() != Ok("false");
    ...
    let handle = sentinel_ai_workspace::connect_local_workspace(
        cwd_path.clone(),
        url,
        auth,
        Some(metadata),
            ...
            "params": {
                ...
                "message": format!(
                    "Client version {client_version} differs from leader version \
                     {leader_version}. Restart the ai binary to use the same version."
                )
            }
        })
        .to_string(),
...
///   ai.com WebSocket relay until this fires, so a leader serving only
...
mod tests {
    ...
    async fn workspace_start_errors_when_cancelled_before_auth() {
        let state = default_test_control_state(Path::new("/tmp/ai-ws-auth-test.sock"));
        let cancel = CancellationToken::new();
        cancel.cancel();
    ...
    /// dashboard, IDE) keeps the ai.com relay off. The first Headless
    /// registration (devbox / `ai agent headless` flow) flips the watch so
    ...
    async fn relay_demand_signals_only_on_headless_registration() {
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let _stdio =
            connect_and_register_with_mode(&sock_path, "ai-tui", ClientMode::Stdio).await;
        ...
        let _headless =
            connect_and_register_with_mode(&sock_path, "ai-headless", ClientMode::Headless).await;
    ...
    async fn initialize_gets_client_identifier_injected() {
        ...
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "ai-tui",
    ...
    async fn initialize_preserves_existing_client_identifier() {
        ...
        .unwrap();
        let _: ServerMessage = read_message(&mut reader).await.unwrap();
        let payload = r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"ai-web"}}}"#;
        write_message(
            &mut writer,
            &ClientMessage::Acp {
                payload: payload.into(),
        ...
        let json: serde_json::Value = serde_json::from_str(&received).unwrap();
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "ai-web",
    ...
    fn inject_capabilities_adds_default_model_to_session_new() {
        ...
        assert_eq!(json["params"]["_meta"]["modelId"], "ai-3-fast");
        assert!(json["params"]["_meta"].get("yoloMode").is_none());
    }
    #[test]
    ...
    fn extract_model_id_from_set_model_returns_value() {
        let payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"sessionId":"sess-123","modelId":"ai-3-fast"}}}}"#,
            AGENT_METHOD_NAMES.session_set_model
        );
        assert_eq!(
            extract_model_id_from_set_model(&pv(&payload)),
            Some("ai-3-fast".to_string())
        );
    }
    #[test]
    fn extract_model_id_from_set_model_handles_snake_case() {
        let payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"session_id":"sess-123","model_id":"ai-3"}}}}"#,
            AGENT_METHOD_NAMES.session_set_model
        );
        assert_eq!(
            extract_model_id_from_set_model(&pv(&payload)),
    ...
    fn extract_model_id_from_set_model_returns_none_for_other_methods() {
        let payload =
            r#"{"jsonrpc":"2.0","method":"other/method","id":1,"params":{"modelId":"ai-3"}}"#;
    ...
    fn patch_initialize_response_patches_current_model_id() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"modelState":{"currentModelId":"ai-3","availableModels":[]}}}}"#,
        );
        let default_model = Some("ai-3-fast".to_string());
    ...
    fn patch_initialize_response_preserves_other_fields() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"aiShell":true,"modelState":{"currentModelId":"ai-3","availableModels":[{"modelId":"ai-3"},{"modelId":"ai-3-fast"}]}}}}"#,
        );
        let default_model = Some("ai-3-fast".to_string());
        assert!(patch_initialize_response_model(&mut json, &default_model));
        assert_eq!(json["result"]["meta"]["aiShell"], true);
        assert_eq!(
            json["result"]["meta"]["modelState"]["currentModelId"],
            "ai-3-fast"
        );
    ...
    fn patch_initialize_response_noop_when_no_default_model() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"modelState":{"currentModelId":"ai-3"}}}}"#,
        );
        let before = json.clone();
    ...
    fn patch_initialize_response_noop_for_non_initialize_response() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"session_id":"sess-1","models":{"currentModelId":"ai-3","availableModels":[]}}}"#,
        );
        let before = json.clone();
        assert!(!patch_initialize_response_model(
            &mut json,
    ...
    fn inject_capabilities_does_not_override_existing_client_identifier() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_session_request_context(&mut json, &caps, "ai-tui", ClientId(1));
    ...
    fn inject_capabilities_adds_leader_client_id_to_session_load() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_session_request_context(&mut json, &caps, "ai-tui", ClientId(42));
        assert_eq!(
    ...
    fn inject_yolo_notification_adds_client_identifier() {
        ...
        assert_eq!(json["params"]["clientIdentifier"], "ai-tui");
    ...
    fn inject_client_identity_adds_identifier_to_initialize() {
        ...
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "ai-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(mutated, "should have injected the identifier");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "ai-tui");
    }
    #[test]
    fn inject_client_identity_does_not_override_existing() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"ai-web"}}}"#,
        );
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "ai-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(!mutated, "existing identifier means nothing was injected");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "ai-web");
    }
    #[test]
    ...
    fn inject_client_identity_preserves_existing_meta() {
        ...
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "ai-code-extension");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(mutated);
        assert_eq!(json["params"]["_meta"]["foo"], "bar");
        assert_eq!(
    ...
    async fn set_model_updates_default_model_for_next_session_new() {
        ...
        write_message(
            ...
            &ClientMessage::Register {
                ...
                capabilities: ClientCapabilities {
                    yolo_mode: false,
                    default_model: Some("ai-original".to_string()),
                    ..Default::default()
                },
            },
        )
        ...
        let set_model_payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"sessionId":"sess-1","modelId":"ai-4.5"}}}}"#,
            AGENT_METHOD_NAMES.session_set_model
        );
        write_message(
            &mut writer,
        ...
        let json: serde_json::Value = serde_json::from_str(&forwarded).unwrap();
        assert_eq!(
            json["params"]["_meta"]["modelId"], "ai-4.5",
    ...
    async fn high_throughput_replay_no_drops() {
        ...
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let (mut reader, mut writer) = connect_and_register(&sock_path, "ai-tui").await;
    ...
    async fn models_update_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_x.ai/models/update","params":{"currentModelId":"ai-new","availableModels":[{"modelId":"ai-new","name":"Ai New"}]}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a.as_deref().is_some_and(|p| p.contains("ai-new")),
            "client A must receive the models broadcast, got {got_a:?}"
        );
        assert!(
            got_b.as_deref().is_some_and(|p| p.contains("ai-new")),
    ...
    async fn mcp_servers_updated_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_x.ai/mcp/servers_updated","params":{"method":"x.ai/mcp/servers_updated","params":{"mcpServers":[{"name":"ai_com_slack","source":"managed"}]}}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a
                .as_deref()
                .is_some_and(|p| p.contains("ai_com_slack")),
    ...
    fn inject_capabilities_sets_code_nav_enabled_true() {
        ...
        let mut json = pv(payload);
        inject_session_request_context(&mut json, &caps, "ai-web", ClientId(1));
        assert_eq!(
    ...
    fn inject_capabilities_sets_code_nav_enabled_false() {
        ...
        let payload = r#"{"jsonrpc":"2.0","method":"session/new","id":1,"params":{"cwd":"/repo","_meta":{"clientIdentifier":"ai-tui"}}}"#;
        let mut json = pv(payload);
        inject_session_request_context(&mut json, &caps, "ai-tui", ClientId(1));
        assert_eq!(
            json["params"]["_meta"]["codeNavEnabled"],
    ...
    fn inject_capabilities_two_clients_stay_isolated() {
        ...
        inject_session_request_context(&mut web_json, &web_caps, "ai-web", ClientId(1));
        let mut tui_json = pv(session_new);
        inject_session_request_context(&mut tui_json, &tui_caps, "ai-tui", ClientId(2));
        assert_eq!(
