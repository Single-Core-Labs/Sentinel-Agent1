...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.ai/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** ai process
    /// sharing the same `~/.ai` (the writer may also be this process;
...
/// Watches `~/.ai/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.ai/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.ai/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let ai_home_buf = ai_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(ai_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(ai_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(ai_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(ai_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %ai_home.display(),
                    error = %e,
                    "failed to watch ai home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.ai/config.toml` so the loop above
        // watches `<cwd>/.ai/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.ai/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.ai/config.toml` trigger a
    ...
    /// `<cwd>/.ai/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.ai/` does not yet
/// exist at session-open time, the `.ai/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.ai/` followed by a
/// write to `<cwd>/.ai/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let ai_dir = cwd.join(".ai");
    if let Err(e) = debouncer
        .watcher()
        .watch(&ai_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .ai directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let ai_dir = cwd.join(".ai");
    if let Err(e) = debouncer.watcher().unwatch(&ai_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .ai directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.ai/` hasn't
...
const VENDOR_CONFIG_ROOT_NAMES: &[&str] = &[".ai", ".agents", ".claude", ".cursor"];

/// Vendor roots (by name or `ai_home`) must use scoped watches — they can
/// contain large non-skill trees (`worktrees/`, etc.).
fn is_vendor_config_root(dir: &Path, ai_home: &Path) -> bool {
    if paths_equal(dir, ai_home) {
        return true;
    }
    dir.file_name()
        .and_then(|n| n.to_str())
...
fn project_ai_refresh_dirs(project_root: &Path) -> Vec<(PathBuf, RecursiveMode)> {
    let project_ai = project_root.join(".ai");
    let mut dirs = vec![(project_ai.clone(), RecursiveMode::NonRecursive)];
    dirs.extend(vendor_skill_refresh_dirs(&project_ai));
    dirs
}
...
fn plan_skills_watch_targets(
    ...
    for dir in dirs_to_watch {
        if is_vendor_config_root(dir, ai_home) {
...
/// Watches project `.ai` skills/commands/workflows for mid-session discovery.
///
...
impl ProjectDiscoveryWatcher {
    pub(crate) fn start(cwd: &Path) -> Option<(Self, mpsc::UnboundedReceiver<DiscoveryChange>)> {
        ...
        let project_ai = project_root.join(".ai");
        let (tx, rx) = mpsc::unbounded_channel();
        let project_ai_for_events = project_ai.clone();
        let mut debouncer =
            new_filtered_debouncer(SKILLS_DEBOUNCE, move |res: DebounceEventResult| {
                ...
                for event in events
                    .iter()
                    .filter(|event| event.path.starts_with(&project_ai_for_events))
                {
        ...
        let initial = if project_ai.is_dir() {
            project_ai.clone()
        } else {
            project_root.clone()
        };
        ...
        let refresh_dirs = project_ai_refresh_dirs(&project_root);
...
impl SkillsFileWatcher {
    ...
    /// [`collect_skill_config_dirs`](sentinel_ai_agent::prompt::skills::collect_skill_config_dirs).
    ///
    ...
    pub fn start(
        ...
        let ai_home = sentinel_ai_tools::util::ai_home::ai_home();
        ...
        let dirs_to_watch = sentinel_ai_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &ai_home,
            config_paths,
        ...
        Self::start_with_dirs(&dirs_to_watch, &ai_home, project_root.as_deref())
    }
    ...
    pub fn start_with_dirs(
        ...
        let plan = plan_skills_watch_targets(dirs_to_watch, ai_home, project_root);
...
mod tests {
    ...
    fn is_vendor_config_root_matches_known_names_at_any_tier() {
        ...
        let home = home.path();
        let ai_home = home.join(".ai");

        assert!(is_vendor_config_root(&ai_home, &ai_home));
        assert!(is_vendor_config_root(&home.join(".claude"), &ai_home));
        assert!(is_vendor_config_root(&home.join(".cursor"), &ai_home));
        assert!(is_vendor_config_root(&home.join(".agents"), &ai_home));
        assert!(is_vendor_config_root(
            &home.join("repo").join(".ai"),
            &ai_home
        ));
        assert!(is_vendor_config_root(
        ...
        assert!(!is_vendor_config_root(&home.join("my-skills"), &ai_home));
        assert!(!is_vendor_config_root(&home.join(".config"), &ai_home));
        assert!(!is_vendor_config_root(
            &home.join("repo").join("my-skills"),
            &ai_home
        ));

        let custom_home = home.join("custom-ai-home");
    ...
    #[test]
    fn project_ai_refresh_dirs_matches_vendor_layout() {
        let project = Path::new("/tmp/repo");
        let ai = project.join(".ai");
        let dirs = project_ai_refresh_dirs(project);

        assert_eq!(dirs.len(), 4);
        assert_eq!(dirs[0], (ai.clone(), RecursiveMode::NonRecursive));
        assert_eq!(
            &dirs[1..],
            [
                (ai.join("skills"), RecursiveMode::Recursive),
                (ai.join("commands"), RecursiveMode::NonRecursive),
                (ai.join("workflows"), RecursiveMode::NonRecursive),
            ]
        );
        assert_eq!(dirs[1..], vendor_skill_refresh_dirs(&ai));
    }
    ...
    fn start_with_dirs_keeps_parent_only_watch() {
        ...
        let ai_home = project.join("home-ai");
        fs::create_dir_all(&ai_home).unwrap();

        let plan = plan_skills_watch_targets(&[], &ai_home, Some(project));
        ...
        let (watcher, _rx) = SkillsFileWatcher::start_with_dirs(&[], &ai_home, Some(project))
    ...
    fn plan_skills_watch_targets_scopes_vendors_and_seeds_refresh() {
        ...
        let project_ai = project.join(".ai");
        let custom = project.join("my-skills");
        fs::create_dir_all(&project_claude).unwrap();
        fs::create_dir_all(&project_ai).unwrap();
        fs::create_dir_all(&custom).unwrap();

        let dirs = vec![project_claude.clone(), project_ai.clone(), custom.clone()];
        let plan = plan_skills_watch_targets(&dirs, &ai_home, Some(project));

        assert_eq!(
            plan.vendor_roots,
            vec![project_claude.clone(), project_ai.clone()]
        );
        ...
        let mut expected_refresh: Vec<(PathBuf, RecursiveMode)> =
            vendor_skill_refresh_dirs(&project_claude)
                .into_iter()
                .chain(vendor_skill_refresh_dirs(&project_ai))
                .collect();
    ...
    fn plan_skills_watch_targets_multi_vendor_refresh_fanout() {
        let ai_home = PathBuf::from("/home/u/.ai");
        let a = PathBuf::from("/repo/.claude");
        let b = PathBuf::from("/repo/.agents");
        let plan = plan_skills_watch_targets(&[a.clone(), b.clone()], &ai_home, None);
    ...
    fn plan_skills_watch_targets_seeds_all_missing_project_vendor_roots() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let ai_home = project.join("elsewhere").join(".ai");
        let plan = plan_skills_watch_targets(&[], &ai_home, Some(project));
    ...
    fn plan_skills_watch_targets_does_not_double_seed_present_vendor_roots() {
        ...
        let plan = plan_skills_watch_targets(&present, &ai_home, Some(project));
    ...
    #[test]
    fn plan_skills_watch_targets_parent_watches_project_when_ai_present_siblings_missing() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let ai_home = project.join("home-ai");
        let project_ai = project.join(".ai");
        ...
        let plan = plan_skills_watch_targets(
            std::slice::from_ref(&project_ai),
            &ai_home,
            Some(project),
        );

        assert_eq!(plan.vendor_roots, vec![project_ai.clone()]);
        assert_eq!(plan.project_parent_watch.as_deref(), Some(project));
        assert!(
            !plan
                .refresh_dirs
                .contains(&(project_ai.clone(), RecursiveMode::NonRecursive))
        );
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        let ai = Path::new("/tmp/project/.ai");
        assert_eq!(
            discovery_change_for_path(ai),
        ...
        assert_eq!(
            discovery_change_for_path(&ai.join("skills")),
            Some(DiscoveryChange::Skills)
        );
        assert_eq!(
            discovery_change_for_path(&ai.join("commands")),
            Some(DiscoveryChange::Skills)
        );
        assert_eq!(
            discovery_change_for_path(&ai.join("workflows")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&ai.join("workflows/review.rhai")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&ai.join("skills/review/SKILL.md")),
            Some(DiscoveryChange::Skills)
        );
    }
    ...
    /// A write to `<ai_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another ai process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.ai/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.ai/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.ai/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_ai = new_cwd.path().join(".ai");
        fs::create_dir_all(&project_ai).unwrap();
        fs::write(project_ai.join("config.toml"), "").unwrap();
