...
fn with_env_var<T>(name: &str, value: &str, f: impl FnOnce() -> T) -> T {
    ...
    match previous {
        Some(prev) => {
            unsafe {
                unsafe { std::env::set_var(name, prev) };
            }
        }
        None => {
            unsafe {
...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "AI_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$AI_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${AI_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$AI_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$AI_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the AI_MEMORY env var.
...
/// Run `f` with AI_MEMORY explicitly unset.
fn without_ai_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("AI_MEMORY", None, f)
}
/// Run `f` with AI_MEMORY set to a specific value.
fn with_ai_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("AI_MEMORY", Some(value), f)
}
#[test]
...
fn memory_config_env_var_zero_does_not_enable() {
    with_ai_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "AI_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_ai_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "AI_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_ai_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "AI_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_ai_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "AI_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_ai_memory(
        ...
        || {
            ...
            assert!(
                mem.enabled,
                "CLI --experimental-memory should override AI_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_ai_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "--no-memory should override --experimental-memory, AI_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_ai_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(!mem.enabled, "--no-memory should override AI_MEMORY=1");
        },
    );
}
#[test]
...
"#;
        ...
        assert_eq!(mem.flush.flush_model.as_deref(), Some("ai-4"));
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$AI_TEST_USER $AI_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the AI_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with AI_SUBAGENTS explicitly unset.
fn without_ai_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("AI_SUBAGENTS", None, f)
}
/// Run `f` with AI_SUBAGENTS set to a specific value.
fn with_ai_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("AI_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_ai_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
...
fn subagent_limit_counts_resolve_env_over_toml_over_remote_over_default() {
    use sentinel_ai_tools::implementations::ai_build::task::admission;
    ...
    with_ai_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(!sa.enabled, "AI_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_ai_subagents(
        ...
        || {
            ...
            assert!(
                !sa.enabled,
                "AI_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_ai_subagents(
        ...
        || {
            ...
            assert!(
                sa.enabled,
                "--subagents CLI flag should override AI_SUBAGENTS=0"
            );
        },
    );
}
...
fn subagents_config_models_parsed() {
    without_ai_subagents(|| {
        ...
        assert_eq!(sa.models.get("explore").unwrap(), "ai-3-fast");
        assert_eq!(sa.models.get("plan").unwrap(), "ai-4.5");
    });
}
#[test]
    ...
    with_ai_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(sa.enabled, "AI_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "ai-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        ...
        managed_mcps,
        || with_env_var_opt("AI_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    with_env_var_opt(
        ...
        ws,
        || with_env_var_opt(
            "AI_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "AI_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("AI_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_ai_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_ai_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "AI_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("AI_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_ai_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_ai_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_ai_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_ai_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "AI_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".ai/prompts/impl.md"
        "#;
    ...
    assert_eq!(researcher.model.as_deref(), Some("ai-3"));
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".ai").join("roles");
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".ai").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".ai").join("bundled");
    std::fs::create_dir_all(workspace.join(".ai").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".ai").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".ai").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".ai").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".ai/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".ai/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".ai/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".ai/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".ai/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".ai/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".ai/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".ai/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.ai/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
/// a repo `.ai/config.toml` structurally cannot supply one.)
#[test]
...
cli_chat_proxy_base_url = "https://cli-chat-proxy.ai.com/v1"

[model.ai-build]
base_url = "https://inference.acme-corp.example/sentinel/v1"
...
[endpoints]
deployment_key = "sentinel-token-ENTERPRISE"
sentinel_api_base_url = "https://inference.acme-corp.example/sentinel/v1"
...
"#,
    ...
    assert_eq!(
            ...
            "https://cli-chat-proxy.ai.com/v1/deployment/config"
        );
    assert!(
            !cfg.endpoints
...
/// RCE guard: a project `.ai/config.toml` must never source
...
fn apply_requirements_value_overrides_user_settings() {
    ...
    assert_eq!(
            ...
            cfg.endpoints.sentinel_api_base_url
        );
    assert_eq!(
...
#[test]
fn validate_hooks_path_rejects_outside_ai_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
            msg.contains("must be under ~/.ai/"),
            "should mention ~/.ai/ restriction, got: {msg}"
        );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let ai_home = crate::util::ai_home::ai_home();
    let traversal = format!("{}/../evil", ai_home.display());
...
#[test]
fn validate_hooks_path_accepts_ai_hooks_subdir() {
    let ai_home = crate::util::ai_home::ai_home();
    let valid_path = ai_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.ai/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use sentinel_ai_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
/// disableBypassPermissionsMode (-> features.disable_yolo) must NOT clamp the user's own ai yolo.
#[test]
...
fn simulate_release_build() -> sentinel_ai_test_support::EnvGuard {
    sentinel_ai_test_support::EnvGuard::set(sentinel_ai_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn base_resolver_without_project_cwd_keeps_project_files_out() {
    let tmp = tempfile::tempdir().unwrap();
    write_subagent_definitions(&tmp.path().join(".ai"), &[("project", "Project")]);
    let base = SubagentsConfig::resolve_base_with_sources(
        false,
...
#[test]
fn explicit_ai_root_is_the_only_user_source() {
    let tmp = tempfile::tempdir().unwrap();
    let ambient = tmp.path().join("ambient-home/.ai");
    let configured = tmp.path().join("configured-ai-home");
...
/// verdicts untouched). AI_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores AI_HOME even on panic). No user-global
/// `$AI_HOME/config.toml` is seeded: `ai_home()` is `OnceLock`-cached,
...
/// in `sentinel-ai-agent`. An ABSOLUTE plugin path is used so the merged
...
/// AI_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use sentinel_ai_agent::plugins::{TrustStore, discover_plugins};
    use sentinel_ai_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("AI_HOME", home.path());
    let _flag = EnvGuard::unset("AI_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. AI_HOME-isolated (empty
/// store); AI_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
fn kill_switched_cold_cwd_stays_allowed_through_plugins_config_read() {
    ...
    let ai = repo.path().join(".ai");
    std::fs::create_dir_all(&ai).unwrap();
    std::fs::write(ai.join("config.toml"), "[plugins]\npaths = [\"./proj-plugin\"]\n")
        .unwrap();
    let cwd = repo.path();
    let remote = crate::util::config::RemoteSettings {
...
/// Writeback requires ai.com auth: remote may advertise it, but a non-sentinel
/// credential is downgraded to `Local`.
#[test]
#[serial_test::serial]
fn from_remote_gated_requires_sentinel_auth_for_writeback() {
