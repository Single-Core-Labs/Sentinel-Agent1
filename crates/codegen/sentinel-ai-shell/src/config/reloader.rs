...
use crate::auth::{AiAuth, read_auth_json};
...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.ai/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.ai/config.toml`,
    ...
    /// (`<cwd>/.ai/config.toml`, `<cwd>/.mcp.json`, or
    ...
    /// ai process sharing the home dir). The agent should consult the cache
    /// thrash unrelated sessions.
    ProjectMcpServersChanged {
        /// The project root whose `.ai/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.ai/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub(crate) async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.ai_home.join("auth.json");
            ...
            // same tick (e.g. `~/.ai/config.toml` AND
    ...
    pub(crate) fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.ai_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.ai/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.ai/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.ai/config.toml` | `<cwd>`           |
...
/// for `.ai/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.ai/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `ai inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> sentinel_ai_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> sentinel_ai_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<AiAuth>, Deref coercion
    ...
    /// The hash must reflect ancestor `.ai/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".ai").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .ai/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.ai/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.ai/skills".to_string()]);
...
"#,
        ...
        assert!(yolo);
        assert_eq!(fork.as_deref(), Some("ai-4.5"));
    }
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.ai/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.ai/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.ai/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.ai/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
