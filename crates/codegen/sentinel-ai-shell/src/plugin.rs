...
use sentinel_ai_agent::plugins::discovery::PluginScope;
use sentinel_ai_agent::plugins::git_install::{self, UpdateStatus};
...
use sentinel_ai_plugin_marketplace::git::{self, SourceCacheLease};
use sentinel_ai_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.ai/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &sentinel_ai_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `ai plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `ai plugin install ./{name}`, or add a \
                     source with `ai plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `ai plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `ai plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
pub(crate) fn marketplace_require_sha() -> bool {
    sentinel_ai_config::load_effective_config_disk_only()
        .map(|c| sentinel_ai_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| sentinel_ai_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &sentinel_ai_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.ai/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user ai via user_ai_home() (None when no home resolves) and
    // home separately, so removal still runs from $AI_HOME when no home dir
    // exists, and never touches a cwd-relative .ai.
    let home = dirs::home_dir();
    let ai = sentinel_ai_config::user_ai_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref ai) = ai {
        settings_candidates.push(ai.join("settings.local.json"));
        settings_candidates.push(ai.join("settings.json"));
    }
    ...
    if let Some(ref ai) = ai {
        known_candidates.push(ai.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            registered_source_label(&git_source(
                "AI Official",
                "https://github.com/sentinel-org/plugin-marketplace.git"
            )),
            "AI Official (sentinel-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "AI Official (pin: sentry@sentinel-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "AI Official (sentinel-org/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - AI Official (sentinel-org/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn ambiguous_qualifier_error_lists_source_names() {
        let err = MarketplaceInstallError::AmbiguousQualifier {
            qualifier: "sentinel-org/plugin-marketplace".into(),
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("ai plugin install ./sentry"), "{msg}");
        assert!(msg.contains("ai plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        let err = MarketplaceInstallError::NameAmbiguous {
            name: "sentry".into(),
            candidates: vec!["AI Official (pin: sentry@sentinel-org/plugin-marketplace)".into()],
        };
        let msg = err.to_string();
        assert!(
        ...
        assert!(
            msg.contains("  - AI Official (pin: sentry@sentinel-org/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("ai plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    const OFFICIAL_URL: &str = "https://github.com/sentinel-org/plugin-marketplace.git";

    #[test]
    fn plan_install_qualifier_unknown_lists_registered_labels() {
        let sources = [
            git_source("AI Official", OFFICIAL_URL),
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "AI Official (sentinel-org/plugin-marketplace)".to_string(),
    ...
    fn plan_install_qualifier_ambiguous_lists_source_names() {
        let sources = [
            ...
            git_source("Mirror B", "git@github.com:sentinel-org/plugin-marketplace.git"),
        ];
        let err = plan_install(
            &sources,
            "sentry",
            Some("sentinel-org/plugin-marketplace"),
            |_| Ok(Vec::new()),
        )
        .expect_err("two sources share the owner/repo");
        match err {
            MarketplaceInstallError::AmbiguousQualifier { qualifier, sources } => {
                assert_eq!(qualifier, "sentinel-org/plugin-marketplace");
                assert_eq!(
                    sources,
    ...
    fn plan_install_qualifier_not_found_when_scan_lacks_name() {
        let sources = [git_source("AI Official", OFFICIAL_URL)];
        let err = plan_install(
            &sources,
            "sentry",
            Some("sentinel-org/plugin-marketplace"),
        ...
        match err {
            MarketplaceInstallError::QualifiedNameNotFound {
                ...
                assert_eq!(source_display, "AI Official");
            }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> sentinel_ai_workspace::permission::resolution::MarketplaceAllowlist {
        sentinel_ai_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = sentinel_ai_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        let name = resolve_qualified_source_name_with(&sources, "sentinel-org/plugin-marketplace")
            .expect("qualifier should match the official source");
        assert_eq!(name, "AI Official");
    }
