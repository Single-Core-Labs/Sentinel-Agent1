//! Tracks open TUI sessions in `~/.ai/active_sessions.json` for crash
...
// -- Public API (delegates to `_in` variants with default ai home) --------

/// Register a session as active (idempotent by session_id).
pub fn register(session: ActiveSession) -> io::Result<()> {
    register_in(&crate::util::ai_home::ai_home(), session)
}
...
pub fn try_unregister(session_id: &acp::SessionId) -> io::Result<bool> {
    try_unregister_in(&crate::util::ai_home::ai_home(), session_id)
}

/// Remove entries with dead PIDs and return them.
pub fn collect_crashed() -> io::Result<Vec<ActiveSession>> {
    collect_crashed_in(&crate::util::ai_home::ai_home())
}
