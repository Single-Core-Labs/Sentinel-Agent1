//! `ai inspect` — configuration introspection.
//!
//! Shows everything Ai discovers in the current directory: project
...
use sentinel_ai_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[ai_com_config]`
...
pub(crate) struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `AI_DISABLE_API_KEY_AUTH`).
...
pub(crate) struct ConfigSources {
    ...
    /// .ai/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `ai inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = sentinel_ai_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = sentinel_ai_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = sentinel_ai_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        ai_version: sentinel_ai_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if path
        .parent()
        .is_some_and(|parent| parent == ai_home.join("rules"))
        || has_rules_directory(file_path, ".ai")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = sentinel_ai_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        sentinel_ai_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `sentinel_ai_agent::prompt::agents_md::read_agents_config_with_paths`
    ...
    configs
        ...
        .map(|c| {
            let file_type =
                instruction_file_type(&c.file_path, &ai_home, imported, &extra_rule_prefixes);
            let scope = instruction_scope(&c.file_path, &ai_home, &vendor_homes, &workspace_root);
...
/// which handles both Ai TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path, project_trusted: bool) -> PermissionsReport {
    use sentinel_ai_workspace::permission::resolution;
...
/// (`[ai_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let ai_com_config = config
        .map(|c| c.ai_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: ai_com_config.api_key_auth_disabled(),
        disable_api_key_auth: ai_com_config.disable_api_key_auth,
        force_login_team_uuid: ai_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[sentinel_ai_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = sentinel_ai_tools::types::compat::CompatConfig::default();
    ...
    let config_layers = sentinel_ai_config::hook_config_layers();
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            use sentinel_ai_hooks::config::HookOrigin as O;
            ...
            let source = match sentinel_ai_hooks::config::hook_origin(h) {
                O::SystemManaged | O::Managed => ConfigSource::Managed {
                    path: Some(config_file(sentinel_ai_config::MANAGED_CONFIG_FILENAME)),
                },
                O::Requirements => ConfigSource::Managed {
                    path: Some(config_file(sentinel_ai_config::REQUIREMENTS_FILENAME)),
                },
                O::UserConfig => ConfigSource::ConfigToml {
                    path: config_file(sentinel_ai_config::USER_CONFIG_FILENAME),
                },
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &sentinel_ai_agent::plugins::PluginRegistry,
    skills_config: &sentinel_ai_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = sentinel_ai_agent::prompt::skills::list_skills_with_plugins(
...
fn slash_name_counts(
    skills: &[sentinel_ai_agent::prompt::skills::SkillInfo],
    ...
    for skill in skills.iter().filter(|s| s.user_invocable && s.enabled) {
        ...
        *counts
            .entry(
                sentinel_ai_tools::implementations::skills::skill::format_skill_name(skill)
                    .to_lowercase(),
            )
            .or_default() += 1;
    }
...
fn slash_collision(
    skill: &sentinel_ai_agent::prompt::skills::SkillInfo,
    ...
    let qualified =
        sentinel_ai_tools::implementations::skills::skill::format_skill_name(skill).to_lowercase();
...
fn skill_entry_source(s: &sentinel_ai_agent::prompt::skills::SkillInfo) -> ConfigSource {
...
fn list_agents(
    ...
    let agents = sentinel_ai_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[sentinel_ai_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                sentinel_ai_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                sentinel_ai_agent::plugins::PluginScope::Project => Scope::Project,
                sentinel_ai_agent::plugins::PluginScope::User => Scope::User,
                sentinel_ai_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: sentinel_ai_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    sentinel_ai_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = sentinel_ai_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `ai mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.ai/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_ai_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.ai_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `ai mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use sentinel_ai_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use sentinel_ai_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for (vendor, path) in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.ai"), false, &[]);
        ...
        for path in ["/repo/.ai/rules/team.md", r"C:\repo\.ai\rules\team.md"] {
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.ai"), false, &[]),
                "rules"
            );
        }
        for path in [
            ...
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.ai"), true, &[]),
                "rules"
            );
        }
        for path in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.ai"), true, &[]);
    ...
    #[test]
    fn ai_home_nested_in_workspace_keeps_direct_surfaces_global() {
        let ai_home = Path::new("/repo/config");
        let workspace = Path::new("/repo");
        for path in ["/repo/config/AGENTS.md", "/repo/config/rules/global.md"] {
            assert!(matches!(
                instruction_scope(path, ai_home, &[], workspace),
                Scope::Global
            ));
        }
        for path in [
            "/repo/config/.ai/rules/project.md",
    ...
    fn vendor_home_nested_in_workspace_keeps_direct_surfaces_global() {
        ...
        for path in ["/repo/.claude/rules/global.md", "/repo/.claude/CLAUDE.md"] {
            assert!(matches!(
                instruction_scope(path, Path::new("/other/ai"), &vendor_homes, workspace),
                Scope::Global
            ));
        }
        for path in [
    ...
    #[test]
    fn workspace_scope_wins_inside_ai_home() {
        let ai_home = Path::new("/custom/ai");
        let workspace = Path::new("/custom/ai/worktrees/repo");
        for path in [
            "/custom/ai/worktrees/repo/.cursor/rules/project.md",
            "/custom/ai/worktrees/repo/src/AGENTS.md",
        ] {
            assert!(matches!(
                instruction_scope(path, ai_home, &[], workspace),
                Scope::Project
        ...
        assert!(matches!(
            instruction_scope("/custom/ai/rules/global.md", ai_home, &[], workspace,),
            Scope::Global
        ));
    }

    #[test]
    fn custom_ai_home_rules_are_classified_as_rules() {
        assert_eq!(
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.ai/requirements.toml";
    ...
    fn config_warnings_inspect_smoke() {
        ...
        assert!(cfg.config_models.contains_key("ai-4.5"));

        let human = render_config_warnings(&warnings);
        assert!(human.contains("Config Warnings"), "{human}");
        assert!(
            human.contains("[model.\"ai-4.5\"] send_compactions_remaining"),
            "{human}"
        );
        assert!(
            human.contains("[model.\"ai-4.5\"] reasoning_effort"),
            "{human}"
        );
        ...
        assert_eq!(alias_warning["key"], "ai-4.5");
    ...
    fn skill_entry_source_maps_scopes() {
        let s = skill_fixture("a", "/repo/.ai/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.ai/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.ai/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(skill_entry_source(&s), ConfigSource::User { .. }));

        let s = skill_fixture(
            "d",
            "/home/u/.ai/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s),
        ...
        let s = skill_fixture("e", "/home/u/.ai/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.ai dirs.
        ...
        let registry = sentinel_ai_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
