...
use sentinel_ai_sampler::SamplerConfig;
use sentinel_ai_tools::implementations::ai_build;
use sentinel_ai_tools::registry::types::ToolConfig;

/// Production ai-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub(crate) fn to_bash_params_json(
        ...
        // production ai-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `AI_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `AI_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub(crate) fn resolve_params(
    ...
    ) -> sentinel_ai_tools::implementations::ai_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("AI_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| sentinel_ai_config::env_bool("AI_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `sentinel_ai_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `AiBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `AiBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "AiBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "AiBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"AiBuild:read_file"));
        assert!(ids.contains(&"AiBuild:search_replace"));
        assert!(ids.contains(&"AiBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"AiBuildHashline:hashline_read"));
        assert!(ids.contains(&"AiBuildHashline:hashline_edit"));
        assert!(ids.contains(&"AiBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            sentinel_ai_agent::config::AgentDefinition::plan(),
            sentinel_ai_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "AiBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"AiBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"AiBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"AiBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production ai-build must set the 10h foreground ceiling"
        );
    }
