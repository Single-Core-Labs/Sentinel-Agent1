//! Notification bridge: translates `sentinel-ai-tools` `ToolNotification` events
//! into `sentinel-ai-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use sentinel_ai_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use sentinel_ai_tools::types::output::{BashOutput, ToolOutput};
use sentinel_ai_workspace::session::file_state::FileStateTracker;
...
pub(crate) struct NotificationBridgeConfig {
    ...
    pub task_wake_suppressed: sentinel_ai_tools::reminders::task_completion::TaskWakeSuppressed,
    ...
    /// next-turn exit reminder. Ai-build leaves this `false` — its
...
async fn handle_scheduled_task_removed(
    config: &NotificationBridgeConfig,
    removed: sentinel_ai_tools::notification::ScheduledTaskRemoved,
    ...
    let result: Result<Box<serde_json::value::RawValue>, String> = async {
        ...
        let update = crate::session::storage::SessionUpdate::Ai(Box::new(notification));
...
async fn emit_current_mode_update(
    config: &NotificationBridgeConfig,
    mode: sentinel_ai_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::BashExecutionBackgrounded(bg) => {
            ...
            let _ = config.persistence.tx.send(PersistenceMsg::Update(
                crate::session::storage::SessionUpdate::Ai(Box::new(notification.clone())),
            ));
            let params = serde_json::to_value(&notification)
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == sentinel_ai_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    sentinel_ai_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    sentinel_ai_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = sentinel_ai_tools::reminders::wrap_reminder(&body);
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, sentinel_ai_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                let _ = config
                    ...
                    .tx
                    .send(PersistenceMsg::PlanModeState(snapshot));
                emit_current_mode_update(config, sentinel_ai_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
...
mod tests {
    use super::*;
    use sentinel_ai_tools::computer::types::TaskKind;
    use sentinel_ai_tools::types::TaskSnapshot;
    ...
    fn make_test_config_full() -> (
        NotificationBridgeConfig,
        mpsc::UnboundedReceiver<sentinel_acp_lib::AcpClientMessage>,
    ...
    fn make_test_config_full_raw() -> (
        ...
        let gateway = sentinel_acp_lib::AcpAgentGatewaySender::new(gateway_tx);
        ...
        let config = NotificationBridgeConfig {
            ...
            task_completion_reservations:
                sentinel_ai_tools::reminders::task_completion::TaskCompletionReservations::default(),
            task_wake_suppressed:
                sentinel_ai_tools::reminders::task_completion::TaskWakeSuppressed::default(),
    ...
    fn task_completed_will_wake(
        gateway_rx: &mut mpsc::UnboundedReceiver<sentinel_acp_lib::AcpClientMessage>,
    ) -> Option<bool> {
        while let Ok(msg) = gateway_rx.try_recv() {
            if let sentinel_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "x.ai/task_completed"
    ...
    async fn task_completed_notification_stamps_will_wake() {
        ...
        *config
        ...
        while let Ok(message) = persistence_rx.try_recv() {
            if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ai(update)) =
                message
                && matches!(
                    &update.update,
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(sentinel_ai_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            sentinel_ai_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
        ...
        match msg {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ai(notif)) => {
                assert!(matches!(
                    &notif.update,
                ...
                assert!(
                    ...
                    "persisted AI bridge lines must carry an eventId"
                );
            }
            _ => panic!("expected PersistenceMsg::Update(Ai(ScheduledTaskCreated))"),
        }
    }
    ...
    async fn bash_output_chunk_forwards_live_without_persisting() {
        ...
        let notification = ToolNotification::BashOutputChunk(
            sentinel_ai_tools::notification::types::BashOutputChunk {
                base: sentinel_ai_tools::notification::types::BashNotificationBase {
        ...
        match gateway_rx.try_recv().expect("chunk must be broadcast") {
            sentinel_acp_lib::AcpClientMessage::SessionNotification(args) => {
                assert!(
                    args.request
                        .meta
                        .as_ref()
    ...
    async fn live_in_progress_event_id_is_not_a_prepare_replay_cursor() {
        ...
        let live_id = match gateway_rx.try_recv().unwrap() {
            sentinel_acp_lib::AcpClientMessage::SessionNotification(args) => args
                .request
                .meta
                .as_ref()
                .and_then(|m| m.get("eventId"))
    ...
    async fn scheduled_task_removed_is_persisted() {
        let (config, _gateway_rx, mut persistence_rx, _cmd_rx) = make_test_config_full();
        let removed = sentinel_ai_tools::notification::ScheduledTaskRemoved {
            task_id: "loop-1".into(),
            generation: "generation-a".into(),
            revision: 2,
        };
        ...
        match msg {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ai(notif)) => {
                ...
                assert!(
                    sentinel_persisted_event_id(&notif).is_some(),
            ...
            _ => panic!("expected PersistenceMsg::Update(Ai(ScheduledTaskDeleted))"),
        }
    }
    #[tokio::test]
    async fn acknowledged_scheduler_removal_appends_before_ack_and_broadcast() {
        ...
        let persistence = async {
            let PersistenceMsg::AppendUpdateDurablyAndAck {
                update: crate::session::storage::SessionUpdate::Ai(notification),
                respond_to,
        ...
        assert!(matches!(
            gateway_rx.try_recv(),
            Ok(sentinel_acp_lib::AcpClientMessage::ExtNotification(_))
        ));
    }
    fn sentinel_persisted_event_id(
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            sentinel_ai_tools::notification::types::BashExecutionBackgrounded {
                base: sentinel_ai_tools::notification::types::BashNotificationBase {
        ...
        match persistence_rx.try_recv().expect("must persist") {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ai(notif)) => {
                assert!(sentinel_persisted_event_id(&notif).is_some());
            }
            _ => panic!("expected Ai update"),
        }
    }
    #[tokio::test]
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            sentinel_ai_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
        ...
        let sentinel_acp_lib::AcpClientMessage::ExtNotification(fired) = fired else {
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        let notification =
            ToolNotification::PlanModeExited(sentinel_ai_tools::notification::types::PlanModeExited {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let sentinel_acp_lib::AcpClientMessage::SessionNotification(args) = msg
    ...
    /// Default (ai) polarity: the exit_plan_mode tool result is the model's
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification = ToolNotification::PlanModeEntered(
            sentinel_ai_tools::notification::types::PlanModeEntered {
    ...
    async fn task_completed_notification_is_frame_bounded() {
        ...
        while let Ok(msg) = persistence_rx.try_recv() {
            if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Ai(saved)) = msg
            {
