...
// rmcp is quarantined in sentinel-ai-mcp; see that crate's docs.
use sentinel_ai_mcp::rmcp;
// `wire::MCP_CALL` is the one cross-SDK contract literal; the agent-only siblings
// live in `mcp_methods` below.
use sentinel_ai_mcp::wire;
...
/// `sentinel_ai_mcp::wire`), these methods are private to the agent↔client channel and
...
pub async fn notify_servers_updated(
    gateway: &sentinel_acp_lib::AcpAgentGatewaySender,
...
pub(crate) async fn build_mcp_status(
    ...
    tool_bridge: &Arc<sentinel_ai_tools::bridge::ToolBridge>,
    event_writer: Option<&sentinel_file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(sentinel_file_utils::events::Event::McpHealthCheck {
...
pub(crate) async fn init_agent_mcp_pool(
    ...
    let noop = sentinel_file_utils::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from sentinel-ai-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `sentinel-ai-mcp` directly.
...
impl sentinel_ai_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<sentinel_ai_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(sentinel_ai_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<sentinel_ai_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(sentinel_ai_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(sentinel_ai_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(sentinel_ai_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
async fn handle_setup(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let allowlist = &sentinel_ai_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
mod tests {
    ...
    fn test_mcp_list_response_serialization() {
            ...
            servers: vec![
                McpServerEntry {
                    ...
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("Ai CLI".to_string()),
                    },
                    source_label: None,
                    setup: None,
                    setup_values: None,
        ...
        assert_eq!(json["servers"][0]["scopeName"], "Ai CLI");
