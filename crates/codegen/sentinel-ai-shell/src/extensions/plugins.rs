...
pub(crate) fn loaded_plugin_to_info(plugin: &sentinel_ai_agent::plugins::LoadedPlugin) -> PluginInfo {
    use sentinel_ai_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &sentinel_ai_agent::plugins::PluginOrigin) -> PluginOrigin {
    use sentinel_ai_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectAi => PluginOrigin::ProjectAi,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserAi => PluginOrigin::UserAi,
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "x.ai/plugins/action" => {
            let req: sentinel_hooks_plugins_types::PluginsActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use sentinel_ai_agent::plugins::PluginOrigin as AgentOrigin;
    use sentinel_ai_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> sentinel_ai_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        sentinel_ai_agent::plugins::LoadedPlugin {
    ...
    fn info_carries_origin_and_marketplace_display_name() {
        let plugin = make_loaded_plugin(AgentOrigin::MarketplaceInstall {
            source_name: Some("AI Official".to_string()),
        ...
        assert_eq!(info.marketplace_source.as_deref(), Some("AI Official"));
    }
