    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session rename requires AI OAuth credentials"),
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `ai sessions delete <id>` CLI path.
...
async fn soft_delete_chat_conversation(agent: &MvpAgent, conversation_id: &str) -> ExtResult {
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session delete requires AI OAuth credentials"),
            other => acp::Error::internal_error()
...
/// hot-reload watcher when `<cwd>/.ai/config.toml`,
...
/// Hot-reload the model catalog from `~/.ai/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `x.ai/commands/list` (the pull used by ai-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .ai/plugins under it, plus
