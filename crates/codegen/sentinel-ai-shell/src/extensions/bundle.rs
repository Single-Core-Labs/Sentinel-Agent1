...
mod tests {
    ...
    fn test_auth() -> crate::auth::AiAuth {
        crate::auth::AiAuth {
            key: "token".to_string(),
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".ai/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".ai/roles")).unwrap();
        std::fs::write(
            project_root.join(".ai/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".ai/roles/reviewer.toml"),
