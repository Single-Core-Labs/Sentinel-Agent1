...
const WORKTREE_EXT_LOG: &str = "sentinel_worktree";
...
pub async fn handle(
    agent: &MvpAgent,
    ops: &sentinel_ai_workspace::WorkspaceOps,
    args: &acp::ExtRequest,
) -> ExtResult {
    ...
    match args.method.as_ref() {
        ...
        "x.ai/git/worktree/create_from_worktree" => {
            ...
            let result = ops
                .dispatch(
                    &sentinel_ai_workspace::workspace_ops::PrepareWorktreeFromWorktreeReq {
                        inner: req.clone(),
                    },
                    None,
                )
        ...
        "x.ai/git/worktree/create_from_worktree_sync" => {
            ...
            let resolved_root = ops
                .dispatch(
                    &sentinel_ai_workspace::workspace_ops::GitResolveRootReq {
            ...
            if let Some(git_root) = resolved_root {
                let vcs_kind = ops
                    .dispatch(
                        &sentinel_ai_workspace::workspace_ops::DetectVcsKindReq {
                            path: git_root.clone(),
                        },
                        None,
                    )
                    .await
                    .unwrap_or(sentinel_ai_workspace::session::git::VcsKind::Git);
                if vcs_kind.is_jj() {
            ...
            let result = ops
                .dispatch(
                    &sentinel_ai_workspace::workspace_ops::CreateWorktreeFromWorktreeSyncReq {
                        inner: req.into_wire(),
                    },
                    None,
                )
        ...
        "x.ai/git/worktree/resume_session" => {
            ...
            let agent_id = sentinel_ai_telemetry::id::agent_id();
        ...
        "x.ai/git/worktree/list" => {
            let req: sentinel_ai_workspace::workspace_ops::WorktreeListReq =
        ...
        "x.ai/git/worktree/show" => {
            ...
            let op = sentinel_ai_workspace::workspace_ops::WorktreeShowReq {
        ...
        "x.ai/git/worktree/gc" => {
            ...
            let op = sentinel_ai_workspace::workspace_ops::WorktreeGcReq {
                dry_run: req.dry_run,
                max_age_secs,
                force: req.force,
            };
        ...
        "x.ai/git/worktree/db/stats" => {
            let result = ops
                .dispatch(
                    &sentinel_ai_workspace::workspace_ops::WorktreeDbStatsReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "x.ai/git/worktree/db/rebuild" => {
            let result = ops
                .dispatch(
                    &sentinel_ai_workspace::workspace_ops::WorktreeDbRebuildReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "x.ai/git/worktree/db/path" => {
            let result = ops
                .dispatch(
                    &sentinel_ai_workspace::workspace_ops::WorktreeDbPathReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
mod tests {
    ...
    fn list_request_with_filters() {
        let json = r#"{"repo": "sentinel", "type": ["session", "fork"], "includeAll": true}"#;
        let req: ListWorktreeRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.repo.as_deref(), Some("sentinel"));
    ...
    fn db_path_response_serializes() {
        let resp = WorktreeDbPathResponse {
            path: "/home/user/.ai/worktrees.db".into(),
        };
        let json = serde_json::to_string(&resp).unwrap();
        assert!(json.contains("\"path\":\"/home/user/.ai/worktrees.db\""));
    }
