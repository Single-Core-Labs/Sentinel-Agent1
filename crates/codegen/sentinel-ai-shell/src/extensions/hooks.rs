...
use sentinel_ai_hooks::event::{HookEventEnvelope, HookEventName};
use sentinel_ai_hooks::matcher::HookMatcher;
use sentinel_hooks_plugins_types::{HookEvent, HookHandlerType, HookInfo};
...
pub(crate) fn hook_spec_to_info(spec: &sentinel_ai_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: sentinel_ai_hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[sentinel_ai_hooks::event::HookEventName] = &[
    sentinel_ai_hooks::event::HookEventName::PreToolUse,
    sentinel_ai_hooks::event::HookEventName::Stop,
    sentinel_ai_hooks::event::HookEventName::SubagentStop,
];
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern)
            if event.traits().matcher == sentinel_ai_hooks::event::MatcherPolicy::Ignored =>
        {
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "x.ai/hooks/action" => {
            let req: sentinel_hooks_plugins_types::HooksActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    ...
    fn advertised_blocking_events_are_gates() {
        use sentinel_ai_hooks::event::GateKind;
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use sentinel_ai_hooks::event::{HookEventEnvelope, HookPayload};
