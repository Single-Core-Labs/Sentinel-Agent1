...
fn handle_get_api_key() -> ExtResult {
    let key = crate::agent::auth_method::read_sentinel_api_key_env().ok();
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `ai-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a ai-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("ai-asset:///{key}")),
        None => None,
    };
    to_raw_response(&AuthInfoResponse {
        ...
        // matching `AuthManager::allows_data_collection` / AiAuth Default.
