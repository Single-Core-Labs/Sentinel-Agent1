...
//!
//! Fetches the authenticated user's Ai Build billing configuration
...
/// Current billing configuration for Ai Build coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `AiBuildBillingConfig` fields
...
/// against both the new `GetAiCreditsConfig` and the legacy
/// `GetAiBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// (`GetAiCreditsConfig.prepaid_balance`); absent in the legacy billing
    /// shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (shared weekly/monthly
    /// pool). From `AiCreditsConfig.is_unified_billing_user`, which billing
    /// sets from remote settings `unified_consumer_billing_enabled`. `None` when
    /// absent (legacy `GetAiBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/ai/credits` + auto-topup-rule).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BillingConfigResponse {
    ...
    /// User-friendly subscription tier name (e.g. "SuperAi Heavy").
...
///
/// Keeps history to a count + the most recent period so `~/.ai/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_sentinel_auth(
        &agent.auth_manager,
        "Authentication required to fetch billing data",
        "Billing data requires auth with ai.com. Run `ai login` to authenticate.",
    )?;
    ...
    // forwards to the backend `GetAiCreditsConfig`.
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-AI-Token-Auth",
            crate::auth::AiComConfig::default().token_header,
        )
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_sentinel_auth(
        ...
        "Auto top-up data requires auth with ai.com. Run `ai login` to authenticate.",
    )?;
...
mod tests {
    ...
    fn billing_unified_log_ctx_includes_credits_and_collapses_history() {
        ...
        assert_eq!(ctx["subscriptionTier"], "SuperAi");
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetAiCreditsConfig` response: percentage-based usage,
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_AI_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
