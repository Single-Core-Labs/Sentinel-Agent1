...
//!
//! Repairs via [`sentinel_chat_state::compaction_utils::repair_history`]. Resident
...
use sentinel_chat_state::compaction_utils::HistoryRepairReport;
...
/// back atomically. `ai_root` is injectable for tests.
async fn repair_on_disk(ai_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &ai_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(ai_root.to_path_buf());
    ...
    let report = sentinel_chat_state::compaction_utils::repair_history(&mut chat_history);
