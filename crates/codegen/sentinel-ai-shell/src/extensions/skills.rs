...
use sentinel_ai_agent::prompt::skills::{
...
fn discover_auto_sources(cwd: &str, skills: &[SkillInfo]) -> Vec<(String, usize)> {
    ...
    for subdir in &subdirs {
        try_add_source(ai_home.join(subdir), None);
    }
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "x.ai/skills/add" => {
            ...
            .await
            {
                sentinel_ai_telemetry::session_ctx::log_event(
                    sentinel_ai_telemetry::events::SkillAdded {
                        added_count: 0,
                        total_skills: 0,
                        success: false,
                    },
            ...
            sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::SkillAdded {
        ...
        "x.ai/skills/remove" => {
            ...
            {
                sentinel_ai_telemetry::session_ctx::log_event(
                    sentinel_ai_telemetry::events::SkillRemoved { success: false },
                );
            ...
            sentinel_ai_telemetry::session_ctx::log_event(sentinel_ai_telemetry::events::SkillRemoved {
                success: true,
            });
