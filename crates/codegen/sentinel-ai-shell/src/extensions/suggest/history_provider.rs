...
///
/// Priority order: local ai bash history > shell history > cross-CWD history.
...
fn scan_cross_cwd_prompts() -> Vec<String> {
    let sessions_dir = crate::util::ai_home::ai_home().join("sessions");
    ...
    for (dir, _) in dirs.iter().take(MAX_CROSS_CWD_DIRS) {
        ...
        let cwd = match crate::util::ai_home::decode_cwd_from_dirname(dir) {
