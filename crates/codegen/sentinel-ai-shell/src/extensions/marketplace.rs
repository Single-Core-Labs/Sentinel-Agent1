...
//! Delegates to `sentinel-ai-plugin-marketplace` crate for scanning and install logic.

use agent_client_protocol as acp;
use sentinel_hooks_plugins_types::{
...
fn load_filtered_marketplace_sources() -> Vec<sentinel_ai_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                sentinel_ai_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                sentinel_ai_plugin_marketplace::SourceKind::Local { path } => {
...
async fn handle_action(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            {
                Ok(outcome) => outcome,
                Err(e) => sentinel_hooks_plugins_types::ActionOutcome {
                    status: sentinel_hooks_plugins_types::OutcomeStatus::InternalError,
...
fn refresh_sources(
    sources: &[sentinel_ai_plugin_marketplace::MarketplaceSource],
    source_url_or_path: Option<&str>,
) -> sentinel_hooks_plugins_types::ActionOutcome {
    let mut refreshed = 0;
    ...
    for source in sources {
        if let Some(filter) = source_url_or_path {
            let identity = match &source.kind {
                ...
                sentinel_ai_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
            };
            if identity != filter {
                continue;
            }
        }
        if let sentinel_ai_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = sentinel_ai_plugin_marketplace::git::default_cache_root();
            if let Err(e) = sentinel_ai_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
    ...
    sentinel_hooks_plugins_types::ActionOutcome {
        status: sentinel_hooks_plugins_types::OutcomeStatus::Success,
        message: msg,
        requires_reload: false,
        requires_restart: false,
    }
...
async fn handle_update(
    ...
    use sentinel_ai_plugin_marketplace::installer;
    use sentinel_hooks_plugins_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &sentinel_ai_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            sentinel_ai_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            sentinel_ai_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match sentinel_ai_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        sentinel_ai_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = sentinel_ai_plugin_marketplace::git::default_cache_root();
            match sentinel_ai_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                sentinel_ai_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = sentinel_ai_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = sentinel_ai_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = sentinel_ai_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, sentinel_hooks_plugins_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(sentinel_ai_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            sentinel_ai_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match sentinel_ai_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    sentinel_ai_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &sentinel_ai_plugin_marketplace::MarketplaceSource,
    ...
    let scan = sentinel_ai_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = sentinel_ai_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match sentinel_ai_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: sentinel_ai_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.ai/config.toml`.
async fn handle_add_source(url: &str) -> sentinel_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &sentinel_ai_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            sentinel_ai_plugin_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
                ...
                return ActionOutcome {
                    ...
                    message: format!(
                        "{e}. Not a reachable git repository — to add it anyway (e.g. a \
                         VPN-gated host), run: ai plugin marketplace add {url} --force"
                    ),
                    requires_reload: false,
                    requires_restart: false,
                };
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if sentinel_ai_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        sentinel_ai_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = sentinel_ai_config::ai_home().join("config.toml");
    let ai_home = sentinel_ai_config::ai_home();
    let _save_guard = crate::util::config::lock_config_writes().await;
    let write = {
        let name = name.clone();
        tokio::task::spawn_blocking(move || {
            let _flock = acquire_init_lock(&ai_home).ok();
...
/// Remove a marketplace source from `~/.ai/config.toml` and uninstall all
/// plugins that were installed from it.
async fn handle_remove_source(source_url_or_path: &str) -> sentinel_hooks_plugins_types::ActionOutcome {
...
fn remove_source_locked(source_url_or_path: &str) -> sentinel_hooks_plugins_types::ActionOutcome {
    use crate::plugin;
    use sentinel_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
    ...
    let config_path = ai_home.join("config.toml");
    let is_official = sentinel_ai_plugin_marketplace::is_official_source_url(source_url_or_path);
...
/// Acquire an advisory exclusive `flock` on `<ai_home>/.config-init.lock`,
...
fn acquire_init_lock(ai_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(ai_home);
    let lock_path = ai_home.join(".config-init.lock");
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a sentinel_ai_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub(crate) fn purge_default_skills_installs(ai_home: &std::path::Path) {
    purge_default_skills_installs_impl(ai_home, || {
        sentinel_ai_agent::plugins::install_registry::InstallRegistry::try_load_from(
            sentinel_ai_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}

fn purge_default_skills_installs_impl(
    ai_home: &std::path::Path,
    load_registry: impl FnOnce() -> Result<
        sentinel_ai_agent::plugins::install_registry::InstallRegistry,
        sentinel_ai_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = ai_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(ai_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %ai_home.join(".config-init.lock").display(),
...
/// Auto-register the official AI marketplace source on first run.
///
...
pub(crate) fn ensure_official_marketplace_source(ai_home: &std::path::Path) {
    let config_path = ai_home.join("config.toml");
    ...
    // (settings.json / known_marketplaces.json) under ai_home. Scoped to
    // ai_home only (not ~/.claude) to keep tests hermetic; a user with the URL
    // solely in ~/.claude gets one duplicate entry that the UI dedupes by URL.
    let toml_sources = sentinel_ai_plugin_marketplace::load_sources(&parsed);
    let json_sources = sentinel_ai_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&ai_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, sentinel_ai_plugin_marketplace::SourceKind::Git { url, .. }
            if sentinel_ai_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            sentinel_ai_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                sentinel_ai_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = sentinel_ai_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
                "auto-registered official AI marketplace source"
            );
        }
        Ok(()) => {}
        Err(e) => {
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<sentinel_ai_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        sentinel_ai_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            sentinel_ai_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                sentinel_ai_plugin_marketplace::SourceKind::Git { url, .. }
                    if sentinel_ai_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            sentinel_ai_plugin_marketplace::SourceKind::Git { url, .. }
                if url == sentinel_ai_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_in_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"sentinel-official":{{"source":{{"source":"git","url":"{}"}}}}}}"#,
                sentinel_ai_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_extra_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"extraKnownMarketplaces":{{"sentinel-official":{{"source":{{"source":"git","url":"{}"}}}}}}}}"#,
                sentinel_ai_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod default_skills_purge_tests {
    use super::*;
    use sentinel_ai_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = sentinel_ai_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            version: Some("1.0.0".into()),
            description: Some("demo".into()),
            category: Some("dev".into()),
            author: Some("sentinel".into()),
            tags: vec!["cli".into()],
            ...
            components: Some(sentinel_hooks_plugins_types::PluginComponents {
                skills: vec![sentinel_hooks_plugins_types::ComponentItem::new(
                    "code-review",
