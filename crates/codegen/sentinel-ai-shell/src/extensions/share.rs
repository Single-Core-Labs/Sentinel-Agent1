...
use sentinel_ai_telemetry::id::agent_id;
...
async fn handle_share_session(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let auth = require_sentinel_auth_for_share(&agent.auth_manager)?;
...
async fn upload_share_data_to_gcs(
    ...
    if let Err(e) = sentinel_file_utils::gcs::upload_bytes_signed(
...
fn require_sentinel_auth_for_share(
    auth_manager: &crate::auth::AuthManager,
) -> Result<crate::auth::AiAuth, acp::Error> {
    super::auth_gate::require_sentinel_auth(
        auth_manager,
        "Authentication required to share session",
        "Share session is disabled. Run `ai login` to authenticate.",
    )
}
...
mod tests {
    ...
    fn make_auth_manager_with_token_expiring_in(
        ...
        // We must explicitly set oidc_issuer to a first-party AI issuer.
        // Only OIDC tokens against https://auth.x.ai (or the local-dev equivalent)
        // return true from is_sentinel_auth(). This is required for the share tests to
        // exercise the happy path through require_sentinel_auth_for_share.
        let auth = AiAuth {
    ...
    fn share_works_outside_the_5m_early_invalidation_window() {
        ...
        assert!(require_sentinel_auth_for_share(&mgr).is_ok());
    }
    ...
    fn share_succeeds_inside_the_5m_early_invalidation_window() {
        ...
        let res = require_sentinel_auth_for_share(&mgr);
        assert!(
            res.is_ok(),
            "require_sentinel_auth_for_share must succeed for a still-valid buffered AI token"
        );
    }
    ...
    fn share_fails_with_no_auth_at_all() {
        ...
        assert!(require_sentinel_auth_for_share(&mgr).is_err());
    }

    #[test]
    fn share_rejects_non_sentinel_auth_with_actionable_ai_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            AiComConfig::default(),
        ...
        // API key is the simplest non-AI credential (External and enterprise OIDC
        // are also rejected the same way).
        let non_sentinel = AiAuth {
            auth_mode: AuthMode::ApiKey,
            key: "sentinel-test-key".into(),
            create_time: Utc::now(),
            ..Default::default()
        };
        mgr.hot_swap(non_sentinel);

        let err = require_sentinel_auth_for_share(&mgr)
            .expect_err("non-AI accounts (API key, External, enterprise IdP) must be rejected");

        // This is the key assertion the review asked for: we must test the *exact*
        // actionable data string for the non-AI path (distinct from the generic
