...
//! AI extensions) and extract the notification payload from `params`.
//!
...
mod tests {
    ...
    fn dummy_gateway() -> sentinel_acp_lib::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        sentinel_acp_lib::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn handle_falls_back_to_id_lookup_for_divergent_cwd() {
        ...
        // Clean up the dir written under the real ai home.
        let _ = std::fs::remove_dir_all(&child_dir);
    }

    fn capturing_gateway() -> (
        sentinel_acp_lib::AcpAgentGatewaySender,
        tokio::sync::mpsc::UnboundedReceiver<sentinel_acp_lib::AcpClientMessage>,
    ) {
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
        (sentinel_acp_lib::AcpAgentGatewaySender::new(tx), rx)
    }
    ...
    fn sentinel_rewind(target: usize) -> String {
        format!(
