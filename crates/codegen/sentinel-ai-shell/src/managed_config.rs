...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    sentinel_ai_config::MANAGED_CONFIG_FILENAME,
    sentinel_ai_config::REQUIREMENTS_FILENAME,
    sentinel_ai_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    sentinel_ai_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            sentinel_ai_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", sentinel_ai_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            sentinel_ai_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// would just 401).
fn eligible_team_principal(auth: AiAuth) -> Option<AiAuth> {
...
/// config is a ai.com feature with one ai.com auth.
fn read_active_team_auth() -> Option<AiAuth> {
    let home = crate::util::ai_home::ai_home();
...
pub fn clear_orphan() {
    ...
    if sentinel_ai_config::fail_closed_policy_armed_at(&home) {
        tracing::info!(
...
/// files (TUI tick vs `ai login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `ai setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `AI_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("AI_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `sentinel-ai-http` classifier.
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = sentinel_ai_config::signed_policy::stored_envelope_nonce(
        &crate::util::ai_home::ai_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            sentinel_ai_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
/// Override with `AI_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("AI_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** AiBuildDeployment UUID (the id
...
/// Resolve deployment key from `AI_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (ai setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("AI_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.ai/`, trying the
...
/// read-only `ai setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if sentinel_ai_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(sentinel_ai_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        sentinel_ai_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(sentinel_ai_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            sentinel_ai_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(sentinel_ai_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &sentinel_ai_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<sentinel_ai_config::signed_policy::SignatureEnvelope> {
    use sentinel_ai_config::signed_policy::now_unix;
    ...
    let claim = match sentinel_ai_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &sentinel_ai_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `ai login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<AiAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `ai setup` could install config for.
...
pub fn classify_auth_mode() -> sentinel_ai_telemetry::startup::AuthMode {
    auth_mode(
...
) -> sentinel_ai_telemetry::startup::AuthMode {
    use sentinel_ai_telemetry::startup::AuthMode;
...
pub async fn ensure_managed_policy_present(
    ...
    sentinel_ai_telemetry::startup::enter(sentinel_ai_telemetry::startup::StartupPhase::ManagedPolicy);
    ...
    sentinel_ai_telemetry::startup::set_auth_mode(auth_mode(has_deployment_key, &signed_in_team));
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(AiAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `ai setup`; ceasing to serve
...
fn bump_managed_rollback_floor() {
    ...
    if !sentinel_ai_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::ai_home::ai_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            sentinel_ai_config::bump_rollback_floor(&home);
        }
...
/// Outcome of the `ai setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.ai`.
    Installed,
...
/// Result of `ai setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `ai setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `ai setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `ai setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `ai setup` would refuse
...
/// Run the `ai setup` sync for the current principal. The caller must check
