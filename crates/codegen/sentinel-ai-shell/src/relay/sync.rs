...
/// Format: https://ai.com/build/{sessionId}
pub(crate) fn build_share_url(session_id: &str) -> String {
    let base_url =
        std::env::var("AI_CODE_WEB_URL").unwrap_or_else(|_| "https://ai.com".to_string());
...
mod tests {
    ...
    fn test_build_share_url_default() {
        ...
        assert_eq!(url, "https://ai.com/build/test-session-123");
    }
    ...
    fn test_build_share_url_with_uuid() {
        ...
        assert_eq!(
            url,
            "https://ai.com/build/01937d8a-1234-7abc-9def-0123456789ab"
        );
    }
