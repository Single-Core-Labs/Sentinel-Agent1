...
//! a throwaway `AI_HOME`, no network, and a provider command that needs no
...
use chrono::Utc;
use sentinel_ai_shell::auth::{AuthMode, AiAuth, AiComConfig, try_ensure_fresh_auth};

const SEED_TOKEN: &str = "stale-token-that-must-be-replaced";

/// Point the process at a throwaway ai home. `ai_home()` memoizes into a
