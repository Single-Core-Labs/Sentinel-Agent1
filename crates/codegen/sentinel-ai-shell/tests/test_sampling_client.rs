...
use sentinel_ai_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, ConversationToolChoice,
    SamplingError, ToolCall, ToolSpec, rs,
};
use sentinel_ai_shell::session::storage::JsonlStorageAdapter;
use sentinel_ai_test_support::sse::responses_api_reasoning_and_text_events;
...
async fn test_chat_completions_streaming_tool_calls() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        ScriptedResponse::sse(chat_completion_tool_call_stream(tool_calls, "ai-test")),
    );
    let client = create_test_client(&server.url(), ApiBackend::ChatCompletions);
...
/// Upgrade path, ai-build / Responses API: a legacy session whose
...
async fn responses_upgrade_roundtrips_reconstructed_reasoning_as_typed_input() {
    // 1. Seed a legacy ai-build chat_history.jsonl (inline reasoning
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy ai-build reasoning","encrypted":"ENC_BLOB_xyz","id":"rs_aibuild_legacy"},"model_id":"ai-build"}"#,
            "\n",
        ),
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        Some("legacy ai-build reasoning")
    );
}
...
async fn messages_upgrade_emits_reconstructed_reasoning_as_thinking_block() {
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy anthropic thinking","encrypted":"SIGNATURE_abc","id":""},"model_id":"ai-4.5"}"#,
            "\n",
        ),
    )
    .unwrap();
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use sentinel_ai_shell::sampling::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use sentinel_ai_shell::sampling::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use sentinel_ai_shell::sampling::rs::OutputMessageContent;
...
async fn test_request_includes_headers() {
    ...
    assert_eq!(request.header("x-ai-conv-id"), Some("conv-12345"));
    assert_eq!(request.header("x-ai-req-id"), Some("req-67890"));
}
...
/// `SamplerConfig::doom_loop_recovery` puts `x-ai-doom-loop-check` on the
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use sentinel_ai_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
    ...
    assert_eq!(logged.header("x-ai-doom-loop-check"), Some("true"));
}
...
async fn test_doom_loop_check_disabled_sends_no_header_and_drops_check_frames() {
    ...
    let logged = server.requests().pop().unwrap();
    assert!(logged.path.contains("/responses"));
    assert_eq!(logged.header("x-ai-doom-loop-check"), None);
}
...
async fn test_responses_backend_hits_responses_endpoint_not_chat_completions() {
    ...
    // The session-level dispatch in `sentinel-ai-sampler` selects the
