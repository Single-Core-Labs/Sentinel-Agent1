...
//!
//! One `#[test]`: the phases share one process-global `AI_HOME` and env, so
...
use sentinel_ai_test_support::{MockInferenceServer, MockModelEntry};
...
fn seed_credential(ai_home: &Path, expires_at: chrono::DateTime<chrono::Utc>) {
    let auth = json!({
    ...
    std::fs::create_dir_all(ai_home).expect("create ai home");
    std::fs::write(
...
fn write_interactive_only_provider(ai_home: &Path) -> String {
    use std::os::unix::fs::PermissionsExt;

    let script = ai_home.join("acme-auth.sh");
    std::fs::write(
        &script,
        "#!/bin/sh\n\
...
fn provider_runs(ai_home: &Path) -> usize {
    std::fs::read_to_string(ai_home.join("provider-runs"))
        .map(|s| s.lines().count())
        .unwrap_or(0)
}
...
fn expired_external_credential_routes_to_the_provider_login_flow() {
    ...
    let provider = write_interactive_only_provider(ai_home.path());
    ...
    unsafe {
        ...
        unsafe { std::env::set_var("AI_MODELS_BASE_URL", server.url()) };
        unsafe { std::env::set_var("AI_AUTH_PROVIDER_COMMAND", &provider) };
        unsafe { std::env::set_var("AI_AUTH_PROVIDER_LABEL", PROVIDER_LABEL) };
    ...
    agent_rt.block_on(local.run_until(async move {
        ...
        std::fs::remove_file(ai_home.path().join("auth.json")).expect("remove auth.json");
