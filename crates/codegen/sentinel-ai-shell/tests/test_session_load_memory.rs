...
//!
//! Run:
//!   cargo test -p sentinel-ai-shell --features dhat-heap,test-support --test test_session_load_memory \
...
use sentinel_ai_shell::session::storage::{JsonlStorageAdapter, StorageAdapter, prepare_replay_lines};
...
mod rss {
    ...
    use sentinel_ai_test_support::resources::{RssOutcome, RssSampler};
    ...
    async fn count_replayed_notifications(session_id: acp::SessionId, cwd: PathBuf) -> u64 {
        ...
        local
            .run_until(async move {
                ...
                let loaded = sentinel_ai_shell::session::testkit::e2e::load_session_via_agent(
    ...
    async fn session_load_e2e_peak_rss() {
        ...
        let (info, dir) = synth::prepare_session(ai_home.path(), cwd.path(), &opts).await;
