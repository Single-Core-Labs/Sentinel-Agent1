...
use sentinel_ai_shell::sampling::ConversationItem;
...
async fn create_test_session(storage: &JsonlStorageAdapter, session_id: &str, cwd: &str) -> Info {
    ...
    let model_id = acp::ModelId::new("ai-code-fast-1");
    ...
    storage
        .append_update(
            &info,
            &sentinel_ai_shell::session::storage::SessionUpdate::Acp(Box::new(notification)),
        )
        .await
        .unwrap();
...
async fn test_fork_session_creates_new_session_with_parent_tracking() {
    ...
    let options = sentinel_ai_shell::session::storage::CopySessionOptions {
        parent_session_id: Some("source-session-123".to_string()),
        new_model_id: Some("ai-3".to_string()),
    ...
    assert_eq!(loaded.summary.current_model_id, acp::ModelId::new("ai-3"));
    assert_eq!(
    ...
    match &loaded.updates[0] {
        sentinel_ai_shell::session::storage::SessionUpdate::Acp(notification) => {
