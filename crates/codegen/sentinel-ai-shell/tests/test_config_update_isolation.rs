...
/// All tests share this as AI_HOME (the `OnceLock` in sentinel-ai-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = sentinel_ai_shell::config::load_effective_config().unwrap();
    let effective_cfg = sentinel_ai_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    sentinel_ai_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = sentinel_ai_shell::util::config::load_config_from_toml(&user_toml);
    ...
    assert_eq!(user_cfg.models.default.as_deref(), Some("ai-3"));
}
