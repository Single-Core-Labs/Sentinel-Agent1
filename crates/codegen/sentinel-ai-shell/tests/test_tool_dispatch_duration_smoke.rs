...
//! ```bash
//! cargo test -p sentinel-ai-shell --test test_tool_dispatch_duration_smoke -- --ignored
//! ```
...
async fn sleep_tool_records_multi_second_dispatch_duration() {
    ...
    assert_headless_success(&result, "ai -p sleep timing smoke", Some(&server));
    assert_no_crashes(&result.stderr);

    // Session artifacts live under the sandbox AI_HOME.
    let home = sandbox.ai_home();
    let events_files = collect_events_jsonl(home);
    assert!(
        !events_files.is_empty(),
        "no events.jsonl under AI_HOME {}\nstderr:\n{}",
        home.display(),
