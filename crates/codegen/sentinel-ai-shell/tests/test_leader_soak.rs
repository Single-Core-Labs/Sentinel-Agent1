...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p sentinel-ai-shell --features test-support \
...
use sentinel_ai_test_support::resources::ResourceSnapshot;
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    let sock_path = ai_home.path().join("leader-soak.sock");
    ...
    local
        .run_until(async {
            ...
            sentinel_ai_shell::leader::in_process::spawn_agent(acp_rx, response_tx.clone());
            ...
            rpc(
                &mut bootstrap,
                r#"{"jsonrpc":"2.0","id":2,"method":"authenticate","params":{"methodId":"sentinel.api_key","_meta":{"headless":true}}}"#.to_string(),
                2,
                "authenticate",
            )
            .await;
