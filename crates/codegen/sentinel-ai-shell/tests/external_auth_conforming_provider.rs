...
//!
//! Operator binaries live outside this repo and read `AI_AUTH_EXPIRED=1` as
...
use chrono::Utc;
use sentinel_ai_shell::auth::{
    AuthMode, AiAuth, AiComConfig, ensure_authenticated, try_ensure_fresh_auth,
};
...
fn write_conforming_provider(home: &Path) -> String {
    ...
    std::fs::write(
        ...
        format!(
            "#!/bin/sh\n\
             echo \"expired=${{AI_AUTH_EXPIRED:-unset}}\" >> {log}\n\
...
fn seed_expired_credential(home: &Path, scope: &str) {
    ...
    let store: BTreeMap<String, AiAuth> = [(scope.to_owned(), expired)].into_iter().collect();
    std::fs::write(
        home.join("auth.json"),
...
async fn a_provider_that_declines_the_headless_run_can_still_sign_the_user_in() {
    let home = tempfile::tempdir().expect("ai home");
    ...
    // environment is spawned. `ai_home()` memoizes, so this must stay the
    // only test in the binary.
    unsafe {
        unsafe { std::env::set_var("AI_HOME", home.path()) };
        unsafe { std::env::set_var("AI_CLI_CHAT_PROXY_BASE_URL", &dead) };
        unsafe { std::env::set_var("AI_AI_API_BASE_URL", &dead) };
