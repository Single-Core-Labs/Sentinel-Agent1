...
//! 1. AI session notifications (e.g., diff_review) can be sent via ext_notification
...
use sentinel_ai_shell::extensions::notification::{
    DiffContent, SessionNotification, SessionUpdate as AiSessionUpdate,
};
use sentinel_ai_shell::session::info::Info as SessionInfo;
use sentinel_ai_shell::session::persistence::default_model_id;
use sentinel_ai_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};

/// Test that AI session notifications round-trip through storage correctly.
#[tokio::test]
async fn test_sentinel_session_notification_storage_roundtrip() {
    ...
    let sentinel_notification = SessionNotification {
        session_id: session_id.clone(),
        update: AiSessionUpdate::DiffReview {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Ai(Box::new(sentinel_notification.clone())),
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        loaded.updates.len(),
        2,
        "Should have 2 updates (1 AI + 1 ACP)"
    );

    // Verify AI notification
    match &loaded.updates[0] {
        SessionUpdate::Ai(notification) => {
            assert_eq!(notification.session_id, session_id);
            match &notification.update {
                AiSessionUpdate::DiffReview { content } => {
        ...
        _ => panic!("Expected AI update as first item"),
    }
...
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Ai(Box::new(sentinel_notification)))
        .await
        .unwrap();
...
/// Test that totalTokens can be extracted from both ACP and AI notifications.
#[tokio::test]
async fn test_extract_total_tokens_from_mixed_updates() {
    ...
    // Add AI notification with totalTokens
    let sentinel_notification = SessionNotification {
        session_id: session_id.clone(),
        update: AiSessionUpdate::DiffReview { content: vec![] },
...
#[test]
fn test_sentinel_session_notification_serialization() {
