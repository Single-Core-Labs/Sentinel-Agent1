...
//! `ai -p` exit.
//!
//! `#[ignore]`d by default — needs the ai binary (`AI_BINARY` or a local
//! debug build):
//! ```bash
//! cargo test -p sentinel-ai-shell --test test_session_end_hook_e2e -- --ignored
//! ```
//!
//! CI coverage of the same machinery without a built binary lives in
//! `sentinel_ai_shell::agent::activity` tests (the flush quiesce loop and its
//! grace expiry) and `sentinel_ai_pager::acp::spawn` tests (the worker join:
...
async fn run_with_session_end_hook() -> (HeadlessResult, MockInferenceServer, tempfile::TempDir) {
    ...
    let hooks_dir = sandbox.ai_home().join("hooks");
