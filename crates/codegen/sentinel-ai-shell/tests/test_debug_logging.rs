...
//!
//! Runs the built ai binary against the mock inference server with a
//! caller-owned `$AI_HOME`, then inspects `~/.ai/debug/`:
...
//!   `~/.ai/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `AI_LOG_SAMPLING`/`AI_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `AI_DEBUG_LOG=1`) writes a per-session
//!   `~/.ai/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.ai/debug/` files).
//! - `AI_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p sentinel-ai-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$AI_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".ai").join("debug")
}

/// List firehose `*.txt` files under `~/.ai/debug` (excluding the `latest.txt`
...
/// Build a headless `ai -p` command with a pinned `$AI_HOME` so the firehose
/// lands under `<home>/.ai/debug`. Firehose env knobs are cleared so the test
...
/// assert it carries first-party (`sentinel_ai`) content. Panics with the captured
/// stderr tail if it never fills. Shared by the live-agent tests.
async fn read_session_firehose_when_ready(path: &Path, client: &AiStdioClient) -> String {
...
/// headless `ai -p` client is near-silent, so its lazily-opened firehose may
...
async fn debug_flag_enables_firehose_without_crashing() {
    ...
    assert_headless_success(&result, "ai --debug headless", Some(&server));
...
async fn no_debug_flag_writes_no_debug_dir() {
    ...
    assert_headless_success(&result, "ai headless (no --debug)", Some(&server));
    assert!(
...
/// A live `agent` session writes `~/.ai/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        sandbox.set_env("AI_DEBUG_LOG", "1");
        let ai_home = sandbox.ai_home().to_path_buf();

        let client =
            AiStdioClient::spawn_with_sandbox(&server, workdir.workspace(), sandbox).await;
        ...
        let session_file = ai_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.ai/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = ai_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `AI_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `AI_LOG_SAMPLING`/`AI_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // Drive `ai --debug agent stdio`: the master switch (which runs before
        // the agent dispatch) must be what enables the firehose — NOT a direct
        // AI_DEBUG_LOG env. The sandbox baseline excludes inherited firehose
        // toggles, so the `--debug` flag is the only thing enabling logging here.
        let client = AiStdioClient::spawn_with_sandbox_env_and_args(
            &server,
            workdir.workspace(),
            sandbox,
            &[],
        ...
        // clears AI_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.ai/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // AI_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = ai_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.ai/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn debug_file_flag_writes_single_file_and_bypasses_routing() {
    ...
    assert_headless_success(&result, "ai --debug-file", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        explicit.exists(),
...
/// `AI_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn ai_log_file_explicit_path_is_written() {
    let server = MockInferenceServer::start()
        .await
        .expect("start mock server");
    let workdir = git_workdir();
    ...
    sandbox.set_env("AI_LOG_FILE", &custom);
    let result = run_headless_in_sandbox(cmd, sandbox).await;

    assert_headless_success(&result, "ai AI_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit AI_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "AI_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
