...
use sentinel_acp_lib::{AcpAgentGatewaySender, AcpClientMessage};
...
/// Shared `AI_HOME` for the entire test binary. The `OnceLock` in
/// `sentinel-ai-config` only allows one value per process, so all tests share
...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::AiPager, |handle, gw, cwd| async move {
...
async fn fallback_client_plain_allow_always_persists_mcp_tool() {
    // Regression: Generic / AiWeb / Extension clients
...
async fn dont_ask_policy_denies_without_prompting() {
    use sentinel_ai_workspace::permission::types::{PermissionConfig, PromptPolicy};
