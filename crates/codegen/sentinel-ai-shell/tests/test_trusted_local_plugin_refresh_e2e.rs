...
//! 4. Smoke-validate session JSON under `$AI_HOME/sessions/` after exit.
//!
//! Requires a built `ai` binary (`AI_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p sentinel-ai-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p sentinel-ai-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use sentinel_ai_agent::plugins::SharedPluginRegistryHandle;
use sentinel_ai_agent::plugins::discovery::DiscoveryConfig;
use sentinel_ai_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _ai_guard = EnvVarGuard::set("AI_HOME", &ai_home);
    ...
    let mut registry = InstallRegistry::empty(ai_home.join("installed-plugins"));
    ...
    // ai_home() caches the first AI_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        sentinel_ai_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built ai binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/AI_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via ai_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _ai_guard = EnvVarGuard::set("AI_HOME", &ai_home);
    ...
    // Smoke: session storage under AI_HOME/sessions has JSON artifacts.
    let sessions_root = ai_home.join("sessions");
    assert!(
