...
//! [`synth`](sentinel_ai_shell::session::testkit::synth) generator (redundant
//! `available_commands_update` + big rewind snapshots; size knobs via
//! `AI_PERF_*`), or a real session dir via `AI_PERF_SESSION_SRC=<session-dir>`.
//!
//! Run (needs the `test-support` feature; on by default under Bazel):
//!   cargo test -p sentinel-ai-shell --features test-support --test session_load_perf -- --nocapture
//!   cargo test -p sentinel-ai-shell --features test-support --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use sentinel_ai_shell::session::info::Info;
use sentinel_ai_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use sentinel_ai_shell::session::testkit::e2e::load_session_via_agent;
use sentinel_ai_shell::session::testkit::synth::{self, SessionSpec};

// ───────────────────────── session spec ─────────────────────────

/// Perf-tool defaults over the shared [`SessionSpec`], tuned to the pathological
/// real session; scale/override via `AI_PERF_*` (e.g. `AI_PERF_TURNS`,
/// `AI_PERF_SCALE`), or point `AI_PERF_SESSION_SRC` at a real session dir.
...
/// `AI_PERF_SESSION_SRC` set, copy a real session over a registered stub
...
async fn prepare_session(root: &Path, cwd: &Path, spec: &SessionSpec) -> (Info, PathBuf) {
    let Ok(src) = std::env::var("AI_PERF_SESSION_SRC") else {
...
fn generate_or_restore_rewind(path: &Path, spec: &SessionSpec) {
    if std::env::var("AI_PERF_SESSION_SRC").is_ok() {
        return;
    }
...
/// `--ignored` (optionally `AI_PERF_SESSION_SRC=...`) to get the numbers.
...
async fn full_session_load_e2e() {
    ...
    let cwd = TempDir::new().unwrap();
    let spec = perf_spec();
    let instr_log = ai_home.path().join("instr.jsonl");

    // SAFETY: single-threaded current-thread runtime; set before any agent code
    // reads these process-globals (ai_home()/instrumentation mode are OnceLock).
    unsafe {
        unsafe { std::env::set_var("AI_HOME", ai_home.path()) };
        unsafe { std::env::set_var("AI_INSTRUMENTATION", "log") };
        unsafe { std::env::set_var("AI_INSTRUMENTATION_LOG", &instr_log) };
        unsafe { std::env::set_var("AI_CLI_CHAT_PROXY_BASE_URL", server.url()) };
        unsafe { std::env::set_var("AI_AI_API_BASE_URL", server.url()) };
        unsafe { std::env::set_var("AI_API_KEY", "test-key-for-ci") };
        unsafe { std::env::set_var("AI_TELEMETRY_ENABLED", "false") };
        unsafe { std::env::set_var("AI_FEEDBACK_ENABLED", "false") };
        unsafe { std::env::set_var("AI_TRACE_UPLOAD", "false") };
    }
    ...
    let _ = tracing_subscriber::registry()
        .with(sentinel_ai_shell::instrumentation::layer::<Registry>())
        .try_init();

    let (info, dir) = prepare_session(ai_home.path(), cwd.path(), &spec).await;
    let updates_path = dir.join("updates.jsonl");
    let rewind_path = dir.join("rewind_points.jsonl");
    eprintln!(
    ...
    local
        .run_until(async move {
            ...
            let _ = sentinel_ai_shell::instrumentation::finalize();
