    ...
    C: acp::Client + 'static,
{
    ...
    let method = init
        ...
        .expect("sentinel.api_key auth method not advertised");
    tokio::time::timeout(
        RPC_TIMEOUT,
...
fn set_test_env(ai_home: &std::path::Path, server_url: &str) {
    // SAFETY: the only live threads are the mock's HTTP workers, which never read env.
    unsafe {
        unsafe { std::env::set_var("AI_HOME", ai_home) };
        unsafe { std::env::set_var("AI_CLI_CHAT_PROXY_BASE_URL", server_url) };
        unsafe { std::env::set_var("AI_AI_API_BASE_URL", server_url) };
...
/// Runs `body` against a mock inference server with `AI_HOME` isolated to a
...
where
    F: FnOnce(std::path::PathBuf, std::rc::Rc<sentinel_ai_test_support::MockInferenceServer>) -> Fut,
    Fut: std::future::Future<Output = ()>,
{
    ...
    let server = std::rc::Rc::new(
        mock_rt
            .block_on(sentinel_ai_test_support::MockInferenceServer::start())
            .expect("mock server"),
    );
    let ai_home = tempfile::TempDir::new().expect("ai home");
    let workdir = tempfile::TempDir::new().expect("workdir");
    set_test_env(ai_home.path(), &server.url());
