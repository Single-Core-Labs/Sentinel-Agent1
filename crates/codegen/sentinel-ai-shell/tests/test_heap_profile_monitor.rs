...
use sentinel_ai_shell::auth::{AuthManager, AuthMode, AiAuth, AiComConfig};
use sentinel_ai_shell::heap_profile::{
...
use sentinel_ai_test_support::{EnvGuard, MockInferenceServer};
use sentinel_ai_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = AiComConfig::default().auth_scope();
    let auth = AiAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[sentinel_ai_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), AiComConfig::default()));
