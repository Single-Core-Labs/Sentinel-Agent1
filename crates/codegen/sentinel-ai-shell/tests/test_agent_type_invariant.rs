...
//! - Session resume
//!
//! Each test spawns a real `ai agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p sentinel-ai-shell --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `model-a`: no agent_type (→ "ai-build")
/// - `model-b`: no agent_type (→ "ai-build")
...
/// `ai-build` harness. The system prompt sent to the LLM should contain
/// the ai-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_ai_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Ai") || sys_prompt.contains("ai"),
            "default model should use ai-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        drop(writer);
        let reader =
            AiStdioClient::spawn_with_sandbox(&server, workdir.workspace(), shared_sandbox).await;
        ...
        let original_has_ai =
            original_sys_prompt.contains("Ai") || original_sys_prompt.contains("ai");
        let resumed_has_ai =
            resumed_sys_prompt.contains("Ai") || resumed_sys_prompt.contains("ai");
        assert_eq!(
            original_has_ai,
            resumed_has_ai,
            "resumed session should use the same harness as the original\n\
             original identity markers: ai={original_has_ai}\n\
             resumed identity markers: ai={resumed_has_ai}\n\
...
/// default to `"ai-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_ai_build() {
    with_local_set(|| async {
            ...
            assert!(
            sys_prompt.contains("Ai") || sys_prompt.contains("ai"),
            "model without agent_type should default to ai-build harness\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
        })
        .await;
}
/// The `AI_AGENT` escape hatch should override the model's agent_type.
/// Setting `AI_AGENT=ai-build` with an alternate-agent model should use
/// ai-build harness.
#[tokio::test]
#[ignore]
async fn test_ai_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            assert!(
            result.is_ok(),
            "prompt with AI_AGENT override failed: {:?}\nstderr:\n{}",
            result.err(),
            client.stderr()
        );
            let sys_prompt = server
            ...
            assert!(
            sys_prompt.contains("Ai") || sys_prompt.contains("ai"),
            "AI_AGENT=ai-build should override catalog model agent_type\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
        })
        .await;
