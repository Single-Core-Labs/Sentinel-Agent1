...
//! `.ai`, `.cursor`, and `.claude` vendor dirs, spawns a real `ai agent
//! stdio` process against the mock inference server (toggling the
//! `AI_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
//! and asserts on the full inference request bodies:
//!
//! - the Ai-native skill is always present regardless of toggles
...
//! ```bash
//! cargo test -p sentinel-ai-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_AI_SKILL: &str = "ZZ_AI_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".ai", "ai-skill", MARKER_AI_SKILL);
...
async fn run_scenario(env: &[(&str, &str)]) -> String {
    ...
    let client = AiStdioClient::spawn_with_sandbox(&server, workdir.workspace(), sandbox).await;
...
/// Defaults (all vendors on): ai + cursor-vendor + claude-vendor skills present; the
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_AI_SKILL),
            "ai-skill must always be present"
        );
        assert!(
...
/// `AI_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; ai stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AI_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_AI_SKILL),
            "ai-skill always present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
...
/// `AI_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; ai stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AI_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_AI_SKILL),
            "ai-skill always present"
        );
...
/// `AI_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AI_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `AI_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AI_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `AI_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AI_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `AI_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AI_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("AI_CURSOR_SKILLS_ENABLED", "false"),
            ("AI_CURSOR_RULES_ENABLED", "false"),
            ("AI_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_AI_SKILL), "ai always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("AI_CLAUDE_SKILLS_ENABLED", "false"),
            ("AI_CLAUDE_RULES_ENABLED", "false"),
            ("AI_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
...
/// All vendor compat OFF: only ai-native skill survives.
#[tokio::test]
#[ignore] // requires pre-built binary
