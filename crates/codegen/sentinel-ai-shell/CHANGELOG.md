...
- Ai skips the project-directory prompt when launched from home or other non-project directories
...
- Sandboxed Ai starts on large directories with many deny-glob matches
...
- Ai retries more server errors during outages
...
- **ai doctor** now warns when tmux is reducing colors and can fix the config.
...
- **AI_EXTRA_CA_BUNDLE** env var allows adding custom TLS root certificates.
...
- **Ai** no longer crashes on startup when the host machine has no free threads.
...
- **MCP servers** can now be enabled or disabled directly from the CLI with `ai mcp enable <name>` and `ai mcp disable <name>`.
- **Full plan markdown** can now be copied to the clipboard with `y` during plan approval or preview.
- **Added support for the new SuperAi Plus subscription tier** in authentication and feature gating.
...
- **Invalid MCP server entries** in config.toml no longer prevent Ai from starting; problems are shown in `ai inspect`.
...
- **Fixed credential loss** across multiple ai processes sharing the same auth file.
...
- **New /tutorial slash command** opens an opt-in nine-topic onboarding tour of Ai.
...
- **`ai doctor fix`** can now repair common tmux clipboard and passthrough problems.
- **Per-provider auth helpers** now work on Windows and can run from a configurable working directory.
- **/resume** now shows only native Ai sessions by default and shows a hint when external sessions are hidden.
- **`ai --resume`** can now resume a session by its title as well as by ID.
...
- **Web search** now defaults to ai-4.5.
...
- You can now run `ai doctor fix` commands directly from inside the TUI instead of only from the CLI.
...
- **npm package** now installs the native binary under `$AI_HOME/bin` (honoring the same override as the Rust CLI).
...
- Ai now automatically stops a turn that keeps repeating the exact same tool call many times in a row.
...
- When the auto-permission classifier times out or fails, Ai now shows a normal permission prompt instead of silently denying.
...
- **ai doctor fix ssh-wrap** can set up `ai wrap ssh` automatically for Bash, zsh, and fish.
...
- **ai doctor** checks terminal, tmux, clipboard, and keyboard setup without opening the TUI.

## Bug Fixes

- **Image paste** over ai wrap now works on headless remotes.
...
- **Added AI_CLIPBOARD_NO_OSC52** env var to stop clipboard sequences from appearing as garbage in unsupported terminals.
...
- **/btw** now works inside `ai --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Default model** is now Ai 4.5 with high/medium/low reasoning effort and improved compaction settings.
...
- **Global rules** from ~/.ai/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `ai wrap ssh <host>` for clipboard and terminal restore.
...
- **ai** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **ai wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `ai login`.
...
- **ai login** now requests Ai Projects scopes so workspace listing works after consent.
...
- **`ai plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using ai -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **ai inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct ai --minimal --resume command.
...
- **Ai no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **ai setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask Ai above the footer shortcuts.
...
- **`ai worktree ls`** now works as a short alias for `ai worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.ai/config.toml`.
...
- **Resumed ai.com chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.ai/worktrees/.
...
- **Terminal command cards** on ai.com now correctly settle after foreground bash tasks.
...
- **ai inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain ai launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive ai agent stdio without silent parse drops on session/* calls.
...
- **ai --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by ai.com" link in the Extensions modal is now clickable and underlined.
...
- **ai --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **ai update** on Windows no longer fails when a previous .old executable is still running.
...
- **ai models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `ai plugin marketplace add`.
...
- **Plan files** now default to .ai/plan.md to match Ai conventions.
...
- **--minimal** flag now shows in `ai --help`.
...
- **Tool result previews** no longer paint opaque panels in `ai --minimal`.
- **ai wrap** now correctly handles quoted strings and shell aliases.
...
- **Ai.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **Chat sessions show the ai.com model catalog** in the picker.
...
- **`ai update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `ai wrap`.
...
- **ai agent stdio** now checks for updates in the background like other modes.
...
- **ai wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Added `ai wrap`** to run any command with local clipboard support.
...
- **Fixed `ai agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or ai-desktop.
...
- You can now pass --json-schema to ai -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `AI_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **OIDC sessions** with AI_API_KEY present no longer lose refresh on idle.
...
- **ai -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in ai -p and ai agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `ai` after running `ai update`.
- Headless (`ai -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`ai plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **ai sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **ai quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, ai sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, AI_HUNK_TRACKER, or config.
...
- **Ai** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`ai dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`ai mcp add`** now accepts positional arguments (e.g. `ai mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`ai update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.ai.
...
- **MCP servers menu** and slash commands now work when starting ai outside a project directory.
...
- **ai --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `ai --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `ai update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`ai --debug`** now produces per-session log files under ~/.ai/debug/ even with a leader process.
...
- **`ai update`** now restarts any older running leader so all clients get the new binary.
...
- **`ai login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `ai agent --leader` without forced interactive login or timeouts.
...
- **Official AI plugin marketplace** now appears automatically in the Marketplace tab on first launch.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the AI_IMAGE_GEN_HARNESS env var.
...
- **ai inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via AI_CURSOR_MCPS_ENABLED / AI_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside Ai and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the ai-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality ai-imagine-image-quality model.
