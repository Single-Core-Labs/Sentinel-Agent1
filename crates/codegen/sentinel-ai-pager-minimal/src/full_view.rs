...
use sentinel_ai_pager::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = sentinel_ai_pager::appearance::cache::load_show_thinking_blocks();
        ...
        sentinel_ai_pager::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: sentinel_ai_pager::app::agent::AgentId, out: String) {
    if out.is_empty() {
        if let Some(agent) = app.agents.get_mut(&id) {
            agent
                .scrollback
                .push_block(sentinel_ai_pager::scrollback::block::RenderBlock::system(
                    "No conversation transcript to view yet",
                ));
        }
        return;
    }
    let path = std::env::temp_dir().join(format!("ai-transcript-{}.ansi", uuid::Uuid::new_v4()));
        ...
        Err(e) => {
            if let Some(agent) = app.agents.get_mut(&id) {
                agent.scrollback.push_block(
                    sentinel_ai_pager::scrollback::block::RenderBlock::system(format!(
...
mod tests {
    use super::*;
    use sentinel_ai_pager::scrollback::block::RenderBlock;
    ...
    fn transcript_expands_thinking_committed_collapsed() {
        let theme = Theme::current();
        let appearance = super::super::commit::committed_appearance(
            &sentinel_ai_pager::appearance::AppearanceConfig {
