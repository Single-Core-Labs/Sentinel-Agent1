//! Minimal (scrollback-native) render mode — `ai --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `sentinel_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `sentinel-ai-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`sentinel_ai_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`sentinel-ai-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `sentinel-ai-pager`.
///
...
/// subsequent calls are ignored (see [`sentinel_ai_pager::minimal_hook`]).
pub fn install() {
    sentinel_ai_pager::minimal_hook::install(sentinel_ai_pager::minimal_hook::MinimalHooks { draw });
}
