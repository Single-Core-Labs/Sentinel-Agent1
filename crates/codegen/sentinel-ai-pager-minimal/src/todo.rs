...
use sentinel_ai_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (sentinel_ai_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    sentinel_ai_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use sentinel_ai_shell::tools::{TodoItem, TodoPriority};
