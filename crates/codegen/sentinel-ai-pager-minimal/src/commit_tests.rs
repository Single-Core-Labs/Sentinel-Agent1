...
fn btw_block_emits_once_across_repeated_frontier_passes() {
    ...
    s.push(ScrollbackEntry::new(RenderBlock::Btw(
        sentinel_ai_pager::scrollback::blocks::BtwBlock::new("original question", "original answer"),
    )));
...
fn terminal_native_lock_paints_only_native_colors() {
    use ratatui::buffer::Buffer;
    use sentinel_ai_pager::theme::cache as theme_cache;
    ...
    impl Drop for LockReset {
        fn drop(&mut self) {
            sentinel_ai_pager::theme::cache::set_terminal_native_lock(false);
        }
    }
    ...
    let hunk = vec![
        sentinel_ai_pager::diff::DiffLine {
            text: "let x = 1;\n".into(),
            lo: 1,
            ln: 1,
            tag: ChangeTag::Equal,
...
fn committed_edit_keeps_diff_line_backgrounds() {
    ...
    use ratatui::layout::Rect;
    use similar::ChangeTag;
    use sentinel_ai_pager::diff::DiffLine;
...
fn only_thinking_spends_the_accent_column() {
    ...
    use ratatui::buffer::Buffer;
    use ratatui::layout::Rect;
    let rail = sentinel_ai_pager::glyphs::accent_bar();
    minimal_api::set_show_thinking_blocks(true);
    for mode in [
...
fn commit_display_mode_lookups_collapse_on_success_only() {
    use sentinel_ai_pager::scrollback::blocks::{
