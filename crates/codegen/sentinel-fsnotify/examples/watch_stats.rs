...
//! startup latency, under either strategy (`AI_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
//! # Generate a synthetic tree, then measure both strategies against it:
//! cargo run --release -p sentinel-fsnotify --example watch_stats -- gen js /tmp/js-repo
//! AI_FSNOTIFY_PER_DIR=0 cargo run --release -p sentinel-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! AI_FSNOTIFY_PER_DIR=1 cargo run --release -p sentinel-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "ai holds 55k inotify watches" report).
...
use sentinel_fsnotify::{FsConfig, FsEventSource};
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("AI_FSNOTIFY_PER_DIR").unwrap_or_default();
