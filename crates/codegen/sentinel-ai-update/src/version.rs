...
use sentinel_ai_shell::env::AiBuildEnvironment;
use sentinel_ai_shell::util::ai_home::ai_home;

const TTL_SECONDS_BEFORE_AUTO_UPDATE: Duration = Duration::from_secs(60 * 30);
const NPM_PACKAGE: &str = "@sentinel-official/ai";
pub const GH_RELEASE_REPO: &str = "sentinel-org-shared/ai-build";
...
pub(crate) const CLI_BASE_URL_FALLBACK: &str =
    "https://storage.googleapis.com/ai-build-public-artifacts/cli";
...
///
/// Constructed once from `AiBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `AiBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.ai.com/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.ai/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (AI_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &AiBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: sentinel_ai_shell::auth::AiComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = ai_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = AiVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<AiVersion>(&version_str)
...
pub use sentinel_ai_version::installed as get_installed_ai_version;

/// Version of the managed ai binary currently on disk, read from the
/// `~/.ai/bin/ai` symlink target (`../downloads/ai-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `ai update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.ai/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = sentinel_ai_shell::util::ai_home::ai_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "ai")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`ai-0.1.150-macos-aarch64`, including
/// pre-releases: `ai-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`ai-0.1.150`,
/// `ai-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`ai-latest`, `ai-pager-*` when `bin_prefix` is
/// `ai`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.ai/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: AiVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(sentinel_ai_version::VERSION, &stable)
    })
}
...
/// `~/.ai/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(sentinel_ai_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = AiVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`ai-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("ai-0.2.46-darwin-arm64", Some("0.2.46")),
            ("ai-0.1.220-linux-x86_64", Some("0.1.220")),
            ("ai-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            ("ai-0.1.220-alpha.4-linux-x86_64", Some("0.1.220-alpha.4")),
            ("ai-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("ai-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("ai-garbage-darwin-arm64", None),               // unparseable version
            ("ai-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),               // wrong prefix
            ("ai-latest", None),                             // symlink alias, not a version
            ("ai", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "ai").as_deref(),
                *expected,
        ...
        // prefix but not under "ai".
        assert_eq!(
            version_from_versioned_binary_name("ai-pager-0.1.5-darwin-arm64", "ai-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // AiVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: AiVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = AiVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: AiVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<AiVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<AiVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = AiVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use sentinel_ai_shell::env::AiBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&AiBuildEnvironment::Production);
