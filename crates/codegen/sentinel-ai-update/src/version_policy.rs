...
use crate::version::get_installed_ai_version;
use semver::Version;
use tracing::warn;
use sentinel_ai_shell::util::config::VersionPolicy;
...
pub(crate) enum VersionPolicyError {
    #[error(
        "Cannot install Ai {target}: the minimum allowed version is {minimum}. \
         Run `ai update` to install the latest allowed version."
    )]
