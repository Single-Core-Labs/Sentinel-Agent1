...
//! its own `OnceLock<AI_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the AI_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes AI_HOME once per binary
...
/// Returns a process-wide test `AI_HOME`, initialized exactly once per test
/// binary. Once initialized, `sentinel_ai_config::ai_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            unsafe { std::env::set_var("AI_HOME", &path) };
            unsafe { std::env::remove_var("AI_TEST_VERSION") };
            unsafe { std::env::remove_var("NPM_TOKEN") };
            unsafe { std::env::remove_var("AI_INSTALLER") };
            unsafe { std::env::remove_var("AI_MANAGED_BY_NPM") };
            unsafe { std::env::remove_var("AI_MANAGED_BY_INTERNAL") };
        }
        path
    })
}

/// Wipe state in `AI_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_ai_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("AI_TEST_VERSION", v) };
}
...
/// Minimal [`sentinel_ai_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> sentinel_ai_update::UpdateConfig {
    sentinel_ai_update::UpdateConfig {
...
/// Backdate every file in `AI_HOME/downloads` by ~2 hours.
///
