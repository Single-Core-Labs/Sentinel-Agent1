...
//!
//! > After any install attempt, `$BIN_DIR/ai` resolves to a binary that runs,
...
//!
//! The installer lives in the sibling `sentinel-ai-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../sentinel-ai-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
const INSTALLER_BLOCK_START: &str = "# >>> ai installer >>>";
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".ai").join("downloads");
    let bin = home.join(".ai").join("bin");
    std::fs::create_dir_all(&downloads).unwrap();
    std::fs::create_dir_all(&bin).unwrap();
    let prev = downloads.join(format!("ai-{platform}"));
    ...
    std::os::unix::fs::symlink(format!("../downloads/ai-{platform}"), &link).unwrap();
    dunce::canonicalize(&prev).unwrap()
}

/// Re-resolve `$BIN_DIR/ai` from disk and re-run it: the active ai must
/// always execute, and never be a `.tmp`/partial file.
fn assert_active_ai_runs(home: &Path) {
    let link = home.join(".ai").join("bin").join("ai");
    assert!(link.is_symlink(), "ai must remain a symlink");
    let resolved =
        dunce::canonicalize(&link).unwrap_or_else(|e| panic!("ai symlink dangles: {e}"));
    let name = resolved.file_name().unwrap().to_string_lossy().to_string();
    assert!(
        !name.contains(".tmp"),
        "active ai must not be a temp file: {name}"
    );
    let ok = Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ok, "active ai must run: {}", resolved.display());
}
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("AI_BIN_DIR", home.join(".ai").join("bin"))
        .env("AI_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
...
fn assert_single_installer_block(path: &Path, preserved: Option<&str>) {
    ...
    assert_eq!(
        n,
        1,
        "{} must contain exactly one ai installer block, got {n}:\n{body}",
        path.display()
    );
...
fn run_shell_rc_case(case: &ShellRcCase) {
    ...
    assert_active_ai_runs(&home_path);
}

#[test]
fn install_sh_blitz_keeps_ai_runnable_under_corruption() {
    ...
    for (mode, expect_ok) in cases {
        ...
        // ai always runs (new good binary on success, previous-good on
        // rejection).
        assert_active_ai_runs(home.path());
    }
}
