...
//!
//! > `~/.ai/bin/ai` resolves to a binary that passes the smoke-test, OR it
...
use sentinel_ai_update::auto_update::install_internal_from_base;
...
/// (`ai` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `ai` should resolve to after an install attempt.
...
/// to both `ai` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p sentinel-ai-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("AI_BLITZ_ITERS")
        .ok()
