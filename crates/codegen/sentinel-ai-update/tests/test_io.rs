...
//!
//! These tests touch global process state — `AI_HOME` (a `OnceLock` in
//! `sentinel-ai-config`), `AI_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `AI_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the AI_HOME isolation used in other
//! integration tests.
...
use sentinel_ai_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_ai_home() {
    let _ = test_home();
    reset();
...
/// Write a `AiVersion`-shaped JSON file with an arbitrary timestamp.
...
/// `AiVersion` JSON layout.
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under AI_HOME,
...
// ─────────────────────────────────────────────────────────────────────────────
// version.json wire format — the on-disk file is read by every ai launch.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_ai_version env override
//
// The function honors `AI_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// Note: `get_installed_ai_version` is not re-exported from `lib.rs`, but
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        unsafe { std::env::set_var("AI_TEST_VERSION", "9.9.9") };
    }
    let v = sentinel_ai_update::version::get_installed_ai_version();
    assert_eq!(v, "9.9.9");
    unsafe {
        unsafe { std::env::remove_var("AI_TEST_VERSION") };
    }
}
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        sentinel_ai_update::version::get_installed_ai_version()
    };

    unsafe {
        unsafe { std::env::set_var("AI_TEST_VERSION", "0.0.0-test") };
    }
    let overridden = sentinel_ai_update::version::get_installed_ai_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        unsafe { std::env::set_var("AI_TEST_VERSION", "0.1.200-alpha.5") };
    }
    let v = sentinel_ai_update::version::get_installed_ai_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        unsafe { std::env::set_var("AI_TEST_VERSION", "not-a-version") };
    }
    let v = sentinel_ai_update::version::get_installed_ai_version();
