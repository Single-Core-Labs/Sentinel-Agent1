//! Ai-owned direct global hook paths shared by shell discovery and sandbox
//! write-deny: `$AI_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    /// `$AI_HOME/hooks/` (discovered + protected).
    HookDirectory,
    /// `$AI_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked AI_HOME is not allowed under sandbox write-deny: {path}")]
    SymlinkedAiHome { path: PathBuf },
    ...
    #[error("Ai hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("Ai hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create Ai hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create Ai hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
...
/// Ensure real `$AI_HOME/hooks` dir + `hooks-paths` file (create if missing).
/// Race-resistant create (`create_dir` / `create_new`+`O_NOFOLLOW`); never
/// truncates an existing registry; rejects symlinks/wrong types.
pub fn ensure_ai_hook_slots(ai_home: &Path) -> Result<(), GlobalHookSourceError> {
    if path_has_symlink_component(ai_home) {
        return Err(GlobalHookSourceError::SymlinkedAiHome {
            path: ai_home.to_path_buf(),
        });
    }

    match std::fs::create_dir(ai_home) {
        Ok(()) => {}
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {}
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            std::fs::create_dir_all(ai_home).map_err(|source| {
                GlobalHookSourceError::CreateHooksDir {
                    path: ai_home.to_path_buf(),
                    source,
                }
    ...
    let ai_meta = std::fs::symlink_metadata(ai_home).map_err(|source| {
        GlobalHookSourceError::CreateHooksDir {
            path: ai_home.to_path_buf(),
            source,
        }
    })?;
    if ai_meta.file_type().is_symlink() || !ai_meta.file_type().is_dir() {
        return Err(GlobalHookSourceError::SymlinkedAiHome {
            path: ai_home.to_path_buf(),
        });
    }

    let hooks = ai_home.join("hooks");
    ...
    let registry = ai_home.join("hooks-paths");
...
/// Resolve Ai-owned direct global hook sources (`reject_symlinks` for sandbox).
pub fn resolve_global_hook_sources(
    ...
    if let Some(ai) = ai_home {
        if reject_symlinks && path_has_symlink_component(ai) {
            return Err(GlobalHookSourceError::SymlinkedAiHome {
                path: ai.to_path_buf(),
            });
        }

        let hooks = ai.join("hooks");
        let hooks_paths = ai.join("hooks-paths");
        if reject_symlinks {
