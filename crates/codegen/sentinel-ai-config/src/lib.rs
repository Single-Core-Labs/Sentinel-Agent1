//! Config file loading for Ai.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/ai/managed_config.toml`
//! 2. `$AI_HOME/managed_config.toml`
//! 3. `$AI_HOME/config.toml`
//! 4. `$AI_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/ai/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.ai`, admin-forced) — macOS only
//!
...
pub use global_hook_sources::{
    ...
    ensure_ai_hook_slots, existing_ancestor_chain, is_direct_hook_json_name,
...
pub use paths::{
    ai_application, ai_application_in, ai_home, claude_managed_settings_path,
    claude_managed_settings_probe_path, decode_cwd_from_dirname, default_ai_home,
    encode_cwd_dirname, ensure_sessions_cwd_dir, sessions_cwd_dir, system_config_dir, user_ai_home,
};
pub use validation::{
