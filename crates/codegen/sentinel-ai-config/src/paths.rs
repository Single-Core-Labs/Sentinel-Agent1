//! Filesystem locations for ai config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static AI_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user ai directory (`~/.ai`, canonicalized) used when
/// `AI_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`ai_home()`] is the default without duplicating the computation.
///
...
/// `~/.ai/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `sentinel_fast_worktree::db::resolve_ai_home` (deliberately standalone crate).
pub fn default_ai_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".ai")
}

/// Per-user config directory: `$AI_HOME` or `~/.ai`. Created if needed.
pub fn ai_home() -> PathBuf {
    AI_HOME
        .get_or_init(|| {
            let ai_home = if let Ok(v) = std::env::var("AI_HOME") {
                PathBuf::from(v)
            } else {
                default_ai_home()
            };
...
/// The user-global ai home, but only when one genuinely resolves: `Some` when
/// `$AI_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`ai_home()`], this never falls back to a cwd-relative `.ai`, so callers
/// that *scan* user-global ai resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.ai` tree for the user-global one when no home resolves.
pub fn user_ai_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("AI_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(ai_home)
}

/// Canonical ai application path: `$AI_HOME/bin/ai` (Unix) or `ai.exe` (Windows).
pub fn ai_application() -> PathBuf {
    ai_application_in(&ai_home())
}

/// [`ai_application`] under an explicit home instead of `$AI_HOME`.
pub fn ai_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "ai.exe" } else { "ai" };
    home.join("bin").join(name)
}

/// System-wide config directory: `/etc/ai/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/ai"))
    } else {
        None
    }
}
...
/// `ai_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    ai_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = ai_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_ai_home_has_no_verbatim_prefix() {
        ...
        let home = default_ai_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".ai"));
    }
