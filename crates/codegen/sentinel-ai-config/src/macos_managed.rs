...
//! preference domain `ai.x.ai` (`requirements_toml_base64`). Only admin-*forced*
...
#[cfg(target_os = "macos")]
const MANAGED_PREFERENCES_DOMAIN: &str = "ai.x.ai";
#[cfg(target_os = "macos")]
...
pub const MDM_REQUIREMENTS_SOURCE: &str = "ai.x.ai:requirements_toml_base64";
...
fn read_forced_requirements() -> Option<String> {
    ...
    // per-user domain, which a local user can set (`defaults write ai.x.ai`)
...
mod tests {
    ...
    fn forced_payload_is_not_env_expanded() {
        ...
        let prior = std::env::var("AI_MDM_NO_EXPAND_TEST").ok();
        unsafe { std::env::set_var("AI_MDM_NO_EXPAND_TEST", "attacker") };
