...
use crate::paths::{system_config_dir, user_ai_home};
...
/// ai config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_ai_home().as_deref(), USER_CONFIG_FILENAME)
}

/// User config filename (`$AI_HOME/config.toml`), shared by the loaders here.
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_ai_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.ai/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.ai` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/ai`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_ai_home().as_deref())
}
...
/// A hook's origin (held by `sentinel_ai_hooks::HookSpec::layer`). Defined here, not
/// in `sentinel-ai-hooks`, since the dep direction is `sentinel-ai-hooks -> sentinel-ai-config`;
/// this crate sets the config tiers, `File`/`Plugin` are set downstream.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum HookProvenance {
    /// `/etc/ai/managed_config.toml`.
    SystemManaged,
    /// `$AI_HOME/managed_config.toml` (server-synced).
    Managed,
...
pub fn hook_config_layers() -> Vec<HookConfigLayer> {
    hook_config_layers_at(system_config_dir().as_deref(), user_ai_home().as_deref())
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `AI_CAMPAIGNS_OVERRIDE` env layer.
...
/// `AI_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("AI_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`sentinel_ai_shell::util::config::load_effective_config`) when remote campaigns
/// or `AI_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// can't be parsed (broken `AI_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match sentinel_ai_version::installed_semver() {
...
mod tests {
    ...
    fn load_user_config_layer_reads_file_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("ai-load-layer-{}", std::process::id()));
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        let dir = std::env::temp_dir().join(format!("ai-toml-leak-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("bad.toml");
        // Duplicate key: the message names the key; the secret-bearing source line is only in Display.
        std::fs::write(
            &path,
            "api_key = \"sentinel-secretmustnotleak\"\napi_key = \"sentinel-secretmustnotleak2\"\n",
        )
        .unwrap();
        ...
        assert!(
            !msg.contains("sentinel-secretmustnotleak"),
    ...
    /// `AI_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `AI_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("AI_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads AI_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("AI_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("AI_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));
