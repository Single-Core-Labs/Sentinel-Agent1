...
pub fn detect_language(filename: &str, source: &str) -> GpuLanguage {
    ...
    if lower.ends_with(".cu") || lower.ends_with(".cuh") {
        if source.contains("CUTE_DEVICE")
            || source.contains("CUTE_HOST")
            || source.contains("Cutlass")
        {
        ...
        if source.contains("TILE_DIM")
            || source.contains("tile_load")
            || source.contains("tile_store")
        {
    ...
    if source.contains("@triton.jit")
        || source.contains("import triton")
        || source.contains("from triton")
    {
        return GpuLanguage::Triton;
    }

    if source.contains("@cuda.jit")
        || source.contains("from numba import cuda")
        || source.contains("numba.cuda")
    {
    ...
    if lower.ends_with(".py")
        && (source.contains("torch.cuda")
            || source.contains("torch.Tensor")
            || source.contains(".cuda()")
            || source.contains("import torch")
            || source.contains("from torch"))
    {
        return GpuLanguage::PyTorch;
    }
...
pub fn recommended_block_sizes(
    lang: GpuLanguage,
    compute_capability: Option<&str>,
) -> Vec<BlockSizeRecommendation> {
    ...
    match lang {
            ...
            if cc_major >= 9 {
                vec![
                    BlockSizeRecommendation {
                        label: "Hopper-optimized",
                        block_x: 128,
                        block_y: 2,
                        block_z: 1,
                        shared_mem: 0,
                        reason: "Best occupancy on SM90 with 128 threads/warpgroup",
                    },
                    BlockSizeRecommendation {
                        label: "Large shared mem",
                        block_x: 128,
                        block_y: 1,
                        block_z: 1,
                        shared_mem: 49152,
                        reason: "TMA + shared memory for Hopper tensor cores",
                    },
                    BlockSizeRecommendation {
                        label: "Max occupancy",
                        block_x: 256,
                        ...
                        block_z: 1,
                        shared_mem: 0,
                        reason: "High occupancy for compute-bound kernels",
                    },
                ]
            } else if cc_major >= 8 {
                vec![
                    BlockSizeRecommendation {
                        label: "Ampere-optimized",
                        block_x: 128,
                        block_y: 2,
                        block_z: 1,
                        shared_mem: 0,
                        reason: "Best balance on SM86/87, 256 threads per block",
                    },
                    BlockSizeRecommendation {
                        ...
                        block_z: 1,
                        shared_mem: 0,
                        reason: "64 warps/SM, hides latency well",
                    },
                    BlockSizeRecommendation {
                        label: "Shared-mem heavy",
                        block_x: 128,
                        block_y: 1,
                        block_z: 1,
                        shared_mem: 32768,
                        reason: "32KB shared mem for matmul/tiling",
                    },
                ]
            } else {
                vec![
                    BlockSizeRecommendation {
                        label: "Turing-optimized",
                        block_x: 128,
                        block_y: 1,
                        block_z: 1,
                        shared_mem: 0,
                        reason: "128 threads/SM optimal on SM75",
                    },
                    BlockSizeRecommendation {
                        label: "Occupancy max",
                        block_x: 256,
                        ...
                        block_z: 1,
                        shared_mem: 0,
                        reason: "64 warps, good for memory-bound kernels",
                    },
                ]
            }
        }
        GpuLanguage::Triton => {
            vec![
                BlockSizeRecommendation {
                    label: "Triton default",
                    block_x: 128,
                    block_y: 128,
                    block_z: 1,
                    shared_mem: 0,
                    reason: "Standard BLOCK_M=128, BLOCK_N=128 for matmul",
                },
                BlockSizeRecommendation {
                    label: "Large tiles",
                    block_x: 256,
                    ...
                    block_z: 1,
                    shared_mem: 0,
                    reason: "Higher arithmetic intensity for large matrices",
                },
                BlockSizeRecommendation {
                    label: "Small tiles",
                    block_x: 64,
                    ...
                    block_z: 1,
                    shared_mem: 0,
                    reason: "Lower register pressure, good for small N",
                },
            ]
        }
        GpuLanguage::Mojo | GpuLanguage::Numba => {
            vec![
                BlockSizeRecommendation {
                    ...
                    block_z: 1,
                    shared_mem: 0,
                    reason: "256 threads/block, good for most kernels",
                },
                BlockSizeRecommendation {
                    label: "2D tiling",
                    block_x: 16,
                    ...
                    block_z: 1,
                    shared_mem: 0,
                    reason: "16x16 for 2D grid workloads",
                },
                BlockSizeRecommendation {
                    label: "Warp-optimized",
                    block_x: 128,
                    block_y: 2,
                    block_z: 1,
                    shared_mem: 0,
                    reason: "128 threads, 4 warps, low divergence",
                },
            ]
        }
        GpuLanguage::PyTorch => {
            vec![
                BlockSizeRecommendation {
                    ...
                    block_z: 0,
                    shared_mem: 0,
                    reason: "Let inductor choose best block size via autotune",
                },
                BlockSizeRecommendation {
                    label: "CUDA graphs",
                    block_x: 0,
                    ...
                    block_z: 0,
                    shared_mem: 0,
                    reason: "Enable CUDA graphs for static workloads",
                },
            ]
        }
        GpuLanguage::Cute | GpuLanguage::CudaTile => {
            vec![
                BlockSizeRecommendation {
                    label: "CUTE warp tile",
                    block_x: 64,
                    block_y: 2,
                    block_z: 1,
                    shared_mem: 0,
                    reason: "CUTE warp-per-tile for tensor cores",
                },
                BlockSizeRecommendation {
                    label: "Large MMA tile",
                    block_x: 128,
                    block_y: 1,
                    block_z: 1,
                    shared_mem: 32768,
                    reason: "32KB shared mem, 128 threads for MMA",
                },
            ]
        }
        GpuLanguage::TileLang => {
            vec![BlockSizeRecommendation {
                ...
                block_z: 1,
                shared_mem: 0,
                reason: "Let TileLang JIT choose tile sizes",
            }]
        }
        GpuLanguage::Unknown => {
            vec![BlockSizeRecommendation {
                ...
                block_z: 1,
                shared_mem: 0,
                reason: "Safe default for unknown GPU kernel",
            }]
        }
    }
}
...
fn analyze_triton(source: &str) -> Vec<KernelIssue> {
    ...
    let block_re = regex::Regex::new(r#"BLOCK_(?:M|N|K|SIZE)\s*=\s*(\d+)"#).unwrap();
    for (i, line) in lines.iter().enumerate() {
        let trimmed = line.trim();
        if let Some(caps) = block_re.captures(trimmed) {
            if let Ok(size) = caps[1].parse::<u32>() {
                if !has_autotune && !(32..=512).contains(&size) {
                    issues.push(KernelIssue {
                        line: i + 1,
...
fn analyze_mojo(source: &str) -> Vec<KernelIssue> {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if trimmed.contains("@parameter")
            && (trimmed.contains("fn") || i + 1 < lines.len() && lines[i + 1].contains("fn"))
        {
            in_kernel = true;
        }
            ...
            && !source.contains("aligned")
        {
            issues.push(KernelIssue {
                ...
                suggestion:
                    "Use aligned variants (aligned=True or align_by) for coalesced memory access."
                        .into(),
            });
        }
...
fn analyze_numba(source: &str) -> Vec<KernelIssue> {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if in_kernel && trimmed.starts_with("def ") {
            ...
            let has_shared =
                source.contains("cuda.shared.array") || source.contains("shared.array");

            // Check for block/thread indexing
            let has_cuda_grid = source.contains("cuda.grid")
                || source.contains("cuda.block")
                || source.contains("cuda.thread");
            ...
            if !has_shared && source.contains("reduction")
                || source.contains("sum")
                || source.contains("histogram")
            {
                issues.push(KernelIssue {
                    line: i + 1,
                    severity: Severity::Warn,
            ...
            if i > 0 {
                ...
                if prev_line.contains("@cuda.jit") && !kernel_def.contains("fastmath") {
                    issues.push(KernelIssue {
                        ...
                        suggestion:
                            "Add fastmath=True to @cuda.jit for ~1.5x speedup on float operations."
                                .into(),
                    });
                }
            }
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if (trimmed.contains("cuda.to_device") || trimmed.contains("cuda.copy_host_to_device"))
            && (trimmed.starts_with("for")
                || source[..i].contains("for")
                || source[..i].contains("while"))
        {
            issues.push(KernelIssue {
                line: i + 1,
                ...
                suggestion:
                    "Move cuda.to_device outside the loop. Pre-allocate device arrays and reuse."
                        .into(),
            });
        }
    }
...
fn analyze_pytorch(source: &str) -> Vec<KernelIssue> {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if (trimmed.contains(".item()") || trimmed.contains(".cpu()"))
            && source.contains("for ")
            && (source.contains("range(") || source.contains("epoch"))
        {
            issues.push(KernelIssue {
                line: i + 1,
                severity: Severity::Warn,
        ...
        if source.contains("for") && source.contains("range") && source.contains("forward")
            || source.contains("__call__")
        {
        ...
        if trimmed.contains("backward()")
            && source.contains("optimizer.step()")
            && source.contains("for")
        {
            for (j, l) in lines.iter().enumerate() {
                if l.contains("for") && j < i && !source.contains("no_grad") {
                    issues.push(KernelIssue {
                                line: j + 1,
                                severity: Severity::Info,
...
fn analyze_cute(source: &str) -> Vec<KernelIssue> {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        let has_arch = source.contains("CUTE_ARCH_SM")
            || source.contains("CUTE_ARCH") && source.contains("SM");
        if !has_arch
            && (trimmed.contains("CUTE_DEVICE") || trimmed.contains("__global__"))
            && trimmed.contains("TiledMMA")
            || trimmed.contains("GMMA")
        {
            issues.push(KernelIssue {
                line: i + 1,
                severity: Severity::Error,
                ...
                suggestion:
                    "Define -DCUTE_ARCH_SM90 (H100) or -DCUTE_ARCH_SM89 (RTX 40xx) at compile time."
                        .into(),
            });
        }
        ...
        if (trimmed.contains("Copy_Atom") || trimmed.contains("copy_atom"))
            && source.contains("CUTE_ARCH_SM90")
        {
            issues.push(KernelIssue {
                    line: i + 1,
                    severity: Severity::Info,
...
fn analyze_cuda_tile(source: &str) -> Vec<KernelIssue> {
    ...
    let tile_re = regex::Regex::new(r"TILE_DIM\s+(\d+)").unwrap();
    for (i, line) in lines.iter().enumerate() {
        let trimmed = line.trim();

        // Detect TILE_DIM
        if trimmed.starts_with("#define") && trimmed.contains("TILE_DIM") {
            if let Some(caps) = tile_re.captures(trimmed) {
                if let Ok(dim) = caps[1].parse::<u32>() {
                    if !(16..=128).contains(&dim) {
                        issues.push(KernelIssue {
                            line: i + 1,
                            severity: Severity::Warn,
        ...
        if trimmed.contains("tile_load")
            || trimmed.contains("tile_store")
                && !trimmed.contains("vector")
                && !trimmed.contains("float4")
                && !trimmed.contains("int4")
        {
            issues.push(KernelIssue {
                line: i + 1,
                severity: Severity::Info,
                ...
                suggestion:
                    "Use float4/int4 vector types for tile loads to maximize memory bandwidth."
                        .into(),
            });
        }
        ...
        if trimmed.contains("__shared__")
            && (trimmed.contains("tile") || trimmed.contains("TILE"))
            && !source.contains("padded")
            && !source.contains("PADDED")
            && !source.contains("padding")
        {
            issues.push(KernelIssue {
                    line: i + 1,
                    severity: Severity::Info,
...
fn analyze_tilelang(source: &str) -> Vec<KernelIssue> {
    ...
    let tile_re = regex::Regex::new(r"(\w+_?TILE\w*)\s*[=:]\s*(\d+)").unwrap();
    for (i, line) in lines.iter().enumerate() {
        let trimmed = line.trim();
            ...
            && !source.contains("target=")
        {
            issues.push(KernelIssue {
                ...
                suggestion:
                    "Specify target='cuda -arch=sm_86' or target='rocm' for your GPU architecture."
                        .into(),
            });
        }

        // Detect tile sizes
        for cap in tile_re.captures_iter(trimmed) {
            let tile_name = &cap[1];
            ...
            if tile_val > 0 && tile_val < 16 {
                issues.push(KernelIssue {
                    ...
                    severity: Severity::Warn,
                    message: format!(
                        "Small tile: {} = {} (may underutilize GPU)",
                        tile_name, tile_val
                    ),
                    suggestion: "Increase tile size to at least 32 for better GPU utilization."
                        .into(),
                });
            }
        }
        ...
        if (trimmed.contains("pipeline") || trimmed.contains("stage"))
            && source.contains("num_stages")
            && !source.contains("num_stages=3")
            && !source.contains("num_stages=4")
            && !source.contains("num_stages=5")
        {
...
mod tests {
    ...
    fn test_detect_cuda() {
        assert_eq!(
            detect_language("kernel.cu", "__global__ void foo() {}"),
            GpuLanguage::Cuda
        );
    }
    ...
    fn test_analyze_triton_missing_autotune() {
        ...
        assert!(result
            .issues
            .iter()
            .any(|i| i.message.contains("Missing @triton.autotune")));
    }
    ...
    fn test_analyze_pytorch_compile() {
        ...
        assert!(result
            .issues
            .iter()
            .any(|i| i.message.contains("torch.compile")));
    }
    ...
    fn test_analyze_cute_arch() {
        ...
        assert!(result
            .issues
            .iter()
            .any(|i| i.message.contains("CUTE architecture macro")));
    }
}
