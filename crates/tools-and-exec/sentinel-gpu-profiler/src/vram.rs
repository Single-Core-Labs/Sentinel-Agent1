...
fn detect_vram_windows() -> Option<f64> {
    let output = std::process::Command::new("powershell")
        .args([
            "-Command",
            r#"(Get-CimInstance Win32_VideoController).AdapterRAM"#,
        ])
        .output()
        .ok()
        ...
        .or_else(|| {
            std::process::Command::new("powershell")
                .args([
                    "-Command",
                    r#"(Get-WmiObject Win32_VideoController).AdapterRAM"#,
                ])
                .output()
                .ok()
                .filter(|o| o.status.success())
...
pub fn query_extended_gpu_info() -> ExtendedGpuInfo {
    ...
    ExtendedGpuInfo {
        ...
        core_clock_mhz: extended.get("Max Clocks").and_then(|v| {
            v.lines().find(|l| l.contains("SM")).and_then(|l| {
                l.split(':')
                    .nth(1)
                    .and_then(|s| s.trim().trim_end_matches(" MHz").parse::<f64>().ok())
            })
        }),
        mem_clock_mhz: extended.get("Max Clocks").and_then(|v| {
            v.lines().find(|l| l.contains("Memory")).and_then(|l| {
                l.split(':')
                    .nth(1)
                    .and_then(|s| s.trim().trim_end_matches(" MHz").parse::<f64>().ok())
            })
        ...
        power_w: extended
            .get("Power Draw")
            .and_then(|v| v.trim_end_matches(" W").trim().parse::<f64>().ok()),
        pcie_gen: extended
            .get("Current PCIe")
            .and_then(|v| v.lines().next().map(|l| l.trim().to_string())),
        pcie_width: extended
            .get("Current PCIe")
            .and_then(|v| v.lines().nth(1).map(|l| l.trim().to_string())),
    }
}
...
pub fn compute_capability_from_name(name: &str) -> Option<String> {
    ...
    if n.contains("h100") || n.contains("h200") {
        Some("9.0".into())
    } else if n.contains("a100") || n.contains("a30") {
        Some("8.0".into())
    } else if n.contains("a10") || n.contains("a16") {
        Some("8.6".into())
    ...
    {
        Some("8.9".into())
    } else if n.contains("rtx 3090")
        || n.contains("rtx 3080")
        || n.contains("rtx 3070")
        || n.contains("rtx 3060")
    {
        Some("8.6".into())
    } else if n.contains("titan") && (n.contains("v") || n.contains("rtx")) {
        Some("7.5".into())
    } else if n.contains("v100") {
        Some("7.0".into())
    } else if n.contains("t4") {
        Some("7.5".into())
    } else if n.contains("p100") || n.contains("p40") {
        Some("6.0".into())
    } else if n.contains("k80") || n.contains("k40") {
        Some("3.7".into())
    } else {
        None
    }
...
pub fn architecture_from_name(name: &str) -> Option<&'static str> {
    ...
    if n.contains("h100") || n.contains("h200") {
        Some("Hopper")
    } else if n.contains("a100") || n.contains("a30") || n.contains("a10") || n.contains("a16") {
        Some("Ampere")
    } else if n.contains("l40") || n.contains("l4") || n.contains("rtx 40") {
        Some("Ada Lovelace")
    } else if n.contains("rtx 30") {
        Some("Ampere")
    } else if n.contains("rtx 20") {
        Some("Turing")
    } else if n.contains("titan") {
        Some("Turing/Volta")
    } else if n.contains("v100") {
        Some("Volta")
    } else if n.contains("t4") {
...
pub fn gpu_context_string(
    name: Option<&str>,
    vram_gb: Option<f64>,
    extended: &ExtendedGpuInfo,
) -> String {
...
fn detect_sm_count() -> Option<u32> {
    ...
    let sm = if name_lower.contains("a100") {
        108
        ...
        || name_lower.contains("rtx 40")
    {
        if name_lower.contains("4090") {
            ...
            36
        } else if name_lower.contains("3090") || name_lower.contains("3090 ti") {
            82
    ...
    } else if name_lower.contains("a10") || name_lower.contains("a16") {
        80
    } else if name_lower.contains("l4") {
        60
    } else if name_lower.contains("l40") || name_lower.contains("l40s") {
        108
    } else {
        48
    };
