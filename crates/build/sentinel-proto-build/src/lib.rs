...
impl AiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        for proto in protos {
            let dep_file = NamedTempFile::new()?;
            let desc_file = NamedTempFile::new()?;
    ...
    pub fn compile_protos(
        ...
        let file_descriptor_set_path: Option<PathBuf> =
            if let Some(file_descriptor_set_path) = file_descriptor_set_path {
                ...
                let file_descriptor_set_path = tempfile.path().join("sentinel-proto-build.pbbin");
...
pub fn configure() -> AiProtoBuilder {
