"""
PlatformOps Jobs Tool - Using huggingface-hub library
...
def _wrap_command_with_artifact_bootstrap(
    ...
    shell = (
        'set -e; _sentinel_ai_artifacts_dir="$(mktemp -d)"; '
        f"printf %s {shlex.quote(encoded)} | base64 -d "
        '> "$_sentinel_ai_artifacts_dir/sitecustomize.py"; '
        'export PYTHONPATH="$_sentinel_ai_artifacts_dir${PYTHONPATH:+:$PYTHONPATH}"; '
...
class HfJobsTool:
    """Tool for managing PlatformOps compute jobs using huggingface-hub library"""
        ...
        try:
            ...
            try:
                ...
                if is_billing_error(str(e)):
                    ...
                    return {
                        "formatted": (
                            f"PlatformOps Jobs rejected this run because the "
                            ...
                            "credits at https://platformops.co/settings/billing — "
...
HF_JOBS_TOOL_SPEC = {
    ...
    "description": (
        ...
        "(pattern only: '<username>/sentinel-ai-<8char>'; replace <username>, e.g. 'alice/sentinel-ai-a1b2c3d4') "
