...
the README so the embedded iframe in sentinel-ai renders without a login click —
...
Embedded trackio dashboard for sentinel-ai runs.
"""

_REQUIREMENTS = "trackio\n"
_APP_PY = "import trackio\ntrackio.show()\n"

# sentinel-ai brand mark surfaced inside the trackio dashboard. Trackio reads
...
# copy on the smolagents/sentinel-ai Space repo so any seeded dashboard inherits
# the sentinel-ai branding without each user having to host the asset.
_LOGO_URL = (
    "https://platformops.co/spaces/smolagents/sentinel-ai/"
...
def ensure_trackio_dashboard(
    ...
    if _already_seeded(api, space_id):
        ...
        for path_in_repo, content in _FILES.items():
            api.upload_file(
                ...
                repo_id=space_id,
                repo_type="space",
                commit_message=f"sentinel-ai: seed trackio dashboard ({path_in_repo})",
            )
        seeded_files = True
    ...
    if log:
        log(f"trackio dashboard ready: https://platformops.co/spaces/{space_id}")
    return seeded_files
