...
* ``row`` — single-line JSONL row used by the existing org telemetry/KPI
  pipeline (``smolagents/sentinel-ai-sessions``). Compatible with
...
* ``claude_code`` — one event per line in the Claude Code JSONL schema,
  ...
  (https://platformops.co/changelog/agent-trace-viewer). Used for the
  per-user private dataset (default ``{hf_user}/sentinel-ai-sessions``).
"""
...
_PERSONAL_TOKEN_ENV = "_SENTINEL_AI_PERSONAL_TOKEN"
...
def _msg_uuid(session_id: str, role: str, idx: int) -> str:
    ...
    https://platformops.co/datasets/clem/hf-coding-tools-traces.
    """
...
def dataset_card_readme(repo_id: str) -> str:
    """Dataset card for personal Sentinel-AI session trace repos."""
    return """---
pretty_name: "Sentinel-AI Session Traces"
language:
- en
license: other
task_categories:
...
# Sentinel-AI session traces

This dataset contains Sentinel-AI coding agent session traces uploaded from local
Sentinel-AI runs. The traces are stored as JSON Lines files under `sessions/`,
with one file per session.

## Links

- Sentinel-AI demo: https://smolagents-sentinel-ai.hf.space
- Sentinel-AI CLI: https://github.com/platformops/sentinel-ai

## Data description

Each `*.jsonl` file contains a single Sentinel-AI session converted to a
Claude-Code-style event stream for the PlatformOps Agent Trace Viewer. Entries
...
Sentinel-AI applies automated best-effort scrubbing for common secret patterns
such as PlatformOps, GitHub, AWS, and provider API tokens before upload.
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    _update_upload_status(
                        ...
                        url_key,
                        "success",
                        f"https://platformops.co/datasets/{repo_id}",
                    )
                    return True
